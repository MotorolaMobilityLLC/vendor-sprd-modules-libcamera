...
struct stateMachine aeStateMachine[] = {
    ...
    {ANDROID_CONTROL_AE_STATE_INACTIVE, AE_STABLE,
     ANDROID_CONTROL_AE_STATE_CONVERGED},
    {ANDROID_CONTROL_AE_STATE_INACTIVE, AE_STABLE_REQUIRE_FLASH,
...
void SprdCamera3OEMIf::PreviewFrameFaceBeauty(struct camera_frame_type *frame,
    ...
    for (int i = 0; i < faceInfo.face_num; i++) {
        ...
        beauty_face.idx = i;
        width = faceInfo.face[i].rect[2] - faceInfo.face[i].rect[0];
        height = faceInfo.face[i].rect[3] - faceInfo.face[i].rect[1];
        if (frame->face_info[i].flag_square == 1) {
            if (width > height) {
                faceInfo.face[i].rect[2] = faceInfo.face[i].rect[0] +height;
            } else if (width < height) {
                faceInfo.face[i].rect[3] = faceInfo.face[i].rect[1] + width;
            }
        }
...
bool SprdCamera3OEMIf::returnPreviewFrame(struct camera_frame_type *frame) {
    ...
    regular_channel->channelCbRoutine(frame_num, timestamp,
                                      CAMERA_STREAM_TYPE_PREVIEW);
    sprddefInfo = mSetting->getSPRDDEFTagPTR();
    if (sprddefInfo->return_previewframe_after_nozsl_cap)
                ...
                if (read_byte > 0) {
                    ...
                    if (read_byte > 0) {
                        ...
                        HAL_LOGD("dual_flag %d, superwide golden txt or bin "
                                 "not exist",
                                 dual_flag);
                        otp_info.dual_otp.data_3d.size = 0;
                        otpInfo.otp_type = 0;
                        otpInfo.dual_otp_flag = 3;
                    }
                }
    ...
    case ANDROID_SPRD_3DNR_ENABLED: {
        ...
        if (sprd_3dnr_enabled == 1) {
            if (sprddefInfo->sprd_auto_3dnr_enable == CAMERA_3DNR_AUTO) {
                ...
                mSprd3dnrType = CAMERA_3DNR_TYPE_PREV_SW_CAP_SW;//for le
#elif CONFIG_NIGHT_3DNR_PREV_HW_CAP_HW
                mSprd3dnrType = CAMERA_3DNR_TYPE_PREV_NULL_CAP_SW;//for l3
#elif CONFIG_NIGHT_3DNR_PREV_HW_CAP_SW
                mSprd3dnrType = CAMERA_3DNR_TYPE_PREV_NULL_CAP_SW;//for l5pro
#else
                mSprd3dnrType = CAMERA_3DNR_TYPE_PREV_NULL_CAP_HW;//l5/l6
#endif
            if(getMultiCameraMode() == MODE_BOKEH){
...
int SprdCamera3OEMIf::allocCameraMemForGpu(cmr_u32 size, cmr_u32 sum,
    ...
    size = (cmr_u32)(mCaptureWidth * mCaptureHeight * 3 / 2);
...
void SprdCamera3OEMIf::EisPreview_init() {
    ...
    property_get("persist.vendor.cam.eis.warp.preview", value, "true");
    if (!strcmp(value, "true")) {
        mSupportEisWarp = 1;
    }
#endif
    ...
    if (mMultiCameraMode == MODE_SINGLE_CAMERA && mSupportEisWarp) {
...
int SprdCamera3OEMIf::allocEisPreviewGpu(cmr_u32 sum, cmr_uint width, cmr_uint height) {
    ...
    if (mEisGraphicsHandle[0].graphicBuffer != NULL) {
        if (mEisGraphicsHandle[0].buf_size == size) {
            HAL_LOGD("use pre-alloc cap mem");
            return ret;
        } else {
            mEisGraphicsHandle[0].graphicBuffer.clear();
...
int SprdCamera3OEMIf::processEisWarpAlgo(struct camera_frame_type *frame,
    ...
    ret = camera_ioctrl(CAMERA_IOCTRL_EIS_WARP_YUV_PROC, &eis_warp_param, NULL);
    if (ret == NO_ERROR) {
