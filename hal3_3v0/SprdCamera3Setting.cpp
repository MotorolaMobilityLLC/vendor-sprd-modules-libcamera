...
const int32_t kavailable_fps_ranges_back[] = {
    5, 15, 5, 20, 5, 24, 24, 24,
    25, 25, 5, 30, 30, 30}; //{5, 20, 8, 20, 10, 20, 5, 25, 10, 25, 5, 30, 10,
// 30};
const int32_t kavailable_fps_ranges_front[] = {
    5, 15, 25, 25, 5,
    30, 15, 30, 30, 30}; //{5, 15, 5, 20, 10, 20, 5, 25, 10, 25, 5, 30, 10, 30};
...
const int32_t front_kavailable_request_keys[] = {
    ...
    ANDROID_CONTROL_AF_MODE, ANDROID_CONTROL_AF_TRIGGER,
    // ANDROID_CONTROL_AF_REGIONS,
    ANDROID_CONTROL_AWB_LOCK, ANDROID_CONTROL_AWB_MODE,
    // ANDROID_CONTROL_AWB_REGIONS,
    ANDROID_CONTROL_CAPTURE_INTENT, ANDROID_CONTROL_EFFECT_MODE,
...
int SprdCamera3Setting::initStaticParameters(int32_t cameraId) {
    ...
    if (cameraId == 0) {
        memcpy(s_setting[cameraId].controlInfo.max_regions, kmax_regions,
               sizeof(kmax_regions));
        if (mSensorFocusEnable[cameraId]) {
            s_setting[cameraId].controlInfo.max_regions[2] = 1;
        }
    } else {
    ...
    if (!cameraId) {
        ...
        if (mSensorFocusEnable[cameraId]) {
            int length = sizeof(kavailable_request_keys) / sizeof(int32_t);
...
int SprdCamera3Setting::constructDefaultMetadata(int type,
    ...
    uint8_t isTakePictureWithFlash = 1;
...
#define GET_VALUE_IF_DIF(x, y, tag)                                            \
    ...
    if (frame_settings.exists(ANDROID_SPRD_METERING_MODE)) {
        ...
        HAL_LOGD("AM region %d %d %d %d %d cnt %zu", area[0], area[1], area[2],
                 area[3], area[4],
    ...
    if (frame_settings.exists(ANDROID_SPRD_BURSTMODE_ENABLED)) {
        s_setting[mCameraId].sprddefInfo.sprd_burstmode_enable =
            frame_settings.find(ANDROID_SPRD_BURSTMODE_ENABLED).data.u8[0];
        pushAndroidParaTag(ANDROID_SPRD_BURSTMODE_ENABLED);
        HAL_LOGD("sprd_burstmode_enable is %d",
                 s_setting[mCameraId].sprddefInfo.sprd_burstmode_enable);
    }
    ...
    if (frame_settings.exists(ANDROID_CONTROL_AE_REGIONS) &&
        ...
        if (frame_settings.find(ANDROID_CONTROL_AE_REGIONS).count == 5) {
            ...
            HAL_LOGD("ANDROID_CONTROL_AE_REGIONS (%d %d %d %d %d cnt=%zu)",
    ...
    if (mSensorFocusEnable[mCameraId]) {
        if (frame_settings.exists(ANDROID_CONTROL_AF_REGIONS) &&
            ...
            if (frame_settings.find(ANDROID_CONTROL_AF_REGIONS).count == 5) {
                ...
                HAL_LOGD("ANDROID_CONTROL_AF_REGIONS AF region(%d %d %d %d %d "
                         "cnt=%zu)",
    ...
    if (frame_settings.exists(ANDROID_SPRD_HIGHISO_ENABLED)) {
        s_setting[mCameraId].sprddefInfo.sprd_highiso_enabled =
            frame_settings.find(ANDROID_SPRD_HIGHISO_ENABLED).data.u8[0];
