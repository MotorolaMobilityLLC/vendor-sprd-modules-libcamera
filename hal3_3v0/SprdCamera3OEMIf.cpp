...
int SprdCamera3OEMIf::startPreviewInternal()
{
	...
	mHalOem->ops->camera_pre_capture_get_buffer_id(mCameraId,mLargestSensorWidth, mLargestSensorHeight);
			...
			if(ret == NO_ERROR) {
				...
				if(frame->type == PREVIEW_FRAME && frame_num >= mPreviewFrameNum && (frame_num > mPictureFrameNum ||frame_num == 0)) {
					...
					if (mIsRecording && rec_stream) {
						if (frame_num > mRecordFrameNum)
							calculateTimestampForSlowmotion(buffer_timestamp);
							if ( sprddefInfo.perfect_skin_level >0 && MODE_SINGLE_CAMERA == mMultiCameraMode ) { //for 2D video recoding face_beauty
								ret=rec_stream->getQBufAddrForNum(frame_num, &videobuf_vir, &videobuf_phy,&fd0);
								if (ret == NO_ERROR && videobuf_vir != 0 ){
...
void * SprdCamera3OEMIf::pre_alloc_cap_mem_thread_proc(void *p_data)
{
	...
	buffer_id = obj->mHalOem->ops->camera_pre_capture_get_buffer_id(obj->mCameraId, obj->mLargestSensorWidth,obj->mLargestSensorHeight);
...
vsOutFrame SprdCamera3OEMIf::processPreviewEIS(vsInFrame frame_in)
{
	...
	if(mGyromaxtimestamp){
		...
		do{
			...
			if(ret_eis == 0){
				HAL_LOGI("frame_in %p, frame_out %p", frame_in.frame_data,frame_out_preview.frame_data);
...
vsOutFrame SprdCamera3OEMIf::processVideoEIS(vsInFrame frame_in)
{
	...
	if(mGyromaxtimestamp){
		...
		do{
			...
			if(ret_eis == 0){
				HAL_LOGI("frame_in %p, frame_out %p", frame_in.frame_data,frame_out_video.frame_data);
...
void *SprdCamera3OEMIf::gyro_monitor_thread_proc(void *p_data) {
    ...
    if (NULL != obj->mCameraHandle) {
        obj->mHalOem->ops->camera_get_sensor_max_fps(
            obj->mCameraHandle, obj->mCameraId, &max_fps);
        if (0 == max_fps){
    ...
    while (true == obj->mGyroInit) {
		...
		if (obj->mRecordingMode && sprddefInfo.sprd_eis_enabled) {
			...
			HAL_LOGV("gyro timestamp %" PRId64", x: %f, y: %f, z: %f",buffer[i].timestamp, buffer[i].data[0], buffer[i].data[1], buffer[i].data[2]);
		}
#endif
