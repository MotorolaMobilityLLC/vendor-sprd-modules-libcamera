                                           ...
                                           void* cb_of_free)
{
	...
	if (CMR_CAMERA_SUCCESS != camera_set_mem_func(camera_handle, cb_of_malloc, cb_of_free, client_data)) {
		ret = -CMR_CAMERA_FAIL;
		CMR_LOGE("fail to camera_set_mem_func() ret %ld",ret);
		return ret;
	}
...
cmr_int camera_set_zsl_buffer(cmr_handle camera_handle, cmr_uint src_phy_addr, cmr_uint src_vir_addr)
{
	cmr_int    ret = CMR_CAMERA_SUCCESS;
	if (!camera_handle || !src_phy_addr || !src_vir_addr) {
		CMR_LOGE("Invalid param error");
