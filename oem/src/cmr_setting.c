				                      ...
				                      struct setting_cmd_parameter *parm)
{
	...
	parm->cmd_type_value = flash_param->flash_status;
				                      ...
				                      struct setting_cmd_parameter *parm)
{
	...
	CMR_LOGI("flash_status %d",  parm->cmd_type_value);
	return ret;
}
				                     ...
				                     struct setting_cmd_parameter *parm)
{
	...
	if (FLASH_OPEN == flash_param->flash_status) {
                                    ...
                                    enum cmr_flash_status flash_status)
{
	...
	flash_param->flash_status = flash_status;
					              ...
					              struct setting_cmd_parameter *parm)
{
	...
	CMR_LOGI("is_active %ld, flash_mode %ld, ctrl_flash_status %d",
		is_active, flash_mode, (cmr_u32)ctrl_flash_status);
			                ...
			                struct setting_cmd_parameter * parm)
{
	...
	if (!cpt || !parm || cmd_type >= SETTING_TYPE_MAX
		...
		CMR_LOGE("param has error cpt 0x%p, camera_id %ld, array_size %d, cmd_type %d",
