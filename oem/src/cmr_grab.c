...
cmr_int cmr_grab_init(struct grab_init_param *init_param_ptr, cmr_handle *grab_handle)
{
	...
	*grab_handle = (cmr_handle)p_grab;
	...
	CMR_LOGI("Start to open GRAB device. %p", *grab_handle);
...
cmr_int cmr_grab_path_capability(cmr_handle grab_handle, struct cmr_path_capability *capability)
{
	...
	capability->capture_pause = 1;
#endif
	CMR_LOGI("video prev %d scale %d capture_no_trim %d capture_pause %d", capability->is_video_prev_diff,
		capability->hw_scale_available, capability->capture_no_trim, capability->capture_pause);
	return ret;
}
