...
#define CAMERA_3DNR_MSG_QUEUE_SIZE 8
...
static cmr_int threednr_open(cmr_handle ipm_handle, struct ipm_open_in *in,
    ...
    if (cam_cxt->nightscepro_flag) {
        ...
        if (ret) {
            CMR_LOGE("failed to get isp param mfnr %ld", ret);
        } else {
...
static cmr_int threednr_process_frame(cmr_handle class_handle,
    ...
    big_buf.gpu_buffer.handle = out->private_data;
    ...
    small_image.cpu_buffer.fd = in->src_frame.fd;
    CMR_LOGD("Call the threednr_function().big Y: %p, small Y: %p."
             " ,threednr_handle->is_stop %ld",
             big_buf.cpu_buffer.bufferY, small_image.cpu_buffer.bufferY,
...
static cmr_int threadnr_scaler_process(cmr_handle class_handle,
    ...
    dst.buf_size = threednr_handle->small_width *
                   threednr_handle->small_height * 3 / 2;
    dst.rect.start_x = 0;
    dst.rect.start_y = 0;
    ...
    dst.addr_phy.addr_y = in->src_frame.addr_phy.addr_y +
                                   threednr_handle->width *
                                       threednr_handle->height * 3 / 2;
    dst.addr_phy.addr_u = dst.addr_phy.addr_y +
        threednr_handle->small_width * threednr_handle->small_height;

    dst.addr_phy.addr_v = dst.addr_phy.addr_u;
    dst.addr_vir.addr_y = in->src_frame.addr_vir.addr_y +
        threednr_handle->width * threednr_handle->height * 3 / 2;
    dst.addr_vir.addr_u = dst.addr_vir.addr_y +
        threednr_handle->small_width * threednr_handle->small_height;
    ...
    ret = threednr_start_scale(
        threednr_handle->common.ipm_cxt->init_in.oem_handle, &src, &dst);
    if (ret) {
...

    {
        ...
        if (!strcmp(flag, "1")) { // save input image.
            ...
            sprintf(filename,"small_in_%ldx%ld_index_%d.yuv",
                      ...
                      threednr_handle->small_height,cur_frm);
            save_yuv(filename, (char *)in->src_frame.addr_vir.addr_y +
                            threednr_handle->width *
                            threednr_handle->height * 3 / 2,
                            threednr_handle->small_width,
