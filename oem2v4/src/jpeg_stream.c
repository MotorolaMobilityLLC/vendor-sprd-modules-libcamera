...
PUBLIC uint32 Jpeg_GetStreamPos(JPEG_READ_STREAM_CONTEXT_T *context_ptr) {
    ...
    if (NULL != context_ptr->read_file_func &&
        NULL != context_ptr->stream_buf) {
        // stream in file
        uint32 offset_in_buf = 0;
        uint32 bytes_in_buf = 0;
        uint32 consumed_file_size = 0;

        if ((unsigned long)context_ptr->stream_ptr >=
            (unsigned long)context_ptr->stream_buf) {
            offset_in_buf = (unsigned long)context_ptr->stream_ptr -
                            (unsigned long)context_ptr->stream_buf;
        ...
        if ((unsigned long)context_ptr->stream_ptr >=
                (unsigned long)context_ptr->stream_buf &&
            (unsigned long)context_ptr->stream_ptr <=
                (unsigned long)context_ptr->stream_buf +
                    context_ptr->stream_buf_size) {
            offset = (unsigned long)context_ptr->stream_ptr -
                     (unsigned long)context_ptr->stream_buf;
        } else {
            JPEG_PRINT(("[Jpeg_GetStreamPos] error of stream ptr"));
            context_ptr->stream_ptr =
                context_ptr->stream_buf + context_ptr->stream_buf_size;
            context_ptr->stream_left_size = 0;
...
**  Parameters:
**                  context_ptr:        context pointer
**                  offset:             position to set, relative to the
*beginning
**                                      of jpeg stream
**	Note:
*****************************************************************************/
PUBLIC void Jpeg_SetStreamPos(JPEG_READ_STREAM_CONTEXT_T *context_ptr,
                              uint32 offset) {
    if (NULL != context_ptr->read_file_func &&
        NULL != context_ptr->stream_buf) {
        // stream in file
        ...
        if (offset < consumed_file_size ||
            ...
            // the data of specified position is not in the stream buffer,
            // reload the data in buffer
            context_ptr->read_file_size = offset;
            context_ptr->stream_ptr = NULL;
            context_ptr->stream_left_size = 0;
        } else {
            // the data of specified position is in the stream buffer, only move
            // the pointer
            context_ptr->stream_ptr = (uint8 *)context_ptr->stream_buf +
                                      (offset - consumed_file_size);
            offset_in_buf = (unsigned long)context_ptr->stream_ptr -
                            (unsigned long)context_ptr->stream_buf;
...
LOCAL BOOLEAN Jpeg_WriteStreamToFile(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr) {
    JPEG_PRINT_LOW(
        ("JPEG Stream: Jpeg_WriteStreamToFile should not called \n"));

    if (PNULL != context_ptr->write_file_func &&
        PNULL != context_ptr->write_buf) {
        uint32 write_size = 0;
        uint32 actual_write_size = 0;
        uint8 *write_buf = context_ptr->write_buf;
        uint32 i = 0;

        if ((unsigned long)context_ptr->write_ptr >
            (unsigned long)context_ptr->write_buf) {
            write_size = (unsigned long)context_ptr->write_ptr -
                         (unsigned long)context_ptr->write_buf;
        }

        context_ptr->write_file_func(context_ptr->write_buf,
                                     context_ptr->write_file_size, write_size,
                                     &actual_write_size);
...
**	Note:           return TRUE if successful else return FALSE
*****************************************************************************/
PUBLIC BOOLEAN Jpeg_SetWritePos(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
                                uint32 offset) {
    uint8 *write_buf = context_ptr->write_buf;
    ...
    if (PNULL != context_ptr->write_file_func) {
        if (offset < write_file_size ||
            offset >= (write_file_size + write_buf_size)) {
            ...
            context_ptr->write_ptr =
                context_ptr->write_buf + (offset - write_file_size);
        }
    } else {
...
PUBLIC uint32 Jpeg_GetWritePos(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr) {
    ...
    if (((unsigned long)write_ptr >= (unsigned long)write_buf) &&
        ((unsigned long)write_ptr <=
         (unsigned long)write_buf + write_buf_size)) {
...
PUBLIC BOOLEAN Jpeg_WriteC(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr, uint8 c) {
    ...
    if (NULL == write_ptr || (((unsigned long)write_ptr + 1) >
                              ((unsigned long)write_buf + write_buf_size))) {
        return FALSE;
    } else {
...
PUBLIC BOOLEAN Jpeg_WriteL(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr, uint32 l) {
    ...
    if (NULL == write_ptr || (((unsigned long)write_ptr + 4) >
                              ((unsigned long)write_buf + write_size))) {
        return FALSE;
    } else {
