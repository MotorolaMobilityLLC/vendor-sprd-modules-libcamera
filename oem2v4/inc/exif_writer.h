...
typedef struct thumbnail_info_tag {
    ...
    uint32 size;
    uint8 *stream_buf_ptr; // Optional when read thumbnail. Set NULL to ignore
                           // it
...
JPEG_RET_E Jpeg_WriteAPP1(uint8 *target_buf, uint32 target_buf_size,
                          JINF_EXIF_INFO_T *exif_info_ptr,
                          uint8 *thumbnail_buf_ptr, uint32 thumbnail_size,
...
*@	Note:           return JINF_SUCCESS if successful
*/
/*PUBLIC JINF_RET_E IMGJPEG_FormatDateTime(JINF_DATE_T *date_ptr, JINF_TIME_T
   *time_ptr,
