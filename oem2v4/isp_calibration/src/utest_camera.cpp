...
static void utest_dcam_usage() {
    ...
    INFO("utest_camera -maincmd cali_main_cmd -subcmd cali_sub_cmd -id "
...
static int utest_callback_cap_mem_alloc(void *handle, unsigned int size,
    ...
    sp<MemoryHeapIon> pHeapIon = new MemoryHeapIon(
        "/dev/ion", size, MemoryHeapIon::NO_CACHING, ION_HEAP_ID_MASK_MM);
    if (pHeapIon == NULL) {
        return -1;
    }
...
static int utest_dcam_preview_mem_alloc(void) {
    ...
    for (i = 0; i < UTEST_PREVIEW_BUF_NUM; i++) {
        cmr_cxt_ptr->preview_pmem_hp[i] =
            new MemoryHeapIon("/dev/ion", buf_size, MemoryHeapIon::NO_CACHING,
                              ION_HEAP_ID_MASK_MM);
        ...
        cmr_cxt_ptr->preview_pmem_hp[i]->get_phy_addr_from_ion(
            (int *)(&cmr_cxt_ptr->preview_physical_addr[i]),
            (int *)(&cmr_cxt_ptr->preview_pmemory_size[i]));
        cmr_cxt_ptr->preview_virtual_addr[i] =
            (unsigned char *)cmr_cxt_ptr->preview_pmem_hp[i]->getBase();
        if (!cmr_cxt_ptr->preview_physical_addr[i]) {
    ...
    if (camera_set_preview_mem(cmr_cxt_ptr->preview_physical_addr,
                               cmr_cxt_ptr->preview_virtual_addr, buf_size,
...
static int utest_dcam_cap_memory_alloc(void) {
    ...
    cmr_cxt_ptr->cap_pmem_hp =
        new MemoryHeapIon("/dev/ion", buffer_size, MemoryHeapIon::NO_CACHING,
                          ION_HEAP_ID_MASK_MM);
    if (cmr_cxt_ptr->cap_pmem_hp->getHeapID() < 0) {
        ERR("failed to alloc capture pmem buffer.\n");
        return -1;
    }
    cmr_cxt_ptr->cap_pmem_hp->get_phy_addr_from_ion(
        (int *)(&cmr_cxt_ptr->cap_physical_addr),
        (int *)(&cmr_cxt_ptr->cap_pmemory_size));
    cmr_cxt_ptr->cap_virtual_addr =
        (unsigned char *)cmr_cxt_ptr->cap_pmem_hp->getBase();
    if (!cmr_cxt_ptr->cap_physical_addr) {
        ERR("failed to alloc capture pmem buffer:addr is null.\n");
        return -1;
    }

    if (camera_set_capture_mem(0, (uint32_t)cmr_cxt_ptr->cap_physical_addr,
                               (uint32_t)cmr_cxt_ptr->cap_virtual_addr,
                               (uint32_t)cmr_cxt_ptr->cap_pmemory_size,
                               (uint32_t)utest_callback_cap_mem_alloc,
                               (uint32_t)utest_callback_cap_mem_release, 0)) {
...
static int32_t
utest_dcam_find_param_index(struct utest_cmr_context *cmr_cxt_ptr) {
...
static int32_t utest_dcam_awb(uint8_t sub_cmd) {
    ...
    if (0 == sub_cmd) {
        ...
        if (ISP_Cali_GetLensTabs(tmp_addr, lsc_grid, img_size,
                                 (uint32_t *)len_tab_addr, index, 0, sub_cmd)) {
            rtn = -1;
    ...
    ISP_Cali_LensCorrection(&tmp_addr, &dst_addr, img_size, lsc_grid,
                            len_tab_addr);
...
static int32_t utest_dcam_flashlight(uint8_t sub_cmd) {
    ...
    default: { goto flashlight_exit; } break;
    }
...
static int32_t utest_dcam_lsc(uint8_t sub_cmd) {
    ...
    if (ISP_Cali_GetLensTabs(dst_addr, lsc_grid, img_size, len_tab_addr, index,
                             0, sub_cmd)) {
        rtn = -1;
        ERR("utest_dcam_lsc: fail to isp_cali_getlenstab.\n");
        goto lsc_exit;
...
static int32_t utest_dcam_preview_flash_eb(void) {
    ...
    if (UTEST_CALIBRATION_FLASHLIGHT == cmr_cxt_ptr->cmd) {
        ...
        flash_param.flash_eb = 0x01;
        flash_param.flash_ratio =
            flash_level.high_light * 256 / flash_level.low_light;
        if (isp_ioctl(NULL, ISP_CTRL_ALG, (void *)&flash_param))
            return -1;
...
static void utest_dcam_preview_cb(camera_cb_type cb, const void *client_data,
...
static void utest_dcam_cap_cb(camera_cb_type cb, const void *client_data,
                              camera_func_type func, int32_t parm4) {
    struct utest_cmr_context *cmr_cxt_ptr = g_utest_cmr_cxt_ptr;
...
static int32_t utest_dcam_capture(void) {
    ...
    if (UTEST_CALIBRATION_CAP_JPG == cmr_cxt_ptr->cmd) {
        if (CAMERA_SUCCESS !=
            camera_take_picture(utest_dcam_cap_cb, NULL, CAMERA_NORMAL_MODE)) {
            rtn = -1;
            goto cap_exit;
        }
    } else {
        if (CAMERA_SUCCESS != camera_take_picture_raw(utest_dcam_cap_cb, NULL,
