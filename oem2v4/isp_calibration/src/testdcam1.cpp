...
static struct _options opts[SENSOR_ID_MAX];
static int opt_sensors;
static int running_sensors;
...
static void process_protect(void) {
    ...
    sigset_t sigset,
        oset;                          /*sigset\u5b58\u653e\u5c4f\u853d\u4fe1\u53f7\uff0coset\u4fdd\u5b58\u5f53\u524d\u5c4f\u853d\u4fe1\u53f7*/
...
int _parse_options(int argc, char *argv[]) {
    ...
    while (argc > 0) {
        ...
        if (!strcmp(arg, "--sensor")) {
            ...
            if (sensors >= 0) {
                if (opts[sensors].param_cnt == 0)
                    return usage();
            }
            opts[++sensors].cameraId = val;
        } else if (!strcmp(arg, "--workmode")) {
            CMR_LOGI("workmode ->%s\n", val);
            opts[sensors].workmode = atoi(val);
        } else if (!strcmp(arg, "--args")) {
            if (opts[sensors].param_cnt >= MAX_ARGS)
                continue;
            CMR_LOGI("new str->%s\n", val);
            opts[sensors].param[opts[sensors].param_cnt++] = String8(val);

        } else if (!strcmp(arg, "--time")) {
            CMR_LOGI("time ->%s\n", val);
            opts[sensors].during = atoi(val);
        } else {
            CMR_LOGE("error %s %s\n", arg, argv);
            return usage();
        }
...
camera_memory_t *get_memory(int fd, size_t buf_size, unsigned int num_bufs,
    ...
    if (mHeap && p) {
        ...
        CMR_LOGI("maloc  0x%x,buf_size * num_bufs =%x * %x fail ", p,
                 buf_size * num_bufs);
        return NULL;
    }
}

void notify_cb(int32_t msg_type, int32_t ext1, int32_t ext2, void *user) {
...
static struct {
    sem_t capture_sem;
    int pic_cnt;
} takepic_action[SENSOR_ID_MAX] = {0};

void data_cb(int32_t msg_type, const camera_memory_t *data, unsigned int index,
    ...
    case CAMERA_MSG_COMPRESSED_IMAGE:
        ...
        if (data) {
            if (takepic_action[opr_index].pic_cnt == 0 ||
                takepic_action[opr_index].pic_cnt ==
                    opts[opr_index].pic_cnt - 1) {
                //					sleep(20);  //for trace
                //dump
            }

            if ((opts[opr_index].pic_cnt <= 1) ||
                (++takepic_action[opr_index].pic_cnt >=
                 opts[opr_index].pic_cnt)) {
                sem_post(&takepic_action[opr_index].capture_sem);
                CMR_LOGW("takepicture all OK !!\n");
                takepic_action[opr_index].pic_cnt = 0;
            } else {
...
void data_cb_timestamp(int64_t timestamp, int32_t msg_type,
...
int start_camera(camera_device_t **pdev, int *index) {
    int ops_status = NO_ERROR;
    camera_device_t *tmp = NULL;

    if (*pdev == NULL) {
        CMR_LOGI("pdev->common.methods->open %d\n", *index);
        ops_status = HAL_MODULE_INFO_SYM.common.methods->open(
            &HAL_MODULE_INFO_SYM.common, opts[*index].cameraId,
            (hw_device_t **)&tmp);
        if (ops_status != 0) {
            CMR_LOGE("\n open %d fail ", opts[*index].cameraId);
            if (tmp)
                tmp->common.close(
                    (hw_device_t *)tmp); // avoid next time retry fail
            mtrace_print_alllog(TRACE_MEMSTAT);
            *pdev = NULL;
            return -1;
        }
        tmp->ops->set_callbacks(tmp, notify_cb, data_cb, data_cb_timestamp,
                                get_memory, index);
        ...
        mergepara(tmp, opts[*index].param[0]);
        *pdev = tmp;
    }
    return ops_status;
}
...
int stop_camera(camera_device_t *pdev) {
    ...
    if (pdev) {
        CMR_LOGI("pdev->common.close %s %s \n", pdev->common.module->name,
                 pdev->common.module->id);
...
struct sensor_thread_tag {
    pthread_t thread_handle;
    int index; // specific data
};

static void *sensor_opr(void *client_data) {
    struct sensor_thread_tag *thread = (struct sensor_thread_tag *)client_data;
    int opr_index = thread->index;
    camera_device_t *pdev = NULL;
    int ops_status = NO_ERROR;
    SprdCameraHardware *pobj;

    CMR_LOGI("sprdcamtest enter:%p, %d", client_data, opr_index);

    ops_status = start_camera(&pdev, &thread->index);
    if (!pdev) {
        CMR_LOGE("\n open %d fail ", opts[opr_index].cameraId);
        goto exit;
    } else {
        sem_init(&takepic_action[opr_index].capture_sem, 0, 0);
        takepic_action[opr_index].pic_cnt = 0;
        //		stop_camera(pdev);
    ...
    switch (opts[opr_index].workmode) {
    case WK_PREVIEW:
        CMR_LOGI("%d\n WK_PREVIEW during %d s=>", opr_index,
                 opts[opr_index].during);
        ops_status = pdev->ops->start_preview(pdev);
        if (opts[opr_index].during != -1) {
            sleep(opts[opr_index].during);
        } else {
            CMR_LOGI("\n opts[opr_index].during = -1\n");
        }
        for (int i = 1; i < opts[opr_index].param_cnt; i++) {
            CMR_LOGI("\n WK_PREVIEW PRARM %d", i);
            mergepara(pdev, opts[opr_index].param[i]);
            sleep(opts[opr_index].during);
        }
        CMR_LOGI("%d\n WK_PREVIEW during %d s  stopping", opr_index,
                 opts[opr_index].during);
        pdev->ops->stop_preview(pdev);
        break;

    case WK_RECVIDEO:
        CMR_LOGI("\n WK_RECVIDEO during %d s=>", opts[opr_index].during);
        ops_status = pdev->ops->start_preview(pdev);
        sleep(1);
        CMR_LOGI("\n WK_RECVIDEO during %d s=>", opts[opr_index].during);
        ...
        CMR_LOGI("\n WK_RECVIDEO during %d s  stopping",
                 opts[opr_index].during);
        sleep(1);
        break;

    case WK_TAKEPIC:
        CMR_LOGI("\n WK_TAKEPIC during %d s=>", opts[opr_index].during);
        ops_status = pdev->ops->start_preview(pdev);
#if (SC_FPGA == 1)
        sleep(1);
#else
        sleep(5);
#endif
        pdev->ops->enable_msg_type(
            pdev, CAMERA_MSG_RAW_IMAGE | CAMERA_MSG_COMPRESSED_IMAGE |
                      CAMERA_MSG_RAW_IMAGE_NOTIFY |
                      CAMERA_MSG_COMPRESSED_IMAGE | CAMERA_MSG_ERROR);
        for (int j = 0; j < opts[opr_index].during; j++) {
            struct timespec ts;
            ...
            /*wait for 200ms*/
            ts.tv_sec += opts[opr_index].during;
            sem_timedwait(&takepic_action[opr_index].capture_sem, &ts);
            break;
        }
        break;

    case WK_TAKEPIC_RAW:
        CMR_LOGI("\n WK_TAKEPIC_RAW during %d s=>", opts[opr_index].during);
        ops_status = pdev->ops->start_preview(pdev);
        sleep(5);
        pobj = obj(pdev);
        if (!pobj) {
    ...
    case WK_AUTOFOCUS:
        sleep(1);
        CMR_LOGI("\n WK_AUTOFOCUS during %d s=>", opts[opr_index].during);
    ...
    CMR_LOGI("sprdcamtest:%d close", opr_index);
    pdev->common.close((hw_device_t *)pdev);

exit:
    free(thread);
    running_sensors--;
    CMR_LOGI("sprdcamtest:%d exit,running_sensors = %d", opr_index,
             running_sensors);
    return (void *)0;
}

cmr_int sensor_thread(cmr_handle *thread_handle, int index) {
    cmr_int rtn = CMR_MSG_SUCCESS;
    pthread_attr_t attr;
    struct sensor_thread_tag *thread = NULL;
    CMR_MSG_INIT(message);

    thread =
        (struct sensor_thread_tag *)malloc(sizeof(struct sensor_thread_tag));
    if (!thread) {
        CMR_LOGE("No mem!");
        return CMR_MSG_NO_MEM;
    }
    CMR_LOGI("thread %p, data %d", thread, index);
    thread->index = index;

    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    rtn = pthread_create(&thread->thread_handle, &attr, sensor_opr, thread);
    if (rtn) {
        CMR_LOGE("Fail to create thread");
        free((void *)thread);
        return rtn;
    }
    pthread_debug_setname(thread->thread_handle, "sensor_opr");
    CMR_LOGI("thread %p, data %d OK", thread, index);
...
int main(int argc, char **arg) {
    ...
    running_sensors = opt_sensors;
    for (i = 0; i < opt_sensors; i++) {
        sensor_thread(NULL, i);
    }

    CMR_LOGI("wait all finish\n");
    while (running_sensors > 0) {
        sleep(1);
    }
    mtrace_print_alllog(TRACE_MEMSTAT);
