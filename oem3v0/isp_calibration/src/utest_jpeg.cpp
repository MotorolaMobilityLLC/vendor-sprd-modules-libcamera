...
static char utest_jpeg_enc_src_y[128];
strcpy(utest_jpeg_enc_src_y, CAMERA_DUMP_PATH);
strcat(utest_jpeg_enc_src_y, "utest_src_y.raw");
static char utest_jpeg_enc_src_uv[128];
strcpy(utest_jpeg_enc_src_uv, CAMERA_DUMP_PATH);
strcat(utest_jpeg_enc_src_uv, "utest_src_uv.raw");
static char utest_jpg_enc_dst_file[128];
strcpy(utest_jpg_enc_dst_file, CAMERA_DUMP_PATH);
strcat(utest_jpg_enc_dst_file, "utest_jpg_file.jpg");
...
static int utest_jpeg_mem_alloc(void) {
    ...
    if (0 == s_mem_method) {
        jpeg_cxt_ptr->input_y_pmem_hp = new MemoryHeapIon(
            "/dev/ion", jpeg_cxt_ptr->width * jpeg_cxt_ptr->height,
    ...
    if (0 == s_mem_method) {
        jpeg_cxt_ptr->input_y_pmem_hp->get_phy_addr_from_ion(
            &jpeg_cxt_ptr->input_y_physical_addr,
            &jpeg_cxt_ptr->input_y_pmemory_size);
    } else {
        jpeg_cxt_ptr->input_y_pmem_hp->get_iova(
            ION_MM, &jpeg_cxt_ptr->input_y_physical_addr,
            &jpeg_cxt_ptr->input_y_pmemory_size);
    }
    jpeg_cxt_ptr->input_y_virtual_addr =
        (unsigned char *)jpeg_cxt_ptr->input_y_pmem_hp->base();
    if (!jpeg_cxt_ptr->input_y_physical_addr) {
        ERR("failed to alloc input_y pmem buffer:addr is null.\n");
        return -1;
    }
    memset(jpeg_cxt_ptr->input_y_virtual_addr, 0x80,
           jpeg_cxt_ptr->width * jpeg_cxt_ptr->height);

    /* alloc input uv buffer */
    if (0 == s_mem_method) {
        jpeg_cxt_ptr->input_uv_pmem_hp = new MemoryHeapIon(
            "/dev/ion", jpeg_cxt_ptr->width * jpeg_cxt_ptr->height / 2,
            MemoryHeapBase::NO_CACHING, ION_HEAP_ID_MASK_MM);
    } else {
        jpeg_cxt_ptr->input_uv_pmem_hp = new MemoryHeapIon(
            "/dev/ion", jpeg_cxt_ptr->width * jpeg_cxt_ptr->height / 2,
    ...
    if (0 == s_mem_method) {
        jpeg_cxt_ptr->input_uv_pmem_hp->get_phy_addr_from_ion(
            &jpeg_cxt_ptr->input_uv_physical_addr,
            &jpeg_cxt_ptr->input_uv_pmemory_size);
    } else {
        jpeg_cxt_ptr->input_uv_pmem_hp->get_iova(
            ION_MM, &jpeg_cxt_ptr->input_uv_physical_addr,
            &jpeg_cxt_ptr->input_uv_pmemory_size);
    }
    jpeg_cxt_ptr->input_uv_virtual_addr =
        (unsigned char *)jpeg_cxt_ptr->input_uv_pmem_hp->base();
    if (!jpeg_cxt_ptr->input_uv_physical_addr) {
        ERR("failed to alloc input_uv pmem buffer:addr is null.\n");
        return -1;
    }
    memset(jpeg_cxt_ptr->input_uv_virtual_addr, 0x80,
           jpeg_cxt_ptr->width * jpeg_cxt_ptr->height / 2);

    /* alloc output buffer */
    if (0 == s_mem_method) {
        jpeg_cxt_ptr->jpg_pmem_hp = new MemoryHeapIon(
            "/dev/ion", jpeg_cxt_ptr->width * jpeg_cxt_ptr->height,
            MemoryHeapBase::NO_CACHING, ION_HEAP_ID_MASK_MM);
    } else {
        jpeg_cxt_ptr->jpg_pmem_hp = new MemoryHeapIon(
    ...
    if (0 == s_mem_method) {
        jpeg_cxt_ptr->jpg_pmem_hp->get_phy_addr_from_ion(
            &jpeg_cxt_ptr->jpg_physical_addr, &jpeg_cxt_ptr->jpg_pmemory_size);
    } else {
        jpeg_cxt_ptr->jpg_pmem_hp->get_iova(ION_MM,
                                            &jpeg_cxt_ptr->jpg_physical_addr,
                                            &jpeg_cxt_ptr->jpg_pmemory_size);
    }
    jpeg_cxt_ptr->jpg_virtual_addr =
        (unsigned char *)jpeg_cxt_ptr->jpg_pmem_hp->base();
    ...
    INFO("utest physical addr 0x%x  0x%x  0x%x\n",
         jpeg_cxt_ptr->input_y_physical_addr,
         jpeg_cxt_ptr->input_uv_physical_addr, jpeg_cxt_ptr->jpg_physical_addr);
...
static int utest_jpg_mem_release(void) {
    ...
    if (jpeg_cxt_ptr->input_y_physical_addr) {
        if (0 == s_mem_method) {
            ...
            jpeg_cxt_ptr->input_y_pmem_hp->free_iova(
                ION_MM, jpeg_cxt_ptr->input_y_physical_addr,
                jpeg_cxt_ptr->input_y_pmemory_size);
        }
    }
    ...
    if (jpeg_cxt_ptr->input_uv_physical_addr) {
        if (0 == s_mem_method) {
            ...
            jpeg_cxt_ptr->input_uv_pmem_hp->free_iova(
                ION_MM, jpeg_cxt_ptr->input_uv_physical_addr,
                jpeg_cxt_ptr->input_uv_pmemory_size);
        }
    }
    ...
    if (jpeg_cxt_ptr->jpg_physical_addr) {
        if (0 == s_mem_method) {
            ...
            jpeg_cxt_ptr->jpg_pmem_hp->free_iova(
                ION_MM, jpeg_cxt_ptr->jpg_physical_addr,
                jpeg_cxt_ptr->jpg_pmemory_size);
        }
    }
...
static void utest_jpg_start(void) {
    ...
    jenc_parm_ptr.quality =
        utest_jpg_quality_covert(jpeg_cxt_ptr->quality_level);
