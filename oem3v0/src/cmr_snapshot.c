...
cmr_int snp_postproc_thread_proc(struct cmr_msg *message, void* p_data)
{
	...
	CMR_LOGI("message.msg_type 0x%x, data 0x%lx, fd 0x%x", message->msg_type, (cmr_uint)message->data, chn_data_ptr->fd);
	switch (message->msg_type) {
...
cmr_int snp_jpeg_enc_cb_handle(cmr_handle snp_handle, void *data)
{
	...
	if (atoi(value) == 5 || atoi(value) & (1<<5)) {
		...
		ret = camera_save_jpg_to_file(FORM_DUMPINDEX(SNP_ENCODE_STREAM, cam_ctx->dump_cnt, 0),
				IMG_DATA_TYPE_JPEG,
...
cmr_int snp_start_encode(cmr_handle snp_handle, void *data)
{
	...
	if (snp_cxt->ops.start_encode) {
		...
		if (atoi(value) == 4 || atoi(value) == 100 || (atoi(value) & (1<<4))) {
			...
			camera_save_yuv_to_file(FORM_DUMPINDEX(SNP_ENCODE_SRC_DATA, cam_ctx->dump_cnt, 0),
...
cmr_int snp_start_encode_thumb(cmr_handle snp_handle)
{
	...
	if (atoi(value) == 7 || atoi(value) & (1<<7)) {
		struct camera_context* cam_ctx = snp_cxt->oem_handle;
		camera_save_jpg_to_file(FORM_DUMPINDEX(SNP_THUMB_STREAM, cam_ctx->dump_cnt, 0),
					 IMG_DATA_TYPE_JPEG,
					 jpeg_in_ptr->src.size.width,
					 jpeg_in_ptr->src.size.height,
...
cmr_int snp_start_decode_sync(cmr_handle snp_handle, void *data)
{
	...
	if (atoi(value) == 1 || atoi(value) & (1<<1)) {
		struct camera_context* cam_ctx = snp_cxt->oem_handle;
		camera_save_jpg_to_file(FORM_DUMPINDEX(SNP_CHN_OUT_DATA, cam_ctx->dump_cnt, 0),
					IMG_DATA_TYPE_JPEG,
...
cmr_int snp_start_rot(cmr_handle snp_handle, void *data)
{
	...
	if (atoi(value) == 2 || (atoi(value) & (1<<2)) ) {
		struct camera_context* cam_ctx = snp_cxt->oem_handle;
		camera_save_yuv_to_file(FORM_DUMPINDEX(SNP_ROT_DATA, cam_ctx->dump_cnt, 0),
					IMG_DATA_TYPE_YUV420,
					dst.size.width,
					dst.size.height,
					&dst.addr_vir);
...
cmr_int snp_start_convet_thumb(cmr_handle snp_handle, void *data)
{
	...
	if (atoi(value) == 6 || (atoi(value) & (1<<6))) {
		struct camera_context* cam_ctx = snp_cxt->oem_handle;
		camera_save_yuv_to_file(FORM_DUMPINDEX(SNP_THUMB_DATA, cam_ctx->dump_cnt, 0),
					IMG_DATA_TYPE_YUV420,
					dst.size.width,
					dst.size.height,
					&dst.addr_vir);
	...
	//	snp_send_msg_notify_thr(snp_handle, SNAPSHOT_FUNC_STATE, SNAPSHOT_EVT_EXIF_JPEG_DONE, (void*)ret, sizeof(cmr_int));
		...
		}
		{
			...
			if (atoi(value) == 8 || atoi(value) == 100 || atoi(value) & (1<<8)) {
				...
				jpeg_addr.addr_y = enc_out_param.output_buf_virt_addr;
				camera_save_jpg_to_file(FORM_DUMPINDEX(SNP_JPEG_STREAM, cam_ctx->dump_cnt, 0),
							IMG_DATA_TYPE_JPEG,
							cxt->req_param.post_proc_setting.actual_snp_size.width,
							cxt->req_param.post_proc_setting.actual_snp_size.height,
...
cmr_int snp_set_channel_out_param(cmr_handle snp_handle)
{
	...
	for (i=0 ; i<CMR_CAPTURE_MEM_SUM ; i++) {
		...
		if ((0 == chn_out_frm_ptr->addr_vir.addr_y || 0 == chn_out_frm_ptr->addr_vir.addr_u) || (0 == chn_out_frm_ptr->fd)) {
			CMR_LOGE("err, frm fd 0x%x addr_vir 0x%lx 0x%lx", chn_out_frm_ptr->fd,chn_out_frm_ptr->addr_vir.addr_y,
	...
	if (!ret) {
		...
		for (i=0 ; i<1/*CMR_CAPTURE_MEM_SUM*/ ; i++) {
			CMR_LOGI("phy fd 0x%x vir addr 0x%lx size %d %d", chn_out_frm_ptr->fd,
...
cmr_int camera_set_frame_type(cmr_handle snp_handle, struct camera_frame_type *frame_type, struct frm_info* info)
{
	...
	default:
		...
		CMR_LOGI("camera id %d, refocus_mode %d",oem_cxt->camera_id, oem_cxt->is_refocus_mode);
	...
	if (atoi(value) == 3 || (atoi(value) & (1<<3))) {
		struct camera_context* cam_ctx = cxt->oem_handle;
		camera_save_yuv_to_file(FORM_DUMPINDEX(SNP_REDISPLAY_DATA, cam_ctx->dump_cnt, 0),
					IMG_DATA_TYPE_YUV420,
					frame_type->width,
					frame_type->height,
...
cmr_int snp_post_proc_for_yuv(cmr_handle snp_handle, void *data)
{
	...
	CMR_LOGI("w %d, h %d, yaddr 0x%lx, fd 0x%x",
	...
	if (atoi(value) == 1 || atoi(value) == 100 || (atoi(value) & (1<<1))) {
		struct camera_context* cam_ctx = cxt->oem_handle;
		camera_save_yuv_to_file(FORM_DUMPINDEX(SNP_CHN_OUT_DATA, cam_ctx->dump_cnt, 0),
			IMG_DATA_TYPE_YUV420,
			chn_param_ptr->chn_frm[chn_data_ptr->frame_id-chn_data_ptr->base].size.width,
			chn_param_ptr->chn_frm[chn_data_ptr->frame_id-chn_data_ptr->base].size.height,
...
cmr_int snp_get_buffer_id(cmr_handle snapshot_handle, void* data)
{
	...
	for (i = 0; i < CMR_CAPTURE_MEM_SUM; i++) {
		CMR_LOGI("chn fd 0x%x data fd 0x%x", cxt->req_param.post_proc_setting.chn_out_frm[i].fd, frame_info_ptr->fd);
		if (cxt->req_param.post_proc_setting.chn_out_frm[i].fd == (cmr_s32)frame_info_ptr->fd) {
			id = i;
			break;
		}
	}
