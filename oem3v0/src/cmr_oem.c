...
static cmr_int camera_preview_set_pd_raw_to_isp(cmr_handle oem_handle,
    ...
    if ((0 != cxt->snp_cxt.channel_bits) &&
        ...
        if (atoi(value) == 9 || (atoi(value) & (1 << 9))) {
            camera_save_yuv_to_file(
                FORM_DUMPINDEX(SNP_HDR_OUT_DATA, cxt->dump_cnt,
                               ipm_cxt->frm_num),
                IMG_DATA_TYPE_YUV420, cxt->snp_cxt.actual_capture_size.width,
                cxt->snp_cxt.actual_capture_size.height,
                (struct img_addr *)&frame->yaddr_vir);
        }
...
void camera_isp_dev_evt_cb(cmr_int evt, void *data, cmr_u32 data_len,
    ...
    struct frm_info *frame = (struct frm_info *)data;
    cmr_u32 channel_id;
    cmr_handle receiver_handle = NULL;
    cmr_int flash_status = FLASH_CLOSE;
    struct setting_cmd_parameter setting_param;

    if (!cxt) {
        CMR_LOGE("error param, handle 0x%lx data 0x%lx ", (cmr_uint)cxt,
                 (cmr_uint)data);
        return;
    }
    ...
    switch (evt) {
    case ISP_MW_CFG_BUF:
        // hal1.0 in high iso mode not call isp_stop_video in the process flow,
        // so call it here to stop isp.
        if (!cxt->isp_to_dram && !cxt->burst_mode) {
            cmr_camera_isp_stop_video(cxt->prev_cxt.preview_handle,
                                      cxt->camera_id);
        }
        cxt->isp_to_dram = 0;
        camera_send_channel_data((cmr_handle)cxt, receiver_handle, evt, data);
...
cmr_int camera_isp_deinit(cmr_handle oem_handle) {
    ...
    if (cxt->isp_malloc_flag == 1) {
        if (cxt->hal_free) {
            cxt->hal_free(CAMERA_ISP_LSC, &cxt->isp_lsc_phys_addr,
                          &cxt->isp_lsc_virt_addr, &fd, ISP_LSC_BUF_NUM,
                          cxt->client_data);
        }
        cxt->isp_malloc_flag = 0;
    }
    ...
    if (cxt->isp_anti_flicker_flag == 1) {
        if (cxt->hal_free) {
            cxt->hal_free(CAMERA_ISP_ANTI_FLICKER,
                          &cxt->isp_anti_flicker_phys_addr,
                          &cxt->isp_anti_flicker_virt_addr, &fd,
...
cmr_int camera_start_encode(cmr_handle oem_handle, cmr_handle caller_handle,
    ...
    // altek high iso mode need set uv_endian as IMG_DATA_ENDIAN_2PLANE_UVUV for
    // jpeg enc
...
void camera_face_makeup(cmr_handle oem_handle, struct img_frm *src) {
    ...
    if (ret) {
        CMR_LOGE("failed to get perfect skinlevel %ld, "
                 "setting_cxt->setting_handle is %p",
    ...
    CMR_LOGD("perfect skinWhitenLevel is %d, skinCleanLevel is %d",
    ...
    if (PerfectSkinLevel != 0 && pic_width > 0 && pic_height > 0) {
        ...
        if (cxt->fd_face_area.face_num > 0) {
            ...
            CMR_LOGI(
                "perfect frameWidth is %d, frameHeight is %d, format is %d",
                pic_width, pic_height, yuvFormat);

            CMR_LOGV("perfect ts_face_beautify will be call");
            int mu_retVal = ts_face_beautify(&inMakeupData, &outMakeupData,
                                             skinCleanLevel, skinWhitenLevel,
                                             &SkinWhitenTsface, 0, yuvFormat);
            if (mu_retVal != TS_OK) {
                ...
                memcpy(uvBuf, (tmpBuf + pic_width * pic_height),
...
cmr_int camera_start_exif_encode(cmr_handle oem_handle,
    ...
    if (strcmp(value, "1") == 0) {
        if (NULL != cxt->isp_exif_info.addr && cxt->isp_exif_info.size != 0) {
            enc_exif_param.exif_isp_debug_info.addr =
                cxt->isp_exif_info.addr; // isp_param.isp_dbg_info.addr;
            enc_exif_param.exif_isp_debug_info.size =
                cxt->isp_exif_info.size; // isp_param.isp_dbg_info.size
        } else {
...
cmr_int camera_capture_zsl_highflash(cmr_handle oem_handle, cmr_u32 camera_id) {
    ...
    if ((1 != camera_get_hdr_flag(cxt))) {
        ...
        CMR_LOGI("setting_param.cmd_type_value = %ld",
...
cmr_int camera_raw_proc(cmr_handle oem_handle, cmr_handle caller_handle,
    ...
    if (1 == param_ptr->slice_num) {
        ...
        cmr_u32 i;
        cmr_u32 raw_buf_size;
        cmr_u32 highiso_buf_size;
        cmr_int tmp_buf_size;
        cmr_int num = 0;
        cmr_u32 highiso_buf_num = 1;
        cmr_u32 isp_raw_buf_num = 1;

        raw_buf_size = ((param_ptr->src_frame.size.width * 4 / 3 + 7) & (~7)) *
                       param_ptr->src_frame.size.height;
        tmp_buf_size = param_ptr->src_frame.size.width *
                       param_ptr->src_frame.size.height * 27 / 10;
        num = tmp_buf_size / BUF_BLOCK_SIZE;
        if (tmp_buf_size > num * BUF_BLOCK_SIZE)
            num++;
        highiso_buf_size = num * BUF_BLOCK_SIZE;
        if (cxt->hal_malloc) {
            cxt->hal_malloc(CAMERA_ISP_RAW_DATA, &raw_buf_size,
                            &isp_raw_buf_num, &cxt->raw_buf_phys_addr[0],
                            &cxt->raw_buf_virt_addr[0], &cxt->raw_buf_fd[0],
                            cxt->client_data);

            cxt->hal_malloc(CAMERA_SNAPSHOT_HIGHISO, &highiso_buf_size,
                            &highiso_buf_num, &cxt->highiso_buf_phys_addr,
                            &cxt->highiso_buf_virt_addr, &cxt->highiso_buf_fd,
                            cxt->client_data);
        }
        memcpy(in_param.raw_buf_fd, cxt->raw_buf_fd,
               sizeof(cmr_s32) * isp_raw_buf_num);
        memcpy(in_param.raw_buf_phys_addr, cxt->raw_buf_phys_addr,
               sizeof(cmr_u32) * isp_raw_buf_num);
        memcpy(in_param.raw_buf_virt_addr, cxt->raw_buf_virt_addr,
               sizeof(cmr_u64) * isp_raw_buf_num);
        ...
        for (i = 0; i < isp_raw_buf_num; i++) {
            CMR_LOGI("raw: fd=0x%x, phys_addr_offset=0x%lx, virt_addr = 0x%lx, "
                     "buf_size=0x%lx",
                     in_param.raw_buf_fd[i], in_param.raw_buf_phys_addr[i],
                     in_param.raw_buf_virt_addr[i], in_param.raw_buf_size);
        }
        CMR_LOGI("cnt=%d, width=%ld, height=%ld", in_param.raw_buf_cnt,
                 in_param.raw_buf_width, in_param.raw_buf_height);
        CMR_LOGI("highiso: fd=0x%x, phys_addr_offset=0x%lx, virt_addr = 0x%lx, "
                 "buf_size=0x%lx",
...

        {
            ...
            if (atoi(value) == 1 || atoi(value) == 100 ||
                ...
                camera_save_yuv_to_file(
                    ...
                    param_ptr->src_frame.size.height,
                    (struct img_addr *)&param_ptr->src_frame.addr_vir);
            }
        }
...

        {
            ...
            if (atoi(value) == 1 || atoi(value) == 100 ||
                (atoi(value) & (1 << 1))) {
                camera_save_yuv_to_file(
                    FORM_DUMPINDEX(0x5000, cxt->dump_cnt, 0),
                    IMG_DATA_TYPE_RAW2, in_param.raw_buf_width,
                    in_param.raw_buf_height,
                    (struct img_addr *)&in_param.raw_buf_phys_addr[0]);
                camera_save_yuv_to_file(
                    FORM_DUMPINDEX(0x6000, cxt->dump_cnt, 0),
                    IMG_DATA_TYPE_YUV420, param_ptr->dst_frame.size.width,
                    param_ptr->dst_frame.size.height,
                    (struct img_addr *)&param_ptr->dst_frame.addr_vir);
            }
        }

        if (cxt->hal_free) {
            cxt->hal_free(CAMERA_ISP_RAW_DATA, &cxt->raw_buf_phys_addr[0],
                          &cxt->raw_buf_virt_addr[0], &cxt->raw_buf_fd[0],
                          isp_raw_buf_num, cxt->client_data);

            cxt->hal_free(CAMERA_SNAPSHOT_HIGHISO, &cxt->highiso_buf_phys_addr,
                          &cxt->highiso_buf_virt_addr, &cxt->highiso_buf_fd,
                          highiso_buf_num, cxt->client_data);
        }
    } else {
...
cmr_int camera_isp_start_video(cmr_handle oem_handle,
    ...
    cmr_int tmp_buf_size;
    cmr_int num = 0;
    cmr_u32 highiso_buf_num = 0;
    cmr_u32 isp_raw_buf_num = 0;
...
//					&cxt->isp_anti_flicker_phys_addr, &cxt->isp_anti_flicker_virt_addr, cxt->client_data);
    ...
    if (setting_param.cmd_type_value == CAMERA_CAP_MODE_HDR) {
        isp_param.capture_mode = ISP_CAP_MODE_HDR;
    } else {
...

    {
        ...
        CMR_LOGI("isp sensor max w h, %d %d",
                 isp_param.resolution_info.sensor_max_size.w,
                 isp_param.resolution_info.sensor_max_size.h);
        CMR_LOGI("isp sensor output w h, %d %d",
                 isp_param.resolution_info.sensor_output_size.w,
                 isp_param.resolution_info.sensor_output_size.h);
        CMR_LOGI("isp sensor crop startx start w h, %d %d %d %d",
        ...
        CMR_LOGI("isp_param:sensor fps info:is_high_fps %d, high_fps_skip_num "
                 "is  %d",
                 isp_param.sensor_fps.is_high_fps,
                 isp_param.sensor_fps.high_fps_skip_num);
        CMR_LOGI("isp_param:sensor fps info:max_fps is %d, min_fps is %d",
    ...
    if (HIGHISO_CAP_MODE == cxt->highiso_mode || cxt->burst_mode) {
        ...
        raw_buf_size =
            (((isp_param.resolution_info.sensor_size.w * 4 / 3 + 7) >> 3)
             << 3) *
            isp_param.resolution_info.sensor_size.h;
        tmp_buf_size = isp_param.resolution_info.sensor_size.w *
                       isp_param.resolution_info.sensor_size.h * 27 / 10;
        num = tmp_buf_size / BUF_BLOCK_SIZE;
        if (tmp_buf_size > num * BUF_BLOCK_SIZE)
            num++;
        highiso_buf_size = num * BUF_BLOCK_SIZE;
        ...
        memcpy(isp_param.raw_buf_fd, cxt->raw_buf_fd,
               sizeof(cmr_s32) * isp_raw_buf_num);
        memcpy(isp_param.raw_buf_phys_addr, cxt->raw_buf_phys_addr,
               sizeof(cmr_u32) * isp_raw_buf_num);
        memcpy(isp_param.raw_buf_virt_addr, cxt->raw_buf_virt_addr,
               sizeof(cmr_u64) * isp_raw_buf_num);
        ...
        for (i = 0; i < (cmr_u32)isp_raw_buf_num; i++) {
            CMR_LOGI("raw: fd=0x%x, phys_addr_offset=0x%lx, virt_addr = 0x%lx, "
                     "buf_size=0x%lx",
                     isp_param.raw_buf_fd[i], isp_param.raw_buf_phys_addr[i],
                     isp_param.raw_buf_virt_addr[i], isp_param.raw_buf_size);
        }
        CMR_LOGI("cnt=%d, width=%ld, height=%ld", isp_param.raw_buf_cnt,
                 isp_param.raw_buf_width, isp_param.raw_buf_height);
        CMR_LOGI("highiso: fd=0x%x, phys_addr_offset=0x%lx, virt_addr = 0x%lx, "
                 "buf_size=0x%lx",
    ...
    isp_param.tuning_mode = cmr_camera_get_isp_tuning_mode(
        cxt->prev_cxt.preview_handle, cxt->camera_id, isp_param.capture_mode);
    CMR_LOGI(
        "work_mode %ld, dv_mode %ld, capture_mode %u, isp_param.tuning_mode %d",
        work_mode, dv_mode, isp_param.capture_mode, isp_param.tuning_mode);
...
cmr_int camera_channel_cfg(cmr_handle oem_handle, cmr_handle caller_handle,
    ...
    CMR_LOGV("param_ptr->cap_inf_cfg.buffer_cfg_isp %u",
    ...
    if (param_ptr->is_lightly) {
        ...
        if (!isp_only) {
            ret =
                cmr_grab_cap_cfg_lightly(cxt->grab_cxt.grab_handle,
...
cmr_int camera_channel_buff_cfg(cmr_handle oem_handle,
    ...
    if ((HIGHISO_CAP_MODE == cxt->highiso_mode || cxt->isp_to_dram ||
         (buf_cfg->base_id == CMR_CAP1_ID_BASE && cxt->burst_mode)) &&
...
cmr_int camera_channel_cap_cfg(cmr_handle oem_handle, cmr_handle caller_handle,
    ...
    if ((HIGHISO_CAP_MODE == cxt->highiso_mode || cxt->isp_to_dram) &&
        !cap_cfg->cfg.pdaf_type3)
        cap_cfg->buffer_cfg_isp = 1;
    else
        cap_cfg->buffer_cfg_isp = 0;
    CMR_LOGE("cap_cfg->buffer_cfg_isp %u", cap_cfg->buffer_cfg_isp);
...
camera_get_tuning_info(cmr_handle oem_handle,
    ...
    CMR_LOGV("adgain=%d, exp_time=%d", adgain_exp_info_ptr->adgain,
             adgain_exp_info_ptr->exp_time);
    return ret;
}
...
cmr_int camera_get_preview_param(cmr_handle oem_handle,
    ...
    ret = cmr_setting_ioctl(setting_cxt->setting_handle,
                            SETTING_GET_SPRD_BURSTMODE_ENABLED, &setting_param);
    out_param_ptr->sprd_burstmode_enabled = setting_param.cmd_type_value;
    CMR_LOGI("sprd_burstmode_enabled=%d",
             out_param_ptr->sprd_burstmode_enabled);
    ...
    ret = cmr_setting_ioctl(setting_cxt->setting_handle,
                            SETTING_GET_SPRD_HIGHISO_ENABLED, &setting_param);
    if (ret) {
        CMR_LOGE("failed to get preview sprd high iso enabled flag %ld", ret);
        goto exit;
    }
    // camera apk open high iso feature, cmd_type_value value is 1,the high iso
    // mode set HIGHISO_CAP_MODE
    if (setting_param.cmd_type_value) {
        // on high iso mode,take  raw data picture,the high iso mode set
    ...
    ret = cmr_setting_ioctl(setting_cxt->setting_handle,
                            SETTING_GET_UHD_RECORDING_ENABLED, &setting_param);
    if (ret) {
        CMR_LOGE("failed to get uhd  enabled flag %ld", ret);
        goto exit;
    }
    out_param_ptr->is_uhd_recording_mode = setting_param.cmd_type_value;
    CMR_LOGD("is_uhd_recording_mode flag %d",
             out_param_ptr->is_uhd_recording_mode);
...
cmr_int camera_set_preview_param(cmr_handle oem_handle,
    ...
    prev_cxt->sensor_datatype_channel_bits =
        preview_out.sensor_datatype_chn_bits;
...
cmr_int camera_local_start_snapshot(cmr_handle oem_handle,
    ...
    cmr_int flash_status = FLASH_CLOSE;
    cmr_s32 sm_val = 0;
    struct common_isp_cmd_param snapshot_isp_param;
    ...
    if (CAMERA_ZSL_MODE == mode) {
        ...
        cmr_sensor_ioctl(cxt->sn_cxt.sensor_handle, cxt->camera_id,
                         SENSOR_BEFORE_SNAPSHOT,
                         (SENSOR_APERT_CAPTURE_MODE |
                          (SENSOR_APERT_PREVIEW_MODE << CAP_MODE_BITS)));
    }
    ...
    ret = camera_isp_ioctl(oem_handle, COM_ISP_GET_EXIF_DEBUG_INFO,
                           &snapshot_isp_param);
    cxt->isp_exif_info = snapshot_isp_param.isp_dbg_info;
    if (ret) {
        CMR_LOGI("get exif info error");
        memset(&cxt->isp_exif_info, 0, sizeof(struct isp_info));
    }
exit:
    CMR_LOGV("done %ld", ret);
    ATRACE_END();
...
cmr_int camera_local_stop_snapshot(cmr_handle oem_handle) {
    ...
    // take picture end on highiso mode,set the highiso_mode as
    // HIGHISO_MODE_NONE
...
cmr_int camera_preview_pd_raw_open_to_isp(cmr_handle oem_handle,
                                          struct pd_raw_open *pd_open) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    struct camera_context *cxt = (struct camera_context *)oem_handle;
    struct common_isp_cmd_param isp_param;
    ...
    ret =
        camera_isp_ioctl(oem_handle, COM_ISP_SET_PREVIEW_PDAF_OPEN, &isp_param);

exit:
    return ret;
}

cmr_int camera_preview_set_pd_raw_to_isp(cmr_handle oem_handle,
                                         struct pd_raw_info *pd_raw) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    struct camera_context *cxt = (struct camera_context *)oem_handle;
    struct common_isp_cmd_param isp_param;
    ...
    ret =
        camera_isp_ioctl(oem_handle, COM_ISP_SET_PREVIEW_PDAF_RAW, &isp_param);
