...
typedef int (*altek_refocus_int)(void *const a_pInData, int a_dInSize,
		int a_dInImgW, int a_dInImgH, void *a_pInOTPBuf, int a_dInOTPSize);
typedef int (*altek_refocus_function)(unsigned char *a_pucOutYCC444, unsigned char *a_pucInYCCNV21Buf, unsigned char *a_puwInDisparityBuf,
		unsigned short a_uwInDisparityW, unsigned short a_uwInDisparityH,
		unsigned short a_uwInX, unsigned short a_uwInY);
...
static cmr_int refocus_open(cmr_handle ipm_handle, struct ipm_open_in *in, struct ipm_open_out *out,
		cmr_handle *out_class_handle);
static cmr_int refocus_close(cmr_handle class_handle);
static cmr_int refocus_transfer_frame(cmr_handle class_handle,struct ipm_frame_in *in, struct ipm_frame_out *out);
static cmr_int refocus_pre_proc(cmr_handle class_handle);
static cmr_int refocus_post_proc(cmr_handle class_handle);
static cmr_int refocus_start(cmr_handle class_handle, struct refocus_start_parameter *param);
static cmr_uint check_size_data_invalid(struct img_size *refocus_size);
static cmr_int refocus_call_init(struct class_refocus *class_handle, const struct refocus_init_parameter *parm);
static cmr_uint refocus_is_busy(struct class_refocus *class_handle);
static void refocus_set_busy(struct class_refocus *class_handle, cmr_uint is_busy);
...
int32_t refocus_save_to_file (cmr_u32 index, cmr_u32 img_fmt, cmr_u32 width, cmr_u32 height,cmr_u32 touchX, cmr_u32 touchY, void *addr);
int32_t refocus_output_save_to_file (cmr_u32 index, cmr_u32 img_fmt, cmr_u32 width, cmr_u32 height,cmr_u32 touchX, cmr_u32 touchY, void *addr);
...
#define CMR__EVT_REFOCUS_MASK_BITS     (cmr_u32)(CMR_EVT_REFOCUS_START | \
		CMR_EVT_REFOCUS_EXIT | \
		...
		cmr_handle *out_class_handle)
{
	...
	refocus_handle = (struct class_refocus *)malloc(sizeof(struct class_refocus));
	if (!refocus_handle) {
		CMR_LOGE("No mem!");
	...
	refocus_handle->mem_size           = in->frame_size.height * in->frame_size.width * 3 / 2;
	refocus_handle->frame_total_num    = in->frame_cnt;
	refocus_handle->frame_cnt          = 0;

	CMR_LOGD("mem_size = 0x%ld ,in->otp_data->otp_size 0x%x,in->otp_data->otp_ptr %p", refocus_handle->mem_size,in->otp_data.otp_size,in->otp_data.otp_ptr);
...
static cmr_int refocus_transfer_frame(cmr_handle class_handle,struct ipm_frame_in *in, struct ipm_frame_out *out)
{
	...
	if (frame_cnt < refocus_handle->frame_total_num) {
		CMR_LOGD("This is fd 0x%ld frame. need the 0x%ld frame,",frame_cnt, refocus_handle->frame_total_num);
		return ret;
	}
	...
	if (!is_busy) {
		...
		CMR_LOGV("in %param.depth_map_data %p. touch %d %d",in,param.depth_map_data, in->touch_x,in->touch_y);

		//memcpy(refocus_handle->alloc_addr, (void *)in->src_frame.addr_vir.addr_y, refocus_handle->mem_size);
...
static cmr_int refocus_thread_proc(struct cmr_msg *message, void *private_data)
{
	...
	switch (evt) {
		case CMR_EVT_REFOCUS_INIT: {
						   struct refocus_init_parameter *init_param = (struct refocus_init_parameter *)message->data;
						   if (!check_size_data_invalid(&init_param->refocus_size)) {
							   ...
							   if (altek_refoucs_ops.init && (init_param->refocus_size.width * init_param->refocus_size.height >= CMR_REFOCUS_LIMIT_SIZE)) {

								   if(init_param->otp_data.otp_ptr!= NULL ){
									   save_file(psPath_otpData, init_param->otp_data.otp_ptr, init_param->otp_data.otp_size);
									   CMR_LOGI("[REFOCUS] init_param.otp_ptr %p, init_param.otp_size %d",init_param->otp_data.otp_ptr,init_param->otp_data.otp_size);
									   ...
									   ret = altek_refoucs_ops.init(NULL,0,init_param->refocus_size.width, init_param->refocus_size.height, init_param->otp_data.otp_ptr, init_param->otp_data.otp_size);
		...
		case CMR_EVT_REFOCUS_START:
					   ...
					   if (altek_refoucs_ops.function && start_param->depth_map_data != NULL) {
						   ...
						   unsigned char *pucDepth = (unsigned char*)malloc(start_param->depth_map_with*start_param->depth_map_height);
						   ...
						   if(start_param->depth_map_data == NULL)
						   {
							   ...
							   memcpy(pucDepth, start_param->depth_map_data,start_param->depth_map_with*start_param->depth_map_height);
							   no_callback = 0;
						   }

						   unsigned char *pucInputBufferNV21 = (unsigned char*)malloc(dBokehImgW*dBokehImgH*3/2);
						   memcpy(pucInputBufferNV21,  start_param->frame_data,dBokehImgW*dBokehImgH*3/2);

						   unsigned char *pucOutYCCNV21= (unsigned char*)malloc(dBokehImgW*dBokehImgH*3/2);
						   ...
						   CMR_LOGD("pucInputBufferNV21 %p,pucDepth %p start_param->depth_map_data %p,%d, %d, %d, %d,dBokehImgW %d dBokehImgH %d",pucInputBufferNV21,pucDepth,start_param->depth_map_data,start_param->depth_map_with,start_param->depth_map_height,start_param->touch_x,start_param->touch_y,dBokehImgW,dBokehImgH);
						   altek_refocus_ret = altek_refoucs_ops.function(pucOutYCCNV21,
								   pucInputBufferNV21,
								   pucDepth,
						   ...
						   if(altek_refocus_ret == 0){
							   memcpy(start_param->frame_data, pucOutYCCNV21,dBokehImgW*dBokehImgH*3/2);
							   CMR_LOGV("&(class_handle->frame_out.dst_frame.addr_vir) %p class_handle->frame_out.dst_frame.addr_vir 0x%x, start_param->frame_data %p", &(class_handle->frame_out.dst_frame.addr_vir),class_handle->frame_out.dst_frame.addr_vir.addr_y,start_param->frame_data);
						   }
						   ...
						   if (atoi(value) == 1 && altek_refocus_ret == 0) {
							   ...
							   {
								   refocus_save_to_file(refocus_frame_num,
										   IMG_DATA_TYPE_RAW,
										   start_param->depth_map_with,
										   start_param->depth_map_height,
										   start_param->touch_x,
								   ...
								   refocus_save_to_file(refocus_frame_num,
										   ...
										   start_param->touch_y,
										   pucInputBufferNV21);
								   refocus_output_save_to_file(refocus_frame_num,
										   IMG_DATA_TYPE_YUV420,
										   dBokehImgW,
										   dBokehImgH,
										   start_param->touch_x,
							   ...
							   memcpy(start_param->frame_data,start_param->depth_map_data,start_param->depth_map_with*start_param->depth_map_height);
							   memset(start_param->frame_data+start_param->depth_map_with*start_param->depth_map_height,128,start_param->depth_map_with*start_param->depth_map_height*1/2);
						   }
						   ...
						   if (class_handle->frame_cb&&no_callback == 0) {
							   ...
							   class_handle->frame_cb(IPM_TYPE_REFOCUS, &class_handle->frame_out);
						   }
					   }
