...
static cmr_int set_exif_specific_basic(JINF_EXIF_INFO_T *jinf_exif_info_ptr,
                                       EXIF_SPECIFIC_INFO_T *exif_specific_info,
                                       EXIF_SPEC_BASIC_T exif_spec_basic);
static cmr_int set_exif_specific_user(JINF_EXIF_INFO_T *jinf_exif_info_ptr,
                                      EXIF_SPEC_USER_T *exif_spec_user);
static cmr_int set_exif_specific_pic_taking_cond(
    JINF_EXIF_INFO_T *jinf_exif_info_ptr,
...
static cmr_int
set_exif_specific_data_time(JINF_EXIF_INFO_T *jinf_exif_info_ptr,
                            EXIF_SPEC_DATE_TIME_T *p_image_date_time);
static cmr_int set_exif_gps(JINF_EXIF_INFO_T *jinf_exif_info,
                            EXIF_GPS_INFO_T *p_exif_gps_info);
...
cmr_int get_time(cmr_int user_time, char *p_time) {
    ...
    if (user_time) {
        sprintf(p_time, "%04d:%02d:%02d %02d:%02d:%02d", (p->tm_year + 1900),
        ...
        sprintf(p_time, "%04d:%02d:%02d %02d:%02d:%02d", cur_date.year,
                cur_date.mon, cur_date.mday, cur_time.hour, cur_time.min,
                cur_time.sec);
#endif
    }
...
cmr_int cmr_exif_init(JINF_EXIF_INFO_T *jinf_exif_info_ptr,
    ...
    EXIF_SPEC_PIC_TAKING_COND_T *img_sensor_exif_ptr =
        (EXIF_SPEC_PIC_TAKING_COND_T *)(*setting_cb)(priv_data);

    CMR_LOGV("E");
    CMR_LOGD("flash=%d denominator=%d", img_sensor_exif_ptr->Flash,
             img_sensor_exif_ptr->ExposureIndex.denominator);
    ...
    set_exif_specific_basic(jinf_exif_info_ptr, &exif_specific_info,
                            exif_spec_basic);
...
cmr_int set_exif_pri_desc(JINF_EXIF_INFO_T *jinf_exif_info_ptr,
                          EXIF_PRI_DESC_T *p_exif_pri_desc) {
    ...
    *(uint32_t *)&(p_exif_pri_desc->valid) = (uint32_t)0x7F;
    ...
    CMR_LOGV("return 0x%p DateTime=%s", p_exif_pri_desc,
             p_exif_pri_desc->DateTime);
...
cmr_int set_exif_specific_user(JINF_EXIF_INFO_T *jinf_exif_info_ptr,
                               EXIF_SPEC_USER_T *p_exif_spec_user) {
    JINF_EXIF_INFO_T *jinf_exif_info = jinf_exif_info_ptr;
...
cmr_int set_exif_specific_pic_taking_cond(
    JINF_EXIF_INFO_T *jinf_exif_info_ptr,
    EXIF_SPEC_PIC_TAKING_COND_T *exif_spec_pic_taking_cond_ptr) {
    JINF_EXIF_INFO_T *jinf_exif_info = jinf_exif_info_ptr;
    ...
    jinf_exif_info->spec_ptr->pic_taking_cond_ptr =
        exif_spec_pic_taking_cond_ptr;
...
cmr_int set_exif_spec_other(JINF_EXIF_INFO_T *jinf_exif_info_ptr,
                            EXIF_SPEC_OTHER_T *p_exif_spec_other) {
    JINF_EXIF_INFO_T *jinf_exif_info = jinf_exif_info_ptr;
    ...
    if (strlen(arr_time) < 33)
        strncpy((char *)p_exif_spec_other->ImageUniqueID, arr_time,
                strlen(arr_time));
...
cmr_int set_exif_gps(JINF_EXIF_INFO_T *jinf_exif_info,
                     EXIF_GPS_INFO_T *p_exif_gps_info) {
