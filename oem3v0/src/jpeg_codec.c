...
#define JPEG_EVT_MASK_BITS                                                     \
    (cmr_u32)(JPEG_EVT_ENC_START | JPEG_EVT_ENC_NEXT | JPEG_EVT_DEC_START |    \
              JPEG_EVT_DEC_NEXT | JPEG_EVT_STOP | JPEG_EVT_KILL |              \
...
static cmr_int _dec_next(cmr_handle dec_handle,
                         ...
                         struct jpeg_dec_next_param *param_ptr);
...
static void save_inputdata(cmr_uint y_buf_addr, cmr_uint uv_buf_addr,
                           cmr_u32 size) {
    FILE *fp = NULL;
...
static void _prc_enc_cbparam(cmr_handle handle,
                             struct jpeg_enc_cb_param *parm_ptr) {
    ...
    if (enc_cxt_ptr->cur_line_num == enc_cxt_ptr->size.height) {
        CMR_LOGI("jpeg:adjust %d,%d.", enc_cxt_ptr->out_size.width,
                 enc_cxt_ptr->out_size.height);
        adjust_jpg_resolution(
            (void *)parm_ptr->stream_buf_vir, parm_ptr->stream_size,
    ...
    CMR_LOGV("slice_height %d,total_height %d,stream_size 0x%x.",
             parm_ptr->slice_height, parm_ptr->total_height,
             parm_ptr->stream_size);
#endif
    /*fp = fopen("/data/1.aw", "wb");
    if(0 != fp) {
            fwrite((void*)parm_ptr->stream_buf_vir, 1, parm_ptr->stream_size,
    fp);
            CMR_LOGV("save jpg.");
            fclose(fp);
    }*/
...
static cmr_int _enc_start(cmr_handle handle,
                          struct jpeg_codec_context *jcontext) {
    ...
    JPEGENC_PARAMS_T *jenc_parm_ptr =
        (JPEGENC_PARAMS_T *)malloc(sizeof(JPEGENC_PARAMS_T));
    ...
    if (1 == enc_cxt_ptr->is_thumbnail) {
        ...
        CMR_LOGV("jpeg:thumbnail, set_slice_height = %d",
                 jenc_parm_ptr->set_slice_height);
    }
    ...
    CMR_LOGI("uv endian, %d y endian %d, src_fd 0x%x",
             enc_cxt_ptr->src_endian.uv_endian,
             enc_cxt_ptr->src_endian.y_endian, jenc_parm_ptr->src_fd);
#ifdef JPEG_CODE_DEBUG
    ...
    CMR_LOGV("jpeg:jpegenc_params[%d]: virt: %lx, phys: %x,size %d.", i,
             (cmr_uint)jenc_parm_ptr->stream_virt_buf[i],
    ...
    /*if frame, and still use slice mode for sc8810 to keep the same  interface
     * to top layer*/
    if (enc_cxt_ptr->slice_height == enc_cxt_ptr->size.height) {
        cmr_u32 cur_slice_height = jenc_parm_ptr->set_slice_height;
        cmr_s32 slice_num =
            (cmr_u32)(enc_cxt_ptr->size.height / cur_slice_height);
        ...
        if (0 != slice_num) {
            do {
                ...
                next_slice_parm.param = jenc_parm_ptr;
                next_slice_parm.slice_height = jenc_parm_ptr->set_slice_height;
                next_slice_parm.yuv_phy_buf =
                    enc_cxt_ptr->src_addr_phy.addr_y +
                    enc_cxt_ptr->cur_line_num * enc_cxt_ptr->size.width;
                next_slice_parm.yuv_u_phy_buf =
                    enc_cxt_ptr->src_addr_phy.addr_u +
                    enc_cxt_ptr->cur_line_num * enc_cxt_ptr->size.width / 2;
    ...
    CMR_LOGV("jpeg:buf addr:0x%lx, size: %d", enc_cxt_ptr->stream_buf_vir,
             enc_cxt_ptr->stream_real_size);
...
static void _enc_start_post(cmr_handle handle,
                            struct jpeg_codec_context *jcontext, cmr_int ret) {
...
static cmr_int _enc_next(cmr_handle handle, struct jpeg_codec_context *jcontext,
                         struct jpeg_enc_next_param *param_ptr) {
    cmr_int ret = JPEG_CODEC_SUCCESS;
    cmr_uint jpeg_enc_buf_phys_addr;
    cmr_uint *jpeg_enc_buf_virt_addr;
    cmr_u32 jpeg_enc_buf_len;
    ...
    enc_cxt_ptr = (struct jpeg_enc *)handle;
    if (((enc_cxt_ptr->cur_line_num + enc_cxt_ptr->slice_height) >
         param_ptr->ready_line_num) &&
        (param_ptr->ready_line_num != enc_cxt_ptr->size.height)) {
        CMR_LOGI("jpeg:ready line num don't enough for a slice.enc line num "
                 "%d,read line num %d",
    ...
    if (0 != param_ptr->src_addr_phy.addr_y) {
        ...
        if (((cur_line_num + enc_cxt_ptr->slice_height) >
             param_ptr->ready_line_num) &&
            (param_ptr->ready_line_num == enc_cxt_ptr->size.height)) {
        ...
        update_parm.yuv_phy_buf = enc_cxt_ptr->src_addr_phy.addr_y +
                                  cur_line_num * enc_cxt_ptr->size.width;
        update_parm.yuv_u_phy_buf = enc_cxt_ptr->src_addr_phy.addr_u +
                                    cur_line_num * enc_cxt_ptr->size.width / 2;
...
static void _enc_next_post(cmr_handle handle,
                           struct jpeg_codec_context *jcontext, cmr_int ret) {
    struct jpeg_enc *enc_cxt_ptr = (struct jpeg_enc *)handle;
...
static void _dec_callback(cmr_u32 buf_id, cmr_u32 stream_size,
    ...
    if (dec_cxt_ptr->cur_line_num + dec_cxt_ptr->set_slice_height >
        dec_cxt_ptr->size.height) {
    ...
    CMR_LOGV("jpeg:dec handle 0x%p, line number %d, cpy_height %d", dec_cxt_ptr,
...
static cmr_int _dec_start(cmr_handle handle,
                          struct jpeg_codec_context *jcontext) {
    cmr_int ret = JPEG_CODEC_SUCCESS;
    ...
    if (JPEG_CODEC_SUCCESS == ret) {
        if (dec_cxt_ptr->slice_height == dec_cxt_ptr->size.height) {
            while (1 != slice_out.is_over) {
                ...
                if (JPEG_CODEC_SUCCESS !=
                    _dec_next(handle, jcontext, &next_param)) {
...
static void _dec_start_post(cmr_handle handle,
                            struct jpeg_codec_context *jcontext, cmr_int ret) {
    if (JPEG_CODEC_SUCCESS == ret) {
        if (!_jpeg_is_stop(jcontext)) {
            _dec_callback(0, 0, 0, jcontext);
...
static cmr_int _dec_next(cmr_handle handle, struct jpeg_codec_context *jcontext,
                         struct jpeg_dec_next_param *param_ptr) {
    cmr_int ret = JPEG_CODEC_SUCCESS;
    JPEGDEC_SLICE_OUT_T slice_out;
    ...
    if (0 == param_ptr->dst_addr_phy.addr_y) {
        ...
        slice_param.yuv_phy_buf =
            dec_cxt_ptr->dst_addr_phy.addr_y +
            dec_cxt_ptr->cur_line_num * dec_cxt_ptr->size.width;
        dec_cxt_ptr->dst_addr_phy.addr_u +=
            dec_cxt_ptr->handle_line_num * dec_cxt_ptr->size.width >> 1;
        dec_cxt_ptr->dst_addr_vir.addr_u +=
            dec_cxt_ptr->handle_line_num * dec_cxt_ptr->size.width >> 1;
    } else {
    ...
    CMR_LOGI("jpeg:update addr:0x%x,0x%x.", slice_param.yuv_phy_buf,
             slice_param.yuv_u_phy_buf);
...
static void _dec_next_post(cmr_handle handle,
                           struct jpeg_codec_context *jcontext, cmr_int ret) {
    if (JPEG_CODEC_SUCCESS == ret) {
        if (!_jpeg_is_stop(jcontext)) {
            _dec_callback(0, 0, 0, jcontext);
...
static cmr_int _jpeg_stop(cmr_handle jpeg_handle) {
    struct jpeg_codec_context *jcontext =
        (struct jpeg_codec_context *)jpeg_handle;
    CMR_LOGV("jpeg:start");
    ...
    if (0 == jcontext->type) {
        struct jpeg_enc *enc_cxt_ptr =
            (struct jpeg_enc *)jcontext->active_handle;
        if (NULL != enc_cxt_ptr) {
            free(enc_cxt_ptr);
            jcontext->active_handle = 0;
        }
    } else if (1 == jcontext->type) {
        struct jpeg_dec *dec_cxt_ptr =
            (struct jpeg_dec *)jcontext->active_handle;
        if (NULL != dec_cxt_ptr) {
            free(dec_cxt_ptr);
            jcontext->active_handle = 0;
        }
...
static cmr_int jpeg_thread_proc(struct cmr_msg *message, void *data) {
    ...
    case JPEG_EVT_ENC_NEXT:
        ...
        } while ((param_ptr->ready_line_num >= enc_cxt_ptr->size.height) &&
                 (enc_cxt_ptr->cur_line_num < enc_cxt_ptr->size.height));
    ...
    case JPEG_EVT_DEC_NEXT:
        handle = (cmr_handle)message->data;
        if (0 != message->data) {
            ret = _dec_next(jcontext->active_handle, jcontext,
                            (struct jpeg_dec_next_param *)message->data);
        } else {
    ...
    case JPEG_EVT_ENC_EXIF:
        if (NULL != message->data) {
            ret = _jpeg_enc_wexif((struct jpeg_enc_exif_param *)message->data,
                                  &wexif_out_param);
            if (JPEG_CODEC_SUCCESS == ret) {
    ...
    thumb_start:
        handle = (cmr_handle)message->data;
        ret = _enc_start(handle, jcontext);
        memset((void *)&jcontext->thumbnail_info, 0,
...
cmr_int jpeg_init(cmr_handle oem_handle, cmr_handle *jpeg_handle) {
    ...
    *jpeg_handle = 0;
    jcontext =
        (struct jpeg_codec_context *)malloc(sizeof(struct jpeg_codec_context));
    if (!jcontext) {
...
static cmr_int _check_enc_start_param(struct jpeg_enc_in_param *in_parm_ptr) {
    ...
    CMR_LOGV("jpeg:w h, %d %d, quality level %d", in_parm_ptr->size.width,
             in_parm_ptr->size.height, in_parm_ptr->quality_level);

#ifdef JPEG_CODE_DEBUG
    CMR_LOGV("jpeg:slice height, %d, slice mode %d", in_parm_ptr->slice_height,
             in_parm_ptr->slice_mod);
    CMR_LOGV("jpeg:phy addr 0x%lx 0x%lx, vir addr 0x%lx 0x%lx",
             in_parm_ptr->src_addr_phy.addr_y, in_parm_ptr->src_addr_phy.addr_u,
             in_parm_ptr->src_addr_vir.addr_y,
             in_parm_ptr->src_addr_vir.addr_u);
    CMR_LOGV("jpeg:endian %d %d", in_parm_ptr->src_endian.y_endian,
             in_parm_ptr->src_endian.uv_endian);
    CMR_LOGV("jpeg:stream phy 0x%lx vir 0x%lx, size 0x%x",
             in_parm_ptr->stream_buf_phy, in_parm_ptr->stream_buf_vir,
...
static cmr_int _get_enc_start_param(struct jpeg_enc *cxt_ptr,
    ...
    CMR_LOGI("jpeg:img_size: w:%d, h:%d", in_parm_ptr->size.width,
             in_parm_ptr->size.height);
#endif
    CMR_LOGI("jpeg:slice_height:%d", in_parm_ptr->slice_height);
    CMR_LOGI("jpeg:src_fd:0x%x, stream_buf_fd 0x%lx ", in_parm_ptr->src_fd,
             in_parm_ptr->stream_buf_fd);
    ...
    cxt_ptr->stream_buf_fd = in_parm_ptr->stream_buf_fd;
...
static cmr_int _get_dec_start_param(struct jpeg_dec *cxt_ptr,
    ...
    CMR_LOGV("jpeg:dst phy  0x%x 0x%x, vir 0x%x 0x%x",
             (cmr_u32)cxt_ptr->dst_addr_phy.addr_y,
             (cmr_u32)cxt_ptr->dst_addr_phy.addr_u,
             (cmr_u32)cxt_ptr->dst_addr_vir.addr_y,
             (cmr_u32)cxt_ptr->dst_addr_vir.addr_u);
...
static cmr_int _check_wexif_param(struct jpeg_enc_exif_param *param_ptr) {
    ...
    CMR_LOGI("jpeg:src addr 0x%x size %d thumb addr 0x%x,size %d target addr "
             "0x%x,size %d.",
...
cmr_int jpeg_enc_start(struct jpeg_enc_in_param *in_parm_ptr) {
    ...
    struct jpeg_codec_context *jcontext =
        (struct jpeg_codec_context *)in_parm_ptr->jpeg_handle;
...
cmr_int jpeg_enc_next(struct jpeg_enc_next_param *param_ptr) {
    ...
    if (JPEG_YUV_SLICE_MUTI_BUF == enc_cxt_ptr->slice_mod) {
        if (0 == param_ptr->src_addr_phy.addr_y ||
            0 == param_ptr->src_addr_vir.addr_y ||
            0 == param_ptr->src_addr_phy.addr_u ||
            0 == param_ptr->src_addr_vir.addr_u) {
            ret = JPEG_CODEC_PARAM_ERR;
            goto enc_next_end;
        }
    }

    data_ptr = (struct jpeg_enc_next_param *)malloc(
        sizeof(struct jpeg_enc_next_param));
    if (0 == data_ptr) {
        CMR_LOGE("jpeg:No mem");
        ret = JPEG_CODEC_NO_MEM;
        goto enc_next_end;
...
cmr_int jpeg_dec_next(struct jpeg_dec_next_param *param_ptr) {
    ...
    if (JPEG_YUV_SLICE_MUTI_BUF == dec_cxt_ptr->slice_mod) {
        if (0 == param_ptr->dst_addr_phy.addr_y ||
            0 == param_ptr->dst_addr_vir.addr_y ||
            0 == param_ptr->dst_addr_phy.addr_u ||
            0 == param_ptr->dst_addr_vir.addr_u) {
            ret = JPEG_CODEC_PARAM_ERR;
            goto dec_next_end;
        }
    }
    data_ptr = (struct jpeg_dec_next_param *)malloc(
        sizeof(struct jpeg_dec_next_param));
    if (0 == data_ptr) {
        ret = JPEG_CODEC_NO_MEM;
        goto dec_next_end;
    }
...
cmr_int jpeg_enc_add_eixf(struct jpeg_enc_exif_param *param_ptr,
    ...
    data = (struct jpeg_enc_exif_param *)malloc(
        sizeof(struct jpeg_enc_exif_param));
