...
setting_set_auto_exposure_mode(struct setting_component *cpt,
    ...
    // if (CAMERA_AE_SPOT_METERING == parm->ae_param.mode)
    {
        if (setting_is_rawrgb_format(cpt, parm)) {
            ...
            CMR_LOGI("trim rect (%d,%d,%d,%d)", trim.start_x, trim.start_y,
...
static cmr_int
setting_get_sprd_highiso_enabled(struct setting_component *cpt,
                                 struct setting_cmd_parameter *parm) {
    cmr_int ret = 0;
    char value[PROPERTY_VALUE_MAX];
    struct setting_hal_param *hal_param = get_hal_param(cpt, parm->camera_id);
...
static cmr_int
setting_get_sprd_burstmode_enable(struct setting_component *cpt,
                                  struct setting_cmd_parameter *parm) {
    cmr_int ret = 0;
    struct setting_hal_param *hal_param = get_hal_param(cpt, parm->camera_id);
...
static cmr_int
setting_set_sprd_highiso_enabled(struct setting_component *cpt,
                                 struct setting_cmd_parameter *parm) {
    cmr_int ret = 0;
    struct setting_hal_param *hal_param = get_hal_param(cpt, parm->camera_id);
...
static cmr_int
setting_set_sprd_burstmode_enable(struct setting_component *cpt,
                                  struct setting_cmd_parameter *parm) {
    cmr_int ret = 0;
    struct setting_hal_param *hal_param = get_hal_param(cpt, parm->camera_id);
...
static cmr_int
setting_set_uhd_recording_enable(struct setting_component *cpt,
                                 struct setting_cmd_parameter *parm) {
    cmr_int ret = 0;
    struct setting_hal_param *hal_param = get_hal_param(cpt, parm->camera_id);

    hal_param->uhd_recording_enable = parm->cmd_type_value;
    CMR_LOGD("uhd_recording_enable=%ld", hal_param->uhd_recording_enable);

    return ret;
}

static cmr_int
setting_get_uhd_recording_enable(struct setting_component *cpt,
                                 struct setting_cmd_parameter *parm) {
    cmr_int ret = 0;
    struct setting_hal_param *hal_param = get_hal_param(cpt, parm->camera_id);

    parm->cmd_type_value = hal_param->uhd_recording_enable;
    CMR_LOGD("uhd_recording_enable=%ld", hal_param->uhd_recording_enable);
...
static cmr_int setting_is_need_flash(struct setting_component *cpt,
    ...
    capture_mode =
        (enum takepicture_mode)parm->ctrl_flash.capture_mode.capture_mode;
...
static cmr_int setting_ctrl_flash(struct setting_component *cpt,
    ...
    if (is_active) {
        ...
        if ((ctrl_flash_status == FLASH_HIGH_LIGHT) ||
            (ctrl_flash_status == FLASH_OPEN)) {
            exif_flash = 1;
        } else {
            exif_flash = 0;
        ...
        if (setting_is_need_flash(cpt, parm)) {
            ...
            if ((uint32_t)CAMERA_FLASH_MODE_TORCH != flash_mode) {
                ...
                CMR_LOGV("parm->ctrl_flash.will_capture=%ld",
                ...
                if ((hal_param->flash_param.has_preflashed == 0) &&
                    hal_param->flash_param
                        .set_flash_mode_off_after_close_flash) {
                    hal_param->flash_param.flash_mode = CAMERA_FLASH_MODE_OFF;
                    hal_param->flash_param
                        .set_flash_mode_off_after_close_flash = 0;
...
static cmr_int
setting_set_roi_convergence_req(struct setting_component *cpt,
                                struct setting_cmd_parameter *parm) {
    cmr_int ret = 0;
    struct setting_hal_param *hal_param = get_hal_param(cpt, parm->camera_id);
    struct setting_init_in *init_in = &cpt->init_in;
    ...
    flash_mode = hal_param->flash_param.flash_mode;
    if (cpt->is_touch_focus == 1 && (flash_mode == CAMERA_FLASH_MODE_AUTO ||
                                     flash_mode == CAMERA_FLASH_MODE_OFF)) {
        cmr_setting_clear_sem(cpt);
        // wait roi converged
        if (init_in->setting_isp_ioctl) {
            ret = (*init_in->setting_isp_ioctl)(init_in->oem_handle,
