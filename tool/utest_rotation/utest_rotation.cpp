...
static char utest_rotation_dst_y_file[] = "/data/like/pic/dst_y_%dx%d-angle%d-format%d_%d.raw";
static char utest_rotation_dst_uv_file[] = "/data/like/pic/dst_uv_%dx%d-angle%d-format%d_%d.raw";
...
{
	INFO("Usage:\n");
	INFO("utest_rotation -if format -iw width -ih height -ia angle(90 180 270 -1) -icnt filenum(1 2)\n");
}
...
static int utest_rotation_mem_alloc(struct utest_rotation_context *g_utest_rotation_cxt_ptr)
{
	...
	rotation_cxt_ptr->input_y_pmem_hp = new MemIon("/dev/ion",
	...
	rotation_cxt_ptr->input_y_pmem_hp->get_phy_addr_from_ion(&rotation_cxt_ptr->input_y_physical_addr,
						&rotation_cxt_ptr->input_y_pmemory_size);

	rotation_cxt_ptr->input_y_virtual_addr = (unsigned char*)rotation_cxt_ptr->input_y_pmem_hp->getBase();
	...
	rotation_cxt_ptr->input_uv_pmem_hp = new MemIon("/dev/ion",
										rotation_cxt_ptr->width* rotation_cxt_ptr->height,
										MemIon::NO_CACHING, ION_HEAP_ID_MASK_MM);
	...
	rotation_cxt_ptr->input_uv_pmem_hp->get_phy_addr_from_ion(&rotation_cxt_ptr->input_uv_physical_addr,
				&rotation_cxt_ptr->input_uv_pmemory_size);

	rotation_cxt_ptr->input_uv_virtual_addr = (unsigned char*)rotation_cxt_ptr->input_uv_pmem_hp->getBase();
	...
	memset(rotation_cxt_ptr->input_uv_virtual_addr, 0x80, rotation_cxt_ptr->width * rotation_cxt_ptr->height);


	/* alloc outout y buffer */
	rotation_cxt_ptr->output_y_pmem_hp = new MemIon("/dev/ion",
										rotation_cxt_ptr->width* rotation_cxt_ptr->height,
										MemIon::NO_CACHING, ION_HEAP_ID_MASK_MM);
	...
	rotation_cxt_ptr->output_y_pmem_hp->get_phy_addr_from_ion(&rotation_cxt_ptr->output_y_physical_addr,
				&rotation_cxt_ptr->output_y_pmemory_size);

	rotation_cxt_ptr->output_y_virtual_addr = (unsigned char*)rotation_cxt_ptr->output_y_pmem_hp->getBase();
	...
	rotation_cxt_ptr->output_uv_pmem_hp = new MemIon("/dev/ion",
											rotation_cxt_ptr->width * rotation_cxt_ptr->height,
											MemIon::NO_CACHING, ION_HEAP_ID_MASK_MM);
	...
	rotation_cxt_ptr->output_uv_pmem_hp->get_phy_addr_from_ion(&rotation_cxt_ptr->output_uv_physical_addr,
						&rotation_cxt_ptr->output_uv_pmemory_size);

	rotation_cxt_ptr->output_uv_virtual_addr = (unsigned char*)rotation_cxt_ptr->output_uv_pmem_hp->getBase();
