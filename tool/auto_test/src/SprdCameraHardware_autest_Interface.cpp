...
static struct auto_test_cmr_context *g_auto_test_cmr_cxt_ptr =
    &auto_test_cmr_cxt;
...
static int g_mem_method =
    USE_PHYSICAL_ADD; /*0: physical address, 1: iommu  address*/
cmr_int read_data_from_sensor();
static int auto_test_dcam_preview_mem_alloc(cmr_u32 size, cmr_u32 sum,
                                            cmr_uint *phy_addr,
                                            cmr_uint *vir_addr);
static int auto_test_dcam_cap_memory_alloc(cmr_u32 size, cmr_u32 sum,
                                           cmr_uint *addr_phy,
                                           cmr_uint *addr_vir);

static void auto_test_dcam_usage() {
    INFO("auto_test_dcam_usage:\n");
    INFO("auto_test_camera -maincmd cali_main_cmd -subcmd cali_sub_cmd -id "
         "sensor_id -w capture_width -h capture_height \n");
...
*
*/
static int auto_test_dcam_param_set(int maincmd, int subcmd, int cameraid,
...
static void auto_test_dcam_preview_mem_release(void) {
    ...
    for (i = 0; i < AUTO_TEST_PREVIEW_BUF_NUM; i++) {
        if (g_mem_method == USE_PHYSICAL_ADD) {
            ...
            cmr_cxt_ptr->preview_pmem_hp[i]->free_iova(
                ION_MM, cmr_cxt_ptr->preview_physical_addr[i],
...
cmr_int Callback_Malloc(enum camera_mem_cb_type type, cmr_u32 *size_ptr,
    ...
    if (!vir_addr || !size_ptr || !sum_ptr || (0 == *size_ptr) ||
        (0 == *sum_ptr)) {
        INFO("alloc param error 0x%lx 0x%lx 0x%lx", (cmr_uint)phy_addr,
...
int auto_test_dcam_preview_mem_alloc(cmr_u32 size, cmr_u32 sum,
    ...
    for (i = 0; i < sum; i++) {
        ...
        if (g_mem_method == USE_PHYSICAL_ADD)
            cmr_cxt_ptr->preview_pmem_hp[i] = new MemoryHeapIon(
                "/dev/ion", buf_size, MemoryHeapBase::NO_CACHING,
                ION_HEAP_ID_MASK_MM);
        else
            cmr_cxt_ptr->preview_pmem_hp[i] = new MemoryHeapIon(
                "/dev/ion", buf_size, MemoryHeapBase::NO_CACHING,
        ...
        if (g_mem_method == USE_PHYSICAL_ADD) {
            cmr_cxt_ptr->preview_pmem_hp[i]->get_phy_addr_from_ion(
                &cmr_cxt_ptr->preview_physical_addr[i],
                &cmr_cxt_ptr->preview_pmemory_size[i]);
        } else {
            cmr_cxt_ptr->preview_pmem_hp[i]->get_iova(
                ION_MM, &cmr_cxt_ptr->preview_physical_addr[i],
                &cmr_cxt_ptr->preview_pmemory_size[i]);
        }

        cmr_cxt_ptr->preview_virtual_addr[i] =
            (unsigned char *)cmr_cxt_ptr->preview_pmem_hp[i]->base();
        if (!cmr_cxt_ptr->preview_physical_addr[i]) {
...
static void auto_test_dcam_cap_memory_release(void) {
    ...
    for (i = 0; i < AUTO_TEST_CAPTURE_BUF_NUM; i++) {
        if (g_mem_method == USE_PHYSICAL_ADD) {
            ...
            if (cmr_cxt_ptr->cap_physical_addr[i])
                cmr_cxt_ptr->cap_pmem_hp[i]->free_iova(
                    ION_MM, cmr_cxt_ptr->cap_physical_addr[i],
...
int auto_test_dcam_cap_memory_alloc(cmr_u32 size, cmr_u32 sum,
                                    cmr_uint *addr_phy, cmr_uint *addr_vir) {
    ...
    for (i = 0; i < sum; i++) {
        if (g_mem_method == USE_PHYSICAL_ADD)
            cmr_cxt_ptr->cap_pmem_hp[i] = new MemoryHeapIon(
                "/dev/ion", buffer_size, MemoryHeapBase::NO_CACHING,
                ION_HEAP_ID_MASK_MM);
        else
            cmr_cxt_ptr->cap_pmem_hp[i] = new MemoryHeapIon(
                "/dev/ion", buffer_size, MemoryHeapBase::NO_CACHING,
        ...
        if (g_mem_method == USE_PHYSICAL_ADD) {
            cmr_cxt_ptr->cap_pmem_hp[i]->get_phy_addr_from_ion(
                &cmr_cxt_ptr->cap_physical_addr[i],
                &cmr_cxt_ptr->cap_pmemory_size[i]);
        } else {
            cmr_cxt_ptr->cap_pmem_hp[i]->get_iova(
                ION_MM, &cmr_cxt_ptr->cap_physical_addr[i],
                &cmr_cxt_ptr->cap_pmemory_size[i]);
        }

        cmr_cxt_ptr->cap_virtual_addr[i] =
            (unsigned char *)cmr_cxt_ptr->cap_pmem_hp[i]->base();
        if (!cmr_cxt_ptr->cap_physical_addr[i]) {
...
*/
int autotest_came_save_to_buf(uint32_t **ppbuf_addr,
                              struct auto_test_cmr_context *save_cmr_cxt_ptr,
                              int image_type) {
    int ret_val = -1;
    ...
    if ((0 == image_type) || (2 == image_type)) {
        g_image_size = save_cmr_cxt_ptr->capture_width *
                       save_cmr_cxt_ptr->capture_height * 2;
...
static int32_t auto_test_dcam_save_raw_data(void) {
    ...
    FILE *fp = 0;
    INFO("debug %s cmr_cxt_ptr->cmd=%d %d E \n", __func__, cmr_cxt_ptr->cmd,
         __LINE__);
    switch (cmr_cxt_ptr->cmd) {
    case AUTO_TEST_CALIBRATION_AWB:
        sprintf(file_name, calibration_raw_data_file, sensor_ptr->name,
                cmr_cxt_ptr->capture_width, cmr_cxt_ptr->capture_height, "awb");
        break;
    case AUTO_TEST_CALIBRATION_LSC:
        sprintf(file_name, calibration_raw_data_file, sensor_ptr->name,
                cmr_cxt_ptr->capture_width, cmr_cxt_ptr->capture_height, "lsc");
        break;
    case AUTO_TEST_CALIBRATION_FLASHLIGHT:
        sprintf(file_name, calibration_raw_data_file, sensor_ptr->name,
                cmr_cxt_ptr->capture_width, cmr_cxt_ptr->capture_height,
                "flashlight");
        break;
    default:
        break;
    ...
    if (fp != NULL) {
        fwrite((void *)cmr_cxt_ptr->capture_raw_vir_addr, 1,
               cmr_cxt_ptr->capture_width * cmr_cxt_ptr->capture_height * 2,
               fp);
        fclose(fp);
...
static int32_t auto_test_save_raw_data(char *file_name, void *buf,
                                       uint32_t len) {
    int32_t rtn = 0;
    FILE *fp = 0;
    INFO("debug %s %d E \n", __func__, __LINE__);
...
static int32_t auto_test_dcam_save_jpg(int image_type) {
    ...
    autotest_came_save_to_buf(&autotest_camer_saveaddr, cmr_cxt_ptr,
                              image_type);
    fp = fopen(file_name, "wb");
    if (fp != NULL) {
...
static int32_t
auto_test_dcam_find_param_index(struct auto_test_cmr_context *cmr_cxt_ptr) {
    ...
    while (1) {
        ...
        INFO("trim: index:%d, width:%d height:%d\n", (int32_t)i,
             (int32_t)trim_ptr[i].trim_width, (int32_t)trim_ptr[i].trim_height);
        if (height2 == trim_ptr[i].trim_height) {
            index = i - 1;
            INFO("find the param i = %d, width = %d, height = %d\n", i,
                 (int32_t)trim_ptr[i].trim_width,
                 (int32_t)trim_ptr[i].trim_height);
            break;
        }
...
static int32_t auto_test_dcam_awb(uint8_t sub_cmd) {
    ...
    snprintf(postfix_name, 128, "%d_%d_%d_%d_%d_%d_%lu",
             (time_stamp->tm_year + 1900), time_stamp->tm_mon,
             time_stamp->tm_mday, (time_stamp->tm_hour + 8), time_stamp->tm_min,
             time_stamp->tm_sec, awb_time_stampusec.tv_usec);
    ...
    if (0 == (sensor_ptr->raw_info_ptr->tune_ptr->blc_bypass)) {
        ...
        img_blc_addr = (uint32_t *)malloc(cmr_cxt_ptr->capture_width *
                                          cmr_cxt_ptr->capture_height * 2);
        if (img_blc_addr) {
            memset((void *)img_blc_addr, 0x00, cmr_cxt_ptr->capture_width *
            ...
            rtn =
                ISP_Cali_BlackLevelCorrection(&img_addr, &tmp_addr, &blc_rect,
                                              &blc_size, bayer_mod, &blc_stat);
    ...
    if (0 == sub_cmd) {
        ...
        uint16_t len = 0;
        fix_ptr =
            (struct sensor_raw_fix_info *)sensor_ptr->raw_info_ptr->fix_ptr;
        ...
        if (mHalIspCali->ISP_Cali_GetLensTabs(tmp_addr, lsc_grid, img_size,
                                              (uint32_t *)len_tab_addr, index,
                                              0, sub_cmd)) {
            rtn = -1;
    ...
    dst_addr.v_addr = 0;
    dst_addr.uv_addr = 0;
    mHalIspCali->ISP_Cali_LensCorrection(&tmp_addr, &dst_addr, img_size,
                                         lsc_grid, len_tab_addr);

    if (mHalIspCali->ISP_Cali_RawRGBStat(&dst_addr, &rect, &img_size,
                                         &stat_param)) {
...
static int32_t auto_test_dcam_flashlight(uint8_t sub_cmd) {
    ...
    switch (sub_cmd) {
    case AUTO_TEST_CALI_SUB_CMD_GOLDEN:
        sprintf(file_name, calibration_flashlight_file, sensor_ptr->name,
                "gldn");
        break;
    case AUTO_TEST_CALI_SUB_CMD_RANDOM:
        sprintf(file_name, calibration_flashlight_file, sensor_ptr->name,
    ...
    if (fp != NULL) {
        ...
        ERR("auto_test_dcam_flashlight: failed to open "
            "calibration_awb_file.\n");
        rtn = -1;
        goto flashlight_exit;
    }
    if (auto_test_dcam_save_raw_data())
...
static int32_t auto_test_dcam_lsc(uint8_t sub_cmd) {
    ...
    if (0 == (sensor_ptr->raw_info_ptr->tune_ptr->blc_bypass)) {
        ...
        if (img_blc_addr) {
            ...
            blc_stat.gb_stat = blc_ptr->offset[0].gb;
            rtn =
                ISP_Cali_BlackLevelCorrection(&img_addr, &dst_addr, &blc_rect,
                                              &blc_size, bayer_mod, &blc_stat);
        } else {
            ERR("auto_test_dcam_flashlight: failed to alloc buffer.\n");
            rtn = -1;
    ...
    if (mHalIspCali->ISP_Cali_GetLensTabs(dst_addr, lsc_grid, img_size,
                                          len_tab_addr, index, 0, sub_cmd)) {
        rtn = -1;
        ERR("auto_test_dcam_lsc: fail to isp_cali_getlenstab.\n");
        goto lsc_exit;
    }
    switch (sub_cmd) {
    case AUTO_TEST_CALI_SUB_CMD_GOLDEN:
        sprintf(file_name, calibration_lsc_file, sensor_ptr->name,
                cmr_cxt_ptr->capture_width, cmr_cxt_ptr->capture_height, 0,
                "gldn");
        break;
    case AUTO_TEST_CALI_SUB_CMD_RANDOM:
        sprintf(file_name, calibration_lsc_file, sensor_ptr->name,
...
static void auto_test_dcam_cap_cb(camera_cb_type cb, const void *client_data,
    ...
    if (CAMERA_FUNC_TAKE_PICTURE == func) {
        ...
        case CAMERA_EVT_CB_SNAPSHOT_DONE:
            INFO("debug %s %d X \n", __func__, __LINE__);
            {
                ...
                cmr_cxt_ptr->capture_raw_vir_addr =
                    (uintptr_t)frame_snap->y_vir_addr;
        ...
        case CAMERA_EXIT_CB_DONE: {
            ...
            struct camera_jpeg_param *encInfo =
                &((struct camera_frame_type *)parm4)->jpeg_param;
...
static int32_t auto_test_dcam_capture(void) {
    ...
    /*	if (auto_test_dcam_cap_memory_alloc())
            ...
            if (auto_test_dcam_capture_flash_eb())
                    return -1;
    */
    if (NULL == cmr_cxt_ptr->camera_handle || NULL == mHalOem ||
        NULL == mHalOem->ops) {
        ERR("auto_test_dcam_capture: oem is null or oem ops is null, do "
            "nothing\n");
        return -1;
    }

    if (CMR_CAMERA_SUCCESS !=
        mHalOem->ops->camera_set_mem_func(
            cmr_cxt_ptr->camera_handle, (void *)Callback_Malloc,
            (void *)Callback_Free, (void *)AUTOTEST_CLIENTDATA)) {
        ERR("auto_test_dcam_capture: failed to camera_set_mem_func");
        return -1;
    }
    if ((AUTO_TEST_CALIBRATION_CAP_JPG == cmr_cxt_ptr->cmd)) {
        if (CMR_CAMERA_SUCCESS !=
            mHalOem->ops->camera_take_picture(cmr_cxt_ptr->camera_handle,
                                              CAMERA_NORMAL_MODE)) {
            rtn = -1;
            INFO("debug %s  line=%d  \n", __func__, __LINE__);
            goto cap_exit;
...
cmr_int read_data_from_sensor() {
    ...
    SET_PARM(mHalOem, cmr_cxt_ptr->camera_handle, CAMERA_PARAM_PREVIEW_SIZE,
             (cmr_uint)&req_size);

    req_size.width = (cmr_u32)cmr_cxt_ptr->capture_width;
    req_size.height = (cmr_u32)cmr_cxt_ptr->capture_height;
    SET_PARM(mHalOem, cmr_cxt_ptr->camera_handle, CAMERA_PARAM_CAPTURE_SIZE,
             (cmr_uint)&req_size);
    ...
    if ((g_sensor_opened == 0) &&
        (CMR_CAMERA_SUCCESS !=
         mHalOem->ops->camera_init(
             cmr_cxt_ptr->sensor_id, auto_test_dcam_cap_cb,
             (void *)AUTOTEST_CLIENTDATA, 1, &cmr_cxt_ptr->camera_handle,
...
int32_t autotest_set_testmode(int camerinterface, int maincmd, int subcmd,
    ...
    INFO("debug %s camerinterface =%d maincmd=%d subcmd= %d cameraid=%d "
         "width=%d height=%d line=%d E\n ",
         __func__, camerinterface, maincmd, subcmd, cameraid, width, height,
         __LINE__);

    if ((maincmd >= AUTO_TEST_CALIBRATION_MAX || maincmd < 0) ||
        (subcmd >= AUTO_TEST_CALI_SUB_CMD_MAX || subcmd < 0) ||
        (cameraid > 1 || cameraid < 0) || (width % 32 || height % 32)) {
