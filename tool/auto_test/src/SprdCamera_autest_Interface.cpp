...
static void  StretchColors(void* pDest, int nDestWidth, int nDestHeight, int nDestBits, void* pSrc, int nSrcWidth, int nSrcHeight, int nSrcBits)
{
	...
	int linesize ;
	ALOGI("AutoTest: nDestWidth = %d, nDestHeight = %d, nSrcWidth = %d, nSrcHeight = %d, offsetX = %d, offsetY= %d \n", \
...
static void auto_dcamtest_switchTB(uint8_t *buffer, uint16_t width, uint16_t height, uint8_t pixel)
{
	uint32_t j;
	uint32_t linesize;
	uint8_t *dst = NULL;
...
static int autotest_rotation(uint32_t agree, uint32_t width, uint32_t height, uint32_t in_addr, uint32_t out_addr)
{
...
void autotest_camera_cb(enum camera_cb_type cb , const void *client_data , enum camera_func_type func , void* parm4)
{
	...
	if(!frame) {
		ALOGI("AutoTest: %s,%d, camera call back parm4 error: NULL, do nothing\n", __func__, __LINE__);
		return;
	}
...
static sprd_camera_memory_t* allocCameraMem(int buf_size, int num_bufs, uint32_t is_cache)
{
	...
	if (pHeapIon == NULL || pHeapIon->getHeapID() < 0) {
		ALOGE("AutoTest: %s,%s,%d, failed: pHeapIon is null or getHeapID failed.\n", __FILE__, __func__, __LINE__);
		goto getpmem_fail;
	}

	if (NULL == pHeapIon->getBase() || MAP_FAILED == pHeapIon->getBase()) {
		ALOGE("AutoTest: error getBase is null. %s,%s,%d, failed: ion get base err.\n", __FILE__, __func__, __LINE__);
		goto getpmem_fail;
	}
	ALOGI("AutoTest: %s,%s,%d IN\n", __FILE__,__func__, __LINE__);
...
int autotest_read_cam_buf(int ** pp_image_addr,int size,int *out_size)
{
	...
	if(!previewHeapArray[target_buffer_id]){
		...
		vir_addr = (cmr_uint)previewHeapArray[target_buffer_id]->ion_heap->getBase(); //virtual address
