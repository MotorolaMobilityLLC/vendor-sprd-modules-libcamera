...
static char utest_scaling_dst_y_file[] = "/data/like/pic/utest_scaling_dst_y_%dx%d_format_%d_%d.raw";
static char utest_scaling_dst_uv_file[] = "/data/like/pic/utest_scaling_dst_uv_%dx%d_format_%d_%d.raw";
...
{
	INFO("Usage:\n");
	INFO("utest_scaling -if format -of format -iw input_width -ih input_height -ow out_width -oh out_height -num file_num\n");
}

static int utest_scaling_param_set(struct utest_scaling_context *g_utest_scaling_cxt_ptr, int argc, char **argv)
{
	int i = 0;
	...
	INFO("utest_scaling_set: input_size=(%d, %d), output_size=(%d, %d), cnt=%d\n",
...
static int utest_scaling_mem_alloc(struct utest_scaling_context *g_utest_scaling_cxt_ptr)
{
	struct utest_scaling_context *scaling_cxt_ptr = g_utest_scaling_cxt_ptr;

	/* alloc input y buffer */
	scaling_cxt_ptr->input_y_pmem_hp = new MemIon("/dev/ion",
										scaling_cxt_ptr->input_width * scaling_cxt_ptr->input_height,
	...
	scaling_cxt_ptr->input_y_pmem_hp->get_phy_addr_from_ion(&scaling_cxt_ptr->input_y_physical_addr,
			&scaling_cxt_ptr->input_y_pmemory_size);

	scaling_cxt_ptr->input_y_virtual_addr = (unsigned char*)scaling_cxt_ptr->input_y_pmem_hp->getBase();
	...
	INFO("LIKE:src y phy addr :0x%lx, virtual addr:%p\n",
			scaling_cxt_ptr->input_y_physical_addr, scaling_cxt_ptr->input_y_virtual_addr);
	...
	scaling_cxt_ptr->input_uv_pmem_hp = new MemIon("/dev/ion",
										scaling_cxt_ptr->input_width * scaling_cxt_ptr->input_height,
										MemIon::NO_CACHING, ION_HEAP_ID_MASK_MM);
	...
	scaling_cxt_ptr->input_uv_pmem_hp->get_phy_addr_from_ion(&scaling_cxt_ptr->input_uv_physical_addr,
				&scaling_cxt_ptr->input_uv_pmemory_size);


	scaling_cxt_ptr->input_uv_virtual_addr = (unsigned char*)scaling_cxt_ptr->input_uv_pmem_hp->getBase();
	...
	INFO("LIKE:src uv phy addr :0x%lx, virtual addr:%p\n",
			scaling_cxt_ptr->input_uv_physical_addr, scaling_cxt_ptr->input_uv_virtual_addr);

	memset(scaling_cxt_ptr->input_uv_virtual_addr, 0x80, scaling_cxt_ptr->input_width * scaling_cxt_ptr->input_height);

	/* alloc outout y buffer */
	scaling_cxt_ptr->output_y_pmem_hp = new MemIon("/dev/ion",
											scaling_cxt_ptr->output_width * scaling_cxt_ptr->output_height,
											MemIon::NO_CACHING, ION_HEAP_ID_MASK_MM);
	...
	scaling_cxt_ptr->output_y_pmem_hp->get_phy_addr_from_ion(&scaling_cxt_ptr->output_y_physical_addr,
			&scaling_cxt_ptr->output_y_pmemory_size);

	scaling_cxt_ptr->output_y_virtual_addr = (unsigned char*)scaling_cxt_ptr->output_y_pmem_hp->getBase();
	...
	INFO("LIKE:dst y phy addr :0x%lx, virtual addr:%p\n",
			scaling_cxt_ptr->output_y_physical_addr, scaling_cxt_ptr->output_y_virtual_addr);

	/* alloc outout uv buffer */
	scaling_cxt_ptr->output_uv_pmem_hp = new MemIon("/dev/ion",
											scaling_cxt_ptr->output_width * scaling_cxt_ptr->output_height,
											MemIon::NO_CACHING, ION_HEAP_ID_MASK_MM);
	...
	scaling_cxt_ptr->output_uv_pmem_hp->get_phy_addr_from_ion(&scaling_cxt_ptr->output_uv_physical_addr,
			&scaling_cxt_ptr->output_uv_pmemory_size);

	scaling_cxt_ptr->output_uv_virtual_addr = (unsigned char*)scaling_cxt_ptr->output_uv_pmem_hp->getBase();
	...
	INFO("LIKE:dst uv phy addr :0x%lx, virtual addr:%p\n",
			scaling_cxt_ptr->output_uv_physical_addr, scaling_cxt_ptr->output_uv_virtual_addr);
...
static int utest_scaling_mem_release(struct utest_scaling_context *g_utest_scaling_cxt_ptr)
{
	struct utest_scaling_context *scaling_cxt_ptr = g_utest_scaling_cxt_ptr;
...
static int utest_scaling_src_cfg(struct utest_scaling_context *g_utest_scaling_cxt_ptr)
{
	FILE* fp = 0;
	struct utest_scaling_context *scaling_cxt_ptr = g_utest_scaling_cxt_ptr;
...
static int utest_scaling_save_raw_data(struct utest_scaling_context *g_utest_scaling_cxt_ptr)
{
	...
	sprintf(file_name, utest_scaling_dst_y_file, scaling_cxt_ptr->output_width,
			scaling_cxt_ptr->output_height, scaling_cxt_ptr->input_format, scaling_cxt_ptr->output_format);
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < 100; i++) {
		...
		utest_do_scaling(fd, scaling_cxt_ptr->input_width, scaling_cxt_ptr->input_height,
						scaling_cxt_ptr->input_y_physical_addr,scaling_cxt_ptr->input_uv_physical_addr,
						scaling_cxt_ptr->input_format,
