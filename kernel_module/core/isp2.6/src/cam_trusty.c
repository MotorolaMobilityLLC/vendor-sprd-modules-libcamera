...
bool camca_isp_pitch_set(uint32_t y_pitch, uint32_t u_pitch,uint32_t v_pitch)
{
	...
	pr_info("camca_isp_pitch_set enter");
	...
	pr_info("camca_isp_pitch_set exit,    ret =%d\n",  ret);
	return ret;
}
											...
											uint32_t pitch)
{
	...
	ret = cam_ca_write(&isp_3dnr_pitch_set, sizeof(struct isp_3dnr_reg_msg));
	if ( !ret ) {
...
bool camca_csi_switch_ctrl_set(uint32_t csi_sel_ctrl)
{
	...
	pr_info(" camca_csi_switch_ctrl_set");
	...
	pr_info(" camca_csi_switch_ctrl_set,  ret =%d\n",  ret);
	return ret;
...
bool camca_enter_tamode(struct  sprd_cam_sec_cfg   * camsec_cfg)
{
	...
	pr_info(" ca chanel_state =%d,  camsec_mode=%d, work_mode=%d",  ca->chanel_state,
				camsec_cfg->camsec_mode, camsec_cfg->work_mode);
...
bool camca_exit_tamode(struct  sprd_cam_sec_cfg   * camsec_cfg)
{
	...
	pr_info("camca_exit_tamode ca chanel_state =%d,  camsec_mode=%d, work_mode=%d",
					ca->chanel_state,
					camsec_cfg->camsec_mode,
	...
	pr_info(" camca_exit_tamode ret =%d\n",  ret);
