	...
	struct isp_dev_rgb_afm_info *rafm_info)
{
	...
	DCAM_REG_MWR(idx, ISP_RAW_AFM_PARAMETERS, BIT_7,
		rafm_info->iir_eb << 7);

	val = ((rafm_info->iir_g1 & 0xFFF) << 16) |
		(rafm_info->iir_g0 & 0xFFF);
	DCAM_REG_WR(idx, ISP_RAW_AFM_IIR_FILTER0, val);

	for (i = 0; i < 10; i += 2) {
		val = ((rafm_info->iir_c[i+1] & 0xFFF) << 16) |
			(rafm_info->iir_c[i] & 0xFFF);
		DCAM_REG_WR(idx, ISP_RAW_AFM_IIR_FILTER1 + 4 * (i>>1), val);
	}
	...
	struct isp_dev_rgb_afm_info *rafm_info)
{
	...
	DCAM_REG_WR(idx, ISP_RAW_AFM_ENHANCE_CTRL, val);

	val = ((rafm_info->fv0_th.max & 0xFFFFF) << 12) |
		(rafm_info->fv0_th.min & 0xFFF);
	DCAM_REG_WR(idx, ISP_RAW_AFM_ENHANCE_FV0_THD, val);

	val = ((rafm_info->fv1_th.max & 0xFFFFF) << 12) |
		(rafm_info->fv1_th.min & 0xFFF);
	DCAM_REG_WR(idx, ISP_RAW_AFM_ENHANCE_FV1_THD, val);
	...
	for (i = 0; i < 4; i++) {
		...
		DCAM_REG_WR(idx, ISP_RAW_AFM_ENHANCE_FV1_COEFF00 + 8 * i, val);
		...
		DCAM_REG_WR(idx, ISP_RAW_AFM_ENHANCE_FV1_COEFF01 + 8 * i, val);
	}
...
int isp_k_raw_afm_block(struct isp_io_param *param, enum isp_id idx)
{
	...
	DCAM_REG_MWR(idx, ISP_RAW_AFM_FRAM_CTRL, BIT_0, rafm_info.bypass);
	if (rafm_info.bypass) {
		ret = 0;
		goto exit;
	}

	val = (rafm_info.source_sel & 0x1) << 16 |
		(rafm_info.lum_stat_chn_sel & 0x3) << 8 |
		(rafm_info.clk_gate_dis & 0x1);
	DCAM_REG_WR(idx, ISP_RAW_AFM_PARAMETERS, val);
...
static int isp_k_raw_afm_bypass(struct isp_io_param *param, enum isp_id idx)
{
	...
	DCAM_REG_MWR(idx, ISP_RAW_AFM_FRAM_CTRL, BIT_0, bypass);
...
static int32_t isp_k_raw_afm_win(struct isp_io_param *param, enum isp_id idx)
{
	...
	DCAM_REG_WR(idx, ISP_RAW_AFM_WIN_RANGE0S,
			(win.y & 0x1FFF) << 16 | (win.x & 0x1FFF));

	DCAM_REG_WR(idx, ISP_RAW_AFM_WIN_RANGE0E,
...
static int isp_k_raw_afm_win_num(struct isp_io_param *param, enum isp_id idx)
{
	...
	DCAM_REG_WR(idx, ISP_RAW_AFM_WIN_RANGE1S,
...
static int isp_k_raw_afm_mode(struct isp_io_param *param, enum isp_id idx)
{
	...
	DCAM_REG_MWR(idx, ISP_RAW_AFM_FRAM_CTRL, BIT_2,
		mode << 2);

	if (mode)
		DCAM_REG_MWR(idx, ISP_RAW_AFM_FRAM_CTRL, BIT_3,
			(0x1 << 3));
	else
		DCAM_REG_MWR(idx, ISP_RAW_AFM_FRAM_CTRL1, BIT_0, 0x1);
...
static int isp_k_raw_afm_skip_num(struct isp_io_param *param, enum isp_id idx)
{
	...
	DCAM_REG_MWR(idx, ISP_RAW_AFM_FRAM_CTRL, 0xF0, (skip_num & 0xF) << 4);
							...
							enum isp_id idx)
{
	...
	DCAM_REG_MWR(idx, ISP_RAW_AFM_FRAM_CTRL1, BIT_1, is_clear << 1);
	...
	enum isp_id idx)
{
	...
	DCAM_REG_MWR(idx, ISP_RAW_AFM_PARAMETERS, BIT_17,
		crop_eb << 17);
	...
	enum isp_id idx)
{
	...
	DCAM_REG_WR(idx, ISP_RAW_AFM_CROP_START,
		(crop_size.y & 0x1FFF) << 16 |
		(crop_size.x & 0X1FFF));
	DCAM_REG_WR(idx, ISP_RAW_AFM_CROP_SIZE,
	...
	enum isp_id idx)
{
	...
	val = (done_tile_num.x & 0x1F) << 4 |
		(done_tile_num.y & 0xF);
	DCAM_REG_WR(idx, ISP_RAW_AFM_DONE_TILE_NUM, val);
