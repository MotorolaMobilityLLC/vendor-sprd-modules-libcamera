		...
		struct isp_k_block *isp_k_param, uint32_t idx)
{
	...
	for (i = 0; i < 3; i++) {
		...
		val = (nlm_info.nlm_first_lum_direction_addback_noise_clip[i][j]
			& 0x3FF) |
			((nlm_info.nlm_first_lum_direction_addback[i][j]
			& 0x7F) << 10) |
		((nlm_info.nlm_radial_1D_radius_threshold_filter_ratio[i][j]
				& 0x7FFF) << 17);
		ISP_REG_WR(idx,
