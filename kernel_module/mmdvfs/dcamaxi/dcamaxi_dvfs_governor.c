		...
		struct device_attribute *attr, char *buf)
{
	...
	if (dcamaxi != NULL)
		err = sprintf(buf, "%d\n",
		dcamaxi->dcamaxi_dvfs_para.u_dvfs_en);
	else
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.u_dvfs_en = dvfs_en;
	dcamaxi->dvfs_enable = dvfs_en;
	if (dcamaxi->dvfs_ops != NULL &&
		dcamaxi->dvfs_ops->ip_hw_dvfs_en != NULL && (err != 0)) {

		dcamaxi->dvfs_ops->ip_hw_dvfs_en(devfreq,
		dcamaxi->dcamaxi_dvfs_para.u_dvfs_en);
	} else
						...
						buf)
{
	...
	if (dcamaxi != NULL)
		err = sprintf(buf, "%d\n",
		dcamaxi->dcamaxi_dvfs_para.u_auto_tune_en);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.u_auto_tune_en = auto_tune_en;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->ip_auto_tune_en != NULL) {

		dcamaxi->dvfs_ops->ip_auto_tune_en(devfreq,
		dcamaxi->dcamaxi_dvfs_para.u_auto_tune_en);
	} else
		pr_info("%s: ip  ops null\n", __func__);
		...
		char *buf)
{
	...
	if (dcamaxi != NULL)
		err = sprintf(buf, "%d\n",
		dcamaxi->dcamaxi_dvfs_para.u_work_freq);
	else
		err = sprintf(buf, "undefined\n");
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.u_work_freq = user_freq;
			...
			char *buf)
{
	...
	if (dcamaxi != NULL)
		err = sprintf(buf, "%d\n",
		dcamaxi->dcamaxi_dvfs_para.u_idle_freq);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.u_idle_freq = idle_freq;
		...
		struct device_attribute *attr,  char *buf)
{
	...
	if (dcamaxi != NULL)
		err = sprintf(buf, "%d\n",
		dcamaxi->dcamaxi_dvfs_para.u_work_index);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
						...
						char *buf, size_t count)
{
	...
	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_ip_dvfs_work_index != NULL
		&& (err != 0)) {

		dcamaxi->dcamaxi_dvfs_para.u_work_index = work_index;
		dcamaxi->dvfs_ops->set_ip_dvfs_work_index(devfreq,
		dcamaxi->dcamaxi_dvfs_para.u_work_index);
	} else
		pr_info("%s: ip  ops null\n", __func__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (dcamaxi != NULL)
		err = sprintf(buf, "%d\n",
		dcamaxi->dcamaxi_dvfs_para.u_idle_index);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.u_idle_index = idle_index;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_ip_dvfs_idle_index
		!= NULL && (err != 0)) {

		dcamaxi->dvfs_ops->set_ip_dvfs_idle_index(devfreq,
		dcamaxi->dcamaxi_dvfs_para.u_idle_index);
	} else
		pr_info("%s: ip  ops null\n", __func__);
		...
		char *buf)
{
	...
	if (dcamaxi != NULL)
		err = sprintf(buf, "%d\n",
		dcamaxi->dcamaxi_dvfs_para.u_fix_volt);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
					...
					const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.u_fix_volt = fix_volt;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_fix_dvfs_value != NULL) {

		dcamaxi->dvfs_ops->set_fix_dvfs_value(devfreq,
		dcamaxi->dcamaxi_dvfs_para.u_fix_volt);
	} else
		pr_info("%s: ip  ops null\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.ip_coffe = dvfs_coffe;
	if (err != 8) {
		mutex_unlock(&devfreq->lock);
		return -EINVAL;
	}
	...
	if (dcamaxi->dvfs_ops  != NULL &&
		dcamaxi->dvfs_ops->set_ip_dvfs_coffe != NULL) {
		err = dcamaxi->dvfs_ops->set_ip_dvfs_coffe(devfreq,
			&dvfs_coffe);
	} else
		pr_info("%s: ip  ops null\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->get_ip_status != NULL) {
		ret = dcamaxi->dvfs_ops->get_ip_status(devfreq, &ip_status);
	} else
		pr_info("%s: dvfs_read_ops is null\n", __func__);

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->top_current_volt != NULL) {
		ret = dcamaxi->dvfs_ops->top_current_volt(devfreq, &top_volt);
	} else
		pr_info("%s: dvfs_read_top_volt is null\n", __func__);

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->mm_current_volt != NULL) {
		ret = dcamaxi->dvfs_ops->mm_current_volt(devfreq, &mm_volt);
	} else
		pr_info("%s: dvfs_read_top_volt is null\n", __func__);

	len = sprintf(buf, "dcamaxi_dvfs_read_clk=%d\n",
		ip_status.current_ip_clk);
	len += sprintf(buf+len, "dcamaxi_dvfs_read_volt=%d\n",
		ip_status.current_sys_volt);
	len += sprintf(buf+len, "dcamaxi_dvfs_read_top_volt=%d\n", top_volt);
	len += sprintf(buf+len, "dcamaxi_dvfs_read_mm_volt=%d\n", mm_volt);
	...
	struct device_attribute *attr,   char *buf)
{
	...
	if (dcamaxi->dvfs_ops  !=  NULL &&   dcamaxi->dvfs_ops->get_ip_dvfs_table !=
	NULL) {

		err = dcamaxi->dvfs_ops->get_ip_dvfs_table(devfreq, dvfs_table);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.ip_dvfs_map[map_index] = dvfs_table;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_ip_dvfs_table != NULL) {

		err = dcamaxi->dvfs_ops->set_ip_dvfs_table(devfreq,
			&dvfs_table);
	} else
		pr_info("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.ip_coffe.gfree_wait_delay = gfree_wait_delay;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_ip_gfree_wait_delay != NULL) {

		dcamaxi->dvfs_ops->set_ip_gfree_wait_delay(
		dcamaxi->dcamaxi_dvfs_para.ip_coffe.gfree_wait_delay);
	} else
		pr_info("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,    const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.ip_coffe.freq_upd_hdsk_en = freq_upd_hdsk_en;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_ip_freq_upd_hdsk_en != NULL) {

		dcamaxi->dvfs_ops->set_ip_freq_upd_hdsk_en(
		dcamaxi->dcamaxi_dvfs_para.ip_coffe.freq_upd_hdsk_en);
	} else
		pr_info("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	if (dcamaxi->dvfs_ops  !=  NULL
		&&  dcamaxi->dvfs_ops->set_ip_freq_upd_delay_en != NULL) {

		dcamaxi->dvfs_ops->set_ip_freq_upd_delay_en(
		dcamaxi->dcamaxi_dvfs_para.ip_coffe.freq_upd_delay_en);
	} else
		pr_info("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.ip_coffe.freq_upd_en_byp = freq_upd_en_byp;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_ip_freq_upd_en_byp != NULL) {

		dcamaxi->dvfs_ops->set_ip_freq_upd_en_byp(
		dcamaxi->dcamaxi_dvfs_para.ip_coffe.freq_upd_en_byp);
	} else
		pr_info("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dcamaxi->dcamaxi_dvfs_para.ip_coffe.freq_upd_en_byp = sw_trig_en;

	if (dcamaxi->dvfs_ops  !=  NULL &&
		dcamaxi->dvfs_ops->set_ip_dvfs_swtrig_en != NULL)
		dcamaxi->dvfs_ops->set_ip_dvfs_swtrig_en(sw_trig_en);
	else
		pr_info("%s: ip ops null\n", __func__);
...
static int userspace_init(struct devfreq *devfreq)
{
	...
	struct dcamaxi_dvfs *dcamaxi = dev_get_drvdata(devfreq->dev.parent);

	dcamaxi->dvfs_ops = get_ip_dvfs_ops("DCAMAXI_DVFS_OPS");
...
static int dcamaxi_dvfs_gov_get_target(struct devfreq *devfreq,
			unsigned long *freq)
{
	struct dcamaxi_dvfs *dcamaxi = dev_get_drvdata(devfreq->dev.parent);
	...
	if (dcamaxi->dvfs_enable) {
		unsigned long adjusted_freq = dcamaxi->user_freq;


		if (dcamaxi->user_freq_type == DVFS_WORK)
			adjusted_freq = dcamaxi->dcamaxi_dvfs_para.u_work_freq;
		else
			adjusted_freq = dcamaxi->dcamaxi_dvfs_para.u_idle_freq;
	...
	*freq = adjusted_freq;
	...
	static int dcamaxi_dvfs_gov_event_handler(struct devfreq *devfreq,
...
struct devfreq_governor dcamaxi_dvfs_gov = {
	.name = "dcam_axi_dvfs",
	.get_target_freq = dcamaxi_dvfs_gov_get_target,
	.event_handler = dcamaxi_dvfs_gov_event_handler,
};
