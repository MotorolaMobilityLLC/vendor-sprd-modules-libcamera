...
{
	{	0, REG_MM_DVFS_AHB_DCAM_AXI_INDEX0_MAP,	VOLT70,   DCAMAXI_CLK1920,	DCAMAXI_CLK_INDEX_1920, 0, 0, 0,	0	},
	{	1, REG_MM_DVFS_AHB_DCAM_AXI_INDEX1_MAP,	VOLT70,   DCAMAXI_CLK2560,	DCAMAXI_CLK_INDEX_2560, 0, 0, 0,	0	},
	{	2, REG_MM_DVFS_AHB_DCAM_AXI_INDEX2_MAP,	VOLT70,   DCAMAXI_CLK3072,	DCAMAXI_CLK_INDEX_3072, 0, 0, 0,	0	},
	{	3, REG_MM_DVFS_AHB_DCAM_AXI_INDEX3_MAP,	VOLT75,   DCAMAXI_CLK3840,	DCAMAXI_CLK_INDEX_3840, 0, 0, 0,	0	},
	{	4, REG_MM_DVFS_AHB_DCAM_AXI_INDEX4_MAP,	VOLT75,   DCAMAXI_CLK3840,	DCAMAXI_CLK_INDEX_3840, 0, 0, 0,	0	},
	{	5, REG_MM_DVFS_AHB_DCAM_AXI_INDEX5_MAP,	VOLT80,   DCAMAXI_CLK3840,	DCAMAXI_CLK_INDEX_3840, 0, 0, 0,	0	},
	{	6, REG_MM_DVFS_AHB_DCAM_AXI_INDEX6_MAP,	VOLT80,   DCAMAXI_CLK3840,	DCAMAXI_CLK_INDEX_3840, 0, 0, 0,	0	},
	{	7, REG_MM_DVFS_AHB_DCAM_AXI_INDEX7_MAP,	VOLT80,   DCAMAXI_CLK3840,	DCAMAXI_CLK_INDEX_3840, 0, 0, 0,	0	},
...
static int  set_ip_dvfs_table(struct devfreq *devfreq,
		struct ip_dvfs_map_cfg  *dvfs_table)
{
	memcpy(dcamaxi_dvfs_config_table, dvfs_table, sizeof(struct
	ip_dvfs_map_cfg));
	pr_info("dvfs ops: %s\n", __func__);
	return 1;
}
...
static int  set_work_freq(struct devfreq *devfreq, unsigned long work_freq)
{
	...
	get_ip_index_from_table(dcamaxi_dvfs_config_table,
		work_freq, &index);
	...
	ip_status)
{
	...
	clk_reg = DVFS_REG_RD(REG_MM_DVFS_AHB_MM_DVFS_CGM_CFG_DBG);
		...
		ip_status->current_ip_clk = (clk_reg >>
			SHFT_BITS_CGM_DCAM_AXI_SEL_DVFS) &
			MASK_BITS_CGM_DCAM_AXI_SEL_DVFS;

		ip_status->current_sys_volt = ((volt_reg >>
			SHFT_BITS_DCAM_AXI_VOLTAGE)
					& MASK_BITS_DCAM_AXI_VOLTAGE);
	...
	index)
{
	...
	index_cfg_reg = (index_cfg_reg & (~0x7)) | index;
	DVFS_REG_WR(REG_MM_DVFS_AHB_DCAM_AXI_DVFS_INDEX_IDLE_CFG,
		index_cfg_reg);
...
static void dcamaxi_dvfs_map_cfg(void)
{
	...
	for (i = 0; i < 8; i++) {
		...
		DVFS_REG_WR((dcamaxi_dvfs_config_table[i].reg_add),
			map_cfg_reg);
	}
}
...
static int  ip_dvfs_init(struct devfreq *devfreq)
{
	...
	pr_info("dvfs ops: no  no dcamaxi %s\n", __func__);
	...
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_FREQ_UPD_EN_BYP);
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_FREQ_UPD_DELAY_EN);
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_FREQ_UPD_HDSK_EN);
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_GFREE_WAIT_DELAY);
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_SW_TRIG_EN);
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_WORK_INDEX_DEF);
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_IDLE_INDEX_DEF);
	pr_info("dvfs : dcamaxi %d\n", DCAM_AXI_AUTO_TUNE);
	...
	ip_hw_dvfs_en(devfreq, DCAM_AXI_AUTO_TUNE);
...
struct ip_dvfs_ops  dcamaxi_dvfs_ops  =  {

	.name = "DCAMAXI_DVFS_OPS",
	.available = 1,
