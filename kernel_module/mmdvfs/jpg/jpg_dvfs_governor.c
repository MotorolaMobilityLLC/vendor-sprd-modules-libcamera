	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	DvfsQ.pid = pid;
	MatchPid_RemoveInvalidNode(&jpg->head, DvfsQ);
			...
			unsigned long *freq)
{
	...
	if (jpg->dvfs_enable) {
		unsigned long adjusted_freq = jpg->user_freq;

		ShowQueue(&jpg->head);
		if (!IsEmpty(&jpg->head)) {
			adjusted_freq = GetFrontNodeFreq(&jpg->head);
			jpg->user_freq = adjusted_freq; /*  */
			pr_info("dvfs GetFrontNodeFreq %lu", adjusted_freq);
		} else {
			jpg->user_freq = devfreq->min_freq; /* TODO */
			adjusted_freq = devfreq->min_freq;
		}
	pr_info("dvfs jpg->user_freq = jpg->head->next.DvfsQ.freq %lu\n",
	jpg->user_freq);

		if (jpg->user_freq_type == DVFS_WORK)
