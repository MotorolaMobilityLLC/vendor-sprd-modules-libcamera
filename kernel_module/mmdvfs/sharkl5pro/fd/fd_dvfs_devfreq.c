	...
	u32 flags)
{
	...
	DVFS_TRACE("devfreq_dev_profile-->target,freq=%lu\n", *freq);
	mutex_lock(&fd->lock);
	if (fd != NULL && fd->dvfs_ops != NULL &&
		fd->dvfs_ops->update_target_freq != NULL){
		fd->dvfs_ops->update_target_freq(
			fd->devfreq, 0,
...
static int fd_dvfs_get_cur_freq(struct device *dev, unsigned long *freq)
{
	...
	if (fd->dvfs_ops != NULL && fd->dvfs_ops->get_ip_work_or_idle != NULL)
		fd->dvfs_ops->get_ip_work_or_idle(&is_work);

	if (is_work == DVFS_WORK)
		*freq = fd->fd_dvfs_para.u_work_freq;
	else
		*freq = fd->fd_dvfs_para.u_idle_freq;
	...
	unsigned long type, void *data)
{
	...
	case MMSYS_POWER_OFF:
		if (fd->dvfs_ops != NULL && fd->dvfs_ops->power_off_nb != NULL)
...
static int fd_dvfs_probe(struct platform_device *pdev)
{
	...
	DVFS_TRACE("fd-dvfs initialized\n");
