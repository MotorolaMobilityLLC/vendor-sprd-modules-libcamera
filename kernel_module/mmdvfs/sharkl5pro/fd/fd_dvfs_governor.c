	...
	struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	if (fd->dvfs_ops != NULL &&
		fd->dvfs_ops->ip_auto_tune_en != NULL) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	DVFS_TRACE("Work:%d ,idle:%d\n",
		fd->fd_dvfs_para.u_work_freq, fd->fd_dvfs_para.u_idle_freq);
						...
						char *buf, size_t count)
{
	...
	if (fd->dvfs_ops  !=  NULL &&
		...
		fd->fd_dvfs_para.u_work_freq = dvfs_table[work_index].clk_freq;
		DVFS_TRACE("u_work_freq:%d\n", fd->fd_dvfs_para.u_work_freq);
	} else {
		DVFS_TRACE("%s: ip ops null\n", __func__);
		mutex_unlock(&devfreq->lock);
		return err;
	}
	DVFS_TRACE("Update fd cur frequence1\n");
	if (fd->dvfs_ops  !=  NULL &&
		fd->dvfs_ops->get_ip_work_or_idle != NULL)
		fd->dvfs_ops->get_ip_work_or_idle(&is_work);
	if (is_work)
		fd->freq = fd->fd_dvfs_para.u_work_freq;
	else
		fd->freq = fd->fd_dvfs_para.u_idle_freq;
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (fd->dvfs_ops  !=  NULL &&
		...
		fd->dvfs_ops->set_ip_dvfs_idle_index(devfreq,
			fd->fd_dvfs_para.u_idle_index);
		err = fd->dvfs_ops->get_ip_dvfs_table(devfreq, dvfs_table);
		fd->fd_dvfs_para.u_idle_freq = dvfs_table[idle_index].clk_freq;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (fd != NULL) {
		err = sprintf(buf, "%d\n",
		fd->fd_dvfs_para.ip_coffe.idle_switch_en);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fd->fd_dvfs_para.ip_coffe.idle_switch_en = idle_switch_en;
	if (fd->dvfs_ops  !=  NULL && fd->dvfs_ops->set_ip_dfs_idle_disable !=
		NULL)
		fd->dvfs_ops->set_ip_dfs_idle_disable(idle_switch_en);
	else
		pr_err("%s: ip ops null\n", __func__);
...
static int fd_dvfs_gov_get_target(struct devfreq *devfreq, unsigned long *freq)
{
	struct fd_dvfs *fd = dev_get_drvdata(devfreq->dev.parent);
	unsigned int is_work;
