	...
	u32 flags)
{
	...
	DVFS_TRACE("devfreq_dev_profile-->target,freq=%lu jpg->freq = %lu\n",
		*freq, jpg->freq);
	mutex_lock(&jpg->lock);
	if (jpg != NULL && jpg->dvfs_ops != NULL &&
		jpg->dvfs_ops->update_target_freq != NULL){
		jpg->dvfs_ops->update_target_freq(
			jpg->devfreq, 0,
...
static int jpg_dvfs_get_cur_freq(struct device *dev, unsigned long *freq)
{
	...
	if (jpg->dvfs_ops != NULL && jpg->dvfs_ops->get_ip_work_or_idle != NULL)
		jpg->dvfs_ops->get_ip_work_or_idle(&is_work);

	if (is_work == DVFS_WORK)
		*freq = jpg->jpg_dvfs_para.u_work_freq;
	else
		*freq = jpg->jpg_dvfs_para.u_idle_freq;
...
static int jpg_dvfs_probe(struct platform_device *pdev)
{
	...
	DVFS_TRACE("jpg-dvfs initialized\n");
