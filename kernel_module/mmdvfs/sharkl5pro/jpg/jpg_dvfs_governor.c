		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.u_dvfs_en = dvfs_en;
	//jpg->dvfs_enable = dvfs_en;
	if (jpg->dvfs_ops != NULL && jpg->dvfs_ops->ip_hw_dvfs_en != NULL && (
		err != 0)) {
		jpg->dvfs_ops->ip_hw_dvfs_en(devfreq,
		jpg->jpg_dvfs_para.u_dvfs_en);
	} else
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (jpg != NULL)
		err = sprintf(buf, "%d\n", jpg->jpg_dvfs_para.u_auto_tune_en);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.u_auto_tune_en = auto_tune_en;

	if (jpg->dvfs_ops  !=  NULL &&     jpg->dvfs_ops->ip_auto_tune_en !=
		NULL) {

		jpg->dvfs_ops->ip_auto_tune_en(devfreq,
		jpg->jpg_dvfs_para.u_auto_tune_en);
	} else
		pr_err("%s: ip  ops null\n", __func__);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.u_work_freq = user_freq;
	DVFS_TRACE("Work:%d ,idle:%d\n",
		jpg->jpg_dvfs_para.u_work_freq, jpg->jpg_dvfs_para.u_idle_freq);
		...
		struct device_attribute *attr,  char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (jpg != NULL)
		err = sprintf(buf, "%d\n", jpg->jpg_dvfs_para.u_work_index);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	DVFS_TRACE("%s: work_index= %lu\n", __func__, work_index);

	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->set_ip_dvfs_work_index != NULL && (err != 0)) {

		jpg->jpg_dvfs_para.u_work_index = work_index;
		jpg->dvfs_ops->set_ip_dvfs_work_index(devfreq,
			jpg->jpg_dvfs_para.u_work_index);
		err = jpg->dvfs_ops->get_ip_dvfs_table(devfreq, dvfs_table);
		jpg->jpg_dvfs_para.u_work_freq =
	...
	DVFS_TRACE("Update jpg cur frequence1\n");
	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->get_ip_work_or_idle != NULL)
		jpg->dvfs_ops->get_ip_work_or_idle(&is_work);
	if (is_work)
		jpg->freq = jpg->jpg_dvfs_para.u_work_freq;
	else
		jpg->freq = jpg->jpg_dvfs_para.u_idle_freq;
	mutex_unlock(&devfreq->lock);
	return err;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (jpg != NULL)
		err = sprintf(buf, "%d\n", jpg->jpg_dvfs_para.u_idle_index);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	err = sscanf(buf, "%lu\n", &idle_index);
	if (err != 1 && idle_index >= MAX_DVFS_INDEX) {
		mutex_unlock(&devfreq->lock);
		return -EINVAL;
	}
	jpg->jpg_dvfs_para.u_idle_index = idle_index;

	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->set_ip_dvfs_idle_index  != NULL && (err != 0)) {

		jpg->dvfs_ops->set_ip_dvfs_idle_index(devfreq,
		jpg->jpg_dvfs_para.u_idle_index);
		err = jpg->dvfs_ops->get_ip_dvfs_table(devfreq, dvfs_table);
		jpg->jpg_dvfs_para.u_idle_freq =
					...
					const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.u_fix_volt = fix_volt;

	if (jpg->dvfs_ops  !=  NULL &&   jpg->dvfs_ops->set_fix_dvfs_value !=
		NULL) {

		jpg->dvfs_ops->set_fix_dvfs_value(devfreq,
		jpg->jpg_dvfs_para.u_fix_volt);
	} else
		pr_err("%s: ip  ops null\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	jpg = dev_get_drvdata(devfreq->dev.parent);
	mutex_lock(&devfreq->lock);
	err = sscanf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
	...
	&dvfs_coffe.auto_tune, &dvfs_coffe.work_index_def,
		&dvfs_coffe.idle_index_def, &dvfs_coffe.idle_switch_en);
	jpg->jpg_dvfs_para.ip_coffe = dvfs_coffe;
	if (err != 8) {
		mutex_unlock(&devfreq->lock);
		return -EINVAL;
	}

	if (jpg->dvfs_ops  !=  NULL &&   jpg->dvfs_ops->set_ip_dvfs_coffe !=
		NULL) {
		err = jpg->dvfs_ops->set_ip_dvfs_coffe(devfreq,  &dvfs_coffe);
	} else
		pr_err("%s: ip  ops null\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->get_ip_status != NULL) {
		ret = jpg->dvfs_ops->get_ip_status(devfreq, &ip_status);
	} else {
	...
	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->top_current_volt != NULL) {
		ret = jpg->dvfs_ops->top_current_volt(devfreq, &top_volt);
	} else {
	...
	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->mm_current_volt != NULL) {
		ret = jpg->dvfs_ops->mm_current_volt(devfreq, &mm_volt);
	} else {
		pr_err("%s: dvfs_read_top_volt is null\n", __func__);
		len = sprintf(buf, "get_ip_status_show failed\n");
		goto err_exit;
	}
	len = sprintf(buf, "jpg_dvfs_read_clk=%d\n", ip_status.current_ip_clk);
	len += sprintf(buf+len, "jpg_dvfs_read_volt=%d\n",
		ip_status.current_sys_volt);
	len += sprintf(buf+len, "jpg_dvfs_read_top_volt=%d\n", top_volt);
	len += sprintf(buf+len, "jpg_dvfs_read_mm_volt=%d\n", mm_volt);
	...
	struct device_attribute *attr,   char *buf)
{
	...
	if (jpg->dvfs_ops  !=  NULL &&   jpg->dvfs_ops->get_ip_dvfs_table !=
	NULL) {
		err = jpg->dvfs_ops->get_ip_dvfs_table(devfreq, dvfs_table);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.ip_dvfs_map[map_index] = dvfs_table;

	if (jpg->dvfs_ops  !=  NULL &&   jpg->dvfs_ops->set_ip_dvfs_table !=
		NULL) {

		err = jpg->dvfs_ops->set_ip_dvfs_table(devfreq,  &dvfs_table);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.ip_coffe.gfree_wait_delay = gfree_wait_delay;

	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->set_ip_gfree_wait_delay != NULL) {

		jpg->dvfs_ops->set_ip_gfree_wait_delay(
		jpg->jpg_dvfs_para.ip_coffe.gfree_wait_delay);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,    const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.ip_coffe.freq_upd_hdsk_en = freq_upd_hdsk_en;

	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->set_ip_freq_upd_hdsk_en != NULL) {

		jpg->dvfs_ops->set_ip_freq_upd_hdsk_en(
		jpg->jpg_dvfs_para.ip_coffe.freq_upd_hdsk_en);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.ip_coffe.freq_upd_delay_en = freq_upd_delay_en;

	if (jpg->dvfs_ops  !=  NULL
		&&  jpg->dvfs_ops->set_ip_freq_upd_delay_en != NULL) {

		jpg->dvfs_ops->set_ip_freq_upd_delay_en(
		jpg->jpg_dvfs_para.ip_coffe.freq_upd_delay_en);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.ip_coffe.freq_upd_en_byp = freq_upd_en_byp;

	if (jpg->dvfs_ops  !=  NULL &&
		jpg->dvfs_ops->set_ip_freq_upd_en_byp != NULL) {

		jpg->dvfs_ops->set_ip_freq_upd_en_byp(
		jpg->jpg_dvfs_para.ip_coffe.freq_upd_en_byp);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.ip_coffe.freq_upd_en_byp = sw_trig_en;

	if (jpg->dvfs_ops  !=  NULL && jpg->dvfs_ops->set_ip_dvfs_swtrig_en !=
		NULL)
		jpg->dvfs_ops->set_ip_dvfs_swtrig_en(sw_trig_en);
	else
		pr_err("%s: ip ops null\n", __func__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (jpg != NULL) {
		err = sprintf(buf, "%d\n",
		jpg->jpg_dvfs_para.ip_coffe.idle_switch_en);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	return err;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	jpg->jpg_dvfs_para.ip_coffe.idle_switch_en = idle_switch_en;
	if (jpg->dvfs_ops  !=  NULL && jpg->dvfs_ops->set_ip_dfs_idle_disable !=
		NULL)
		jpg->dvfs_ops->set_ip_dfs_idle_disable(idle_switch_en);
	else
		pr_err("%s: ip ops null\n", __func__);
...
static int userspace_init(struct devfreq *devfreq)
{
	...
	struct jpg_dvfs *jpg = dev_get_drvdata(devfreq->dev.parent);

	jpg->dvfs_ops = get_ip_dvfs_ops("JPG_DVFS_OPS");
...
static int jpg_dvfs_gov_get_target(struct devfreq *devfreq,
			unsigned long *freq)
{
	struct jpg_dvfs *jpg = dev_get_drvdata(devfreq->dev.parent);
	unsigned int is_work = 0;
	unsigned long adjusted_freq;
	...
	if (jpg->dvfs_enable) {
		...
		if (is_work == DVFS_WORK)
			adjusted_freq = jpg->jpg_dvfs_para.u_work_freq;
		else
			adjusted_freq = jpg->jpg_dvfs_para.u_idle_freq;
		*freq = adjusted_freq;
	} else
...
static int jpg_dvfs_gov_event_handler(struct devfreq *devfreq,
...
struct devfreq_governor jpg_dvfs_gov = {
	.name = "jpg_dvfs",
	.get_target_freq = jpg_dvfs_gov_get_target,
	.event_handler = jpg_dvfs_gov_event_handler,
};
