		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (dcam->dvfs_ops != NULL && dcam->dvfs_ops->ip_hw_dvfs_en != NULL && (
		err != 0)) {
		dcam->dvfs_ops->ip_hw_dvfs_en(devfreq, dvfs_en);
	} else
						...
						buf)
{
	...
	if (dcam != NULL && dcam->dvfs_ops != NULL &&
		dcam->dvfs_ops->get_ip_auto_tune_en != NULL) {
		dcam->dvfs_ops->get_ip_auto_tune_en(devfreq, &auto_tune_en);
		err = sprintf(buf, "%lu\n", auto_tune_en);
	} else {
		err = sprintf(buf, "undefined\n");
	}
	...
	struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	if (dcam->dvfs_ops  !=  NULL && dcam->dvfs_ops->ip_auto_tune_en !=
		...
		dcam->dvfs_ops->ip_auto_tune_en(devfreq, auto_tune_en);
	} else
		pr_err("%s: ip  ops null\n", __func__);
		...
		struct device_attribute *attr,  char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if ((dcam != NULL) && dcam->dvfs_ops != NULL &&
		dcam->dvfs_ops->get_ip_dvfs_work_index != NULL) {
		dcam->dvfs_ops->get_ip_dvfs_work_index(devfreq, &index);
		err = sprintf(buf, "%d\n", index);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
						...
						char *buf, size_t count)
{
	...
	if (dcam->dvfs_ops  !=  NULL &&
		...
		dcam->dvfs_ops->set_ip_dvfs_work_index(devfreq, work_index);
	} else
		pr_err("%s: ip  ops null\n", __func__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if ((dcam != NULL) && dcam->dvfs_ops != NULL &&
		dcam->dvfs_ops->get_ip_dvfs_idle_index != NULL) {
		dcam->dvfs_ops->get_ip_dvfs_idle_index(devfreq, &index);
		err = sprintf(buf, "%d\n", index);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (dcam->dvfs_ops  !=  NULL &&
		...
		dcam->dvfs_ops->set_ip_dvfs_idle_index(devfreq, idle_index);
	} else
		pr_err("%s: ip  ops null\n", __func__);
	...
	struct device_attribute *attr,   char *buf)
{
	...
	if (dcam->dvfs_ops != NULL &&
		dcam->dvfs_ops->get_ip_dvfs_table != NULL) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (dcam != NULL) {
		err = sprintf(buf, "%d\n",
		dcam->dcam_dvfs_para.ip_coffe.idle_switch_en);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dcam->dcam_dvfs_para.ip_coffe.idle_switch_en = idle_switch_en;
	if (dcam->dvfs_ops != NULL &&
		dcam->dvfs_ops->set_ip_dfs_idle_disable != NULL)
		dcam->dvfs_ops->set_ip_dfs_idle_disable(idle_switch_en);
	else
		pr_err("%s: ip ops null\n", __func__);
