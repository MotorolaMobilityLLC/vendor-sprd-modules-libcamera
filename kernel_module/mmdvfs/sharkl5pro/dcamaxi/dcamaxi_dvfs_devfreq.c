	...
	u32 flags)
{
	...
	if (dcamaxi != NULL && dcamaxi->dvfs_ops != NULL &&
		dcamaxi->dvfs_ops->update_target_freq != NULL){
		dcamaxi->dvfs_ops->update_target_freq(dcamaxi->devfreq, 0,
...
static int dcamaxi_dvfs_probe(struct platform_device *pdev)
{
	...
	DVFS_TRACE("dcamaxi-dvfs initialized\n");
