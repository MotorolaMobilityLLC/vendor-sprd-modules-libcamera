		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.u_dvfs_en = dvfs_en;
	//isp->dvfs_enable = dvfs_en;
	if (isp->dvfs_ops != NULL && isp->dvfs_ops->ip_hw_dvfs_en != NULL && (
		err != 0)) {
		isp->dvfs_ops->ip_hw_dvfs_en(devfreq,
			isp->isp_dvfs_para.u_dvfs_en);
	} else
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (isp != NULL)
		err = sprintf(buf, "%d\n", isp->isp_dvfs_para.u_auto_tune_en);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.u_auto_tune_en = auto_tune_en;

	if (isp->dvfs_ops  !=  NULL &&     isp->dvfs_ops->ip_auto_tune_en !=
		NULL) {

		isp->dvfs_ops->ip_auto_tune_en(devfreq,
		isp->isp_dvfs_para.u_auto_tune_en);
	} else
		pr_err("%s: ip  ops null\n", __func__);
	err = count;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.u_work_freq = user_freq;
	DVFS_TRACE("Work:%d ,idle:%d\n",
		isp->isp_dvfs_para.u_work_freq, isp->isp_dvfs_para.u_idle_freq);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.u_idle_freq = idle_freq;
	err = update_devfreq(devfreq);
	if (err == 0)
		err = count;
	mutex_unlock(&devfreq->lock);
		...
		struct device_attribute *attr,  char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (isp != NULL)
		err = sprintf(buf, "%d\n", isp->isp_dvfs_para.u_work_index);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (isp->dvfs_ops != NULL &&
		isp->dvfs_ops->set_ip_dvfs_work_index != NULL && (err != 0)) {
		isp->isp_dvfs_para.u_work_index = work_index;
		isp->dvfs_ops->set_ip_dvfs_work_index(devfreq,
			isp->isp_dvfs_para.u_work_index);
		err = isp->dvfs_ops->get_ip_dvfs_table(devfreq, dvfs_table);
		isp->isp_dvfs_para.u_work_freq =
	...
	DVFS_TRACE("Update isp cur frequence1\n");
	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->get_ip_work_or_idle != NULL)
		isp->dvfs_ops->get_ip_work_or_idle(&is_work);
	if (is_work)
		isp->freq = isp->isp_dvfs_para.u_work_freq;
	else
		isp->freq = isp->isp_dvfs_para.u_idle_freq;
	mutex_unlock(&devfreq->lock);
	return err;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (isp != NULL)
		err = sprintf(buf, "%d\n", isp->isp_dvfs_para.u_idle_index);
	else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.u_idle_index = idle_index;

	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->set_ip_dvfs_idle_index  != NULL && (err != 0)) {
		isp->dvfs_ops->set_ip_dvfs_idle_index(devfreq,
			isp->isp_dvfs_para.u_idle_index);
		err = isp->dvfs_ops->get_ip_dvfs_table(devfreq, dvfs_table);
		isp->isp_dvfs_para.u_idle_freq =
					...
					const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.u_fix_volt = fix_volt;

	if (isp->dvfs_ops  !=  NULL &&   isp->dvfs_ops->set_fix_dvfs_value !=
		NULL) {

		isp->dvfs_ops->set_fix_dvfs_value(devfreq,
		isp->isp_dvfs_para.u_fix_volt);
	} else
		pr_err("%s: ip  ops null\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_coffe = dvfs_coffe;
	if (err != 8) {
		mutex_unlock(&devfreq->lock);
		return -EINVAL;
	}

	if (isp->dvfs_ops  !=  NULL &&   isp->dvfs_ops->set_ip_dvfs_coffe !=
		NULL) {
		err = isp->dvfs_ops->set_ip_dvfs_coffe(devfreq,  &dvfs_coffe);
	} else
		pr_err("%s: ip  ops null\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->get_ip_status != NULL) {
		ret = isp->dvfs_ops->get_ip_status(devfreq, &ip_status);
	} else {
	...
	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->top_current_volt != NULL) {
		ret = isp->dvfs_ops->top_current_volt(devfreq, &top_volt);
	} else {
	...
	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->mm_current_volt != NULL) {
		ret = isp->dvfs_ops->mm_current_volt(devfreq, &mm_volt);
	} else {
		pr_err("%s: dvfs_read_top_volt is null\n", __func__);
		len = sprintf(buf, "get_ip_status_show failed\n");
		goto err_exit;
	}
	len = sprintf(buf, "isp_dvfs_read_clk=%d\n", ip_status.current_ip_clk);
	len += sprintf(buf+len, "isp_dvfs_read_volt=%d\n",
		ip_status.current_sys_volt);
	len += sprintf(buf+len, "isp_dvfs_read_top_volt=%d\n", top_volt);
	len += sprintf(buf+len, "isp_dvfs_read_mm_volt=%d\n", mm_volt);
	...
	struct device_attribute *attr,   char *buf)
{
	...
	if (isp->dvfs_ops  !=  NULL &&   isp->dvfs_ops->get_ip_dvfs_table !=
	NULL) {
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_dvfs_map[map_index] = dvfs_table;

	if (isp->dvfs_ops  !=  NULL &&   isp->dvfs_ops->set_ip_dvfs_table !=
		NULL) {

		err = isp->dvfs_ops->set_ip_dvfs_table(devfreq,  &dvfs_table);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_coffe.gfree_wait_delay = gfree_wait_delay;

	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->set_ip_gfree_wait_delay != NULL) {

		isp->dvfs_ops->set_ip_gfree_wait_delay(
		isp->isp_dvfs_para.ip_coffe.gfree_wait_delay);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,    const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_coffe.freq_upd_hdsk_en = freq_upd_hdsk_en;

	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->set_ip_freq_upd_hdsk_en != NULL) {

		isp->dvfs_ops->set_ip_freq_upd_hdsk_en(
		isp->isp_dvfs_para.ip_coffe.freq_upd_hdsk_en);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,   const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_coffe.freq_upd_delay_en = freq_upd_delay_en;

	if (isp->dvfs_ops  !=  NULL
		&&  isp->dvfs_ops->set_ip_freq_upd_delay_en != NULL) {

		isp->dvfs_ops->set_ip_freq_upd_delay_en(
		isp->isp_dvfs_para.ip_coffe.freq_upd_delay_en);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_coffe.freq_upd_en_byp = freq_upd_en_byp;

	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->set_ip_freq_upd_en_byp != NULL) {

		isp->dvfs_ops->set_ip_freq_upd_en_byp(
		isp->isp_dvfs_para.ip_coffe.freq_upd_en_byp);
	} else
		pr_err("%s: ip ops null\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_coffe.freq_upd_en_byp = sw_trig_en;

	if (isp->dvfs_ops  !=  NULL && isp->dvfs_ops->set_ip_dvfs_swtrig_en !=
		NULL)
		isp->dvfs_ops->set_ip_dvfs_swtrig_en(sw_trig_en);
	else
		pr_err("%s: ip ops null\n", __func__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (isp != NULL) {
		err = sprintf(buf, "%d\n",
		isp->isp_dvfs_para.ip_coffe.idle_switch_en);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	isp->isp_dvfs_para.ip_coffe.idle_switch_en = idle_switch_en;
	if (isp->dvfs_ops  !=  NULL && isp->dvfs_ops->set_ip_dfs_idle_disable !=
		NULL)
		isp->dvfs_ops->set_ip_dfs_idle_disable(idle_switch_en);
	else
		pr_err("%s: ip ops null\n", __func__);
...
static int userspace_init(struct devfreq *devfreq)
{
	...
	struct isp_dvfs *isp = dev_get_drvdata(devfreq->dev.parent);

	isp->dvfs_ops = get_ip_dvfs_ops("ISP_DVFS_OPS");
...
static int isp_dvfs_gov_get_target(struct devfreq *devfreq,
			unsigned long *freq)
{
	struct isp_dvfs *isp = dev_get_drvdata(devfreq->dev.parent);
	unsigned int is_work;
	...
	if (isp->dvfs_enable) {
		...
		if (is_work == DVFS_WORK)
			adjusted_freq = isp->isp_dvfs_para.u_work_freq;
		else
			adjusted_freq = isp->isp_dvfs_para.u_idle_freq;
		*freq = adjusted_freq;
	} else
...
static int isp_dvfs_gov_event_handler(struct devfreq *devfreq,
...
struct devfreq_governor isp_dvfs_gov = {
	.name = "isp_dvfs",
	.get_target_freq = isp_dvfs_gov_get_target,
	.event_handler = isp_dvfs_gov_event_handler,
};
