		...
		struct ip_dvfs_map_cfg *dvfs_table)
{
	...
	for (i = 0; i < MAX_DVFS_INDEX; i++) {
		if (cpp_dvfs_config_table[i].reg_add == dvfs_table->reg_add) {
			memcpy(&cpp_dvfs_config_table[i],
...
static int  set_idle_freq(struct devfreq *devfreq, unsigned long  idle_freq)
{
	...
	get_ip_index_from_table(cpp_dvfs_config_table,  idle_freq, &
		index);
	//Add lock here.
	...
	ip_status)
{
	...
	ip_status->current_ip_clk = (clk_reg >>
		SHFT_BITS_CGM_CPP_SEL_DVFS) &
		MASK_BITS_CGM_CPP_SEL_DVFS;

	ip_status->current_sys_volt = ((volt_reg >>
		SHFT_BITS_CPP_VOLTAGE)
		& MASK_BITS_CPP_VOLTAGE);
...
static void  set_ip_dfs_idle_disable(unsigned int on)
{
	...
	if (on)
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_IDLE_DISABLE_CFG,
			reg_temp | BIT_CPP_DFS_IDLE_DISABLE);
	else
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_IDLE_DISABLE_CFG,
			reg_temp & (~(BIT_CPP_DFS_IDLE_DISABLE)));
	mutex_unlock(&mmsys_glob_reg_lock);
...
static void get_ip_dfs_idle_disable(unsigned int *on)
{
	...
	*on = (reg_value >> 3) & BIT_CPP_DFS_IDLE_DISABLE;
}
...
static void get_ip_dvfs_swtrig_en(unsigned int *en)
{
	...
	reg_value = DVFS_REG_RD(REG_MM_DVFS_AHB_MM_DFS_SW_TRIG_CFG);
	*en = (reg_value >> 2) & BIT_CPP_DFS_SW_TRIG;
}
	...
	index)
{
	...
	top_mm_dvfs_current_volt(devfreq);
	mm_dvfs_read_current_volt(devfreq);
}
...
static void  get_ip_dvfs_idle_index(struct devfreq *devfreq, unsigned int *
	index)
{
	*index = DVFS_REG_RD(REG_MM_DVFS_AHB_CPP_DVFS_INDEX_IDLE_CFG);
	DVFS_TRACE("dvfs ops: %s, work_index=%d\n", __func__, *index);
}
...
static int ip_dvfs_init(struct devfreq *devfreq)
{
	...
	DVFS_TRACE("dvfs ops: cpp %s\n", __func__);
	cpp = dev_get_drvdata(devfreq->dev.parent);
	if (!cpp) {
		pr_err("undefined cpp_dvfs\n");
		return 1;
	}
	...
	cpp->cpp_dvfs_para.ip_coffe.freq_upd_en_byp = CPP_FREQ_UPD_EN_BYP;
	set_ip_freq_upd_delay_en(CPP_FREQ_UPD_DELAY_EN);
	cpp->cpp_dvfs_para.ip_coffe.freq_upd_delay_en = CPP_FREQ_UPD_DELAY_EN;
	set_ip_freq_upd_hdsk_en(CPP_FREQ_UPD_HDSK_EN);
	cpp->cpp_dvfs_para.ip_coffe.freq_upd_hdsk_en = CPP_FREQ_UPD_HDSK_EN;
	set_ip_gfree_wait_delay(CPP_GFREE_WAIT_DELAY);
	cpp->cpp_dvfs_para.ip_coffe.gfree_wait_delay = CPP_GFREE_WAIT_DELAY;
	set_ip_dvfs_swtrig_en(CPP_SW_TRIG_EN);
	set_ip_dvfs_work_index(devfreq, CPP_WORK_INDEX_DEF);
	cpp->cpp_dvfs_para.ip_coffe.work_index_def = CPP_WORK_INDEX_DEF;
	cpp->cpp_dvfs_para.u_work_index = CPP_WORK_INDEX_DEF;
	cpp->cpp_dvfs_para.u_work_freq =
		cpp_dvfs_config_table[CPP_WORK_INDEX_DEF].clk_freq;
	set_ip_dvfs_idle_index(devfreq, CPP_IDLE_INDEX_DEF);
	cpp->cpp_dvfs_para.ip_coffe.idle_index_def = CPP_IDLE_INDEX_DEF;
	cpp->cpp_dvfs_para.u_idle_index = CPP_IDLE_INDEX_DEF;
	cpp->cpp_dvfs_para.u_idle_freq =
		cpp_dvfs_config_table[CPP_IDLE_INDEX_DEF].clk_freq;
	ip_hw_dvfs_en(devfreq, CPP_AUTO_TUNE);
	cpp->freq = cpp_dvfs_config_table[CPP_IDLE_INDEX_DEF].clk_freq;
	cpp->dvfs_enable = DVFS_ON;
	DVFS_TRACE("CPP FREQ:%lu\n", cpp->freq);
...
static int get_ip_work_or_idle(unsigned int *is_work)
{
	...
	temp_status = (DVFS_REG_RD_AHB(REG_MM_AHB_AHB_EB)
		>> SHIFT_BIT_MM_CPP_EB) & MASK_DVFS_ONE_BIT;
	if (temp_status == 0) {
	...
	temp_dfs_en = (DVFS_REG_RD(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL)
		>> SHIFT_BIT_FD_DFS_EN) & MASK_DVFS_ONE_BIT;
	if (temp_dfs_en == 1) {
		...
		temp_status = (DVFS_REG_RD_AHB(REG_MM_IP_BUSY)
			>> SHIFT_BIT_MM_CPP_BUSY) & (MASK_DVFS_ONE_BIT);
		if (temp_status == 1) {
