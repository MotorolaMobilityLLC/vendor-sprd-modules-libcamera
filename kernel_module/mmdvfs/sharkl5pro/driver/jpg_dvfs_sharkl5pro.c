...
struct ip_dvfs_map_cfg jpg_dvfs_config_table[] = {
	{0, REG_MM_DVFS_AHB_JPG_INDEX0_MAP, VOLT70,
		JPG_CLK768, JPG_CLK_INDEX_768, 0, 0, 0, MTX_CLK_INDEX_768},
	{1, REG_MM_DVFS_AHB_JPG_INDEX1_MAP, VOLT70,
		JPG_CLK1280, JPG_CLK_INDEX_1280, 0, 0, 0, MTX_CLK_INDEX_1280},
	{2, REG_MM_DVFS_AHB_JPG_INDEX2_MAP, VOLT70,
		JPG_CLK2560, JPG_CLK_INDEX_2560, 0, 0, 0, MTX_CLK_INDEX_2560},
	{3, REG_MM_DVFS_AHB_JPG_INDEX3_MAP, VOLT75,
		JPG_CLK3840, JPG_CLK_INDEX_3840, 0, 0, 0, MTX_CLK_INDEX_2560},
	{4, REG_MM_DVFS_AHB_JPG_INDEX4_MAP, VOLT75,
		JPG_CLK3840, JPG_CLK_INDEX_3840, 0, 0, 0, MTX_CLK_INDEX_3072},
	{5, REG_MM_DVFS_AHB_JPG_INDEX5_MAP, VOLT75,
		JPG_CLK3840, JPG_CLK_INDEX_3840, 0, 0, 0, MTX_CLK_INDEX_3840},
	{6, REG_MM_DVFS_AHB_JPG_INDEX6_MAP, VOLT75,
		JPG_CLK3840, JPG_CLK_INDEX_3840, 0, 0, 0, MTX_CLK_INDEX_4680},
...
static int set_ip_dvfs_table(struct devfreq *devfreq,
		struct ip_dvfs_map_cfg *dvfs_table)
{
	memcpy(jpg_dvfs_config_table, dvfs_table, sizeof(struct
		ip_dvfs_map_cfg));
	DVFS_TRACE("dvfs ops: %s\n", __func__);
	return 1;
}
...
static void get_ip_dvfs_swtrig_en(unsigned int *en)
{
	...
	*en = (reg_temp >> SHIFT_BIT_JPG_DFS_SW_TRIG) && MASK_DVFS_ONE_BIT;
	mutex_unlock(&mmsys_glob_reg_lock);
	DVFS_TRACE("dvfs ops: %s\n", __func__);
}
...
static void get_ip_dfs_idle_disable(unsigned int *on)
{
	...
	*on = (reg_value >> SHIFT_BIT_JPG_DFS_IDLE_DISABLE)
		& BIT_JPG_DFS_IDLE_DISABLE;
}
...
static int get_ip_work_or_idle(unsigned int *is_work)
{
	...
	temp_status = (DVFS_REG_RD_AHB(REG_MM_AHB_AHB_EB)
		>> SHIFT_BIT_MM_JPG_EB) & MASK_DVFS_ONE_BIT;
	if (temp_status == 0) {
	...
	temp_dfs_en = (DVFS_REG_RD(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL)
		>> SHIFT_BIT_JPG_DFS_EN) & MASK_DVFS_ONE_BIT;
	if (temp_dfs_en == 1) {
		...
		temp_status = (DVFS_REG_RD_AHB(REG_MM_IP_BUSY) >>
			SHIFT_BIT_MM_JPG_BUSY) & (MASK_DVFS_ONE_BIT);
		if (temp_status == 1) {
...
static int  ip_dvfs_init(struct devfreq *devfreq)
{
	...
	DVFS_TRACE("dvfs ops: jpg%s\n", __func__);

	jpg = dev_get_drvdata(devfreq->dev.parent);

	if (!jpg) {
		pr_err("undefined jpg_dvfs\n");
		return 1;
	}
	...
	jpg->jpg_dvfs_para.ip_coffe.freq_upd_en_byp = JPEG_FREQ_UPD_EN_BYP;
	set_ip_freq_upd_delay_en(JPEG_FREQ_UPD_DELAY_EN);
	jpg->jpg_dvfs_para.ip_coffe.freq_upd_delay_en = JPEG_FREQ_UPD_DELAY_EN;
	set_ip_freq_upd_hdsk_en(JPEG_FREQ_UPD_HDSK_EN);
	jpg->jpg_dvfs_para.ip_coffe.freq_upd_hdsk_en = JPEG_FREQ_UPD_HDSK_EN;
	set_ip_gfree_wait_delay(JPEG_GFREE_WAIT_DELAY);
	jpg->jpg_dvfs_para.ip_coffe.gfree_wait_delay = JPEG_GFREE_WAIT_DELAY;
	set_ip_dvfs_swtrig_en(JPEG_SW_TRIG_EN);
	set_ip_dvfs_work_index(devfreq, JPEG_WORK_INDEX_DEF);
	jpg->jpg_dvfs_para.ip_coffe.work_index_def = JPEG_WORK_INDEX_DEF;
	jpg->jpg_dvfs_para.u_work_index = JPEG_WORK_INDEX_DEF;
	jpg->jpg_dvfs_para.u_work_freq =
		jpg_dvfs_config_table[JPEG_WORK_INDEX_DEF].clk_freq;
	set_ip_dvfs_idle_index(devfreq, JPEG_IDLE_INDEX_DEF);
	jpg->jpg_dvfs_para.ip_coffe.idle_index_def = JPEG_IDLE_INDEX_DEF;
	jpg->jpg_dvfs_para.u_idle_index = JPEG_IDLE_INDEX_DEF;
	jpg->jpg_dvfs_para.u_idle_freq =
		jpg_dvfs_config_table[JPEG_IDLE_INDEX_DEF].clk_freq;
	ip_hw_dvfs_en(devfreq, JPEG_AUTO_TUNE);
	jpg->freq = jpg_dvfs_config_table[JPEG_IDLE_INDEX_DEF].clk_freq;
	jpg->dvfs_enable = DVFS_ON;
	DVFS_TRACE("JPEG FREQ:%lu\n", jpg->freq);
