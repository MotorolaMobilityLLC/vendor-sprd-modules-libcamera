...
struct ip_dvfs_map_cfg  mtx_dvfs_config_table[] = {
	{0, REG_MM_DVFS_AHB_MM_MTX_INDEX0_MAP, VOLT70,
		MTX_CLK768, MTX_CLK_INDEX_768,   0, 0, 0, 0},
	{1, REG_MM_DVFS_AHB_MM_MTX_INDEX1_MAP, VOLT70,
		MTX_CLK1280, MTX_CLK_INDEX_1280, 0, 0, 0, 0},
	{2, REG_MM_DVFS_AHB_MM_MTX_INDEX2_MAP, VOLT70,
		MTX_CLK2560, MTX_CLK_INDEX_2560, 0, 0, 0, 0},
	{3, REG_MM_DVFS_AHB_MM_MTX_INDEX3_MAP, VOLT70,
		MTX_CLK3072, MTX_CLK_INDEX_3072, 0, 0, 0, 0},
	{4, REG_MM_DVFS_AHB_MM_MTX_INDEX4_MAP, VOLT75,
		MTX_CLK3840, MTX_CLK_INDEX_3840, 0, 0, 0, 0},
	{5, REG_MM_DVFS_AHB_MM_MTX_INDEX5_MAP, VOLT75,
		MTX_CLK4680, MTX_CLK_INDEX_4680, 0, 0, 0, 0},
	{6, REG_MM_DVFS_AHB_MM_MTX_INDEX6_MAP, VOLT80,
		MTX_CLK5120, MTX_CLK_INDEX_5120, 0, 0, 0, 0},
		...
		struct ip_dvfs_map_cfg *dvfs_table)
{
	...
	for (i = 0; i < MAX_DVFS_INDEX; i++) {
		if (mtx_dvfs_config_table[i].reg_add ==
				dvfs_table->reg_add) {
			memcpy(&mtx_dvfs_config_table[i],
...
static void  set_ip_dfs_idle_disable(unsigned int on)
{
	...
	if (on)
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_IDLE_DISABLE_CFG,
			reg_temp | BIT_MTX_DFS_IDLE_DISABLE);
	else
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_IDLE_DISABLE_CFG,
			reg_temp & (~(BIT_MTX_DFS_IDLE_DISABLE)));
	mutex_unlock(&mmsys_glob_reg_lock);
...
static int  ip_dvfs_init(struct devfreq *devfreq)
{
	...
	DVFS_TRACE("dvfs ops: mtx %s\n", __func__);

	mtx = dev_get_drvdata(devfreq->dev.parent);

	if (!mtx) {
		pr_err("undefined mtx_dvfs\n");
		return 1;
	}
