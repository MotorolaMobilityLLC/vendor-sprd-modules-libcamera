...
struct ip_dvfs_map_cfg  fd_dvfs_config_table[] = {
	{0, REG_MM_DVFS_AHB_FD_INDEX0_MAP, VOLT70,
		FD_CLK768, FD_CLK_INDEX_768, 0, 0, 0, MTX_CLK_INDEX_768},
	{1, REG_MM_DVFS_AHB_FD_INDEX1_MAP, VOLT70,
		FD_CLK1920, FD_CLK_INDEX_1920, 0, 0, 0, MTX_CLK_INDEX_1280},
	{2, REG_MM_DVFS_AHB_FD_INDEX2_MAP, VOLT70,
		FD_CLK3072, FD_CLK_INDEX_3072, 0, 0, 0, MTX_CLK_INDEX_2560},
	{3, REG_MM_DVFS_AHB_FD_INDEX3_MAP, VOLT75,
		FD_CLK3840, FD_CLK_INDEX_3840, 0, 0, 0, MTX_CLK_INDEX_3840},
	{4, REG_MM_DVFS_AHB_FD_INDEX4_MAP, VOLT75,
		FD_CLK3840, FD_CLK_INDEX_3840, 0, 0, 0, MTX_CLK_INDEX_4680},
	{5, REG_MM_DVFS_AHB_FD_INDEX5_MAP, VOLT80,
		FD_CLK3840, FD_CLK_INDEX_3840, 0, 0, 0, MTX_CLK_INDEX_5120},
		...
		struct ip_dvfs_map_cfg *dvfs_table)
{
	...
	for (i = 0; i < MAX_DVFS_INDEX; i++) {
		if (fd_dvfs_config_table[i].reg_add ==
				dvfs_table->reg_add) {
			memcpy(&fd_dvfs_config_table[i],
	...
	struct ip_dvfs_status *ip_status)
{
	...
	ip_status->current_sys_volt = ((volt_reg >>
		SHFT_BITS_FD_VOLTAGE) & MASK_BITS_FD_VOLTAGE);
	DVFS_TRACE("dvfs ops: %s\n", __func__);
...
static void get_ip_dvfs_swtrig_en(unsigned int *en)
{
	...
	*en = (reg_temp >> SHIFT_BIT_FD_DFS_SW_TRIG) && MASK_DVFS_ONE_BIT;
	mutex_unlock(&mmsys_glob_reg_lock);
	DVFS_TRACE("dvfs ops: %s\n", __func__);
}
...
static void  get_ip_dvfs_idle_index(struct devfreq *devfreq, unsigned int *
	index)
{
	*index = DVFS_REG_RD(REG_MM_DVFS_AHB_FD_DVFS_INDEX_IDLE_CFG);
	DVFS_TRACE("dvfs ops: %s, work_index=%d\n", __func__, *index);
}
...
static void  set_ip_dfs_idle_disable(unsigned int on)
{
	...
	if (on)
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_IDLE_DISABLE_CFG,
			reg_temp | BIT_FD_DFS_IDLE_DISABLE);
	else
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_IDLE_DISABLE_CFG,
			reg_temp & (~(BIT_FD_DFS_IDLE_DISABLE)));
	mutex_unlock(&mmsys_glob_reg_lock);
...
static void get_ip_dfs_idle_disable(unsigned int *on)
{
	...
	*on = (reg_value >> 0) & BIT_FD_DFS_IDLE_DISABLE;
}
...
static int get_ip_work_or_idle(unsigned int *is_work)
{
	...
	temp_status = (DVFS_REG_RD_AHB(REG_MM_AHB_AHB_EB)
		>> SHIFT_BIT_MM_FD_EB) & MASK_DVFS_ONE_BIT;
	if (temp_status == 0) {
	...
	if (temp_dfs_en == 1) {
		...
		temp_status = (DVFS_REG_RD_AHB(REG_MM_IP_BUSY) >>
			SHIFT_BIT_MM_FD_BUSY) & (MASK_DVFS_ONE_BIT);
		if (temp_status == 1) {
...
static int  ip_dvfs_init(struct devfreq *devfreq)
{
	...
	DVFS_TRACE("dvfs ops: fd %s\n", __func__);
	fd = dev_get_drvdata(devfreq->dev.parent);
	if (!fd) {
		pr_err("undefined fd_dvfs\n");
		return 1;
	}
	fd_dvfs_map_cfg();
	...
	fd->fd_dvfs_para.ip_coffe.freq_upd_en_byp = FD_FREQ_UPD_EN_BYP;
	set_ip_freq_upd_delay_en(FD_FREQ_UPD_DELAY_EN);
	fd->fd_dvfs_para.ip_coffe.freq_upd_delay_en = FD_FREQ_UPD_DELAY_EN;
	set_ip_freq_upd_hdsk_en(FD_FREQ_UPD_HDSK_EN);
	fd->fd_dvfs_para.ip_coffe.freq_upd_hdsk_en = FD_FREQ_UPD_HDSK_EN;
	set_ip_gfree_wait_delay(FD_GFREE_WAIT_DELAY);
	fd->fd_dvfs_para.ip_coffe.gfree_wait_delay = FD_GFREE_WAIT_DELAY;
	set_ip_dvfs_swtrig_en(FD_SW_TRIG_EN);
	set_ip_dvfs_work_index(devfreq, FD_WORK_INDEX_DEF);
	fd->fd_dvfs_para.ip_coffe.work_index_def = FD_WORK_INDEX_DEF;
	fd->fd_dvfs_para.u_work_index = FD_WORK_INDEX_DEF;
	fd->fd_dvfs_para.u_work_freq =
		fd_dvfs_config_table[FD_WORK_INDEX_DEF].clk_freq;
	set_ip_dvfs_idle_index(devfreq, FD_IDLE_INDEX_DEF);
	fd->fd_dvfs_para.ip_coffe.idle_index_def = FD_IDLE_INDEX_DEF;
	fd->fd_dvfs_para.u_idle_index = FD_IDLE_INDEX_DEF;
	fd->fd_dvfs_para.u_idle_freq =
		fd_dvfs_config_table[FD_IDLE_INDEX_DEF].clk_freq;
	ip_hw_dvfs_en(devfreq, FD_AUTO_TUNE);
	fd->freq = fd_dvfs_config_table[FD_IDLE_INDEX_DEF].clk_freq;
	fd->dvfs_enable = DVFS_ON;
	DVFS_TRACE("FD FREQ:%lu\n", fd->freq);
