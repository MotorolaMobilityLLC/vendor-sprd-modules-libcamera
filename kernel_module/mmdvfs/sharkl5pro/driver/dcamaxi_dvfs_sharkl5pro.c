...
struct ip_dvfs_map_cfg  dcamaxi_dvfs_config_table[] = {
	{0, REG_MM_DVFS_AHB_DCAM_AXI_INDEX0_MAP, VOLT70,
		DCAMAXI_CLK2560, DCAMAXI_CLK_INDEX_2560, 0, 0, 0, 0},
	{1, REG_MM_DVFS_AHB_DCAM_AXI_INDEX1_MAP, VOLT70,
		DCAMAXI_CLK3072, DCAMAXI_CLK_INDEX_3072, 0, 0, 0, 0},
	{2, REG_MM_DVFS_AHB_DCAM_AXI_INDEX2_MAP, VOLT75,
		DCAMAXI_CLK3840, DCAMAXI_CLK_INDEX_3840, 0, 0, 0, 0},
	{3, REG_MM_DVFS_AHB_DCAM_AXI_INDEX3_MAP, VOLT80,
		DCAMAXI_CLK4680, DCAMAXI_CLK_INDEX_4680, 0, 0, 0, 0},
...
static int ip_auto_tune_en(struct devfreq *devfreq, unsigned long dvfs_eb)
{
	...
	if (dvfs_eb)
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL,
			dfs_en_reg | BIT_DCAM_AXI_AUTO_TUNE_EN);
	else
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL,
			dfs_en_reg & (~BIT_DCAM_AXI_AUTO_TUNE_EN));
	mutex_unlock(&mmsys_glob_reg_lock);

	DVFS_TRACE("dvfs ops: %s, dvfs_eb=%lu\n", __func__, dvfs_eb);
	return 1;
}

static int get_ip_auto_tune_en(struct devfreq *devfreq, unsigned long *dvfs_eb)
{
	u32 dfs_en_reg;

	mutex_lock(&mmsys_glob_reg_lock);
	dfs_en_reg = DVFS_REG_RD(REG_MM_DVFS_AHB_MM_DFS_EN_CTRL);
	*dvfs_eb = (dfs_en_reg | BIT_DCAM_AXI_AUTO_TUNE_EN)
		>> SHIFT_BIT_DCAM_AXI_AUTO_TUNE_EN;
	mutex_unlock(&mmsys_glob_reg_lock);
	DVFS_TRACE("dvfs ops: %s, dvfs_eb=%lu\n", __func__, *dvfs_eb);
		...
		struct ip_dvfs_map_cfg  *dvfs_table)
{
	...
	for (i = 0; i < MAX_DVFS_INDEX; i++) {
		dvfs_table[i].map_index =
			dcamaxi_dvfs_config_table[i].map_index;
		...
		dvfs_table[i].fdiv_denom =
			dcamaxi_dvfs_config_table[i].fdiv_denom;
		dvfs_table[i].fdiv_num = dcamaxi_dvfs_config_table[i].fdiv_num;
		dvfs_table[i].axi_index =
			dcamaxi_dvfs_config_table[i].axi_index;
		dvfs_table[i].mtx_index =
			dcamaxi_dvfs_config_table[i].mtx_index;
	}
		...
		struct ip_dvfs_map_cfg *dvfs_table)
{
	...
	for (i = 0; i < MAX_DVFS_INDEX; i++) {
		if (dcamaxi_dvfs_config_table[i].reg_add ==
				dvfs_table->reg_add) {
			memcpy(&dcamaxi_dvfs_config_table[i],
	...
	ip_status)
{
	...
	ip_status->current_sys_volt = ((volt_reg >>
		SHFT_BITS_DCAM_AXI_VOLTAGE) & MASK_BITS_DCAM_AXI_VOLTAGE);
	DVFS_TRACE("dvfs ops: %s\n", __func__);
...
static int  ip_dvfs_init(struct devfreq *devfreq)
{
	...
	DVFS_TRACE("dvfs ops: no  no dcamaxi %s\n", __func__);
	dcamaxi = dev_get_drvdata(devfreq->dev.parent);

	if (!dcamaxi) {
		pr_err("undefined dcamaxi_dvfs\n");
		return 1;
	}
	...
	dcamaxi->dvfs_enable = DVFS_ON;
	dcamaxi->freq =
		dcamaxi_dvfs_config_table[DCAM_AXI_WORK_INDEX_DEF].clk_freq;
...
struct ip_dvfs_ops  dcamaxi_dvfs_ops  =  {
	...
	.get_ip_auto_tune_en = get_ip_auto_tune_en,
