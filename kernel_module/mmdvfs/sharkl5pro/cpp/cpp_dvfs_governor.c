		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cpp->dvfs_ops != NULL && cpp->dvfs_ops->ip_hw_dvfs_en != NULL
		&& (err != 0)) {
		cpp->dvfs_ops->ip_hw_dvfs_en(devfreq, dvfs_en);
	} else
		pr_err("fail to:%s ops null\n", __func__);
						...
						buf)
{
	...
	struct cpp_dvfs *cpp;
	int err = 0;
	unsigned long auto_tune_en = 0;

	cpp = dev_get_drvdata(devfreq->dev.parent);
	mutex_lock(&devfreq->lock);
	if (cpp != NULL && cpp->dvfs_ops != NULL &&
		cpp->dvfs_ops->get_ip_auto_tune_en != NULL) {
		cpp->dvfs_ops->get_ip_auto_tune_en(devfreq, &auto_tune_en);
		err = sprintf(buf, "%lu\n", auto_tune_en);
	} else {
		err = sprintf(buf, "undefined\n");
	}
	...
	struct device_attribute *attr,  const char *buf, size_t count)
{
	...
	if (cpp->dvfs_ops != NULL &&
		cpp->dvfs_ops->ip_auto_tune_en != NULL) {
		cpp->dvfs_ops->ip_auto_tune_en(devfreq, auto_tune_en);
	} else
		pr_err("fail to:%s ops null\n", __func__);
		...
		struct device_attribute *attr,  char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if ((cpp != NULL) && cpp->dvfs_ops != NULL &&
		cpp->dvfs_ops->get_ip_dvfs_work_index != NULL) {
		cpp->dvfs_ops->get_ip_dvfs_work_index(devfreq, &index);
		err = sprintf(buf, "%d\n", index);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
						...
						char *buf, size_t count)
{
	...
	if (cpp->dvfs_ops  !=  NULL &&
		...
		cpp->dvfs_ops->set_ip_dvfs_work_index(devfreq, work_index);
	} else
		pr_err("fail to:%s ops null\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&devfreq->lock);
	if ((cpp != NULL) && cpp->dvfs_ops != NULL &&
		cpp->dvfs_ops->get_ip_dvfs_idle_index != NULL) {
		cpp->dvfs_ops->get_ip_dvfs_idle_index(devfreq, &index);
		err = sprintf(buf, "%d\n", index);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cpp->dvfs_ops  !=  NULL &&
		...
		cpp->dvfs_ops->set_ip_dvfs_idle_index(devfreq, idle_index);
	} else
		pr_err("fail to:%s ops null\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cpp->dvfs_ops != NULL &&
		cpp->dvfs_ops->set_ip_dvfs_coffe != NULL) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (cpp->dvfs_ops  !=  NULL &&
		...
		pr_err("%s:fail to dvfs_read_ops is null\n", __func__);
	...
	if (cpp->dvfs_ops !=  NULL &&
		...
		pr_err("%s:fail to dvfs_read_top_volt is null\n", __func__);
		len = sprintf(buf, "get_ip_status_show failed\n");
		goto err_exit;
	}
	if (cpp->dvfs_ops !=  NULL &&
	...
	struct device_attribute *attr,   char *buf)
{
	...
	if (cpp->dvfs_ops  !=  NULL && 
		cpp->dvfs_ops->get_ip_dvfs_table != NULL) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cpp->cpp_dvfs_para.ip_coffe.sw_trig_en = sw_trig_en;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (cpp != NULL) {
		err = sprintf(buf, "%d\n",
		cpp->cpp_dvfs_para.ip_coffe.idle_switch_en);
	} else
		err = sprintf(buf, "undefined\n");
	mutex_unlock(&devfreq->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cpp->cpp_dvfs_para.ip_coffe.idle_switch_en = idle_switch_en;

	if (cpp->dvfs_ops  !=  NULL && cpp->dvfs_ops->set_ip_dfs_idle_disable !=
		NULL)
		cpp->dvfs_ops->set_ip_dfs_idle_disable(idle_switch_en);
	else
		pr_err("%s: ip ops null\n", __func__);
			...
			unsigned long *freq)
{
	...
	if (cpp->dvfs_enable) {
		if (cpp->dvfs_ops != NULL &&
			cpp->dvfs_ops->get_ip_work_or_idle != NULL)
			cpp->dvfs_ops->get_ip_work_or_idle(&is_work);
