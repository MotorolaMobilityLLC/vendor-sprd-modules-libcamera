	...
	u32 flags)
{
	...
	if (cpp != NULL && cpp->dvfs_ops != NULL &&
		cpp->dvfs_ops->update_target_freq != NULL){
		cpp->dvfs_ops->update_target_freq(cpp->devfreq, 0,
			cpp->cpp_dvfs_para.u_work_freq,
			cpp->cpp_dvfs_para.u_idle_freq);
	}

	if (err) {
		dev_err(dev, "fail to set freq:%lu to cpp, err: %d\n",
		*freq, err);
		goto out;
	}
...
static int cpp_dvfs_get_cur_freq(struct device *dev, unsigned long *freq)
{
	...
	if (cpp->dvfs_ops != NULL && cpp->dvfs_ops->get_ip_work_or_idle != NULL)
		cpp->dvfs_ops->get_ip_work_or_idle(&is_work);
	if (is_work == DVFS_WORK)
		*freq = cpp->cpp_dvfs_para.u_work_freq;
	else
		*freq = cpp->cpp_dvfs_para.u_idle_freq;
...
static int cpp_dvfs_probe(struct platform_device *pdev)
{
	...
	DVFS_TRACE("cpp-dvfs initialized\n");
	...
	return 0;
err:
	pr_err("fail to initialize cpp-dvfs\n");
	return ret;
}
