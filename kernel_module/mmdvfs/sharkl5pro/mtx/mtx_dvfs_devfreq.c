	...
	u32 flags)
{
	...
	mutex_lock(&mtx->lock);
	if (mtx != NULL && mtx->dvfs_ops != NULL &&
		mtx->dvfs_ops->update_target_freq != NULL){
		mtx->dvfs_ops->update_target_freq(
			mtx->devfreq, 0,
...
static int mtx_dvfs_get_cur_freq(struct device *dev, unsigned long *freq)
{
	...
	if (mtx->dvfs_ops != NULL && mtx->dvfs_ops->get_ip_work_or_idle != NULL)
		mtx->dvfs_ops->get_ip_work_or_idle(&is_work);
	if (is_work == DVFS_WORK)
		*freq = mtx->mtx_dvfs_para.u_work_freq;
	else
		*freq = mtx->mtx_dvfs_para.u_idle_freq;
...
static int mtx_dvfs_probe(struct platform_device *pdev)
{
	...
	DVFS_TRACE("mtx-dvfs initialized\n");
