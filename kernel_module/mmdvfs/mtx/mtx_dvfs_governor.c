	...
	struct device_attribute *attr,   char *buf)
{
	...
	if (mtx->dvfs_ops  !=  NULL &&   mtx->dvfs_ops->get_ip_dvfs_table !=
	NULL) {
			...
			unsigned long *freq)
{
	...
	if (mtx->dvfs_enable) {
		unsigned long adjusted_freq = mtx->user_freq;


		if (mtx->user_freq_type == DVFS_WORK)
			adjusted_freq = mtx->mtx_dvfs_para.u_work_freq;
		else
			adjusted_freq = mtx->mtx_dvfs_para.u_idle_freq;
