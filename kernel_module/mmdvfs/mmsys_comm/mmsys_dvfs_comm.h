...
struct ip_dvfs_ops {
	...
	void  (*set_ip_dvfs_work_index)(struct devfreq *devfreq, unsigned int
		index);
	void  (*get_ip_dvfs_work_index)(struct devfreq *devfreq, unsigned
		int *index);

	void  (*set_ip_dvfs_idle_index)(struct devfreq *devfreq, unsigned
		int  index);
	void  (*get_ip_work_index_from_table)(struct ip_dvfs_map_cfg
		*dvfs_cfg, unsigned long work_freq, unsigned int  *index);
	...
	int (*mmsys_dvfs_wait_window)(struct devfreq *devfreq, unsigned int
		wait_window);
	int (*mmsys_dvfs_min_volt)(struct devfreq *devfreq, unsigned int
		min_volt);
	void (*mmsys_power_ctrl)(struct devfreq *devfreq, unsigned int  on);
	int (*top_current_volt)(struct devfreq *devfreq, unsigned int *
		top_volt);
...
extern int mmsys_adjust_target_freq(mmsys_module md_type,
			unsigned long  volt,  unsigned long freq, unsigned int
				en_sysgov);
