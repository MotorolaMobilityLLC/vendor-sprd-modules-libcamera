	...
	struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  pr_info("%s: ip  ops \n", __func__);
	...
	if (isp->dvfs_ops  !=  NULL &&
		isp->dvfs_ops->set_idle_freq != NULL && (err != 0)) {
		
	isp->isp_dvfs_para.u_idle_freq = idle_freq;
		err= isp->dvfs_ops->set_idle_freq(devfreq,  isp->isp_dvfs_para.u_idle_freq);
	} else
		pr_info("%s: ip  ops null\n", __func__);
			...
			unsigned long *freq)
{
	...
	if (isp->dvfs_enable) {
		unsigned long adjusted_freq = isp->user_freq;


		if (isp->user_freq_type == DVFS_WORK)
