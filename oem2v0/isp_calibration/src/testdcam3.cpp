...
//	struct private_handle_t prv(-1,(uint32_t)(cs.width*cs.height)*2,0);
//	private_handle_t(int flags, int usage, int size, uint64_t base, int lock_state, int fb_file, int fb_offset):

	struct private_handle_t prv(0, 0, (int)(cs.width*cs.height)*2, (uint64_t) 0, -1, 0, 0, 0);
	prv.fd = -1;
	prv.size = cs.width*cs.height*2;
	prv.flags = 0;
