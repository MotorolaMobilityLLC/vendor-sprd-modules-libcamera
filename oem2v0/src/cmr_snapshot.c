...
struct process_status {
	...
	cmr_u32                             slice_width_in;
	cmr_u32                             slice_width_out;
	cmr_u32                             is_encoding;
	cmr_u32                             slice_num;
...
exit:
	property_get("debug.camera.save.snpfile", value, "0");
	if (atoi(value) == 9) {
		camera_save_to_file(SNP_SCALE_DATA, IMG_DATA_TYPE_YUV420,
							dst.size.width,
							dst.size.height,
							&dst.addr_vir);
	}
...
#if 1
#define FILE_DIR "/data/misc/cameraserver/"
#else
...
#define FILE_NAME_LEN 200
	...
	isp_ioctl(isp_handle, ISP_CTRL_GET_RGB_GAIN, (void*)&rgbgain);
	...
	strcat(file_name, "_");
	strcat(file_name, "rgbgain");
	strcat(file_name, "_");
	sprintf(tmp_str, "%d", rgbgain);
	strcat(file_name, tmp_str);
...
cmr_int snp_start_isp_next_proc(cmr_handle snp_handle, void *data)
{
	...
	CMR_LOGI("slice_out %d, dst_height %d", slice_out, dst_height);
	if (slice_out == dst_height) {
		process_ptr->slice_height_out = 0;
	...
	cmr_copy((void*)&isp_in_param, (void*)&chn_param_ptr->isp_proc_in[index], sizeof(struct raw_proc_param));
	process_ptr = &chn_param_ptr->isp_process[index];
	process_ptr->slice_width_out += isp_out_ptr->output_width;
	slice_out = process_ptr->slice_width_out;
	dst_width = chn_param_ptr->isp_proc_in[index].dst_frame.size.width;
	dst_height = chn_param_ptr->isp_proc_in[index].dst_frame.size.height;
	CMR_LOGI("slice_out %d, dst_width %d, dst_height %d", slice_out, dst_width, dst_height);
	chn_param_ptr->isp_process[index].slice_num++;
	if (slice_out >= dst_width) {
		process_ptr->slice_height_out = 0;
		process_ptr->slice_width_out = 0;
		CMR_LOGI("isp proc done");
		return CMR_CAMERA_NORNAL_EXIT;
	} else {
		isp_in_param.src_slice_width = dst_width - slice_out;
	}
...
cmr_int snp_start_cvt(cmr_handle snp_handle, void *data)
{
	...
	//	snp_send_msg_notify_thr(snp_handle, SNAPSHOT_FUNC_STATE, SNAPSHOT_EVT_START_CVT, (void*)ret, sizeof(cmr_int));
		...
		//frm_ptr->fmt = IMG_DATA_TYPE_YUV420;//TO DO...
		frm_ptr->fmt = IMG_DATA_TYPE_YVU420;
//		snp_send_msg_notify_thr(snp_handle, SNAPSHOT_FUNC_STATE, SNAPSHOT_EVT_START_CVT, (void*)ret, sizeof(cmr_int));
	}
	if (ret) {
...
cmr_int snp_set_scale_param(cmr_handle snp_handle)
{
	...
	cmr_uint                        i;
	cmr_u32                         offset[CMR_CAPTURE_MEM_SUM];
	...
	for (i=0 ; i<1/*CMR_CAPTURE_MEM_SUM*/ ; i++) {
		CMR_LOGI("src fd 0x%x src addr 0x%lx 0x%lx dst add 0x%lx 0x%lx fd 0x%x",
...
cmr_int snp_set_isp_proc_param(cmr_handle snp_handle)
{
	...
	for (i=0 ; i<CMR_CAPTURE_MEM_SUM ; i++) {
		...
		chn_param_ptr->isp_proc_in[i].dst_frame.size = req_param_ptr->post_proc_setting.sn_orig_size;
		//chn_param_ptr->isp_proc_in[i].dst_frame.size = req_param_ptr->post_proc_setting.dealign_actual_snp_size;
		CMR_LOGI("dealign_actual_snp_size width %d, height %d",
		...
		CMR_LOGI("post_proc_setting.snp_size width %d, height %d",
			req_param_ptr->post_proc_setting.snp_size.width,
...
#else
		/*if (CMR_FIRST_SLICE_WIDTH % 0x2) {
			chn_param_ptr->isp_proc_in[i].src_slice_width = CMR_FIRST_SLICE_WIDTH + 1;
		} else {
			chn_param_ptr->isp_proc_in[i].src_slice_width = CMR_FIRST_SLICE_WIDTH;
		}*/
		chn_param_ptr->isp_proc_in[i].src_slice_width = req_param_ptr->post_proc_setting.snp_size.width / 2;
#endif
	...
	case SNAPSHOT_EVT_CHANNEL_DONE:
		...
		if (1 == cxt->req_param.is_hdr) {
			...
			if (1 == cxt->req_param.is_video_snapshot || 1 == cxt->req_param.is_zsl_snapshot) {
				...
				src_vir = (unsigned long)chn_data.uaddr_vir;
#else
				if (ZOOM_POST_PROCESS == proc_param_ptr->channel_zoom_mode) {
					src_vir = (unsigned long)chn_data.yaddr_vir + width * height;
				} else if (ZOOM_POST_PROCESS_WITH_TRIM == proc_param_ptr->channel_zoom_mode) {
