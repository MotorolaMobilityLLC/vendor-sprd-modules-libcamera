				...
				uint32_t *io_channel_size)
{
	...
	y_to_raw = (uint32_t)(sn_size->height * sn_size->width);
	...
	*io_channel_size = channel_size;
	...
	cap_mem->cap_yuv.fd = cap_mem->cap_yuv.fd;
	...
	if (uv_size < mem_res) {
		...
		signed int fd;
		if (cap_2_frm->alloc_mem(cap_2_frm->handle, uv_size, &addr_phy, &addr_vir, &fd) != 0) {
				...
				uint32_t *io_channel_size)
{
	...
	cap_mem->target_jpeg.addr_phy.addr_y = cap_mem->cap_yuv.addr_phy.addr_y;
	cap_mem->target_jpeg.addr_vir.addr_y = cap_mem->cap_yuv.addr_vir.addr_y;
	cap_mem->target_jpeg.fd = cap_mem->target_yuv.fd;
				...
				uint32_t *io_channel_size)
{
	...
	if (mem_res > size_pixel) {
		...
		cap_mem->cap_yuv_rot.fd = cap_2_frm->mem_frm.fd;
