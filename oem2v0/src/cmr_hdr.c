...
static cmr_int hdr_notify_isp_finish(cmr_handle class_handle);
				...
				cmr_handle *class_handle)
{
	...
	hdr_handle->ev_effect_frame_interval = in->adgain_valid_frame_num;
...
//for new 3A version
	...
	hdr_handle->ev_effect_frame_interval = in->hdr_ev_effect_value;
#endif
...
static cmr_int hdr_transfer_frame(cmr_handle class_handle, struct ipm_frame_in *in, struct ipm_frame_out *out)
{
	...
	CMR_LOGI("in->src_frame.size width %d, height %d, hdr_handle->height %ld hdr_handle->width %ld",
		in->src_frame.size.width, in->src_frame.size.height, hdr_handle->height, hdr_handle->width);
	if ((hdr_handle->common.save_frame_count == 0) &&
		((hdr_handle->height < size.height)||(hdr_handle->width < size.width))) {
		hdr_handle->height = size.height;
		hdr_handle->width = size.width;

		for (i = 0; i < HDR_CAP_NUM; i++) {
			if (NULL != hdr_handle->alloc_addr[i]) {
				free(hdr_handle->alloc_addr[i]);
				hdr_handle->alloc_addr[i] = NULL;
			}
		}

		hdr_handle->mem_size = (cmr_uint)(size.width * size.height * 3 / 2);
		for (i = 0; i < HDR_CAP_NUM; i++) {
			hdr_handle->alloc_addr[i] = (cmr_u8*)malloc(hdr_handle->mem_size);
			if (NULL == hdr_handle->alloc_addr[i]){
				CMR_LOGE("mem alloc failed i = %ld", i);
				goto free_all;
			}
...
//new 3A version need oem notify stop command
#ifdef CONFIG_CAMERA_ISP_AE_VERSION_V1
static cmr_int hdr_notify_isp_finish(cmr_handle class_handle)
{
	cmr_int               ret = CMR_CAMERA_SUCCESS;
	...
	ret = isp_ioctl(oem_handle,COM_ISP_STOP_HDR,(void *)&isp_param);
	CMR_LOGI("COM_ISP_STOP_HDR");

	if (ret) {
		CMR_LOGE("HDR failed to notify isp.");
	}
...
//if use new 3A version, hdr needn't set ev at oem
#ifdef CONFIG_CAMERA_ISP_AE_VERSION_V1
	ret = isp_ioctl(oem_handle,COM_ISP_START_HDR,(void *)&isp_param);
	CMR_LOGI("isp start hdr");
#else
...
static cmr_int hdr_arithmetic(cmr_handle class_handle, struct img_addr *dst_addr,cmr_u32 width,cmr_u32 height)
{
	...
	CMR_LOGD("hdr arithmetic dst_addr 0x%lx", dst_addr->addr_y);
...
//for new 3A version
	...
	CMR_LOGI("width%d, height%d", in->src_frame.size.width, in->src_frame.size.height);
	CMR_LOGI("y addr_phy %x, uv addr_phy %x", in->src_frame.addr_phy.addr_y, in->src_frame.addr_phy.addr_u);
	CMR_LOGI("y addr_vir %x, uv addr_vir %x", in->src_frame.addr_vir.addr_y, in->src_frame.addr_vir.addr_u);
...
static cmr_int hdr_thread_proc(struct cmr_msg *message, void *private_data)
{
	...
	case CMR_EVT_HDR_START:
		...
		//  modify "+1 , -1 , 0" to "-1 , 0 , +1" , need modify index "0-1-2" to "1-2-0"
...
static cmr_int hdr_save_yuv(cmr_handle class_handle, cmr_u32 width,cmr_u32 height)
{
	...
	strcpy(file_name, "/data/misc/cameraserver/hdr_");
	strcpy(file_name1, "/data/misc/cameraserver/hdr_");
	strcpy(file_name2, "/data/misc/cameraserver/hdr_");
