...
enum setting_general_type {
	...
	SETTING_GENERAL_3A_LOCK_UNLOCK,
				                   ...
				                   struct setting_cmd_parameter *parm)
{
	...
	struct setting_general_item   general_list[] = {
		...
		{SETTING_GENERAL_3A_LOCK_UNLOCK, &hal_param->is_3a_lock, COM_ISP_SET_3A_LOCK_UNLOCK, COM_SN_TYPE_MAX},
	};
	...
	case SETTING_GENERAL_3A_LOCK_UNLOCK:
		if (setting_is_rawrgb_format(cpt, parm)) {
			ret = setting_isp_ctrl(cpt, item->isp_cmd, parm);
		}
		break;
	...
	if (SETTING_GENERAL_AE_LOCK_UNLOCK == type || SETTING_GENERAL_3A_LOCK_UNLOCK == type) {
		goto setting_out;
	}
...
static cmr_int setting_set_ae_metering_area(struct setting_component *cpt,
                                          struct setting_cmd_parameter *parm)
{
    cmr_int                             ret = 0;
    struct setting_init_in              *init_in = &cpt->init_in;
...
static cmr_int setting_set_lens_facing(struct setting_component *cpt,
                                               struct setting_cmd_parameter *parm)
{
	cmr_int                               ret = 0;
	struct setting_hal_param        *hal_param = get_hal_param(cpt, parm->camera_id);

	hal_param->lens_facing = parm->cmd_type_value;
	CMR_LOGD("lens_facing=%ld", hal_param->lens_facing);
...
*/
	...
	rotation_angle = hal_param->encode_angle;
	...
	if(rotation_angle == IMG_ANGLE_90 || rotation_angle == IMG_ANGLE_270){
		p_exif_info->primary.basic.ImageWidth = exif_unit->actual_picture_size.height;
		p_exif_info->primary.basic.ImageLength = exif_unit->actual_picture_size.width;
	}else{
...
static cmr_int setting_get_lens_facing(struct setting_component *cpt,
                                               struct setting_cmd_parameter *parm)
{
	cmr_int ret = 0;
	struct setting_hal_param        *hal_param = get_hal_param(cpt, parm->camera_id);
	...
	CMR_LOGD("lens_facing=%ld", hal_param->lens_facing);
	parm->cmd_type_value = hal_param->lens_facing;
...
static cmr_int setting_set_3a_lock_unlock(struct setting_component *cpt,
								struct setting_cmd_parameter *parm)
{
	cmr_int ret = 0;

	CMR_LOGD("setting_set_3a_lock_unlock");
	ret = setting_set_general(cpt, SETTING_GENERAL_3A_LOCK_UNLOCK, parm);
									   ...
									   struct setting_cmd_parameter *parm)
{
	...
	if (setting_is_rawrgb_format(cpt, parm)) {
		...
		if (init_in->setting_isp_ioctl) {
			...
			ret = (*init_in->setting_isp_ioctl)(init_in->oem_handle, COM_ISP_SET_CAPTURE_RAW_MODE, &isp_param);
		}
	}
...
static cmr_int cmr_setting_parms_init() {
    ...
    cmr_add_cmd_fun_to_table(CAMERA_PARAM_ISP_3A_LOCK_UNLOCK,
                             setting_set_3a_lock_unlock);
    cmr_add_cmd_fun_to_table(CAMERA_PARAM_AE_METERING_AREA,
                             setting_set_ae_metering_area);
...
#endif
    cmr_add_cmd_fun_to_table(SETTING_GET_LENS_FACING, setting_get_lens_facing);
