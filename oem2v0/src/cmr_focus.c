...
struct af_isp_mode_pairs af_isp_focus_mode [CAMERA_FOCUS_MODE_MAX]={
	...
	{CAMERA_FOCUS_MODE_TAKE_PICTURE , ISP_FOCUS_TAKE_PICTURE},
};
	...
	case CAMERA_PARAM_FOCUS_RECT:
		if(param != NULL){
			...
			for(loop = 0; loop < temp.zone_cnt; loop++)
				CMR_LOGD("temp.zone_cnt=%d x=%d ,y=%d, w=%d ,h=%d ",temp.zone_cnt, temp.zone[loop].start_x, temp.zone[loop].start_y, temp.zone[loop].width, temp.zone[loop].height);
