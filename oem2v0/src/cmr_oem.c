...
//	ret = sensor_merge_isp_param(&isp_param,sensor_info_ptr->raw_info_ptr);
	...
	if ((CAMERA_ID_0 == cxt->camera_id || CAMERA_ID_1 == cxt->camera_id) && cxt->is_refocus_mode) {
		...
		if (CAMERA_ID_0 == cxt->camera_id) {
			...
			ret = cmr_sensor_get_info(sn_cxt->sensor_handle,CAMERA_ID_3, &sn_cxt->sensor_info_slv);
			if (ret) {
				CMR_LOGE("get_sensor info failed!");
				ret = CMR_CAMERA_FAIL;
				goto exit;
...
cmr_int camera_preview_set_yuv_to_isp(cmr_handle oem_handle, cmr_u32 camera_id, struct yuv_info_t *yuv)
{
	cmr_int                        ret = CMR_CAMERA_SUCCESS;
	struct camera_context          *cxt = (struct camera_context*)oem_handle;
	struct common_isp_cmd_param    isp_param = {0};
