	                        ...
	                        cmr_u32 *sum_ptr, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	...
	if (cxt->hal_malloc) {
		...
		CMR_LOGI("0x%lx 0x%lx 0x%lx", (cmr_uint)phy_addr, (cmr_uint)vir_addr, (cmr_uint)fd);
	} else {
		CMR_LOGE("null");
	}
}
...
//	ret = sensor_merge_isp_param(&isp_param,sensor_info_ptr->raw_info_ptr);
	...
	if (ret) {
		...
		isp_cxt->inited = 1;
		CMR_LOGD("isp_cxt->width_limit:%d",isp_cxt->width_limit);
	}
...
cmr_int camera_isp_deinit(cmr_handle  oem_handle)
{
	...
	if(cxt->isp_malloc_flag == 1) {
		if (cxt->hal_free) {
			cxt->hal_free(CAMERA_ISP_LSC,&cxt->isp_lsc_phys_addr,&cxt->isp_lsc_virt_addr, &fd,
	...
	if(cxt->isp_anti_flicker_flag == 1) {
		if (cxt->hal_free) {
			cxt->hal_free(CAMERA_ISP_ANTI_FLICKER,&cxt->isp_anti_flicker_phys_addr,&cxt->isp_anti_flicker_virt_addr, &fd,
...
{
	cmr_int                        ret = CMR_CAMERA_SUCCESS;
	cmr_int                        ret_res_deinit = CMR_CAMERA_SUCCESS;
	cmr_int                        ret_sensor_deinit = CMR_CAMERA_SUCCESS;
...
res_deinit:
    ret_res_deinit = camera_res_deinit(oem_handle);
    if(ret_res_deinit){
        CMR_LOGE("failed to camera_res_deinit %ld", ret_res_deinit);
    }

sensor_deinit:
    ret_sensor_deinit = camera_sensor_deinit(oem_handle);
    if(ret_sensor_deinit){
        CMR_LOGE("failed to camera_sensor_deinit %ld", ret_sensor_deinit);
    }
                                         ...
                                         struct img_frm *src, struct img_frm *dst, struct cmr_op_mean *mean)
{
	...
	CMR_LOGI("dst->fd 0x%lx, src->fd 0x%lx enc_in_param.stream_buf_fd 0x%lx,enc_in_param.src_fd 0x%lx", dst->fd,  src->fd,
			 enc_in_param.stream_buf_fd, enc_in_param.src_fd);
	if (1 != mean->is_thumb) {
#ifdef CONFIG_FACE_BEAUTY
...
cmr_int camera_raw_proc(cmr_handle oem_handle, cmr_handle caller_handle, struct raw_proc_param *param_ptr)
{
	...
	CMR_LOGI("fd: src.fd=0x%x, dst.fd=0x%x, dst2.fd=0x%x",
		param_ptr->src_frame.fd, param_ptr->dst_frame.fd, param_ptr->dst2_frame.fd);
	...
	if (cxt->isp_malloc_flag == 0) {
		...
		if (cxt->hal_malloc) {
			cxt->hal_malloc(CAMERA_ISP_LSC, &isp_param.lsc_buf_size, &isp_param.lsc_buf_num,
					&cxt->isp_lsc_phys_addr,&cxt->isp_lsc_virt_addr, NULL, cxt->client_data);
...
//	isp_param.dv_mode = dv_mode;
{
	...
	if (cxt->is_highiso_mode == 1) {
		highiso_buf_num = 1;
		isp_raw_buf_num = 1;
		raw_buf_size = isp_param.size.w * isp_param.size.h * 4 / 3 * 2;
		...
		if (cxt->hal_malloc) {
			cxt->hal_malloc(CAMERA_ISP_RAW_DATA,
					&raw_buf_size,
					&isp_raw_buf_num,
					&cxt->raw_buf_phys_addr,
					&cxt->raw_buf_virt_addr,
					&cxt->raw_buf_fd,
					cxt->client_data);
		...
		isp_param.capture_mode = ISP_CAP_MODE_HIGHISO;

		isp_param.raw_buf_fd = cxt->raw_buf_fd;
		isp_param.raw_buf_phys_addr = cxt->raw_buf_phys_addr;
		isp_param.raw_buf_virt_addr = cxt->raw_buf_virt_addr;
		...
		CMR_LOGD("raw: fd=0x%x, phys_addr_offset=0x%lx, virt_addr = 0x%lx, buf_size=0x%lx",
			isp_param.raw_buf_fd, isp_param.raw_buf_phys_addr,
			isp_param.raw_buf_virt_addr, isp_param.raw_buf_size);
		CMR_LOGD("width=%ld, height=%ld", isp_param.raw_buf_width, isp_param.raw_buf_height);
		CMR_LOGD("highiso: fd=0x%x, phys_addr_offset=0x%lx, virt_addr = 0x%lx, buf_size=0x%lx",
...
cmr_int camera_isp_buff_cfg(cmr_handle oem_handle, struct buffer_cfg *buf_cfg)
{
	...
	isp_buf_cfg.index = buf_cfg->index[0];
	isp_buf_cfg.start_buf_id = buf_cfg->start_buf_id;
	isp_buf_cfg.addr.chn0 = buf_cfg->addr[0].addr_y;
	isp_buf_cfg.addr.chn1= buf_cfg->addr[0].addr_u;
	isp_buf_cfg.addr.chn2= buf_cfg->addr[0].addr_v;
	isp_buf_cfg.addr_vir.chn0 = buf_cfg->addr_vir[0].addr_y;
	isp_buf_cfg.addr_vir.chn1 = buf_cfg->addr_vir[0].addr_u;
	isp_buf_cfg.addr_vir.chn2 = buf_cfg->addr_vir[0].addr_v;
	isp_buf_cfg.is_reserved_buf = buf_cfg->is_reserved_buf;
	isp_buf_cfg.zsl_private = buf_cfg->zsl_private;
	isp_buf_cfg.img_fd.y = buf_cfg->fd[0];
	isp_buf_cfg.img_fd.u = buf_cfg->fd[0];
	isp_buf_cfg.img_fd.v = buf_cfg->fd[0];
	                                               ...
	                                               cmr_uint is_snapshot, struct preview_param *out_param_ptr)
{
	...
	cxt->is_highiso_mode = setting_param.cmd_type_value;
...
cmr_int camera_local_set_zsl_snapshot_buffer(cmr_handle oem_handle, cmr_uint src_phy_addr, cmr_uint src_vir_addr, cmr_s32 fd)
{
	...
	if (TAKE_PICTURE_NEEDED == camera_get_snp_req((cmr_handle)cxt)) {
		...
		img_frame.fd                         = fd;
		CMR_LOGI("in src_phy_addr 0x%lx src_vir_addr 0x%lx, img_frame.fd 0x%x", src_phy_addr, src_vir_addr, img_frame.fd);
