...
static cmr_int camera_isp_set_flash(cmr_handle oem_handle,cmr_u32 camera_id, cmr_u32 is_restart);
...
extern int sensor_merge_isp_param(struct isp_init_param *init_param_ptr,struct sensor_raw_info *sensor_info_ptr);
	                        ...
	                        cmr_u32 *sum_ptr, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	...
	if (!oem_handle || !fd || !vir_addr || !size_ptr || !sum_ptr) {
		CMR_LOGE("error param 0x%lx %p 0x%lx", (cmr_uint)oem_handle, fd, (cmr_uint)vir_addr);
		return CMR_CAMERA_INVALID_PARAM;
	}
	...
	if (cxt->hal_malloc) {
		...
		CMR_LOGI("%p 0x%lx", fd, (cmr_uint)vir_addr);
	} else {
		CMR_LOGE("null");
	}
	return ret;
						...
						cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	...
	if (!oem_handle || !fd || !vir_addr) {
		CMR_LOGE("error param 0x%lx %p 0x%lx", (cmr_uint)oem_handle, fd, (cmr_uint)vir_addr);
		return CMR_CAMERA_INVALID_PARAM;
	}
	CMR_LOGI("mem type %d sum %d", mem_type, sum);
	CMR_LOGV("free %p 0x%lx", fd, (cmr_uint)vir_addr);
...
{
	camera_snapshot_cb_to_hal(oem_handle, SNAPSHOT_EXIT_CB_PREPARE, SNAPSHOT_FUNC_TAKE_PICTURE, 0);
}
...
void camera_send_channel_data(cmr_handle oem_handle, cmr_handle receiver_handle, cmr_uint evt, void *data)
{
	...
	CMR_LOGI("%d %d %d %d", frm_ptr->channel_id, cxt->prev_cxt.channel_bits, cxt->snp_cxt.channel_bits, cxt->snp_cxt.total_num);
	...
	if (cxt->snp_cxt.channel_bits & chn_bit) {
		if (TAKE_PICTURE_NEEDED == camera_get_snp_req((cmr_handle)cxt) && CAMERA_ZSL_MODE != cxt->snp_cxt.snp_mode) {
			if (need_pause) {
				...
				//pause path before sending snp data
...
void camera_grab_handle(cmr_int evt, void* data, void* privdata)
{
	...
	cmr_u32                         frm_id;
	struct buffer_cfg               buf_cfg;
	cmr_s64                         timestamp;//for new 3a hdr
	cmr_s64                         isp_timestamp;//for new 3a hdr
	cmr_s64                         compare_timestamp;//for new 3a hdr
	cmr_u32                         compare_time;//for new 3a hdr
	cmr_u32                         total_num;//for new 3a hdr
	cmr_u32                         compare_num;//for new 3a hdr
	struct common_isp_cmd_param     isp_param;//for new 3a hdr
	cmr_s32                          i;//for new 3a hdr
...
//for new 3A
		...
		timestamp = frame->sec * 1000000LL + frame->usec;
		//timestamp = 10010;
		cxt->timestamp_list[cxt->timestamp_num] = timestamp;
		i = cxt->timestamp_num;
		CMR_LOGI("timestamp_list[%d] %lld", cxt->timestamp_num, cxt->timestamp_list[cxt->timestamp_num]);

		ret = camera_isp_ioctl((cmr_handle)cxt,COM_ISP_GET_HDR_TIMESTAMP,(void *)(&isp_param.hdr_param));
		isp_timestamp = isp_param.hdr_param.level;
		CMR_LOGI("isp_hdr_time %llu %d", isp_param.hdr_param.level, isp_param.hdr_param.skip_frame_num);
		if (isp_timestamp && (cxt->timestamp_list[i] > isp_timestamp)) {

			if (0 == frame->hdr_start) {
				for (i = cxt->timestamp_num; i >= 0; i--) {
					compare_timestamp = cxt->timestamp_list[i] - isp_timestamp;
					//10*1000, it depends
					if (compare_timestamp > 0) {
						continue;
					} else {
						break;
					}
				}
				CMR_LOGI("correct num is %d", i);
				frame->ev_skip_num = isp_param.hdr_param.skip_frame_num;
				/*for test
				property_get("debug.camera.hdr", value, "0");
				if (atoi(value)) {
					frame->ev_skip_num = atoi(value);
				}
				*/
				compare_num = cxt->timestamp_num - i;
				frame->ev_skip_num = (frame->ev_skip_num < compare_num) ? 0 : (frame->ev_skip_num - compare_num);

			}

			cxt->timestamp_num++;
			frame->hdr_start++;
			total_num = frame->ev_skip_num + HDR_CAP_NUM;
			if (total_num == frame->hdr_start) {
				frame->hdr_start = 0;
				cxt->timestamp_num = 0;
				memset(cxt->timestamp_list, 0, sizeof(cxt->timestamp_list));
				ret = ipm_notify_isp_finish(ipm_cxt->hdr_handle);
				usleep(50*1000);
			}
			CMR_LOGI("ev_skip_num %d", frame->ev_skip_num);
			ret = cmr_preview_receive_data(cxt->prev_cxt.preview_handle, cxt->camera_id, evt, data);
			if (ret) {
			...
			if (camera_is_need_change_fmt((cmr_handle)cxt, frame)) {
				...
				CMR_LOGI("cmr_snapshot_format_convert");
				ret = cmr_snapshot_format_convert(cxt->snp_cxt.snapshot_handle, data, &out_param);
				if (ret) {
			...
			ipm_in_param.hdr_ev_effect_value = frame->ev_skip_num;
...
void camera_grab_evt_cb(cmr_int evt, void* data, void* privdata)
{
	...
	case CMR_GRAB_TX_DONE:
		if( 0==cxt->camera_id ){
			timestamp = frame->sec*1000000000LL+frame->usec*1000LL;
			//isp_SetDcamTimeStamp(cxt->isp_cxt.isp_handle,&timestamp,0);
		}
...
cmr_int camera_isp_evt_cb(cmr_handle oem_handle, cmr_u32 evt, void* data, cmr_u32 data_len)
{
	...
	CMR_LOGV("evt 0x%x, handle 0x%lx len %d", evt, (cmr_uint)oem_handle, data_len);
...
void camera_snapshot_cb_to_hal(cmr_handle oem_handle, enum snapshot_cb_type cb, enum snapshot_func_type func, void* param)
{
	...
	case SNAPSHOT_EXIT_CB_DONE:
		if( 0==cxt->camera_id ){
			//isp_SetDcamTimeStamp(cxt->isp_cxt.isp_handle,&((struct camera_frame_type*)param)->timestamp,1);
		}
...
cmr_int camera_isp_deinit(cmr_handle  oem_handle)
{
	...
	if(cxt->isp_malloc_flag == 1) {
		if (cxt->hal_free) {
			cxt->hal_free(CAMERA_ISP_LSC,&cxt->isp_lsc_phys_addr,&cxt->isp_lsc_virt_addr, &cxt->lsc_fd,
	...
	if(cxt->isp_anti_flicker_flag == 1) {
		if (cxt->hal_free) {
			cxt->hal_free(CAMERA_ISP_ANTI_FLICKER,&cxt->isp_anti_flicker_phys_addr,&cxt->isp_anti_flicker_virt_addr, &cxt->anti_flicker_fd,
...
cmr_int camera_preview_init(cmr_handle  oem_handle)
{
	...
	init_param.ops.isp_set_flash = camera_isp_set_flash;
                                         ...
                                         struct img_frm *src, struct img_frm *dst, struct cmr_op_mean *mean)
{
	...
	if (1 != mean->is_thumb) {
		...
		if (ret) {
			...
			CMR_LOGD("kinlin perfectskinlevel is %d", PerfectSkinLevel);
		}
			...
			if (pic_width > 0 && pic_height > 0) {
				...
				if(cxt->fd_face_area.face_num>0){
					if (!snp_cxt->is_cfg_rot_cap) {
                                                ...
                                                SkinWhitenTsface.left = cxt->fd_face_area.face_info[0].sy * pic_height / cxt->fd_face_area.frame_width;
                                                SkinWhitenTsface.top = pic_height - cxt->fd_face_area.face_info[0].ex * pic_height / cxt->fd_face_area.frame_width ;
                                                SkinWhitenTsface.right =cxt->fd_face_area.face_info[0].ey * pic_height / cxt->fd_face_area.frame_width;
                                                SkinWhitenTsface.bottom = pic_height - cxt->fd_face_area.face_info[0].sx * pic_height / cxt->fd_face_area.frame_width;
					}
				...
				if(mu_retVal !=  TS_OK) {
					CMR_LOGD("perfect ts_face_beautify ret is %d", mu_retVal);
				} else {
...
cmr_int camera_capture_pre_proc(cmr_handle oem_handle, cmr_u32 camera_id, cmr_u32 preview_mode, cmr_u32 capture_mode, cmr_u32 is_restart, cmr_u32 is_sn_reopen)
{
	...
	if ((CAMERA_ZSL_MODE != snp_cxt->snp_mode) && (!is_restart || is_sn_reopen)) {
		...
		ret = camera_isp_ioctl(oem_handle,COM_ISP_SET_CAPTURE_RAW_MODE,(void *)&isp_param);
		if (ret) {
...
cmr_int camera_capture_ZSL_highflash(cmr_handle oem_handle, cmr_u32 camera_id)
{
	...
	CMR_LOGI("camera id %d, capture mode %d",
			camera_id,  snp_cxt->snp_mode);
...
cmr_int camera_raw_proc(cmr_handle oem_handle, cmr_handle caller_handle, struct raw_proc_param *param_ptr)
{
	...
	if (1 == param_ptr->slice_num) {
		...
		in_param.src_slice_width = param_ptr->src_slice_width;
		...
		in_param.dst_frame.img_size.w = param_ptr->src_frame.size.width;
		in_param.dst_frame.img_size.h = param_ptr->src_frame.size.height;
		//depend on kernel
...
cmr_int camera_isp_start_video(cmr_handle oem_handle, struct video_start_param *param_ptr)
{
	...
	if (cxt->isp_malloc_flag == 0) {
		...
		if (cxt->hal_malloc) {
			cxt->hal_malloc(CAMERA_ISP_LSC, (cmr_u32 *)&isp_param.lsc_buf_size, (cmr_u32 *)&isp_param.lsc_buf_num,
					&cxt->isp_lsc_phys_addr,&cxt->isp_lsc_virt_addr, &cxt->lsc_fd, cxt->client_data);
	...
	if (cxt->isp_anti_flicker_flag == 0) {
		...
		if (cxt->hal_malloc) {
			cxt->hal_malloc(CAMERA_ISP_ANTI_FLICKER, (cmr_u32 *)&isp_param.anti_flicker_buf_size, (cmr_u32 *)&isp_param.anti_flicker_buf_num,
					&cxt->isp_anti_flicker_phys_addr, &cxt->isp_anti_flicker_virt_addr, &cxt->anti_flicker_fd, cxt->client_data);
	...
	isp_param.anti_flicker_fd = cxt->anti_flicker_fd;
...
cmr_int camera_ioctl_for_setting(cmr_handle oem_handle, cmr_uint cmd_type, struct setting_io_parameter *param_ptr)
{
	...
	case SETTING_IO_CTRL_FLASH:
		...
		if (FLASH_HIGH_LIGHT == param_ptr->cmd_value && CAMERA_ZSL_MODE == cxt->snp_cxt.snp_mode) {
			//cmr_grab_cap_skip_num(cxt->grab_cxt.grab_handle, 0);// high flash effect suddenly
		}
...
cmr_int camera_sensor_ioctl(cmr_handle oem_handle, cmr_uint cmd_type, struct common_sn_cmd_param *param_ptr)
{
	...
	case COM_SN_SET_FOCUS:
		...
		CMR_LOGD("COM_SN_SET_FOCUS cmd =%ld ",cmd);
		break;
...
cmr_int camera_isp_ioctl(cmr_handle oem_handle, cmr_uint cmd_type, struct common_isp_cmd_param *param_ptr)
{
	...
	case COM_ISP_SET_AE_METERING_AREA:
		...
		CMR_LOGI("ae metering area %d %d %d %d", trim.start_x, trim.start_y, trim.end_x, trim.end_y);
	...
	case COM_ISP_SET_AF_MODE:
		...
		isp_param = param_ptr->cmd_value;
		if (isp_param == ISP_FOCUS_TAKE_PICTURE) {
			//ret = isp_set_take_picture_mode(isp_cxt->isp_handle, &isp_param);
			return ret;
		}
	...
	case COM_ISP_SET_3A_LOCK_UNLOCK:
		isp_cmd = ISP_CTRL_SET_3A_LOCK_UNLOCK;
		isp_param = param_ptr->cmd_value;
		CMR_LOGI("set 3A Lock & Unlock %d", isp_param);
		break;
	...
	case COM_ISP_START_HDR:
		isp_cmd = ISP_CTRL_START_HDR;
		isp_param = param_ptr->cmd_value;
		break;

	case COM_ISP_STOP_HDR:
		isp_cmd = ISP_CTRL_STOP_HDR;
		isp_param = param_ptr->cmd_value;
		break;

	case COM_ISP_GET_HDR_TIMESTAMP:
		isp_cmd = ISP_CTRL_GET_HDR_TIMESTAMP;
		isp_param_ptr = (void*)param_ptr;
		ptr_flag = 1;
		set_isp_flag = 0;
		break;
	...
	if (ptr_flag) {
		if(!cxt->mIspReleaseFlag){
			...
			ret = 0;
			CMR_LOGI("ptr_flag, isp is already closed,force return ");
			return ret;
		}
	}
	...
	if (set_isp_flag) {
		if(!cxt->mIspReleaseFlag){
			...
			ret = 0;
			CMR_LOGI("set_isp_flag, isp is already closed,force return");
			return ret;
		}
	}
	                                               ...
	                                               cmr_uint is_snapshot, struct preview_param *out_param_ptr)
{
	...
	ret = cmr_setting_ioctl(setting_cxt->setting_handle, SETTING_GET_LENS_FACING, &setting_param);
	if (ret) {
			CMR_LOGE("failed to get cap fmt %ld", ret);
			goto exit;
	}
	out_param_ptr->lens_facing = setting_param.cmd_type_value;
	...
	/* For pike2 brinup, dcam can not support 3 path, so close snapshot path for workaround in video recording scenario */
	if (out_param_ptr->snapshot_eb && out_param_ptr->video_eb) {
		out_param_ptr->snapshot_eb = 0;
	}

	CMR_LOGE("test:out_param_ptr->snapshot_eb %d out_param_ptr->video_eb %d", out_param_ptr->snapshot_eb, out_param_ptr->video_eb);
...
#ifdef CONFIG_SPRD_PRIVATE_ZSL
	if (out_param_ptr->sprd_zsl_enabled == 1 && out_param_ptr->flip_on == 1) {
		out_param_ptr->flip_on = 0x3; // mirror
	}
#endif
	...
	if (1 == camera_get_hdr_flag(cxt)) {
		...
		//need add sensor IF adgain_valid_frame_num
		//in_param.adgain_valid_frame_num = cxt->sn_cxt.sensor_info.adgain_valid_frame_num;
		in_param.adgain_valid_frame_num = 2;
...
cmr_int camera_get_snapshot_param(cmr_handle oem_handle, struct snapshot_param *out_ptr)
{
	...
	ret = cmr_setting_ioctl(setting_cxt->setting_handle, SETTING_GET_JPEG_QUALITY, &setting_param);
	if (ret) {
		CMR_LOGE("failed to get jpeg quality %ld", ret);
		goto exit;
	}
	jpeg_cxt->param.quality= setting_param.cmd_type_value;
	ret = cmr_setting_ioctl(setting_cxt->setting_handle, SETTING_GET_THUMB_QUALITY, &setting_param);
	...
	jpeg_cxt->param.thum_size = setting_param.size_param;
	out_ptr->post_proc_setting.thumb_size = setting_param.size_param;
...
cmr_int camera_set_setting(cmr_handle oem_handle, enum camera_param_type id, cmr_uint param)
{
	...
	case CAMERA_PARAM_ZOOM:
		...
		break;
	case CAMERA_PARAM_AE_METERING_AREA:
	...
	case CAMERA_PARAM_ISP_AE_LOCK_UNLOCK:
		...
		ret = cmr_setting_ioctl(cxt->setting_cxt.setting_handle, id, &setting_param);
		break;
	case CAMERA_PARAM_ISP_3A_LOCK_UNLOCK:
		CMR_LOGD("CAMERA_PARAM_ISP_3A_LOCK_UNLOCK");
		setting_param.cmd_type_value = param;
		ret = cmr_setting_ioctl(cxt->setting_cxt.setting_handle, id, &setting_param);
		break;
/*	case CAMERA_PARAM_LENS_FACING:
		setting_param.cmd_type_value = param;
		ret = cmr_setting_ioctl(cxt->setting_cxt.setting_handle, id, &setting_param);
		break;
*/
...
cmr_int camera_local_highflash_ae_measure (cmr_handle oem_handle)
{
	...
	if ((CAMERA_ISP_TUNING_MODE != cxt->snp_cxt.snp_mode)
		&& (CAMERA_ISP_SIMULATION_MODE != cxt->snp_cxt.snp_mode)) {
		setting_param.camera_id = cxt->camera_id;
...
cmr_int camera_local_start_snapshot(cmr_handle oem_handle, enum takepicture_mode mode, cmr_uint is_snapshot)
{
	...
	if(0 == cxt->camera_id && CAMERA_ZSL_MODE == cxt->snp_cxt.snp_mode){// back camera with auto focus lens and zsl mode
		CMR_LOGI("set focus af mode %d",CAMERA_FOCUS_MODE_TAKE_PICTURE);
		ret = cmr_focus_set_param(cxt->focus_cxt.focus_handle, cxt->camera_id, CAMERA_PARAM_AF_MODE,(void*)CAMERA_FOCUS_MODE_TAKE_PICTURE);
	}
	...
	if (CAMERA_ISP_SIMULATION_MODE == mode) {
		...
		cmr_sensor_ioctl(cxt->sn_cxt.sensor_handle, cxt->camera_id, SENSOR_BEFORE_SNAPSHOT, (SENSOR_EXP_TIME_CAPTURE_MODE | (SENSOR_EXP_TIME_PREVIEW_MODE  << CAP_MODE_BITS)));
		}
...
cmr_int camera_local_redisplay_data(cmr_handle oem_handle, cmr_s32 output_fd, cmr_uint output_addr,
                                                   cmr_uint output_width, cmr_uint output_height,
                                                   cmr_s32 input_fd, cmr_uint input_addr_y,
                                                   cmr_uint input_addr_uv, cmr_uint input_width,
                                                   cmr_uint input_height)
{
	cmr_int                        ret = CMR_CAMERA_SUCCESS;
	struct camera_context          *cxt = (struct camera_context*)oem_handle;
...
cmr_int camera_local_set_preview_buffer(cmr_handle oem_handle, cmr_uint src_phy_addr, cmr_uint src_vir_addr,  cmr_s32 fd)
{
	...
	cxt = (struct camera_context*)oem_handle;
	ret = cmr_preview_set_preview_buffer(cxt->prev_cxt.preview_handle, cxt->camera_id, src_phy_addr, src_vir_addr, fd);
	if (ret) {
...
cmr_int camera_local_set_zsl_snapshot_buffer(cmr_handle oem_handle, cmr_uint src_phy_addr, cmr_uint src_vir_addr, cmr_s32 fd)
{
	...
	if (TAKE_PICTURE_NEEDED == camera_get_snp_req((cmr_handle)cxt)) {
		...
		if(max_width !=0 && max_height != 0){
			...
			buffer_size         = cxt->snp_cxt.actual_capture_size.width * cxt->snp_cxt.actual_capture_size.height;
			CMR_LOGI("max_width %d, max_height %d, buffer_size %d", max_width, max_height, buffer_size);
		}
#else
		buffer_size         = cxt->snp_cxt.actual_capture_size.width * cxt->snp_cxt.actual_capture_size.height;
#endif
...
cmr_int camera_isp_set_flash(cmr_handle oem_handle,cmr_u32 camera_id, cmr_u32 is_restart)
{
	cmr_int                        ret = CMR_CAMERA_SUCCESS;
	struct camera_context          *cxt = (struct camera_context*)oem_handle;
	struct snapshot_context        *snp_cxt;
