...
struct prev_context {
	...
	pthread_mutex_t                 cap_zsl_mem_valid_num_mutex;
...
cmr_int cmr_update_hdr_ev_effect_value(cmr_handle preview_handle, cmr_s32 value)
{
	cmr_int                ret = CMR_CAMERA_SUCCESS;
	struct prev_handle     *handle = (struct prev_handle*)preview_handle;

	handle->hdr_ev_effect_value = value;
	return ret;
}
...
cmr_int cmr_preview_receive_data(cmr_handle preview_handle, cmr_u32 camera_id, cmr_uint evt, void *data)
{
	...
	/* revert original code because this modify will cause dead lock,and this
	* modify is for supporting 13M processing rate,
	if (PREVIEW_CHN_PAUSE == evt) {
		message.sync_flag  = CMR_MSG_SYNC_PROCESSED;
	} else {
...
cmr_int prev_assist_thread_proc(struct cmr_msg *message, void *p_data)
{
	...
	case PREV_EVT_RECEIVE_DATA:
		...
		if (handle->frame_active == 1) {
			...
			CMR_LOGE("handle->frame_active %d", handle->frame_active);
		if (frm_data) {
			free(frm_data);
			frm_data = NULL;
		}
...
//for new 3A hdr
	...
	if (FRAME_HDR_PROC == prev_cxt->prev_param.frame_ctrl) {
		prev_cxt->prev_param.frame_count = HDR_CAP_NUM + data->ev_skip_num;
	}
#endif
		...
		if (FRAME_HDR_PROC == prev_cxt->prev_param.frame_ctrl) {
			...
			if (prev_cxt->prev_param.snapshot_eb && !prev_cxt->prev_param.preview_eb) {
				if (handle->ops.capture_pre_proc) {
					...
					if (handle->ops.isp_set_flash) {
						handle->ops.isp_set_flash(handle->oem_handle,
									camera_id,
									1);
					} else {
...
//for new 3A hdr
#ifdef  XXXX
			hdr_skip_frame_num = data->ev_skip_num;
			CMR_LOGI("new 3A");
			if (prev_cxt->cap_frm_cnt <= hdr_skip_frame_num) {
#else
			if (prev_cxt->cap_frm_cnt <= (prev_cxt->prev_param.frame_count - hdr_num)) {
#endif
...
cmr_int prev_alloc_cap_buf(struct prev_handle *handle, cmr_u32 camera_id, cmr_u32 is_restart, struct buffer_cfg *buffer)
{
	...
	buffer_id = camera_pre_capture_buf_id(camera_id, 0, 0);
	...
	if (!is_restart) {
		...
		for (i = 0; i < CMR_CAPTURE_MEM_SUM; i++) {
			CMR_LOGI("%d,fd 0x%x phys_addr 0x%lx, virt_addr 0x%lx",
				i,
				prev_cxt->cap_fd_array[i],
	...
	for (i = 0; i < buffer->count; i++){
		if (IMG_DATA_TYPE_RAW == prev_cxt->cap_org_fmt) {
			...
			CMR_LOGE("is_normal_cap %d NO_SCALING 0x%x", is_normal_cap, NO_SCALING);
			if (is_normal_cap) {
				if ((IMG_ANGLE_0 != prev_cxt->prev_param.cap_rot) || prev_cxt->prev_param.is_cfg_rot_cap) {
		...
		CMR_LOGI("fd 0x%x capture addr, y 0x%x uv 0x%x", fd, y_addr, u_addr);
		if (0 == cur_img_frm->fd) {
			ret = CMR_CAMERA_FAIL;
			break;
		}
		...
		buffer->addr[i].addr_y     = prev_cxt->cap_frm[i].addr_phy.addr_y;// -(prev_cxt->cap_org_size.width * prev_cxt->cap_org_size.height * 3) /2;
		buffer->addr[i].addr_u     = prev_cxt->cap_frm[i].addr_phy.addr_u;// - (prev_cxt->cap_org_size.width * prev_cxt->cap_org_size.height * 3) /2;
		...
		CMR_LOGE("i %d buffer_fd 0x%x yaddr 0x%x uaddr 0x%x cap_org_fmt %c",
			i, buffer->fd[i], buffer->addr[i].addr_y, buffer->addr[i].addr_u, prev_cxt->cap_org_fmt);
		if ((FRAME_HDR_PROC == prev_cxt->prev_param.frame_ctrl)
			&& (IMG_DATA_TYPE_YUV420 == prev_cxt->cap_org_fmt || IMG_DATA_TYPE_YVU420 == prev_cxt->cap_org_fmt)
			&& is_normal_cap && i > 0) {
...
#ifdef CONFIG_SPRD_PRIVATE_ZSL
//bugFix:567908
	if (handle->prev_cxt[camera_id].prev_param.sprd_zsl_enabled) {
		width    = prev_cxt->max_size.width;
		height   = prev_cxt->max_size.height;
	}
	...
	if (IMG_DATA_TYPE_YUV420 == prev_cxt->cap_org_fmt || IMG_DATA_TYPE_YVU420 == prev_cxt->cap_org_fmt) {
		...
		//prev_cxt->cap_zsl_mem_size = (width * height * 3) >> 1;
		prev_cxt->cap_zsl_mem_size = width * height * RAWRGB_BIT_WIDTH / 8;
		CMR_LOGI("IMG_DATA_TYPE_RAW cap_zsl_mem_size %d", prev_cxt->cap_zsl_mem_size);
...
//bugFix:567908
		...
		CMR_LOGV("%lld width %d height %d", frame_type->timestamp, frame_type->width, frame_type->height);
		#if 0
		camera_save_to_file(prev_cxt->prev_frm_cnt,
				IMG_DATA_TYPE_YUV420,
				frame_type->width,
...
cmr_int prev_set_video_param(struct prev_handle *handle, cmr_u32 camera_id, cmr_u32 is_restart, struct preview_out_param *out_param_ptr)
{
	...
	if ((prev_cxt->prev_param.flip_on)&&(PREVIEW_LENS_FACING_FRONT == prev_cxt->prev_param.lens_facing)){
		chn_param.cap_inf_cfg.cfg.flip_on = prev_cxt->prev_param.flip_on;
	}else {
		chn_param.cap_inf_cfg.cfg.flip_on = 0;
	}
...
cmr_int prev_set_cap_param(struct prev_handle *handle, cmr_u32 camera_id, cmr_u32 is_restart, cmr_u32 is_lightly, struct preview_out_param *out_param_ptr)
{
	...
	if (capability.yuv_available_cnt) {
		...
		/* for capture, not skip frame for now, for cts testMandatoryOutputCombinations issu
e */
				...
				struct snp_proc_param *cap_post_proc_param)
{
	...
	if (IMG_ANGLE_90 == rot || IMG_ANGLE_270 == rot) {
		if (NO_SCALING_FOR_ROT) {
			...
			//for raw input post scale with rot
			zoom_param->zoom_info.output_ratio = 1/(zoom_param->zoom_info.output_ratio);
		}
	} else {
		if (NO_SCALING) {
			cap_post_proc_param->is_need_scaling = 0;
...
cmr_int prev_pop_preview_buffer(struct prev_handle *handle, cmr_u32 camera_id, struct frm_info *data, cmr_u32 is_to_hal)
{
	...
	CMR_LOGE("prev_frm[0].fd 0x%x data->fd 0x%x valid_num %d", prev_cxt->prev_frm[0].fd, data->fd, valid_num);
...
exit:
	CMR_LOGE("done cnt %ld fd %d yaddr 0x%x uaddr 0x%x ", data->fd, prev_cxt->prev_mem_valid_num, data->yaddr, data->uaddr);
	CMR_LOGD("done cnt %ld yaddr 0x%x ret %d ", prev_cxt->prev_mem_valid_num, data->yaddr, ret);
	return ret;
}
...
exit:
	CMR_LOGV("out yaddr 0x%x uaddr 0x%x cnt %ld", data->yaddr, data->uaddr, prev_cxt->video_mem_valid_num);
	return ret;
}
...
cmr_int prev_set_zsl_buffer(struct prev_handle *handle, cmr_u32 camera_id, cmr_uint src_phy_addr, cmr_uint src_vir_addr, cmr_s32 fd)
{
	...
	buffer_size = width * height;
	frame_size = prev_cxt->cap_zsl_mem_size;
	pthread_mutex_lock(&prev_cxt->cap_zsl_mem_valid_num_mutex);
	...
	pthread_mutex_unlock(&prev_cxt->cap_zsl_mem_valid_num_mutex);
...
cmr_uint prev_get_src_rot_buffer(struct prev_context *prev_cxt, struct frm_info *data, cmr_uint *index)
{
	...
	if (!ret && (frm_ptr != NULL)) {
		for (count = 0; count < PREV_ROT_FRM_CNT; count++){
			...
			if (1 == *(frm_is_lock + count) && (cmr_s32)data->fd == (frm_ptr + count)->fd) {
				*index = count;
					...
					struct snp_proc_param *out_param_ptr)
{
	...
	out_param_ptr->sn_orig_size      = prev_cxt->cap_org_size;
	CMR_LOGI("prev_cxt->cap_org_size w %d h %d",
			   ...
			   struct frm_info *data)
{
	...
	if (!handle->ops.channel_pause) {
		CMR_LOGE("ops channel_pause is null");
		ret = CMR_CAMERA_INVALID_PARAM;
		goto exit;
	}
