...
static cmr_int cmr_grab_cap_cfg_common(cmr_handle grab_handle, struct cap_cfg *config, cmr_u32 channel_id, struct img_data_end *endian)
{
	...
	if (found) {
		...
		if (endian != NULL) {
			struct img_data_end tmp_end;
			memcpy((void*)&tmp_end,(void*)&img_fmt.reserved[0],sizeof(struct img_data_end));
			if ((0 == img_fmt.channel_id) &&
			((IMG_ENDIAN_MAX != tmp_end.reserved0)||(IMG_ENDIAN_MAX != tmp_end.reserved1))) {
				tmp_end.y_endian = tmp_end.reserved0;//1;
				tmp_end.uv_endian = tmp_end.reserved1;//2;
...
cmr_int cmr_grab_buff_cfg (cmr_handle grab_handle, struct buffer_cfg *buf_cfg)
{
	...
	CMR_LOGD("chan_id %d  count %d base_id 0x%x ", buf_cfg->channel_id, buf_cfg->count, buf_cfg->base_id);
	CMR_LOGD("is_reserved_buf %d", buf_cfg->is_reserved_buf);
	...
	for (i = 0; i < buf_cfg->count; i++) {
		...
		CMR_LOGD("buf %d: Y 0x%lx, U 0x%lx, V 0x%lx \n",
			i, buf_cfg->addr[i].addr_y, buf_cfg->addr[i].addr_u, buf_cfg->addr[i].addr_v);
		CMR_LOGD("%d: fd 0x%x is_reserved_buf %d", i, parm.fd_array[i], parm.is_reserved_buf);
		if (buf_cfg->fd[i] > 0) {
...
cmr_int cmr_grab_path_capability(cmr_handle grab_handle, struct cmr_path_capability *capability)
{
	...
	CMR_LOGV("video prev %d scale %d capture_no_trim %d capture_pause %d zoom_post_proc %d", capability->is_video_prev_diff,
...
static cmr_u32 cmr_grab_get_data_endian(struct img_data_end* in_endian, struct img_data_end* out_endian)
{
	...
	if (IMG_ENDIAN_MAX == in_endian->y_endian) {
		out_endian->y_endian = IMG_ENDIAN_LITTLE;
	} else {
