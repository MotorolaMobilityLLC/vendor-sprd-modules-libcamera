								...
								struct img_size image_size)
{
	...
	for (nIndex = 0; nIndex < face_num; nIndex++) {   /*** Face Loop ***/
		...
		/* Faces on image border are unstable. If part of the face is out of image region, skip it */
		if ((sx < 0) || (sy < 0) || (ex >= (cmr_int)image_size.width) || (ey >= (cmr_int)image_size.height)) {
			continue;
		}
