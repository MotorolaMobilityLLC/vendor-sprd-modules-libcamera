...
int SprdCameraHardware::getNumberOfCameras()
{
	...
	if (1 == getPropertyAtv()) {
		...
		j = sizeof(SprdCameraHardware::kCameraInfo) / sizeof(SprdCameraHardware::kCameraInfo[0]);
		for(int i = 0; i< j; i++) {
...
int SprdCameraHardware::getCameraIndex()
{
	if (1 == getPropertyAtv()) {
		...
		int vindex = 0;
		int i;
		for(i = 0; i< sizeof(SprdCameraHardware::kCameraInfo) / sizeof(SprdCameraHardware::kCameraInfo[0]); i++) {
			if(SprdCameraHardware::kCameraInfo[i].orientation != -1) {
				if(vindex == mCameraId){
					break;
				}
...
/*	camera_set_capture_trace(1);*/
	...
	if (CAMERA_ISP_TUNING_MODE == mode) {
		...
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_THUMB_SIZE, (cmr_uint)&req_size);

		if(!mHalOem->ops->camera_get_sensor_info_for_raw(mCameraHandle, mode_info)) {
	...
	if (mCameraHandle == NULL || mHalOem == NULL || mHalOem->ops == NULL || CMR_CAMERA_SUCCESS != mHalOem->ops->camera_take_picture(mCameraHandle, mode)) {
...
/*			if (camera_get_rot_set()) {
		...
		if (isPreviewing())
			mHalOem->ops->camera_set_preview_buffer(mCameraHandle, mPreviewHeapArray_phy[index], mPreviewHeapArray_vir[index], mPreviewHeapArray_fd[index]);
	} else {
...
status_t SprdCameraHardware::autoFocus()
{
	...
	if (mCameraHandle != NULL && mHalOem != NULL && mHalOem->ops != NULL && 0 != mHalOem->ops->camera_start_autofocus(mCameraHandle)) {
...
status_t SprdCameraHardware::setParametersInternal(const SprdCameraParameters& params)
{
	...
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_PREVIEW_SIZE, (cmr_uint)&req_size);
	req_size.width = (cmr_u32)mRawWidth;
	req_size.height = (cmr_u32)mRawHeight;
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_CAPTURE_SIZE, (cmr_uint)&req_size);
	...
	if (1 == mParameters.getRecordingHint()) {
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_VIDEO_SIZE, (cmr_uint)&req_size);
	...
	mHalOem->ops->camera_is_change_size(mCameraHandle, mRawWidth, mRawHeight, mPreviewWidth, mPreviewHeight, mVideoWidth, mVideoHeight, &is_change);
	if (is_change) {
		if (isPreviewing()) {
			mPreviewLock.lock();
...
void SprdCameraHardware::setCameraPreviewMode(bool isRecordMode)
{
	...
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_PREVIEW_FPS, (cmr_uint)&fps_param);
	...
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_RANGE_FPS, (cmr_uint)&ae_fps_param);
}
...
bool SprdCameraHardware::startCameraIfNecessary()
{
	...
	if (!isCameraInit()) {
		...
		if (CMR_CAMERA_SUCCESS != mHalOem->ops->camera_init(getCameraIndex(), camera_cb, this, 0, &mCameraHandle, (void*)Callback_Malloc, (void*)Callback_Free)) {
			setCameraState(SPRD_INIT, STATE_CAMERA);
...
bool SprdCameraHardware::allocatePreviewMemFromGraphics(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr,cmr_s32 *fd)
{
	if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage) {
		...
		for (i = 0; i < (int)sum; i++ ) {
			...
			LOGI(" allocatePreviewMemFromGraphics: fd:0x%lx, base:0x%lx, size:0x%x, stride:0x%x ",
				 mPreviewHeapArray_fd[i],(uintptr_t)private_h->base,private_h->size, stride);
			mCancelBufferEb[i] = 0;
		}
...
int SprdCameraHardware::Callback_PreviewMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	...
	*fd = 0;
	...
	mPreviewHeapNum = sum;
	if (allocatePreviewMemFromGraphics(size, kPreviewBufferCount, phy_addr, vir_addr,fd)) {
	...
	LOGI("Callback_PreviewMalloc DcamAllocBufferCnt %d, mPreviewHeapArray %p, start_id %ld, end_id %ld real cnt %d",
		mPreviewDcamAllocBufferCnt,
		mPreviewHeapArray,
		buffer_start_id,
		buffer_end_id,
		HEAP_COUNTS(mPreviewDcamAllocBufferCnt));
	...
	if (mPreviewDcamAllocBufferCnt>0 && NULL == mPreviewHeapArray) {
		...
		for (i=buffer_start_id ; i<buffer_end_id ; i++) {
			...
			if (i < sum) {
				...
				*fd++ = PreviewHeap->fd;
				LOGI("Callback_PreviewMalloc: DCAM %ld, phys_addr 0x%x fd 0x%x", i, PreviewHeap->phys_addr, PreviewHeap->fd);
			}
		}
...
int SprdCameraHardware::Callback_VideoMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	cmr_uint buffer_start_id = 0, buffer_end_id = 0;
	cmr_uint i = 0, j = 0;
		...
		for (i=buffer_start_id ; i<buffer_end_id ; i++) {
			...
			if (i < sum) {
				...
				*fd++ = (cmr_s32)VideoHeap->fd;
				LOGI("Callback_VideoMalloc: DCAM %ld, fd 0x%x", i, VideoHeap->fd);
			}
		}
	}
...
int SprdCameraHardware::Callback_CapturePathMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	sprd_camera_memory_t *memory = NULL;
	cmr_int              i = 0;
...
int SprdCameraHardware::Callback_OtherMalloc(enum camera_mem_cb_type type, cmr_u32 size, cmr_u32 *sum_ptr, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	...
	*fd = 0;
	...
	if (type == CAMERA_PREVIEW_RESERVED || type == CAMERA_VIDEO_RESERVED || type == CAMERA_SNAPSHOT_ZSL_RESERVED) {
		sprd_camera_memory_t *memory = NULL;
		if(NULL == mCommonHeapReserved) {
			buffer_id = mHalOem->ops->camera_pre_capture_get_buffer_id(mCameraId);
			if (mHalOem->ops->camera_get_reserve_buffer_size(mCameraId, buffer_id, &mem_size, &mem_sum) != CMR_CAMERA_SUCCESS) {
		...
		if(mIspLscHeapReserved == NULL) {
			...
			LOGI("malloc Video memory, malloced type %d,request num %d, request size 0x%x", type, sum, size);
												...
												cmr_uint *vir_addr, cmr_s32 *fd, void* private_data)
{
	...
	if (CAMERA_PREVIEW == type) {
		...
		ret = camera->Callback_OtherMalloc(type, size, sum_ptr, phy_addr, vir_addr, fd);
...
int SprdCameraHardware::Callback_PreviewFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i, j = 0;
#if 0//wjp
	if (mIsPerformanceTestable) {
...
int SprdCameraHardware::Callback_VideoFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i;
...
int SprdCameraHardware::Callback_ZslFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i;
...
int SprdCameraHardware::Callback_CaptureFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i;
...
int SprdCameraHardware::Callback_CapturePathFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i;
...
int SprdCameraHardware::Callback_OtherFree(enum camera_mem_cb_type type, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i;
...
int SprdCameraHardware::Callback_Free(enum camera_mem_cb_type type, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum, void* private_data)
{
	SprdCameraHardware* camera = (SprdCameraHardware*)private_data;
	int                 ret = 0;

	if (!private_data || !vir_addr || !fd) {
		LOGE("error param 0x%lx 0x%lx 0x%x", fd, (cmr_uint)vir_addr, (cmr_uint)private_data);
		return -1;
	}
	if (type >= CAMERA_MEM_CB_TYPE_MAX) {
		LOGE("mem type error %ld", (cmr_uint)type);
...
sprd_camera_memory_t* SprdCameraHardware::allocCameraMem(int buf_size, uint32_t is_cache)
{
	...
	memory->fd = pHeapIon->getHeapID();
	if (!mIOMMUEnabled) {
		LOGI("X: fd 0x%lx, data: 0x%p, phys_size: 0x%lx heap=%p",
...
uint32_t SprdCameraHardware::getVideoBufferIDForFd(cmr_s32 fd)
{
	...
	int i = 0;
	for (i = 0; i < kPreviewBufferCount+kPreviewRotBufferCount+1; i++) {
		if ((NULL != mVideoHeapArray_fd[i]) &&
			(mVideoHeapArray_fd[i] == fd)) {
			id = i;
			break;
		}
	}
	...
	if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage
		...
		if (0 != mPreviewWindow->dequeue_buffer(mPreviewWindow, &buffer_handle, &stride)) {
			...
			if (mPreviewCancelBufHandle[free_buffer_id]  == mPreviewBufferHandle[free_buffer_id]) {
				...
				//mHalOem->ops->camera_set_preview_buffer(mCameraHandle, mPreviewHeapArray_phy[free_buffer_id], mPreviewHeapArray_vir[free_buffer_id]);
			} else {
				Mutex::Autolock pgbcl(&mGraphBufCntLock);
				if (isRecordingMode()) {
				...
				if((mGraphBufferCount[free_buffer_id] == 0) && isPreviewing())
					mHalOem->ops->camera_set_preview_buffer(mCameraHandle, mPreviewHeapArray_phy[free_buffer_id], mPreviewHeapArray_vir[free_buffer_id], mPreviewHeapArray_fd[free_buffer_id]);
			}
		}
#endif
	}
...
bool SprdCameraHardware::allocatePreviewMemByGraphics()
{
	...
	if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage) {
		...
		for (i = 0; i < kPreviewBufferCount; i++ ) {
			...
			LOGI("allocatePreviewMemByGraphics: fd:0x%lx, base:0x%lx, size:0x%x, stride:0x%x ",
...
takepicture_mode SprdCameraHardware::getCaptureMode()
{
	...
	if (0 == strcmp("hdr", mParameters.get_SceneMode()) && (1 != mParameters.getRecordingHint())) {
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SCENE_MODE, CAMERA_SCENE_MODE_HDR);
		mCaptureMode = CAMERA_NORMAL_MODE;
	} else if (mHalOem->ops->camera_is_vendor_hdr(mCameraHandle) || mHalOem->ops->camera_get_lls_shot_mode(mCameraHandle)) {
		mCaptureMode = CAMERA_NORMAL_MODE;
	} else {
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SCENE_MODE, 0);
		mCaptureMode = CAMERA_ZSL_MODE;
...
status_t SprdCameraHardware::initDefaultParameters()
{
	...
	SprdCameraParameters::ConfigType config = (1 & getCameraIndex())
...
//		return UNKNOWN_ERROR;
	...
	if (mHalOem->ops->camera_get_lls_shot_mode(mCameraHandle) && (mParameters.getInt("shot-mode") != 27)) {
		...
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SHOT_NUM, mParameters.getInt("capture-mode"));
	}
	...
	ret = mHalOem->ops->camera_get_preview_rect(mCameraHandle, (cmr_uint*)&preview_rect.x, (cmr_uint*)&preview_rect.y,
	...
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_FOCUS_RECT, (cmr_uint)&focus_param);
	...
	if (0 == getCameraIndex()) {
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_AF_MODE, mParameters.getFocusMode());
		if ((NULL != mParameters.get("flash-mode-supported"))
			...
			SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_FLASH, mParameters.getFlashMode());
			}
		} else {
			SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_FLASH, CAMERA_FLASH_MODE_OFF);
		}
	}

	mTimeCoeff = mParameters.getSlowmotion();
	LOGI("mTimeCoeff:%d",mTimeCoeff);
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_WB, mParameters.getWhiteBalance());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_JPEG_QUALITY, mParameters.getJpegQuality());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_THUMB_QUALITY, mParameters.getJpegThumbnailQuality());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_EFFECT, mParameters.getEffect());
	if (mHalOem->ops->camera_is_lls_enabled(mCameraHandle)) {
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SCENE_MODE, CAMERA_SCENE_MODE_NIGHT);
	} else {
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SCENE_MODE, mParameters.getSceneMode());
	}
	struct cmr_zoom_param zoom_param;
	zoom_param.mode = ZOOM_LEVEL;
	zoom_param.zoom_level = mParameters.getZoom();
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_ZOOM, (cmr_uint)&zoom_param);
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_BRIGHTNESS, mParameters.getBrightness());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SHARPNESS, mParameters.getSharpness());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_CONTRAST, mParameters.getContrast());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SATURATION, mParameters.getSaturation());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_EXPOSURE_COMPENSATION, mParameters.getExposureCompensation());
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_ANTIBANDING, mParameters.getAntiBanding());
	if (mParameters.get_Iso() == NULL) {
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_ISO, 0); /*6 = CAMERA_ISO_MAX*/
	} else {
		...
		SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_ISO, mParameters.getIso());
		}
	}
/*	SET_PARM(mHalOem, CAMERA_PARM_DCDV_MODE, mParameters.getRecordingHint());*/
...
*/
	...
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_FOCAL_LENGTH, (cmr_uint)focal_len);
...
bool SprdCameraHardware::displayOneFrameForCapture(uint32_t width, uint32_t height,  int fd, uintptr_t phy_addr, char *virtual_addr)
{
...
bool SprdCameraHardware::displayOneFrame(uint32_t width, uint32_t height, cmr_s32 fd, uintptr_t phy_addr, char *virtual_addr, uint32_t id)
{
	...
	if (PREVIEW_BUFFER_USAGE_DCAM == mPreviewBufferUsage) {
		...
		if (mCancelBufferEb[id] && (mPreviewCancelBufHandle[id] == mPreviewBufferHandle[id])) {
			...
			if(isPreviewing())
				mHalOem->ops->camera_set_preview_buffer(mCameraHandle, mPreviewHeapArray_phy[id], mPreviewHeapArray_vir[id],mPreviewHeapArray_fd[id]);
			return true;
		} else {
			if (!mPreviewWindow || !mGrallocHal) {
				return false;
...
void SprdCameraHardware::sendPreviewFrameToVideo(struct camera_frame_type *frame)
{
	...
	LOGI("test timestamp = %lld, mIsStoreMetaData: %d. buffer_id 0x%x,y_vir_addr 0x%x,y_phy_addr  0x%x, fd 0x%x",
		timestamp,
		mIsStoreMetaData,
		frame->buf_id, frame->y_vir_addr,frame->y_phy_addr,frame->fd);
	if (mTimeCoeff > 1) {
	...
	if (PREVIEW_ZSL_FRAME == frame->type) {
		if (1 == mZslShotPushFlag) {
			...
			for (i=getZSLQueueFrameNum(); i>mZSLFrameNum; i--) {
				...
				if (!need_pause) {
					mHalOem->ops->camera_set_zsl_buffer(mCameraHandle, zsl_frame.frame.y_phy_addr, zsl_frame.frame.y_vir_addr, (cmr_uint)zsl_frame.frame.fd);
				}
			}
			if (0 == getZSLQueueFrameNum()) {
				...
				Mutex::Autolock zsllock(&mZslBufLock);
				zsl_frame = popZSLQueue();
				mHalOem->ops->camera_set_zsl_snapshot_buffer(mCameraHandle,zsl_frame.frame.y_phy_addr, zsl_frame.frame.y_vir_addr,(cmr_uint)zsl_frame.frame.fd);
				if (!need_pause) {
					mHalOem->ops->camera_set_zsl_buffer(mCameraHandle, zsl_frame.frame.y_phy_addr, zsl_frame.frame.y_vir_addr, (cmr_uint)zsl_frame.frame.fd);
				}
			}
...
void SprdCameraHardware::receivePreviewFrame(struct camera_frame_type *frame)
{
	...
	if (isPreviewing()) {
		...
		if (PREVIEW_FRAME == frame->type) {
			if (!displayOneFrame(width, height, frame->fd ,(uintptr_t)frame->y_phy_addr, (char *)frame->y_vir_addr, frame->buf_id)) {
				mHalOem->ops->camera_set_preview_buffer(mCameraHandle, frame->y_phy_addr, frame->y_vir_addr, frame->fd);
...
void SprdCameraHardware::receiveRawPicture(struct camera_frame_type *frame)
{
    ...
    if (mHalOem->ops->camera_get_lls_shot_mode(mCameraHandle) || mHalOem->ops->camera_is_vendor_hdr(mCameraHandle)) {
		...
		if (iSDisplayCaptureFrame()) {
			...
			if (!displayOneFrameForCapture(dst_width, dst_height, dst_fd, dst_paddr, (char *)mReDisplayHeap->data)) {
...
void SprdCameraHardware::receiveJpegPicture(struct camera_frame_type *frame)
{
	...
	if (mData_cb) {
		...
		if (mHalOem->ops->camera_get_isp_info(mCameraHandle, &isp_info_addr, &isp_info_size)) {
			isp_info_size = 0;
			isp_info_addr = NULL;
		}
		...
		if (1 == mCaptureNum) {
			if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE){
				...
				if((mCaptureMode == CAMERA_ISP_TUNING_MODE) || (!strcmp(debug_value, "debug"))) {
					...
					mHalOem->ops->dump_jpeg_file((void *)mem->data, encInfo->size+isp_info_size, mRawWidth, mRawHeight);
				}
...
void SprdCameraHardware::HandleTakePicture(enum camera_cb_type cb, void* parm4)
{
	...
	case CAMERA_EVT_CB_SNAPSHOT_JPEG_DONE:
		exitFromPostProcess();
	        ...
	        if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
	                LOGE("HandleTakePicture: CAMERA_EXIT_CB_DONE error: oem is null or oem ops is null");
                	break;
        	}
...
void * SprdCameraHardware::switch_monitor_thread_proc(void *p_data)
{
	...
	while (1) {
		...
		if (CMR_MSG_NO_OTHER_MSG == ret) {
			...
			LOGI("message.msg_type 0x%x, sub-type 0x%x",
				message.msg_type,
				message.sub_msg_type);
...
static int HAL_IspVideoStartPreview(uint32_t param1, uint32_t param2)
{
	LOGI("%s", __func__);
	int rtn=0x00;
	SprdCameraHardware * fun_ptr = dynamic_cast<SprdCameraHardware *>((SprdCameraHardware *)g_cam_device[0]->priv);
	if (NULL != fun_ptr) {
