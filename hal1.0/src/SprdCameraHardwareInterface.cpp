...
#define HEAP_COUNTS(n) (n)
#define HEAP_INDEX(i) (i)

#if __WORDSIZE == 64
#define HEAPION_INVALIDPTR 0xFFFFFFFFFFFFFFFF
#else
#define HEAPION_INVALIDPTR 0xFFFFFFFF
#endif
 ...
 * RETURN		: 0 -- success
 *==========================================================================*/
int SprdCameraHardware::getCameraInfo(int cameraId,
...
SprdCameraHardware::SprdCameraHardware(int cameraId)
    : mPreviewHeapSize(0), mVideoHeapSize(0), mZslHeapSize(0),
      mPreviewHeapNum(0), mVideoHeapNum(0), mZslHeapNum(4), /*Total zsl number*/
      mZslChannelStatus(1),
      mZSLFrameNum(2), /*Snapshot, which frame select, 2 means the last 2nd
                          frame will be sent to jpge encode*/
      mPreviewDcamAllocBufferCnt(0), mPreviewHeapArray(NULL),
      mVideoHeapArray(NULL), mZslHeapArray(NULL), mRawHeap(NULL),
      mRawHeapSize(0), mSubRawHeapNum(0), mSubRawHeapSize(0),
      mPathRawHeapNum(0), mPathRawHeapSize(0), mFDAddr(0), mMetadataHeap(NULL),
      mParameters(), mSetParameters(), mSetParametersBak(),
      mPreviewHeight_trimy(0), mPreviewWidth_trimx(0), mPreviewHeight_backup(0),
      mPreviewWidth_backup(0), mPreviewHeight(-1), mPreviewWidth(-1),
      mRawHeight(-1), mRawWidth(-1), mPreviewFormat(1), mPictureFormat(1),
      mPreviewStartFlag(0), mIsDvPreview(0),
      //#if defined(CONFIG_CAMERA_PREVIEW_YV12)
      mIsYuv420p(0), mYV12Buf(NULL),
      //#endif
      mRecordingMode(0), mBakParamFlag(0), mRecordingFirstFrameTime(0),
      mZoomLevel(0), mJpegSize(0), mNotify_cb(0), mData_cb(0),
      mData_cb_timestamp(0), mGetMemory_cb(0), mUser(0), mPreviewWindow(NULL),
      mMsgEnabled(0), mIsStoreMetaData(false), mIsFreqChanged(false),
      mCameraId(cameraId), miSPreviewFirstFrame(1),
      mCaptureMode(CAMERA_ZSL_MODE), mCaptureRawMode(0),
      ...
      mPreviewBufferUsage(PREVIEW_BUFFER_USAGE_GRAPHICS), mSetDDRFreqCount(0),
      mSetDDRFreq(NO_FREQ_REQ), mCaptureNum(1), mCPURaised(0),
      mSwitchMonitorMsgQueHandle(0), mSwitchMonitorInited(0),
    ...
    memset(mPreviewHeapArray_fd, 0, sizeof(mPreviewHeapArray_fd));
#if (MINICAMERA != 1)
    ...
    if (!mHalOem) {
        ...
        LOGV("loaded HAL libcamoem handle=%p", handle);
    }
...
SprdCameraHardware::~SprdCameraHardware() {
    if (mIsPerformanceTestable) {
        sprd_startPerfTracking("closeCameraHardware: E cameraId: %d.",
                               mCameraId);
    } else {
    ...
    if (mIsPerformanceTestable) {
        sprd_stopPerfTracking("closeCameraHardware: X cameraId: %d.",
                              mCameraId);
    } else {
...
void SprdCameraHardware::release() {
    ...
    if (isCameraInit()) {
        ...
        if (NULL != mHalOem && NULL != mHalOem->ops &&
            CMR_CAMERA_SUCCESS != mHalOem->ops->camera_deinit(mCameraHandle)) {
    ...
    /*	if (false == mPreviewHeapInfoBak.busy_flag) {
                    ...
                    memset(&mPreviewHeapInfoBak, 0,
       sizeof(mPreviewHeapInfoBak));
            } else {
...
int SprdCameraHardware::getCameraId() const { return mCameraId; }

int SprdCameraHardware::getCameraIndex() {
    if (1 == getPropertyAtv()) {
        return mCameraId;
    } else {
        int vindex = 0;
...
status_t SprdCameraHardware::startPreview() {
    ...
    bool isRecordingMode =
        (mMsgEnabled & CAMERA_MSG_VIDEO_FRAME) > 0 ? true : false;
...
void SprdCameraHardware::stopPreview() {
    ...
    if (NULL == mParameters.get("video-size-values")) {
        if (isRecordingMode()) {
            LOGW("mLock: stopPreview: X. can't stop preview in recording for "
                 "no-video-size case! direct return!");
            return;
        }
    }
...
status_t SprdCameraHardware::setPreviewWindow(preview_stream_ops *w) {
    ...
    if (!w) {
        switch_ret =
            switchBufferMode(mPreviewBufferUsage, PREVIEW_BUFFER_USAGE_DCAM);
        if (switch_ret) {
            ret = NO_ERROR;
        } else {
        ...
        mPreviewWindowLock.unlock();
        switch_ret =
            switchBufferMode(mPreviewBufferUsage, mOriginalPreviewBufferUsage);
        if (!switch_ret) {
            ret = INVALID_OPERATION;
    ...
    if (min_bufs >= kPreviewBufferCount) {
        LOGE("%s: min undequeued buffer count %d is too high (expecting at "
             "most %d)",
             __func__, min_bufs, kPreviewBufferCount - 1);
    }
    ...
    if (mParameters.getRecordingHint()) {
        mIsDvPreview = 1;
        if (w->set_buffers_geometry(w, SIZE_ALIGN(preview_width),
                                    SIZE_ALIGN(preview_height),
                                    hal_pixel_format)) {
            LOGE("%s: could not set buffers geometry to %s X", __func__,
                 str_preview_format);
            mPreviewWindowLock.unlock();
            return INVALID_OPERATION;
        }
    ...
    if (w->set_crop(w, 0, 0, preview_width - 1, preview_height - 1)) {
        LOGE("%s: could not set crop to %s X", __func__, str_preview_format);
        mPreviewWindowLock.unlock();
        return INVALID_OPERATION;
    }
    mPreviewWindowLock.unlock();
...
status_t SprdCameraHardware::takePicture() {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("takePicture: oem is null or oem ops is null");
    ...
    if (!iSZslMode()) {
        ...
        if (mIsPerformanceTestable) {
            sprd_perfInfo(
                "ok to stopPreviewInternal in takePicture. preview state = %d",
                getPreviewState());
        } else {
            LOGI("ok to stopPreviewInternal in takePicture. preview state = %d",
                 getPreviewState());
        }

        if (isPreviewing()) {
            LOGE("takePicture: stop preview error!, preview state = %d",
                 getPreviewState());
            return UNKNOWN_ERROR;
        }
        if (!initCapture()) {
    ...
    if (CAMERA_ISP_TUNING_MODE == mode) {
        ...
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SCENE_MODE, 0);

        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_JPEG_QUALITY, 95);
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_THUMB_QUALITY, 0);
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_THUMB_SIZE,
                 (cmr_uint)&req_size);

        if (!mHalOem->ops->camera_get_sensor_info_for_raw(mCameraHandle,
                                                          mode_info)) {
            for (i = SENSOR_MODE_PREVIEW_ONE; i < SENSOR_MODE_MAX; i++) {
                LOGI("trim w=%d, h=%d", mode_info[i].trim_width,
    ...
    LOGI("INTERPOLATION::takePicture:mRawWidth=%d,mZoomLevel=%d", mRawWidth,
         mZoomLevel);
    if (mCameraHandle == NULL || mHalOem == NULL || mHalOem->ops == NULL ||
        CMR_CAMERA_SUCCESS !=
            mHalOem->ops->camera_take_picture(mCameraHandle, mode)) {
...
status_t SprdCameraHardware::setTakePictureSize(uint32_t width,
                                                uint32_t height) {
    mRawWidth = width;
    mRawHeight = height;
...
status_t SprdCameraHardware::waitSetParamsOK() {
    ...
    while (SPRD_IDLE != getSetParamsState() || mBakParamFlag) {
        ...
        if (i_count < SET_PARAMS_TIMEOUT) {
            ...
            LOGE(
                "waitSetParamsOK timeout to wait set Parameter OK, skip that!");
            ret = TIMED_OUT;
            break;
        }
    }
...
status_t SprdCameraHardware::startRecording() {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("startRecording: oem is null or oem ops is null");
        return UNKNOWN_ERROR;
    }

    if (isPreviewing()) {
        if (mHalOem->ops->camera_is_need_stop_preview(mCameraHandle) ||
            (isZslSupport && (0 == strcmp("true", isZslSupport)) &&
...
void SprdCameraHardware::releaseRecordingFrame(const void *opaque) {
    ...
    if (!isPreviewing() || NULL == mHalOem || NULL == mHalOem->ops ||
        NULL == mCameraHandle) {
    ...
    if (PREVIEW_BUFFER_USAGE_DCAM == mPreviewBufferUsage) {
        ...
        if (index >= mPreviewHeapNum) {
            LOGE("releaseRecordingFrame error: index: %d, data: %lx, w=%d, "
                 "h=%d \n",
                 index, (unsigned long)addr, mPreviewWidth, mPreviewHeight);
            return;
        }

        flush_buffer(CAMERA_FLUSH_PREVIEW_HEAP, index, (void *)vaddr,
                     (void *)paddr, (int)mPreviewHeapSize);

        // camera_release_frame(mCameraHandle, CAMERA_VIDEO_DATA, index);
        if (isPreviewing())
            mHalOem->ops->camera_set_preview_buffer(
                mCameraHandle, mPreviewHeapArray_phy[index],
                mPreviewHeapArray_vir[index], mPreviewHeapArray_fd[index]);
    } else {
        Mutex::Autolock pgbcl(&mGraphBufCntLock);
        if (isRecordingMode()) {
            mGraphBufferCount[index]--;
            LOGV("releasePreviewFrame 0x%x count %d", index,
                 mGraphBufferCount[index]);
...
status_t SprdCameraHardware::autoFocus() {
    ...
    if (mCameraHandle != NULL && mHalOem != NULL && mHalOem->ops != NULL &&
        0 != mHalOem->ops->camera_start_autofocus(mCameraHandle)) {
...
status_t SprdCameraHardware::checkSetParametersEnvironment() {
    ...
    if (isCapturing()) {
        LOGE("warning, camera HAL in capturing process, abnormal calling "
             "sequence!");
    ...
    if ((SPRD_IDLE != getPreviewState()) &&
        ...
        LOGE("camera HAL in preview changing process, not allow to "
             "setParameter");
        return PERMISSION_DENIED;
    }
...
status_t
SprdCameraHardware::copyParameters(SprdCameraParameters &cur_params,
    ...
    {
        const char *new_ExposureCompensationStep =
            params.get_ExposureCompensationStep();
        if (new_ExposureCompensationStep)
            cur_params.setExposureCompensationStep(
                new_ExposureCompensationStep);
    }
    /*MaxExposureCompensation*/
    {
        const char *new_MaxExposureCompensation =
            params.get_MaxExposureCompensation();
    ...
    {
        const char *new_MinExposureCompensation =
            params.get_MinExposureCompensation();
    ...
    {
        const char *new_SupportedPreviewSizes =
            params.get_SupportedPreviewSizes();
    ...
    {
        const char *new_SupportedPreviewFrameRate =
            params.get_SupportedPreviewFrameRate();
        if (new_SupportedPreviewFrameRate)
            cur_params.setSupportedPreviewFrameRate(
                new_SupportedPreviewFrameRate);
    }

    /*SupportedPreviewFpsRange*/
    {
        const char *new_SupportedPreviewFpsRange =
            params.get_SupportedPreviewFpsRange();
        if (new_SupportedPreviewFpsRange)
            cur_params.setSupportedPreviewFpsRange(
                new_SupportedPreviewFpsRange);
    }

    /*SupportedPictureSizes*/
    {
        const char *new_SupportedPictureSizes =
            params.get_SupportedPictureSizes();
    ...
    {
        const char *new_AutoExposureLockSupported =
            params.get_AutoExposureLockSupported();
        if (new_AutoExposureLockSupported)
            cur_params.setAutoExposureLockSupported(
                new_AutoExposureLockSupported);
    }

    /*AutoWhiteBalanceLock*/
    {
        const char *new_AutoWhiteBalanceLock =
            params.get_AutoWhiteBalanceLock();
    ...
    {
        const char *new_AutoWhiteBalanceLockSupported =
            params.get_AutoWhiteBalanceLockSupported();
        if (new_AutoWhiteBalanceLockSupported)
            cur_params.setAutoWhiteBalanceLockSupported(
                new_AutoWhiteBalanceLockSupported);
    }
    ...
    {
        const char *new_thumbnail_size_val =
            params.get_JpegThumbnailSizeValue();
    ...
    {
        const char *new_GPSProcessingMethod =
            params.get_GPS_Processing_Method();
    ...
    {
        const char *new_MaxNumDetectedFacesHW =
            params.get_MaxNumDetectedFacesHW();
    ...
    {
        const char *new_MaxNumDetectedFacesSW =
            params.get_MaxNumDetectedFacesSW();
    ...
    {
        const char *new_SupportedAntibanding =
            params.get_SupportedAntibanding();
    ...
    {
        const char *new_SupportedPictureFormat =
            params.get_SupportedPictureFormat();
    ...
    {
        const char *new_SupportedWhiteBalance =
            params.get_SupportedWhiteBalance();
        if (new_SupportedWhiteBalance)
            cur_params.setSupportedWhiteBalance(new_SupportedWhiteBalance);
    }

    /*supported auto exposeure values*/
    const char *new_Supported_auto_expos_valuses =
        params.get_Supported_Auto_Exposure_Values();
    if (new_Supported_auto_expos_valuses)
        cur_params.set_Supported_Auto_Exposure_Values(
            new_Supported_auto_expos_valuses);
    ...
    {
        const char *new_exposure_Compensation =
            params.get_ExposureCompensation();
    ...
    {
        const char *new_VideoStabilitionSupported =
            params.get_VideoStabilitionSupported();
        if (new_VideoStabilitionSupported)
            cur_params.setVideoStabilitionSupported(
                new_VideoStabilitionSupported);
    }
    /*FocusDistances*/
    {
    ...
    /*sensor rotation (sensor rotation should be worked with sensor
     * orientation)*/
    {
    ...
    /*sensor orientation (sensor orientation should be worked with sensor
     * rotation)*/
    {
...
status_t
SprdCameraHardware::checkSetParameters(const SprdCameraParameters &params,
    ...
    if (NULL == mParameters.get("video-size-values")) {
        if (isRecordingMode()) {
            if (mPreviewWidth != w || mPreviewHeight != h) {
                ((SprdCameraParameters &)params)
                    .setPreviewSize(mPreviewWidth, mPreviewHeight);
                LOGW("should not change preview size in recording for "
                     "no-video-size!");
            }
            params.getPreviewSize(&w, &h);
        }
...
status_t SprdCameraHardware::checkFlashParameter(SprdCameraParameters &params) {
    ...
    if (0 == strcmp("hdr", params.get_SceneMode())) {
        ...
        if (0 ==
            strcmp("true", (char *)mParameters.get("flash-mode-supported")))
...
status_t
SprdCameraHardware::checkEffectParameter(SprdCameraParameters &params) {
    status_t ret = NO_ERROR;
...
status_t
SprdCameraHardware::setParametersInternal(const SprdCameraParameters &params) {
    status_t ret = NO_ERROR;
    ...
    if (NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("setParametersInternal: oem is null or oem ops is null");
        return UNKNOWN_ERROR;
    }
    ...
    LOGI("setParametersInternal: mPreviewFormat=%d,mPictureFormat=%d.",
         mPreviewFormat, mPictureFormat);

    if (0 == checkSetParameters(params, mParameters)) {
        LOGI("setParametersInternal X: same parameters with system, directly "
             "return!");
        mParamLock.unlock();
        return NO_ERROR;
    }
    ...
    if (NO_ERROR != ret) {
        ...
        if (!mBakParamFlag) {
            ...
            LOGW("setParametersInternal cannot set parameters, but not wait in "
                 "capturing");
    ...
    LOGI("recording-hint %d zsl %d", mParameters.getRecordingHint(),
    ...
    if ((1 == mParameters.getRecordingHint()) && isPreviewing() &&
        oriZsl != mParameters.getInt("zsl")) {
        isZslSupport = (char *)mParameters.get("zsl-supported");
        if ((isZslSupport) && (0 == strcmp("true", isZslSupport))) {
            LOGI("setParametersInternal in dc previewing, in zsl param is %d, "
                 "set to 1",
                 mParameters.getInt("zsl"));
    ...
    LOGI("setParametersInternal: requested preview size %d x %d", width,
         height);
    ...
    LOGI("setParametersInternal:requested picture size %d x %d", rawWidth,
         rawHeight);
    ...
    (cmr_uint)&req_size);
            ...
            SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_CAPTURE_SIZE,
    (cmr_uint)&req_size);
            LOGI("setParametersInternal:video requested picture size %d x %d",
    rawWidth, rawHeight);
            LOGI("setParametersInternal:requested video size %d x %d",
    ...
    // mPreviewHeight = (height + 1) & ~1;
    antiShakeParamSetup();
    LOGI("setParametersInternal: requested picture size %d x %d  preview size "
         "%d x %d",
         mRawWidth, mRawHeight, mPreviewWidth, mPreviewHeight);

    cmr_uint is_change = 0;
    mHalOem->ops->camera_is_change_size(mCameraHandle, mRawWidth, mRawHeight,
                                        mPreviewWidth, mPreviewHeight,
                                        mVideoWidth, mVideoHeight, &is_change);
    if (is_change) {
        if (isPreviewing()) {
            mPreviewLock.lock();
            LOGI(
                "setParametersInternal preview or ZSL size should be changed!");
            stopPreviewInternal();
            if (NO_ERROR != setPreviewWindow(mPreviewWindow)) {
                LOGE("setParametersInternal X: setPreviewWindow fail, unknown "
                     "error!");
                ret = UNKNOWN_ERROR;
                mPreviewLock.unlock();
                goto setParamEnd;
            ...
            if (NO_ERROR != startPreviewInternal(isRecordingMode())) {
                LOGE("setParametersInternal X: change size "
                     "startPreviewInternal fail, unknown error!");
                ret = UNKNOWN_ERROR;
                mPreviewLock.unlock();
                goto setParamEnd;
            } else {
    ...
    if ((1 == mParameters.getInt("zsl")) && (mCaptureMode != CAMERA_ZSL_MODE)) {
        if (isPreviewing()) {
            ...
            stopPreviewInternal();
            if (NO_ERROR != startPreviewInternal(isRecordingMode())) {
                LOGE("setParametersInternal X: open ZSL startPreviewInternal "
                     "fail, unknown error!");
                ret = UNKNOWN_ERROR;
                mPreviewLock.unlock();
                goto setParamEnd;
    ...
    if ((oldCaptureMode != curCaptureMode) &&
        ...
        if (isPreviewing()) {
            ...
            stopPreviewInternal();
            if (NO_ERROR != startPreviewInternal(isRecordingMode())) {
                LOGE("setParametersInternal X: close ZSL startPreviewInternal "
                     "fail, unknown error!");
                ret = UNKNOWN_ERROR;
                mPreviewLock.unlock();
                goto setParamEnd;
...
void SprdCameraHardware::setCallbacks(
...
status_t SprdCameraHardware::sendCommand(int32_t cmd, int32_t arg1,
                                         int32_t arg2) {
    uint32_t addr = 0;
    status_t ret = NO_ERROR;
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("sendCommand: oem is null or oem ops is null");
        return UNKNOWN_ERROR;
    }

    if (CAMERA_CMD_START_FACE_DETECTION == cmd) {
        if (0 == mParameters.getInt("max-num-detected-faces-hw")) {
            LOGE("sendCommand: not support the "
        ...
        if (0 == mParameters.getInt("max-num-detected-faces-hw")) {
            LOGE(
                "sendCommand: not support the CAMERA_CMD_STOP_FACE_DETECTION.");
            mHalOem->ops->camera_fd_enable(mCameraHandle, 0);
            ret = BAD_VALUE;
            goto sendCommand_end;
        } else {
        ...
        mHalOem->ops->camera_start_burst_notice(mCameraHandle);
    } else if (SPRD_CMD_DELETE_BURST_TAKE == cmd) {
        LOGD("sendCommand: delete burst take");
        mHalOem->ops->camera_end_burst_notice(mCameraHandle);
    } else if (SPRD_CMD_STOP_BURST_TAKE == cmd) {
        LOGD("sendCommand: stop burst take");
        mHalOem->ops->camera_end_burst_notice(mCameraHandle);
    } else {
        mHalOem->ops->camera_flip_enable(mCameraHandle, arg1);
    }
sendCommand_end:
...
status_t SprdCameraHardware::storeMetaDataInBuffers(bool enable) {
    ...
    if (NULL == mMetadataHeap) {
        if (NULL == (mMetadataHeap = mGetMemory_cb(
                         -1, METADATA_SIZE, kPreviewBufferCount, NULL))) {
            LOGE("fail to alloc memory for the metadata for "
...
const char *
SprdCameraHardware::getCameraStateStr(SprdCameraHardware::Sprd_camera_state s) {
...
void SprdCameraHardware::setCameraPreviewMode(bool isRecordMode) {
    ...
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_PREVIEW_FPS,
             (cmr_uint)&fps_param);
    ...
    LOGI("setCameraPreviewMode:state: %d, owner: %d", ae_fps_param.min_fps,
         ae_fps_param.max_fps);
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_RANGE_FPS,
             (cmr_uint)&ae_fps_param);
}

bool SprdCameraHardware::isRecordingMode() { return mRecordingMode; }

void SprdCameraHardware::setRecordingMode(bool enable) {
    mRecordingMode = enable;
}

void SprdCameraHardware::setCameraState(Sprd_camera_state state,
                                        state_owner owner) {
    ...
    LOGI("setCameraState:state: %s, owner: %d", getCameraStateStr(state),
         owner);
    switch (owner) {
    case STATE_CAMERA:
    ...
    if (mIsPerformanceTestable) {
        ...
        LOGI("setCameraState: X camera state = %s, preview state = %s, capture "
...
bool SprdCameraHardware::isCapturing() {
    ...
    if ((SPRD_INTERNAL_RAW_REQUESTED == mCameraState.capture_state) ||
        ...
        ret = true;
    } else if ((SPRD_INTERNAL_CAPTURE_STOPPING_DONE ==
                mCameraState.capture_state) ||
               (SPRD_ERROR == mCameraState.capture_state)) {
    ...
    if ((SPRD_INTERNAL_CAPTURE_STOPPING == tmpState) ||
        ...
        if (iSZslMode()) {
            if (SPRD_PREVIEW_IN_PROGRESS != tmpState) {
                LOGI("incorrect preview status %d of ZSL capture mode",
                     (uint32_t)tmpState);
                ret = false;
            }
        } else {
            if (SPRD_IDLE != tmpState) {
                LOGI("incorrect preview status %d of normal capture mode",
                     (uint32_t)tmpState);
                ret = false;
            }
        }
...
bool SprdCameraHardware::WaitForCaptureStart() {
    ...
    while (SPRD_WAITING_RAW != mCameraState.capture_state &&
           ...
           SPRD_ERROR != mCameraState.capture_state &&
           SPRD_ERROR != mCameraState.camera_state) {
        LOGI("WaitForCaptureStart: waiting for SPRD_WAITING_RAW or "
             "SPRD_WAITING_JPEG");
        mStateWait.wait(mStateLock);
...
bool SprdCameraHardware::WaitForCaptureJpegState() {
    if (mIsPerformanceTestable) {
        ...
        LOGD(" WaitForCaptureJpegState E.\n");
    }
    Mutex::Autolock stateLock(&mStateLock);
    ...
    if (mIsPerformanceTestable) {
        ...
        LOGD(" WaitForCaptureJpegState X.\n");
    }
...
bool SprdCameraHardware::startCameraIfNecessary() {
    ...
    if (NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("startCameraIfNecessary: oem is null or oem ops is null");
        return false;
    }
    if (!isCameraInit()) {
        LOGI("waiting for camera_init to initialize.startCameraIfNecessary");
        if (CMR_CAMERA_SUCCESS !=
            mHalOem->ops->camera_init(getCameraIndex(), camera_cb, this, 0,
        ...
        /*if (CAMERA_SUCCESS != camera_start(camera_cb, this, mPreviewHeight,
        mPreviewWidth)) {
                setCameraState(SPRD_ERROR, STATE_CAMERA);
    ...
    ret = mHalOem->ops->camera_get_viewangle(mCameraHandle, &view_angle);
    if (!ret) {
...
void SprdCameraHardware::cancelPreviewMemFromGraphics(cmr_u32 sum) {
    ...
    if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage &&
        ...
        cnt = ((cmr_int)sum > kPreviewBufferCount) ? kPreviewBufferCount
                                                   : (cmr_int)sum;
        for (i = 0; i < (cmr_int)cnt; i++) {
            if (mPreviewBufferHandle[i]) {
                if (0 !=
                    mPreviewWindow->cancel_buffer(mPreviewWindow,
                                                  mPreviewBufferHandle[i])) {
                    LOGE("cancelPreviewMemFromGraphics: cancel_buffer error id "
                         "%ld",
                         i);
                }
...
bool SprdCameraHardware::allocatePreviewMemFromGraphics(cmr_u32 size,
                                                        cmr_u32 sum,
                                                        cmr_uint *phy_addr,
                                                        cmr_uint *vir_addr,
                                                        cmr_s32 *fd) {
    if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage) {
        ...
        if (0 !=
            mPreviewWindow->get_min_undequeued_buffer_count(mPreviewWindow,
                                                            &miniUndequeued)) {
        ...
        if (miniUndequeued >= (int)sum) {
            LOGE(
                "allocatePreviewMemFromGraphics: minUndequeued value error: %d",
                miniUndequeued);
            return -1;
        }
        ...
        for (i = 0; i < (int)sum; i++) {
            if (0 !=
                mPreviewWindow->dequeue_buffer(mPreviewWindow, &buffer_handle,
                                               &stride)) {
            ...
            LOGI("get buffer handle 0x%lx buffer_handle 0x%x",
                 (unsigned long)private_h, buffer_handle);
            if (NULL == private_h) {
            ...
            }
            {
                ...
                mPreviewHeapArray_fd[i] = private_h->share_fd;
                *phy_addr++ = (cmr_uint)iova_addr;
                *vir_addr++ = (cmr_uint)private_h->base;
                *fd++ = private_h->share_fd;
            }
            LOGI(" allocatePreviewMemFromGraphics: fd:0x%lx, base:0x%lx, "
                 "size:0x%x, stride:0x%x ",
                 mPreviewHeapArray_fd[i], (uintptr_t)private_h->base,
        ...
        for (i = ((int)sum - miniUndequeued); i < (int)sum; i++) {
            if (0 !=
                mPreviewWindow->cancel_buffer(mPreviewWindow,
                                              mPreviewBufferHandle[i])) {
                LOGE("allocatePreviewMemFromGraphics: cancel_buffer error: %d",
                     i);
            } else
...
int SprdCameraHardware::Callback_PreviewMalloc(cmr_u32 size, cmr_u32 sum,
                                               cmr_uint *phy_addr,
                                               cmr_uint *vir_addr,
                                               cmr_s32 *fd) {
    ...
    *fd = 0;
    ...
    mPreviewHeapNum = sum;
    if (allocatePreviewMemFromGraphics(size, kPreviewBufferCount, phy_addr,
                                       vir_addr, fd)) {
    ...
    LOGI("Callback_PreviewMalloc DcamAllocBufferCnt %d, mPreviewHeapArray %p, "
         "start_id %ld, end_id %ld real cnt %d",
         mPreviewDcamAllocBufferCnt, mPreviewHeapArray, buffer_start_id,
         buffer_end_id, HEAP_COUNTS(mPreviewDcamAllocBufferCnt));

    if (mPreviewDcamAllocBufferCnt > 0 && NULL == mPreviewHeapArray) {
        mPreviewHeapArray = (sprd_camera_memory_t **)malloc(
            mPreviewDcamAllocBufferCnt * sizeof(sprd_camera_memory_t *));
        if (mPreviewHeapArray == NULL) {
            return false;
        } else {
            memset(&mPreviewHeapArray[0], 0,
                   mPreviewDcamAllocBufferCnt * sizeof(sprd_camera_memory_t *));
        }
        ...
        for (i = buffer_start_id; i < buffer_end_id; i++) {
            ...
            if (NULL == PreviewHeap) {
                LOGE("Callback_PreviewMalloc: error PreviewHeap is null, index "
                     "%ld",
                     i);
                return false;
            }
            ...
            if (PreviewHeap->phys_addr & 0xFF) {
                LOGE("Callback_PreviewMalloc: error mPreviewHeap is not 256 "
                     "bytes aligned, index %ld",
                     i);
            ...
            if (i < sum) {
                ...
                *fd++ = PreviewHeap->fd;
                LOGI("Callback_PreviewMalloc: DCAM %ld, phys_addr 0x%x fd 0x%x",
                     i, PreviewHeap->phys_addr, PreviewHeap->fd);
            }
        }

        if (0 == strcmp(mParameters.getPreviewFormat(),
                        "yuv420p")) { // bug 440213 for cts verifier
            uint32_t y_stride = 0, y_size = 0, uv_stride = 0, uv_size = 0,
                     frame_size = 0;
            ...
            sprd_camera_memory_t *PreviewHeap =
                allocCameraMem(frame_size, true);
            if (NULL == PreviewHeap) {
                LOGE("allocatePreviewMem: 420p error PreviewHeap is null, "
                     "index=%d",
                     i);
                return false;
            }
            ...
            if (PreviewHeap->phys_addr & 0xFF) {
                LOGE("allocatePreviewMem: 420p error mPreviewHeap is not 256 "
                     "bytes aligned, index=%d",
                     i);
                freeCameraMem(PreviewHeap);
                freePreviewMem();
                return false;
...
int SprdCameraHardware::Callback_VideoMalloc(cmr_u32 size, cmr_u32 sum,
                                             cmr_uint *phy_addr,
                                             cmr_uint *vir_addr, cmr_s32 *fd) {
    cmr_uint buffer_start_id = 0, buffer_end_id = 0;
    cmr_uint i = 0, j = 0;
    sprd_camera_memory_t *VideoHeap;
    ...
    *fd = 0;
    ...
    if (NULL == mVideoHeapArray) {
        mVideoHeapArray = (sprd_camera_memory_t **)malloc(
            mVideoHeapNum * sizeof(sprd_camera_memory_t *));
        if (mVideoHeapArray == NULL) {
            LOGE("Callback_VideoMalloc: error mVideoHeapArray is null");
            return false;
        } else {
            memset(&mVideoHeapArray[0], 0,
                   mVideoHeapNum * sizeof(sprd_camera_memory_t *));
        }
        ...
        for (i = buffer_start_id; i < buffer_end_id; i++) {
            ...
            if (NULL == VideoHeap) {
                LOGE("Callback_VideoMalloc: error PreviewHeap is null, index "
                     "%ld",
                     i);
                return false;
            }
            ...
            mVideoHeapArray_fd[i] = VideoHeap->fd;
            if (i < sum) {
                *phy_addr++ = (cmr_uint)VideoHeap->phys_addr;
                *vir_addr++ = (cmr_uint)VideoHeap->data;
                *fd++ = (cmr_s32)VideoHeap->fd;
                LOGI("Callback_VideoMalloc: DCAM %ld, fd 0x%x", i,
                     VideoHeap->fd);
            }
        }
    }
...
void SprdCameraHardware::setZslBuffers(void) {
    ...
    for (i = 0; i < mZslHeapNum; i++) {
        mHalOem->ops->camera_set_zsl_buffer(
            mCameraHandle, (cmr_uint)mZslHeapArray[i]->phys_addr,
            (cmr_uint)mZslHeapArray[i]->data, (cmr_uint)mZslHeapArray[i]->fd);
    }
}

int SprdCameraHardware::Callback_ZslMalloc(cmr_u32 size, cmr_u32 sum,
                                           cmr_uint *phy_addr,
                                           cmr_uint *vir_addr, cmr_s32 *fd) {
    cmr_uint i = 0, j = 0;
    int ret = 0;
    ...
    if (NULL == mZslHeapArray) {
        releaseZSLQueue();
        mZslHeapArray = (sprd_camera_memory_t **)malloc(
            mZslHeapNum * sizeof(sprd_camera_memory_t *));
        if (mZslHeapArray == NULL) {
            LOGE("Callback_ZslMalloc: error mZslHeapArray is null");
            return false;
        }
        memset(&mZslHeapArray[0], 0,
               mZslHeapNum * sizeof(sprd_camera_memory_t *));
...
int SprdCameraHardware::Callback_CaptureMalloc(cmr_u32 size, cmr_u32 sum,
                                               cmr_uint *phy_addr,
                                               cmr_uint *vir_addr,
                                               cmr_s32 *fd) {
    ...
    LOGI("Callback_CaptureMalloc: size %d sum %d mSubRawHeapNum %d "
         "mSubRawHeapSize %d",
         size, sum, mSubRawHeapNum, mSubRawHeapSize);
    ...
    *fd = 0;
    ...
    if ((mSubRawHeapNum + sum) >= MAX_SUB_RAWHEAP_NUM) {
        LOGE("Callback_CaptureMalloc: malloc is too more %d %d", mSubRawHeapNum,
             sum);
        return -1;
    }
    if (0 == mSubRawHeapNum) {
...
int SprdCameraHardware::Callback_CapturePathMalloc(cmr_u32 size, cmr_u32 sum,
                                                   cmr_uint *phy_addr,
                                                   cmr_uint *vir_addr,
                                                   cmr_s32 *fd) {
    sprd_camera_memory_t *memory = NULL;
...
int SprdCameraHardware::Callback_OtherMalloc(enum camera_mem_cb_type type,
    ...
    *fd = 0;
    ...
    if (type == CAMERA_PREVIEW_RESERVED || type == CAMERA_VIDEO_RESERVED ||
        ...
        if (NULL == mCommonHeapReserved) {
            buffer_id =
                mHalOem->ops->camera_pre_capture_get_buffer_id(mCameraId);
            if (mHalOem->ops->camera_get_reserve_buffer_size(
                    mCameraId, buffer_id, &mem_size, &mem_sum) !=
                CMR_CAMERA_SUCCESS) {
            ...
            *fd++ = memory->fd;
            ...
            LOGI("malloc Common memory for preview, video, and zsl, malloced "
        ...
        if (mIspLscHeapReserved == NULL) {
            ...
            mIspLscHeapReserved = (sprd_camera_memory_t *)memory;
        } else {
        ...
        *vir_addr++ = (cmr_uint)memory->data;
        *fd++ = memory->fd;
        LOGI("tony malloc Video memory,  mfd  0x%x", memory->fd);
        ...
        if (mIspAntiFlickerHeapReserved == NULL) {
            ...
            mIspAntiFlickerHeapReserved = (sprd_camera_memory_t *)memory;
        } else {
            LOGI("malloc Video memory, malloced type %d,request num %d, "
                 "request size 0x%x",
                 type, sum, size);
            memory = (sprd_camera_memory_t *)mIspAntiFlickerHeapReserved;
        }
        ...
        *fd++ = memory->fd;
        ...
        for (i = 0; i < sum; i++) {
            ...
            *phy_addr++ = kaddr; //(cmr_uint)memory->phys_addr;
            *phy_addr = kaddr >> 32;
            *fd++ = memory->fd;
        }
    } else if (type == CAMERA_ISP_ANTI_FLICKER) {
...
int SprdCameraHardware::Callback_Malloc(enum camera_mem_cb_type type,
    ...
    uint32_t size, sum;
    LOGD("start %d", ret);
    if (!private_data || !vir_addr || !size_ptr || !sum_ptr ||
        (0 == *size_ptr) || (0 == *sum_ptr) || !fd) {
        LOGE("error param 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx", fd,
...
int SprdCameraHardware::Callback_PdafRawFree(cmr_uint *phy_addr,
...
int SprdCameraHardware::Callback_PdafRawMalloc(cmr_u32 size, cmr_u32 sum,
                                               cmr_uint *phy_addr,
                                               cmr_uint *vir_addr,
                                               cmr_s32 *fd) {
    sprd_camera_memory_t *memory = NULL;
    ...
    *vir_addr = 0;
    ...
    if (sum >= kPdafRawBufferCount) {
        ...
        LOGD("Do not need malloc, malloced num %d,request num %d, request size "
             "0x%x",
...
int SprdCameraHardware::Callback_PreviewFree(cmr_uint *phy_addr,
                                             cmr_uint *vir_addr, cmr_s32 *fd,
                                             cmr_u32 sum) {
    cmr_u32 i, j = 0;
#if 0 // wjp
...
int SprdCameraHardware::Callback_VideoFree(cmr_uint *phy_addr,
                                           cmr_uint *vir_addr, cmr_s32 *fd,
                                           cmr_u32 sum) {
    cmr_u32 i;
...
int SprdCameraHardware::Callback_ZslFree(cmr_uint *phy_addr, cmr_uint *vir_addr,
...
int SprdCameraHardware::Callback_CaptureFree(cmr_uint *phy_addr,
                                             cmr_uint *vir_addr, cmr_s32 *fd,
                                             cmr_u32 sum) {
    cmr_u32 i;
...
int SprdCameraHardware::Callback_CapturePathFree(cmr_uint *phy_addr,
                                                 cmr_uint *vir_addr,
                                                 cmr_s32 *fd, cmr_u32 sum) {
    cmr_u32 i;
...
int SprdCameraHardware::Callback_OtherFree(enum camera_mem_cb_type type,
                                           cmr_uint *phy_addr,
                                           cmr_uint *vir_addr, cmr_s32 *fd,
                                           cmr_u32 sum) {
    cmr_u32 i;
...
int SprdCameraHardware::Callback_Free(enum camera_mem_cb_type type,
                                      cmr_uint *phy_addr, cmr_uint *vir_addr,
                                      cmr_s32 *fd, cmr_u32 sum,
                                      void *private_data) {
    SprdCameraHardware *camera = (SprdCameraHardware *)private_data;
    ...
    if (!private_data || !vir_addr || !fd) {
        LOGE("error param 0x%lx 0x%lx 0x%x", fd, (cmr_uint)vir_addr,
    ...
    if (CAMERA_PREVIEW == type) {
        ...
        ret = camera->Callback_PdafRawFree(phy_addr, vir_addr, sum);
               ...
               CAMERA_SNAPSHOT_ZSL_RESERVED == type || CAMERA_ISP_LSC == type ||
               CAMERA_ISP_BINGING4AWB == type ||
               CAMERA_ISP_ANTI_FLICKER == type || CAMERA_ISP_RAWAE == type ||
               CAMERA_SNAPSHOT_HIGHISO == type || CAMERA_ISP_RAW_DATA == type ||
...
sprd_camera_memory_t *SprdCameraHardware::allocReservedMem(int buf_size,
                                                           uint32_t is_cache) {
...
sprd_camera_memory_t *SprdCameraHardware::allocCameraMem(int buf_size,
                                                         uint32_t is_cache) {
    sprd_camera_memory_t *memory =
        (sprd_camera_memory_t *)malloc(sizeof(*memory));
    MemIon *pHeapIon = NULL;
    ...
    LOGI("allocCameraMem E: size 0x%x,mIOMMUEnabled  = %d", buf_size,
         mIOMMUEnabled);
    if (!mIOMMUEnabled) {
        if (is_cache) {
            pHeapIon = new MemIon("/dev/ion", buf_size, 0,
                                  (1 << 31) | ION_HEAP_ID_MASK_MM);
        } else {
            pHeapIon = new MemIon("/dev/ion", buf_size, MemIon::NO_CACHING,
                                  ION_HEAP_ID_MASK_MM);
        }
    } else {
        if (is_cache) {
    ...
    if (NULL == pHeapIon->getBase() ||
        HEAPION_INVALIDPTR == (uintptr_t)pHeapIon->getBase()) {
        LOGE("allocCameraMem: error getBase is 0x%lx.",
             (unsigned long)pHeapIon->getBase());
        result = -1;
        goto getpmem_end;
    }
	...
	if (result < 0) {
		LOGE("allocCameraMem: error get pHeapIon addr - method %d result 0x%x ",mIOMMUEnabled, result);
		goto getpmem_end;
	}
    ...
    memory->phys_addr = 0; // paddr;
    memory->phys_size = psize;
    memory->data = pHeapIon->getBase();
    memory->fd = pHeapIon->getHeapID();
    if (!mIOMMUEnabled) {
        LOGI("X: fd 0x%lx, data: 0x%p, phys_size: 0x%lx heap=%p", memory->fd,
             memory->data, memory->phys_size, pHeapIon);
    } else {
        LOGI("X: mm_iova: fd 0x%lx, data: 0x%p, phys_size: 0x%lx.heap=%p",
             memory->fd, memory->data, memory->phys_size, pHeapIon);
    }
...
uint32_t
SprdCameraHardware::getPreviewBufferID(buffer_handle_t *buffer_handle) {
...
uint32_t SprdCameraHardware::getPreviewBufferIDForFd(cmr_s32 fd) {
    uint32_t id = 0xFFFFFFFF;
    int i = 0;
    for (i = 0; i < kPreviewBufferCount + kPreviewRotBufferCount + 1; i++) {
        if ((NULL != mPreviewHeapArray_fd[i]) &&
            (mPreviewHeapArray_fd[i] == fd)) {
            id = i;
            break;
        }
    }
...
uint32_t SprdCameraHardware::getVideoBufferIDForFd(cmr_s32 fd) {
    uint32_t id = 0xFFFFFFFF;
    int i = 0;
    for (i = 0; i < kPreviewBufferCount + kPreviewRotBufferCount + 1; i++) {
        if ((NULL != mVideoHeapArray_fd[i]) && (mVideoHeapArray_fd[i] == fd)) {
            id = i;
            break;
        }
    }
...
int SprdCameraHardware::releasePreviewFrame() {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("releasePreviewFrame: oem is null or oem ops is null");
        return -1;
    }
    ...
    if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage &&
        ...
        if (0 !=
            mPreviewWindow->dequeue_buffer(mPreviewWindow, &buffer_handle,
                                           &stride)) {
            ret = -1;
            LOGE("releasePreviewFrame fail: Could not dequeue gralloc "
                 "buffer!\n");
        } else {
            free_buffer_id = getPreviewBufferID(buffer_handle);
            if (mPreviewCancelBufHandle[free_buffer_id] ==
                mPreviewBufferHandle[free_buffer_id]) {
                mPreviewCancelBufHandle[free_buffer_id] = NULL;
                LOGE("It's cancelled buf 0x%x, no need to release",
                     free_buffer_id);
                // mHalOem->ops->camera_set_preview_buffer(mCameraHandle,
                // mPreviewHeapArray_phy[free_buffer_id],
                // mPreviewHeapArray_vir[free_buffer_id]);
            } else {
                Mutex::Autolock pgbcl(&mGraphBufCntLock);
                if (isRecordingMode()) {
                ...
                LOGV("releasePreviewFrame 0x%x count %d", free_buffer_id,
                     mGraphBufferCount[free_buffer_id]);
                if ((mGraphBufferCount[free_buffer_id] == 0) && isPreviewing())
                    mHalOem->ops->camera_set_preview_buffer(
                        mCameraHandle, mPreviewHeapArray_phy[free_buffer_id],
                        mPreviewHeapArray_vir[free_buffer_id],
                        mPreviewHeapArray_fd[free_buffer_id]);
            }
        }
#endif
    }
...
bool SprdCameraHardware::allocatePreviewMemByGraphics() {
    ...
    if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage) {
        ...
        if (0 !=
            mPreviewWindow->set_buffer_count(mPreviewWindow,
                                             kPreviewBufferCount)) {
        ...
        if (miniUndequeued >= kPreviewBufferCount) {
            LOGE("allocatePreviewMemByGraphics: minUndequeued value error: %d",
                 miniUndequeued);
            return -1;
        }
        ...
        for (i = 0; i < kPreviewBufferCount; i++) {
            ...
            if (!mIOMMUEnabled) {
                ...
                size_t ion_size = 0;
                if (0 != MemIon::Get_phy_addr_from_ion(private_h->share_fd,
                                                       &ion_addr, &ion_size)) {
                    LOGE("allocatePreviewMemByGraphics: Get_phy_addr_from_ion "
                         "error");
                    return -1;
                }
                LOGI("MemIon::Get_mm_ion: %d addr 0x%lx size 0x%x", i, ion_addr,
                     ion_size);
                mPreviewBufferHandle[i] = buffer_handle;
            ...
            LOGI("allocatePreviewMemByGraphics: fd:0x%lx, base:0x%lx, "
                 "size:0x%x, stride:0x%x ",
                 mPreviewHeapArray_phy[i], (unsigned long)private_h->base,
                 private_h->size, stride);
            mCancelBufferEb[i] = 0;
        }

        for (i = (kPreviewBufferCount - miniUndequeued);
             i < kPreviewBufferCount; i++) {
            if (0 !=
                mPreviewWindow->cancel_buffer(mPreviewWindow,
                                              mPreviewBufferHandle[i])) {
                LOGE("allocatePreviewMemByGraphics: cancel_buffer error: %d",
                     i);
            }
            mPreviewCancelBufHandle[i] = mPreviewBufferHandle[i];
            mCancelBufferEb[i] = 1;
...
int SprdCameraHardware::getRedisplayMem() {
    uint32_t buffer_size =
        SIZE_ALIGN(mPreviewWidth) * (SIZE_ALIGN(mPreviewHeight)) * 3 / 2;
...
bool SprdCameraHardware::switchBufferMode(uint32_t src, uint32_t dst) {
    ...
    LOGI("switchBufferMode preview_state %s",
         getCameraStateStr(getPreviewState()));
    ...
    if ((src != dst) && (PREVIEW_BUFFER_USAGE_GRAPHICS == dst) &&
        isPreviewing()) {
        LOGI("switchBufferMode need stop preview");
        mIsRestartPreview = 1;
...
bool SprdCameraHardware::allocateCapMem(uint32_t mem_size) {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("allocateCapMem: oem is null or oem ops is null");
        return false;
    }

    buffer_size = mHalOem->ops->camera_get_size_align_page(mem_size);
...
bool SprdCameraHardware::allocateCaptureMem(bool isPreAllocCapMem,
                                            uint32_t mem_size) {
    bool rtn = true;
        ...
        if (NO_ERROR == mCbCapDataBusyLock.tryLock()) {
            ...
            if (mRawHeap) {
                if (true == mRawHeap->busy_flag) {
                    ...
                    LOGE("freeCaptureMem, raw buffer is busy, skip, bakup and "
                         "free later!");
...
status_t SprdCameraHardware::set_ddr_freq(uint32_t mhzVal) {
    ...
    LOGI("set_ddr_freq to %d now count %d freq %d E", mhzVal, mSetDDRFreqCount,
         mSetDDRFreq);
    ...
    const char *const set_freq = "/sys/devices/platform/scxx30-dmcfreq.0/"
                                 "devfreq/scxx30-dmcfreq.0/ondemand/set_freq";
    ...
    LOGI("set_ddr_freq to %skhz now count %d freq %d X", freq_in_khz,
         mSetDDRFreqCount, mSetDDRFreq);
    fclose(fp);
    return NO_ERROR;
}
...
status_t SprdCameraHardware::startPreviewInternal(bool isRecording) {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("startPreviewInternal: oem is null or oem ops is null");
        return UNKNOWN_ERROR;
    }
    ...
    takepicture_mode mode = getCaptureMode();
    LOGI("startPreviewInternal isRecording=%d.captureMode=%d", isRecording,
         mCaptureMode);
    ...
    while (isCapturing()) {
        ...
        if (ON_OFF_ACT_TIMEOUT < wait_cnt) {
            LOGE("startPreviewInternal X Capture state is %s, expecting "
                 "SPRD_IDLE!",
                 getCameraStateStr(mCameraState.capture_state));
            return INVALID_OPERATION;
        } else {
    ...
    if (!initPreview()) {
        LOGE("startPreviewInternal X initPreview failed.  Not starting "
             "preview.");
        deinitPreview();
        return UNKNOWN_ERROR;
    }
    if (1 == mParameters.getInt("zsl")) {
        ...
        if (!initCapture()) {
            deinitCapture(mIsPreAllocCapMem);
            set_ddr_freq(BASE_FREQ_REQ);
            LOGE("startPreviewInternal X initCapture failed. Not taking "
                 "picture.");
            return UNKNOWN_ERROR;
        }
    }
    ...
    cmr_int qret = mHalOem->ops->camera_start_preview(mCameraHandle, mode);
    ...
    if (mIsPerformanceTestable) {
        sprd_stopPerfTracking("startPreviewInternal X,mRecordingMode=%d.",
                              isRecordingMode());
    } else {
...
void SprdCameraHardware::stopPreviewInternal() {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("stopPreviewInternal: oem is null or oem ops is null");
        return;
    }
    ...
    if (mIsPerformanceTestable) {
        sprd_stopPerfTracking("stopPreviewInternal X Time:%lld(ms).",
                              (end_timestamp - start_timestamp) / 1000000);
    } else {
        LOGI("stopPreviewInternal X Time:%lld(ms).",
             (end_timestamp - start_timestamp) / 1000000);
    }
}
...
takepicture_mode SprdCameraHardware::getCaptureMode() {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("getCaptureMode: oem is null or oem ops is null");
        return mCaptureMode;
    }

    isZslSupport = (char *)mParameters.get("zsl-supported");
    if ((1 == mParameters.getRecordingHint()) && (isZslSupport) &&
    ...
    } else if (mHalOem->ops->camera_is_vendor_hdr(mCameraHandle) ||
               mHalOem->ops->camera_get_lls_shot_mode(mCameraHandle)) {
        mCaptureMode = CAMERA_NORMAL_MODE;
    } else {
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SCENE_MODE, 0);
    ...
    if ((CAMERA_FLASH_MODE_ON == flash_mode) ||
        (CAMERA_FLASH_MODE_AUTO == flash_mode)) {
        mCaptureMode = CAMERA_NORMAL_MODE;
    }
#else
...
status_t SprdCameraHardware::cancelPictureInternal() {
    bool result = true;
    if (mIsPerformanceTestable) {
        sprd_startPerfTracking("cancelPictureInternal: E, state = %s",
                               getCameraStateStr(getCaptureState()));
    } else {
        LOGI("cancelPictureInternal: E, state = %s",
             getCameraStateStr(getCaptureState()));
    }

    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("cancelPictureInternal: oem is null or oem ops is null");
        return UNKNOWN_ERROR;
    }
    ...
    case SPRD_WAITING_RAW:
    case SPRD_WAITING_JPEG:
        LOGI("camera state is %s, stopping picture.",
             getCameraStateStr(getCaptureState()));
    ...
    default:
        LOGW("not taking a picture (state %s)",
             getCameraStateStr(getCaptureState()));
        break;
    }
...
status_t SprdCameraHardware::initDefaultParameters() {
    ...
    SprdCameraParameters::ConfigType config =
        (1 & getCameraIndex()) ? SprdCameraParameters::kFrontCameraConfig
...
status_t SprdCameraHardware::setCameraParameters() {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("setCameraParameters: oem is null or oem ops is null");
        return UNKNOWN_ERROR;
    }
    ...
    LOGI("setCameraParameters: preview fps range: min = %d, max = %d", min,
         max);
    ...
    if (rotation == -1) {
        ...
        LOGI(
            "rotation %d is not a multiple of 90 degrees!  Defaulting to zero.",
            rotation);
        rotation = 0;
    } else {
    ...
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_ENCODE_ROTATION, rotation);
    ...
    if (-1 == rotation)
        rotation = 0;
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SENSOR_ROTATION, rotation);
    ...
    if (mHalOem->ops->camera_get_lls_shot_mode(mCameraHandle) &&
        (mParameters.getInt("shot-mode") != 27)) {
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SHOT_NUM, 5);
    } else if (mParameters.getInt("shot-mode") == 27) {
        mHalOem->ops->camera_set_lls_shot_mode(mCameraHandle, 1);
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SHOT_NUM, 1);
    } else if (mHalOem->ops->camera_is_vendor_hdr(mCameraHandle)) {
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SHOT_NUM, 3);
    } else {
        mHalOem->ops->camera_set_lls_shot_mode(mCameraHandle, 0);
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SHOT_NUM,
                 mParameters.getInt("capture-mode"));
    }

    int is_mirror = (getCameraIndex() & 1) ? 1 : 0;
    int ret = 0;
    ...
    ret = mHalOem->ops->camera_get_preview_rect(
        mCameraHandle, (cmr_uint *)&preview_rect.x, (cmr_uint *)&preview_rect.y,
    ...
    mParameters.getFocusAreas(&area[1], &area[0], &preview_size, &preview_rect,
                              kCameraInfo[getCameraIndex()].orientation,
                              is_mirror);
    focus_param.zone_cnt = area[0];
    cnt = (FOCUS_ZONE_CNT_MAX < focus_param.zone_cnt) ? FOCUS_ZONE_CNT_MAX
                                                      : focus_param.zone_cnt;
    for (i = 0; i < cnt; i++) {
        ...
        LOGI("start_x =%d start_y=%d", focus_param.zone[i].start_x,
             focus_param.zone[i].start_y);
        LOGI("width =%d height=%d", focus_param.zone[i].width,
             focus_param.zone[i].height);
    }
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_FOCUS_RECT,
             (cmr_uint)&focus_param);

    int ae_mode = mParameters.getAutoExposureMode();
    struct cmr_ae_param ae_param = {0};
    if (2 == ae_mode) {
        mParameters.getMeteringAreas(
            &area[1], &area[0], &preview_size, &preview_rect,
    ...
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_AUTO_EXPOSURE_MODE,
             (cmr_uint)&ae_param);
    LOGI("ae_param.mode = %d,start_x=%d,start_y =%d,width =%d height=%d",
         ae_param.mode, ae_param.win_area.rect[0].start_x,
         ae_param.win_area.rect[0].start_y, ae_param.win_area.rect[0].width,
    ...
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_WB,
             mParameters.getWhiteBalance());
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_JPEG_QUALITY,
             mParameters.getJpegQuality());
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_THUMB_QUALITY,
             mParameters.getJpegThumbnailQuality());
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_EFFECT,
             mParameters.getEffect());
    if (mHalOem->ops->camera_is_lls_enabled(mCameraHandle)) {
        SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SCENE_MODE,
    ...
    /*	SET_PARM(mHalOem, CAMERA_PARM_DCDV_MODE,
     * mParameters.getRecordingHint());*/

    /*	int ns_mode = mParameters.getInt("nightshot-mode");
            if (ns_mode < 0) ns_mode = 0;
            SET_PARM(mHalOem, CAMERA_PARM_NIGHTSHOT_MODE, ns_mode);

            int luma_adaptation = mParameters.getInt("luma-adaptation");
            if (luma_adaptation < 0) luma_adaptation = 0;
            SET_PARM(mHalOem, CAMERA_PARM_LUMA_ADAPTATION, luma_adaptation);
    */
	...
	SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_PREVIEW_LLS_FPS, (cmr_uint)&fps_param);
#endif
    if (mIsPerformanceTestable) {
...
int SprdCameraHardware::uv420CopyTrim(struct _dma_copy_cfg_tag dma_copy_cfg) {
    ...
    if (DMA_COPY_YUV400 <= dma_copy_cfg.format ||
        ...
        0 == dma_copy_cfg.src_addr.y_addr ||
        0 == dma_copy_cfg.src_addr.uv_addr ||
        0 == dma_copy_cfg.dst_addr.y_addr ||
        0 == dma_copy_cfg.dst_addr.uv_addr || 0 == dma_copy_cfg.src_rec.w ||
        0 == dma_copy_cfg.src_rec.h || 0 == dma_copy_cfg.src_size.w ||
        0 == dma_copy_cfg.src_size.h ||
        (dma_copy_cfg.src_rec.x + dma_copy_cfg.src_rec.w >
         dma_copy_cfg.src_size.w) ||
        (dma_copy_cfg.src_rec.y + dma_copy_cfg.src_rec.h >
         dma_copy_cfg.src_size.h)) {
        LOGE("uv420CopyTrim: param is error. \n");
        return -1;
    }

    src_y_addr = dma_copy_cfg.src_addr.y_addr +
                 dma_copy_cfg.src_rec.y * dma_copy_cfg.src_size.w +
                 dma_copy_cfg.src_rec.x;
    src_uv_addr = dma_copy_cfg.src_addr.uv_addr +
                  ((dma_copy_cfg.src_rec.y * dma_copy_cfg.src_size.w) >> 1) +
                  dma_copy_cfg.src_rec.x;
    ...
    for (i = 0; i < dma_copy_cfg.src_rec.h; i++) {
        ...
        if (0 == (i & 0x01)) {
            memcpy((void *)dst_uv_addr, (void *)src_uv_addr,
                   dma_copy_cfg.src_rec.w);
...
int SprdCameraHardware::uv420SPTouv420P(char *dst_virtual_addr,
                                        char *virtual_addr, uint32_t src_w,
                                        uint32_t src_h) {
    uint32_t y_stride = 0, y_size = 0, uv_stride = 0, uv_size = 0, i = 0, j = 0,
             src_index = 0;
    char *dst_u_ptr = NULL;
    char *dst_v_ptr = NULL;
    ...
    if (dst_virtual_addr == NULL || virtual_addr == NULL || src_w == 0 ||
        src_h == 0)
        return 0;
    ...
    // LOGE("copy y start src_w =%d,src_h=%d dst_virtual_addr =
    // 0x%x,virtual_addr = 0x%x",src_w,src_h,dst_virtual_addr,virtual_addr);
...
int SprdCameraHardware::displayCopy(uintptr_t dst_phy_addr,
    ...
    if (!mIOMMUEnabled) {
        ...
        dma_copy_cfg.src_addr.uv_addr =
            src_phy_addr + dma_copy_cfg.src_size.w * dma_copy_cfg.src_size.h;
        ...
        if ((0 == dma_copy_cfg.src_rec.x) && (0 == dma_copy_cfg.src_rec.y) &&
            ...
            dma_copy_cfg.dst_addr.uv_addr =
                dst_phy_addr +
                dma_copy_cfg.src_size.w * dma_copy_cfg.src_size.h;
        } else {
            dma_copy_cfg.dst_addr.uv_addr =
                dst_phy_addr + dma_copy_cfg.src_rec.w * dma_copy_cfg.src_rec.h;
        }
        ...
        if ((0 == dma_copy_cfg.src_rec.x) && (0 == dma_copy_cfg.src_rec.y) &&
            ...
            dma_copy_cfg.dst_addr.uv_addr =
                dst_virtual_addr +
                dma_copy_cfg.src_rec.w * dma_copy_cfg.src_rec.h;
        }
        ...
        if (mIsDvPreview) {
            ...
            char *src = (char *)src_virtual_addr + src_w * src_h +
                        SIZE_ALIGN(src_w / 2) * src_h / 2;
            int skip = SIZE_ALIGN64(SIZE_ALIGN(src_w / 2) * src_h / 2) -
                       SIZE_ALIGN(src_w / 2) * src_h / 2;
            char *dst = (char *)dst_virtual_addr + src_w * src_h +
                        SIZE_ALIGN(src_w / 2) * src_h / 2 + skip;
            memcpy((void *)dst_virtual_addr, (void *)src_virtual_addr,
                   src_w * src_h + SIZE_ALIGN(src_w / 2) * src_h / 2);
...
bool SprdCameraHardware::displayOneFrameForCapture(uint32_t width,
                                                   uint32_t height, int fd,
                                                   uintptr_t phy_addr,
                                                   char *virtual_addr) {
    LOGI("%s: size = %dx%d, fd = 0x%x", __func__, width, height, fd);
    if (mIsPerformanceTestable) {
    ...
    ret = mGrallocHal->lock(mGrallocHal, *buf_handle,
                            GRALLOC_USAGE_SW_WRITE_OFTEN, 0, 0,
                            SIZE_ALIGN(width), SIZE_ALIGN(height), &vaddr);

    if (0 != ret || NULL == vaddr) {
        LOGE("%s: failed to lock gralloc buffer ret=%d,vaddr=0x%lx", __func__,
    ...
    if (!mIOMMUEnabled) {
        unsigned long ion_addr = 0;
        size_t ion_size = 0;
        if (0 != MemIon::Get_phy_addr_from_ion(private_h->share_fd, &ion_addr,
                                               &ion_size)) {
            LOGE("displayOneFrameForCapture: Get_phy_addr_from_ion error");
            return false;
        }
        LOGI("MemoryHeapIon::Get_phy_addr_from_ion: addr 0x%lx size 0x%x",
             ion_addr, ion_size);
        dst_phy_addr = (uintptr_t)ion_addr;
    }
#endif
    LOGI("displayOneFrameForCapture,0x%lx.", (unsigned long)virtual_addr);
    ret = displayCopy(dst_phy_addr, (uintptr_t)vaddr, phy_addr,
                      (uintptr_t)virtual_addr, width, height);
...
bool SprdCameraHardware::displayOneFrame(uint32_t width, uint32_t height,
                                         cmr_s32 fd, uintptr_t phy_addr,
                                         char *virtual_addr, uint32_t id) {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("displayOneFrame: oem is null or oem ops is null");
        return false;
    }
    ...
    if (PREVIEW_BUFFER_USAGE_DCAM == mPreviewBufferUsage) {
        ...
        LOGV("%s: size = %dx%d, fd = 0x%lx", __func__, width, height, fd);
        ...
        ret = mPreviewWindow->dequeue_buffer(mPreviewWindow, &buf_handle,
                                             &stride);
        if (0 != ret) {
            LOGE("%s: failed to dequeue gralloc buffer!", __func__);
            return false;
        ...
        if (mIsDvPreview) {
            ret = mGrallocHal->lock(
                mGrallocHal, *buf_handle, GRALLOC_USAGE_SW_WRITE_OFTEN, 0, 0,
                SIZE_ALIGN(width), SIZE_ALIGN(height), &vaddr);
        } else {
            ret = mGrallocHal->lock(mGrallocHal, *buf_handle,
                                    GRALLOC_USAGE_SW_WRITE_OFTEN, 0, 0, width,
                                    height, &vaddr);
        }

        if (0 != ret || NULL == vaddr) {
            LOGE("%s: failed to lock gralloc buffer ret=%d, vaddr=0x%lx",
                 __func__, ret, (unsigned long)vaddr);
            ret = mPreviewWindow->cancel_buffer(mPreviewWindow, buf_handle);
            if (0 != ret) {
                LOGE("%s cancel buffer fail", __func__);
            }
        ...
        if (mCancelBufferEb[id] &&
            (mPreviewCancelBufHandle[id] == mPreviewBufferHandle[id])) {
            LOGW("displayOneFrame skip: Could not enqueue cancel buffer!\n");
            // camera_release_frame(mCameraHandle, CAMERA_PREVIEW_DATA, id);
            if (isPreviewing())
                mHalOem->ops->camera_set_preview_buffer(
                    mCameraHandle, mPreviewHeapArray_phy[id],
                    mPreviewHeapArray_vir[id], mPreviewHeapArray_fd[id]);
            return true;
        } else {
            if (!mPreviewWindow || !mGrallocHal) {
                return false;
            ...
            if (mPreviewBufferHandle[id] != NULL) {
                if (mIsDvPreview) {
                    ret = mGrallocHal->lock(
                        mGrallocHal, *mPreviewBufferHandle[id],
                        GRALLOC_USAGE_SW_WRITE_OFTEN, 0, 0, SIZE_ALIGN(width),
                        SIZE_ALIGN(height), &vaddr);
                } else {
                    ret = mGrallocHal->lock(mGrallocHal,
                                            *mPreviewBufferHandle[id],
                                            GRALLOC_USAGE_SW_WRITE_OFTEN, 0, 0,
                                            width, height, &vaddr);
                }
                if (0 != ret || NULL == vaddr) {
                    LOGE("%s: failed to lock buffer ret=%d, vaddr=0x%lx id=%d, "
                         "BufferHandle=0x%x, handle=0x%x",
                         __func__, ret, (unsigned long)vaddr, id,
                         mPreviewBufferHandle[id], *mPreviewBufferHandle[id]);
                    return false;
                }
            }

            mGrallocHal->unlock(mGrallocHal, *mPreviewBufferHandle[id]);
            LOGI("displayOneFrame enqueue buff: mPreviewHeapArray_phy[%d] = "
                 "0x%lx, mPreviewHeapArray_vir[%d] = 0x%lx",
                 id, mPreviewHeapArray_phy[id], id, mPreviewHeapArray_vir[id]);
            if (0 !=
                mPreviewWindow->enqueue_buffer(mPreviewWindow,
                                               mPreviewBufferHandle[id])) {
                LOGE("displayOneFrame fail: Could not enqueue gralloc "
                     "buffer!\n");
...
void SprdCameraHardware::receivePreviewFDFrame(
    ...
    if (mMsgEnabled & CAMERA_MSG_PREVIEW_METADATA) {
        ...
        handleFDDataCallback(CAMERA_MSG_PREVIEW_METADATA, tmpIndex, 0,
                             &metadata, mUser, 1);
#if 0
		if (PREVIEW_BUFFER_USAGE_DCAM == mPreviewBufferUsage) {
			uint32_t tmpIndex = offset;
...
void SprdCameraHardware::handleDataCallback(int32_t msg_type,
    ...
    if (frame_index > (mPreviewDcamAllocBufferCnt - 1) ||
        (HEAPION_INVALIDPTR == (uintptr_t)data->camera_memory->data)) {
        LOGE("error,index addr %d array num %d", frame_index,
...
void SprdCameraHardware::cameraBakMemCheckAndFree() {
    ...
    if (NO_ERROR == mCbCapDataBusyLock.tryLock()) {
        Mutex::Autolock cbdLock(&mCapBakDataLock);
        /* capture head check and free*/
        if ((false == mRawHeapInfoBak.busy_flag) && (1 == mRawHeapBakUseFlag)) {
...
int SprdCameraHardware::allocOneFrameMem(
    struct SprdCameraHardware::OneFrameMem *one_frame_mem_ptr) {
    struct SprdCameraHardware::OneFrameMem *tmp_one_frame_mem_ptr =
        one_frame_mem_ptr;
    int ret = 0;
    ...
    if (!mIOMMUEnabled) {
        tmp_one_frame_mem_ptr->input_y_pmem_hp =
            new MemIon("/dev/ion", tmp_one_frame_mem_ptr->width *
                                       tmp_one_frame_mem_ptr->height,
    ...
    if (!mIOMMUEnabled) {
        ret = tmp_one_frame_mem_ptr->input_y_pmem_hp->get_phy_addr_from_ion(
            (unsigned long *)(&tmp_one_frame_mem_ptr->input_y_physical_addr),
    ...
    tmp_one_frame_mem_ptr->input_y_virtual_addr =
        (unsigned char *)tmp_one_frame_mem_ptr->input_y_pmem_hp->getBase();
...
int SprdCameraHardware::relaseOneFrameMem(
    struct SprdCameraHardware::OneFrameMem *one_frame_mem_ptr) {
    struct SprdCameraHardware::OneFrameMem *tmp_one_frame_mem_ptr =
        one_frame_mem_ptr;
...
#define ENABLE_CPU_HOTPLUG "0"
    const char *const hotplug =
        "/sys/devices/system/cpu/cpufreq/sprdemand/cpu_hotplug_disable";
...
#define ENABLE_CPU_DVFS "95"
    const char *const dvfs =
        "/sys/devices/system/cpu/cpufreq/sprdemand/up_threshold";
...
int SprdCameraHardware::overwritePreviewFrameMemInit(
    struct SprdCameraHardware::OneFrameMem *one_frame_mem_ptr) {
    struct SprdCameraHardware::OneFrameMem *tmp_one_frame_mem_ptr =
        one_frame_mem_ptr;
    uint32_t overwrite_offset =
        tmp_one_frame_mem_ptr->width * tmp_one_frame_mem_ptr->height * 2 / 3;
    uint32_t addr_offset = overwrite_offset;
    LOGI("overwritePreviewFrameMemInit 0 %d %d 0x%x", overwrite_offset,
         addr_offset,
         mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][0]);
    memset(tmp_one_frame_mem_ptr->input_y_virtual_addr,
           mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][0],
           overwrite_offset);
    overwrite_offset =
        tmp_one_frame_mem_ptr->width * tmp_one_frame_mem_ptr->height / 3;
    LOGI("overwritePreviewFrameMemInit 1 %d %d 0x%x", overwrite_offset,
         addr_offset,
         mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][1]);
    memset((tmp_one_frame_mem_ptr->input_y_virtual_addr + addr_offset),
           mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][1],
           overwrite_offset);
...
void SprdCameraHardware::overwritePreviewFrame(camera_frame_type *frame) {
    ...
    static struct SprdCameraHardware::OneFrameMem *one_frame_mem_ptr =
    ...
    if (mIsPerformanceTestable) {
        sprd_perfInfo("performance autotest of camera shake test E. color=%d",
                      mShakeTest.mShakeTestColorCount);
    } else {
    ...
    memcpy((void *)(frame->y_vir_addr + overwrite_offset),
           (void *)tmp_one_frame_mem_ptr->input_y_virtual_addr,
           (tmp_one_frame_mem_ptr->width * tmp_one_frame_mem_ptr->height));
    relaseOneFrameMem(&overwrite_preview_frame);
    if (mIsPerformanceTestable) {
        if (mShakeTest.mShakeTestColorCount) {
            sprd_perfInfo(
                "performance autotest of camera shake test X color=%d",
                (mShakeTest.mShakeTestColorCount - 1));
        } else {
            sprd_perfInfo(
                "performance autotest of camera shake test X color=%d",
                (MAX_LOOP_COLOR_COUNT - 1));
        }
    } else {
...
void SprdCameraHardware::sendPreviewFrameToApp(
    struct camera_frame_type *frame, camera_frame_metadata_t *metadata) {
    // Mutex::Autolock pcpl(&mPrevBufLock);
    if (PREVIEW_BUFFER_USAGE_DCAM == mPreviewBufferUsage) {
        uint32_t tmpIndex = frame->order_buf_id;
        if (mPreviewWindow && isPreviewing()) {
            if (HandleAPPCallBackInterLock()) {
                handleDataCallback(CAMERA_MSG_PREVIEW_FRAME, tmpIndex, 0,
                                   metadata, mUser, 1);
            }
        } else {
            LOGW("condition not fit, w is %p Previewing state: %s skip the cb",
                 mPreviewWindow, getCameraStateStr(mCameraState.preview_state));
            return;
        }
    } else {
        uint32_t dataSize = frame->width * frame->height * 3 / 2;
        if (mPreviewWindow && isPreviewing()) {
            {
                ...
                if (mPreviewHeapArray) {
                    if (mGetMemory_cb) {
                        camera_memory_t *mem =
                            mGetMemory_cb(-1, dataSize, 1, 0);
                        mPreviewHeapArray[mPreviewDcamAllocBufferCnt - 1]
                            ->camera_memory = mem;
                    }

                    LOGI("sendPreviewFrameToApp: camera_memory addr is 0x%lx, "
                         "dataSize = %d",
                         mPreviewHeapArray[mPreviewDcamAllocBufferCnt - 1]
                             ->camera_memory,
                         dataSize);
                    ...
                    if (0 == strcmp(mParameters.getPreviewFormat(), "yuv420p"))
                        uv420SPTouv420P(
                            (char *)
                                mPreviewHeapArray[mPreviewDcamAllocBufferCnt -
                                                  1]
                                    ->camera_memory->data,
                            (char *)frame->y_vir_addr, frame->width,
                            frame->height);
                    else
                        memcpy(mPreviewHeapArray[mPreviewDcamAllocBufferCnt - 1]
                                   ->camera_memory->data,
                               (void *)frame->y_vir_addr, dataSize);
                }
            }
            ...
            if (HandleAPPCallBackInterLock()) {
                handleDataCallback(CAMERA_MSG_PREVIEW_FRAME,
                                   mPreviewDcamAllocBufferCnt - 1, 0, metadata,
                                   mUser, 1);
            }
        } else {
            LOGW("condition not fit, w is %p Previewing state: %s skip the cb",
...
void SprdCameraHardware::sendPreviewFrameToVideo(
    struct camera_frame_type *frame) {
    ssize_t offset = frame->buf_id;
    camera_frame_metadata_t metadata;
    int width, height;
    ...
    LOGI("test timestamp = %lld, mIsStoreMetaData: %d. buffer_id "
         "0x%x,y_vir_addr 0x%x,y_phy_addr  0x%x, fd 0x%x",
         timestamp, mIsStoreMetaData, frame->buf_id, frame->y_vir_addr,
         frame->y_phy_addr, frame->fd);
    if (mTimeCoeff > 1) {
        if (0 != mRecordingFirstFrameTime) {
            timestamp = mRecordingFirstFrameTime +
                        (timestamp - mRecordingFirstFrameTime) * mTimeCoeff;
        } else {
    ...
    if (mIsStoreMetaData) {
        uint32_t tmpIndex = frame->order_buf_id;
        uint32_t *data =
            (uint32_t *)mMetadataHeap->data + offset * METADATA_SIZE / 4;
        ...
        *data = frame->fd;
        ...
        if (mPreviewHeapArray) {
            handleDataCallbackTimestamp(timestamp, CAMERA_MSG_VIDEO_FRAME,
                                        mPreviewHeapArray[tmpIndex], 0, mUser);
        }
    }
}
...
void SprdCameraHardware::receiveZslFrame(struct camera_frame_type *frame) {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("receiveZslFrame: oem is null or oem ops is null");
        return;
    }
    ...
    if (PREVIEW_ZSL_FRAME == frame->type) {
        if (1 == mZslShotPushFlag) {
            LOGI("receiveZslFrame getZSLQueueFrameNum %d",
                 getZSLQueueFrameNum());
            for (i = getZSLQueueFrameNum(); i > mZSLFrameNum; i--) {
                ...
                if (!need_pause) {
                    mHalOem->ops->camera_set_zsl_buffer(
                        ...
                        zsl_frame.frame.y_vir_addr,
                        (cmr_uint)zsl_frame.frame.fd);
                }
            }
            if (0 == getZSLQueueFrameNum()) {
                ...
                mHalOem->ops->camera_set_zsl_snapshot_buffer(
                    mCameraHandle, zsl_frame.frame.y_phy_addr,
                    zsl_frame.frame.y_vir_addr, (cmr_uint)zsl_frame.frame.fd);
                if (!need_pause) {
                    mHalOem->ops->camera_set_zsl_buffer(
                        mCameraHandle, zsl_frame.frame.y_phy_addr,
                        zsl_frame.frame.y_vir_addr,
...
void SprdCameraHardware::receivePreviewFrame(struct camera_frame_type *frame) {
    ...
    if ((PREVIEW_FRAME == frame->type) ||
        (PREVIEW_CANCELED_FRAME == frame->type)) {
        frame->buf_id = getPreviewBufferIDForFd(frame->fd);
    } else if ((PREVIEW_VIDEO_FRAME == frame->type) ||
               (PREVIEW_VIDEO_CANCELED_FRAME == frame->type)) {
        frame->buf_id = getVideoBufferIDForFd(frame->fd);
    } else if ((PREVIEW_ZSL_FRAME == frame->type) ||
               (PREVIEW_ZSL_CANCELED_FRAME == frame->type)) {
        frame->buf_id = getZslBufferIDForFd(frame->fd);
    }
    ...
    //	LOGI("receivePreviewFrame lls_info = %d frame->type = %d",
    // metadata.light_condition, frame->type);
    camera_face_t face_info[FACE_DETECT_NUM];
    uint32_t k = 0;
    ...
    if (isPreviewing()) {
        ...
        if (PREVIEW_FRAME == frame->type) {
            if (!displayOneFrame(width, height, frame->fd,
                                 (uintptr_t)frame->y_phy_addr,
                                 (char *)frame->y_vir_addr, frame->buf_id)) {
                mHalOem->ops->camera_set_preview_buffer(
                    mCameraHandle, frame->y_phy_addr, frame->y_vir_addr,
                    frame->fd);
    ...
    if (isPreviewing()) {
        ...
        mHalOem->ops->camera_zsl_snapshot_need_pause(mCameraHandle,
                                                     &need_pause);
        if (PREVIEW_ZSL_FRAME == frame->type) {
            LOGI("receivePreviewFrame: zsl buff fd=0x%x, frame type=%d",
                 frame->fd, frame->type);
            pushZslFrame(frame);
    ...
    if (mData_cb != NULL) {
        LOGI("receivePreviewFrame mMsgEnabled: 0x%x  type %d", mMsgEnabled,
             frame->type);
    ...
    while (1) {
        if (NO_ERROR == mLock.tryLock()) {
            ...
            if ((SPRD_INTERNAL_CAPTURE_STOPPING == getCaptureState()) ||
                ...
                LOGW("HandleTakePictureInterLock, capture state = "
                     "SPRD_INTERNAL_CAPTURE_STOPPING, return \n");
                ret = 0;
                break;
            } else {
                if (wait_cnt) {
...
void SprdCameraHardware::receiveRawPicture(struct camera_frame_type *frame) {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("receiveRawPicture: oem is null or oem ops is null");
        return;
    }
    ...
    if (SPRD_INTERNAL_CAPTURE_STOPPING == getCaptureState()) {
        LOGW("receiveRawPicture: warning: capture state = "
             "SPRD_INTERNAL_CAPTURE_STOPPING, return \n");
        return;
    }

    if (mHalOem->ops->camera_get_lls_shot_mode(mCameraHandle) ||
        mHalOem->ops->camera_is_vendor_hdr(mCameraHandle)) {
        uint32_t frame_size = frame->width * frame->height;
        if (mData_cb) {
            LOGI("yuv addr 0x%lx 0x%lx, w h %d %d, format %d",
                 frame->y_vir_addr, frame->y_phy_addr, frame->width,
                 frame->height, frame->format);

            if (1 == mCaptureNum) {
                if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE) {
                    camera_memory_t *mem =
                        mGetMemory_cb(-1, frame_size * 3 / 2, 1, 0);
                    memcpy(mem->data, (void *)(frame->y_vir_addr), frame_size);
                    memcpy((void *)((cmr_uint)mem->data + frame_size),
                           (void *)(frame->uv_vir_addr), frame_size >> 1);
        ...
        if (iSDisplayCaptureFrame()) {
            ...
            if (0 !=
                mHalOem->ops->camera_get_redisplay_data(
                    mCameraHandle, dst_fd, dst_paddr, dst_width, dst_height,
                    frame->fd, frame->y_phy_addr, frame->uv_phy_addr,
            ...
            if (!displayOneFrameForCapture(dst_width, dst_height, dst_fd,
                                           dst_paddr,
    ...
    if (mData_cb != NULL) {
        ...
        if (mMsgEnabled & CAMERA_MSG_RAW_IMAGE) {
            if (HandleTakePictureInterLock()) {
                handleDataCallback(CAMERA_MSG_RAW_IMAGE, 0, offset, NULL, mUser,
                                   0);
            }
        }
...
void SprdCameraHardware::receiveJpegPicture(struct camera_frame_type *frame) {
    ...
    if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
        LOGE("receiveJpegPicture: oem is null or oem ops is null");
        return;
    }
    ...
    if (mData_cb) {
        ...
        LOGD("receiveJpegPicture: mMsgEnabled: 0x%x mJpegSize 0x%x",
             mMsgEnabled, mJpegSize);
        if (mHalOem->ops->camera_get_isp_info(mCameraHandle, &isp_info_addr,
                                              &isp_info_size)) {
            isp_info_size = 0;
            isp_info_addr = NULL;
        }
        ...
        if (1 == mCaptureNum) {
            if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE) {
                camera_memory_t *mem =
                    mGetMemory_cb(-1, (mJpegSize + isp_info_size), 1, 0);
                memcpy(mem->data, encInfo->outPtr, mJpegSize);
                if (isp_info_addr) {
                    memcpy((void *)((cmr_uint)mem->data + mJpegSize),
                           isp_info_addr, isp_info_size);
                }
                ...
                if ((mCaptureMode == CAMERA_ISP_TUNING_MODE) ||
                    ...
                    LOGI("CAMERA_ISP_TUNING_MODE,mRawWidth,mRawHeight =%d,%d",
                         mRawWidth, mRawHeight);
                    mHalOem->ops->dump_jpeg_file((void *)mem->data,
            ...
            memcpy(mem->data, encInfo->outPtr, mJpegSize);
            if (isp_info_addr) {
                memcpy((void *)((cmr_uint)mem->data + mJpegSize), isp_info_addr,
                       isp_info_size);
            }
            mData_cb(CAMERA_MSG_COMPRESSED_IMAGE, mem, 0, NULL, mUser);
            mem->release(mem);
...
SprdCameraHardware::transitionState(SprdCameraHardware::Sprd_camera_state from,
                                    ...
                                    SprdCameraHardware::state_owner owner,
                                    bool lock) {
...
void SprdCameraHardware::HandleStartPreview(enum camera_cb_type cb,
                                            void *parm4) {
    LOGI("HandleStartPreview in: cb = %d, parm4 = 0x%lx, state = %s", cb,
    ...
    case CAMERA_EXIT_CB_FAILED:
        LOGE("SprdCameraHardware::camera_cb: @CAMERA_EXIT_CB_FAILURE(%ld) in "
             "state %s",
             (cmr_uint)parm4, getCameraStateStr(getPreviewState()));
        ...
        LOGE("CAMERA_EVT_CB_FLUSH, 0x%x", mPreviewHeapSize);
        {
            ...
            if (isPreviewing()) {
                flush_buffer(CAMERA_FLUSH_PREVIEW_HEAP, frame->buf_id,
                             ...
                             (void *)frame->y_phy_addr, mPreviewHeapSize);
            }
            mPrevBufLock.unlock();
        }
        break;
    ...
    case CAMERA_EVT_CB_RESUME:
        ...
        if (isPreviewing() && iSZslMode() &&
            0 != strcmp("hdr", mParameters.get_SceneMode())
            /*&& 1 != mParameters.getRecordingHint()*/) {
            setZslBuffers();
            mZslChannelStatus = 1;
        }
    ...
    LOGI("HandleStartPreview out, state = %s",
         getCameraStateStr(getPreviewState()));
}

void SprdCameraHardware::HandleStopPreview(enum camera_cb_type cb,
                                           void *parm4) {
    Sprd_camera_state tmpPrevState = SPRD_IDLE;
    tmpPrevState = getPreviewState();
    LOGI("HandleStopPreview in: cb = %d, parm4 = 0x%lx, state = %s", cb,
    ...
    LOGI("HandleStopPreview out, state = %s",
         getCameraStateStr(getPreviewState()));
}

void SprdCameraHardware::HandleTakePicture(enum camera_cb_type cb,
                                           void *parm4) {
    if (mIsPerformanceTestable) {
    ...
    LOGI("HandleTakePicture E: cb = %d, parm4 = 0x%lx, state = %s", cb,
    ...
    case CAMERA_EVT_CB_SNAPSHOT_JPEG_DONE:
        ...
        if (NULL == mCameraHandle || NULL == mHalOem || NULL == mHalOem->ops) {
            LOGE("HandleTakePicture: CAMERA_EXIT_CB_DONE error: oem is null or "
                 "oem ops is null");
            break;
        }
        ...
        if (mHalOem->ops->camera_get_lls_shot_mode(mCameraHandle) ||
            ...
            LOGI("HandleTakePicture state = %d, need_free = %d", tmpCapState,
                 ((struct camera_frame_type *)parm4)->need_free);
    ...
    LOGI("HandleTakePicture X, state = %s",
         getCameraStateStr(getCaptureState()));
}

void SprdCameraHardware::HandleCancelPicture(enum camera_cb_type cb,
                                             void *parm4) {
    LOGI("HandleCancelPicture E: cb = %d, parm4 = 0x%lx, state = %s", cb,
         (cmr_uint)parm4, getCameraStateStr(getCaptureState()));
    if (SPRD_INTERNAL_CAPTURE_STOPPING != getCaptureState() &&
    ...
    LOGI("HandleCancelPicture X, state = %s",
         getCameraStateStr(getCaptureState()));
}
...
void SprdCameraHardware::HandleEncode(enum camera_cb_type cb, void *parm4) {
    ...
    LOGI("HandleEncode E: cb = %d, parm4 = 0x%lx, state = %s", cb,
         (cmr_uint)parm4, getCameraStateStr(getCaptureState()));
...
void SprdCameraHardware::HandleFocus(enum camera_cb_type cb, void *parm4) {
    ...
    LOGI("HandleFocus E: cb = %d, parm4 = 0x%lx, state = %s", cb,
         (cmr_uint)parm4, getCameraStateStr(getPreviewState()));
...
void SprdCameraHardware::HandleStartCamera(enum camera_cb_type cb,
                                           void *parm4) {
    LOGI("HandleCameraStart in: cb = %d, parm4 = 0x%lx, state = %s", cb,
         (cmr_uint)parm4, getCameraStateStr(getCameraState()));

    transitionState(SPRD_INIT, SPRD_IDLE, STATE_CAMERA);

    LOGI("HandleCameraStart out, state = %s",
         getCameraStateStr(getCameraState()));
}

void SprdCameraHardware::HandleStopCamera(enum camera_cb_type cb, void *parm4) {
    LOGI("HandleStopCamera in: cb = %d, parm4 = 0x%lx, state = %s", cb,
         (cmr_uint)parm4, getCameraStateStr(getCameraState()));

    transitionState(SPRD_INTERNAL_STOPPING, SPRD_INIT, STATE_CAMERA);

    LOGI("HandleStopCamera out, state = %s",
         getCameraStateStr(getCameraState()));
}
...
int SprdCameraHardware::switch_monitor_thread_init(void *p_data) {
    ...
    if (!obj->mSwitchMonitorInited) {
        ret = cmr_msg_queue_create(
            SWITCH_MONITOR_QUEUE_SIZE,
            (cmr_handle *)&obj->mSwitchMonitorMsgQueHandle);
        if (ret) {
        ...
        ret = pthread_create(&obj->mSwitchMonitorThread, &attr,
                             switch_monitor_thread_proc, (void *)obj);
        obj->mSwitchMonitorInited = 1;
        message.msg_type = CMR_EVT_SW_MON_INIT;
        message.data = NULL;
        ret = cmr_msg_post((cmr_handle)obj->mSwitchMonitorMsgQueHandle,
                           &message, 1);
        if (ret) {
...
cmr_int SprdCameraHardware::ZSLMode_monitor_thread_proc(struct cmr_msg *message,
                                                        void *p_data) {
    int exit_flag = 0;
    int ret = NO_ERROR;
    SprdCameraHardware *obj = (SprdCameraHardware *)p_data;
...
void *SprdCameraHardware::switch_monitor_thread_proc(void *p_data) {
    ...
    while (1) {
        ret = cmr_msg_timedget((cmr_handle)obj->mSwitchMonitorMsgQueHandle,
                               &message);
        if (CMR_MSG_NO_OTHER_MSG == ret) {
            if (obj->checkSetParameters(obj->mParameters,
                                        obj->mSetParametersBak) &&
                obj->mBakParamFlag) {
                obj->setCameraState(SPRD_SET_PARAMS_IN_PROGRESS,
                                    STATE_SET_PARAMS);
            ...
            LOGE("Message queue destroyed");
            break;
        } else {
            LOGI("message.msg_type 0x%x, sub-type 0x%x", message.msg_type,
        ...
        if (exit_flag) {
            LOGI("switch monitor thread exit ");
            break;
        }
    }
    return NULL;
...
int SprdCameraHardware::pre_alloc_cap_mem_thread_init(void *p_data) {
    ...
    LOGI("pre_alloc_cap_mem_thread_init: inited=%d",
         obj->mPreAllocCapMemInited);
...
int SprdCameraHardware::pre_alloc_cap_mem_thread_deinit(void *p_data) {
    ...
    LOGI("pre_alloc_cap_mem_thread_deinit: inited=%d",
         obj->mPreAllocCapMemInited);
...
SprdCameraHardware::MemPool::MemPool(int buffer_size, int num_buffers,
                                     ...
                                     const char *name)
    : mBufferSize(buffer_size), mNumBuffers(num_buffers),
      mFrameSize(frame_size), mFrameOffset(frame_offset),
#if (MINICAMERA != 1)
      mBuffers(NULL),
#endif
      mName(name) {
...
void SprdCameraHardware::MemPool::completeInitialization() {
    ...
    if (mFrameSize > 0) {
        ...
        for (int i = 0; i < mNumBuffers; i++) {
            mBuffers[i] = new MemoryBase(mHeap, i * mBufferSize + mFrameOffset,
                                         mFrameSize);
        }
#endif
    }
...
SprdCameraHardware::AshmemPool::AshmemPool(int buffer_size, int num_buffers,
                                           int frame_size, int frame_offset,
                                           const char *name)
    : SprdCameraHardware::MemPool(buffer_size, num_buffers, frame_size,
                                  frame_offset, name) {
    LOGI("constructing MemPool %s backed by ashmem: "
         "%d frames @ %d bytes, offset %d, "
         "buffer size %d",
         mName, num_buffers, frame_size, frame_offset, buffer_size);
...
status_t SprdCameraHardware::MemPool::dump(int fd,
...
int SprdCameraHardware::flush_buffer(camera_flush_mem_type_e type, int index,
    ...
    case CAMERA_FLUSH_RAW_HEAP_ALL:
        for (i = 0; i < mSubRawHeapNum; i++) {
            ...
            if ((mSubRawHeapNum - 1) != i) {
                ...
                if (pHeapIon) {
                    LOGE("flush_buffer index=%d,vaddr=0x%lx, "
                         "paddr=0x%lx,size=0x%x",
                         i, (unsigned long)v_addr, (unsigned long)p_addr, size);
                    ret = pHeapIon->flush_ion_buffer(v_addr, p_addr, size);
                    if (ret) {
                        LOGW("flush_buffer abnormal ret=%d", ret);
                        LOGW("flush_buffer index=%d,vaddr=0x%lx, paddr=0x%lx",
                             i, (unsigned long)v_addr, (unsigned long)p_addr);
                    }
                }
            } else {
                index = i;
    ...
    if (pHeapIon) {
        LOGV("flush_buffer index=%d,vaddr=%p, paddr=%p,size=0x%x", index,
             v_addr, p_addr, size);
        ret = pHeapIon->flush_ion_buffer(v_addr, p_addr, size);
        if (ret) {
            LOGW("flush_buffer abnormal ret=%d", ret);
            LOGW("flush_buffer index=%d,vaddr=%p, paddr=%p", index, v_addr,
                 p_addr);
        }
    }
...
static camera_device_t *g_cam_device[SENSOR_ID_MAX] = {0, 0, 0, 0, 0};
...
static int HAL_camera_device_close(struct hw_device_t *device) {
    ...
    if (device) {
        ...
        id = static_cast<SprdCameraHardware *>(cam_device->priv)->getCameraId();
        delete static_cast<SprdCameraHardware *>(cam_device->priv);
        free(cam_device);
        g_cam_device[id] = NULL;
    }
...
static int
HAL_camera_device_set_preview_window(struct camera_device *dev,
...
static void HAL_camera_device_set_callbacks(
    ...
    LOGV("%s", __func__);
    obj(dev)
        ->setCallbacks(notify_cb, data_cb, data_cb_timestamp, get_memory, user);
...
static void HAL_camera_device_enable_msg_type(struct camera_device *dev,
                                              int32_t msg_type) {
    LOGV("%s", __func__);
 ...
 */
static void HAL_camera_device_disable_msg_type(struct camera_device *dev,
                                               int32_t msg_type) {
    LOGV("%s", __func__);
 ...
 * will return false.
 */
static int HAL_camera_device_msg_type_enabled(struct camera_device *dev,
                                              int32_t msg_type) {
    LOGV("%s", __func__);
 ...
 */
static int
HAL_camera_device_store_meta_data_in_buffers(struct camera_device *dev,
                                             int enable) {
    LOGV("%s", __func__);
...
static int HAL_IspVideoStartPreview(uint32_t param1, uint32_t param2) {
    LOGI("%s", __func__);
    int rtn = 0x00;
    SprdCameraHardware *fun_ptr = dynamic_cast<SprdCameraHardware *>(
        (SprdCameraHardware *)g_cam_device[0]->priv);
    if (NULL != fun_ptr) {
...
static int HAL_IspVideoSetParam(uint32_t width, uint32_t height) {
    ...
    SprdCameraHardware *fun_ptr = dynamic_cast<SprdCameraHardware *>(
        (SprdCameraHardware *)g_cam_device[0]->priv);
    if (NULL != fun_ptr) {
        LOGE("ISP_TOOL: HAL_IspVideoSetParam width:%d, height:%d", width,
             height);
...
static int HAL_camera_device_open(const struct hw_module_t *module,
    ...
    LOGI("%s: new camera %s", __func__, id);

    g_cam_device[cameraId]->priv = new SprdCameraHardware(cameraId);
    ...
    if (!(((SprdCameraHardware *)(g_cam_device[cameraId]->priv))
              ->isCameraInit())) {
        LOGE("camera init failed!");
        ((SprdCameraHardware *)(g_cam_device[cameraId]->priv))->release();
        delete g_cam_device[cameraId]->priv;
        delete g_cam_device[cameraId];
 ...
 * DESCRIPTION	: Register callbacks for flash to update whether flash is on
 *of off.
 * PARAMETERS	:
...
int HAL_camera_device_open_wrapper(const struct hw_module_t *module,
                                   const char *id,
                                   struct hw_device_t **device) {
    return android::HAL_camera_device_open(module, id, device);
}
#endif
