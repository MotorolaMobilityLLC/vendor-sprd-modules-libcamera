...
class SprdCameraHardware : public virtual RefBase {
  public:
    ...
    int displayCopy(uintptr_t dst_phy_addr, uintptr_t dst_virtual_addr,
                    uintptr_t src_phy_addr, uintptr_t src_virtual_addr,
                    uint32_t src_w, uint32_t src_h);
    int uv420SPTouv420P(char *dst_virtual_addr, char *virtual_addr,
                        uint32_t src_w, uint32_t src_h);
  ...
  private:
    ...
    void notifyShutter();
    //	void                  receiveJpegPictureFragment(struct
    // camera_jpeg_param *encInfo);
    ...
    bool displayOneFrame(uint32_t width, uint32_t height, cmr_s32 fd,
                         uintptr_t phy_addr, char *virtual_addr, uint32_t id);
    bool displayOneFrameForCapture(uint32_t width, uint32_t height, int fd,
                                   uintptr_t phy_addr, char *virtual_addr);
    void handleDataCallback(int32_t msg_type, uint32_t frame_index,
                            unsigned int index,
                            camera_frame_metadata_t *metadata, void *user,
                            uint32_t isPrev);
    void handleDataCallbackTimestamp(int64_t timestamp, int32_t msg_type,
                                     sprd_camera_memory_t *data,
                                     unsigned int index, void *user);
    void handleFDDataCallback(int32_t msg_type, uint32_t frame_index,
                              unsigned int index,
                              camera_frame_metadata_t *metadata, void *user,
                              uint32_t isPrev);
    ...
    int
    allocOneFrameMem(struct SprdCameraHardware::OneFrameMem *one_frame_mem_ptr);
    int relaseOneFrameMem(
        struct SprdCameraHardware::OneFrameMem *one_frame_mem_ptr);
    ...
    cmr_s32
        mPreviewHeapArray_fd[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uintptr_t
        mPreviewHeapArray_phy[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uintptr_t
        mPreviewHeapArray_vir[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uint32_t mPreviewHeapArray_size[kPreviewBufferCount +
                                    kPreviewRotBufferCount + 1];
    uint32_t
        mVideoHeapArray_fd[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uint32_t
        mVideoHeapArray_phy[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uint32_t
        mVideoHeapArray_vir[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uint32_t
        mVideoHeapArray_size[kPreviewBufferCount + kPreviewRotBufferCount + 1];

    uint32_t mZslHeapArray_fd[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uintptr_t
        mZslHeapArray_phy[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uintptr_t
        mZslHeapArray_vir[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    uint32_t
        mZslHeapArray_size[kPreviewBufferCount + kPreviewRotBufferCount + 1];
    ...
    int mPictureFormat; // 0:YUV422;1:YUV420;2:RGB;3:YUV420 3
                        // plane,4:YVU420,5:JPEG
