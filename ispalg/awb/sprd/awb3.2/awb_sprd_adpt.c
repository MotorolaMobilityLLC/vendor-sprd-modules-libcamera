...
struct awbsprd_lib_ops {
	...
	cmr_s32(*awb_debug_get_lib_version_v3_2) (void);
...
static cmr_u32 _awb_set_gain_manualwb_v3(struct awb_ctrl_cxt *cxt)
{
	...
	if (AWB_CTRL_WB_MODE_AUTO != cxt->wb_mode) {
		if (mawb_id == AWB_CTRL_AWB_MODE_OFF) {
			...
			rtn = cxt->lib_ops.awb_ioctrl_v3_2(cxt->alg_handle, AWB_IOCTRL_GET_MWB_BY_MODEID_3_0, &mawb_id, &out_gain_mwb);
			...
			rtn = cxt->lib_ops.awb_ioctrl_v3_2(cxt->alg_handle, AWB_IOCTRL_GET_MWB_BY_CT_3_0, &mawb_id, &out_gain_mwb);
			cxt->output_gain.r = out_gain_mwb.r_gain;
			cxt->output_gain.g = out_gain_mwb.g_gain;
			cxt->output_gain.b = out_gain_mwb.b_gain;
			cxt->output_ct     = out_gain_mwb.ct;
...
static cmr_u32 _awb_set_flash_status(struct awb_ctrl_cxt *cxt, void *param)
{
	...
	enum awb_flash_status_3_0 *flash_status2 = (enum awb_flash_status_3_0 *)param;

	cxt->flash_info.flash_status = *flash_status;
	cxt->awb_flash_info_v3.flash_status= *flash_status2;
...
static cmr_u32 _awb_set_mainflash_en(struct awb_ctrl_cxt *cxt, void *param)
{
	UNUSED(param);
	cmr_u32 rtn = AWB_CTRL_SUCCESS;
	cmr_u8 *mainFlash_en = (cmr_u8 *)param;
...
static cmr_u32 _awb_set_flash_ratio(struct awb_ctrl_cxt *cxt, void *param)
{
	UNUSED(param);
	cmr_u32 rtn = AWB_CTRL_SUCCESS;
	cmr_s32 *flash_ratio = (cmr_s32 *)param;
...
static cmr_u32 _awb_set_tuning_param(struct awb_ctrl_cxt *cxt, void *param0)
{
	...
	if (AWB_CTRL_SUCCESS != rtn) {
		...
		ISP_LOGE("success to check init param");
	}
...
static cmr_u32 awbsprd_load_lib(struct awb_ctrl_cxt *cxt)
{
	...
	//awblib 3.x
	if(v_id == 2) {
		ISP_LOGV("[AWB] version is isp3.2 awblib.so");
		cxt->lib_ops.awb_init_v3_2 = dlsym(cxt->lib_handle, "awb_init");
		if (!cxt->lib_ops.awb_init_v3_2) {
			ISP_LOGE("fail to dlsym awb_init");
			rtn = AWB_CTRL_ERROR;
			goto load_error;
		}

		cxt->lib_ops.awb_calc_v3_2 = dlsym(cxt->lib_handle, "awb_calc");
		if (!cxt->lib_ops.awb_calc_v3_2) {
			ISP_LOGE("fail to dlsym awb_calculation");
			rtn = AWB_CTRL_ERROR;
			goto load_error;
		}

		cxt->lib_ops.awb_deinit_v3_2 = dlsym(cxt->lib_handle, "awb_deinit");
		if (!cxt->lib_ops.awb_deinit_v3_2) {
			ISP_LOGE("fail to dlsym awb_deinit");
			rtn = AWB_CTRL_ERROR;
			goto load_error;
		}

		cxt->lib_ops.awb_ioctrl_v3_2 = dlsym(cxt->lib_handle, "awb_ioctrl");
		if (!cxt->lib_ops.awb_ioctrl_v3_2) {
			ISP_LOGE("fail to dlsym awb_ioctrl");
			rtn = AWB_CTRL_ERROR;
			goto load_error;
		}

		cxt->lib_ops.awb_debug_get_lib_version_v3_2 = dlsym(cxt->lib_handle, "awb_debug_get_lib_version");
		if (!cxt->lib_ops.awb_debug_get_lib_version_v3_2) {
			ISP_LOGE("fail to dlsym awb_debug_get_lib_version_v3_2");
			rtn = AWB_CTRL_ERROR;
			goto load_error;
		}

	} else {
		ISP_LOGE("[AWB] ERROR! the version is not isp3.2 awblib3v2.so");
	}
...
static cmr_u32 _awb_get_flash_ct_table_v3(struct awb_ctrl_cxt *cxt, void *result)
{
	...
	//awblib 3.x
	rtn = cxt->lib_ops.awb_ioctrl_v3_2(cxt->alg_handle, AWB_IOCTRL_GET_CTTABLE20_3_0, param,param);
...
awb_ctrl_handle_t awb_sprd_ctrl_init_v3_2(void *in, void *out)
{
	...
	if (NULL == cxt) {
		...
		ISP_LOGV("success to malloc awb_ctrl cxt");
	}
	...
	cxt->awb_result.gain.g = cxt->recover_gain.g;
	cxt->awb_result.ct = cxt->recover_ct;
	ISP_LOGD("AWB init: (%d,%d,%d), AWB_OTP rr = %d, rg = %d, rb = %d", cxt->output_gain.r, cxt->output_gain.g, cxt->output_gain.b,
...
cmr_s32 awb_sprd_ctrl_deinit_v3_2(void *handle, void *in, void *out)
{
	UNUSED(in);
	UNUSED(out);
	...
	rtn = cxt->lib_ops.awb_deinit_v3_2(cxt->alg_handle);
	if (AWB_CTRL_SUCCESS != rtn) {
...
cmr_s32 awb_sprd_ctrl_calculation_v3_2(void *handle, void *in, void *out)
{
	cmr_s32 rtn = AWB_CTRL_SUCCESS;
	struct awb_ctrl_cxt *cxt = (struct awb_ctrl_cxt *)handle;
	...
	//Flash info
	calc_param_v3.flash_info = &cxt->awb_flash_info_v3;
	ISP_LOGV("Flash_info,status:%d,mainflash_en:%d,ratio:%d,ratio1:%d",cxt->awb_flash_info_v3.flash_status,cxt->awb_flash_info_v3.mainFlash_enable,cxt->awb_flash_info_v3.flash_ratio,cxt->awb_flash_info_v3.flash_ratio1);
...
cmr_s32 awb_sprd_ctrl_ioctrl_v3_2(void *handle, cmr_s32 cmd, void *in, void *out)
{
	cmr_s32 rtn = AWB_CTRL_SUCCESS;
	struct awb_ctrl_cxt *cxt = (struct awb_ctrl_cxt *)handle;
...
struct adpt_ops_type awb_sprd_adpt_ops_ver3v2 = {
	.adpt_init = awb_sprd_ctrl_init_v3_2,
	.adpt_deinit = awb_sprd_ctrl_deinit_v3_2,
	.adpt_process = awb_sprd_ctrl_calculation_v3_2,
	.adpt_ioctrl= awb_sprd_ctrl_ioctrl_v3_2,
};
