...
#define MIN_ROI_SIZE 40
	if (STATE_FAF == af->state && FACE_NONE != af->f_orientation) {	//face roi settings
		...
		if (win[9].end_x - win[9].start_x < win_num.x * MIN_ROI_SIZE) {
			ISP_LOGI("s.x: %d,e.x: %d", win[9].start_x, win[9].end_x);
			cmr_u32 center_x = (win[9].start_x + win[9].end_x) >> 1;
			if (center_x < win_num.x * (MIN_ROI_SIZE >> 1)) {
				win[9].start_x = win[9].start_x < 10 ? 10 : win[9].start_x;
				win[9].end_x = win[9].start_x + win_num.x * MIN_ROI_SIZE;
				...
				win[9].start_x = win[9].end_x - win_num.x * MIN_ROI_SIZE;
			} else {
				win[9].end_x = center_x + win_num.x * (MIN_ROI_SIZE >> 1);
				win[9].start_x = center_x - win_num.x * (MIN_ROI_SIZE >> 1);
			}
			ISP_LOGI("s.x: %d,e.x: %d", win[9].start_x, win[9].end_x);
		}
...
static cmr_s32 afm_set_lum(af_ctrl_t * af, void *in)
{
	cmr_u32 *afm_data = (cmr_u32 *) in;
	cmr_u8 i = 0;
	char prop[256];
	int val = 0;
	property_get("debug.isp.af.lumlog", prop, "0");
	val = atoi(prop);

#if defined(CONFIG_ISP_2_5) || defined(CONFIG_ISP_2_6) || defined(CONFIG_ISP_2_7) || defined(CONFIG_ISP_2_8) || defined(CONFIG_ISP_2_9)

#if defined(CONFIG_ISP_2_5)
#define LUM_INDEX(block) (6 * ((block) >> 1) + ((block) & 0x01))
#endif

#if defined(CONFIG_ISP_2_6) || defined(CONFIG_ISP_2_7) || defined(CONFIG_ISP_2_8) || defined(CONFIG_ISP_2_9)
#define LUM_INDEX(block) (((block) * 3) + 2)
#endif

	if (STATE_FAF == af->state && FACE_NONE != af->f_orientation) {	//face lum mapping
		ISP_LOGI("roll_angle %d", af->roll_angle);
		if (FACE_UP == af->f_orientation || FACE_DOWN == af->f_orientation) {	//4*6
			if (FACE_UP == af->f_orientation) {
				for (i = 0; i < 6; i++) {	//3
					af->afm_lum[i] = afm_data[LUM_INDEX(i / 3 * 4 + i % 3 + af->win_offset)];
					if (val == 1) {
						ISP_LOGI("afm_lum[%d]: %d, block: %d, afm_data[%d]:%d", i, af->afm_lum[i], i / 3 * 4 + i % 3 + af->win_offset,
							 LUM_INDEX(i / 3 * 4 + i % 3 + af->win_offset), afm_data[LUM_INDEX(i / 3 * 4 + i % 3 + af->win_offset)]);
					}
				}
				if (0 == af->win_offset) {
				...
				for (i = 0; i < 6; i++) {	//3
					af->afm_lum[i + 3] = afm_data[LUM_INDEX(i / 3 * 4 + i % 3 + 16 + af->win_offset)];
					if (val == 1) {
						ISP_LOGI("afm_lum[%d]: %d, block: %d, afm_data[%d]:%d", i + 3, af->afm_lum[i + 3], i / 3 * 4 + i % 3 + 16 + af->win_offset,
							 LUM_INDEX(i / 3 * 4 + i % 3 + 16 + af->win_offset), afm_data[LUM_INDEX(i / 3 * 4 + i % 3 + 16 + af->win_offset)]);
					}
				}
				if (0 == af->win_offset) {
			...
			if (FACE_LEFT == af->f_orientation) {
				for (i = 0; i < 6; i++) {	//3
					af->afm_lum[i / 2 * 3 + i % 2] = afm_data[LUM_INDEX(i / 2 * 6 + i % 2)];
					if (val == 1) {
						ISP_LOGI("afm_lum[%d]: %d, block: %d, afm_data[%d]:%d", (i / 2 * 3 + i % 2), af->afm_lum[i / 2 * 3 + i % 2], i / 2 * 3 + i % 2,
							 LUM_INDEX(i / 2 * 3 + i % 2), afm_data[LUM_INDEX(i / 2 * 3 + i % 2)]);
					}
				}
		...
		for (i = 0; i < 9; i++) {
			...
			af->afm_lum[i] = afm_data[LUM_INDEX(j)] + afm_data[LUM_INDEX(j + 1)];
			af->afm_lum[9] += af->afm_lum[i];
			if (val == 1) {
				ISP_LOGI("afm_lum[%d]: %d, block:%d, afm_data[%d]:%d, block:%d, afm_data[%d]:%d",
					 i, af->afm_lum[i], j, LUM_INDEX(j), afm_data[LUM_INDEX(j)], j + 1, LUM_INDEX(j + 1), afm_data[LUM_INDEX(j + 1)]);
			}
		}
	}
#endif
...
#define FV1_INDEX(block) (((block) * 3) + 1)
	...
	if (STATE_FAF == af->state && FACE_NONE != af->f_orientation) { //face FV mapping
		...
		for (i = 0; i < 9; i++) {
			...
			af->af_fv_val.af_fv1[9] += af->af_fv_val.af_fv1[i];
			if (val == 1) {
				ISP_LOGE("i: %d, af_fv0[%d]: %15" PRIu64 ", j:%d, FV0_INDEX(%d):%d ,FV0_INDEX(%d):%d, af_fv_val[%d]:%d, af_fv_val[%d]:%d",
...
#else
		c_y_sum->y_sum[index] = af->afm_lum[index];
#endif
		index += 1;
	}
	return 0;
...
static cmr_u32 EnQueue(cmr_handle handle, void *param0)
{
	...
	af->queue.af_params[af->queue.rear].min_real_focus_distance = af_params->min_real_focus_distance;
	af->queue.af_params[af->queue.rear].set_cts_params_flag = 1;
	ISP_LOGI("rear= %d,focus_distance = %f,frame_num = %d,min_real_focus_distance : %d", af->queue.rear, af->queue.af_params[af->queue.rear].focus_distance,
		 af->queue.af_params[af->queue.rear].frame_num, af->queue.af_params[af->queue.rear].min_real_focus_distance);
...
static cmr_u32 af_sprd_set_focus_distance(cmr_handle handle, void *param0)
{
	...
	struct cts_af_params *af_params = (struct cts_af_params *)param0;
	ISP_LOGI("focus_distance = %f,frame_num = %d, min_real_focus_distance = %f", af_params->focus_distance, af_params->frame_num, af_params->min_real_focus_distance);
...
static cmr_u32 af_sprd_get_cts_result(cmr_handle handle, void *param0)
{
	...
	if (1 == af->queue.af_params[af->queue.rear].set_cts_params_flag) {
		...
		max_focus_distance = 100.0 / (af->queue.af_params[af->queue.rear].min_real_focus_distance);
		ISP_LOGI("focus_distance : %f,frame_num  : %d,min_real_focus_distance : %d,max_focus_distance : %f", cts_result->focus_distance,
			 cts_result->frame_num, af->queue.af_params[af->queue.rear].min_real_focus_distance, max_focus_distance);
		for (i = 0; i < 3; i++) {
		...
		if (((af->focus_distance_result[1] - af->focus_distance_result[2]) <= 1) && ((af->focus_distance_result[0] - af->focus_distance_result[1] <= 1))) {
			if (ABS_AB(max_focus_distance, focus_distance) < 0.5) {
				cts_result->lens_state = AF_MOTOR_IDLE;
			} else {
				cts_result->lens_state = AF_MOTOR_SET;
			}
		} else {
			cts_result->lens_state = AF_MOTOR_IDLE;
		}
	...
	if (NULL != pdaf_otp_info_ptr && NULL != module_info_ptr) {
		...
		if (NULL != pdaf_otp_info_ptr->rdm_info.data_addr && NULL != module_info) {
			if ((module_info[4] == 0 && module_info[5] == 1)
				...
				ISP_LOGV("type1 pdaf otp map v1.0 or v1.1, Addr:%p, 0:[%d]1:[%d]", af->pdaf_rdm_otp_data, *(af->pdaf_rdm_otp_data + 0), *(af->pdaf_rdm_otp_data + 1));
			} else {
...
{
	char value[PROPERTY_VALUE_MAX] = { '\0' };
	char *take_value = NULL;
	cmr_u32 property_id = 0;
	cmr_u32 property_dac = 0;
	af_ctrl_t *af = (af_ctrl_t *) handle;
	...
	property_get("debug.isp.af.idpos", value, "0");
	take_value = strtok(value, ":");
	if (take_value != NULL) {
		property_id = atoi(take_value);
