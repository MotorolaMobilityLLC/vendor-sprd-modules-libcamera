...
{
	cmr_u32 i, j;
	cmr_u16 pre_r_tab [ 32 * 32 ]={0};
	cmr_u16 pre_gr_tab[ 32 * 32 ]={0};
	cmr_u16 pre_gb_tab[ 32 * 32 ]={0};
	cmr_u16 pre_b_tab [ 32 * 32 ]={0};
	cmr_u16 new_r_tab [ 32 * 32 ]={0};
	cmr_u16 new_gr_tab[ 32 * 32 ]={0};
	cmr_u16 new_gb_tab[ 32 * 32 ]={0};
	cmr_u16 new_b_tab [ 32 * 32 ]={0};
	...
	ISP_LOGV("src_w=%d, src_h=%d, dst_w=%d, dst_h=%d, plane_flag=%d",
			src_w, src_h, dst_w, dst_h, is_plane);
	...
	if(src_w == dst_w && src_h == dst_h){
		...
		if(is_plane == 1){
			memcpy(out_r_tab , ch_r , src_w * src_h * sizeof(cmr_u16));
			memcpy(out_gr_tab, ch_gr, src_w * src_h * sizeof(cmr_u16));
			memcpy(out_gb_tab, ch_gb, src_w * src_h * sizeof(cmr_u16));
			memcpy(out_b_tab , ch_b , src_w * src_h * sizeof(cmr_u16));
		}else{
			lsc_interlace_to_channel(src_w, src_h, 3, out_r_tab, out_gr_tab, out_gb_tab, out_b_tab, src_tab);
		}
		ISP_LOGV("lsc_table_linear_scaler, src_rggb[%d,%d,%d,%d]", out_r_tab[0], out_gr_tab[0], out_gb_tab[0], out_b_tab[0]);

		// get contain from pre_tab
		for(j=0; j<src_h-2; j++){
			for(i=0; i<src_w-2; i++){
				pre_r_tab [ j*(src_w-2) + i ] = out_r_tab [ (j+1)*src_w + (i+1)];
				pre_gr_tab[ j*(src_w-2) + i ] = out_gr_tab[ (j+1)*src_w + (i+1)];
				pre_gb_tab[ j*(src_w-2) + i ] = out_gb_tab[ (j+1)*src_w + (i+1)];
				pre_b_tab [ j*(src_w-2) + i ] = out_b_tab [ (j+1)*src_w + (i+1)];
			}
		}

		// scale pre_contain to new_contain
		lsc_scale_bilinear_short(pre_r_tab , src_w-2, src_h-2, new_r_tab , dst_w-2, dst_h-2);
		lsc_scale_bilinear_short(pre_gr_tab, src_w-2, src_h-2, new_gr_tab, dst_w-2, dst_h-2);
		lsc_scale_bilinear_short(pre_gb_tab, src_w-2, src_h-2, new_gb_tab, dst_w-2, dst_h-2);
		lsc_scale_bilinear_short(pre_b_tab , src_w-2, src_h-2, new_b_tab , dst_w-2, dst_h-2);

		// set contain to output tab
		for(j=0; j<dst_h-2; j++){
			for(i=0; i<dst_w-2; i++){
				out_r_tab [(j+1)*dst_w + (i+1)] = new_r_tab [j*(dst_w-2) + i];
				out_gr_tab[(j+1)*dst_w + (i+1)] = new_gr_tab[j*(dst_w-2) + i];
				out_gb_tab[(j+1)*dst_w + (i+1)] = new_gb_tab[j*(dst_w-2) + i];
				out_b_tab [(j+1)*dst_w + (i+1)] = new_b_tab [j*(dst_w-2) + i];
			}
		}

		// set top and bottom edge
		for(i=1; i<dst_w-1; i++){
			out_r_tab [0*dst_w + i] = 3*out_r_tab [1*dst_w + i] - 3*out_r_tab [2*dst_w + i] + out_r_tab [3*dst_w + i];
			out_gr_tab[0*dst_w + i] = 3*out_gr_tab[1*dst_w + i] - 3*out_gr_tab[2*dst_w + i] + out_gr_tab[3*dst_w + i];
			out_gb_tab[0*dst_w + i] = 3*out_gb_tab[1*dst_w + i] - 3*out_gb_tab[2*dst_w + i] + out_gb_tab[3*dst_w + i];
			out_b_tab [0*dst_w + i] = 3*out_b_tab [1*dst_w + i] - 3*out_b_tab [2*dst_w + i] + out_b_tab [3*dst_w + i];
			out_r_tab [(dst_h-1)*dst_w + i] = 3*out_r_tab [(dst_h-2)*dst_w + i] - 3*out_r_tab [(dst_h-3)*dst_w + i] + out_r_tab [(dst_h-4)*dst_w + i];
			out_gr_tab[(dst_h-1)*dst_w + i] = 3*out_gr_tab[(dst_h-2)*dst_w + i] - 3*out_gr_tab[(dst_h-3)*dst_w + i] + out_gr_tab[(dst_h-4)*dst_w + i];
			out_gb_tab[(dst_h-1)*dst_w + i] = 3*out_gb_tab[(dst_h-2)*dst_w + i] - 3*out_gb_tab[(dst_h-3)*dst_w + i] + out_gb_tab[(dst_h-4)*dst_w + i];
			out_b_tab [(dst_h-1)*dst_w + i] = 3*out_b_tab [(dst_h-2)*dst_w + i] - 3*out_b_tab [(dst_h-3)*dst_w + i] + out_b_tab [(dst_h-4)*dst_w + i];
		}

		// set left and right edge
		for(j=0; j<dst_h; j++){
			out_r_tab [j*dst_w + 0] = 3*out_r_tab [j*dst_w + 1] - 3*out_r_tab [j*dst_w + 2] + out_r_tab [j*dst_w + 3];
			out_gr_tab[j*dst_w + 0] = 3*out_gr_tab[j*dst_w + 1] - 3*out_gr_tab[j*dst_w + 2] + out_gr_tab[j*dst_w + 3];
			out_gb_tab[j*dst_w + 0] = 3*out_gb_tab[j*dst_w + 1] - 3*out_gb_tab[j*dst_w + 2] + out_gb_tab[j*dst_w + 3];
			out_b_tab [j*dst_w + 0] = 3*out_b_tab [j*dst_w + 1] - 3*out_b_tab [j*dst_w + 2] + out_b_tab [j*dst_w + 3];
			out_r_tab [j*dst_w + (dst_w-1)] = 3*out_r_tab [j*dst_w + (dst_w-2)] - 3*out_r_tab [j*dst_w + (dst_w-3)] + out_r_tab [j*dst_w + (dst_w-4)];
			out_gr_tab[j*dst_w + (dst_w-1)] = 3*out_gr_tab[j*dst_w + (dst_w-2)] - 3*out_gr_tab[j*dst_w + (dst_w-3)] + out_gr_tab[j*dst_w + (dst_w-4)];
			out_gb_tab[j*dst_w + (dst_w-1)] = 3*out_gb_tab[j*dst_w + (dst_w-2)] - 3*out_gb_tab[j*dst_w + (dst_w-3)] + out_gb_tab[j*dst_w + (dst_w-4)];
			out_b_tab [j*dst_w + (dst_w-1)] = 3*out_b_tab [j*dst_w + (dst_w-2)] - 3*out_b_tab [j*dst_w + (dst_w-3)] + out_b_tab [j*dst_w + (dst_w-4)];
		}
		...
		if(is_plane == 1){
			...
			memcpy(ch_r , out_r_tab , dst_w * dst_h * sizeof(cmr_u16));
			memcpy(ch_gr, out_gr_tab, dst_w * dst_h * sizeof(cmr_u16));
			memcpy(ch_gb, out_gb_tab, dst_w * dst_h * sizeof(cmr_u16));
			memcpy(ch_b , out_b_tab , dst_w * dst_h * sizeof(cmr_u16));
		}else{
			lsc_interlace_to_channel(dst_w, dst_h, 3, out_r_tab, out_gr_tab, out_gb_tab, out_b_tab, dst_tab);
		}
	}
}
...
static cmr_s32 lsc_master_slave_sync(struct lsc_sprd_ctrl_context* cxt, struct alsc_fwprocstart_info* fwprocstart_info)
{
	...
	memcpy(lsc_new_table, fwprocstart_info->lsc_tab_address_new[0], new_width * new_height * 4 * sizeof(unsigned short));
	ISP_LOGV("slave size[%d,%d], slave_DNP[%d,%d,%d,%d]", new_width, new_height,
		lsc_new_table[0], lsc_new_table[1], lsc_new_table[2], lsc_new_table[3]);

	new_pattern = lsc_raw_pattern_to_lsc_pattern(fwprocstart_info->image_pattern_new);

	// scale master dnp table to slave size
	lsc_table_linear_scaler(lsc_pre_table, pre_width, pre_height, output_tab, new_width, new_height, 0);
	...
	lsc_interlace_to_channel(new_width, new_height, new_pattern, output_r_tab_new,
		output_gr_tab_new, output_gb_tab_new, output_b_tab_new, lsc_new_table);
	...
	for(i=0; i< new_width * new_height ;i++){
		...
		if( out_gr_tab[i] == 0 || out_gr_tab[i]==1024 ){
			...
			rate_gr=(float)output_gr_tab_new[i]/(float)out_gr_tab[i];
		}
		...
		if( out_gb_tab[i] == 0 || out_gb_tab[i]==1024 ){
			...
			rate_gb=(float)output_gb_tab_new[i]/(float)out_gb_tab[i];
		}
		...
		if( out_r_tab[i] == 0 || out_r_tab[i]==1024 ){
			...
			rate_r=(float)output_r_tab_new[i]/(float)out_r_tab[i];
		}
		...
		if( out_b_tab[i] == 0 || out_b_tab[i]==1024 ){
			...
			rate_b=(float)output_b_tab_new[i]/(float)out_b_tab[i];
		}
	...
	lsc_table_linear_scaler(lsc_pre_reslut_table, pre_width, pre_height, lsc_result_address_new, new_width, new_height, 0);
...
static cmr_int lsc_parser_otp(struct lsc_adv_init_param *lsc_param)
{
	...
	if((3264-100 <= full_img_width && full_img_width <= 3264+100 && 2448-100 <= full_img_height && full_img_height <= 2448+100 && lsc_param->grid == 128)
	 ||(4224-100 <= full_img_width && full_img_width <= 4224+100 && 3136-100 <= full_img_height && full_img_height <= 3136+100 && lsc_param->grid == 128))
		lsc_otp_grid = 96;
	lsc_get_otp_size_info(full_img_width, full_img_height, &lsc_otp_width, &lsc_otp_height, lsc_otp_grid);
	...
	if ((lsc_otp_addr != NULL) && (lsc_otp_len != 0)) {
		...
		lsc_table = lsc_table_wrapper(lsc_16_bits, lsc_otp_grid, full_img_width, full_img_height, &gain_w, &gain_h);	//  wrapper otp table
		...
		if((3264-100 <= full_img_width && full_img_width <= 3264+100 && 2448-100 <= full_img_height && full_img_height <= 2448+100 && lsc_param->grid == 128)
		 ||(4224-100 <= full_img_width && full_img_width <= 4224+100 && 3136-100 <= full_img_height && full_img_height <= 3136+100 && lsc_param->grid == 128)){
			lsc_get_otp_size_info(full_img_width, full_img_height, &lsc_otp_width, &lsc_otp_height, 128);
			lsc_table_linear_scaler(lsc_table, gain_w, gain_h, lsc_table, lsc_otp_width+2, lsc_otp_height+2, 1);
		...
		ISP_LOGV("lsc_table0_RGGB=[%d,%d,%d,%d]", lsc_table[0], lsc_table[gain_w * gain_h], lsc_table[gain_w * gain_h * 2], lsc_table[gain_w * gain_h * 3]);
		ISP_LOGV("lsc_table1_RGGB=[%d,%d,%d,%d]", lsc_table[gain_w + 1], lsc_table[gain_w * gain_h + gain_w + 1],
			lsc_table[gain_w * gain_h * 2 + gain_w + 1], lsc_table[gain_w * gain_h * 3 + gain_w + 1]);
	} else {
...
static cmr_s32 lsc_set_init_param(struct lsc_adv_init_param *init_param, struct lsc_sprd_ctrl_context *cxt,
	struct lsc_sprd_init_in *sprd_init_param)
{
	...
	if( init_param->tune_param_ptr != NULL){
		...
		lsc_gen_flash_y_gain(cxt->flash_y_gain, cxt->gain_width, cxt->gain_height,
			param->flash_enhance_en, param->flash_enhance_max_strength, param->flash_enahnce_gain);
	}else{
		cxt->alg_bypass = 1;
	...
	if(cxt->is_planar == 1){
		for(i=0; i<8; i++){
			lsc_table_interlace2planar(init_param->lsc_tab_address[i], cxt->init_gain_width, cxt->init_gain_height, cxt->gain_pattern, cxt->output_gain_pattern);
		}
	} else {
		for(i=0; i < 8; i++)
			lsc_interlace_change_pattern(init_param->lsc_tab_address[i], cxt->init_gain_width, cxt->init_gain_height, cxt->gain_pattern, cxt->output_gain_pattern);
	}
...
static cmr_s32 lsc_fwstart_update_first_tab(struct lsc_sprd_ctrl_context *cxt, struct alsc_fwstart_info* fwstart_info, cmr_u32 planar_flag)
{
	...
	ISP_LOGV("FW_START, pre:[%d, %d, %d], new:[%d, %d, %d]",
			pre_width, pre_height, pre_pattern, new_width, new_height, new_pattern);

	if(pre_width == new_width && pre_height == new_height){
		memcpy(lsc_result_address_new, lsc_pre_reslut_table, pre_width*pre_height*4*sizeof(cmr_u16));
	}else{
		...
		lsc_table_linear_scaler(lsc_pre_table, pre_width, pre_height, output_tab, new_width, new_height, planar_flag);

		if (cxt->is_planar == 0) {
			lsc_interlace_to_channel(new_width, new_height, pre_pattern, output_r_tab, output_gr_tab, output_gb_tab, output_b_tab, output_tab);
		} else {
			lsc_planar_to_channel(new_width, new_height, pre_pattern, output_r_tab, output_gr_tab, output_gb_tab, output_b_tab, output_tab);
		}
		...
		if (cxt->is_planar == 0) {
			lsc_interlace_to_channel(new_width, new_height, new_pattern, output_r_tab_new, output_gr_tab_new, output_gb_tab_new, output_b_tab_new, lsc_new_table);
		} else {
			lsc_planar_to_channel(new_width, new_height, new_pattern, output_r_tab_new, output_gr_tab_new, output_gb_tab_new, output_b_tab_new, lsc_new_table);
		}
		...
		if(cxt->is_planar == 0) {
			lsc_interlace_to_channel(new_width, new_height, cxt->output_gain_pattern, output_r, output_gr,
				output_gb, output_b, lsc_result_address_new);
		} else {
			lsc_planar_to_channel(new_width, new_height, cxt->output_gain_pattern, output_r, output_gr,
				output_gb, output_b, lsc_result_address_new);
		}
		ISP_LOGV("FW_START, pre_scaled_DNP[%d,%d,%d,%d], pre_scaled_result[%d,%d,%d,%d]",
		...
		if(cxt->is_planar == 0){
			for(i = 0; i< new_width * new_height; i++){
				lsc_result_address_new[4*i + off_gr] = RANGE(output_gr[i] * lsc_new_weight_tab_gr[i], 1024, 16383);
				lsc_result_address_new[4*i + off_r] = RANGE(output_r [i] * lsc_new_weight_tab_r[i], 1024, 16383);
				lsc_result_address_new[4*i + off_b] = RANGE(output_b [i] * lsc_new_weight_tab_b[i], 1024, 16383);
				lsc_result_address_new[4*i + off_gb] = RANGE(output_gb[i] * lsc_new_weight_tab_gb[i], 1024, 16383);
			}
		}else{
			for(i = 0; i< new_width * new_height; i++){
				lsc_result_address_new[i + gr_chnl_off] = RANGE(output_gr[i] * lsc_new_weight_tab_gr[i], 1024, 16383);
				lsc_result_address_new[i + r_chnl_off] = RANGE(output_r [i] * lsc_new_weight_tab_r[i], 1024, 16383);
				lsc_result_address_new[i + b_chnl_off] = RANGE(output_b [i] * lsc_new_weight_tab_b[i], 1024, 16383);
				lsc_result_address_new[i + gb_chnl_off] = RANGE(output_gb[i] * lsc_new_weight_tab_gb[i], 1024, 16383);
			}
		}

		ISP_LOGV("FW_START, pre_scaled_weighted_result[%d,%d,%d,%d]",
				lsc_result_address_new[0], lsc_result_address_new[1], lsc_result_address_new[2], lsc_result_address_new[3]);
	}

	// update zone
	memcpy(cxt->fwstart_new_scaled_table, lsc_result_address_new, new_width*new_height*4*sizeof(cmr_u16));

	// binning preflash -> fullsize main flash
	memcpy(flash_param->preflash_guessing_mainflash_output_table, lsc_result_address_new, new_width*new_height*4*sizeof(cmr_u16));

	ISP_LOGV("FW_START, fwstart_update_first_tab done, lsc_result_address_new=[%d,%d,%d,%d]",
		lsc_result_address_new[0], lsc_result_address_new[1], lsc_result_address_new[2], lsc_result_address_new[3]);
    return 0;
}
...
static int lsc_preprocess_fwstart_info(struct lsc_sprd_ctrl_context *cxt, struct alsc_fwstart_info* fwstart_info)
{
	...
	if(cxt->LSC_SPD_VERSION <= 5){
		if(cxt->is_planar == 1){
			for(i=0; i<8; i++){
				lsc_table_interlace2planar(fwstart_info->lsc_tab_address_new[i], fwstart_info->gain_width_new,
					fwstart_info->gain_height_new, fwstart_info->image_pattern_new, cxt->output_gain_pattern);
			}
		}else{
			for(i=0; i<8; i++){
				lsc_interlace_change_pattern(fwstart_info->lsc_tab_address_new[i], fwstart_info->gain_width_new,
					fwstart_info->gain_height_new, fwstart_info->image_pattern_new, cxt->output_gain_pattern);
			}
		}
	} else {
...
static cmr_s32 lsc_sprd_calculation(void *handle, void *in, void *out)
{
	...
	ISP_LOGD("frame_count=%d, alg_in=%d, lsc_id=%d, alg_count = %d, can_update_dest=%d",
		cxt->frame_count, alg_in, cxt->lsc_id, cxt->alg_count, cxt->can_update_dest);
	...
	if(!cxt->cmd_alsc_bypass){
		if(cxt->alg_count <= cxt->alg_quick_in_frame + 1
			&& cxt->frame_count < cxt->calc_freq*3 + 1
			&& cxt->can_update_dest == 1
			&& param->bv*cxt->fw_start_bv > 0){
			memcpy(cxt->output_lsc_table, cxt->fwstart_new_scaled_table, gain_width * gain_height * 4 * sizeof(unsigned short));
	...
	ISP_LOGV("final output [%d,%d,%d,%d]", cxt->lsc_buffer[0], cxt->lsc_buffer[1], cxt->lsc_buffer[2], cxt->lsc_buffer[3]);
	// alsc_calc --
...
static cmr_s32 lsc_sprd_ioctrl(void *handle, cmr_s32 cmd, void *in, void *out)
{
	...
	switch(cmd){
		case ALSC_FW_START:// You have to update two table in FW_START: 1.fwstart_info->lsc_result_address_new, 2.cxt->fwstart_new_scaled_table
			...
			// preprocess fwstart_info->lsc_tab_address_new
			...
			if(cxt->LSC_SPD_VERSION <= 5){
				...
				if(fwstart_info->gain_width_new == 23 && fwstart_info->gain_height_new == 15 && fwstart_info->grid_new == 32){
					...
					if(cxt->lsc_id == 1 && fwstart_info->gain_width_new  == lsc_last_info->gain_width && fwstart_info->gain_height_new == lsc_last_info->gain_height){
						...
						ISP_LOGV("FW_START, Send TL84 table");
						memcpy(fwstart_info->lsc_result_address_new, fwstart_info->lsc_tab_address_new[DEFAULT_TAB_INDEX], chnl_gain_num *4*sizeof(cmr_u16));
						will_do_post_gain = 1;
					}
				}else{
					...
					// normal change mode & flash change mode, with different param file
						...
						ISP_LOGV("FW_START, Normal change mode, prepare to update first tab");
						change_mode_rtn = lsc_fwstart_update_first_tab(cxt, fwstart_info, cxt->is_planar);
					}

					if(change_mode_rtn == -1)
						ISP_LOGV("FW_START, Change Mode Failed");
				}
			} else {
				// when cxt->LSC_SPD_VERSION >= 6
		...
		case  ALSC_FW_STOP:
			...
			cxt->alsc_update_flag = 0;
			ISP_LOGV("FW_STOP can_update_dest=%d, alsc_update_flag=%d, is_touch_preflash=%d", cxt->can_update_dest,
				cxt->alsc_update_flag, flash_param->is_touch_preflash);
			...
			if(cxt->flash_y_gain[0] == 0){
				...
				if(cxt->is_planar){
					...
					for(i=0; i<cxt->gain_width*cxt->gain_height; i++){
						cxt->lsc_buffer[i + 0 * chnl_gain_num] = flash_param->preflash_guessing_mainflash_output_table[i + 0 * chnl_gain_num] * cxt->flash_y_gain[i] / 1024;
						cxt->lsc_buffer[i + 1 * chnl_gain_num] = flash_param->preflash_guessing_mainflash_output_table[i + 1 * chnl_gain_num] * cxt->flash_y_gain[i] / 1024;
						cxt->lsc_buffer[i + 2 * chnl_gain_num] = flash_param->preflash_guessing_mainflash_output_table[i + 2 * chnl_gain_num] * cxt->flash_y_gain[i] / 1024;
						cxt->lsc_buffer[i + 3 * chnl_gain_num] = flash_param->preflash_guessing_mainflash_output_table[i + 3 * chnl_gain_num] * cxt->flash_y_gain[i] / 1024;
					}
				}else{
					for(i=0; i<cxt->gain_width*cxt->gain_height; i++){
		...
		case ALSC_FW_PROC_START: //for sbs feature now
			...
			ISP_LOGV("FW_PROC_START, is_master=%d, is_multi_mode=%d, frame_count=%d, lsc_id=%d",
				cxt->is_master, cxt->is_multi_mode, cxt->frame_count, cxt->lsc_id);
			...
			if(cxt->LSC_SPD_VERSION <= 5){
				...
				// pre process fwprocstart_info->lsc_tab_address_new
				if(cxt->is_planar == 1){
					for(i=0; i<8; i++){
						lsc_table_interlace2planar(fwprocstart_info->lsc_tab_address_new[i], fwprocstart_info->gain_width_new,
							fwprocstart_info->gain_height_new, fwprocstart_info->image_pattern_new, cxt->output_gain_pattern);
					}
				}else{
					for(i=0; i<8; i++){
						lsc_interlace_change_pattern(fwprocstart_info->lsc_tab_address_new[i], fwprocstart_info->gain_width_new,
							fwprocstart_info->gain_height_new, fwprocstart_info->image_pattern_new, cxt->output_gain_pattern);
					}
				}
				...
				if(cxt->is_multi_mode == 2 ){
					...
					if(cxt->lsc_id == 1){
						...
						lsc_table_linear_scaler(cxt->output_lsc_table, cxt->gain_width, cxt->gain_height,
							proc_start_output_table, proc_start_gain_w, proc_start_gain_h, 1); // change plane_flag from 0 to 1
		...
		case ALSC_GET_UPDATE_INFO:
			...
			//for debug:  planar2interlace for driver
			if(cxt->is_planar == 1){
				tmp_buffer = (cmr_u16*)malloc(32 * 32 * 4 * sizeof(cmr_u16));
				memcpy(tmp_buffer, cxt->lsc_buffer, cxt->gain_width*cxt->gain_height*4*sizeof(cmr_u16));
				for(i = 0; i<cxt->gain_width*cxt->gain_height; i++){
					cxt->lsc_buffer[4*i + 0] = tmp_buffer[cxt->gain_width*cxt->gain_height*0 + i];
					cxt->lsc_buffer[4*i + 1] = tmp_buffer[cxt->gain_width*cxt->gain_height*1 + i];
					cxt->lsc_buffer[4*i + 2] = tmp_buffer[cxt->gain_width*cxt->gain_height*2 + i];
					cxt->lsc_buffer[4*i + 3] = tmp_buffer[cxt->gain_width*cxt->gain_height*3 + i];
				}
		...
		case ALSC_DO_SIMULATION:
			...
			memcpy(lsc_calc_in->last_lsc_table, cxt->std_init_lsc_table_param_buffer[DEFAULT_TAB_INDEX],
				cxt->init_gain_width*cxt->init_gain_height*4*sizeof(cmr_u16));
