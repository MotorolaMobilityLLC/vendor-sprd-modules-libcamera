...
static cmr_int seq_get_cur_actual_element(struct seq_cxt *cxt_ptr, struct seq_cell *out_ptr)
{
	...
	if (NULL == cxt_ptr || NULL == out_ptr) {
		ISP_LOGE("fail to get cur actual ele, cxt_ptr %p, out_ptr %p", cxt_ptr, out_ptr);
		goto exit;
	}
...
static cmr_int seq_push_actual_element(struct seq_cxt *cxt_ptr, cmr_u32 is_add_idx, cmr_u32 offset, struct seq_cell *in_ptr)
{
	...
	if (NULL == cxt_ptr || NULL == in_ptr) {
		ISP_LOGE("fail to push actual ele, cxt_ptr %p, in_ptr %p", cxt_ptr, in_ptr);
		goto exit;
	}
...
static cmr_int seq_get_cur_write_element(struct seq_cxt *cxt_ptr, struct seq_cell *out_ptr)
{
	...
	if (NULL == cxt_ptr || NULL == out_ptr) {
		ISP_LOGE("fail to get cur write ele, cxt_ptr %p, out_ptr %p", cxt_ptr, out_ptr);
		goto exit;
	}
...
static cmr_int seq_push_write_element(struct seq_cxt *cxt_ptr, cmr_u32 is_add_idx, cmr_u32 offset, struct seq_cell *in_ptr)
{
	...
	if (NULL == cxt_ptr || NULL == in_ptr) {
		ISP_LOGE("fail to push write ele, cxt_ptr %p, in_ptr %p", cxt_ptr, in_ptr);
		goto exit;
	}
...
cmr_int seq_init(cmr_u32 queue_num, struct seq_init_in * in_ptr, void **handle)
{
	...
	if ((queue_num < 4)
		...
		ISP_LOGE("fail to init seq, queue_num %d, in_ptr %p, handle %p", queue_num, in_ptr, handle);
		goto exit;
	}
	*handle = NULL;
...
cmr_int seq_deinit(void *handle)
{
	...
	if (NULL == handle) {
		ISP_LOGE("fail to deinit seq, handle %p ", handle);
		goto exit;
	}
...
cmr_int seq_put(void *handle, struct seq_item * in_est_ptr, struct seq_cell * out_actual_ptr, struct seq_cell * out_write_ptr)
{
	...
	if (NULL == handle || NULL == in_est_ptr || NULL == out_actual_ptr || NULL == out_write_ptr) {
		ISP_LOGE("fail to put seq, handle %p, in_est_ptr %p, out_actual_ptr %p, out_write_ptr %p", handle, in_est_ptr, out_actual_ptr, out_write_ptr);
		goto EXIT;
	}
