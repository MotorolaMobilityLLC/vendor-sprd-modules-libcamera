		...
		if (FACE_UP == af->f_orientation || FACE_DOWN == af->f_orientation) {	//6x4
			if (FACE_UP == af->f_orientation) {
				...
				if (0 == af->win_offset) {
					...
					if (val == 1) {
						...
						ISP_LOGE("i: 1, af_fv0[1]: %15" PRIu64 ", j: 13, FV0_INDEX(13):%d , af_fv_val[%d]:%d", af->af_fv_val.af_fv0[1], FV0_INDEX(13),
					...
					if (val == 1) {
						...
						ISP_LOGE("i: 1, af_fv0[1]: %15" PRIu64 ", j: 14, FV0_INDEX(14):%d , af_fv_val[%d]:%d", af->af_fv_val.af_fv0[1], FV0_INDEX(14),
							 FV0_INDEX(14), af_fv_val[FV0_INDEX(14)]);
			...
			if (FACE_LEFT == af->f_orientation) {
				for (i = 0; i < 6; i++) {	//3
					...
					if (val == 1) {
						ISP_LOGE("i: %d, af_fv0[%d]: %15" PRIu64 ", j:%d, FV0_INDEX(%d):%d , af_fv_val[%d]:%d",
							 i / 2 * 3 + i % 2, i / 2 * 3 + i % 2, af->af_fv_val.af_fv0[i / 2 * 3 + i % 2], i / 2 * 6 + i % 2, i / 2 * 6 + i % 2,
							 FV0_INDEX(i / 2 * 6 + i % 2), FV0_INDEX(i / 2 * 6 + i % 2), af_fv_val[FV0_INDEX(i / 2 * 6 + i % 2)]);
					}
				}
				...
				if (val == 1) {
					...
					ISP_LOGE("i: 5, af_fv0[5]: %15" PRIu64 ", j: 8, FV0_INDEX(8):%d , af_fv_val[%d]:%d", af->af_fv_val.af_fv0[5], FV0_INDEX(8), FV0_INDEX(8),
						 af_fv_val[FV0_INDEX(8)]);
				...
				if (val == 1) {
					...
					ISP_LOGE("i: 3, af_fv0[3]: %15" PRIu64 ", j: 9, FV0_INDEX(9):%d , af_fv_val[%d]:%d",
						 af->af_fv_val.af_fv0[3], FV0_INDEX(9), FV0_INDEX(9), af_fv_val[FV0_INDEX(9)]);
		...
		for (i = 0; i < 9; i++) {
			...
			if (val == 1) {
				ISP_LOGI("i: %d, af_fv0[%d]: %15" PRIu64 ", j:%d, FV0_INDEX(%d):%d ,FV0_INDEX(%d):%d, af_fv_val[%d]:%d, af_fv_val[%d]:%d",
					 i, i, af->af_fv_val.af_fv0[i], j, j, FV0_INDEX(j), j + 1, FV0_INDEX(j + 1), FV0_INDEX(j), af_fv_val[FV0_INDEX(j)], FV0_INDEX(j + 1),
					 af_fv_val[FV0_INDEX(j + 1)]);
			}
		}
	}
#endif
...
cmr_s32 sprd_afv1_process(cmr_handle handle, void *in, void *out)
{
	...
	struct af_status_info status_tele;
	struct aft_proc_result sync_result;
	cmr_u32 pd_workable = 0;

	memset(&status_info, 0, sizeof(struct af_status_info));
	memset(&status_wide, 0, sizeof(struct af_status_info));
	memset(&status_tele, 0, sizeof(struct af_status_info));
	...
	if (af->bridge_ctrl != NULL && AF_ALG_DUAL_W_T == af->is_multi_mode && AF_ROLE_TELE == af->sensor_role) {
		...
		if (AF_SEARCHING == status_wide.af_status && (AF_IDLE == af->focus_state || AF_STOPPED == af->focus_state)) {
			...
			return rtn;
		} else if (AF_SEARCHING == status_wide.af_status && AF_SEARCHING == af->focus_state) {
		} else if (AF_STOPPED == status_wide.af_status && (AF_IDLE == af->focus_state || AF_STOPPED == af->focus_state)) {
		} else if (AF_STOPPED == status_wide.af_status && AF_SEARCHING == af->focus_state) {
			af_stop_search(af);
			return rtn;
		}
	}
