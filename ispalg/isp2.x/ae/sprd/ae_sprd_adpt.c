...
static cmr_s32 ae_sync_write_to_sensor(struct ae_ctrl_cxt *cxt, struct ae_exposure_param *write_param)
{
	...
	if (0 !=  write_param->exp_line && 0 != write_param->sensor_gain) {
		...
		if ((write_param->exp_line != prv_param->exp_line)
			...
			ISP_LOGV("ae_info[0] exposure %d dummy %d size_index %d gain %d",
				ae_info[0].exp.exposure, ae_info[0].exp.dummy,
				ae_info[0].exp.size_index, ae_info[0].gain);
			...
			ISP_LOGV("ae_info[1] exposure %d dummy %d size_index %d gain %d",
				ae_info[1].exp.exposure, ae_info[1].exp.dummy,
				ae_info[1].exp.size_index, ae_info[1].gain);
...
static cmr_s32 ae_adjust_exp_gain(struct ae_ctrl_cxt *cxt,
...
static cmr_s32 ae_cvt_exposure_time2line(struct ae_exp_gain_table* src[AE_FLICKER_NUM],
...
SET_SCENE_MOD_EXIT:
	ISP_LOGV("cam-id %d, change scene mode from %d to %d, rtn=%d", cxt->camera_id,
...
static cmr_s32 ae_make_isp_result(struct ae_ctrl_cxt *cxt,
										struct ae_alg_calc_result *alg_rt,
										struct ae_ctrl_callback_in *result)
{
	cmr_s32 rtn = AE_SUCCESS;
	cmr_u32 i = 0x00;
...
static void ae_set_video_stop(struct ae_ctrl_cxt *cxt)
{
	if (0 == cxt->is_snapshot) {
		...
		ISP_LOGI("AE_VIDEO_STOP(in preview) cam-id %d BV %d BV_backup %d E %d G %d lt %d W %d H %d,enable: %d", cxt->camera_id , cxt->last_exp_param.bv, s_bakup_exp_param[cxt->camera_id].bv, cxt->last_exp_param.exp_line,
		...
		ISP_LOGI("AE_VIDEO_STOP(in capture) cam-id %d BV %d BV_backup %d E %d G %d lt %d W %d H %d, enable: %d", cxt->camera_id , cxt->last_exp_param.bv, s_bakup_exp_param[cxt->camera_id].bv, cxt->last_exp_param.exp_line,
				cxt->last_exp_param.gain, cxt->last_exp_param.line_time, cxt->snr_info.frame_size.w, cxt->snr_info.frame_size.h, cxt->last_enable);
	}
}
...
static cmr_s32 ae_set_video_start(struct ae_ctrl_cxt *cxt, cmr_handle *param)
{
	...
	else cxt->binning_factor_before = 1;
	...
	else cxt->binning_factor_after = 1;
	...
	for (j = 0; j < AE_SCENE_NUM; ++j) {
		...
		if (1 == cxt->tuning_param[mode].scene_info[j].table_enable) {
			ae_cvt_exposure_time2line(src, dst, cxt->cur_status.line_time,
	...
	ISP_LOGV("Camera_id %d, sync_BV %d, cur_BV %d, Backup_bv %d, Backup_exp_line %d, Backup_exp_time %d, Backup_dummy %d, Backup_gain %d",
		...
		s_bakup_exp_param[cxt->camera_id].dummy,
		s_bakup_exp_param[cxt->camera_id].gain);
...
static cmr_s32 ae_set_ev_offset(struct ae_ctrl_cxt *cxt, void *param)
{
	if (param) {
		...
		if (ev->level < AE_LEVEL_MAX) {
			...
			cxt->cur_status.target_lum = ae_calc_target_lum(cxt->cur_param->target_lum,
...
static cmr_s32 ae_set_hdr_start(struct ae_ctrl_cxt *cxt, void *param)
{
	...
	ISP_LOGV("AE_SET_HDR: hdr_enable %d, hdr_cb_cnt %d, base_ae_idx %d",
		cxt->hdr_enable, cxt->hdr_cb_cnt, cxt->hdr_base_ae_idx);
	}
...
static cmr_s32 ae_get_led_ctrl(struct ae_ctrl_cxt *cxt, cmr_handle result)
{
	if (result) {
		...
		ISP_LOGV("AE_SET_LED led0_enable=%d, led1_enable=%d", ae_leds_ctrl->led0_ctrl,
...
/*update parameters to sensor*/
	...
	if (cxt->is_multi_mode == ISP_ALG_DUAL_SBS && (!cxt->sensor_role))
	{
		...
		ISP_LOGV("cur_bv %d cur_again %d cur_exp_line %d exposure_time %d",
			current_result->cur_bv,
			current_result->wts.cur_again,
			current_result->wts.cur_exp_line,
