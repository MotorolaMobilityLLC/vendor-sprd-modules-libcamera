...
adb push vdsp_firmware.bin /vendor/firmware
adb push hdr_cadence.bin /vendor/firmware
adb push libsprdhdr.so /vendor/%libdir%

adb shell chmod 777 /data/test_hdr
adb shell setprop persist.vendor.cam.hdr2.dump_midbuf yes
adb shell setprop persist.vendor.cam.hdr2.vdsp_log_level 3


echo cd data > temp
echo rm /data/vendor/cameraserver/*.bin >> temp
echo rm /data/vendor/cameraserver/*.yuv >> temp
echo rm hdr_output.bin >> temp
