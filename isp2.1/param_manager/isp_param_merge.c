										...
										void* pack_buf,uint32_t* pack_buf_size)
{
	...
	if (PNULL == awb_ctrl_param) {
		ISP_LOGE("alloc awb_ctrl_param mem error");
		rtn = ISP_ERROR;
		goto EXIT;
	}
	...
	ISP_LOGE("awb_ctrl_param = %d",sizeof(struct awb_param_ctrl));
	...
	if (PNULL == awb_alg_common_param) {
		ISP_LOGE("alloc awb_alg_common_param mem error");
		rtn = ISP_ERROR;
		goto EXIT;
	}
	memset(awb_alg_common_param,0x00,sizeof(struct awb_alg_param_common));
	ISP_LOGE("awb_alg_common_param = %d",sizeof(struct awb_alg_param_common));
	...
	if (PNULL == ctrl_pack_buf) {
		ISP_LOGE("alloc ctrl_pack_buf mem error");
		rtn = ISP_ERROR;
		goto EXIT;
	}
	...
	if (PNULL == alg_pack_buf) {
		ISP_LOGE("alloc alg_pack_buf mem error");
		rtn = ISP_ERROR;
		goto EXIT;
	}
