...
static cmr_u32 isp_pm_active_mode_param_update(struct isp_context *isp_cxt_ptr, struct isp_pm_mode_param *mode_param, struct isp_pm_mode_update_status *mode_convert_status)
{
	...
	if ((PNULL == isp_cxt_ptr)
		...
		ISP_LOGE("param ptr is null error:%p %p %p", isp_cxt_ptr, mode_param, mode_convert_status);
		rtn = ISP_ERROR;
		return rtn;
	}
	...
	if ((PNULL != blk_cfg_ptr) && (PNULL != blk_header_ptr)) {
		if (blk_cfg_ptr->ops) {
			...
			if ((PNULL == blk_ptr)
			    || (PNULL == blk_header_ptr)) {
				rtn = ISP_ERROR;
				ISP_LOGE("param is null error: blk_addr:%p, param data_ptr:%p, header:%p", blk_ptr, param_data_ptr, blk_header_ptr);
			} else {
