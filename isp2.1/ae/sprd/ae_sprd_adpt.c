...
static cmr_s32 _set_restore_cnt(struct ae_ctrl_cxt *cxt)
{
	...
	if (2 > cxt->cur_status.settings.pause_cnt) {
		cxt->cur_status.settings.lock_ae = cxt->bakup_ae_lock_status;
	...
	if (0 != cxt->flash_ver) {
		...
		if ((FLASH_MAIN_BEFORE == current_status->settings.flash) || (FLASH_MAIN == current_status->settings.flash)) {
			if (cxt->flash_esti_result.isEnd) {
				...
				ISP_LOGV("ae_flash main_b to sensor: %f, %d, %d", cxt->flash_esti_result.captureExposure, current_status->settings.exp_line, current_status->settings.gain);
			} else {
...
cmr_handle ae_sprd_init(cmr_handle param, cmr_handle in_param)
{
	...
	init_in.preview_skip_num = cxt->preview_skip_num;
	init_in.capture_skip_num = cxt->capture_skip_num;
...
static void _set_ae_video_stop(struct ae_ctrl_cxt *cxt)
{
	...
	ISP_LOGI("AE_VIDEO_STOP cam-id %d E %d G %d lt %d W %d H %d", cxt->camera_id ,cxt->last_expline,
		cxt->last_aegain, cxt->last_linetime, cxt->snr_info.frame_size.w, cxt->snr_info.frame_size.h);
}
...
cmr_s32 ae_sprd_io_ctrl(cmr_handle handle, cmr_s32 cmd, cmr_handle param, cmr_handle result)
{
	...
	case AE_HDR_START:
		...
		ISP_LOGI("AE_SET_HDR: hdr_enable %d", cxt->hdr_enable);
		}
		break;
