...
static int32_t _set_video_start(struct ae_ctrl_cxt *cxt, void *param)
{
	int32_t rtn = AE_SUCCESS;
	int32_t i;
	int32_t again;
...
void* ae_sprd_init(void *param, void *in_param)
{
	int32_t rtn = AE_SUCCESS;
	int32_t i 	= 0;
	...
	*/
	{
		if (cxt->isp_ops.set_shutter_gain_delay_info) {
			...
			cxt->isp_ops.set_shutter_gain_delay_info(cxt->isp_ops.isp_handler,(void*)(&delay_info));
		} else {
...
int32_t ae_sprd_calculation(void *handle, void* param, void* result)
{
	...
	if (current_status->ae_start_delay <= current_status->frame_id) {
	   ...
	   if (cxt->manual_ae_on == 1) {
		...
		ae_calc_result_queue_write(&cxt->ae_result_queue, &cxt->actual_cell[0]);
	}
...
int32_t ae_sprd_io_ctrl(void *handle, enum ae_io_ctrl_cmd cmd, void *param, void *result)
{
	...
	case AE_VIDEO_START:
		if (param) {
			rtn = _set_video_start(cxt, param);
		}
		break;
