...
isp_s32 isp_u_nawbm_block(isp_handle handle, void *block_info)
{
	...
	if (!handle || !block_info) {
		ISP_LOGE("handle is null error: 0x%lx ox%lx",
	...
	param.sub_block = ISP_BLOCK_NAWBM;
	param.property = ISP_PRO_NAWBM_BLOCK;
...
isp_s32 isp_u_nawbm_bypass(isp_handle handle, isp_u32 bypass)
{
	...
	param.isp_id = file->isp_id;
	param.sub_block = ISP_BLOCK_NAWBM;
	param.property = ISP_PRO_NAWBM_BYPASS;
