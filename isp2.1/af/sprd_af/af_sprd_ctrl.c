...
static int32_t afsprd_load_lib(struct af_context_t *cxt)
{
	...
	v_count = sizeof(libaf_path) / sizeof(libaf_path[0]);
	...
	AF_LOGI("af lib v_count:%d, v_id:%d, version_name:%s", v_count,
				v_id, libaf_path[v_id]);

	cxt->lib_handle = dlopen(libaf_path[v_id], RTLD_NOW);
	if (!cxt->lib_handle) {
	...
	if (NULL == cxt->lib_ops.af_init ||
		...
		NULL == cxt->lib_ops.af_deinit ||
		NULL == cxt->lib_ops.af_ioctrl) {
...
INIT_ERROR_EXIT:
	if (af_cxt) {
		rtn = af_sprd_deinit((cmr_handle)af_cxt,NULL,NULL);
		if(rtn){
...
cmr_int af_sprd_process(af_handle_t handle, void *in, void *out)
{
	...
	rtn = af_cxt->lib_ops.af_calculation(af_cxt->af_alg_handle,
					&alg_calc_param,
					&alg_calc_result);
...
struct adpt_ops_type af_sprd_adpt_ops_ver0 = {
	.adpt_init = af_sprd_init,
	.adpt_deinit = af_sprd_deinit,
	.adpt_process = af_sprd_process,
	.adpt_ioctrl = af_ioctrl,
};
/*------------------------------------------------------------------------------*
*					Compiler Flag				*
*-------------------------------------------------------------------------------*/
