...
static cmr_u32 awbsprd_unload_lib(struct awb_ctrl_cxt *cxt)
{
	...
	if (NULL == cxt) {
		AWB_CTRL_LOGE("awb load lib param is NULL");
		rtn = AWB_CTRL_ERROR;
		goto exit;
	}
...
static cmr_u32 awbsprd_load_lib(struct awb_ctrl_cxt *cxt)
{
	...
	if (v_id >= v_count) {
		AWB_CTRL_LOGE("awb lib version id is error version_id: %d", v_id);
		rtn = AWB_CTRL_ERROR;
		goto exit;
	}
	...
	if (!cxt->lib_handle) {
		AWB_CTRL_LOGE("failed to dlopen awb lib");
		rtn = AWB_CTRL_ERROR;
		goto exit;
	}
	...
	if (!cxt->lib_ops.awb_init_v1) {
		AWB_CTRL_LOGE("failed to dlsym awb_init");
		rtn = AWB_CTRL_ERROR;
		goto load_error;
	}
	...
	if (!cxt->lib_ops.awb_init_v1) {
		AWB_CTRL_LOGE("failed to dlsym awb_calculation");
		rtn = AWB_CTRL_ERROR;
		goto load_error;
	}
	...
	if (!cxt->lib_ops.awb_deinit_v1) {
		AWB_CTRL_LOGE("failed to dlsym awb_deinit");
		rtn = AWB_CTRL_ERROR;
		goto load_error;
	}
	...
	if (!cxt->lib_ops.awb_ioctrl_v1) {
		AWB_CTRL_LOGE("failed to dlsym awb_ioctrl");
		rtn = AWB_CTRL_ERROR;
		goto load_error;
	}
...
awb_ctrl_handle_t awb_sprd_ctrl_init(void *in, void *out)
{
	...
	if (AWB_CTRL_SUCCESS != rtn) {
		AWB_CTRL_LOGE("failed to load awb lib");
		goto ERROR_EXIT;
	}
...
cmr_s32 awb_sprd_ctrl_deinit(void *handle, void *in, void *out)
{
	...
	if (AWB_CTRL_SUCCESS != rtn) {
		AWB_CTRL_LOGE("failed to deinit awb_v1 \n");
	}

	rtn = awbsprd_unload_lib(cxt);
	if (AWB_CTRL_SUCCESS != rtn) {
		AWB_CTRL_LOGE("failed to unload awb lib");
		goto EXIT;
	}
...
cmr_s32 awb_sprd_ctrl_calculation(void *handle, void *in, void *out)
{
	...
	AWB_CTRL_LOGD("AWB %dx%d: (%d,%d,%d) %dK, pg=%d, %dus", calc_param.stat_img_w, calc_param.stat_img_h, calc_result.awb_gain[0].r_gain, calc_result.awb_gain[0].g_gain, calc_result.awb_gain[0].b_gain, calc_result.awb_gain[0].ct, result->pg_flag, (cmr_s32)((time1-time0)/1000));
