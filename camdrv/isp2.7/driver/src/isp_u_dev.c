...
cmr_s32 isp_dev_update_param_start(cmr_handle handle)
{
	...
	ret = ioctl(file->fd, SPRD_ISP_IO_UPDATE_PARAM_START, &isp_id);
	if (ret) {
		ISP_LOGE("fail to do cfg start.");
	}
...
cmr_s32 isp_dev_update_param_end(cmr_handle handle)
{
	cmr_s32 ret = 0;
	cmr_u32 isp_id = 0;
	struct isp_file *file = NULL;
	...
	ret = ioctl(file->fd, SPRD_ISP_IO_UPDATE_PARAM_END, &isp_id);
	if (ret) {
		ISP_LOGE("fail to do cfg start.");
	}
...
cmr_s32 isp_dev_set_slice_raw_info(cmr_handle handle, struct isp_raw_proc_info *param)
{
	...
	if (ret) {
		ISP_LOGE("fail to set raw slice info %d.", ret);
	}
