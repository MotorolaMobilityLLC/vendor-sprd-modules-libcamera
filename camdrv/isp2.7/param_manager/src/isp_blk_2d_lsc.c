...
cmr_s32 _pm_2d_lsc_init(void *dst_lnc_param, void *src_lnc_param, void *param1, void *param2)
{
	...
	for (i = 0; i < ISP_COLOR_TEMPRATURE_NUM; ++i) {
		...
		dst_ptr->map_tab[i].gain_w = _pm_get_lens_grid_pitch(src_ptr->tab_info.lsc_2d_info[i].lsc_2d_map_info.grid, img_size_ptr->w, ISP_ONE);
	...
	dst_ptr->cur.grid_x_num = _pm_get_lens_grid_pitch(dst_ptr->cur.grid_width, img_size_ptr->w, ISP_ZERO) + 2;
	dst_ptr->cur.grid_y_num = _pm_get_lens_grid_pitch(dst_ptr->cur.grid_width, img_size_ptr->h, ISP_ZERO) + 2;
