...
static cmr_int ispctl_flash_notice(cmr_handle isp_alg_handle, void *param_ptr)
{
	...
	case ISP_FLASH_PRE_AFTER:
		...
		if (cxt->ops.lsc_ops.ioctrl)
			ret = cxt->ops.lsc_ops.ioctrl(cxt->lsc_cxt.handle, ALSC_FLASH_PRE_AFTER, NULL, NULL);
	...
	case ISP_FLASH_MAIN_BEFORE:
		...
		if (cxt->ops.lsc_ops.ioctrl)
			ret = cxt->ops.lsc_ops.ioctrl(cxt->lsc_cxt.handle, ALSC_FLASH_ON, NULL, NULL);
		break;
...
static cmr_int ispctl_fix_param_update(cmr_handle isp_alg_handle, void *param_ptr)
{
...

	{
		...
		if (cxt->ops.ae_ops.get_flash_param)
			ret = cxt->ops.ae_ops.get_flash_param(cxt->handle_pm, &flash_param_ptr);
		if (ISP_SUCCESS == ret) {
			if (cxt->ops.ae_ops.ioctrl)
