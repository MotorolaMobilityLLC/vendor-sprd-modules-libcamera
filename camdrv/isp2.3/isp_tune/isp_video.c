...
cmr_s32 down_isp_param(cmr_handle isp_handler, struct isp_data_header_normal * write_cmd, struct msg_head_tag * msg, cmr_u8 * isp_data_ptr, cmr_s32 value)
{
	...
	ISP_LOGD("[mode_id %d] down_isp_param write_cmd->main_type %d\n", mode_id, write_cmd->main_type);
	...
	case MODE_TUNE_INFO:
		{
			if (0 == flag) {
				data_len = mode_param_info.len;
				ISP_LOGD("mode tune data len = %d", data_len);
			...
			if (0x01 == write_cmd->packet_status) {
				...
				if (MAX_MODE_NUM > mode_id){
					if (!sensor_raw_info_ptr->mode_ptr[mode_id].addr) {
						ISP_LOGD(" sensor raw mode ptr addr is , mode_id=%d", mode_id);
					}
	...
	case MODE_AE_FLASH_TABLE:
		{
			if (0 == flag) {
				...
				if(is_compat_ae3)
					rtn = get_ae_table_cus_param_length(sensor_raw_fix, write_cmd->sub_type, &data_len);
				else
					rtn = get_ae_flash_table_param_length(sensor_raw_fix, write_cmd->sub_type, &data_len);
				data_addr = (cmr_u8 *) ispParserAlloc(data_len);
				if (NULL == data_addr) {
					ISP_LOGE("fail to malloc mem!");
					rtn = 0x01;
