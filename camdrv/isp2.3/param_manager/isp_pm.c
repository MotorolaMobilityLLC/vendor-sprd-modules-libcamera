	...
	for (i = 0; i < blk_num; i++) {
		if (id == blk_header[i].block_id) {
			if(id == ISP_BLK_MFNR || id == ISP_BLK_CNR3)
				ISP_LOGV("id=0x%x,blk_header[i].block_id=%d",
				     ...
				     struct isp_pm_init_output *output)
{
	...
	is_ae3x = 0;
	 if ((version_id & PM_VER_CHIP_MASK) == 0x00070000 &&(version_id & PM_VER_SW_MASK) == 0x000B){
	 	is_ae3x = 1;
	 }
					     ...
					     struct isp_pm_param_data *data_param_ptr, cmr_u32 * rtn_idx)
{
	...
	for (i = 0; i < 1; i++) {	//just support get only one block info
		...
		blk_cfg_ptr = isp_pm_get_block_cfg(id);
		if(id == ISP_BLK_MFNR ||id == ISP_BLK_CNR3) {
			ISP_LOGV("id = 0x%x,cmd=%d,blk_cfg_ptr=%p,blk_header_ptr=%p",
