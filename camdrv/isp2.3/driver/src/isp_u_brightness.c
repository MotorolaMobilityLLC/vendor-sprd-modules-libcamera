...
cmr_s32 isp_u_brightness_slice_size(cmr_handle handle, void *param_ptr)
{
	...
	size.width = brightness_ptr->size.width;
	size.height = brightness_ptr->size.height;

	param.isp_id = file->isp_id;
	param.scene_id = brightness_ptr->scene_id;
	param.sub_block = ISP_BLOCK_BRIGHTNESS;
	param.property = ISP_PRO_BRIGHT_SLICE_SIZE;
...
cmr_s32 isp_u_brightness_slice_info(cmr_handle handle, void *param_ptr)
{
	...
	param.scene_id = brightness_ptr->scene_id;
	param.sub_block = ISP_BLOCK_BRIGHTNESS;
	param.property = ISP_PRO_BRIGHT_SLICE_INFO;
	param.property_param = &brightness_ptr->info;
