...
cmr_s32 isp_u_nlc_block(cmr_handle handle, void *param_ptr)
{
	...
	struct isp_u_blocks_info *nlc_ptr = NULL;
	...
	nlc_ptr = (struct isp_u_blocks_info *)param_ptr;

	param.isp_id = file->isp_id;
	param.scene_id = nlc_ptr->scene_id;
	param.sub_block = ISP_BLOCK_NLC;
	param.property = ISP_PRO_NLC_BLOCK;
	param.property_param = nlc_ptr->block_info;
...
cmr_s32 isp_u_nlc_bypass(cmr_handle handle, void *param_ptr)
{
	...
	param.property_param = &nlc_ptr->bypass;
...
cmr_s32 isp_u_nlc_r_node(cmr_handle handle, void *param_ptr)
{
	...
	param.property_param = nlc_ptr->r_node_ptr;
...
cmr_s32 isp_u_nlc_g_node(cmr_handle handle, void *param_ptr)
{
	...
	param.property_param = nlc_ptr->g_node_ptr;
...
cmr_s32 isp_u_nlc_b_node(cmr_handle handle, void *param_ptr)
{
	...
	param.property_param = nlc_ptr->b_node_ptr;
...
cmr_s32 isp_u_nlc_l_node(cmr_handle handle, void *param_ptr)
{
	...
	param.property_param = nlc_ptr->l_node_ptr;
