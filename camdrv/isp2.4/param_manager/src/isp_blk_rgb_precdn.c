 ...
 */
#define LOG_TAG "isp_blk_rgb_precdn"
#include "isp_blocks_cfg.h"

static cmr_u32 _pm_rgb_precdn_convert_param(void *dst_param, cmr_u32 strength_level, cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	cmr_s32 rtn = ISP_SUCCESS;
	cmr_u32 total_offset_units = 0;
	cmr_u32 *multi_nr_map_ptr = PNULL;
	struct isp_rgb_pre_cdn_param *dst_ptr = (struct isp_rgb_pre_cdn_param*)dst_param;
	struct sensor_rgb_precdn_level* precdn_param = PNULL;

	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->nr_mode_setting) {
		precdn_param = (struct sensor_rgb_precdn_level*)(dst_ptr->param_ptr);
	} else {
		multi_nr_map_ptr = (cmr_u32 *)dst_ptr->scene_ptr;
		total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		precdn_param = (struct sensor_rgb_precdn_level*)((cmr_u8 *)dst_ptr->param_ptr+ total_offset_units * dst_ptr->level_num * sizeof(struct sensor_rgb_precdn_level));
	}
...
cmr_s32 _pm_rgb_precdn_init(void *dst_pre_cdn_param, void *src_pre_cdn_param, void* param1, void* param2)
{
	cmr_s32 rtn = ISP_SUCCESS;
	...
	rtn=_pm_rgb_precdn_convert_param(dst_ptr, dst_ptr->cur_level, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_ptr->cur.bypass |= header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
cmr_s32 _pm_rgb_precdn_set_param(void *pre_cdn_param, cmr_u32 cmd, void* param_ptr0, void* param_ptr1)
{
	cmr_s32 rtn = ISP_SUCCESS;
	struct isp_rgb_pre_cdn_param *dst_ptr = (struct isp_rgb_pre_cdn_param*)pre_cdn_param;
...
cmr_s32 _pm_rgb_precdn_get_param(void *pre_cdn_param, cmr_u32 cmd, void* rtn_param0, void* rtn_param1)
{
	cmr_s32 rtn = ISP_SUCCESS;
