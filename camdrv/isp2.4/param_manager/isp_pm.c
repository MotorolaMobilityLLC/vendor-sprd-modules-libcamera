				     ...
				     struct isp_pm_init_output *output)
{
	...
	ISP_LOGD("p_m version id:%x.\n",version_id);

	is_ae3x = 0;
	 if ((version_id & PM_VER_CHIP_MASK) == 0x00080000 &&(version_id & PM_VER_SW_MASK) == 0x000C){
	 	is_ae3x = 1;
	 }
	ISP_LOGD("p_m AE version:%d.\n",is_ae3x);
	...
	for (i = 0; i < input->num; i++) {
		...
		if(is_ae3x == 1){
			add_ae_len = fix_data_ptr->ae3x.ae_param.ae_len;//ae3x
			fix_ae_datap = (void *)fix_data_ptr->ae3x.ae_param.ae;//ae3x
			ISP_LOGD("p_m AE3x tuning param:%p,size:%d.\n",fix_ae_datap,add_ae_len);
		}
		...
		for (j = 0; j < src_mod_ptr->block_num; j++) {
			...
			case ISP_BLK_NLM:{
					...
					nr_param_update_info.param_type = ISP_BLK_NLM_VST_T;
					nr_param_update_info.nr_param_ptr = (cmr_uint *) (fix_data_ptr->nr.nr_set_group.vst);
					nr_param_update_info.size_of_per_unit = sizeof(struct sensor_vst_level) * nr_level_number_ptr->nr_level_map[ISP_BLK_NLM_VST_T];
					isp_nr_param_update(&nr_param_update_info);
					dst_nlm_data ->param1_ptr = (cmr_uint *)fix_data_ptr->nr.nr_set_group.vst;

					nr_param_update_info.param_type = ISP_BLK_NLM_IVST_T;
					nr_param_update_info.nr_param_ptr = (cmr_uint *) (fix_data_ptr->nr.nr_set_group.ivst);
					nr_param_update_info.size_of_per_unit = sizeof(struct sensor_ivst_level) * nr_level_number_ptr->nr_level_map[ISP_BLK_NLM_IVST_T];
					isp_nr_param_update(&nr_param_update_info);
			...
			case ISP_BLK_BPC:{
				...
				break;
			case ISP_BLK_BDN:{
					isp_blk_nr_type = ISP_BLK_BDN_T;
					nr_set_addr = (intptr_t) (fix_data_ptr->nr.nr_set_group.bdn);
					nr_set_size = sizeof(struct sensor_bdn_level);
				}
				break;
			case ISP_BLK_GRGB:{
			...
			case ISP_BLK_CFA:{
					...
					nr_set_addr = (intptr_t) (fix_data_ptr->nr.nr_set_group.cfae);
					nr_set_size = sizeof(struct sensor_cfae_level);
				}
				break;
			case ISP_BLK_RGB_AFM:{
			...
			case ISP_BLK_UVDIV:{
					...
					nr_set_addr = (intptr_t) (fix_data_ptr->nr.nr_set_group.cce_uvdiv);
			...
			case ISP_BLK_YUV_PRECDN:{
				...
				break;
			case ISP_BLK_RGB_PRECDN:{
					isp_blk_nr_type = ISP_BLK_RGB_PRECDN_T;
					nr_set_addr = (intptr_t) (fix_data_ptr->nr.nr_set_group.rgb_precdn);
					nr_set_size = sizeof(struct sensor_rgb_precdn_level);
				}
				break;
			case ISP_BLK_UV_CDN:{
					isp_blk_nr_type = ISP_BLK_UV_CDN_T;
					nr_set_addr = (intptr_t) (fix_data_ptr->nr.nr_set_group.uv_cdn);
					nr_set_size = sizeof(struct sensor_uv_cdn_level);
				}
				break;
			case ISP_BLK_UV_POSTCDN:{
					isp_blk_nr_type = ISP_BLK_UV_POSTCDN_T;
					nr_set_addr = (intptr_t) (fix_data_ptr->nr.nr_set_group.uv_postcdn);
			...
			case ISP_BLK_EDGE:{
					...
					nr_set_addr = (intptr_t) (fix_data_ptr->nr.nr_set_group.ee);
					nr_set_size = sizeof(struct sensor_ee_level);
				}
				break;
			case ISP_BLK_IIRCNR_IIR:{
					isp_blk_nr_type = ISP_BLK_IIRCNR_IIR_T;
			...
			if (src_header[j].block_id == ISP_BLK_BPC
			    || src_header[j].block_id == ISP_BLK_BDN
#ifdef NOT_EXIST_IN_ISP2_0
			    || src_header[j].block_id == ISP_BLK_RGB_DITHER
			    || src_header[j].block_id == ISP_BLK_PDAF_CORRECT
