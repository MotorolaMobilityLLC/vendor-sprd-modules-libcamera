...
static cmr_int ispalg_set_aem_win(cmr_handle isp_alg_handle, struct ae_monitor_info *aem_info)
{
	...
	ISP_LOGI("aem win: 0x%x 0x%x 0x%x 0x%x", aem_info->trim.x, aem_info->trim.y, aem_info->win_size.w, aem_info->win_size.h);
...
static cmr_s32 ispalg_cfg(cmr_handle isp_alg_handle)
{
	...
	for (i = 0; i < output.param_num; i++) {
		...
		if (ISP_BLK_NLM == param_data->id) {
			struct isp_dev_nlm_info *nlm_info = param_data->data_ptr;
			ISP_LOGI("<tang>, vst_len: %d ivst_len: %d, nlm_len: %d\n", nlm_info->vst_len, nlm_info->ivst_len, nlm_info->nlm_len);
		}
...
static cmr_int ispalg_ae_set_work_mode(cmr_handle isp_alg_handle, cmr_u32 new_mode, cmr_u32 fly_mode, struct isp_video_start *param_ptr)
{
	...
	ISP_LOGI("config ae win:0x%x 0x%x", ae_param.win_size.w, ae_param.win_size.h);
	if (cxt->ops.ae_ops.ioctrl) {
