...
static cmr_int isp_ae_set_cb(cmr_handle isp_alg_handle, cmr_int type, void *param0, void *param1)
{
	...
	case ISP_AE_GET_RGB_GAIN:
		ret = isp_get_rgb_gain(cxt, param0);
		break;
	default:
		break;
	}
		  ...
		  cmr_u32 ae_stable)
{
	...
	if (lsc_ver.LSC_SPD_VERSION >= 2) {
		...
		if (cxt->ops.ae_ops.ioctrl)
			ret = cxt->ops.ae_ops.ioctrl(cxt->ae_cxt.handle, AE_GET_BV_BY_LUM_NEW, NULL, (void *)&bv0);
...
static cmr_int ispalg_handle_sensor_sof(cmr_handle isp_alg_handle)
{
	...
	if (cxt->ops.ae_ops.ioctrl)
		ret = cxt->ops.ae_ops.ioctrl(cxt->ae_cxt.handle, AE_SET_PROC, NULL, NULL);
	ISP_TRACE_IF_FAIL(ret, ("fail to set ae proc"));
...
cmr_int ispalg_awb_pre_process(cmr_handle isp_alg_handle,
				struct isp_awb_calc_info * in_ptr,
				struct awb_ctrl_calc_param * out_ptr)
{
	...
	if (cxt->ops.ae_ops.ioctrl) {
		...
		ret = cxt->ops.ae_ops.ioctrl(cxt->ae_cxt.handle, AE_GET_ISO, NULL, (void *)&iso);
		ret = cxt->ops.ae_ops.ioctrl(cxt->ae_cxt.handle, AE_GET_GAIN, NULL, (void *)&gain);
		ret = cxt->ops.ae_ops.ioctrl(cxt->ae_cxt.handle, AE_GET_EXP, NULL, (void *)&exposure);
		ret = cxt->ops.ae_ops.ioctrl(cxt->ae_cxt.handle, AE_GET_EV, NULL, (void *)&ae_ev);
	}
...
cmr_int ispalg_awb_post_process(cmr_handle isp_alg_handle, struct awb_ctrl_calc_result *awb_output)
{
	...
	if (awb_output->use_ccm) {
		...
		BLOCK_PARAM_CFG(input, param_data, ISP_PM_BLK_CMC10, ISP_BLK_CMC10, awb_output->ccm, 9 * sizeof(cmr_u16));
				...
				struct awb_ctrl_calc_result * awb_output)
{
	...
	if (!isp_alg_handle || !awb_calc_info || !awb_output) {
		ret = ISP_PARAM_NULL;
		goto exit;
	}
					 ...
					 struct awb_ctrl_calc_result *awb_output)
{
	...
	if (1 == cxt->smart_cxt.isp_smart_eb) {
		...
		ISP_TRACE_IF_FAIL(ret, ("_smart_calc fail "));
		...
		ret =
		    ispalg_alsc_calc(isp_alg_handle,
...
cmr_int ispalg_ae_awb_process(cmr_handle isp_alg_handle)
{
	...
	ret = ispalg_aeawb_post_process((cmr_handle) cxt, &awb_calc_info, &awb_result);
...
cmr_int isp_alg_thread_proc(struct cmr_msg *message, void *p_data)
{
	...
	case ISP_CTRL_EVT_TX:
		ret = _ispProcessEndHandle((cmr_handle) cxt);
		break;
...
static cmr_u32 isp_alg_sw_init(struct isp_alg_fw_context *cxt, struct isp_alg_sw_init_in *input_ptr)
{
	...
	ret = isp_ae_sw_init(cxt);
	ISP_TRACE_IF_FAIL(ret, ("fail to do ae_ctrl_init"));

	ret = isp_afl_sw_init(cxt, input_ptr);
	ISP_RETURN_IF_FAIL(ret, ("fail to do anti_flicker param update"));

	ret = isp_awb_sw_init(cxt);
	ISP_TRACE_IF_FAIL(ret, ("fail to do awb_ctrl_init"));

	ret = isp_smart_sw_init(cxt);
	ISP_TRACE_IF_FAIL(ret, ("fail to do _smart_init"));

	ret = isp_af_sw_init(cxt);
	ISP_TRACE_IF_FAIL(ret, ("fail to do af_ctrl_init"));

	ret = isp_pdaf_sw_init(cxt, input_ptr);
	ISP_TRACE_IF_FAIL(ret, ("fail to do pdaf_ctrl_init"));

	ret = isp_lsc_sw_init(cxt);
