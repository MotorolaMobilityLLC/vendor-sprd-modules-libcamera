...
cmr_s32 _pm_rgb_ltm_init(void *dst_rgb_ltm_param, void *src_rgb_ltm_param, void *param1, void *param2)
{
	...
	if (!ltm_header_ptr->bypass){
		for ( i = 0; i < SENSOR_YUV_LTM_NUM; i++) {
			...
			text_point_alpha[i] = src_ptr->rgb_ltm_param[i].rgb_ltm_stat.ltm_text.text_point_alpha;
		...
		dst_ptr->cur.ltm_stat.channel_sel = dst_ptr->ltm_param[i].channel_sel;
