...
cmr_s32 _pm_bchs_init(void *dst_bchs_param, void *src_bchs_param, void *param1, void *param2)
{
	...
	dst_ptr->cur_level.brta_factor = dst_ptr->brigntness.bright_tab[dst_ptr->brigntness.cur_index];
	dst_ptr->cur_level.cnta_factor = dst_ptr->contrast.tab[dst_ptr->contrast.cur_index];
	dst_ptr->cur_level.hua_cos_value = dst_ptr->hue.tab_cos[dst_ptr->hue.cur_idx];
	dst_ptr->cur_level.hua_sina_value = dst_ptr->hue.tab_sin[dst_ptr->hue.cur_idx];
	dst_ptr->cur_level.csa_factor_u = dst_ptr->saturation.tab[0][dst_ptr->saturation.cur_u_idx];
	dst_ptr->cur_level.csa_factor_v = dst_ptr->saturation.tab[1][dst_ptr->saturation.cur_v_idx];
	ISP_LOGV("dst_ptr->cur.brta_factor =%d, dst_ptr->cur.hua_cos_value=%d, dst_ptr->cur.cnta_factor =%d, dst_ptr->cur.csa_factor_u %d,%d,%d",
		dst_ptr->cur_level.brta_factor, dst_ptr->cur_level.hua_cos_value, dst_ptr->cur_level.cnta_factor, dst_ptr->cur_level.csa_factor_u,dst_ptr->cur_level.csa_factor_v,dst_ptr->cur_level.hua_sina_value);
...
cmr_s32 _pm_bchs_set_param(void *bchs_param, cmr_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_AI_SCENE_BCHS_CUR:
		{
			memcpy((void *)&dst_ptr->final_level, param_ptr0, sizeof(dst_ptr->final_level));

			dst_ptr->cur.brta_factor = dst_ptr->final_level.brta_factor;
			ISP_LOGV("set dst_ptr->cur.brta_factor %x",dst_ptr->cur.brta_factor);
			dst_ptr->cur.cnta_factor = dst_ptr->final_level.cnta_factor;
			dst_ptr->cur.hua_cos_value = dst_ptr->final_level.hua_cos_value;
			dst_ptr->cur.hua_sina_value = dst_ptr->final_level.hua_sina_value;
			dst_ptr->cur.csa_factor_u = dst_ptr->final_level.csa_factor_u;
			dst_ptr->cur.csa_factor_v = dst_ptr->final_level.csa_factor_v;
	...
	case ISP_PM_BLK_BRIGHT:
		...
		dst_ptr->cur_level.brta_factor = dst_ptr->brigntness.bright_tab[dst_ptr->brigntness.cur_index];
		ISP_LOGV("set dst_ptr->cur_level.brta_factor %x",dst_ptr->cur_level.brta_factor);
		break;
...
cmr_s32 _pm_bchs_get_param(void *bchs_param, cmr_u32 cmd, void *rtn_param0, void *rtn_param1)
{
	...
	case ISP_PM_BLK_AI_SCENE_BCHS:
		param_data_ptr->data_ptr = (void *)&dst_ptr->cur_level;
		param_data_ptr->data_size = sizeof(dst_ptr->cur_level);
		*update_flag = 0;
		break;
