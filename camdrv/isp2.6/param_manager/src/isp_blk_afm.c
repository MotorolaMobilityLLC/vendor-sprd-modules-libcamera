...
static cmr_u32 _pm_rgb_afm_convert_param(void *dst_param, cmr_u32 strength_level, cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->nr_mode_setting) {
		...
		rgb_afm_param = (struct sensor_rgb_afm_level *)((cmr_u8 *) dst_ptr->param_ptr +
				total_offset_units * dst_ptr->level_num * sizeof(struct sensor_rgb_afm_level));
	...
	if (rgb_afm_param != NULL) {
		for (i=0; i<10; i++)
			dst_ptr->cur.afm_iir_c[i] = rgb_afm_param[strength_level].afm_iir_denoise.iir_c[i];
		dst_ptr->cur.afm_iir_enable = rgb_afm_param[strength_level].afm_iir_denoise.afm_iir_en;
		dst_ptr->cur.afm_iir_g0 = rgb_afm_param[strength_level].afm_iir_denoise.iir_g[0];
		dst_ptr->cur.afm_iir_g1 = rgb_afm_param[strength_level].afm_iir_denoise.iir_g[1];

		dst_ptr->cur.afm_channel_sel = rgb_afm_param[strength_level].afm_enhanced.afm_enhanced_pre.channel_sel;
		dst_ptr->cur.afm_denoise_mode = rgb_afm_param[strength_level].afm_enhanced.afm_enhanced_pre.denoise_mode;
		dst_ptr->cur.afm_center_weight = rgb_afm_param[strength_level].afm_enhanced.afm_enhanced_pre.center_wgt;
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 9; j++) {
				dst_ptr->cur.afm_fv1_coeff[i][j] = rgb_afm_param[strength_level].afm_enhanced.afm_enhanced_process.fv1_coeff[i][j];
			}
		}
		dst_ptr->cur.afm_clip_en0 = rgb_afm_param[strength_level].afm_enhanced.afm_enhanced_post.enhanced_fv0.clip_en;
		...
		dst_ptr->cur.afm_fv1_th.min = rgb_afm_param[strength_level].afm_enhanced.afm_enhanced_post.enhanced_fv1.fv_min_th;
		dst_ptr->cur.afm_fv1_th.max = rgb_afm_param[strength_level].afm_enhanced.afm_enhanced_post.enhanced_fv1.fv_max_th;
	}
	return rtn;
}
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			if (is_need_update && (cur_level != dst_ptr->cur_level || nr_tool_flag[9] || block_result->mode_flag_changed)) {
				...
				rtn = _pm_rgb_afm_convert_param(dst_ptr, dst_ptr->cur_level, header_ptr->mode_id, block_result->scene_flag);
				dst_ptr->cur.bypass |= header_ptr->bypass;
				if (ISP_SUCCESS != rtn) {
					ISP_LOGE("fail to  convert pm rgb afm param!");
					return rtn;
...
cmr_s32 _pm_rgb_afm_get_param(void *rgb_aem_param, cmr_u32 cmd, void *rtn_param0, void *rtn_param1)
{
	...
	param_data_ptr->id = DCAM_BLK_RGB_AFM;
