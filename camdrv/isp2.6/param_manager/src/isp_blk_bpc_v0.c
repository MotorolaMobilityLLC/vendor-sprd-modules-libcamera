	...
	cmr_u32 strength_level, cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	...
	struct sensor_bpc_level_v0 *bpc_param;

	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->nr_mode_setting) {
		bpc_param = (struct sensor_bpc_level_v0 *)(dst_ptr->param_ptr);
	} else {
		cmr_u32 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (cmr_u32 *) dst_ptr->scene_ptr;
		total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		bpc_param = (struct sensor_bpc_level_v0 *)((cmr_u8 *) dst_ptr->param_ptr + \
			total_offset_units * dst_ptr->level_num * sizeof(struct sensor_bpc_level_v0));
	}
