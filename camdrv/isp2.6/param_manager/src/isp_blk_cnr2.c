	...
	cmr_u32 strength_level, cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->nr_mode_setting) {
		...
		cnr2_param = (struct sensor_cnr_level *)((cmr_u8 *) dst_ptr->param_ptr +
				total_offset_units * dst_ptr->level_num * sizeof(struct sensor_cnr_level));
	}
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			if (level != dst_ptr->cur_level || nr_tool_flag[ISP_BLK_CNR2_T] || block_result->mode_flag_changed) {
...
cmr_s32 _pm_cnr2_get_param(void *cnr2_param, cmr_u32 cmd, void *rtn_param0, void *rtn_param1)
{
	...
	param_data_ptr->id = ISP_BLK_CNR2_V1;
