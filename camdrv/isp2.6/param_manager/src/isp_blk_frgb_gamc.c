...
cmr_s32 _pm_frgb_gamc_init(
	void *dst_gamc_param, void *src_gamc_param,
	void *param1, void *param_ptr2)
{
	...
	if (1 == header_ptr->param_id) {
		for (j = 0; j < ISP_FRGB_GAMMA_PT_NUM; j++) {
			dst_ptr->cur.gain_r[j] = src_ptr->curve_tab[index].points_r[j].y;
			dst_ptr->cur.gain_g[j] = src_ptr->curve_tab[index].points_g[j].y;
			dst_ptr->cur.gain_b[j] = src_ptr->curve_tab[index].points_b[j].y;
		}
	} else {
		for (j = 0; j < ISP_FRGB_GAMMA_PT_NUM; j++) {
			dst_ptr->cur.gain_r[j] = src_ptr->curve_tab[index].points_r[j].y;
			dst_ptr->cur.gain_g[j] = src_ptr->curve_tab[index].points_r[j].y;
			dst_ptr->cur.gain_b[j] = src_ptr->curve_tab[index].points_r[j].y;
		}
	}
...
cmr_s32 _pm_frgb_gamc_set_param(
	void *gamc_param, cmr_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	cmr_s32 rtn = ISP_SUCCESS;
	...
	case ISP_PM_BLK_GAMMA:
		{
			...
			if (1 == gamc_header_ptr->param_id) {
				for (i = 0; i < ISP_FRGB_GAMMA_PT_NUM; i++) {
					gamc_ptr->cur.gain_r[i] = gamc_ptr->curve_tab[gamma_idx].points_r[i].y;
					gamc_ptr->cur.gain_g[i] = gamc_ptr->curve_tab[gamma_idx].points_g[i].y;
					gamc_ptr->cur.gain_b[i] = gamc_ptr->curve_tab[gamma_idx].points_b[i].y;
				}
			} else {
				for (i = 0; i < ISP_FRGB_GAMMA_PT_NUM; i++) {
					gamc_ptr->cur.gain_r[i] = gamc_ptr->curve_tab[gamma_idx].points_r[i].y;
					gamc_ptr->cur.gain_g[i] = gamc_ptr->curve_tab[gamma_idx].points_r[i].y;
					gamc_ptr->cur.gain_b[i] = gamc_ptr->curve_tab[gamma_idx].points_r[i].y;
				}
	...
	case ISP_PM_BLK_GAMMA_CUR:
		{
			...
			if (1 == gamc_header_ptr->param_id) {
				for (i = 0; i < ISP_FRGB_GAMMA_PT_NUM; i++) {
					gamc_ptr->cur.gain_r[i] = gamc_ptr->final_curve.points_r[i].y;
					gamc_ptr->cur.gain_g[i] = gamc_ptr->final_curve.points_g[i].y;
					gamc_ptr->cur.gain_b[i] = gamc_ptr->final_curve.points_b[i].y;
				}
			} else {
				for (i = 0; i < ISP_FRGB_GAMMA_PT_NUM; i++) {
					gamc_ptr->cur.gain_r[i] = gamc_ptr->final_curve.points_r[i].y;
					gamc_ptr->cur.gain_g[i] = gamc_ptr->final_curve.points_r[i].y;
					gamc_ptr->cur.gain_b[i] = gamc_ptr->final_curve.points_r[i].y;
				}
			}
			gamc_header_ptr->is_update = ISP_ONE;
		}
