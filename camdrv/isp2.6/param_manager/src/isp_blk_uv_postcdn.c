...
static cmr_u32 _pm_uv_postcdn_convert_param(
	void *dst_postcdn_param, cmr_u32 strength_level, cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			if (cur_level != dst_ptr->cur_level || nr_tool_flag[ISP_BLK_POSTCDN_T] || block_result->mode_flag_changed) {
				dst_ptr->cur_level = cur_level;
				header_ptr->is_update = ISP_ONE;
				nr_tool_flag[ISP_BLK_POSTCDN_T] = 0;

				rtn = _pm_uv_postcdn_convert_param(dst_ptr, dst_ptr->cur_level, header_ptr->mode_id, block_result->scene_flag);
				dst_ptr->cur.bypass |= header_ptr->bypass;
				if (ISP_SUCCESS != rtn) {
					ISP_LOGE("fail to  convert pm uv postcdn param!");
					return rtn;
...
cmr_s32 _pm_uv_postcdn_get_param(void *postcdn_param, cmr_u32 cmd, void *rtn_param0, void *rtn_param1)
{
	...
	param_data_ptr->id = ISP_BLK_UV_POSTCDN_V1;
