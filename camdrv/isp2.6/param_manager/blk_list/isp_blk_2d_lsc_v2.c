...
cmr_s32 _pm_2d_lsc_init(void *dst_lnc_param, void *src_lnc_param, void *param1, void *param2)
{
	...
	for (i = 0; i < ISP_COLOR_TEMPRATURE_NUM; ++i) {
		...
		dst_ptr->map_tab[i].gain_w = _pm_get_lens_grid_pitch(dst_ptr->map_tab[i].grid_x, img_size_ptr->w, ISP_ONE);
		dst_ptr->map_tab[i].gain_h = _pm_get_lens_grid_pitch(dst_ptr->map_tab[i].grid_y, img_size_ptr->h, ISP_ONE);
	...
	dst_ptr->lsc_info.grid_x = dst_ptr->map_tab[dst_ptr->lsc_info.cur_idx.x0].grid_x;
	dst_ptr->lsc_info.grid_y = dst_ptr->map_tab[dst_ptr->lsc_info.cur_idx.x0].grid_y;
	...
	case ISP_PM_BLK_LSC_UPDATE_GRID:
		{
			...
			cmr_u32 lsc_grid_x = adaptive_size_info[2];
