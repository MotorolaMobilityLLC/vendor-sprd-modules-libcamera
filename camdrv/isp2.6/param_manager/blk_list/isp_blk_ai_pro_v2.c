...
cmr_s32 _pm_ai_pro_init(void *dst_ai_param, void *src_ai_param, void *param1, void *param2)
{
	...
	for (i = 0; i < AI_SCENE_PM_PRO_MAX;i++) {
		...
		for (j = 0; j< 25; j++) {
			for( k = 0; k < 17; k++)
				dst_ptr->isp_ai_hsv_v1[i].hue_offset[j][k] = src_ptr->sensor_ai_hsv[i].hsv_offset.hue_offset[j][k];
		}
		for (j = 0; j< 25; j++) {
			for( k = 0; k < 17; k++)
				dst_ptr->isp_ai_hsv_v1[i].sat_offset[j][k] = src_ptr->sensor_ai_hsv[i].hsv_offset.sat_offset[j][k];
		}
		dst_ptr->isp_ai_hsv_v1[i].hue_offset_enable = src_ptr->sensor_ai_hsv[i].hue_offset_enable;
		dst_ptr->isp_ai_hsv_v1[i].y_blending_offset = src_ptr->sensor_ai_hsv[i].hsv_offset.y_blending_offset;
	}
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			if (ISP_SMART_AI_BCHS == block_result->smart_id) {
				...
				dst_ptr->hsv_cur_v1.hue_offset_enable = dst_ptr->isp_ai_hsv_v1[ai_scene].hue_offset_enable;
				dst_ptr->hsv_cur_v1.y_blending_offset = dst_ptr->isp_ai_hsv_v1[ai_scene].y_blending_offset;
				dst_ptr->smooth_frame_ai_cur = dst_ptr->smooth_frame_ai[ai_scene];

				if (dst_ptr->hsv_cur_v1.hue_offset_enable) {
					for (i = 0; i< 25; i++) {
						for( j = 0; j < 17; j++)
							dst_ptr->hsv_cur_v1.hue_offset[i][j] = dst_ptr->isp_ai_hsv_v1[ai_scene].hue_offset[i][j];
					}
					for (i = 0; i< 25; i++) {
						for( j = 0; j < 17; j++)
							dst_ptr->hsv_cur_v1.sat_offset[i] [j]= dst_ptr->isp_ai_hsv_v1[ai_scene].sat_offset[i][j];
					}
				}
...
cmr_s32 _pm_ai_pro_get_param(void *ai_param, cmr_u32 cmd, void *rtn_param0, void *rtn_param1)
{
	...
	case ISP_PM_BLK_AI_SCENE_HSV_PARAM:
		param_data_ptr->data_ptr = &ai_ptr->hsv_cur_v1;
		param_data_ptr->data_size = sizeof(ai_ptr->hsv_cur_v1);
		break;
