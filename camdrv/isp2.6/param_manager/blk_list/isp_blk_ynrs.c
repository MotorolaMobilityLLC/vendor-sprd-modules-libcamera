		...
		cmr_u32 strength_level, cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->nr_mode_setting) {
		...
		offset = total_offset_units * dst_ptr->level_num * sizeof(struct sensor_ynrs_level);
		ynrs_param = (struct sensor_ynrs_level *)((cmr_uint) dst_ptr->param_ptr + offset);
	}
	strength_level = PM_CLIP(strength_level, 0, dst_ptr->level_num - 1);

	if(ynrs_param != PNULL){
		for(i = 0; i < 5; i++){
			dst_ptr->cur.ynrs_param.gf_rnr_ratio[i] =
					ynrs_param[strength_level].gf_rnr_ratio[i];
			dst_ptr->cur.ynrs_param.gf_addback_enable[i] =
					ynrs_param[strength_level].gf_addback_enable[i];
			dst_ptr->cur.ynrs_param.gf_addback_ratio[i] =
					ynrs_param[strength_level].gf_addback_ratio[i];
			dst_ptr->cur.ynrs_param.gf_addback_clip[i] =
					ynrs_param[strength_level].gf_addback_clip[i];
			dst_ptr->cur.ynrs_param.gf_enable[i] =
					ynrs_param[strength_level].gf_enable[i];
			dst_ptr->cur.ynrs_param.gf_radius[i] =
					ynrs_param[strength_level].gf_radius[i];
			dst_ptr->cur.ynrs_param.gf_rnr_offset[i] =
					ynrs_param[strength_level].gf_rnr_offset[i];
			dst_ptr->cur.ynrs_param.gf_epsilon[i][0] =
					ynrs_param[strength_level].gf_epsilon[i][0];
			dst_ptr->cur.ynrs_param.gf_epsilon[i][1] =
					ynrs_param[strength_level].gf_epsilon[i][1];
			dst_ptr->cur.ynrs_param.gf_epsilon[i][2] =
		...
		dst_ptr->cur.ynrs_param.Radius_factor = ynrs_param[strength_level].Radius;
		dst_ptr->cur.radius_base = 0;
		ISP_LOGV("level %d radius_base = 0x%x imgCenterX = 0x%x imgCenterY = 0x%x "
			"bypass = 0x%x lumi_thresh[0] = 0x%x gf_rnr_ratio[0]= 0x%x dst_ptr->cur.Radius_factor = 0x%x\n",
			strength_level, dst_ptr->cur.radius_base,
			dst_ptr->cur.ynrs_param.imgCenterX, dst_ptr->cur.ynrs_param.imgCenterY,
			dst_ptr->cur.ynrs_param.bypass, dst_ptr->cur.ynrs_param.lumi_thresh[0],
			dst_ptr->cur.ynrs_param.gf_rnr_ratio[0],dst_ptr->cur.ynrs_param.Radius_factor);
	}
...
cmr_s32 _pm_ynrs_init(void *dst_ynrs_param, void *src_ynrs_param, void *param1, void *param2)
{
	...
	rtn = _pm_ynrs_convert_param(dst_ptr, dst_ptr->cur_level,
					ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_ptr->cur.ynrs_param.bypass |= header_ptr->bypass;
	dst_ptr->cur.bypass = dst_ptr->cur.ynrs_param.bypass;
	if (ISP_SUCCESS != rtn) {
