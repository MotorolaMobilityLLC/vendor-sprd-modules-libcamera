...
static denoise_param_read_t s_adapt_ioctl_nr_read = denoise_param_read_v29;
...
static cmr_int ispctl_ae_exp_compensation(cmr_handle isp_alg_handle, void *param_ptr)
{
	...
	exp_comp.mode = exp_compensation->ae_mode;
	if (cxt->ops.ae_ops.ioctrl) {
...
static cmr_int ispctl_get_info(cmr_handle isp_alg_handle, void *param_ptr)
{
	...
	if (cxt->awb_cxt.alc_awb) {
		...
		fdr_msg.msg_log = cxt->fdr_cxt.log_fdr;
		fdr_msg.msg_size = cxt->fdr_cxt.log_fdr_size;
		fdr_msg.frame_id = frame_id;
		...
		ISP_LOGV("hdr_msg  %p, %d, %d", current_hdr_msg->msg_log, current_hdr_msg->msg_size, current_hdr_msg->frame_id);
...
static cmr_int ispctl_set_app_mode(cmr_handle isp_alg_handle, void *param_ptr)
{
	...
	if(cxt->ops.lsc_ops.ioctrl)
		ret = cxt->ops.lsc_ops.ioctrl(cxt->lsc_cxt.handle, ALSC_SET_APP_MODE, (void *)&app_mode, NULL);
	return ret;
}
