...
static cmr_int ispalg_lsc_init(struct isp_alg_fw_context *cxt)
{
	...
	ISP_LOGD("[ALSC] full_img_size[%d,%d], table_w_h_gird[%d,%d,%d,%d]",
		...
		lsc_tab_param_ptr->lsc_info.grid_x,lsc_tab_param_ptr->lsc_info.grid_y);
...
cmr_int isp_alg_fw_start(cmr_handle isp_alg_handle, struct isp_video_start * in_ptr)
{
	...
	char value[PROPERTY_VALUE_MAX] = { 0x00 };
	struct isp_alg_fw_context *cxt = (struct isp_alg_fw_context *)isp_alg_handle;
	struct alsc_fwstart_info fwstart_info = { NULL, {NULL}, 0, 0, 5, 0, 0, 0, 0, 0};
