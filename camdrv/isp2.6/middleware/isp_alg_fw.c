...
static cmr_int ispalg_ai_callback(cmr_handle isp_alg_handle, cmr_int cb_type, void *data)
{
	...
	case AI_CB_FOR_AE: {
		...
		if (cxt->is_ai_scene_pro || scene_info.cur_scene_id == AI_SCENE_NIGHT)
		...
		cmr_int type, void *param0, void *param1)
{
	...
	case ISP_AWB_SET_WBC_GAIN:
		...
		struct awb_ctrl_gain *awb_gain = (struct awb_ctrl_gain*)param0;
		gain.r = awb_gain->r;
		gain.gr = awb_gain->g;
		gain.gb = awb_gain->g;
		gain.b = awb_gain->b;
		...
		if (cxt->remosaic_type) {
			...
			cfg.block_info = &gain;
			ret = isp_dev_access_ioctl(cxt->dev_access_handle, ISP_DEV_SET_AWB_GAIN, (void*)&cfg, param1);
			if(ret != ISP_SUCCESS){
		  ...
		  cmr_u32 ae_stable, struct ae_ctrl_callback_in *ae_in)
{
	...
	if (cxt->lsc_cxt.LSC_SPD_VERSION >= 2) {
		...
		calc_param.gridx = lsc_info->grid_x;
                calc_param.gridy = lsc_info->grid_y;
#else
	        calc_param.gridx = lsc_info->grid;
                calc_param.gridy = lsc_info->grid;
#endif
					 ...
					 struct awb_ctrl_calc_result *awb_output)
{
	...
	if (!cxt->smart_cxt.sw_bypass) {
		...
		smart_proc_in.cal_para.flash_ratio = (cmr_s32)(ae_in->flash_param.captureFlashEnvRatio * 256);
		smart_proc_in.cal_para.flash_ratio1 = (cmr_s32)(ae_in->flash_param.captureFlash1ofALLRatio * 256);
...
cmr_int ispalg_afl_process(cmr_handle isp_alg_handle, void *data)
{
	...
	afl_input.vir_addr_region = statis_info->uaddr + AFL_RBUF_OFFSET;
...
static cmr_int ispalg_afl_init(struct isp_alg_fw_context *cxt)
{
	...
	memset((void *)&output, 0, sizeof(output));
	ret = isp_pm_ioctl(cxt->handle_pm, ISP_PM_CMD_GET_INIT_AFL, NULL, &output);
	if (ret == ISP_SUCCESS && output.param_data != NULL) {
		cxt->afl_cxt.sw_bypass = output.param_data[0].user_data[0];
		ISP_LOGD("cxt->afl_cxt.sw_bypass %d\n", cxt->afl_cxt.sw_bypass);

		afl_input.afl_tune_param = output.param_data->data_ptr;
		afl_input.pm_param_size = output.param_data->data_size;
	}
	pthread_mutex_unlock(&cxt->pm_getting_lock);
	ISP_LOGV("param addr is %p size %d", afl_input.afl_tune_param, afl_input.pm_param_size);
...
#if defined (CONFIG_ISP_2_9)
	lsc_param.gridx = lsc_info->grid_x;
	lsc_param.gridy = lsc_info->grid_y;
#else
	lsc_param.gridx = lsc_info->grid;
	lsc_param.gridy = lsc_info->grid;
#endif
...
#if defined (CONFIG_ISP_2_9)
	cxt->lsc_cxt.full_size_grid_x = lsc_tab_param_ptr->lsc_info.grid_x;
	cxt->lsc_cxt.full_size_grid_y = lsc_tab_param_ptr->lsc_info.grid_y;
#else
	cxt->lsc_cxt.full_size_grid = lsc_tab_param_ptr->lsc_info.grid;
#endif

	ISP_LOGD("[ALSC] full_img_size[%d,%d], table_w_h_gird_gridx_gridy[%d,%d,%d,%d,%d]",
		...
		lsc_tab_param_ptr->lsc_info.grid,lsc_tab_param_ptr->lsc_info.grid_x,lsc_tab_param_ptr->lsc_info.grid_y);
...
static cmr_int ispalg_update_alsc_result(cmr_handle isp_alg_handle, cmr_handle out_ptr)
{
	...
	cmr_u32 adaptive_size_info[4] = {0, 0, 0, 0};
	...
	ISP_LOGD("size0 (%d, %d), size1 (%d %d), lsc size (%d %d),  grid %d, gridx %d, gridy %d, gain_w %d gain_h %d\n",
		pic_size.w, pic_size.h, cxt->commn_cxt.src.w, cxt->commn_cxt.src.h,
		lsc_tab_param_ptr->resolution.w, lsc_tab_param_ptr->resolution.h,
		lsc_info_new->grid, lsc_info_new->grid_x, lsc_info_new->grid_y, lsc_info_new->gain_w, lsc_info_new->gain_h);
...
#if defined (CONFIG_ISP_2_9)
	fwstart_info->gridx_new = lsc_info_new->grid_x;
	fwstart_info->gridy_new = lsc_info_new->grid_y;
#else
	fwstart_info->gridx_new = lsc_info_new->grid;
	fwstart_info->gridy_new = lsc_info_new->grid;
#endif
...
cmr_int isp_alg_fw_start(cmr_handle isp_alg_handle, struct isp_video_start * in_ptr)
{
	...
	if (cxt->ops.awb_ops.ioctrl) {
		...
		ISP_TRACE_IF_FAIL(ret, ("fail to get flash awb enable"));
	}
	...
	if (cxt->ops.awb_ops.ioctrl) {
		...
		ISP_TRACE_IF_FAIL(ret, ("fail to set mainflash_en"));
	}
...
cmr_int isp_alg_fw_proc_start(cmr_handle isp_alg_handle, struct ips_in_param *in_ptr)
{
	...
	if (cxt->takepicture_mode != CAMERA_ISP_SIMULATION_MODE && cxt->ops.lsc_ops.ioctrl) {
		ISP_LOGD("ALSC_FW_START_END\n");
		ret = cxt->ops.lsc_ops.ioctrl(cxt->lsc_cxt.handle, ALSC_FW_START_END, (void *)&fwstart_info, NULL);
		ISP_TRACE_IF_FAIL(ret, ("fail to end alsc_fw_start_end"));
	}
