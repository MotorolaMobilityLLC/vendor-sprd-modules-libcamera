 ...
 */
#define LOG_TAG "isp_blk_rgb_dither"
#include "isp_blocks_cfg.h"

cmr_u32 _pm_rgb_dither_convert_param(void *dst_param, cmr_u32 strength_level, cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->nr_mode_setting) {
		...
		rgb_dither_param =
		    (struct sensor_rgb_dither_level *)((cmr_u8 *) dst_ptr->param_ptr + total_offset_units * dst_ptr->level_num * sizeof(struct sensor_rgb_dither_level));
...
cmr_s32 _pm_rgb_dither_set_param(void *rgb_dither_param, cmr_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	ISP_LOGV("ISP_SMART: cmd=%d, update=%d, rgb_dither_level=%d", cmd, header_ptr->is_update, dst_ptr->cur_level);
	return rtn;
}
