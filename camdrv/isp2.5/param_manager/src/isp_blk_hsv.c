...
cmr_s32 _pm_hsv_new_init(void *dst_hsv_param, void *src_hsv_param_new, void *param1, void *param2)
{
	...
	cmr_u32 *buf_ptr = PNULL;
	struct isp_data_bin_info *specialeffect = PNULL;
	struct isp_hsv_new_param *dst_ptr = (struct isp_hsv_new_param *)dst_hsv_param;
	struct sensor_hsv_new_param *src_ptr_new= (struct sensor_hsv_new_param *)src_hsv_param_new;
	struct isp_pm_block_header *header_ptr = (struct isp_pm_block_header *)param1;
	UNUSED(param2);

	index = src_ptr_new->cur_idx.x0;
	dst_ptr->cur_idx.x0 = src_ptr_new->cur_idx.x0;
	dst_ptr->cur_idx.x1 = src_ptr_new->cur_idx.x1;
	dst_ptr->cur_idx.weight0 = src_ptr_new->cur_idx.weight0;
	dst_ptr->cur_idx.weight1 = src_ptr_new->cur_idx.weight1;

	for (i = 0; i < SENSOR_HSV_NUM_NEW; i++) {
		dst_ptr->map[i].size = src_ptr_new->map_new[i].size;
		addr = (intptr_t) & src_ptr_new->data_area_new+ src_ptr_new->map_new[i].offset;
		dst_ptr->map[i].data_ptr = (void *)addr;
	}
	addr += src_ptr_new->map_new[SENSOR_HSV_NUM_NEW - 1].size;
	...
	if (PNULL == dst_ptr->final_map.data_ptr) {
		dst_ptr->final_map.data_ptr = (void *)malloc(src_ptr_new->map_new[index].size);
		if (PNULL == dst_ptr->final_map.data_ptr) {
			ISP_LOGE("fail to malloc !");
			rtn = ISP_ERROR;
			return rtn;
	...
	for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
		if (PNULL == dst_ptr->ct_result[i]) {
			ISP_LOGV("ct result malloc size : %d", src_ptr_new->map_new[index].size);
			dst_ptr->ct_result[i] = (cmr_u32 *)malloc(src_ptr_new->map_new[index].size);
			if (PNULL == dst_ptr->ct_result[i]) {
				ISP_LOGE("fail to malloc !");
				rtn = ISP_ERROR;
				if (dst_ptr->final_map.data_ptr != PNULL) {
					ISP_LOGE("fail to free dst final_map data ptr : %p", dst_ptr->final_map.data_ptr);
					free(dst_ptr->final_map.data_ptr);
				}
				for (j = 0; j < i; j++) {
					ISP_LOGE("fail to free dst ct result : j = %d, i = %d", j, i);
					free(dst_ptr->ct_result[j]);
				}
				return rtn;
			}
...
{
	cmr_s32 rtn = ISP_SUCCESS;
	struct isp_hsv_new_param *dst_hsv_ptr = (struct isp_hsv_new_param *)hsv_param;
	struct isp_pm_block_header *hsv_header_ptr = (struct isp_pm_block_header *)param_ptr1;
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			switch (block_result->ai_scene_id){
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "10");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "11");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "12");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "13");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "14");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				case ISP_PM_AI_SCENE_BACKLIGHT:
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "15");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "16");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "17");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "18");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				case ISP_PM_AI_SCENE_LANDSCAPE:
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "19");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "20");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "21");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "22");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "23");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
				...
				{
					char prop[256];
					property_get("debug.isp.hsv.hsv_value.level", prop, "24");
					hsv_level = atoi(prop);
					for (i = 0; i < ISP_PM_HSV_CTRESULT_NUM; i++) {
						ct_value[i]->value[0] = hsv_level;
						ct_value[i]->value[1] = hsv_level;
...
{
	cmr_s32 rtn = ISP_SUCCESS;
	struct isp_hsv_new_param *hsv_ptr = (struct isp_hsv_new_param *)hsv_param;
	struct isp_pm_param_data *param_data_ptr = (struct isp_pm_param_data *)rtn_param0;
	cmr_u32 *update_flag = (cmr_u32 *) rtn_param1;

	param_data_ptr->id = ISP_BLK_HSV_NEW;
	param_data_ptr->cmd = cmd;
