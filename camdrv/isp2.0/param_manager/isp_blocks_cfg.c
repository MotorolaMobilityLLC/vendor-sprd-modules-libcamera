...
static isp_u32 _pm_calc_nr_addr_offset(isp_u32 mode_flag, isp_u32 scene_flag, isp_u16 * one_multi_mode_ptr)
{
...
static isp_u32 _pm_pwd_convert_param(void *dst_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	struct isp_pre_wavelet_param_v1 *dst_ptr = (struct isp_pre_wavelet_param_v1*)dst_param;
	struct sensor_pwd_level* pwd_param = PNULL;
	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->scene_flag_verify) {
		pwd_param = (struct sensor_pwd_level*)(dst_ptr->pwd_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_ptr->pwd_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		pwd_param = (struct sensor_pwd_level*)((isp_u8 *)dst_ptr->pwd_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_pwd_level));
	}
...
static isp_s32 _pm_pre_wavelet_init_v1(void *dst_pwd, void *src_pwd, void* param1, void* param2)
{
	...
	dst_ptr->scene_flag_verify = src_ptr->scene_flag_verify;
	dst_ptr->pwd_scene_ptr= src_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_pwd_convert_param(dst_ptr, SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_ptr->cur.bypass |= pwd_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_pre_wavelet_set_param_v1(void *pwd_param, isp_u32 cmd, void* param_ptr0, void* param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != pwd_param_ptr->cur_level || nr_tool_flag[0] || block_result->mode_flag_changed) {
			...
			rtn = _pm_pwd_convert_param(pwd_param_ptr, cur_level, block_result->mode_flag, block_result->scene_flag);
...
static isp_s32 _pm_2d_lsc_init(void * dst_lnc_param,void * src_lnc_param,void * param1,void * param2)
{
	...
	if (dst_lnc_ptr->final_lsc_param.size < src_lnc_ptr->map[0].len) {
		...
		if (PNULL != dst_lnc_ptr->final_lsc_param.param_ptr) {
			free(dst_lnc_ptr->final_lsc_param.param_ptr);
			dst_lnc_ptr->final_lsc_param.param_ptr = PNULL;
		}
...
static isp_s32 _pm_2d_lsc_set_param(void *lnc_param, isp_u32 cmd, void* param_ptr0, void *param_ptr1)
{
	...
	break;
		...
		case ISP_PM_BLK_LSC_INFO:
		{
			lnc_header_ptr->is_update |= ISP_PM_BLK_LSC_UPDATE_MASK_PARAM;
			dst_lnc_ptr->update_flag = lnc_header_ptr->is_update;
			//ISP_LOGI("ISP_PM_BLK_LSC_INFO");gerald merge http://review.source.spreadtrum.com/gerrit/#/c/281621
			...
			#endif
				//ISP_LOGI("lsc[0]: 0x%0x, 0x%0x, 0x%0x, 0x%0x", *ptr, *(ptr+1), *(ptr+2), *(ptr+3));gerald merge http://review.source.spreadtrum.com/gerrit/#/c/281621
				//ISP_LOGI("lsc[1]: 0x%0x, 0x%0x, 0x%0x, 0x%0x", *(ptr+4), *(ptr+5), *(ptr+6), *(ptr+7));gerald merge http://review.source.spreadtrum.com/gerrit/#/c/281621
			}
		}
		break;
...
static isp_s32 _pm_2d_lsc_get_param(void *lnc_param, isp_u32 cmd, void* rtn_param0, void* rtn_param1)
{
	...
	break;
 	...
 	case ISP_PM_BLK_LSC_GET_LSCTAB://gerald merge http://review.source.spreadtrum.com/gerrit/#/c/279914 start
	...
	break;//gerald merge http://review.source.spreadtrum.com/gerrit/#/c/279914 end
...
static isp_u32 _pm_bpc_convert_param(void *dst_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	struct isp_bpc_param_v1 *dst_ptr = (struct isp_bpc_param_v1*)dst_param;
	struct sensor_bpc_level* bpc_param = PNULL;

	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->scene_flag_verify) {
		bpc_param = (struct sensor_bpc_level*)(dst_ptr->bpc_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_ptr->bpc_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		bpc_param = (struct sensor_bpc_level*)((isp_u8 *)dst_ptr->bpc_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_bpc_level));
...
static isp_s32 _pm_bpc_init_v1(void *dst_bpc_param, void *src_bpc_param, void* param1, void* param2)
{
	...
	dst_ptr->scene_flag_verify = src_ptr->scene_flag_verify;
	dst_ptr->bpc_scene_ptr= src_ptr->multi_nr_map_ptr;
	...
	rtn=_pm_bpc_convert_param(dst_ptr, SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_ptr->cur.bypass |=  header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_bpc_set_param_v1(void *bpc_param, isp_u32 cmd, void* param_ptr0, void* param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != bpc_ptr->cur_level || nr_tool_flag[1] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[1] = 0;
			block_result->mode_flag_changed = 0;
			rtn=_pm_bpc_convert_param(bpc_ptr, bpc_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_bl_nr_convert_param(void *dst_param, isp_u32 strength_level,  isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->scene_flag_verify) {
		...
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_ptr->bdn_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		bdn_param = (struct sensor_bdn_level*)((isp_u8 *)dst_ptr->bdn_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_bdn_level));
...
static isp_s32 _pm_bl_nr_init_v1(void *dst_bdn_param, void *src_bdn_param, void* param1, void* param2)
{
	...
	dst_ptr->scene_flag_verify = src_ptr->scene_flag_verify;
	dst_ptr->bdn_scene_ptr = src_ptr->multi_nr_map_ptr;
	...
	rtn=_pm_bl_nr_convert_param(dst_ptr, SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_ptr->cur.bypass |=  header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_bl_nr_set_param_v1(void *bdn_param, isp_u32 cmd, void* param_ptr0, void* param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level[0] != bdn_ptr->cur.ran_level || cur_level[1] != bdn_ptr->cur.dis_level || nr_tool_flag[2] ||
			...
			nr_tool_flag[2] = 0;
			block_result->mode_flag_changed = 0;
			rtn=_pm_bl_nr_convert_param(bdn_ptr, bdn_ptr->cur.ran_level,  block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_grgb_convert_param(void *dst_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	struct isp_grgb_param_v1 *dst_grgb_ptr = (struct isp_grgb_param_v1*)dst_param;
	struct sensor_grgb_v1_level* grgb_param = PNULL;

	if (SENSOR_MULTI_MODE_FLAG != dst_grgb_ptr->scene_flag_verify) {
		grgb_param = (struct sensor_grgb_v1_level*)(dst_grgb_ptr->grgb_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_grgb_ptr->grgb_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		grgb_param = (struct sensor_grgb_v1_level*)((isp_u8 *)dst_grgb_ptr->grgb_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_grgb_v1_level));
...
static isp_s32 _pm_grgb_init_v1(void *dst_grgb_param, void *src_grgb_param, void* param1, void* param2)
{
	...
	dst_ptr->scene_flag_verify = src_ptr->scene_flag_verify;
	dst_ptr->grgb_scene_ptr = src_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_grgb_convert_param(dst_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_ptr->cur.bypass |= grgb_header_ptr->bypass;
...
static isp_u32 _pm_nlm_convert_param(void *dst_nlm_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_nlm_ptr->scene_flag_verify) {
		...
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_nlm_ptr->nlm_scene_ptr;

		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		nlm_param = (struct sensor_nlm_level*)((isp_u8 *)dst_nlm_ptr->nlm_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_nlm_level));

		vst_param = (struct sensor_vst_level*)((isp_u8 *)dst_nlm_ptr->vst_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_vst_level));

		ivst_param = (struct sensor_ivst_level*)((isp_u8 *)dst_nlm_ptr->ivst_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_ivst_level));

		flat_offset_level = (struct sensor_flat_offset_level*)((isp_u8 *)dst_nlm_ptr->flat_offset_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_flat_offset_level));
...
static uint8_t nlm_lock_state=0;
isp_s32 _set_nlm_lock_state(uint8_t state){
	nlm_lock_state = state;// 1 is lock, 0 is unlock
	return 0;
}
static isp_s32 _get_nlm_lock_state(){
	return nlm_lock_state;// 1 is lock, 0 is unlock
}
...
static isp_s32 _pm_nlm_init(void *dst_nlm_param, void *src_nlm_param, void* param1, void* param_ptr2)
{
	...
	dst_nlm_ptr->scene_flag_verify = src_nlm_ptr->scene_flag_verify;
	dst_nlm_ptr->nlm_scene_ptr = src_nlm_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_nlm_convert_param(dst_nlm_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
...
static isp_s32 _pm_nlm_set_param(void *nlm_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if( 1==_get_nlm_lock_state() ){
			ISP_LOGD("lock nlm block");
			return rtn;
		}
		...
		if (nlm_level != nlm_ptr->nlm_level || nr_tool_flag[4] || block_result->mode_flag_changed) {
			...
			rtn = _pm_nlm_convert_param(nlm_ptr,nlm_ptr->nlm_level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_nlm_convert_param_v2(void *dst_nlm_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
...
static isp_s32 _pm_nlm_init_v2(void *dst_nlm_param, void *src_nlm_param, void* param1, void* param_ptr2)
{
	...
	rtn = _pm_nlm_convert_param_v2(dst_nlm_ptr, SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);;
	dst_nlm_ptr->cur.bypass |= nlm_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
		ISP_LOGE("ISP_PM_NLM_CONVERT_PARAM: error!");
		return rtn;
...
static isp_s32 _pm_nlm_set_param_v2(void *nlm_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (nlm_level != nlm_ptr->nlm_level || nr_tool_flag[5] || block_result->mode_flag_changed) {
			...
			rtn = _pm_nlm_convert_param_v2(nlm_ptr,nlm_ptr->nlm_level,block_result->mode_flag, block_result->scene_flag);
			nlm_ptr->cur.bypass |= nlm_header_ptr->bypass;
			if (ISP_SUCCESS != rtn) {
				ISP_LOGE("ISP_PM_NLM_CONVERT_PARAM: error!");
				return rtn;
...
static isp_u32 _pm_cfa_convert_param(void *dst_cfae_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	struct isp_cfa_param_v1 *dst_cfae_ptr = (struct isp_cfa_param_v1*)dst_cfae_param;
	struct sensor_cfae_level* cfae_param = PNULL;
	if (SENSOR_MULTI_MODE_FLAG != dst_cfae_ptr->scene_flag_verify) {
		cfae_param = (struct sensor_cfae_level*)(dst_cfae_ptr->cfae_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_cfae_ptr->cfa_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		cfae_param = (struct sensor_cfae_level*)((isp_u8 *)dst_cfae_ptr->cfae_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_cfae_level));
	}
...
static isp_s32 _pm_cfa_init_v1(void *dst_cfae_param, void *src_cfae_param, void* param1, void* param_ptr2)
{
	...
	dst_cfae_ptr->scene_flag_verify = src_cfae_ptr->scene_flag_verify;
	dst_cfae_ptr->cfa_scene_ptr = src_cfae_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_cfa_convert_param(dst_cfae_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	if (ISP_SUCCESS != rtn) {
...
static isp_u32 _pm_rgb_precdn_convert_param(void *dst_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_ptr->scene_flag_verify) {
		...
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_ptr->rgb_precdn_scene_ptr;

		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		precdn_param = (struct sensor_rgb_precdn_level*)((isp_u8 *)dst_ptr->rgb_precdn_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_rgb_precdn_level));
...
static isp_s32 _pm_rgb_pre_cdn_init_v1(void *dst_pre_cdn_param, void *src_pre_cdn_param, void* param1, void* param2)
{
	...
	dst_ptr->scene_flag_verify = src_ptr->scene_flag_verify;
	dst_ptr->rgb_precdn_scene_ptr= src_ptr->multi_nr_map_ptr;
	...
	rtn=_pm_rgb_precdn_convert_param(dst_ptr, SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_ptr->cur.bypass |= pre_cdn_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_rgb_pre_cdn_set_param_v1(void *pre_cdn_param, isp_u32 cmd, void* param_ptr0, void* param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != dst_pre_cdn_ptr->cur_level || nr_tool_flag[7] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[7] = 0;
			block_result->mode_flag_changed = 0;
			rtn=_pm_rgb_precdn_convert_param(dst_pre_cdn_ptr, dst_pre_cdn_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_yiq_afm_convert_param(void *dst_yiq_afm_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	struct isp_yiq_afm_param_v1 *dst_yiq_afm_ptr = (struct isp_yiq_afm_param_v1*)dst_yiq_afm_param;

	struct sensor_y_afm_level* yiq_afm_param = PNULL;

	if (SENSOR_MULTI_MODE_FLAG != dst_yiq_afm_ptr->scene_flag_verify) {
		yiq_afm_param = (struct sensor_y_afm_level*)(dst_yiq_afm_ptr->yiq_afm_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_yiq_afm_ptr->yiq_afm_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		yiq_afm_param = (struct sensor_y_afm_level*)((isp_u8 *)dst_yiq_afm_ptr->yiq_afm_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_y_afm_level));
...
static isp_s32 _pm_yiq_afm_init(void *dst_yiq_afm_param, void *src_yiq_afm_param, void *param1, void *param2)
{
	...
	dst_yiq_afm_ptr->scene_flag_verify = src_yiq_afm_ptr->scene_flag_verify;
	dst_yiq_afm_ptr->yiq_afm_scene_ptr= src_yiq_afm_ptr->multi_nr_map_ptr;
	...
	for (i = 0; i < 11; i++) {
       ...
       rtn =  _pm_yiq_afm_convert_param(dst_yiq_afm_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
...
static isp_s32 _pm_yiq_afm_set_param(void *yiq_afm_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != yiq_afm_ptr->cur_level || nr_tool_flag[8] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[8] = 0;
			block_result->mode_flag_changed = 0;
			rtn = _pm_yiq_afm_convert_param(yiq_afm_ptr,yiq_afm_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
			if (ISP_SUCCESS != rtn) {
				ISP_LOGE("ISP_PM_YIQ_AFM_CONVERT_PARAM: error!");
				return rtn;
			}
...
static isp_u32 _pm_yuv_precdn_convert_param(void *dst_precdn_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	struct isp_yuv_precdn_param *dst_precdn_ptr = (struct isp_yuv_precdn_param*)dst_precdn_param;
	struct sensor_yuv_precdn_level* yuv_precdn_param = PNULL;
	if (SENSOR_MULTI_MODE_FLAG != dst_precdn_ptr->scene_flag_verify) {
		yuv_precdn_param = (struct sensor_yuv_precdn_level*)(dst_precdn_ptr->yuv_precdn_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_precdn_ptr->yuv_precdn_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		yuv_precdn_param = (struct sensor_yuv_precdn_level*)((isp_u8 *)dst_precdn_ptr->yuv_precdn_ptr + total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_yuv_precdn_level));
...
static isp_s32 _pm_yuv_precdn_init(void *dst_precdn_param, void *src_precdn_param, void *param1, void *param2)
{
	...
	dst_precdn_ptr->scene_flag_verify = src_precdn_ptr->scene_flag_verify;
	dst_precdn_ptr->yuv_precdn_scene_ptr= src_precdn_ptr->multi_nr_map_ptr;

	rtn = _pm_yuv_precdn_convert_param(dst_precdn_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_precdn_ptr->cur.bypass |= precdn_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_yuv_precdn_set_param(void *precdn_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != precdn_ptr->cur_level || nr_tool_flag[9] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[9] = 0;
			block_result->mode_flag_changed = 0;
			rtn = _pm_yuv_precdn_convert_param(precdn_ptr,precdn_ptr->cur.level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_prfy_convert_param(void *dst_prfy_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	struct isp_prfy_param *dst_prfy_ptr = (struct isp_prfy_param*)dst_prfy_param;
	struct sensor_prfy_level* prfy_param = PNULL;
	if (SENSOR_MULTI_MODE_FLAG != dst_prfy_ptr->scene_flag_verify) {
		prfy_param = (struct sensor_prfy_level*)(dst_prfy_ptr->prfy_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_prfy_ptr->prfy_scene_ptr;

		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		prfy_param = (struct sensor_prfy_level*)((isp_u8 *)dst_prfy_ptr->prfy_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_prfy_level));
...
static isp_s32 _pm_prfy_init(void *dst_prfy_param, void *src_prfy_param, void *param1, void *param2)
{
	...
	dst_prfy_ptr->scene_flag_verify = src_prfy_ptr->scene_flag_verify;
	dst_prfy_ptr->prfy_scene_ptr= src_prfy_ptr->multi_nr_map_ptr;

	rtn = _pm_prfy_convert_param(dst_prfy_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_prfy_ptr->cur.bypass |= prfy_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_prfy_set_param(void *prfy_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != prfy_ptr->cur_level || nr_tool_flag[10] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[10] = 0;
			block_result->mode_flag_changed = 0;
			rtn = _pm_prfy_convert_param(prfy_ptr,prfy_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_uv_cdn_convert_param(void *dst_cdn_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_cdn_ptr->scene_flag_verify) {
		uv_cdn_param = (struct sensor_uv_cdn_level*)(dst_cdn_ptr->uv_cdn_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_cdn_ptr->uv_cdn_scene_ptr;

		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		uv_cdn_param = (struct sensor_uv_cdn_level*)((isp_u8 *)dst_cdn_ptr->uv_cdn_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_uv_cdn_level));
...
static isp_s32 _pm_uv_cdn_init_v1(void *dst_cdn_param, void *src_cdn_param, void *param1, void *param2)
{
	...
	dst_cdn_ptr->scene_flag_verify = src_cdn_ptr->scene_flag_verify;
	dst_cdn_ptr->uv_cdn_scene_ptr = src_cdn_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_uv_cdn_convert_param(dst_cdn_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_cdn_ptr->cur.bypass |= cdn_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_uv_cdn_set_param_v1(void *cdn_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != cdn_ptr->cur_level || nr_tool_flag[11] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[11] = 0;
			block_result->mode_flag_changed = 0;
			rtn = _pm_uv_cdn_convert_param(cdn_ptr,cdn_ptr->cur.level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_uv_postcdn_convert_param(void *dst_postcdn_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0, j = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_postcdn_ptr->scene_flag_verify) {
		postcdn_param = (struct sensor_uv_postcdn_level*)(dst_postcdn_ptr->uv_postcdn_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_postcdn_ptr->uv_postcdn_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		postcdn_param = (struct sensor_uv_postcdn_level*)((isp_u8 *)dst_postcdn_ptr->uv_postcdn_ptr + total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_uv_postcdn_level));
	}
...
static isp_s32 _pm_uv_postcdn_init(void *dst_postcdn_param, void *src_postcdn_param, void *param1, void *param_ptr2)
{
	...
	dst_postcdn_ptr->scene_flag_verify = src_postcdn_ptr->scene_flag_verify;
	dst_postcdn_ptr->uv_postcdn_scene_ptr = src_postcdn_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_uv_postcdn_convert_param(dst_postcdn_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_postcdn_ptr->cur.bypass |= postcdn_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_uv_postcdn_set_param(void *postcdn_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != postcdn_ptr->cur_level || nr_tool_flag[12] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[12] = 0;
			block_result->mode_flag_changed = 0;
			rtn = _pm_uv_postcdn_convert_param(postcdn_ptr,postcdn_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_edge_convert_param(void *dst_edge_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_edge_ptr->scene_flag_verify) {
		edge_param = (struct sensor_ee_level*)(dst_edge_ptr->edge_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_edge_ptr->edge_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		edge_param = (struct sensor_ee_level*)((isp_u8 *)dst_edge_ptr->edge_ptr+ total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_ee_level));
...
static isp_s32 _pm_edge_init_v1(void *dst_edge_param, void *src_edge_param, void *param1, void *param2)
{
	...
	dst_edge_ptr->scene_flag_verify = src_edge_ptr->scene_flag_verify;
	dst_edge_ptr->edge_scene_ptr = src_edge_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_edge_convert_param(dst_edge_ptr,SENSOR_SMART_LEVEL_DEFAULT,ISP_MODE_ID_COMMON,ISP_SCENEMODE_AUTO);
	dst_edge_ptr->cur.bypass |= edge_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_edge_set_param_v1(void *edge_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (level != edge_ptr->cur.ee_level || nr_tool_flag[13] || block_result->mode_flag_changed) {
			...
			rtn = _pm_edge_convert_param(edge_ptr,edge_ptr->level, block_result->mode_flag,block_result->scene_flag);
...
static isp_u32 _pm_iircnr_iir_convert_param(void *dst_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_iircnr_ptr->scene_flag_verify) {
		iir_cnr_param = (struct sensor_iircnr_level*)(dst_iircnr_ptr->iir_cnr_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_iircnr_ptr->iircnr_iir_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		iir_cnr_param = (struct sensor_iircnr_level *)((isp_u8 *)dst_iircnr_ptr->iir_cnr_ptr + total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_iircnr_level));
...
static isp_s32 _pm_iircnr_iir_init(void *dst_iircnr_param, void *src_iircnr_param, void *param1, void *param_ptr2)
{
	...
	dst_iircnr_ptr->scene_flag_verify = src_iir_nr_ptr->scene_flag_verify;
	dst_iircnr_ptr->iircnr_iir_scene_ptr = src_iir_nr_ptr->multi_nr_map_ptr;
	...
	dst_iircnr_ptr->cur.factor = src_iir_nr_ptr->iircnr_level.iircnr_middle_factor;
        ...
        rtn=_pm_iircnr_iir_convert_param(dst_iircnr_ptr, SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_iircnr_ptr->cur.bypass |= iircnr_header_ptr->bypass;
	 if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_iircnr_iir_set_param(void *iircnr_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != iircnr_ptr->cur_level || nr_tool_flag[14] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[14] = 0;
			block_result->mode_flag_changed = 0;
			rtn=_pm_iircnr_iir_convert_param(iircnr_ptr, iircnr_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
...
static isp_u32 _pm_iir_yrandom_convert_param(void *dst_iir_yrandom_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_iir_yrandom_ptr->scene_flag_verify) {
		iir_yrandom_param = (struct sensor_iircnr_yrandom_level*)(dst_iir_yrandom_ptr->iir_yrandom_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_iir_yrandom_ptr->iircnr_yrandom_scene_ptr;
		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		iir_yrandom_param = (struct sensor_iircnr_yrandom_level*)((isp_u8 *)dst_iir_yrandom_ptr->iircnr_yrandom_scene_ptr +
			total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_iircnr_yrandom_level));
...
static isp_s32 _pm_iircnr_yrandom_init(void *dst_iircnr_param, void *src_iircnr_param, void *param1, void *param_ptr2)
{
	...
	dst_iir_yrandom_ptr->scene_flag_verify = src_iir_yrandom_ptr->scene_flag_verify;
	dst_iir_yrandom_ptr->iircnr_yrandom_scene_ptr = src_iir_yrandom_ptr->multi_nr_map_ptr;
	...
	rtn = _pm_iir_yrandom_convert_param(dst_iir_yrandom_ptr,SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_iir_yrandom_ptr->cur.bypass |= iir_yrandom_header_ptr->bypass;
...
static isp_s32 _pm_iircnr_yrandom_set_param(void *iircnr_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_level != iircnr_ptr->cur_level || nr_tool_flag[15] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[15] = 0;
			block_result->mode_flag_changed = 0;
			rtn = _pm_iir_yrandom_convert_param(iircnr_ptr,iircnr_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
			iircnr_ptr->cur.bypass |= iircnr_header_ptr->bypass;
			if (ISP_SUCCESS != rtn) {
				ISP_LOGE("ISP_PM_IIR_YRANDOM_CONVERT_PARAM: error!");
				return rtn;
...
static isp_u32 _pm_uv_div_convert_param(void *dst_param, isp_u32 strength_level, isp_u32 mode_flag, isp_u32 scene_flag)
{
	isp_s32 rtn = ISP_SUCCESS;
	isp_s32 i = 0;
	...
	if (SENSOR_MULTI_MODE_FLAG != dst_uvdiv_ptr->scene_flag_verify) {
		cce_uvdiv_param = (struct sensor_cce_uvdiv_level*)(dst_uvdiv_ptr->uv_div_ptr);
	} else {
		isp_u16 *multi_nr_map_ptr = PNULL;
		multi_nr_map_ptr = (isp_u16 *)dst_uvdiv_ptr->uvdiv_scene_ptr;

		isp_u32 total_offset_units = _pm_calc_nr_addr_offset(mode_flag, scene_flag, multi_nr_map_ptr);
		cce_uvdiv_param = (struct sensor_cce_uvdiv_level*)((isp_u8 *)dst_uvdiv_ptr->uv_div_ptr + total_offset_units * SENSOR_SMART_LEVEL_NUM * sizeof(struct sensor_cce_uvdiv_level));
...
static isp_s32 _pm_uv_div_init_v1(void *dst_uv_div_param, void *src_uv_div_param, void *param1, void *param_ptr2)
{
	...
	dst_uvdiv_ptr->scene_flag_verify = src_uvdiv_ptr->scene_flag_verify;
	dst_uvdiv_ptr->uvdiv_scene_ptr= src_uvdiv_ptr->multi_nr_map_ptr;
	...
	dst_uvdiv_ptr->cur.base = src_uvdiv_ptr->cce_uvdiv_level.base;
        ...
        rtn=_pm_uv_div_convert_param(dst_uvdiv_ptr, SENSOR_SMART_LEVEL_DEFAULT, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
	dst_uvdiv_ptr->cur.uvdiv_bypass |= uvdiv_header_ptr->bypass;
	if (ISP_SUCCESS != rtn) {
...
static isp_s32 _pm_uv_div_set_param_v1(void *uv_div_param, isp_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	case ISP_PM_BLK_SMART_SETTING:
	{
		...
		if (cur_idx != uvdiv_ptr->cur_level || nr_tool_flag[16] || block_result->mode_flag_changed) {
			...
			nr_tool_flag[16] = 0;
			block_result->mode_flag_changed = 0;
			 rtn=_pm_uv_div_convert_param(uvdiv_ptr, uvdiv_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
...
struct isp_block_cfg s_blk_cfgs[] = {
	...
	{ISP_BLK_AWB_NEW, array_offset(struct isp_context,awb_v1), sizeof(struct isp_awb_param_v1), &s_awb_ops_v1},
	...
	{ISP_BLK_AWB_NEW, array_offset(struct isp_context,awb_v2), sizeof(struct isp_awb_param_v2), &s_awb_ops_v2},
	{ISP_BLK_AE_V1, array_offset(struct isp_context,ae_v1), sizeof(struct isp_rgb_ae_param), &s_rgb_ae_ops},
	{ISP_BLK_BPC_V1,array_offset(struct isp_context,bpc_v1),sizeof(struct isp_bpc_param_v1),&s_bpc_ops_v1},
	{ISP_BLK_BL_NR_V1,array_offset(struct isp_context,bl_nr_v1),sizeof(struct isp_bl_nr_param_v1),&s_bl_nr_ops_v1},
	{ISP_BLK_GRGB_V1,array_offset(struct isp_context,grgb_v1),sizeof(struct isp_grgb_param_v1),&s_grgb_ops_v1},
