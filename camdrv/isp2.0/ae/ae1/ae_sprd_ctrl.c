	...
	switch (cmd) {
		...
		case AE_VIDEO_START:
			if (param){
				...
				cxt->cur_status.snr_max_fps = 30;//work_info->sensor_fps.max_fps;//because no this interface from sensor driver
				cxt->cur_status.snr_min_fps = 30;//work_info->sensor_fps.min_fps;
		...
		case AE_GET_AF_INFO:
			if (param){
				cxt->cur_status.settings.af_info = *((int8_t *)param);
			}else{
