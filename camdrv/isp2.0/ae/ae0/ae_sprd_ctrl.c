...
enum ae_tc_status {
	AE_TOUCH_STATUS_BEFORE,
	AE_TOUCH_STATUS_ACTIVE,
	AE_TOUCH_STATUS_ACTIVE_LOCK,
	AE_TOUCH_STATUS_RELEASE,
	AE_TOUCH_STATUS_MAX,
};

struct touch_zone_param {
	uint32_t enable;
    //uint32_t status;// will remove
	enum ae_tc_status tc_status;
                                   ...
                                   uint32_t is_force)
{
	...
	if ((AE_WORK_MODE_VIDEO != next_work_mode)
		...
		//bethany: for face
		if(FDAE_STATE_ON_LOCK_WEIGHT == cxt->fdae.state){
			misc_work_in.weight_table = &(cxt->fdae.w_work_tbl);
		}
		rtn = ae_misc_io_ctrl(cxt->misc_handle, AE_MISC_CMD_SET_WORK_PARAM, &misc_work_in, &misc_work_out);
		if (AE_SUCCESS == rtn) {
			cxt->cur_param = cur_param;
...
static int32_t _set_scene_mode(struct ae_ctrl_context *cxt, enum ae_scene_mode scene_mode)
{
	...
	if ((i < AE_SCENE_MAX) && (AE_SCENE_NORMAL != scene_mode)) {
		...
		if (AE_SCENE_NORMAL != scene_mode) {
			...
			struct ae_weight_tbl_param wght_tbl = {0x00};
            wght_tbl.flag = 0;
            wght_tbl.wght = &cur_param->weight_table[weight_mode];
		...
		/*weight table*/
		AE_LOGD("fdae:weight change at set_scene_mode(normal)");
		struct ae_weight_tbl_param wght_tbl = {0x00};
		wght_tbl.flag = 0;
		wght_tbl.wght = &cur_param->weight_table[weight_mode];
		rtn = ae_misc_io_ctrl(cxt->misc_handle, AE_MISC_CMD_SET_WEIGHT_TABLE,
...
static int32_t is_in_map_win(uint32_t i, uint32_t j, struct ae_rect *rect)

{
	...
	if (rect->start_x <= i && rect->start_y <= j
			&& i <= rect->end_x && j <= rect->end_y) {
		is_in = 1;
	}
...
static int32_t _set_touch_zone(struct ae_ctrl_context *cxt, struct ae_trim *touch_zone)
{
	...
	//define the related variables of touch window coordinate at image coordinate system
	uint32_t touch_x = 0;//
	uint32_t touch_y = 0;
	uint32_t touch_w = 0;//
	uint32_t touch_h = 0;
	...
	uint32_t ROI_end_x = 0;
	uint32_t ROI_end_y = 0;
	uint32_t ROI_W = 0;
	uint32_t ROI_H = 0;
	//scene info
	uint32_t org_w = 0;
	...
	uint32_t num_w = 0;
	uint32_t num_h = 0;
	uint32_t win_small_w = 0;
	uint32_t win_small_h = 0;
	//mapping touch window to base window, get the start and end coordinate
	struct ae_rect roi_map_trim;
	uint32_t i = 0;
	uint32_t j = 0;
	//update the weight depended on mapping window
	uint8_t *tab_ptr = NULL;
	int32_t weight_val = 0;
	int32_t roi_win_weight = 0;//
	
	struct ae_weight_table *weight_tab = NULL;
	struct ae_weight_tbl_param wght_tbl = {0x00};
	ROI_W = cxt->zone.w;//272;////from tuning setting
	ROI_H = cxt->zone.h;//204;//from tuning setting
	AE_LOGI("AESPRDROI:ROI_W: %d,ROI_H:%d",ROI_W,ROI_H);

	if (NULL == cxt || NULL == touch_zone) {
		AE_LOGE("cxt %p touch_zone %p param is error!", cxt, touch_zone);
		return AE_ERROR;
	}
	if (1 < touch_zone->w && 1 < touch_zone->h){
	// got the coordinate from userspace,
	touch_x = touch_zone->x;//868;//=
	touch_y = touch_zone->y;//658;//=
	touch_w = touch_zone->w;//162;//=
	touch_h = touch_zone->h;//120;//=
	// translation the coordination from userspace setting to tuning setting
	center_x = touch_x + touch_w/2;
	center_y = touch_y + touch_h/2;
    ROI_start_x = center_x - ROI_W/2;
	ROI_start_y = center_y - ROI_H/2;
	ROI_end_x = center_x + ROI_W/2;
	ROI_end_y = center_y + ROI_H/2;
	//AE_LOGI("AESPRDcor:APP touch start x, y, w,h: %d,%d,%d,%d",touch_x,touch_y,touch_w,touch_h);
	//AE_LOGI("AESPRDcor:center x, y,ROI start x, y,end: x,y: %d,%d,%d,%d,%d,%d",center_x,center_y,ROI_start_x,ROI_start_y,ROI_end_x,ROI_end_y);
	// some original info, such as image size and statistic small size
	org_w = cxt->resolution_info.frame_size.w;
	org_h = cxt->resolution_info.frame_size.h;
	num_w = cxt->monitor_unit.win_num.w;
	num_h = cxt->monitor_unit.win_num.h;
	win_small_w = cxt->monitor_unit.win_size.w;// 
	win_small_h = cxt->monitor_unit.win_size.h;//
	//AE_LOGI("AESPRD:win_small_w: %d,win_small_h:%d,num_w:%d,num_h:%d,org_w:%d,org_h:%d",win_small_w,win_small_h,num_w,num_h,org_w,org_h);
	// calculate the start and end coordinate in the statistic window of (win_num.w*win_num.h)
	roi_map_trim.start_x = ROI_start_x/win_small_w;
	roi_map_trim.start_y = ROI_start_y/win_small_h;
	roi_map_trim.end_x = ROI_end_x/win_small_w;
	roi_map_trim.end_y = ROI_end_y/win_small_h;
	//AE_LOGI("AESPRD:touch ROI MAP startx, starty, endx,endy: %d,%d,%d,%d",roi_map_trim.start_x,roi_map_trim.start_y,roi_map_trim.end_x,roi_map_trim.end_y);
	//for update AE  weight
	roi_win_weight = cxt->touch_zone_param.zone_param.level_0_weight;//= 2;//now giving it one fixed value,but finally setting in tuning parameters
	tab_ptr = cxt->touch_zone_param.weight_table.weight;
	//AE_LOGI("AESPRD:roi_win_weight:%d!", roi_win_weight);
	for (j = 0; j < num_h; ++j) {
		for (i = 0; i < num_w; ++i) {
				weight_val = 0;
				roi_win_weight = 1;
				if (is_in_map_win(i, j, &roi_map_trim))
						weight_val = roi_win_weight;
		
					tab_ptr[i + num_w * j] = weight_val;
		
				}
			}
	// Write the new table to ae	
	weight_tab = &cxt->touch_zone_param.weight_table; //
	wght_tbl.flag = 1;
	wght_tbl.wght = weight_tab;
	rtn = ae_misc_io_ctrl(cxt->misc_handle, AE_MISC_CMD_SET_WEIGHT_TABLE,(void *)&wght_tbl, NULL);
	if (AE_SUCCESS == rtn) {
		cxt->cur_status.touch_zone = *touch_zone;
 		}
	cxt->touch_zone_param.enable = 1;
	cxt->touch_zone_param.tc_status = AE_TOUCH_STATUS_ACTIVE;// 1
	//AE_LOGI("AESPRD:TOUCH_ZONE_ON:%d!", cxt->touch_zone_param.enable);
	}else {
	uint32_t wght_mod = cxt->cur_status.weight;
	wght_tbl.flag = 1;
	wght_tbl.wght = NULL;
	rtn = ae_misc_io_ctrl(cxt->misc_handle, AE_MISC_CMD_SET_WEIGHT_TABLE,(void *)&wght_tbl, NULL);
	if (AE_SUCCESS == rtn) {
	...
	cxt->nxt_status.touch_zone.x = 0; //the coordination should be zero after release touch ae 
	cxt->nxt_status.touch_zone.y = 0; //the coordination should be zero after release touch ae 
	cxt->nxt_status.touch_zone.w = 1; //the coordination should be zero after release touch ae 
	cxt->nxt_status.touch_zone.h = 1; //the coordination should be zero after release touch ae 
	cxt->touch_zone_param.enable = 0; 
	cxt->touch_zone_param.tc_status = AE_TOUCH_STATUS_RELEASE;// 3
	//AE_LOGI("AESPRD:TOUCH_ZONE_release:%d!", cxt->touch_zone_param.enable);
	}
	return rtn;
}
...
static int32_t _set_weight(struct ae_ctrl_context *cxt, enum ae_weight_mode mode)
{
	...
	//AE_LOGI("AESPRD:set weight from %d to %d, rtn=%d", cxt->cur_status.weight, mode, rtn);

	wght_tbl.flag = 0;
	wght_tbl.wght = &cxt->cur_param->weight_table[mode];
	rtn = ae_misc_io_ctrl(cxt->misc_handle, AE_MISC_CMD_SET_WEIGHT_TABLE,
					(void *)&wght_tbl, NULL);
	...
	if (AE_SUCCESS == rtn) {
		...
		cxt->touch_zone_param.enable = 0;
		//AE_LOGI("AESPRD:cxt->cur_status.weight: %d , cxt->touch_zone_param.enable=%d", cxt->cur_status.weight, cxt->touch_zone_param.enable);
	}
...
static int32_t _update_status(struct ae_ctrl_context *cxt)
{
	...
	update_list->is_weight = 0;
	AE_LOGI("AESPRD: TOUCH ZONE triger:%d",update_list->is_touch_zone);
	//AE_LOGI("AESPRDCTRL2: next_status->touch_zone.x = %d, next_status->touch_zone.y = %d, next_status->touch_zone.w = %d, next_status->touch_zone.h = %d", 
	//						next_status->touch_zone.x,		next_status->touch_zone.y,		next_status->touch_zone.w,		next_status->touch_zone.h);
	//AE_LOGI("AESPRDCTRL2: cur_status->touch_zone.x = %d, cur_status->touch_zone.y = %d, cur_status->touch_zone.w = %d, cur_status->touch_zone.h = %d", 
	//						 cur_status->touch_zone.x,		cur_status->touch_zone.y,	   cur_status->touch_zone.w,	  cur_status->touch_zone.h);

	//if (update_list->is_touch_zone
	if ((1 == cxt->tuning_touch_on_off)\
		    && (1 == update_list->is_touch_zone)\
			...
			|| cur_status->touch_zone.h != next_status->touch_zone.h )
			   &&(1 != next_status->touch_zone.w)
			     &&(1 != next_status->touch_zone.h)) {
			uint weight = 0;
			uint high =0;
			...
			if ((next_status->touch_zone.x < weight)
				...
				AE_LOGD("Beth:start touch function");
...
static void _convert_result_to_out(struct ae_misc_calc_out *from, struct ae_calc_out *to)
{
	if (from && to) {
		...
		to->base_lum = from->base_lum;//for release touch ae
		to->roi_lum = from->roi_lum;//for release touch ae
	}
}
...
static int32_t _set_init_param(struct ae_ctrl_context *cxt, struct ae_init_in *init_param)
{
	...
	/* to keep the camera can work well when the setting is zero or NULL in the tuning file*/
	//bethany for debug
	/*
		if((0 == cxt->cur_param->touch_info.zone.w)\
		||(0 == cxt->cur_param->touch_info.zone.h)\
		||((0 == cxt->cur_param->touch_info.win1_weight)&&(0 == cxt->cur_param->touch_info.win2_weight))) {
		cxt->zone.w = init_param->resolution_info.frame_size.w/8;
		cxt->zone.h = init_param->resolution_info.frame_size.h/8;
		cxt->win1_weight = 4;
		cxt->win2_weight = 3;
		cxt->tuning_touch_on_off = 1;
		AE_LOGD("Beth:to debug, give these tuning params one group default setting");
	} else {
		//cxt->zone = cxt->cur_param->touch_info.zone;
		cxt->zone.w = 408;
		cxt->zone.h = 306;
		cxt->win1_weight = cxt->cur_param->touch_info.win1_weight;
		cxt->win2_weight = cxt->cur_param->touch_info.win2_weight;
		cxt->tuning_touch_on_off = cxt->cur_param->touch_info.enable;
		AE_LOGD("these params setting from tuning");
	}
	*/
...
/* to keep the camera can work well when the setting is zero or NULL in the tuning file*/
    ...
    cxt->tuning_touch_on_off = cxt->cur_param->touch_info.enable;
	if ((1 == cxt->tuning_touch_on_off)\
		&&((0 == cxt->cur_param->touch_info.zone.w)\
		||(0 == cxt->cur_param->touch_info.zone.h)\
		||((0 == cxt->cur_param->touch_info.win1_weight)&&(0 == cxt->cur_param->touch_info.win2_weight)))) {
		cxt->zone.w = init_param->resolution_info.frame_size.w/10;
		cxt->zone.h = init_param->resolution_info.frame_size.h/10;
		cxt->win1_weight = 4;
		cxt->win2_weight = 3;
		AE_LOGD("give these tuning params one group default setting");
	} else {
		//cxt->tuning_touch_on_off = cxt->cur_param->touch_info.enable;
		cxt->zone = cxt->cur_param->touch_info.zone;
		cxt->win1_weight = cxt->cur_param->touch_info.win1_weight;
		cxt->win2_weight = cxt->cur_param->touch_info.win2_weight;
		AE_LOGD("WIN_W and win_h and weight(%d,%d)and(%d,%d)",cxt->zone.w,cxt->zone.h,cxt->win1_weight,cxt->win2_weight);
		AE_LOGD("these params setting from tuning");
	}
	
	//AE_LOGD("test tuing on-off: %d",cxt->tuning_touch_on_off);
		...
		if (cxt->flash_param.enable) {
			...
			case AE_FLASH_STATE_ACTIVE:
				...
				if (cxt->isp_ops.callback) {
					...
					(*cxt->isp_ops.callback)(cxt->isp_ops.isp_handler, AE_CB_FLASHING_CONVERGED, NULL);
				}
				need_exp_write_sensor = 0;
...
static int32_t _set_frame_cnt(struct ae_ctrl_context *cxt)
{
	...
	if (cxt->flash_param.highflash_skip_en) {
		...
		if (cxt->flash_param.highflash_skip_cnt == cxt->cap_skip_num) {
			if (cxt->isp_ops.callback) {
				(*cxt->isp_ops.callback)(cxt->isp_ops.isp_handler, AE_CB_QUICKMODE_DOWN, NULL);
...
static int32_t _get_average_lum(struct ae_ctrl_context *ae_cxt, struct ae_misc_calc_in *misc_calc_in, uint32_t *cur_lum)
{
	int32_t rtn = AE_SUCCESS;
	uint32_t i = 0x00;
	uint32_t valid_stat = 0x00;
	uint32_t *stat_img = NULL;
	__u64 stat_pixel = 0x00;
	__u64 max_stat = 0x00;
	__u64 check_stat = 0x00;
	__u64 r_stat = 0x00;
	__u64 g_stat = 0x00;
	__u64 b_stat = 0x00;
	__u64 y_stat = 0x00;
	...
	uint32_t r_index = 0x00;
	uint32_t g_index = misc_calc_in->win_num.w * misc_calc_in->win_num.h;
	uint32_t b_index = misc_calc_in->win_num.w * misc_calc_in->win_num.h * 2;
	uint8_t *cur_weight_ptr = &ae_cxt->cur_param->weight_table[ae_cxt->cur_status.weight].weight[0];
	//uint32_t *static_img = (uint32_t *)misc_calc_in->stat_img;

	if ((NULL == misc_calc_in)
		|| (NULL == cur_lum)) {
		AE_RETURN_IF_FAIL(rtn, ("in param : %p , cur lum : %p  |^_^|", misc_calc_in, cur_lum));
	}
	
    stat_img = (uint32_t*)misc_calc_in->stat_img;
	stat_pixel = misc_calc_in->win_size.w * misc_calc_in->win_size.h;
	max_stat = stat_pixel * 255;

	while (i < (misc_calc_in->win_num.w * misc_calc_in->win_num.h)) {
		check_stat = (stat_img[r_index + i] * misc_calc_in->awb_gain.r) >> 10;
		if (max_stat < check_stat) {
			check_stat = max_stat;
		}
		r_stat = check_stat / stat_pixel;

		check_stat = (stat_img[g_index + i] * misc_calc_in->awb_gain.g) >> 10;
		if (max_stat < check_stat) {
			check_stat = max_stat;
		}
		g_stat = check_stat / stat_pixel;

		check_stat = (stat_img[b_index + i] * misc_calc_in->awb_gain.b) >> 10;
		if (max_stat < check_stat) {
			check_stat = max_stat;
		}
		b_stat = check_stat / stat_pixel;

		y_temp= ((uint32_t)RGB_TO_Y(r_stat, g_stat, b_stat));
		if (255 < y_temp) {
			y_temp = 255;
		}

		/*Y stat for ae adjust*/
		y_stat += y_temp * cur_weight_ptr[i];
		valid_stat += cur_weight_ptr[i];
		++i;
	}
	...
	if (0x00 != valid_stat) {
		...
		rtn = AE_ERROR;
		AE_RETURN_IF_FAIL(rtn, ("alg0 init handler (rtn : 0x%x) |^_^|", rtn));
	}
	*cur_lum = ((uint32_t)y_stat);
...
int32_t ae_sprd_calculation(void* handle, struct ae_calc_in *param, struct ae_calc_out *result)
{
	...
	uint32_t ae_pause = 0;
	uint32_t current_lum_0 = 0;
	uint32_t current_lum_1 = 0;
	uint32_t next_lum_0 = 0;
	uint32_t next_lum_1 = 0;
	int32_t deltaLUM_0 = 0;
	int32_t deltaLUM_1 = 0;
	uint32_t current_roi_lum = 0;
	uint32_t next_roi_lum = 0;
	int32_t deltaROIlum = 0;
	uint32_t cur_lum_4_tc = 0;
	uint32_t deltaAVGlum = 0;
	if ((NULL == param) || (NULL == result)) {
		AE_LOGE("in %p out %p param is error!", param, result);
		return AE_PARAM_NULL;
	}
	...
	misc_calc_in.win1_weight = cxt->win1_weight;
	misc_calc_in.win2_weight = cxt->win2_weight;
	current_lum_0 = cxt->cur_result.cur_lum;
	current_lum_1 = cxt->cur_result.base_lum;
	current_roi_lum = cxt->cur_result.roi_lum;
	//AE_LOGE("AESPRD-ROI:current_roi_lum:%d",cxt->cur_result.roi_lum);
	//AE_LOGE("AESPRD-whole:cur_lum:%d,base_lum:%d",cxt->cur_result.cur_lum,cxt->cur_result.base_lum);
/*jwh*/
...
/*jwh*/
	...
	/*bethany*/
	next_lum_0 = misc_calc_out.cur_lum;
	next_lum_1 = misc_calc_out.base_lum;
	next_roi_lum = misc_calc_out.roi_lum;
	//AE_LOGI("AESPRD-whole:next_cur_lum:%d,next_base_lum:%d",misc_calc_out.cur_lum,misc_calc_out.base_lum);
	//AE_LOGE("AESPRD-ROI:next_roi_lum:%d",misc_calc_out.roi_lum);
	deltaLUM_0 = next_lum_0 - current_lum_0;
	deltaLUM_1 = next_lum_1 - current_lum_1;
	deltaROIlum = next_roi_lum - current_roi_lum;
	AE_LOGI("AESPRD-ROI:deltaROIlum:%d",deltaROIlum);
	if (AE_SUCCESS == rtn) {
		cxt->valid_stat = 1;
		cxt->cur_result = misc_calc_out;
...
DROP_STAT:

	{
        ...
        misc_calc_in.win1_weight = cxt->win1_weight;
        misc_calc_in.win2_weight = cxt->win2_weight;
		cxt->prv_lum = cxt->cur_lum_4tc;
		rtn = _get_average_lum(cxt,&misc_calc_in,&cur_lum_4_tc);
		if (AE_SUCCESS == rtn) {
                AE_LOGE("cur_lum is :%d!\n", cur_lum_4_tc);
        }
		deltaAVGlum = cur_lum_4_tc - cxt->prv_lum;
		cxt->cur_lum_4tc = cur_lum_4_tc;
		AE_LOGD("delta_avg_lum is %d\n",deltaAVGlum);
		//AE_LOGD("cur_lum and prv_lum is %d,%d\n",cur_lum_4_tc,cxt->prv_lum);
	}
	...
	if (result) {
		...
		result->is_stab = _is_stab(cxt);//current frame
		AE_LOGD("TC0_release_touch_CON %d,%d,%d",deltaAVGlum,cxt->touch_zone_param.enable,cxt->touch_zone_param.tc_status);
		if ((abs(deltaAVGlum) > 4)\
			   && (1 == cxt->touch_zone_param.enable)\
				&& (2 == cxt->touch_zone_param.tc_status)){
			struct ae_trim trim = {0x00};
			trim.x = 0;
			trim.y = 0;
			trim.w = 1;	
			trim.h = 1;
			_set_touch_zone(cxt,&trim);
			cxt->touch_zone_param.tc_status = AE_TOUCH_STATUS_RELEASE;//3    
		}
		//(*cxt->isp_ops.callback)(cxt->isp_ops.isp_handler, AE_CB_TOUCH_AE_NOTIFY, NULL);
		//cxt->touch_zone_param.status = result->is_stab; //last frame
	}
	AE_LOGD("TC0_start_lock_CON %d,%d",cxt->touch_zone_param.tc_status,result->is_stab);
		/*for touch lock*/
		if(1 == cxt->touch_zone_param.tc_status && 1 == result->is_stab){
			AE_LOGD("start lock ae");
			rtn = _set_pause(cxt);
			cxt->touch_zone_param.tc_status = AE_TOUCH_STATUS_ACTIVE_LOCK;//2
		}
	AE_LOGD("TC0_release_lock_CON %d,%d",cxt->touch_zone_param.tc_status,cxt->touch_zone_param.enable);
	if(3 == cxt->touch_zone_param.tc_status && 0 == cxt->touch_zone_param.enable){
		rtn = _set_restore(cxt,result);
		AE_LOGD("start release lock ae");
		cxt->touch_zone_param.tc_status = 0;
	}
...
static int32_t _set_pause(struct ae_ctrl_context *cxt)
{
	...
	if (0 >= cxt->pause_ref_cnt) {
		...
		if (cxt->isp_ops.callback) {
			...
			(*cxt->isp_ops.callback)(cxt->isp_ops.isp_handler, AE_CB_AE_LOCK_NOTIFY, NULL);
		}
	}
...
static int32_t _set_restore(struct ae_ctrl_context *cxt, struct ae_calc_out *result)
{
	...
	if (AE_STATE_LOCKED == cxt->ae_state
		...
		if (cxt->isp_ops.callback) {
			...
			(*cxt->isp_ops.callback)(cxt->isp_ops.isp_handler, AE_CB_AE_UNLOCK_NOTIFY, NULL);
		}
	}
...
if(0 == cxt->cur_param->face_info.param_face_weight && 0 == cxt->cur_param->face_info.param_convergence_speed && 0 == cxt->cur_param->face_info.param_lock_ae && 0 == cxt->cur_param->face_info.param_lock_weight_has_face && 0 == cxt->cur_param->face_info.param_lock_weight_no_face && 0 == cxt->cur_param->face_info.param_shrink_face_ratio){
	cxt->fdae.param_face_weight = 500;
	cxt->fdae.param_convergence_speed = 6;
	cxt->fdae.param_lock_ae = 3;
	cxt->fdae.param_lock_weight_has_face = 10;
	cxt->fdae.param_lock_weight_no_face = 10;
	cxt->fdae.param_shrink_face_ratio = 30;

}else{
	cxt->fdae.param_face_weight = cxt->cur_param->face_info.param_face_weight;
	cxt->fdae.param_convergence_speed = cxt->cur_param->face_info.param_convergence_speed;
	cxt->fdae.param_lock_ae = cxt->cur_param->face_info.param_lock_ae;
	cxt->fdae.param_lock_weight_has_face = cxt->cur_param->face_info.param_lock_weight_has_face;
	cxt->fdae.param_lock_weight_no_face = cxt->cur_param->face_info.param_lock_weight_no_face;
	cxt->fdae.param_shrink_face_ratio = cxt->cur_param->face_info.param_shrink_face_ratio;
}
	// For tuning parameter: read parameter from file
	{
		FILE *fp = fopen("/data/misc/media/fdae_param.txt", "rt");
		if (!fp)
			AE_LOGI("FDAE: failed open fdae_param.txt");
		if (fp) {
			int32_t val0 = 0, val1 = 0, val2 = 0, val3 = 0, val4 = 0;
			fscanf(fp, "%d,%d,%d,%d,%d", &val0, &val1, &val2, &val3, &val4);
			...
			if (val4 > 0) cxt->fdae.param_shrink_face_ratio = val4;
			fclose(fp);
		}
	}
...
static void _fdae_leave_fdae_mode(struct ae_ctrl_context *cxt)
{
	...
	/* to match the touch ae,so change the below type*/
    {
        int16_t weight_mode = cxt->cur_status.weight;
		struct ae_weight_tbl_param wght_tbl = {0x00};
        wght_tbl.flag = 0;
        wght_tbl.wght = &cxt->cur_param->weight_table[weight_mode];
							   ...
							   int32_t *total_face_grid_num)
{
	...
	int32_t shrink_ratio = cxt->fdae.param_shrink_face_ratio;
	int32_t i = 0;
	...
	*total_face_grid_num = 0;
	...
	for (i = 0; i < cxt->fdae.face_info.face_num; i++) {
		...
		/* Shrink the face to ensure the face region does not include background */
		int32_t shrink_x = (f->rect.end_x - f->rect.start_x + 1) * shrink_ratio / 200;
		int32_t shrink_y = (f->rect.end_y - f->rect.start_y + 1) * shrink_ratio / 200;
		
		int32_t start_x = (f->rect.start_x + shrink_x) * win_num_w / frame_w;
		int32_t end_x = (f->rect.end_x - shrink_x) * win_num_w / frame_w;
		int32_t start_y = (f->rect.start_y + shrink_y) * win_num_h / frame_h;
		int32_t end_y = (f->rect.end_y - shrink_y) * win_num_h / frame_h;
...
{
	static const uint32_t PERCENT_THR = 80;
	uint32_t percent = 0;
...
static void _fdae_update_ae_weight_table(struct ae_ctrl_context *cxt)
{
	...
	if (total_face_grid_num > 0) {
		...
		for (j = 0; j < win_num_h; j++) {
			for (i = 0; i < win_num_w; i++) {
				...
				int32_t new_w = smooth_w + base_tbl->weight[idx];
				if (new_w > 255) new_w = 255; // avoid overflow
				work_tbl->weight[idx] = (uint8_t)new_w;
			}
		}
	...
	struct ae_weight_tbl_param wght_tbl = {0x00};
	wght_tbl.flag = 0;
	wght_tbl.wght = &work_tbl->weight;//work_tbl;
	ae_misc_io_ctrl(cxt->misc_handle, AE_MISC_CMD_SET_WEIGHT_TABLE, (void *)&wght_tbl, NULL);
