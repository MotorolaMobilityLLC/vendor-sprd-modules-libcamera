					...
					struct awb_ctrl_calc_result *result)
{
	...
	calc_param.scalar_factor = param->scalar_factor;
					...
					struct awb_ctrl_calc_result *result)
{
	...
	if (AWB_CTRL_FLASH_PRE == cxt->flash_info.flash_mode ||
		AWB_CTRL_FLASH_MAIN == cxt->flash_info.flash_mode) {
		param->quick_mode = 1;
	}
...
*/
awb_ctrl_handle_t awb_sprd_ctrl_init(struct awb_ctrl_init_param *param,
				struct awb_ctrl_init_result *result)
{
	uint32_t rtn = AWB_CTRL_SUCCESS;
...
*@	     others: failed
*/
uint32_t awb_sprd_ctrl_deinit(awb_ctrl_handle_t handle, void *param, void *result)
{
	UNUSED(param);
	UNUSED(result);
	uint32_t rtn = AWB_CTRL_SUCCESS;
...
*@	     others: failed
*/
uint32_t awb_sprd_ctrl_calculation(awb_ctrl_handle_t handle,
...
static uint32_t awb_get_debug_info(struct awb_ctrl_cxt *cxt, void *result)
{
	...
	param->golden_r = cxt->init_param.otp_info.gldn_stat_info.r;
	param->golden_g = cxt->init_param.otp_info.gldn_stat_info.g;
	param->golden_b = cxt->init_param.otp_info.gldn_stat_info.b;
	/* awb calibration of random sensor */
	param->random_r = cxt->init_param.otp_info.rdm_stat_info.r;
	param->random_g = cxt->init_param.otp_info.rdm_stat_info.g;
	param->random_b = cxt->init_param.otp_info.rdm_stat_info.b;
...
static uint32_t awb_get_debug_info_for_display(struct awb_ctrl_cxt *cxt, void *result)
{
	...
	/* awb calibration of golden sensor */
	emParam->golden_r = cxt->init_param.otp_info.gldn_stat_info.r;
	emParam->golden_g = cxt->init_param.otp_info.gldn_stat_info.g;
	emParam->golden_b = cxt->init_param.otp_info.gldn_stat_info.b;
	/* awb calibration of random sensor */
	emParam->random_r = cxt->init_param.otp_info.rdm_stat_info.r;
	emParam->random_g = cxt->init_param.otp_info.rdm_stat_info.g;
	emParam->random_b = cxt->init_param.otp_info.rdm_stat_info.b;
	//cur_bv & cur_iso were worked out before enter this function
	return rtn;
}
...
*@	     others: failed
*/
uint32_t awb_sprd_ctrl_ioctrl(awb_ctrl_handle_t handle, enum awb_ctrl_cmd cmd,
				void *param0, void *param1)
{
	UNUSED(param1);
	uint32_t rtn = AWB_CTRL_SUCCESS;
