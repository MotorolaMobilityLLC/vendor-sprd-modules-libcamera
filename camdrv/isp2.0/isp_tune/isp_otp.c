...
//		}
	...
	if (!ret) {
		isp_calibrationconfigure_save(otp_start_addr, otp_data_len);
	}
#ifdef OTP_DATA_TESTLOG
	for ( j = 0; j < otp_data_len; j++)
	{
...
int write_sensor_shutter(uint32_t shutter_val)
{
	...
	SCI_Trace_Dcam("%s:shutter_val=%d line_time=%d frame_line=%d expsure_line=%ld\n",__func__,
		shutter_val, line_time, frame_line, expsure_line);
...
void isp_otp_needreload(uint32_t *isReload)
{
	if (NULL != isReload) {
		*isReload = reload_on;
	}
