...
#endif
#ifdef WIN32
#include "../kernel_include/sprd_isp_r6p91.h"
#else
...
**				 Micro Define					*
**----------------------------------------------------------------------------*/
#define ISP_SLICE_WIDTH_OVERLAP           60
...
/* [Vendor_32/40] */
#define ISP_RAWAFM_BUF_SIZE   (4*25)
#define ISP_BQ_BIN_CNT                      3
#define ISP_BQ_AEM_CNT                      3
/* [Vendor_33/40] */
#define ISP_BQ_AFM_CNT                      3
...
struct isp_slice_param_v1{
	...
	struct isp_size all_slice_num;
	struct isp_size cur_slice_num;
	struct isp_slice_window cur_slice_window;
...
typedef struct {
	...
	uint32_t isp_bq_fd;
/* [Vendor_34/42] */
	isp_uint isp_af_bq_mem_size;
	isp_uint isp_af_bq_mem_num;
	uint64_t isp_af_bq_k_addr_array[16];
	uint64_t isp_af_bq_u_addr_array[16];
	uint32_t isp_af_bq_fd;
	...
	struct lsc_lib_fun* lsc_lib_fun;
	...
	uint32_t lock_ae;
	/* for D-gain */
	uint32_t global_ob_gain;
	uint32_t global_gain;
	/* for AEM-INT */
	uint32_t aem_irq;
	/* for AEM-data */
	struct isp_ae_grgb_statistic_info aem_statistic;
	/* for non-zsl fast-ae and etc. */
	struct ae_in_out	ae_reserve;
	uint32_t lock_awb_calc;

	uint32_t mode_flag;
	uint32_t scene_flag;
	isp_u16 multi_nr_flag;
	isp_s8 sensor_name[24];
	uint32_t takepicture_mode;
} isp_ctrl_context;
...
struct isp_pre_wavelet_param_v1 {
	...
	isp_u32 cur_level;
	isp_uint *pwd_ptr;
	isp_uint *pwd_scene_ptr;
	isp_u16 scene_flag_verify;
};
...
struct isp_iircnr_yrandom_param {
	...
	isp_uint *iircnr_yrandom_scene_ptr;
	isp_u16 scene_flag_verify;
};
