...
static int32_t ae_set_rgb_gain(isp_handle *isp_handler, double rgb_gain_coeff)
{
	...
	ISP_LOGI("D-gain--global_ob_gain = %d, rgb_gain_coeff = %.4f", handle->global_ob_gain, rgb_gain_coeff);

	isp_u_rgb_gain_block(handle->handle_device, &gain_info);

	handle->global_gain = gain_info.global_gain;
...
static int32_t ae_callback(void *handler, enum ae_cb_type cb_type, void *data)
{
	isp_ctrl_context           *ctrl_context = (isp_ctrl_context*)handler;
	...
	if (NULL != ctrl_context) {
		...
		if (ctrl_context->system.callback) {
			ctrl_context->system.callback(ctrl_context->system.caller_id, ISP_CALLBACK_EVT|cmd, in, 0);
		}
	}
...
static int32_t _ae_init(isp_ctrl_context *handle)
{
	...
	if(handle == NULL) {
		ISP_LOGE("AE_TAG: handl is null error");
		return ISP_ERROR;
	}
	...
	init_in_param.af_enable = (handle->ioctrl_ptr->set_focus != NULL);
...
// simulation info
	if (1)
	{
		...
		rtn = isp_pm_ioctl(handle->handle_pm, ISP_PM_CMD_GET_SINGLE_SETTING, &io_pm_input, &io_pm_output);
		ISP_TRACE_IF_FAIL(rtn, ("pm set gamma error"));
...
uint32_t _alsc_init(isp_ctrl_context* handle)
{
	...
	if(pm_tab_output.param_data != NULL) {
	...
	if(NULL == io_pm_output.param_data) {
		ISP_LOGE("ALSC_TAG: io_pm_output param_data is mull");
		return ISP_ERROR;
	}
	struct isp_lsc_info *lsc_info = (struct isp_lsc_info *)io_pm_output.param_data->data_ptr;

	ISP_LOGI(" _alsc_init_20170807");
...
{
	int32_t	rtn = ISP_SUCCESS;
	if(handle->handle_lsc_adv == NULL){
		ISP_LOGI("stop _alsc_calc after deinit");
		return rtn;
	}
...
if( lsc_ver.LSC_SPD_VERSION >= 2 ){
	...
	struct isp_lsc_info *lsc_info = (struct isp_lsc_info *)io_pm_output.param_data->data_ptr;
	struct isp_2d_lsc_param * lsc_tab_param_ptr =  (struct isp_2d_lsc_param *)(handle->lsc_tab_address);
	if(lsc_info == NULL || lsc_tab_param_ptr == NULL) {
		ALOGE("invalid lsc param");
		return ISP_ERROR;
	}
...
static int32_t _isp_change_lsc_param(isp_handle isp_handler, uint32_t ct)
{
	...
	if(output.param_data == NULL) {
		ISP_LOGI("ISP_LSC: output is null");
		return ISP_ERROR;
	}
...
static int32_t _ispTransBuffAddr(isp_handle isp_handler)
{
	...
	isp_u_2d_lsc_transaddr(handle->handle_device, handle->isp_lsc_fd);

	isp_u_anti_flicker_transaddr(handle->handle_device, handle->isp_anti_flicker_fd);
...
static int32_t _ispMemfree(isp_handle  handle)
{
	int32_t                         rtn = ISP_SUCCESS;

	rtn = isp_dev_mem_free(handle);
	ISP_RETURN_IF_FAIL(rtn, ("_ispMemfree error"));
...
//oem will send is HDR flag  by this function,
static cmr_int _ispHdrFlagIOCtrl(isp_handle isp_handler, void *param_ptr, int (*call_back)()){
    cmr_int rtn = ISP_SUCCESS;
    isp_ctrl_context                *ctrl_context = (isp_ctrl_context *)isp_handler;
    cmr_u32 *hdr_flag = (cmr_u32*)param_ptr;
    UNUSED(call_back);
    ISP_LOGE("hdr_flag :%d",*hdr_flag);
    return rtn;
}
...
static int32_t _ispToolSetSceneParam(isp_handle isp_handler, void *param_ptr, int(*call_back)())
{
	...
	handle->takepicture_mode = CAMERA_ISP_SIMULATION_MODE;
...
static struct isp_io_ctrl_fun _s_isp_io_ctrl_fun_tab[] = {
	...
	{ISP_CTRL_SET_HDR_FLAG,                       _ispHdrFlagIOCtrl},
...
static int _isp_video_start(isp_handle isp_handler, struct isp_video_start *param_ptr)
{
	...
	handle->isp_lsc_fd = param_ptr->lsc_fd;
...
/* [Vendor_26/40] */
		...
		if (handle->cb_of_malloc) {
			...
			cb_malloc(type, &handle->isp_bq_mem_size, &handle->isp_bq_mem_num,
					(isp_uint *)handle->isp_bq_k_addr_array,(isp_uint *)handle->isp_bq_u_addr_array,
					(isp_s32 *)handle->isp_bq_fd, handle->buffer_client_data);
/* [Vendor_27/40] */
			cb_malloc(CAMERA_ISP_RAWAFM_FLAG, &handle->isp_af_bq_mem_size, &handle->isp_af_bq_mem_num,
					(isp_uint *)handle->isp_af_bq_k_addr_array,(isp_uint *)handle->isp_af_bq_u_addr_array,
					(isp_s32 *)handle->isp_af_bq_fd, handle->buffer_client_data);
		} else {
	...
	handle->isp_anti_flicker_fd = param_ptr->anti_flicker_fd;
...
static int _isp_video_stop(isp_handle isp_handler)
{
	...
	rtn = _ispMemfree(handle->handle_device);
	ISP_RETURN_IF_FAIL(rtn, ("isp Mem free error"));
...
static int _isp_proc_start(isp_handle isp_handler, struct ips_in_param* param_ptr)
{
	...
	if( SENSOR_MULTI_MODE_FLAG == handle->multi_nr_flag) {
		...
		if (org_size.w != handle->src.w) {
			...
			rtn = isp_pm_ioctl(handle->handle_pm, ISP_PM_CMD_SET_MODE, &handle->isp_mode, NULL);
			ISP_RETURN_IF_FAIL(rtn, ("isp capture set isp mode error"));
		}
	}
	...
	if (CAMERA_ISP_SIMULATION_MODE != handle->takepicture_mode) {
		rtn = _ispChangeAwbGain(handle);
		ISP_RETURN_IF_FAIL(rtn, ("isp change awb gain error"));
	...
	handle->takepicture_mode = CAMERA_NORMAL_MODE;
	return rtn;
}
...
static cmr_int  _isp_ctrl_routine(struct cmr_msg *message, void *client_data)
{
	...
	case ISP_CTRL_EVT_SOF:
		...
		if (0 < handle->aem_irq){
			...
			rtn = cmr_thread_msg_send(isp_system_ptr->thread_proc, &msg);
			ISP_RETURN_IF_FAIL(rtn, ("cmc thread msg send error"));
		}else{
			;
		}
#endif
	...
	case ISP_CTRL_EVT_AEM2:
	{
		if (handle->ae_lib_fun->product_id) {
			...
			rtn = isp_pm_ioctl(handle->handle_pm,
				...
				(void*)&input,
				(void*)&output);
			if (ISP_SUCCESS == rtn && (output.param_data != NULL)) {
				if(output.param_data->data_ptr != NULL)
...
static isp_int _isp_lsc_routine(struct cmr_msg *message, void *client_data)
{
	...
	if(handle->handle_lsc_adv == NULL){
		ISP_LOGI("stop _isp_lsc_routine after deinit");
		return rtn;
	}
...
*/
	...
	case ISP_PROC_EVT_AE:
	{
		...
		if (ISP_SUCCESS == ae_rtn) {
			...
			/*enable smart & alsc*/
			gCntSendMsgLsc++;
			if ((1 == handle->isp_smart_eb) && (0 == gCntSendMsgLsc % 1)) {
				/*smart lsc calc*/
	...
	{
		struct isp_aem_info aem_info = {0, NULL, NULL};
		struct isp_ae_grgb_statistic_info* stat_ptr = NULL;
		struct isp_pm_param_data param_data;
		struct isp_pm_ioctl_input input = {NULL, 0};
		struct isp_pm_ioctl_output output = {NULL, 0};
		struct awb_size stat_img_size = {0};
			...
			if (1 == handle->isp_smart_eb) {
				...
				if(0 == gCntSendMsgLsc % 1){
					...
					if(param_data_alsc_output.param_data == NULL) {
						ISP_LOGI("ISP_AWB: param_data_alsc_output is null error");
						return ISP_ERROR;
					}
					handle->lsc_tab_address = param_data_alsc_output.param_data->data_ptr;
					handle->lsc_tab_size = param_data_alsc_output.param_data->data_size;
	...
	if(handle->isp_bq_alloc_flag == 1) {
		...
		if (handle->cb_of_free) {
			...
			cb_free(type,(isp_uint *)handle->isp_bq_k_addr_array,(isp_uint *)handle->isp_bq_u_addr_array,
					(isp_s32 *)handle->isp_bq_fd, handle->isp_bq_mem_num, handle->buffer_client_data);

/* [Vendor_30/40] */
			cb_free(CAMERA_ISP_RAWAFM_FLAG,(isp_uint *)handle->isp_af_bq_k_addr_array,(isp_uint *)handle->isp_af_bq_u_addr_array,
					(isp_s32 *)handle->isp_af_bq_fd, handle->isp_af_bq_mem_num, handle->buffer_client_data);
