...
/* [Vendor_31/40] */
#define CAMERA_ISP_RAWAFM_FLAG 12 // buffer queue flag, sync with hal
...
enum isp_callback_cmd {
	...
	ISP_AE_CB_FLASH_FIRED = 0x00000F00,
...
enum isp_ctrl_cmd {
	...
	ISP_CTRL_EM_GET_PARAM,
	ISP_CTRL_SET_CAPTURE_RAW_MODE,
	ISP_CTRL_GET_RGB_GAIN, 
	ISP_CTRL_START_HDR,//for new 3A hdr
	ISP_CTRL_STOP_HDR,//for new 3A hdr
	ISP_CTRL_GET_HDR_TIMESTAMP,//for new 3A hdr 
	ISP_CTRL_TOOL_SIMULATOR_GET_LSC_ADDR,
	ISP_CTRL_SET_HDR_FLAG,
	ISP_CTRL_MAX
};
...
typedef isp_uint (*isp_cb_of_malloc)(isp_uint type, isp_uint *size_ptr, isp_uint *sum_ptr, isp_uint *phy_addr,
		isp_uint *vir_addr, isp_s32 *mfd,void* private_data);
typedef isp_uint (*isp_cb_of_free)(isp_uint type, isp_uint *phy_addr, isp_uint *vir_addr, isp_s32 *mfd,
		isp_uint sum, void* private_data);

struct ips_in_param {
	struct isp_img_frm src_frame;
	uint32_t src_avail_height;
	uint32_t src_slice_height;
	uint32_t src_slice_width;
...
struct isp_hdr_ev_param {
	uint64_t level;
	uint32_t skip_frame_num; //return from isp
};

struct isp_fix_param_mode_info {
    uint32_t version_id;
    uint32_t mode_id;
    uint32_t width;
    uint32_t height;
...
struct lsc_isp_addr {
	uint32_t lsc_phys_addr;
	uint32_t lsc_vir_addr;
	uint32_t lsc_buf_num;
	uint32_t lsc_buf_size;
};
