...
struct lsc2_tune_param{     // if modified, please contact to TOOL team
	...
	// alsc2_param
	unsigned int lambda_r;    // quick_in_param_flag, new flash path (quick in 3 frame when status changes.)
	unsigned int lambda_b;    // front_cam_slsc2_only, front cam apply slcs2 only
	unsigned int weight_r;	  // post gain low light flag
...
struct lsc2_context{
	...
	unsigned int hdr_mode;               // HDR signal
	...
	void* alsc2_param;
	void* lsc1d_param;
	void* post_shading_gain_param; //CY
