...
isp_s32 isp_get_fetch_addr_v1(struct isp_interface_param_v1 *isp_context_ptr, struct isp_dev_fetch_info_v1 *fetch_ptr)
{
	isp_s32 rtn = ISP_SUCCESS;
	struct isp_size *cur_slice_ptr = &isp_context_ptr->slice.cur_slice_num;
	struct isp_slice_window *cur_slice_win = &isp_context_ptr->slice.cur_slice_window;
	isp_u32 ch0_offset = ISP_ZERO;
	isp_u32 ch1_offset = ISP_ZERO;
	isp_u32 ch2_offset = ISP_ZERO;
	isp_u32 slice_w_offset = ISP_ZERO;
	...
	switch (fetch_ptr->color_format) {
	case ISP_FETCH_YUV422_3FRAME:
		ch0_offset = cur_slice_win->start_row * fetch_ptr->pitch.chn0 + cur_slice_win->start_col;
		ch1_offset = cur_slice_win->start_row * fetch_ptr->pitch.chn1 + ((cur_slice_win->start_col + 1) >> 1);
		ch2_offset = cur_slice_win->start_row * fetch_ptr->pitch.chn2 + ((cur_slice_win->start_col + 1) >> 1);
		break;
	case ISP_FETCH_YUV422_2FRAME:
	case ISP_FETCH_YVU422_2FRAME:
		ch0_offset = cur_slice_win->start_row * fetch_ptr->pitch.chn0 + cur_slice_win->start_col;
		ch1_offset = cur_slice_win->start_row * fetch_ptr->pitch.chn1 + cur_slice_win->start_col;
		break;
	case ISP_FETCH_YUYV:
	case ISP_FETCH_UYVY:
	case ISP_FETCH_YVYU:
	case ISP_FETCH_VYUY:
	case ISP_FETCH_NORMAL_RAW10:
		ch0_offset = cur_slice_win->start_row * fetch_ptr->pitch.chn0 + cur_slice_win->start_col * 2;
		break;
	case ISP_FETCH_CSI2_RAW10:
		ch0_offset = cur_slice_win->start_row * fetch_ptr->pitch.chn0 + (cur_slice_win->start_col >> 2) * 5 +(cur_slice_win->start_col & 0x3);
		break;
	default:
		break;
	}
	...
	start_col = cur_slice_win->start_col;
	end_col = start_col + fetch_width - ISP_ONE;
...
isp_s32 isp_get_store_addr(struct isp_interface_param *isp_context_ptr, struct isp_dev_store_info *store_ptr)
{
	...
	store_ptr->subtract = isp_context_ptr->store.subtract;
...
isp_s32 isp_get_store_addr_v1(struct isp_interface_param_v1 *isp_context_ptr, struct isp_dev_store_info_v1 *store_ptr)
{
	...
	isp_u32 ch1_offset = ISP_ZERO;
	isp_u32 ch2_offset = ISP_ZERO;
	isp_u16 slice_width = isp_context_ptr->data.slice_width;
	...
	store_ptr->border.up_border = isp_context_ptr->store.border.up_border;
	store_ptr->border.down_border = isp_context_ptr->store.border.down_border;
	store_ptr->border.left_border = isp_context_ptr->store.border.left_border;
	store_ptr->border.right_border = isp_context_ptr->store.border.right_border;

	switch (isp_context_ptr->com.store_color_format) {
	case ISP_STORE_YUV422_3FRAME:
	case ISP_STORE_YUV420_3FRAME:
		ch0_offset = cur_slice_ptr->w *(cur_slice_win->start_col + ISP_SLICE_WIDTH_OVERLAP);
		ch1_offset = cur_slice_ptr->w *(cur_slice_win->start_col + ISP_SLICE_WIDTH_OVERLAP) >> 0x01;
		ch2_offset = cur_slice_ptr->w *(cur_slice_win->start_col + ISP_SLICE_WIDTH_OVERLAP) >> 0x01;
		break;
	case ISP_STORE_YUV422_2FRAME:
	case ISP_STORE_YVU422_2FRAME:
	case ISP_STORE_YUV420_2FRAME:
	case ISP_STORE_YVU420_2FRAME:
		ch0_offset = cur_slice_ptr->w *(cur_slice_win->start_col + ISP_SLICE_WIDTH_OVERLAP);
		ch1_offset = cur_slice_ptr->w *(cur_slice_win->start_col + ISP_SLICE_WIDTH_OVERLAP);
		break;
	case ISP_STORE_UYVY:
		ch0_offset = cur_slice_ptr->w *(cur_slice_win->start_col + ISP_SLICE_WIDTH_OVERLAP) << 0x01;
		break;
	default:
		break;
	}
...
isp_s32 isp_set_store_param(isp_handle isp_handler)
{
	...
	store_param_ptr->subtract = ISP_ONE;
...
isp_s32 isp_set_store_param_v1(isp_handle isp_handler)
{
	...
	store_param_ptr->subtract = 0x0; /* pike2 set 0 */
	...
	isp_get_store_pitch(&(store_param_ptr->pitch), isp_context_ptr->data.input_size.w,
		isp_context_ptr->data.output_format);
	store_param_ptr->border.up_border = ISP_ZERO;
	store_param_ptr->border.down_border = ISP_ZERO;
	store_param_ptr->border.left_border = ISP_ZERO;
	store_param_ptr->border.right_border = ISP_ZERO;
...
isp_s32 isp_set_store_proc_param_v1(isp_handle isp_handler)
{
	...
	store_param_ptr->border.down_border = ISP_ZERO;
	store_param_ptr->border.left_border = ISP_ZERO;
	store_param_ptr->border.right_border = ISP_SLICE_WIDTH_OVERLAP;

	isp_context_ptr->slice.size[ISP_STORE_V1].w = isp_context_ptr->data.slice_width;
	isp_context_ptr->slice.size[ISP_STORE_V1].h = isp_context_ptr->data.input_size.h;
...
isp_s32 isp_set_store_next_param_v1(isp_handle isp_handler)
{
	...
	store_param_ptr->bypass = 0;
	store_param_ptr->subtract = 0x0;//0x1; for fpga
	store_param_ptr->color_format = isp_get_store_format(isp_context_ptr->data.output_format);
	store_param_ptr->addr.chn0 = isp_context_ptr->data.output_addr.chn0;
	store_param_ptr->addr.chn1 = isp_context_ptr->data.output_addr.chn1;
	store_param_ptr->addr.chn2 = isp_context_ptr->data.output_addr.chn2;
	...
	store_param_ptr->border.up_border = ISP_ZERO;
	store_param_ptr->border.down_border = ISP_ZERO;
	store_param_ptr->border.left_border = ISP_SLICE_WIDTH_OVERLAP;
	store_param_ptr->border.right_border = ISP_ZERO;
...
isp_s32 isp_set_slice_size_v1(isp_handle isp_handler)
{
	...
	isp_s32 start_col, start_row, end_col, end_row;
	isp_s32 i = 0;

	start_col = isp_slice_ptr->cur_slice_window.start_col;
	start_row = isp_slice_ptr->cur_slice_window.start_row;
	end_col = isp_slice_ptr->cur_slice_window.end_col;
	end_row = isp_slice_ptr->cur_slice_window.end_row;

	for (i = 0; i < ISP_SLICE_TYPE_MAX_V1; i++) {
		isp_slice_ptr->size[i].w = end_col - start_col + 1;
		isp_slice_ptr->size[i].h = end_row -start_row +1;
	}
...
isp_s32 isp_cfg_slice_size_v1(isp_handle handle, struct isp_slice_param_v1 *slice_ptr)
{
	...
	rtn = isp_u_cfa_slice_size(handle, slice_ptr->size[ISP_FETCH_V1].w, slice_ptr->size[ISP_FETCH_V1].h);
	ISP_RETURN_IF_FAIL(rtn, ("isp cfa slice size error"));
	...
	isp_u_yuv_postcdn_slice_size(handle, slice_ptr->cur_slice_window.start_col);
