...
static uint32_t Is_ae_stable(af_ctrl_t *af){
	...
	return af->ae.stable || (af->ae.gain_index>=GAIN_16x ? 1:0);
	...
	if( 1!=af->af_tuning_data.flag || 1!=af->win_config->win_strategic ){//default window config
		...
		#else
		{
			...
			default:
				...
				sum += (1==af->touch)? (spsmd[0]) : (0);
				fv[T_SPSMD] = sum;
			break;
			}
			AF_LOGD("[%d][%d]spsmd sum %lld Touch %d",af->state,af->roi.num, sum,af->touch);
			/*AF_LOGD("sx %d sy %d ex %d ey %d area %d x %d"
				,af->roi.win[0].start_x
				,af->roi.win[0].start_y
				,af->roi.win[0].end_x
				,af->roi.win[0].end_y
				,af->roi.win[0].end_x-af->roi.win[0].start_x
				,af->roi.win[0].end_y-af->roi.win[0].start_y);*/
...
static ERRCODE if_get_motor_pos(uint16 *motor_pos, void *cookie)
{
	...
	if( NULL!=af->vcm_ops.get_motor_pos ){
		...
		*motor_pos = (uint16)lens_get_pos(af);
	}
...
static void set_ae_gain_exp(af_ctrl_t *af,char* ae_param){
	...
	struct sensor_exposure ex_exposure;
...
static int32_t image_data_update(isp_ctrl_context *isp)
{
    ...
    isp_pm_ioctl(af->isp_ctx->handle_pm, ISP_PM_CMD_GET_SINGLE_SETTING, &in, &out);
    if (out.param_data && out.param_data->data_ptr)
    {
