...
#define FD_ANGLE_NONE                                                          \
    0x00 /* Each face direction cover 0 degreess, which means not to do face   \
            detection */
#define FD_ANGLE_RANGE_30                                                      \
    0x01 /* Each face direction cover 30 degrees:[-15,+15] */
#define FD_ANGLE_RANGE_90                                                      \
    0x02 /* Each face direction cover 90 degrees:[-45,+45] */
...
typedef struct {
    ...
    unsigned int minFaceSize; /* Minimum face size to detect.(In [20, 8192],
                                 default:20)     */
    unsigned int maxFaceSize; /* Maximum face size to detect.(In [20, 8192],
                                 default:8192)   */
    unsigned int
        directions; /* Face directions to detect.(default: FD_DIRECTION_0)    */
    unsigned int angleFrontal; /* Frontal face: angle range for each direction.
                                  (default: FD_ANGLE_RANGE_90) */
    unsigned int angleHalfProfile; /* Half profile face: angle range for each
    ...
    unsigned int initFrames;   /* Divide the initial face searching task into N
                                  frames; (In [1, 3])                    */
    unsigned int detectFrames; /* Divide the new face searching task into N
                                  frames; (In [1, 3])                        */
    unsigned int
        detectInterval; /* The frame intervals between the searches for new
                           faces during tracking; (In [0, 50]) */
    unsigned int trackDensity; /* Face tracking density.(In [3, 10]) */
    unsigned int
        lostRetryCount; /* The number of frames to try to search a face that was
                           lost during tracking. (In [0, 10]) */
    unsigned int lostHoldCount;    /* The number of frames to keep outputting a
                                      lost face. (In [0, 10]);                   */
    unsigned int holdPositionRate; /* If the displacement rate during tracking
                                      is below the rate, the face center will be
                                      corrected back to the previous one. (In
                                      [0, 20]) */
    unsigned int
        holdSizeRate; /* If the size change during tracking is below the rate,
                         the face size will be corrected back to the previous
                         one. (In [0, 30]) */
    unsigned int
        swapFaceRate; /* When the detected face count is larger than
                         "maxFaceNum", only if the new face is larger than the
                         old face by the rate, the old face is replaced by the
                         new face. */
    unsigned int guessFaceDirection; /* 1-->TRUE; 0 --> FALSE; If set as TRUE,
                                        new face search will only be performed
                                        on the guessed directions, which can
...
FDAPI(int)
FdGetFaceInfo(const FD_DETECTOR_HANDLE hDT, int faceIndex,
              FD_FACEINFO *faceInfo);
