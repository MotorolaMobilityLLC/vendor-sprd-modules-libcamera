...
int blend_trigger(c3dnr_buffer_t *pfirst_blendimg , c3dnr_buffer_t  *psecond_blendimg, c3dnr_buffer_t *pblend_out, int mv_y, int mv_x, int blending_no)
{	
	...
	if (NULL != p3dnr_info->isp_ioctrl)
		//ret = p3dnr_info->isp_ioctrl(p3dnr_info->isp_handle, ISP_CTRL_POST_3DNR, (void *)&isp_3dnr);
...
__attribute__ ((visibility("default"))) int threednr_function_pre(c3dnr_buffer_t *small_image,c3dnr_buffer_t *orig_image)//fix on 5.20,struct to struct pointer
{
	int i,ret=0;
	c3dnr_buffer_t* pblend_out , *pfirst_blendimg , *psecond_blendimg;
	FILE *fp;
	struct tm *pt;
	time_t t;
	...
	int8_t mv_xy[2];
#ifdef SAVE_IMG
	static c3dnr_buffer_t temp_first;
	static c3dnr_buffer_t temp_second;
	static c3dnr_buffer_t temp_out;
	FILE *file;
	char filename[128];
#endif
	{
...

	{
		...
		{
			BL_LOGE("the width of input images is %d,the height of input images is %d,is lager than values of threednr_init function\n" , width,height);
			return -1;
		}
		//
	...
	{//save the preview frame for algo team.
		...
		if(!strcmp(save_flag , "yes")) {
			if (g_cur_num < DUMP_IMG_NUM) {
				...
				memcpy(g_buf_blend_pre_pos, orig_image->bufferY, p3dnr_info->orig_width * p3dnr_info->orig_height * 3 / 2);
	...
	if (1 == p3dnr_info->is_first_frame) {
		...
		memcpy(p3dnr_info->out_img[0].bufferY, orig_image->bufferY, p3dnr_info->orig_width * p3dnr_info->orig_height * 3 / 2);
	} else {
		//calculate mv
		if (1 == p3dnr_info->curr_frameno) {
			ProjectMatching(p3dnr_info->xProj1D[0] , p3dnr_info->yProj1D[0] , 
				 p3dnr_info->xProj1D[1], p3dnr_info->yProj1D[1] ,
				width, height, mv_xy, 2, p3dnr_info->extra);
		} else {			
			ProjectMatching(p3dnr_info->xProj1D[1], p3dnr_info->yProj1D[1],
				p3dnr_info->xProj1D[0] , p3dnr_info->yProj1D[0],	 
				width, height, mv_xy, 2, p3dnr_info->extra);
		}
		BL_LOGI("-----------ProjectMatching framno:%d,%d , mv:%d,%d\n" , 1 - p3dnr_info->curr_frameno, p3dnr_info->curr_frameno , mv_xy[0],mv_xy[1]);
		...
		blend_trigger(p3dnr_info->pfirst_blendimg, p3dnr_info->psecond_blendimg, p3dnr_info->pout_blendimg, mv_xy[0], mv_xy[1], p3dnr_info->blend_num + 10);
		...
		BL_LOGI("1blend trigger framno:%d , firstbuff:%p , secondbuff:%p , outbuff:%p\n" ,p3dnr_info->curr_frameno, p3dnr_info->pfirst_blendimg->bufferY , p3dnr_info->psecond_blendimg->bufferY , p3dnr_info->pout_blendimg->bufferY);
		...
		{//save the preview frame for algo team.
			property_get("save_3dnr_preview_data" , save_flag , "no");
				...
				if (g_cur_num < DUMP_IMG_NUM) {
					...
					memcpy(g_buf_blend_post_pos, p3dnr_info->pout_blendimg->bufferY, img_size);
					g_buf_blend_post_pos += img_size;
					BL_LOGI("end to save the preview data after blending. num: %d.", g_cur_num);
					//g_cur_num++;
				} else if (0 == g_save_file) {
					FILE *fp = NULL;
					int size = p3dnr_info->orig_width * p3dnr_info->orig_height * 3 / 2 * DUMP_IMG_NUM;
					...
					sprintf(filename , "/data/misc/cameraserver/%dx%d_img_preview_after_blend_%d.yuv",
						p3dnr_info->orig_width , p3dnr_info->orig_height, DUMP_IMG_NUM - 1);
					if (NULL != (fp = fopen(filename, "wb"))) {
						fwrite(g_buf_blend_post, 1, p3dnr_info->orig_width * p3dnr_info->orig_height * 3 / 2 * (DUMP_IMG_NUM - 1), fp);
						fclose(fp);
					} else {
						BL_LOGE("Fail to open file : %s.", filename);
					}
		...
		{//save the preview frame for debug.
			...
			property_get("save_preview_data" , save_flag , "no");
		                ...
		                if(num < 10)
		                {
		                        sprintf(filename , "/data/misc/cameraserver/%dx%d_ref_img_index_%d.yuv" , p3dnr_info->orig_width , p3dnr_info->orig_height, num);
		                        save_yuv(filename , (char*)p3dnr_info->pfirst_blendimg->bufferY, p3dnr_info->orig_width , p3dnr_info->orig_height, p3dnr_info->orig_height);
		                        sprintf(filename , "/data/misc/cameraserver/%dx%d_cur_img_index_%d.yuv" , p3dnr_info->orig_width , p3dnr_info->orig_height, num);
		                        save_yuv(filename , (char*)p3dnr_info->psecond_blendimg->bufferY, p3dnr_info->orig_width , p3dnr_info->orig_height, p3dnr_info->orig_height);
		                        sprintf(filename , "/data/misc/cameraserver/%dx%d_blend_img_index_%d.yuv" , p3dnr_info->orig_width , p3dnr_info->orig_height, num);
		                        save_yuv(filename , (char*)p3dnr_info->pout_blendimg->bufferY, p3dnr_info->orig_width , p3dnr_info->orig_height, p3dnr_info->orig_height);
					
		                }
				num++;
			}
		}
		BL_LOGI("size: %dx%d, dst: 0x%x, src: 0x%x", p3dnr_info->orig_width, p3dnr_info->orig_height, orig_image->bufferY, p3dnr_info->pout_blendimg->bufferY);
		memcpy(orig_image->bufferY, p3dnr_info->pout_blendimg->bufferY, p3dnr_info->orig_width * p3dnr_info->orig_height * 3 / 2);
		BL_LOGI("copy done.");
		{//save the preview frame for debug.
			char flag[8] = {0};
			static int num0 = 0;
			property_get("save_preview_data" , save_flag , "no");
		                ...
		                if(num0 < 10)
		                {
		                        sprintf(filename , "/data/misc/cameraserver/%dx%d_pre_img_index_%d.yuv" , p3dnr_info->orig_width , p3dnr_info->orig_height, num0);
		                        save_yuv(filename , (char*)orig_image->bufferY, p3dnr_info->orig_width , p3dnr_info->orig_height, p3dnr_info->orig_height);
		                }
				num0++;
			}
		}	
...
__attribute__ ((visibility("default"))) int threednr_function(c3dnr_buffer_t *small_image,c3dnr_buffer_t *orig_image)//fix on 5.20,struct to struct pointer
{
...
#ifdef SAVE_IMG
	{//wxz: save input image.
			property_get(SAVE_IMG_PROPERTY , save_flag , "no");
        	        if(!strcmp(save_flag , "yes"))
                        {
                                sprintf(filename , "/data/misc/cameraserver/%dx%d_big_img_index_%d.yuv" , p3dnr_info->orig_width , IMG_SAVE_H/*p3dnr_info->orig_height*/,p3dnr_info->curr_frameno);
                                save_yuv(filename , (char*)orig_image->bufferY , p3dnr_info->orig_width , p3dnr_info->orig_height, IMG_SAVE_H);
                                sprintf(filename , "/data/misc/cameraserver/%dx%d_small_img_index_%d.yuv" , p3dnr_info->small_width , IMG_SAVE_H/*p3dnr_info->orig_height*/ ,p3dnr_info->curr_frameno);
                                save_yuv(filename , (char*)small_image->bufferY , p3dnr_info->small_width , p3dnr_info->small_height, IMG_SAVE_H);
                        }
	}
#endif
	...
	//1d process
	threeDNR_1D_process(small_image , width , height , p3dnr_info->curr_frameno);
	BL_LOGI("------------threeDNR_1D_process frameno:%d , width:%d , height:%d , smallimage:%x,%x,%x\n" , p3dnr_info->curr_frameno , width , height , small_image->bufferY[0],small_image->bufferY[1] ,small_image->bufferY[2]);
		
	buffer_copy(&p3dnr_info->porigimg[p3dnr_info->curr_frameno], orig_image);	
	if(p3dnr_info->curr_frameno > 0)
	{
		//calculate mv
		ProjectMatching(p3dnr_info->xProj1D[p3dnr_info->curr_frameno - 1] , p3dnr_info->yProj1D[p3dnr_info->curr_frameno -1] , 
				 p3dnr_info->xProj1D[p3dnr_info->curr_frameno], p3dnr_info->yProj1D[p3dnr_info->curr_frameno] ,
				width, height, mv_xy, 2, p3dnr_info->extra);
		BL_LOGI("-----------ProjectMatching framno:%d,%d , mv:%d,%d\n" , p3dnr_info->curr_frameno - 1 , p3dnr_info->curr_frameno , mv_xy[0],mv_xy[1]);
	...
	{//wxz: save blend image.
			property_get(SAVE_IMG_PROPERTY , save_flag , "no");
				...
				if(((p3dnr_info->curr_frameno-1) == 1)) {
					...
					buf = &p3dnr_info->porigimg[p3dnr_info->curr_frameno-2];
                                ...
                                sprintf(filename , "/data/misc/cameraserver/%dx%d_blend_img_index_%d.yuv" , p3dnr_info->orig_width , IMG_SAVE_H/*p3dnr_info->orig_height*/,p3dnr_info->curr_frameno-1);
                                save_yuv(filename , (char*)buf->bufferY , p3dnr_info->orig_width , p3dnr_info->orig_height, IMG_SAVE_H);
                        }
	}
#endif
		...
		blend_trigger(p3dnr_info->pfirst_blendimg, p3dnr_info->psecond_blendimg, p3dnr_info->pout_blendimg, mv_xy[0], mv_xy[1], p3dnr_info->curr_frameno - 1);
		BL_LOGI("1blend trigger framno:%d , firstbuff:%p , secondbuff:%p , outbuff:%p\n" ,p3dnr_info->curr_frameno, p3dnr_info->pfirst_blendimg->bufferY , p3dnr_info->psecond_blendimg->bufferY , p3dnr_info->pout_blendimg->bufferY);
#ifdef SAVE_IMG
	{//wxz: save blend image.
			property_get(SAVE_IMG_PROPERTY , save_flag , "no");
        	        if(!strcmp(save_flag , "yes"))
                        {
                                sprintf(filename , "/data/misc/cameraserver/%dx%d_ref_img_index_%d.yuv" , p3dnr_info->orig_width , IMG_SAVE_H/*p3dnr_info->orig_height*/,p3dnr_info->curr_frameno-1);
                                save_yuv(filename , (char*)p3dnr_info->pfirst_blendimg->bufferY , p3dnr_info->orig_width , p3dnr_info->orig_height, IMG_SAVE_H);
                                sprintf(filename , "/data/misc/cameraserver/%dx%d_cur_img_index_%d.yuv" , p3dnr_info->orig_width , IMG_SAVE_H/*p3dnr_info->orig_height*/,p3dnr_info->curr_frameno-1);
                                save_yuv(filename , (char*)p3dnr_info->psecond_blendimg->bufferY , p3dnr_info->orig_width , p3dnr_info->orig_height, IMG_SAVE_H);
                        }
	}
#endif
		...
		if(p3dnr_info->curr_frameno == (p3dnr_info->imageNum - 1))
		{
			...
			if (2 == p3dnr_info->imageNum) {
				memcpy((char*)p3dnr_info->porigimg[0].bufferY, (char*)p3dnr_info->pdst_img.bufferY, p3dnr_info->orig_width * p3dnr_info->orig_height * 3 / 2);
			}
			...
			property_get(SAVE_IMG_PROPERTY , save_flag , "no");
                        if(!strcmp(save_flag , "yes"))
                        {
                                sprintf(filename , "/data/misc/cameraserver/%dx%d_blend_img_index_%d.yuv" , p3dnr_info->orig_width , IMG_SAVE_H/*p3dnr_info->orig_height*/,p3dnr_info->curr_frameno);
                                save_yuv(filename , (char*)p3dnr_info->porigimg[0].bufferY , p3dnr_info->orig_width , p3dnr_info->orig_height, IMG_SAVE_H);
				BL_LOGI("save_yuv last buf: 0x%x.", p3dnr_info->porigimg[0].bufferY);
                        }
#endif
			p3dnr_info->status = C3DNR_STATUS_IDLE;
...
void threeDNR_1D_process(c3dnr_buffer_t *curr_image ,int32_t width ,int32_t height , int32_t fram_no)
{
		int i;
	int8_t mv_xy[2];
	FILE *fp;
	...
	if(T3DNR_CAPTURE_MODE_CAPTURE_STAGE != p3dnr_info->operation_mode)
	{
		if(T3DNR_FULLSIZE_MODE_FUSE_STAGE != p3dnr_info->operation_mode) 
		{
			...
			IntegralProjection1D_process(curr_image->bufferY, width, height, p3dnr_info->xProj1D[fram_no], p3dnr_info->yProj1D[fram_no], p3dnr_info->extra);
			if(!(strcmp(timeout,"yes")))
			{
...
int get_weight(char *weight,int32_t imageNum)
{
	char *ptr=NULL;
	int i;
	char *str=weight;
	...
	for(i=0;i<imageNum-1;i++)
	{
		...
		if(!strcmp(debug,"yes"))
			BL_LOGI("the %dth frame value:%d,%d,%d\n",i,p3dnr_info->y_src_weight[i],p3dnr_info->u_src_weight[i],p3dnr_info->v_src_weight[i]);
...
int initModule(int32_t small_width,int32_t small_height,int32_t orig_width , int32_t orig_height , int32_t imageNum)
{
	int i,ret=0;
	char str[256];
	char weight[PROPERTY_VALUE_MAX]={0};
	char weight_default[PROPERTY_VALUE_MAX]="_128_128_128_128_128_128_128_128_128_128_128_128_128_128_128_128_128_128";
	property_get("debug" ,debug , "no");
	BL_LOGI("debug=%s\n",debug);
	init_blend_sem();
	int32_t *mem0=new int32_t[(small_width*small_height)*(imageNum)+THREAD_NUM_MAX*small_width+2*(imageNum-1)];
	if(NULL==mem0)
	{
		return -1;
	}
	
	p3dnr_info->xProj1D = new int32_t* [imageNum]; //1d
	...
	p3dnr_info->extra = mem0 + (small_height+small_width)*(imageNum);//ImgWidth;
	...
	if(imageNum > 2)
	{
		...
		}
		{
			...
			p3dnr_info->intermedbuf.bufferV = p3dnr_info->intermedbuf.bufferU + orig_width*orig_height/4;
		}
	}
	...
	else
	{
		...
		if(ret<0)
		{
			BL_LOGI("the input weight table can't meet the demand of 3DNR,please use default weight table\n");
			ret=get_weight(weight_default,imageNum);
			if(ret<0)
				BL_LOGE("get default weight table fails!\n");
		}
...
void Config3DNR(c3dnr_param_info_t *param) 
{
	...
	p3dnr_info->is_first_frame = 1;//wxz: for preview
	...
	p3dnr_info->isp_handle = param->isp_handle; //wxz: get the isp handle.
...
__attribute__ ((visibility("default"))) int threednr_init(c3dnr_param_info_t *param)
{
	...
	{
		BL_LOGE("the size of input image doesn't meet require:width=%d , height=%d\n" ,param->small_width,param->small_height);
		return -1;		
	}
	...
	Config3DNR(param);
	ret=initModule(param->small_width,param->small_height, param->orig_width , param->orig_height , param->total_frame_num);
	if(ret<0)
	{
