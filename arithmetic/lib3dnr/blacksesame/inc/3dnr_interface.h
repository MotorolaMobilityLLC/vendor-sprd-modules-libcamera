...
// typedef cmr_int (* isp_ioctl_fun)(cmr_handle isp_handler, enum isp_ctrl_cmd
// cmd, void *param_ptr);
...
typedef struct c3dnr_pre_inparam { uint16_t gain; } c3dnr_pre_inparam_t;
...
#endif
#define CTRL_SYNC_EN                                                           \
    (1 << 0) // enable GPU synchronize every frame, if this bit is 0, GPU will
             // run till next GPU command in next frame
#define CTRL_TIME_EN (1 << 1)  // enable cpu and gpu timing check
#define CTRL_DEBUG_EN (1 << 2) // enable debug information
