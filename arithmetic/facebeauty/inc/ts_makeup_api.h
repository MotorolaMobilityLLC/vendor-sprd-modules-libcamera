...
void ts_SetImageOritation(int angle); // for algorithm face detection
                                       ...
                                       *   @param[in] pfaceRect: pointer to facerect, if NULL,use ts facedetect, else
                                       * use system facedata.
                                       *   @param[in] IsneedFaceDetection:Whether it needs face beauty lib FD,1 for
                                       * yes ,0 for no.
                                       *   @param[in] nImageFormat: Image format,default value 0 is for
                                       * NV21,TSFB_FMT_NV12 for NV12.
                                       * RETURN    : TS_OK if success, otherwise failed.
                                       *
                                       */
int ts_face_beautify(TSMakeupData *pInData, TSMakeupData *pOutData,
                     int skinCleanLevel, int skinWhitenLevel, TSRect *pfaceRect,
                     bool IsneedFaceDetection, int nImageFormat);
