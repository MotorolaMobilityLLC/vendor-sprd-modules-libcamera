...
#define SNAPSHOT_LINE_TIME		16700
#define PREVIEW_LINE_TIME		16700
...
static uint32_t ov5675_init_mode_fps_info(SENSOR_HW_HANDLE handle)
{
	...
	if(!s_ov5675_mode_fps_info.is_init) {
		...
		for(i = 0;i < NUMBER_OF_ARRAY(s_ov5675_resolution_trim_tab); i++) {
			...
			tempfps = s_ov5675_resolution_trim_tab[i].line_time*s_ov5675_resolution_trim_tab[i].frame_line;
				if(0 != tempfps) {
			...
			SENSOR_PRINT("mode %d,max_fps: %d ",
					i,s_ov5675_mode_fps_info.sensor_mode_fps[i].max_fps);
...
static uint32_t ov5675_get_static_info(SENSOR_HW_HANDLE handle, uint32_t *param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	...
	ex_info->adgain_valid_frame_num = s_ov5675_static_info.adgain_valid_frame_num;
	...
	ex_info->name = g_ov5675_mipi_raw_info.name;
	ex_info->sensor_version_info = g_ov5675_mipi_raw_info.sensor_version_info;
...
static uint32_t ov5675_get_fps_info(SENSOR_HW_HANDLE handle, uint32_t *param)
{
	...
	fps_info->max_fps = s_ov5675_mode_fps_info.sensor_mode_fps[sensor_mode].max_fps;
	fps_info->min_fps = s_ov5675_mode_fps_info.sensor_mode_fps[sensor_mode].min_fps;
	fps_info->is_high_fps = s_ov5675_mode_fps_info.sensor_mode_fps[sensor_mode].is_high_fps;
	fps_info->high_fps_skip_num = s_ov5675_mode_fps_info.sensor_mode_fps[sensor_mode].high_fps_skip_num;
...
#define param_update(x1,x2) sprintf(name,"/data/misc/cameraserver/ov5675_%s.bin",x1);\
 ...
 *
 *============================================================================*/
static uint32_t ov5675_get_default_frame_length(SENSOR_HW_HANDLE handle,uint32_t mode)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov5675_write_frame_length(SENSOR_HW_HANDLE handle,uint32_t frame_len)
{
 ...
 *============================================================================*/
static uint16_t ov5675_update_exposure(SENSOR_HW_HANDLE handle,uint32_t shutter,uint32_t dummy_line)
{
 ...
 *============================================================================*/
static uint32_t ov5675_before_snapshot(SENSOR_HW_HANDLE handle,uint32_t param)
{
	...
	s_current_default_frame_length = ov5675_get_default_frame_length(handle,capture_mode);
 ...
 *============================================================================*/
static uint32_t ov5675_write_exposure(SENSOR_HW_HANDLE handle,uint32_t param)
{
	...
	s_current_default_frame_length = ov5675_get_default_frame_length(handle,mode);

	s_sensor_ev_info.preview_shutter = ov5675_update_exposure(handle,exposure_line,dummy_line);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t ov5675_write_gain_value(SENSOR_HW_HANDLE handle,uint32_t param)
{
	uint32_t ret_value = SENSOR_SUCCESS;
	uint32_t real_gain = 0;
 ...
 *
 *============================================================================*/
static void ov5675_increase_hdr_exposure(SENSOR_HW_HANDLE handle,uint8_t ev_multiplier)
{
	...
	if (shutter_multiply >= ev_multiplier) {
		ov5675_update_exposure(handle,s_hdr_info.capture_shutter * ev_multiplier,0);
		ov5675_write_gain(handle,s_hdr_info.capture_gain);
	} else {
		gain = s_hdr_info.capture_gain * ev_multiplier / shutter_multiply;
		ov5675_update_exposure(handle,s_hdr_info.capture_shutter * shutter_multiply,0);
 ...
 *
 *============================================================================*/
static void ov5675_decrease_hdr_exposure(SENSOR_HW_HANDLE handle,uint8_t ev_divisor)
{
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t ov5675_set_hdr_ev(SENSOR_HW_HANDLE handle,unsigned long param)
{
	uint32_t ret = SENSOR_SUCCESS;
...
static unsigned long ov5675_access_val(SENSOR_HW_HANDLE handle, unsigned long param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	SENSOR_VAL_T* param_ptr = (SENSOR_VAL_T*)param;
	uint16_t tmp;

	SENSOR_LOGI("SENSOR_ov5675: _ov5675_access_val E param_ptr = %p", param_ptr);
	if(!param_ptr){
		return rtn;
	}
