...
#define sr352_I2C_ADDR_W        0x20//0x20
#define sr352_I2C_ADDR_R         0x20//0x20

#define I2C_WRITE_BURST_LENGTH    512

static uint32_t  g_flash_mode_en = 0;

static uint32_t _sr352_InitExifInfo(void);
static unsigned long _sr352_PowerOn(unsigned long power_on);
static unsigned long _sr352_GetResolutionTrimTab(unsigned long param);
static unsigned long _sr352_Identify(unsigned long param);
static unsigned long _sr352_set_brightness(unsigned long level);
static unsigned long _sr352_set_contrast(unsigned long level);
static unsigned long _sr352_set_saturation(unsigned long level);
static unsigned long _sr352_set_image_effect(unsigned long effect_type);
static unsigned long _sr352_set_ev(unsigned long level);
static unsigned long _sr352_set_anti_flicker(unsigned long mode);
static unsigned long _sr352_set_awb(unsigned long mode);
static unsigned long _sr352_set_work_mode(unsigned long mode);
static unsigned long _sr352_BeforeSnapshot(unsigned long param);
static unsigned long _sr352_check_image_format_support(unsigned long param);
static unsigned long _sr352_after_snapshot(unsigned long param);
static unsigned long _sr352_GetExifInfo(unsigned long param);
static unsigned long sr352_InitExt(unsigned long param);

static const SENSOR_REG_T sr352_common[]=
{
	{0x03, 0x26},
	{0x10, 0x89},
	...
	{0x02, 0x07},	// pk_drive = 000b, i2c_drive = 111b
	{0x0c, 0x07},	// d_pad_drive = 000b, gpio_pad_drive = 111b
	{0x07, 0x25},	//mode_pll1  24mhz / (5+1) = 4mhz
	{0x08, 0x6c},	//mode_pll2  isp k = 108Mhz;
	{0x09, 0x85},	//mode_pll3  // MIPI 4x div 1/2 // isp k div = 1/4 //Preview
	{0x07, 0xA5},
	{0x07, 0xA5},
	{0x07, 0xA5},
	...
	{0x03, 0x26},
	{0x1B, 0x03},		// bus k div = 1/4

	// 7 Page(memory configuration)//
	{0x03, 0x07},
	{0x21, 0x01},	// SSD sram ock inv on
	...
	{0x14, 0x00},
	{0x23, 0x8F},	// Frame BLC avg accumulation for 8 frame
	...
	{0xf2, 0x8a},//pga_dark1_hi //Enter Dark1


	{0xf3, 0x82},//pga_dark_lo  //Escape Dark1
	...
	{0xc0, 0x00},//Yc2d_ee_lip_gain_n1


	{0xc1, 0x00},//Yc2d_ee_lip_gain_n2


	{0xc2, 0x00},//Yc2d_ee_lip_gain_n3


	{0xc3, 0x00},//Yc2d_ee_lip_gain_n4


	{0xc4, 0x01},//Yc2d_ee_lip_gain_n5
	...
	{0x12, 0x60},// 13.7.19 3P lens dark mode issue 20 -> 60  digtial gain enable
	...
	{0x71, 0x8a},//Digital gain max  // 13.7.19 3P lens dark mode issue
	...
	{0x31, 0x27},//dwOutdoorCondInTh_n02


	{0x32, 0x10},//dwOutdoorCondInTh_n03
	...
	{0x35, 0x2e},//dwOutdoorCondOutTh_n02


	{0x36, 0xe0},//dwOutdoorCondOutTh_n03
	...
	{0x39, 0x05},//dwEvTh_n02 //780fps


	{0x3a, 0x02},//dwEvTh_n03
	...
	{0x3d, 0x0a},//dwEvTh_a01_n02


	{0x3e, 0x47},//dwEvTh_a01_n03 //380fps
	...
	{0x96, 0x10}, //Indoor_bRgStep


	{0x97, 0x10}, //Indoor_bBgStep
	...
	{0xb6, 0x66},//Indoor_aTgtWhtRgnRgRtLmt_a07


	{0xb7, 0x5e},//Indoor_aTgtWhtRgnRgRtLmt_a08


	{0xb8, 0x57},//Indoor_aTgtWhtRgnRgRtLmt_a09


	{0xb9, 0x4d},//Indoor_aTgtWhtRgnRgRtLmt_a10
	...
	{0xd7, 0x77},//Indoor_aOptWhtRgnRgRtLmt_a07


	{0xd8, 0x66},//Indoor_aOptWhtRgnRgRtLmt_a08


	{0xd9, 0x5b},//Indoor_aOptWhtRgnRgRtLmt_a09
	...
	{0x82, 0x30}, //Outdoor_wRgIntOfs_n01    h40->h20  13.5.27  LSC update shift


	{0x83, 0x01}, //Outdoor_wBgIntOfs


	{0x84, 0x50}, //Outdoor_wBgIntOfs_n01


	{0x85, 0x0f}, //Outdoor_bRgStep
	...
	{0x1a, 0x18},//Outdoor_aRgTgtOfs_a01


	{0x1b, 0x08},//Outdoor_aRgTgtOfs_a02


	{0x1c, 0x02},//Outdoor_aRgTgtOfs_a03
	...
	{0x1f, 0x05},//Outdoor_aBgTgtOfs_a01


	{0x20, 0x02},//Outdoor_aBgTgtOfs_a02


	{0x21, 0x80},//Outdoor_aBgTgtOfs_a03


	{0x22, 0x82},//Outdoor_aBgTgtOfs_a04
	...
	{0x56, 0x52},// LSC Ctmp LTStr In     h56->h56   13.7.19 3P TL84 LSC R reduce  56->52
	...
	{0x78, 0x5a},	// UniScn_Cb_Ratio

	{0x79, 0x5a},	// UniScn_Cr_Ratio
	...
	{0xa9, 0x81},//LSC Out_M Gain B

	{0xaa, 0x81},//LSC Out_M Gain R
	...
	{0xed, 0x1a}, //Outdoor_allgain_y7


	{0xee, 0x19}, //Outdoor_allgain_y8
	...
	{0xf6, 0x13},//Indoor_delta3	light green


	{0xf7, 0xb8},//Indoor_center3	light green


	{0xf8, 0x15},//Indoor_delta4


	{0xf9, 0xe9},//Indoor_center4
	...
	{0x17, 0x00}, //Hi-Temp_rgb_ofs_s_r1


	{0x18, 0x00}, //Hi-Temp_rgb_ofs_s_g1


	{0x19, 0x00}, //Hi-Temp_rgb_ofs_s_b1


	{0x1a, 0x00}, //Hi-Temp_rgb_ofs_s_r2


	{0x1b, 0x00}, //Hi-Temp_rgb_ofs_s_g2


	{0x1c, 0x00}, //Hi-Temp_rgb_ofs_s_b2


	{0x1d, 0x00}, //Hi-Temp_rgb_ofs_s_r3


	{0x1e, 0x00}, //Hi-Temp_rgb_ofs_s_g3


	{0x1f, 0x00}, //Hi-Temp_rgb_ofs_s_b3


	{0x20, 0x00}, //Hi-Temp_rgb_ofs_s_r4


	{0x21, 0x00}, //Hi-Temp_rgb_ofs_s_g4


	{0x22, 0x00}, //Hi-Temp_rgb_ofs_s_b4


	{0x23, 0x00}, //Hi-Temp_rgb_ofs_s_r5


	{0x24, 0x00}, //Hi-Temp_rgb_ofs_s_g5


	{0x25, 0x00}, //Hi-Temp_rgb_ofs_s_b5


	{0x26, 0x00}, //Hi-Temp_rgb_ofs_s_r6


	{0x27, 0x00}, //Hi-Temp_rgb_ofs_s_g6


	{0x28, 0x00}, //Hi-Temp_rgb_ofs_s_b6
	...
	{0x35, 0x27},//Indoor 1141 //Pre flat R1 STDH
	...
	{0xf5, 0x05},//Indoor 11e0 //Bayer Flat R3 LumL
	...
	{0x2d, 0x22},//Dark1 113d //Pre Flat R1 DyH
	...
	{0x35, 0x17},//Dark1 1141 //Pre Flat R1 STDH
	...
	{0xe5, 0x32},//Dark1 11d5 //Bayer Flat R1 STD H
	...
	{0xed, 0x17},//Dark1 11d9 //Bayer Flat R1 DY H
	...
	{0xcb, 0x1f},//Dark1 1340 //Y flat R1/R2 rate


	{0xcc, 0x41},


	{0xcd, 0x01},//Dark1 1341 //Y flat R1_LumL


	{0xce, 0x42},


	{0xcf, 0x62},//Dark1 1342 //Y flat R1_LumH


	{0xd0, 0x43},


	{0xd1, 0x63},//Dark1 1343 //Y flat R2_LumL


	{0xd2, 0x44},


	{0xd3, 0xff},//Dark1 1344 //Y flat R2_LumH


	{0xd4, 0x45},


	{0xd5, 0x01},//Dark1 1345 //Y flat R1_STDL


	{0xd6, 0x46},


	{0xd7, 0xa0},//Dark1 1346 //Y flat R1_STDH


	{0xd8, 0x47},


	{0xd9, 0x01},//Dark1 1347 //Y flat R2_STDL


	{0xda, 0x48},


	{0xdb, 0x8d},//Dark1 1348 //Y flat R2_STDH


	{0xdc, 0x49},


	{0xdd, 0x01},//Dark1 1349 //Y flat R1_DYL


	{0xde, 0x4a},


	{0xdf, 0x51},//Dark1 134a //Y flat R1_DYH


	{0xe0, 0x4b},


	{0xe1, 0x01},//Dark1 134b //Y flat R2_DYL


	{0xe2, 0x4c},


	{0xe3, 0x79},//Dark1 134c //Y flat R2_DYH
	...
	{0xc1, 0x01},//Dark1 1A39 //RGB Flat R2_Lum L
	...
	{0xe5, 0x32},//Dark2 11d5 //Bayer Flat R1 STD H
	...
	{0xed, 0x17},//Dark2 11d9 //Bayer Flat R1 DY H
	...
	{0xcb, 0x1f},//Dark2 1340 //Y flat R1/R2 rate


	{0xcc, 0x41},


	{0xcd, 0x01},//Dark2 1341 //Y flat R1_LumL


	{0xce, 0x42},


	{0xcf, 0x62},//Dark2 1342 //Y flat R1_LumH


	{0xd0, 0x43},


	{0xd1, 0x63},//Dark2 1343 //Y flat R2_LumL


	{0xd2, 0x44},


	{0xd3, 0xff},//Dark2 1344 //Y flat R2_LumH


	{0xd4, 0x45},


	{0xd5, 0x01},//Dark2 1345 //Y flat R1_STDL


	{0xd6, 0x46},


	{0xd7, 0xa0},//Dark2 1346 //Y flat R1_STDH


	{0xd8, 0x47},


	{0xd9, 0x01},//Dark2 1347 //Y flat R2_STDL


	{0xda, 0x48},


	{0xdb, 0x8d},//Dark2 1348 //Y flat R2_STDH


	{0xdc, 0x49},


	{0xdd, 0x01},//Dark2 1349 //Y flat R1_DYL


	{0xde, 0x4a},


	{0xdf, 0x51},//Dark2 134a //Y flat R1_DYH


	{0xe0, 0x4b},


	{0xe1, 0x01},//Dark2 134b //Y flat R2_DYL


	{0xe2, 0x4c},


	{0xe3, 0x79},//Dark2 134c //Y flat R2_DYH
	...
	{0xc0, 0x39},
	{0xc1, 0x01},//Dark2 1A39 //RGB Flat R2_Lum L
	{0xc2, 0x3a},
	...
	{0x32, 0x0f},  // k_zero_time
	{0x33, 0x0b},  // k_post_time
	{0x34, 0x03},  // k_prepare_time
	{0x35, 0x04},  // k_trail_time
	{0x36, 0x01},  // k_tlpx_time_dp
	{0x37, 0x06},  // k_tlpx_time_dn
	...
	{0x50, 0x0A},  // cntns_k_wait_h
	{0x51, 0x00},  // cntns_k_wait_l
	...
	{0x21, 0x61}, // Band1 Gain 30fps
	{0x22, 0x72}, // Band2 Gain 15fps
	{0x23, 0x80},// Band3 Gain 12fps
...
static const SENSOR_REG_T sr352_640X480[]=
{
	{0x03, 0xc1},
	{0x10, 0x06},
	{0xff, 0x0a},
	...
	{0x03, 0x00},
	{0x10, 0x41},
	{0x11, 0x80},	// 0frame skip variable frame
	{0x17, 0x00},
	{0x20, 0x00},
	{0x21, 0x00},
	{0x22, 0x00},
...
static const SENSOR_REG_T sr352_1024X768[] =
{
	{0x03, 0xc1},
	{0x10, 0x06},
	{0xff, 0x01},
...
//1280X960  YUV   Mode
static const SENSOR_REG_T sr352_1280X960[] =
{
	{0x03, 0xc8},   //AWB Off
	{0x10, 0x52},
	...
	{0x7f, 0x00},   // DMA off
	{0x7e, 0x01},   // DMA set
	{SENSOR_WRITE_DELAY,0x78}, //delay 120ms
	...
	{0x20, 0x00},
	{0x21, 0x06},   // row start set
	{0x22, 0x00},
	{0x23, 0x06},   // col start set
...
//1600X1200  YUV   Mode
static const SENSOR_REG_T sr352_1600X1200[] =
{
	{0x03, 0xc8},
	{0x10, 0x52},
	{0x03, 0xc1},
...
//20480X1536  JPG   Mode
static const SENSOR_REG_T sr352_2048X1536[] =
{
	{0x03, 0xc8},
	{0x10, 0x52},
	{0x03, 0xc1},
	...
	{0x20, 0x00},
	{0x21, 0x08},	// row start set XY noflip
	{0x22, 0x00},
	{0x23, 0x01},	// col start set XY noflip
	{0x03, 0x0b},
	{0x11, 0x11}, //Full HD(crop) 0410
	{0x12, 0x00}, //Full HD(crop) 0410
	{0x03, 0x11},
...
static SENSOR_REG_TAB_INFO_T s_sr352_resolution_Tab_YUV[]=
{
	// COMMON INIT
	{ADDR_AND_LEN_OF_ARRAY(sr352_common),   0, 0, 24, SENSOR_IMAGE_FORMAT_YUV422},

//	{ADDR_AND_LEN_OF_ARRAY(sr352_640X480),   640,  480,   24, SENSOR_IMAGE_FORMAT_YUV422},
	{ADDR_AND_LEN_OF_ARRAY(sr352_1024X768),   1024,  768,   24, SENSOR_IMAGE_FORMAT_YUV422},
...
static SENSOR_TRIM_T s_sr352_resolution_Trim_Tab[] = {
	{0, 0, 640, 480, 0, 0, 0, {0, 0, 640, 480}},

//	{0, 0, 640, 480, 68, 568, 0x03b8, {0, 0, 640, 480}},
	{0, 0, 1024, 768, 68, 850, 0x03b8, {0, 0, 1024, 768}},
//	{0, 0, 1600, 1200, 68, 850, 0x03b8, {0, 0, 1600, 1200}},
//	{0, 0, 2048, 1536, 68, 850, 0x03b8, {0, 0, 2048, 1536}},
...
static unsigned long sr352_InitExt(unsigned long param)	//wujinyou, 2012.11.14
{
...
static SENSOR_IOCTL_FUNC_TAB_T s_sr352_ioctl_func_tab = {
	// Internal
	PNULL,
	_sr352_PowerOn,
	PNULL,
	...
	PNULL,							// read  register
	PNULL,//sr352_InitExt,				//PNULL,	//sr352_InitExt,
	_sr352_GetResolutionTrimTab,
	...
	_sr352_set_brightness,
	_sr352_set_contrast,

	PNULL,
	PNULL,							//_sr352_set_saturation,

	_sr352_set_work_mode,
	_sr352_set_image_effect,


	_sr352_BeforeSnapshot,
	_sr352_after_snapshot,
	PNULL,
	PNULL,
	PNULL,
	PNULL,
	...
	PNULL,
	_sr352_set_ev,
	_sr352_check_image_format_support,
	PNULL,
	PNULL,
	PNULL,							//_sr352_GetExifInfo,
	PNULL,							//_sr352_ExtFunc,
	PNULL,							//_sr352_set_anti_flicker
...
SENSOR_INFO_T g_sr352_mipi_yuv_info =
{
	...
	PNULL,//&g_sr352_ext_info,                // extend information about sensor
	//5,
...
static unsigned long _sr352_PowerOn(unsigned long power_on)
{
	SENSOR_AVDD_VAL_E dvdd_val = g_sr352_mipi_yuv_info.dvdd_val;
	SENSOR_AVDD_VAL_E avdd_val = g_sr352_mipi_yuv_info.avdd_val;
...
static const SENSOR_REG_T sr352_brightness_tab[][1]=
{
	{{0xff, 0xff}},
	{{0xff, 0xff}},
	{{0xff, 0xff}},
...
static const SENSOR_REG_T sr352_contrast_tab[][15]=
{
	{
		{0xff, 0xff}
	},
...
static const SENSOR_REG_T sr352_saturation_tab[][40]=
{
	{//level -3
		{0xff, 0xff}
	},
...
static const SENSOR_REG_T sr352_image_effect_tab[][10]=
{
	// effect normal
	{
		{0xff, 0xff},
...
static const SENSOR_REG_T sr352_ev_tab[][3]=
{
	{
		{0xff,0xff},
	},
...
static const SENSOR_REG_T sr352_anti_banding_flicker_tab[][5]=
{
	{//50hz
		{0xff, 0xff}
	},
...
static const SENSOR_REG_T sr352_video_mode_tab[][56]=
{
	{
		{0xff, 0xff}
	},
...
static unsigned long _sr352_set_video_mode(unsigned long mode)
{//65.6 us
...
static const SENSOR_REG_T sr352_awb_tab[][15] =
{
	//AUTO
	{
		/*WB Auto*/
...
static unsigned long _sr352_set_awb(unsigned long mode)
{
 ...
 static const SENSOR_REG_T sr352_work_mode_tab[][175]=
{
	{//auto
		{0xff,0xff},
	},
...
{
	uint16_t i=0x00;
	SENSOR_REG_T_PTR sensor_reg_ptr=(SENSOR_REG_T_PTR)sr352_work_mode_tab[mode];
