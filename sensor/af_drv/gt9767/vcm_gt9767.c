...
#define LOG_TAG "vcm_gt9767"
#include "vcm_gt9767.h"
...
static uint32_t _gt9767_set_motor_bestmode(cmr_handle sns_af_drv_handle) {
    ...
    cmd_val[0] = 0x02;
    cmd_val[1] = 0x02;
    hw_Sensor_WriteI2C(af_drv_cxt->hw_handle, GT9767_VCM_SLAVE_ADDR,
                       (uint8_t *)&cmd_val[0], 2);
    usleep(200);
    cmd_val[0] = 0x06;
    cmd_val[1] = 0x80;
...
static uint32_t _gt9767_get_test_vcm_mode(cmr_handle sns_af_drv_handle) {
    ...
    cmd_val[0] = 0x02;
    cmd_val[1] = 0x02;
    hw_Sensor_ReadI2C(af_drv_cxt->hw_handle, GT9767_VCM_SLAVE_ADDR,
                      (uint8_t *)&cmd_val[0], 1);
    ctrl = cmd_val[0];
    usleep(200);
    cmd_val[0] = 0x06;
...
static int gt9767_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _gt9767_drv_power_on(*sns_af_drv_handle, AF_TRUE);
        ret = _gt9767_drv_init(*sns_af_drv_handle);
        if (ret != AF_SUCCESS)
            ret = AF_FAIL;
    }
...
static int gt9767_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    cmr_int ret = AF_SUCCESS;
    CHECK_PTR(sns_af_drv_handle);
    _gt9767_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int gt9767_drv_ioctl(cmr_handle sns_af_drv_handle, enum sns_cmd cmd,
    ...
    case CMD_SNS_AF_SET_BEST_MODE:
        _gt9767_set_motor_bestmode(sns_af_drv_handle);
        break;
    case CMD_SNS_AF_GET_TEST_MODE:
        _gt9767_get_test_vcm_mode(sns_af_drv_handle);
        break;
    case CMD_SNS_AF_SET_TEST_MODE:
        _gt9767_set_test_vcm_mode(sns_af_drv_handle, param);
        break;
    default:
        break;
    }
...
struct sns_af_drv_entry gt9767_drv_entry = {
    .motor_avdd_val = SENSOR_AVDD_2800MV,
    .default_work_mode = 0,
    .af_ops =
        {
            .create = gt9767_drv_create,
            .delete = gt9767_drv_delete,
            .set_pos = gt9767_drv_set_pos,
            .get_pos = NULL,
            .ioctl = gt9767_drv_ioctl,
        },
};
...
static int _gt9767_drv_power_on(cmr_handle sns_af_drv_handle,
    ...
    if (AF_TRUE == power_on) {
        hw_sensor_set_monitor_val(af_drv_cxt->hw_handle,
                                  gt9767_drv_entry.motor_avdd_val);
        usleep(GT9767_POWERON_DELAY * 1000);
    } else {
...
static int _gt9767_drv_init(cmr_handle sns_af_drv_handle) {
    ...
    slave_addr = GT9767_VCM_SLAVE_ADDR;
    CMR_LOGV("E");
    cmd_len = 2;
    cmd_val[0] = 0x02;
    cmd_val[1] = 0x00;
