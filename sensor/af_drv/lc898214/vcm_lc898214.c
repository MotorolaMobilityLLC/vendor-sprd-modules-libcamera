...
static int lc898214_drv_set_pos(cmr_handle sns_af_drv_handle, uint32_t pos) 
{
	...
	uint16_t ivalue2;
	uint16_t temp;
	double k = 1.0*(pri_data->Hall_Max- pri_data->Hall_Min) / (Max_Pos - Min_Pos);
	...
	if ((short)ivalue2 < (short)pri_data->Hall_Min)
	{
		ivalue2 = pri_data->Hall_Min;
	}
	if ((short)ivalue2 > (short)pri_data->Hall_Max)
	{
		ivalue2 = pri_data->Hall_Max;
	}
	//BOOL bRet = I2C_Write(SlaveID, 0xA0 ,ivalue2, 2);
	cmd_len = 3;
	cmd_val[0] = 0xA0;
	cmd_val[1] = pos;//ivalue2;//TODO
	ret_value = hw_Sensor_WriteI2C(af_drv_cxt->hw_handle,LC898214_VCM_SLAVE_ADDR,(uint8_t*)&cmd_val[0], cmd_len);
	//SENSOR_LOGI("set position %d SlvCh %d %d", pos,ivalue2,ret_value);

	uint16_t SlvCh;
	for(int i=0;i<30;i++)
	{
		SlvCh = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle,LC898214_VCM_SLAVE_ADDR,0x8F, BITS_ADDR8_REG8);
		ret_value = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle,LC898214_VCM_SLAVE_ADDR,0xA0, BITS_ADDR8_REG16);
		usleep(50);
		if (SlvCh == 0x00)
		{
			break;
...
static int _lc898214_drv_init(cmr_handle sns_af_drv_handle)
{
	...
	for(int i=0;i<30;i++)
	{
		SlvCh = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle, slave_addr, 0xF0, BITS_ADDR8_REG8);
		usleep(50);
		if (SlvCh == 0x42)
		{
			break;
	...
	Max_H = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle,EepromSlaveID, 0x38, BITS_ADDR8_REG8);
	//I2C_Read(EepromSlaveID, 0x38, &Max_H, 0); //读取Eeprom内部HALL MAX、MIN数据数据
	usleep(50);
	Max_L = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle,EepromSlaveID, 0x39, BITS_ADDR8_REG8);
	//I2C_Read(EepromSlaveID, 0x39, &Max_L, 0); //读取Eeprom内部HALL MAX、MIN数据数据
	usleep(50);
	Min_H = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle,EepromSlaveID, 0x3A, BITS_ADDR8_REG8);
	//I2C_Read(EepromSlaveID, 0x3A, &Min_H, 0); //读取Eeprom内部HALL MAX、MIN数据数据
	usleep(50);
	Min_L = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle,EepromSlaveID, 0x3B, BITS_ADDR8_REG8);
	...
	pri_data->Hall_Max = (int16_t)Max;
	pri_data->Hall_Min = (int16_t)Min; 
	...
	for(int i=0;i<30;i++)
	{
		EepCh = hw_sensor_grc_read_i2c(af_drv_cxt->hw_handle, slave_addr,  0xE0, BITS_ADDR8_REG8);
		usleep(50);
		if (EepCh == 0x00)
		{
			break;
