...
uint32_t vcm_LC898214_init(SENSOR_HW_HANDLE handle) {
    ...
    // I2C_Read(EepromSlaveID, 0x38, &Max_H, 0); //读取Eeprom内部HALL
    // MAX、MIN数据数据
    usleep(50);
    Max_L = sensor_grc_read_i2c(EepromSlaveID, 0x39, BITS_ADDR8_REG8);
    // I2C_Read(EepromSlaveID, 0x39, &Max_L, 0); //读取Eeprom内部HALL
    // MAX、MIN数据数据
    usleep(50);
    Min_H = sensor_grc_read_i2c(EepromSlaveID, 0x3A, BITS_ADDR8_REG8);
    // I2C_Read(EepromSlaveID, 0x3A, &Min_H, 0); //读取Eeprom内部HALL
    // MAX、MIN数据数据
    usleep(50);
    Min_L = sensor_grc_read_i2c(EepromSlaveID, 0x3B, BITS_ADDR8_REG8);
    //		I2C_Read(EepromSlaveID, 0x3B, &Min_L, 0); //读取Eeprom内部HALL
    // MAX、MIN数据数据
    usleep(50);
    ...
    // SENSOR_LOGI("DriverIC Init Max_H  %d %d %d %d! %d %d",Max_H ,Max_L ,Min_H
    // ,Min_L,Max, Min);
...
uint32_t vcm_LC898214_set_position(SENSOR_HW_HANDLE handle, uint32_t pos) {
    ...
    ret_value = Sensor_WriteI2C(LC898214_VCM_SLAVE_ADDR, (uint8_t *)&cmd_val[0],
                                cmd_len);
    ...
    for (int i = 0; i < 30; i++) {
        SlvCh =
            sensor_grc_read_i2c(LC898214_VCM_SLAVE_ADDR, 0x8F, BITS_ADDR8_REG8);
        // I2C_Read(SlaveID, 0x8F, &SlvCh, 0);
        ret_value = sensor_grc_read_i2c(LC898214_VCM_SLAVE_ADDR, 0xA0,
                                        BITS_ADDR8_REG16);
        usleep(50);
        //	SENSOR_LOGI("set position %d SlvCh1 %d %d",
        // pos,SlvCh,ret_value);
        if (SlvCh == 0x00) {
            break;
        }
