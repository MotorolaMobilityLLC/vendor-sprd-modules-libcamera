...
typedef struct af_ioctl_func_tab_tag
{
	...
	cmr_uint (*motor_deinit)(SENSOR_HW_HANDLE handle, uint32_t mode); /*can be NULL*/

	cmr_uint (*set_motor_pos)(SENSOR_HW_HANDLE handle, uint32_t pos); /*customer must realize*/
	cmr_uint (*get_motor_pos)(SENSOR_HW_HANDLE handle, uint16_t *pos); /*can be NULL*/
	...
	/*expend*/
	cmr_uint (*motor_process)(SENSOR_HW_HANDLE handle, uint32_t cmd, void* param);
}af_ioctl_func_tab_t;
