 ...
 *============================================================================*/
static uint32_t _dw9768v_write_dac_code(cmr_handle sns_af_drv_handle,
                                        int32_t code) {
    ...
    uint16_t slave_addr = af_drv_cxt->i2c_addr & 0xff;//DW9768V_VCM_SLAVE_ADDR;
    uint16_t cmd_len = 0;
...
static int dw9768v_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _dw9768v_drv_power_on(*sns_af_drv_handle, AF_TRUE);
        ret = _dw9768v_drv_set_mode(*sns_af_drv_handle);
        if (ret != AF_SUCCESS)
            ret = AF_FAIL;
    }
...
static int dw9768v_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    cmr_int ret = AF_SUCCESS;
    CHECK_PTR(sns_af_drv_handle);
    dw9768v_drv_set_pos(sns_af_drv_handle, 0);
    _dw9768v_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int dw9768v_drv_set_pos(cmr_handle sns_af_drv_handle, uint16_t pos) {
    uint32_t ret_value = AF_SUCCESS;
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
    CHECK_PTR(sns_af_drv_handle);
...
static int dw9768v_drv_ioctl(cmr_handle sns_af_drv_handle, enum sns_cmd cmd,
                             void *param) {
    uint32_t ret_value = AF_SUCCESS;
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
...
struct sns_af_drv_entry dw9768v_drv_entry = {
    .motor_avdd_val = SENSOR_AVDD_2800MV,
    .default_work_mode = 2,
    .af_ops =
        {
            .create = dw9768v_drv_create,
            .delete = dw9768v_drv_delete,
            .set_pos = dw9768v_drv_set_pos,
            .get_pos = NULL,
            .ioctl = dw9768v_drv_ioctl,
        },
};

static int _dw9768v_drv_power_on(cmr_handle sns_af_drv_handle,
                                 uint16_t power_on) {
    CHECK_PTR(sns_af_drv_handle);
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
...
static int _dw9768v_drv_set_mode(cmr_handle sns_af_drv_handle) {
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
    CHECK_PTR(sns_af_drv_handle);
    ...
    uint16_t cmd_len = 0;
    uint32_t ret_value = AF_SUCCESS;
    uint32_t mode = dw9768v_drv_entry.default_work_mode;
