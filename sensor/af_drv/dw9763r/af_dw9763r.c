 ...
 *============================================================================*/
static uint32_t _dw9763r_write_dac_code(cmr_handle sns_af_drv_handle,
                                        int32_t code) {
    ...
    uint16_t slave_addr = DW9763R_VCM_SLAVE_ADDR;
    uint16_t cmd_len = 0;
...
static int dw9763r_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        ret = _dw9763r_drv_set_mode(*sns_af_drv_handle);
        if (ret != AF_SUCCESS)
            ret = AF_FAIL;
    }
...
static int dw9763r_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    cmr_int ret = AF_SUCCESS;
    CHECK_PTR(sns_af_drv_handle);
    dw9763r_drv_set_pos(sns_af_drv_handle, 0);
...
static int dw9763r_drv_set_pos(cmr_handle sns_af_drv_handle, uint16_t pos) {
    uint32_t ret_value = AF_SUCCESS;
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
    CHECK_PTR(sns_af_drv_handle);
    ...
    if (!target_code) {
        while ((m_cur_dac_code - target_code) >= MOVE_CODE_STEP_MAX) {
            ...
            _dw9763r_write_dac_code(sns_af_drv_handle, m_cur_dac_code);
            CMR_LOGI("dac_target_code = %d\n", m_cur_dac_code);
            usleep(WAIT_STABLE_TIME * 1000);
        }
    }
    _dw9763r_write_dac_code(sns_af_drv_handle, target_code);
...
static int dw9763r_drv_ioctl(cmr_handle sns_af_drv_handle, enum sns_cmd cmd,
                             void *param) {
    uint32_t ret_value = AF_SUCCESS;
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
...
struct sns_af_drv_entry dw9763r_drv_entry = {
    .motor_avdd_val = SENSOR_AVDD_2800MV,
    .default_work_mode = 2,
    .af_ops =
        {
            .create = dw9763r_drv_create,
            .delete = dw9763r_drv_delete,
            .set_pos = dw9763r_drv_set_pos,
            .get_pos = NULL,
            .ioctl = dw9763r_drv_ioctl,
        },
};

static int _dw9763r_drv_set_mode(cmr_handle sns_af_drv_handle) {
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
    CHECK_PTR(sns_af_drv_handle);
    ...
    if (af_drv_cxt->af_work_mode) {
        ...
        mode = dw9763r_drv_entry.default_work_mode;
    }
