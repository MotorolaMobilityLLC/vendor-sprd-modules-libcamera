 ...
 *============================================================================*/
static uint32_t _dw9714_write_dac_code(cmr_handle sns_af_drv_handle,
                                       int32_t code) {
...
static int dw9714_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _dw9714_drv_power_on(*sns_af_drv_handle, AF_TRUE);
...
static int dw9714_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    cmr_int ret = AF_SUCCESS;
    CHECK_PTR(sns_af_drv_handle);
    _dw9714_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int dw9714_drv_ioctl(cmr_handle sns_af_drv_handle, enum sns_cmd cmd,
                            void *param) {
    uint32_t ret_value = AF_SUCCESS;
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
...
static int _dw9714_drv_power_on(cmr_handle sns_af_drv_handle,
                                uint16_t power_on) {
    CHECK_PTR(sns_af_drv_handle);
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
    ...
    if (AF_TRUE == power_on) {
        hw_sensor_set_monitor_val(af_drv_cxt->hw_handle,
                                  dw9714_drv_entry.motor_avdd_val);
        usleep(DW9714_POWERON_DELAY * 1000);
    } else {
