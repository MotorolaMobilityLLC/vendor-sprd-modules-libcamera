...
LOCAL cmr_uint _ov5640_GetResolutionTrimTab(cmr_uint param);
LOCAL cmr_uint _ov5640_PowerOn(cmr_uint power_on);
LOCAL cmr_uint _ov5640_Identify(cmr_uint param);
LOCAL cmr_uint _ov5640_set_brightness(cmr_uint level);
LOCAL cmr_uint _ov5640_set_contrast(cmr_uint level);
LOCAL cmr_uint _ov5640_set_saturation(cmr_uint level);
LOCAL cmr_uint _ov5640_set_image_effect(cmr_uint effect_type);
LOCAL cmr_uint _ov5640_set_ev(cmr_uint level);
LOCAL cmr_uint _ov5640_set_anti_flicker(cmr_uint mode);
LOCAL cmr_uint _ov5640_set_video_mode(cmr_uint mode);
LOCAL cmr_uint _ov5640_set_awb(cmr_uint mode);
LOCAL cmr_uint _ov5640_set_work_mode(cmr_uint mode);
LOCAL cmr_uint _ov5640_BeforeSnapshot(cmr_uint param);
LOCAL cmr_uint _ov5640_check_image_format_support(cmr_uint param);
LOCAL cmr_uint _ov5640_pick_out_jpeg_stream(cmr_uint param);
LOCAL cmr_uint _ov5640_after_snapshot(cmr_uint param);
LOCAL cmr_uint _ov5640_GetExifInfo(cmr_uint param);
LOCAL cmr_uint _ov5640_ExtFunc(cmr_uint ctl_param);
LOCAL cmr_uint _ov5640_check_status(cmr_uint param);
LOCAL uint32_t _ov5640_get_cur_af_gain(void);
LOCAL cmr_uint _ov5640_set_iso(cmr_uint mode);
LOCAL uint32_t _ov5640_ReadGain(uint32_t param);
LOCAL cmr_uint _ov5640_flash(cmr_uint param);
LOCAL int _ov5640_init_firmware(void);
LOCAL cmr_uint _ov5640_StreamOn(cmr_uint param);
LOCAL cmr_uint _ov5640_StreamOff(cmr_uint param);
...
LOCAL cmr_uint _ov5640_GetResolutionTrimTab(cmr_uint param)
{
	return (cmr_uint) s_ov5640_Resolution_Trim_Tab;
}
LOCAL cmr_uint _ov5640_PowerOn(cmr_uint power_on)
{
	...
	SENSOR_LOGI("dvdd_val %ld, dvdd_val %d, avdd_val %d, iovdd_val %d",
			power_on,
			dvdd_val,
			avdd_val,
			iovdd_val);
...
LOCAL cmr_uint _ov5640_set_image_effect(cmr_uint effect_type)
{
	...
	SENSOR_LOGI("0x%02x,data=0x%x", effect_type, reg_value);
	return 0;
}
...
LOCAL cmr_uint _ov5640_set_awb(cmr_uint mode)
{
	...
	SENSOR_LOGI("SENSOR: _ov5640_set_awb = 0x%02x,reg_value=0x%x", mode, reg_value);
	return 0;
}
...
LOCAL cmr_uint _ov5640_set_work_mode(cmr_uint mode)
{
	...
	SENSOR_LOGI("SENSOR: _ov5640_set_work_mode: mode = %ld", mode);
	return 0;
}
...
LOCAL int OV5640_set_AE_target(int target)
{
	...
	SENSOR_LOGI("Target ae %d, AE high %d, AE low %d", ae_target, ae_high, ae_low);
	return 0;
}
...
LOCAL int OV5640_capture(uint32_t param)
{
	...
	SENSOR_LOGI("before, ae_ag_ctrl 0x%x", ae_ag_ctrl);
	ae_ag_ctrl = ae_ag_ctrl | 0x03;
	Sensor_WriteReg(0x3503, ae_ag_ctrl);
	SENSOR_LOGI("after, ae_ag_ctrl 0x%x", ae_ag_ctrl);
	...
	SENSOR_LOGI("preview_shutter %d, preview_gain16 %d",
		preview_shutter,
		preview_gain16);
	...
	SENSOR_LOGI("capture_VTS %d, capture_HTS %d, capture_sysclk %d",
		capture_VTS,
		capture_HTS,
		capture_sysclk);
	...
	SENSOR_LOGI("light_frequency %d, capture_bandingfilter %d, capture_max_band %d",
		light_frequency,
	...
	SENSOR_LOGI("capture_gain16_shutter %d", (int)capture_gain16_shutter);
	// gain to shutter
	...
	SENSOR_LOGI("capture_shutter %d, capture_gain16 %d",
		capture_shutter,
		capture_gain16);
...
LOCAL cmr_uint _ov5640_pick_out_jpeg_stream(cmr_uint param)
{
	...
	SENSOR_LOGI("SENSOR: ov5640 jpeg capture head: 0x%x, 0x%x \n",
...
#define TAIL_VAL 0xffd9
	...
	if (i < buf_len) {
		...
		SENSOR_LOGI("SENSOR: ov5640 can not find the jpeg tail: %d \n",
			     i);
		i = 0x00;
	}
...
LOCAL uint32_t _ov5640_chang_image_format(uint32_t param)
{
	...
	switch (param) {
	case SENSOR_IMAGE_FORMAT_YUV422:
		SENSOR_LOGI("SENSOR: ov5640  chang_image_format  YUV422 \n");
		ret_val = Sensor_SendRegTabToSensor(&st_yuv422_reg_table_info);
		break;

	case SENSOR_IMAGE_FORMAT_JPEG:
		SENSOR_LOGI("SENSOR: ov5640  chang_image_format  jpg \n");
...
LOCAL uint32_t _ov5640_AutoFocusTrig(SENSOR_EXT_FUN_PARAM_T_PTR param_ptr)
{
	...
	do {
		if (0x00 == i) {
			...
			SENSOR_LOGI("_ov5640_AutoFocusTrig af will quit!");
			af_quit_flag = 0;
			break;
		}
		usleep(10*1000);
...
LOCAL uint32_t _ov5640_AutoFocusZone(SENSOR_EXT_FUN_PARAM_T_PTR param_ptr)
{
	...
	SENSOR_LOGI("%d, %d", param_ptr->zone[0].x, param_ptr->zone[0].y);
	SENSOR_LOGI("0x3001=0x%x,0x3005=0x%x.",
		Sensor_ReadReg(0x3001),Sensor_ReadReg(0x3005));
	SENSOR_LOGI("0x3000=0x%x,0x3004=0x%x,0x3f00=0x%x.",
	...
	if (SENSOR_SUCCESS == rtn) {
		...
		do {
			if (0x00 == i) {
				...
				SENSOR_LOGI("af will be quit!");
				af_quit_flag = 0;
				break;
			}
			usleep(10*1000);
			reg_value = Sensor_ReadReg(CMD_ACK);
			SENSOR_LOGI("cycles %d, reg %d", i, reg_value);
			i--;
		} while (0x00 != reg_value);
		...
		SENSOR_LOGI("0x3029 = %x", reg_value);*/
	}
	if (SENSOR_SUCCESS == rtn)
		SENSOR_LOGI("success!");
	return rtn;
...
LOCAL uint32_t _ov5640_CheckAFGain(SENSOR_EXT_FUN_PARAM_T_PTR param_ptr)
{
	...
	SENSOR_LOGI("delta_gain %d, s_is_dv_mode %d", delta_gain, s_is_dv_mode);
...
LOCAL uint32_t _ov5640_StartAutoFocus(cmr_uint param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	SENSOR_EXT_FUN_PARAM_T_PTR ext_ptr = (SENSOR_EXT_FUN_PARAM_T_PTR) param;
	SENSOR_LOGI("SENSOR: _ov5640_StartAutoFocus param =%d", ext_ptr->param);
...
LOCAL uint32_t _ov5640_QuitAutoFocus(void)
{
	...
	af_quit_flag = 0x1;
	SENSOR_LOGI("af quit flag setted!");
	return ret;
}
...
LOCAL uint32_t _ov5640_ExposureAuto(void)
{
	...
	SENSOR_LOGI("SENSOR: _ov5640_ExposureAuto");
...
LOCAL uint32_t _ov5640_ExposureZone(SENSOR_EXT_FUN_T_PTR param_ptr)
{
	...
	SENSOR_LOGI("%d, %d", ext_ptr->zone.x, ext_ptr->zone.y);
...
LOCAL uint32_t _ov5640_StartExposure(cmr_uint param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	SENSOR_EXT_FUN_T_PTR ext_ptr = (SENSOR_EXT_FUN_T_PTR) param;

	SENSOR_LOGI("SENSOR: _ov5640_StartExposure param =%d",
		     ext_ptr->param);
...
{
	SENSOR_LOGI("in param capture_gain16 0x%x, capture_VTS 0x%x capture_shutter 0x%x",
				capture_gain16,
				capture_VTS,
				capture_shutter);
	// write capture gain
	...
	if (capture_shutter > (capture_VTS - 4)) {
		capture_VTS = capture_shutter + 4;
		SENSOR_LOGI("actual capture_VTS 0x%x", capture_VTS);
		OV5640_set_VTS_ori(capture_VTS);
	}
	OV5640_set_shutter(capture_shutter);
}

LOCAL uint32_t _ov5640_SetEV(cmr_uint param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	SENSOR_EXT_FUN_PARAM_T_PTR ext_ptr = (SENSOR_EXT_FUN_PARAM_T_PTR) param;
...
LOCAL uint32_t _ov5640_saveLoad_exposure(cmr_uint param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	...
	if (sl_param) {
		...
		SENSOR_LOGI("_ov5640_saveLoad_exposure load shutter 0x%x gain 0x%x",
		...
		SENSOR_LOGI("_ov5640_saveLoad_exposure save shutter 0x%x gain 0x%x",
					s_ov5640_shutter_bak,
					s_ov5640_gain_bak);
	}
	return rtn;
...
LOCAL int _ov5640_init_firmware(void)
{
	...
	SENSOR_LOGI("init firmware start!");
	...
	for (i = 0; i < 4; i++) {
		...
		if (ret != 0) {
			SENSOR_LOGI("write sensor reg fai, ret : %d", ret);
			continue;
		} else {
			ret = 0;
			break;
...
LOCAL void OV5640_set_bandingfilter(void)
{
	...
	SENSOR_LOGI("preview_sysclk %d, preview_hts %d",
		preview_sysclk,
		preview_hts);
	// read preview VTS
...
LOCAL cmr_uint _ov5640_set_iso(cmr_uint mode)
{
	...
	SENSOR_LOGI("read 0x%x,0x%x", Sensor_ReadReg(0x3a18),Sensor_ReadReg(0x3a19));
	return 0;
}
...
LOCAL cmr_uint _ov5640_flash(cmr_uint param)
{
	...
	if (autoflash) {
		...
		SENSOR_LOGI("  NULL pointer error! ");
		return SENSOR_FAIL;
	}

	SENSOR_LOGI(" value = %d, autoflash = 0x%x, auto_flash_mode",  value, *autoflash);
	return SENSOR_SUCCESS;
}
