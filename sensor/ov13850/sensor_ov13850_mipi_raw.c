...
LOCAL uint32_t _ov13850_read_otp_gain(uint32_t *param);
LOCAL uint32_t _ov13850_write_otp_gain(uint32_t *param);
LOCAL unsigned long _ov13850_access_val(unsigned long param);
static unsigned long _ov13850_GetExifInfo(unsigned long param);
...
#define param_update(x1,x2) sprintf(name,"/data/s5k4h5yc_%s.bin",x1);\
...
LOCAL uint32_t Sensor_ov13850_InitRawTuneInfo(void)
{
	uint32_t rtn=0x00;
#if 0
	isp_raw_para_update_from_file(&g_ov13850_mipi_raw_info,0);
...
LOCAL unsigned long _ov13850_access_val(unsigned long param)
{
	...
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_SHUTTER:
			*((uint32_t*)param_ptr->pval) = _ov13850_get_shutter();
			break;
		case SENSOR_VAL_TYPE_READ_OTP_GAIN:
			rtn = _ov13850_read_otp_gain(param_ptr->pval);
			break;
		default:
			break;
	}
...
LOCAL unsigned long _ov13850_write_exposure(unsigned long param)
{
	...
	linetime=s_ov13850_Resolution_Trim_Tab[size_index].line_time;
