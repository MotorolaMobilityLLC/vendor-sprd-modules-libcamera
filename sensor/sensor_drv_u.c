...
static cmr_int sns_cfg_otp_update_isparam(struct sensor_drv_context *sensor_cxt, cmr_u32 sensor_id, int cmd, void* data);
...
static cmr_int sensor_cfg_otp_update_isparam(struct sensor_drv_context *sensor_cxt,
						cmr_u32 sensor_id);
static cmr_int sensor_ctrl_otp_data(struct sensor_drv_context *sensor_cxt,
						cmr_u32 sensor_id,uint8_t cmd,void* data);
...
{
	return (void *)&s_local_sensor_cxt[0];
}
						...
						cmr_u32 vdd_value)
{
	...
	if (0 != ret) {
		SENSOR_LOGE("failed,  vdd_value = %d, ret=%ld ",
			vdd_value, ret);
		ret = -1;
	}
				...
				cmr_u8 *buff, cmr_u32 size)
{
	...
	if (0 != ret) {
		SENSOR_LOGE("failed,  buff[0] = %d, size=%d, ret=%ld ", buff[0], size, ret);
		ret = -1;
	}
...
cmr_int sns_dev_set_i2c_addr(struct sensor_drv_context *sensor_cxt)
{
	...
	if (0 != ret) {
		SENSOR_LOGE("failed,  addr = 0x%x, ret=%ld ",
			sensor_cxt->i2c_addr,
			ret);
		ret = -1;
	}
 ...
 * cmd_length: the high 16 bits is read data length and the low 16 bits is register length
...
cmr_int hw_Sensor_WriteI2C(SENSOR_HW_HANDLE handle, cmr_u16 slave_addr, cmr_u8 *cmd, cmr_u16 cmd_length)
{
	...
	SENSOR_LOGV("slave_addr=0x%x, ptr=0x%p, count=%d",
		i2c_tab.slave_addr, i2c_tab.i2c_data, i2c_tab.i2c_count);
...
cmr_int hw_sensor_init_otp_module(SENSOR_HW_HANDLE handle)
{
	...
	if (module && (module->otp_drv_info) && (!sensor_cxt->sensor_otp_cxt)){
		otp_ctrl_cxt_t *ctr_cxt = malloc(sizeof(otp_ctrl_cxt_t)); /*malloc a otp handle*/
		ctr_cxt->otp_params.buffer = NULL;
		ctr_cxt->otp_data = NULL;
		ctr_cxt->otp_drv_cxt = module->otp_drv_info; /*save otp driver context*/
		sensor_cxt->sensor_otp_cxt = ctr_cxt;
	} else {
		SENSOR_LOGE("error:otp driver module:0x%x,otp_drv:0x%x",module,module->otp_drv_info);
	}
	SENSOR_LOGI("out");
	return ret;
}

cmr_int hw_sensor_deinit_otp_module(SENSOR_HW_HANDLE handle,cmr_u32 sensor_id)
{
	cmr_int ret;
	if (NULL == handle || NULL == handle->privatedata)
		return SENSOR_CTX_ERROR;
	...
	if (module && module->otp_drv_info && sensor_cxt->sensor_otp_cxt)
		sensor_ctrl_otp_data(sensor_cxt,sensor_id,RELEASE_OTP_DATA,NULL); /*releas otp data*/
	if (sensor_cxt->sensor_otp_cxt){
		free(sensor_cxt->sensor_otp_cxt);
		sensor_cxt->sensor_otp_cxt = NULL;
	} else {
		SENSOR_LOGE("Otp_module is null ,no need release otp_module");
	}
	SENSOR_LOGI("out");
	return ret;
}

cmr_int hw_sensor_init_af_dev(SENSOR_HW_HANDLE handle)
{
	cmr_int ret;
	if (NULL == handle || NULL == handle->privatedata)
		return SENSOR_CTX_ERROR;
...
static cmr_int sns_grc_read_i2c_Ex(cmr_u16 slave_addr, cmr_u16 addr, cmr_u16 *reg, cmr_int bits, cmr_u32 camera_id)
{
	cmr_int ret = -SENSOR_FAIL;
	cmr_u8 cmd[2] = { 0x00 };
	struct sensor_i2c_tag i2c_tab;
	struct sensor_drv_context *sensor_cxt = (struct sensor_drv_context *)sensor_get_dev_cxt_Ex(camera_id);
...
static cmr_int sns_grc_write_i2c(struct sensor_drv_context *sensor_cxt, cmr_u16 slave_addr, cmr_u16 addr, cmr_u16 reg, cmr_int bits)
{
	cmr_int ret = -SENSOR_FAIL;
	cmr_u8 cmd[4] = { 0x00 };
	struct sensor_i2c_tag i2c_tab;
...
static cmr_int sns_grc_write_i2c_Ex(cmr_u16 slave_addr, cmr_u16 addr, cmr_u16 reg, cmr_int bits, cmr_u32 camera_id)
{
	cmr_int ret = -SENSOR_FAIL;
	cmr_u8 cmd[4] = { 0x00 };
	struct sensor_i2c_tag i2c_tab;
...
cmr_int hw_Sensor_SetVoltage(SENSOR_HW_HANDLE handle, SENSOR_AVDD_VAL_E dvdd_val, SENSOR_AVDD_VAL_E avdd_val,
			SENSOR_AVDD_VAL_E iodd_val)
{
	...
	SENSOR_LOGI("avdd_val = %d,  dvdd_val=%d, iodd_val=%d ", avdd_val, dvdd_val, iodd_val);
...
void Sensor_PowerOn(struct sensor_drv_context *sensor_cxt, cmr_u32 power_on)
{
	...
	SENSOR_LOGI("power_on = %d, power_down_level = %d, avdd_val = %d",
...
void Sensor_SetExportInfo(struct sensor_drv_context *sensor_cxt)
{
	...
	//SENSOR_MATCH_T               *module = container_of(sensor_cxt->sensor_info_ptr,SENSOR_MATCH_T,sensor_info);
	SENSOR_MATCH_T *module = sensor_cxt->module_cxt;
	if (PNULL == sensor_cxt->sensor_info_ptr) {
	...
	if (PNULL != sensor_info_ptr->ioctl_func_tab_ptr->get_trim) {
		...
		resolution_trim_ptr =
			(SENSOR_TRIM_T_PTR)sensor_info_ptr->ioctl_func_tab_ptr->get_trim(sensor_cxt->sensor_hw_handler, 0x00);
	}
	for (i = SENSOR_MODE_COMMON_INIT; i < SENSOR_MODE_MAX; i++) {
		...
		if ((NULL != exp_info_ptr)
			...
			if(module && module->af_dev_info){
				exp_info_ptr->raw_info_ptr->ioctrl_ptr->caller_handler = sensor_cxt->sensor_hw_handler;
				exp_info_ptr->raw_info_ptr->ioctrl_ptr->set_pos = module->af_dev_info->af_ops.set_motor_pos;
				exp_info_ptr->raw_info_ptr->ioctrl_ptr->get_otp = NULL;
				exp_info_ptr->raw_info_ptr->ioctrl_ptr->get_motor_pos = module->af_dev_info->af_ops.get_motor_pos;
				exp_info_ptr->raw_info_ptr->ioctrl_ptr->set_motor_bestmode = module->af_dev_info->af_ops.set_motor_bestmode;
				exp_info_ptr->raw_info_ptr->ioctrl_ptr->get_test_vcm_mode = module->af_dev_info->af_ops.get_test_motor_mode;
				exp_info_ptr->raw_info_ptr->ioctrl_ptr->set_test_vcm_mode = module->af_dev_info->af_ops.set_test_motor_mode;
			} else {
				CMR_LOGE("AF device driver has problem,please double check it.module:0x%x,af_dev:0x%x",module,module?module->af_dev_info:NULL);
			}
#endif
...
cmr_int hw_Sensor_WriteReg(SENSOR_HW_HANDLE handle, cmr_u16 subaddr, cmr_u16 data)
{
	...
	if (PNULL != write_reg_func) {
		if (SENSOR_OP_SUCCESS != write_reg_func(sensor_cxt->sensor_hw_handler, (subaddr << S_BIT_4) + data)) {
...
cmr_u32 hw_Sensor_ReadReg(SENSOR_HW_HANDLE handle, cmr_u16 reg_addr)
{
	...
	if (PNULL != read_reg_func) {
		ret_val = (cmr_u16)read_reg_func(sensor_cxt->sensor_hw_handler, (cmr_u32)(reg_addr & SENSOR_LOW_SIXTEEN_BIT));
	} else {
		SENSOR_REG_BITS_T reg;
...
cmr_int hw_Sensor_SendRegTabToSensor(SENSOR_HW_HANDLE handle, SENSOR_REG_TAB_INFO_T *sensor_reg_tab_info_ptr)
{
	...
	if (PNULL != write_reg_func) {
		for (i = 0; i < sensor_reg_tab_info_ptr->reg_count; i++) {
			...
			if (SENSOR_OP_SUCCESS != write_reg_func(sensor_cxt->sensor_hw_handler, (subaddr << S_BIT_4) + data))
				SENSOR_LOGI("SENSOR: IIC write : reg:0x%04x, val:0x%04x error", subaddr, data);
		}
	}else{
...
cmr_int sns_set_id(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);
	SENSOR_LOGI("id %d,is_register_sensor %ld",
...
void sns_i2c_init(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	SENSOR_LOGI("sensor_id=%d, is_register_sensor=%ld",
				sensor_id, sensor_cxt->is_register_sensor);
}
...
cmr_u32 sns_identify_search(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	for (sensor_index = 0x00; sensor_index < valid_tab_index_max; sensor_index++) {
		...
		if (NULL==sensor_info_ptr) {
			SENSOR_LOGW("%d info of Sensor table %d is null", sensor_index, sensor_id);
			continue ;
		}
		...
		if (PNULL != sensor_info_ptr->ioctl_func_tab_ptr->identify) {
			...
			if (SENSOR_SUCCESS == sensor_info_ptr->ioctl_func_tab_ptr->identify(sensor_cxt->sensor_hw_handler,SENSOR_ZERO_I2C)) {
				...
				retValue = SCI_TRUE;
				SENSOR_LOGI("sensor_id :%d,img_sensor_num=%d",
...
cmr_int sns_register(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	if (sensor_cxt->sensor_identified && (SENSOR_ATV != sensor_id)) {
		...
		if (0xFF != sensor_index) {
			...
			sensor_info_ptr = sensor_strinfo_tab_ptr[sensor_index].sensor_info;
			if (NULL == sensor_info_ptr) {
				SENSOR_LOGE("index %d info of Sensor table %d is null", sensor_index, sensor_id);
				return SENSOR_FAIL;
			}

			if (NULL == sensor_info_ptr->raw_info_ptr) {
				SENSOR_LOGE("index %d info of Sensor table %d raw_info_ptr  is null", sensor_index, sensor_id);
				return SENSOR_FAIL;
			}
			sensor_cxt->sensor_info_ptr = sensor_info_ptr;
			sensor_cxt->sensor_list_ptr[sensor_id] = sensor_info_ptr;
...
void sns_load_sensor_type(struct sensor_drv_context *sensor_cxt)
{
	...
	if(NULL == fp){
		...
		if(NULL == fp){
			SENSOR_LOGE("sns_load_sensor_type: file %s open error:%s", SENSOR_PARA, strerror(errno));
		}
		cmr_bzero(&sensor_param[0], SENSOR_PARAM_NUM);
	}else{
		len = fread(sensor_param, 1, SENSOR_PARAM_NUM, fp);
		SENSOR_LOGI("rd sns param len %d %x,%x,%x,%x,%x,%x,%x,%x ",
						...
						SENSOR_ID_E sensor_id)
{
	...
	while(1) {
		...
		if (file_size != raw_fix_info_ptr->lnc.map[i-1][0].len) {
			SENSOR_LOGI("file size dis-match, do not replace, w:%d, h:%d, ori: %d, now:%d/n",
...
cmr_int sns_ctrl_thread_proc(struct cmr_msg *message, void *p_data)
{
	...
	case SENSOR_CTRL_EVT_CFGOTP:
		...
		sns_cfg_otp_update_isparam(sensor_cxt, camera_id, otp_ctrl_data->cmd, otp_ctrl_data->data);
		break;
...
cmr_int sns_open(struct sensor_drv_context *sensor_cxt, cmr_u32 sensor_id)
{
	...
	if (SENSOR_TRUE == sensor_register_info_ptr->is_register[sensor_id]) {
		...
		SENSOR_LOGI("3:sensor_id: %d, addr = 0x%x", sensor_id, sensor_cxt->i2c_addr);
		...
		if (SENSOR_SUCCESS == ret_val) {
			...
			//sensor_ctrl_otp_data(sensor_cxt,sensor_id,READ_FORMAT_OTP_DATA,NULL);
	...
	if (sensor_cxt->sensor_mode[snr_get_cur_id(sensor_cxt)] == mode) {
		...
		if (PNULL != sensor_cxt->sensor_info_ptr->resolution_tab_info_ptr[mode].sensor_reg_tab_ptr) {
			...
			if((SENSOR_MODE_COMMON_INIT == mode) && set_reg_tab_func){
				set_reg_tab_func(sensor_cxt->sensor_hw_handler, SENSOR_MODE_COMMON_INIT);
			}else{
				hw_Sensor_SendRegTabToSensor(sensor_cxt->sensor_hw_handler, &sensor_cxt->sensor_info_ptr->resolution_tab_info_ptr[mode]);
			}
...
cmr_int sensor_close_common(struct sensor_drv_context *sensor_cxt, cmr_u32 sensor_id)
{
	...
	sensor_register_info_ptr = &sensor_cxt->sensor_register_info;
	hw_sensor_deinit_otp_module(sensor_cxt->sensor_hw_handler,sensor_id);
...
cmr_int sensor_get_exif_common(struct sensor_drv_context *sensor_cxt, EXIF_SPEC_PIC_TAKING_COND_T **sensor_exif_info_pptr)
{
	...
	sensor_get_info_common(sensor_cxt, &sensor_info_ptr);
	if (!sensor_info_ptr) {
		SENSOR_LOGE("ZERO poiner sensor_cxt %p sensor_info_ptr %p",
			sensor_cxt,
			sensor_info_ptr);
		return SENSOR_SUCCESS;
	}
	...
	if (PNULL != sensor_info_ptr->ioctl_func_ptr->get_exif) {
		*sensor_exif_info_pptr =
			...
			sensor_info_ptr->ioctl_func_ptr->get_exif(sensor_cxt->sensor_hw_handler, 0x00);
...
cmr_int sensor_ctrl_otp_data(struct sensor_drv_context *sensor_cxt,
                                  cmr_u32 sensor_id,uint8_t cmd,void* data)
{
	CMR_MSG_INIT(message);
	cmr_int ret = CMR_CAMERA_SUCCESS;
...
cmr_int sns_cfg_otp_update_isparam(struct sensor_drv_context *sensor_cxt,
						cmr_u32 sensor_id, int cmd, void* data)
{
	ATRACE_BEGIN(__FUNCTION__);
	...
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);
	if(module && module->otp_drv_info && sensor_cxt->sensor_otp_cxt){
		ret = sensor_otp_process(sensor_cxt->sensor_hw_handler,cmd,data);
	} else {
		if ((NULL != sensor_cxt->sensor_info_ptr)
				&& (NULL != sensor_cxt->sensor_info_ptr->ioctl_func_tab_ptr)){
