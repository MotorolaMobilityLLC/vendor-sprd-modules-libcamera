 ...
 **/
static cmr_int _s5k5e8yx_jd_section_checksum(cmr_u8 *buf, cmr_uint offset,
                                             cmr_uint data_count,
...
static cmr_int _s5k5e8yx_jd_parse_module_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
static cmr_int _s5k5e8yx_jd_parse_af_data(cmr_handle otp_drv_handle) {
    ...
    ret = _s5k5e8yx_jd_section_checksum(
...
static int _s5k5e8yx_jd_parse_ae_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
static cmr_int s5k5e8yx_jd_otp_drv_read(cmr_handle otp_drv_handle,
    ...
    if (!otp_raw_data->buffer) {
        ...
        _s5k5e8yx_jd_buffer_init(otp_drv_handle);
    }
...
static cmr_int s5k5e8yx_jd_otp_drv_parse(cmr_handle otp_drv_handle,
    ...
    otp_base_info_cfg_t *base_info =
        &(s5k5e8yx_jd_otp_entry.otp_cfg.base_info_cfg);
    otp_params_t *otp_raw_data = &(otp_cxt->otp_raw_data);
    ...
    if (sensor_otp_get_buffer_state(otp_cxt->sensor_id)) {
        ...
        _s5k5e8yx_jd_parse_module_data(otp_drv_handle);
        _s5k5e8yx_jd_parse_af_data(otp_drv_handle);
        _s5k5e8yx_jd_parse_awb_data(otp_drv_handle);
        _s5k5e8yx_jd_parse_lsc_data(otp_drv_handle);
        _s5k5e8yx_jd_parse_pdaf_data(otp_drv_handle);
        _s5k5e8yx_jd_parse_ae_data(otp_drv_handle);
...
static cmr_int s5k5e8yx_jd_otp_drv_calibration(cmr_handle otp_drv_handle) {
    ...
    otp_calib_items_t *cali_items = &(s5k5e8yx_jd_otp_entry.otp_cfg.cali_items);

    if (cali_items) {
        if (cali_items->is_pdafc && cali_items->is_pdaf_self_cal)
            _s5k5e8yx_jd_pdaf_calibration(otp_drv_handle);
        /*calibration at sensor or isp */
        if (cali_items->is_awbc && cali_items->is_awbc_self_cal)
            _s5k5e8yx_jd_awb_calibration(otp_drv_handle);
        if (cali_items->is_lsc && cali_items->is_awbc_self_cal)
            _s5k5e8yx_jd_lsc_calibration(otp_drv_handle);
    }
...
static cmr_int s5k5e8yx_jd_compatible_convert(cmr_handle otp_drv_handle,
                                              void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGD("in");
...
static cmr_int s5k5e8yx_jd_otp_drv_ioctl(cmr_handle otp_drv_handle,
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        s5k5e8yx_jd_compatible_convert(otp_drv_handle, params);
        break;
    default:
        break;
    }
