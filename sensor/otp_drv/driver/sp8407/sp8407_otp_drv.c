#include "sp8407_otp_drv.h"

/** sp8407_section_checksum:
 ...
 **/
static cmr_int _sp8407_section_checksum(cmr_u8 *buf, cmr_uint offset,
                                        cmr_uint data_count,
...
static cmr_int _sp8407_buffer_init(cmr_handle otp_drv_handle) {
...
static cmr_int _sp8407_parse_module_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
    ...
    cmr_u32 module_id, calib_version, year, month, day;
    ...
    module_id = (module_info[1] << 16) | (module_info[2] << 8) | module_info[3];
    calib_version = (module_info[4] << 8) | module_info[5];
    year = module_info[6];
    month = module_info[7];
    day = module_info[8];
    OTP_LOGI("module_id = 0x%x, calibration_version = %d, calibration_date = "
             "%d-%d-%d",
             module_id, calib_version, year, month, day);

    return ret;
}
static cmr_int _sp8407_parse_af_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    ...
    ret = _sp8407_section_checksum(otp_cxt->otp_raw_data.buffer, AF_INFO_OFFSET,
    ...
    if (OTP_CAMERA_SUCCESS != ret) {
        ...
        OTP_LOGI("AF otp checksum passed");
    }
    cmr_u8 *af60_poss_dat = otp_cxt->otp_raw_data.buffer + AF_60CM_POS;
    af_cali_dat->gld_info.buffer = af60_poss_dat;
    af_cali_dat->gld_info.size = 2;

    return ret;
}

static cmr_int _sp8407_parse_awb_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    ...
    if (OTP_CAMERA_SUCCESS != ret) {
        ...
        OTP_LOGI("AWB otp checksum passed");
    }
...
static cmr_int _sp8407_parse_lsc_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
    ...
    if (OTP_CAMERA_SUCCESS != ret) {
        ...
        OTP_LOGI("LSC otp checksum passed");
    }
...
static cmr_int _sp8407_parse_pdaf_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static int _sp8407_parse_ae_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
static cmr_int _sp8407_parse_dualcam_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static cmr_int _sp8407_awb_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGV("in");
    CHECK_PTR(otp_drv_handle);
    ...
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
    /*
        otp_calib_items_t *cal_items = &(sp8407_otp_entry.otp_cfg.cali_items);
...
static cmr_int _sp8407_lsc_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGV("in");
    CHECK_PTR(otp_drv_handle);
...
static cmr_int _sp8407_pdaf_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGV("in");
    CHECK_PTR(otp_drv_handle);
...
static cmr_int sp8407_otp_drv_create(otp_drv_init_para_t *input_para,
                                     cmr_handle *sns_af_drv_handle) {
    return sensor_otp_drv_create(input_para, sns_af_drv_handle);
}

static cmr_int sp8407_otp_drv_delete(cmr_handle otp_drv_handle) {
    return sensor_otp_drv_delete(otp_drv_handle);
}

static cmr_int sp8407_otp_drv_read(cmr_handle otp_drv_handle, void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    cmr_u8 cmd_val[3];
    cmr_uint i = 0;
    char value[255];
    ...
    if (!otp_raw_data->buffer) {
        ...
        _sp8407_buffer_init(otp_drv_handle);
    }
...
static cmr_int sp8407_otp_drv_write(cmr_handle otp_drv_handle, void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    CHECK_PTR(p_data);
    OTP_LOGV("in");
...
static cmr_int sp8407_otp_drv_parse(cmr_handle otp_drv_handle, void *params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;

    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
    otp_base_info_cfg_t *base_info = &(sp8407_otp_entry.otp_cfg.base_info_cfg);
    otp_params_t *otp_raw_data = &(otp_cxt->otp_raw_data);
    ...
    if (sensor_otp_get_buffer_state(otp_cxt->sensor_id)) {
        ...
        _sp8407_parse_module_data(otp_drv_handle);
        _sp8407_parse_af_data(otp_drv_handle);
        _sp8407_parse_awb_data(otp_drv_handle);
        _sp8407_parse_lsc_data(otp_drv_handle);
        _sp8407_parse_pdaf_data(otp_drv_handle);
        _sp8407_parse_ae_data(otp_drv_handle);
        _sp8407_parse_dualcam_data(otp_drv_handle);
        ...
        if ((base_info->compress_flag != GAIN_ORIGIN_BITS) &&
            ...
            ret = sensor_otp_lsc_decompress(
                &sp8407_otp_entry.otp_cfg.base_info_cfg,
...
static cmr_int sp8407_otp_drv_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;

    otp_calib_items_t *cali_items = &(sp8407_otp_entry.otp_cfg.cali_items);

    if (cali_items) {
        if (cali_items->is_pdafc && cali_items->is_pdaf_self_cal)
            _sp8407_pdaf_calibration(otp_drv_handle);
        /*calibration at sensor or isp */
        if (cali_items->is_awbc && cali_items->is_awbc_self_cal)
            _sp8407_awb_calibration(otp_drv_handle);
        if (cali_items->is_lsc && cali_items->is_awbc_self_cal)
            _sp8407_lsc_calibration(otp_drv_handle);
    }
...
static cmr_int sp8407_compatible_convert(cmr_handle otp_drv_handle,
                                         void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
/*just for expend*/
static cmr_int sp8407_otp_drv_ioctl(cmr_handle otp_drv_handle, cmr_uint cmd,
                                    void *params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        sp8407_compatible_convert(otp_drv_handle, params);
        break;
    default:
        break;
    }
