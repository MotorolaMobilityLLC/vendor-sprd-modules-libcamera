...
static int _ov13855_sunny_awb_calibration(void *otp_drv_handle) {
  ...
  otp_calib_items_t *cal_items = &(ov13855_sunny_drv_entry.otp_cfg.cali_items);
  awbcalib_data_t *awb_cali_dat = &(otp_cxt->otp_data->awb_cali_dat);
...
static int ov13855_sunny_otp_drv_parse(void *otp_drv_handle, void *params) {
  ...
  if (ret == OTP_CAMERA_SUCCESS) {
     cmr_int  min_buf_len =MIN(OTP_LEN,otp_cxt->otp_data_len);
    ...
    if (!otp_raw_data->buffer) {
       ...
       OTP_LOGI("otp_raw_data->buffer is %d,%p,%p",min_buf_len,otp_raw_data->buffer,otp_cxt->otp_data);
       if(otp_raw_data->buffer && otp_cxt->otp_data)
           memcpy((void *)otp_raw_data->buffer,(void *)otp_cxt->otp_data,min_buf_len);
    }
...
static int ov13855_sunny_otp_drv_calibration(void *otp_drv_handle) {
  ...
  otp_calib_items_t *cali_items = &(ov13855_sunny_drv_entry.otp_cfg.cali_items);
...
static int _ov13855_sunny_compatible_convert(otp_drv_cxt_t *otp_drv_handle,
  ...
  /*module data*/
  convert_data->dual_otp.master_module_info.year = format_data->module_dat.year;
  ...
  /*other*/
  convert_data->single_otp.program_flag = format_data->extend_dat.program_flag;
