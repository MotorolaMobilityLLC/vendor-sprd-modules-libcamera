...
/** ov5675_sunny_section_data_checksum:
...
static int _ov5675_sunny_parse_checksum(void *otp_drv_handle) {
    uint32_t ret = OTP_CAMERA_SUCCESS;
    unsigned int i = 0, sum = 0;
    OTP_LOGI("in");
    ...
    ret = _ov5675_sunny_section_checksum(
        ...
        AE_INFO_END_OFFSET - AE_INFO_OFFSET, AE_INFO_CHECKSUM);
...
static int _ov5675_sunny_buffer_init(void *otp_drv_handle) {
    ...
    otp_len = sizeof(otp_format_data_t) + LSC_FORMAT_SIZE + RESERVE_DATA_SIZE;
...
static int _ov5675_sunny_parse_ae_data(void *otp_drv_handle) {
    ...
    uint8_t *ae_src_dat = otp_cxt->otp_raw_data.buffer + AE_INFO_OFFSET;
    /*ae data*/
    ret = _ov5675_sunny_section_checksum(
        otp_cxt->otp_raw_data.buffer, AE_INFO_OFFSET,
        AE_INFO_END_OFFSET - AE_INFO_OFFSET, AE_INFO_CHECKSUM);
...
static int _ov5675_sunny_parse_lsc_data(void *otp_drv_handle) {
    ...
    /*gold data*/
    uint8_t *buffer = malloc(LSC_INFO_CHECKSUM - LSC_INFO_OFFSET);
    if (NULL == buffer) {
        OTP_LOGE("malloc otp data buffer failed.\n");
        ret = OTP_CAMERA_FAIL;
        return ret;
        ...
        memset(buffer, 0, LSC_INFO_CHECKSUM - LSC_INFO_OFFSET);
    }
    memcpy(buffer, golden_lsc,
           sizeof(golden_lsc)); // LSC_INFO_CHECKSUM - LSC_INFO_OFFSET
    lsc_dst->lsc_calib_golden.length = LSC_INFO_CHECKSUM - LSC_INFO_OFFSET;
    gld_dst = buffer;
...
#if 0
static int _ov5675_sunny_parse_reserve_data(void *otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static int ov5675_sunny_otp_drv_read(void *otp_drv_handle, otp_params_t *p_data)
...
static int _ov5675_sunny_parse_module_data(void *otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
...
static int _ov5675_sunny_split_data(void *otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI(" split in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
    _ov5675_sunny_parse_module_data(otp_drv_handle);
    //  _ov5675_sunny_parse_af_data(otp_drv_handle);
    ...
    // _ov5675_sunny_parse_reserve_data(otp_drv_handle);

    return ret;
};
static int ov5675_sunny_otp_drv_parse(void *otp_drv_handle, void *params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
    ...
    ret = sensor_otp_rawdata_from_file(OTP_READ_FORMAT_FROM_BIN,
                                       otp_cxt->dev_name, &otp_data, OTP_LEN);
    if (ret == OTP_CAMERA_SUCCESS) {
        ...
        if (otp_raw_data->buffer) {
            memcpy((void *)otp_raw_data->buffer, otp_data, OTP_LEN);
        }

        if (!otp_cxt->otp_data) {
            otp_len =
                sizeof(otp_format_data_t) + LSC_FORMAT_SIZE + RESERVE_DATA_SIZE;
        ...
        _ov5675_sunny_split_data(otp_drv_handle);
        return ret;
    } else if (otp_raw_data->buffer) {
        /*begain read raw data, save module info */
        OTP_LOGI("drver has read otp raw data,start parsed.");
        ret = _ov5675_sunny_parse_checksum(otp_drv_handle);
        if (ret != OTP_CAMERA_SUCCESS) {
...
static int _ov5675_sunny_compatible_convert(otp_drv_cxt_t *otp_drv_handle,
                                            void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
    ...
    /*otp raw data*/
    convert_data->total_otp.sub_data_ptr = otp_cxt->otp_raw_data.buffer;
    convert_data->total_otp.sub_size = otp_cxt->otp_raw_data.num_bytes;
    /*module data*/
...
static int ov5675_sunny_otp_drv_ioctl(otp_drv_cxt_t *otp_drv_handle, int cmd,
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        _ov5675_sunny_compatible_convert(otp_drv_handle, params);
        break;
    default:
        break;
    }
