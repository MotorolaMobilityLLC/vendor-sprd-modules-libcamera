...
static int ov5675_sunny_otp_drv_parse(void *otp_drv_handle, void *params) {
  ...
  ret =
      sensor_otp_rw_data_from_file(OTP_READ_FORMAT_FROM_BIN, otp_cxt->dev_name,
                                   &otp_cxt->otp_data, &otp_cxt->otp_data_len);
  if (ret == OTP_CAMERA_SUCCESS) {
  ...
  ret =
      sensor_otp_rw_data_from_file(OTP_WRITE_FORMAT_TO_BIN, otp_cxt->dev_name,
                                   &otp_cxt->otp_data, &otp_cxt->otp_data_len);
  OTP_LOGI("out");
  return ret;
}
...
static int ov5675_sunny_otp_drv_calibration(void *otp_drv_handle) {
  ...
  otp_calib_items_t *cali_items = &(ov5675_sunny_drv_entry.otp_cfg.cali_items);
...
static int _ov5675_sunny_compatible_convert(otp_drv_cxt_t *otp_drv_handle,
  ...
  /*module data*/
  convert_data->dual_otp.slave_module_info.year = format_data->module_dat.year;
