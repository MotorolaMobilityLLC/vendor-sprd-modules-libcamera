...
static cmr_int _ov13855_altek_buffer_init(cmr_handle otp_drv_handle) {
    ...
    otp_len = sizeof(otp_format_data_t) + LSC_FORMAT_SIZE + OTP_RESERVE_BUFFER + DUAL_DATA_SIZE;
    ...
    if (NULL == otp_data) {
        ...
        lsc_data->lsc_calib_golden.length = LSC_INFO_END_OFFSET- LSC_INFO_OFFSET;
        lsc_data->lsc_calib_golden.offset = sizeof(lsccalib_data_t);

        lsc_data->lsc_calib_random.length = LSC_INFO_END_OFFSET - LSC_INFO_OFFSET;
        lsc_data->lsc_calib_random.offset = sizeof(lsccalib_data_t) + lsc_data->lsc_calib_golden.length;
    }
...
static cmr_int _ov13855_altek_parse_module_data(cmr_handle otp_drv_handle) {
	...
	/*END*/
    OTP_LOGI("moule_id:0x%x vendor_id:0x%x calib_version:%d work_stat_id:0x%x  env_record :0x%x",
...
static cmr_int _ov13855_altek_parse_awb_data(cmr_handle otp_drv_handle) {
	...
	OTP_LOGD("rdm:R/G=0x%x,Gr/Gb=0x%x,B/G=0x%x.gold:R/G=0x%x,Gr/Gb=0x%x,B/G=0x%x",
             awb_cali_dat->awb_rdm_info[0].rg_ratio, awb_cali_dat->awb_rdm_info[0].GrGb_ratio,
             awb_cali_dat->awb_rdm_info[0].bg_ratio, awb_cali_dat->awb_gld_info[0].rg_ratio,
             awb_cali_dat->awb_gld_info[0].GrGb_ratio, awb_cali_dat->awb_gld_info[0].bg_ratio);
...
static cmr_int _ov13855_altek_compatible_convert(cmr_handle otp_drv_handle,void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
	...
	convert_data->dual_otp.master_lsc_info.full_img_width =
        ov13855_altek_drv_entry.otp_cfg.base_info_cfg.full_img_width;
	convert_data->dual_otp.master_lsc_info.full_img_height =
        ov13855_altek_drv_entry.otp_cfg.base_info_cfg.full_img_height;
	convert_data->dual_otp.master_lsc_info.lsc_otp_grid =
        ov13855_altek_drv_entry.otp_cfg.base_info_cfg.lsc_otp_grid;
...
static cmr_int ov13855_altek_otp_drv_create(otp_drv_init_para_t *input_para, cmr_handle *sns_af_drv_handle){
    OTP_LOGI("in");
...
static cmr_int ov13855_altek_otp_drv_read(cmr_handle otp_drv_handle,void *p_params){
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);	
    //CHECK_PTR(p_params);
    OTP_LOGI("in");
    ...
    if (!otp_cxt->otp_raw_data.buffer) {
        ...
        buffer = sensor_otp_get_raw_buffer(OTP_RAW_DATA_LEN, otp_cxt->sensor_id);
        if (NULL == buffer) {
...
static cmr_int ov13855_altek_otp_drv_write(cmr_handle otp_drv_handle,void *p_params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    CHECK_PTR(p_params);
    OTP_LOGI("in");
...
static cmr_int ov13855_altek_otp_drv_parse(cmr_handle otp_drv_handle,void *p_params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    //CHECK_PTR(p_params);
    OTP_LOGI("in");
...
/*just for expend*/
static cmr_int ov13855_altek_otp_drv_ioctl(cmr_handle otp_drv_handle, cmr_uint cmd, void *params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
