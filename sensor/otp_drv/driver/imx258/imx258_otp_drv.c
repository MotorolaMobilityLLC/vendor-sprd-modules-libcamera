...
static int _imx258_parse_af_data(void *otp_drv_handle)
{
	...
	ret = _imx258_section_checksum(otp_cxt->otp_raw_data.buffer, AF_INFO_OFFSET, AF_INFO_SIZE - 1, AF_INFO_CHECKSUM);
...
static int _imx258_parse_awb_data(void *otp_drv_handle)
{
	...
	ret = _imx258_section_checksum(otp_cxt->otp_raw_data.buffer, AWB_INFO_OFFSET,
...
static int _imx258_parse_lsc_data(void *otp_drv_handle)
{
	...
	ret = _imx258_section_checksum(otp_cxt->otp_raw_data.buffer, OPTICAL_INFO_OFFSET,
...
static int _imx258_parse_pdaf_data(void *otp_drv_handle)
{
	...
	ret = _imx258_section_checksum(otp_cxt->otp_raw_data.buffer, PDAF_INFO_OFFSET,
	...
	if (OTP_CAMERA_SUCCESS != ret){
		...
		otp_cxt->otp_data->pdaf_cali_dat.size   = PDAF_INFO_CHECKSUM - PDAF_INFO_OFFSET;
	}
	OTP_LOGI("out");
	return ret;
}
...

{
	...
	if(!otp_cxt->otp_raw_data.buffer) {
		...
		//if(!try) {
			...
			for(i = 0; i < OTP_LEN; i++) {
				...
				hw_Sensor_ReadI2C(otp_cxt->hw_handle,GT24C64A_I2C_ADDR,(uint8_t*)&cmd_val[0],2);
				buffer[i] = cmd_val[0];
			}
		//}
	...
	sensor_otp_dump_raw_data(otp_cxt->otp_raw_data.buffer,OTP_LEN,otp_cxt->dev_name);
	OTP_LOGI("out");
	return ret;
}
...
int imx258_otp_drv_write(void *otp_drv_handle, otp_params_t* p_data)
{
	...
	if(NULL != otp_write_data->buffer){
		...
		for(i=0; i<otp_write_data->num_bytes; i++){
			hw_Sensor_WriteI2C(otp_cxt->hw_handle,GT24C64A_I2C_ADDR, &otp_write_data->buffer[i],2);
		}
...
int imx258_otp_drv_parse(void *otp_drv_handle, void* params)
{
	...
	if(ret == OTP_CAMERA_SUCCESS) {
		...
		module_info = (module_info_t *)(otp_cxt->otp_raw_data.buffer + MODULE_INFO_OFFSET);
		...
		{
			ret = sensor_otp_lsc_decompress(&imx258_drv_entry.otp_cfg.base_info_cfg,&otp_cxt->otp_data->lsc_cali_dat);
			if(ret != OTP_CAMERA_SUCCESS){
...
int imx258_compatible_convert(otp_drv_cxt_t *otp_drv_handle,void* p_data)
{
	...
	struct sensor_otp_cust_info *convert_data = malloc(sizeof(struct sensor_otp_cust_info));
	/*otp raw data*/
	convert_data->total_otp.data_ptr = otp_cxt->otp_raw_data.buffer;
	convert_data->total_otp.size = otp_cxt->otp_raw_data.num_bytes;
	/*module data*/
	convert_data->single_otp.module_info.vcm_id = format_data->module_dat.vcm_id;
	convert_data->single_otp.module_info.driver_ic_id = format_data->module_dat.drvier_ic_id;
	/*awb convert*/
	...
	/*pdaf convert*/
	convert_data->single_otp.pdaf_info.pdaf_data_addr = format_data->pdaf_cali_dat.buffer;
	convert_data->single_otp.pdaf_info.pdaf_data_size = format_data->pdaf_cali_dat.size;
