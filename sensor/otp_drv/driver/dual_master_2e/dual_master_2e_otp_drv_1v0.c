#include "dual_master_2e_otp_drv_1v0.h"
...
static struct module_info_t dual_master_module_info;

static cmr_int _dual_master_otp_parse_module_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    ...
    ret = _dual_master_otp_section_checksum(otp_cxt->otp_raw_data.buffer, 0x00,
                                            80, 0x50);
    if (OTP_CAMERA_SUCCESS != ret) {
...
static cmr_int _dual_master_otp_parse_af_data(cmr_handle otp_drv_handle) {
    ...
    ret = _dual_master_otp_section_checksum(
...
static cmr_int _dual_master_otp_parse_ae_data(void *otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
...
static cmr_int _dual_master_otp_parse_dualcam_data(cmr_handle otp_drv_handle) {
    ...
    if (atoi(value) == 1) {
        ...
        dualcam_data.dualcam_checksum = dualcam_src_dat[256];
...
static cmr_int dual_master_otp_drv_read(cmr_handle otp_drv_handle,
                                        void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
...
static cmr_int dual_master_otp_drv_parse(cmr_handle otp_drv_handle,
    ...
    if (otp_cxt->otp_raw_data.buffer) {
        ...
        _dual_master_otp_parse_module_data(otp_drv_handle);
        _dual_master_otp_parse_af_data(otp_drv_handle);
        _dual_master_otp_parse_awb_data(otp_drv_handle);
        _dual_master_otp_parse_lsc_data(otp_drv_handle);
        _dual_master_otp_parse_pdaf_data(otp_drv_handle);
        _dual_master_otp_parse_ae_data(otp_drv_handle);
        _dual_master_otp_parse_dualcam_data(otp_drv_handle);
...
static cmr_int dual_master_otp_drv_ioctl(cmr_handle otp_drv_handle,
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        dual_master_otp_compatible_convert(otp_drv_handle, param);
        break;
    default:
        break;
    }
