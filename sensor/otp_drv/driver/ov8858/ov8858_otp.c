...
uint32_t ov8858_format_wbdata(SENSOR_HW_HANDLE handle)
{
	...
	ret = ov8858_section_checksum(ctr_cxt->otp_params.buffer, WB_INFO_OFFSET, 
...
uint32_t ov8858_format_lensdata(SENSOR_HW_HANDLE handle)
{
	...
	ret = ov8858_section_checksum(ctr_cxt->otp_params.buffer, LSC_INFO_OFFSET,
...
uint32_t ov8858_format_calibration_data(SENSOR_HW_HANDLE handle)
{
	...
	ov8858_format_wbdata(handle);
	ov8858_format_lensdata(handle);
...
uint32_t ov8858_read_otp_data(SENSOR_HW_HANDLE handle)
{
	...
	if (NULL == buffer){
		...
		ov8858_formatdata_buffer_init(handle);
		/*start read otp data one time*/
		while(try--){
			//ret = sns_dev_i2c_read_seq(OV8858_I2C_ADDR, buffer, SENSOR_I2C_REG_8BIT, OTP_LEN);
		...
		if(!try && (ret != OTP_CAMERA_SUCCESS)) {
			...
			for(i = 0; i < OTP_LEN; i++){
				...
				Sensor_ReadI2C(OV8858_I2C_ADDR,(uint8_t*)&cmd_val[0],2);
				buffer[i] = cmd_val[0];
			}
		}
	}
...
uint32_t ov8858_write_otp_data(SENSOR_HW_HANDLE handle)
{
	...
	if(NULL != otp_params->buffer){
		/*start read otp data one time*/
		//sns_dev_i2c_read_seq(OV8858_I2C_ADDR, otp_params->buffer,
