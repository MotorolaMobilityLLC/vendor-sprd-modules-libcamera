#include "sp8407_cmk_otp_drv.h"
// OTP Map 0.4
/** sp8407_cmk_section_checksum:
 ...
 **/
static cmr_int _sp8407_cmk_section_checksum(cmr_u8 *buf, cmr_uint offset,
                                            cmr_uint data_count,
...
static cmr_int _sp8407_cmk_parse_module_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
static cmr_int _sp8407_cmk_parse_af_data(cmr_handle otp_drv_handle) {
    ...
    ret = _sp8407_cmk_section_checksum(
...
static cmr_int _sp8407_cmk_parse_pdaf_data(cmr_handle otp_drv_handle) {
    ...
    if (OTP_CAMERA_SUCCESS != ret) {
        ...
        OTP_LOGI("PDAF otp checksum passed");
    }
...
static cmr_int _sp8407_cmk_awb_calibration(cmr_handle otp_drv_handle) {
    ...
    /*
       ...
       &(sp8407_cmk_otp_entry.otp_cfg.cali_items);
...
static cmr_int sp8407_cmk_otp_drv_read(cmr_handle otp_drv_handle, void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    cmr_u8 cmd_val[3];
    cmr_uint i = 0;
    char value[255];
    ...
    if (!otp_raw_data->buffer) {
        ...
        _sp8407_cmk_buffer_init(otp_drv_handle);
    }
...
static cmr_int sp8407_cmk_otp_drv_parse(cmr_handle otp_drv_handle,
    ...
    otp_base_info_cfg_t *base_info =
        &(sp8407_cmk_otp_entry.otp_cfg.base_info_cfg);
    otp_params_t *otp_raw_data = &(otp_cxt->otp_raw_data);
    ...
    if (sensor_otp_get_buffer_state(otp_cxt->sensor_id)) {
        ...
        _sp8407_cmk_parse_module_data(otp_drv_handle);
        _sp8407_cmk_parse_af_data(otp_drv_handle);
        _sp8407_cmk_parse_awb_data(otp_drv_handle);
        _sp8407_cmk_parse_lsc_data(otp_drv_handle);
        _sp8407_cmk_parse_pdaf_data(otp_drv_handle);
...
static cmr_int sp8407_cmk_otp_drv_calibration(cmr_handle otp_drv_handle) {
    ...
    otp_calib_items_t *cali_items = &(sp8407_cmk_otp_entry.otp_cfg.cali_items);

    if (cali_items) {
        if (cali_items->is_pdafc && cali_items->is_pdaf_self_cal)
            _sp8407_cmk_pdaf_calibration(otp_drv_handle);
        /*calibration at sensor or isp */
        if (cali_items->is_awbc && cali_items->is_awbc_self_cal)
            _sp8407_cmk_awb_calibration(otp_drv_handle);
        if (cali_items->is_lsc && cali_items->is_awbc_self_cal)
            _sp8407_cmk_lsc_calibration(otp_drv_handle);
    }
...
static cmr_int sp8407_cmk_compatible_convert(cmr_handle otp_drv_handle,
                                             void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGD("in");
...
static cmr_int sp8407_cmk_otp_drv_ioctl(cmr_handle otp_drv_handle, cmr_uint cmd,
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        sp8407_cmk_compatible_convert(otp_drv_handle, params);
        break;
    default:
        break;
    }
