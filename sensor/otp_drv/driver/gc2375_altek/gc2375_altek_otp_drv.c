#include "gc2375_altek_otp_drv.h"

/** gc2375_altek_section_data_checksum:
 ...
 **/
static cmr_int _gc2375_altek_i2c_read(void *otp_drv_handle, uint16_t slave_addr,
...
static cmr_int _gc2375_altek_i2c_write(void *otp_drv_handle,
                                       uint16_t slave_addr,
                                       uint16_t memory_addr,
...
static cmr_int _gc2375_altek_section_checksum(unsigned char *buf,
                                              unsigned int offset,
...
static cmr_int _gc2375_altek_buffer_init(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    cmr_int otp_len;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
...
static cmr_int _gc2375_altek_parse_ae_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
...
static cmr_int _gc2375_altek_parse_af_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static cmr_int _gc2375_altek_parse_oc_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
...
static cmr_int _gc2375_altek_parse_lsc_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
...
static cmr_int _gc2375_altek_parse_pdaf_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
...
static cmr_int _gc2375_altek_parse_dualcam_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static cmr_int _gc2375_altek_awb_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGI("in");
    CHECK_PTR(otp_drv_handle);
...
static cmr_int _gc2375_altek_lsc_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGI("in");
    CHECK_PTR(otp_drv_handle);
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
...
static int _gc2375_altek_pdaf_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGI("in");
    CHECK_PTR(otp_drv_handle);
...
static cmr_int _gc2375_altek_split_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
    otp_calib_items_t *cal_items = &(gc2375_altek_drv_entry.otp_cfg.cali_items);

    ret = _gc2375_altek_parse_module_data(otp_drv_handle);
    if (OTP_CAMERA_SUCCESS != ret) {
        OTP_LOGE("parse module info failed");
        return ret;
    }
    if (cal_items->is_self_cal) {
        ret = _gc2375_altek_parse_oc_data(otp_drv_handle);
        if (OTP_CAMERA_SUCCESS != ret) {
    ...
    if (cal_items->is_afc) {
        ret = _gc2375_altek_parse_af_data(otp_drv_handle);
        if (OTP_CAMERA_SUCCESS != ret) {
    ...
    if (cal_items->is_awbc) {
        ret = _gc2375_altek_parse_awb_data(otp_drv_handle);
        if (OTP_CAMERA_SUCCESS != ret) {
    ...
    if (cal_items->is_lsc) {
        ret = _gc2375_altek_parse_lsc_data(otp_drv_handle);
        if (OTP_CAMERA_SUCCESS != ret) {
    ...
    if (cal_items->is_pdafc) {
        ret = _gc2375_altek_parse_pdaf_data(otp_drv_handle);
        if (OTP_CAMERA_SUCCESS != ret) {
    ...
    ret = _gc2375_altek_parse_ae_data(otp_drv_handle);
    if (OTP_CAMERA_SUCCESS != ret) {
    ...
    if (cal_items->is_dul_camc) {
        ret = _gc2375_altek_parse_dualcam_data(otp_drv_handle);
        if (OTP_CAMERA_SUCCESS != ret) {
...
static cmr_int _gc2375_altek_compatible_convert(cmr_handle otp_drv_handle,
                                                void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
...
static cmr_int gc2375_altek_otp_drv_create(otp_drv_init_para_t *input_para,
                                           cmr_handle *sns_af_drv_handle) {
    OTP_LOGI("in");
    return sensor_otp_drv_create(input_para, sns_af_drv_handle);
}

static cmr_int gc2375_altek_otp_drv_delete(cmr_handle otp_drv_handle) {

    OTP_LOGI("in");
    return sensor_otp_drv_delete(otp_drv_handle);
}

static cmr_int gc2375_altek_otp_drv_read(cmr_handle otp_drv_handle,
                                         void *p_params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    char value[255];
    CHECK_PTR(otp_drv_handle);
...
static cmr_int gc2375_altek_otp_drv_write(cmr_handle otp_drv_handle,
                                          void *p_params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    CHECK_PTR(p_params);
...
static cmr_int gc2375_altek_otp_drv_parse(cmr_handle otp_drv_handle,
                                          void *p_params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    // CHECK_PTR(p_params);
    ...
    if (!otp_raw_data->buffer) {
        ...
        ret = gc2375_altek_otp_drv_read(otp_drv_handle, NULL);
    ...
    ret = _gc2375_altek_split_data(otp_drv_handle);
    if (ret != OTP_CAMERA_SUCCESS) {
        return OTP_CAMERA_FAIL;
    }
...
static cmr_int gc2375_altek_otp_drv_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGI("in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;
...
/*just for expend*/
static cmr_int gc2375_altek_otp_drv_ioctl(cmr_handle otp_drv_handle,
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        _gc2375_altek_compatible_convert(otp_drv_handle, params);
        break;
    default:
        break;
    }
