#include "dual_slave_2e_otp_drv_1v0.h"

/*
 *  dual camera two eeproms - slave camera otp driver
 *  for otp v1.0
...
*                Internal Functions
====================================================*/
static cmr_int _dual_slave_otp_section_checksum(cmr_u8 *buffer,
                                                cmr_uint start_addr,
                                                cmr_uint size,
...
static struct module_info_t dual_slave_module_info;

static cmr_int _dual_slave_otp_parse_module_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    ...
    struct module_info_t *module_info = &dual_slave_module_info;
    ...
    ret = _dual_slave_otp_section_checksum(otp_cxt->otp_raw_data.buffer, 0x00,
                                           80, 0x50);
    if (OTP_CAMERA_SUCCESS != ret) {
...
static cmr_int _dual_slave_otp_parse_af_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
    ...
    ret = _dual_slave_otp_section_checksum(
...
static cmr_int _dual_slave_otp_parse_awb_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
...
static cmr_int _dual_slave_otp_parse_lsc_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
    ...
    if (atoi(value) == 1) {
        ...
        lsc_data.lsc_checksum =
            lsc_src_dat[module_info->slave_size.slave_lsc_size];
    }
...
static cmr_int _dual_slave_otp_parse_ae_data(void *otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
    ...
    if (atoi(value) == 1) {
        ...
        ae_data.ae_checksum = ae_src_dat[25];
...
static cmr_int dual_slave_otp_drv_create(otp_drv_init_para_t *input_para,
                                         cmr_handle *otp_drv_handle) {
    return sensor_otp_drv_create(input_para, otp_drv_handle);
}

static cmr_int dual_slave_otp_drv_delete(cmr_handle otp_drv_handle) {
    return sensor_otp_drv_delete(otp_drv_handle);
}

/*malloc buffer and read otp raw data from eeprom or bin file*/
static cmr_int dual_slave_otp_drv_read(cmr_handle otp_drv_handle, void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
    ...
    if (atoi(value2) == 1) {
        ...
        snprintf(otp_bin_ext_path, sizeof(otp_bin_ext_path), "%s%s_otp.bin",
                 "/data/vendor/cameraserver/", "dual_slave");
...
static cmr_int dual_slave_otp_drv_write(cmr_handle otp_drv_handle,
                                        void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    CHECK_PTR(param);
...
static cmr_int dual_slave_otp_drv_parse(cmr_handle otp_drv_handle,
                                        void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
    ...
    if (otp_cxt->otp_raw_data.buffer) {
        ...
        _dual_slave_otp_parse_module_data(otp_drv_handle);
        _dual_slave_otp_parse_af_data(otp_drv_handle);
        _dual_slave_otp_parse_awb_data(otp_drv_handle);
        _dual_slave_otp_parse_lsc_data(otp_drv_handle);
        _dual_slave_otp_parse_ae_data(otp_drv_handle);
...
static cmr_int dual_slave_otp_drv_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
...
static cmr_int dual_slave_otp_compatible_convert(cmr_handle otp_drv_handle,
                                                 void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("E");
...
/*for expand*/
static cmr_int dual_slave_otp_drv_ioctl(cmr_handle otp_drv_handle, cmr_uint cmd,
                                        void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGD("E");
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        dual_slave_otp_compatible_convert(otp_drv_handle, param);
        break;
    default:
        break;
    }
