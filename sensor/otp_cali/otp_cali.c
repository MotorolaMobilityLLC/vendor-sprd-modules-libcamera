...
cmr_u16 read_calibration_otp_from_file(cmr_u8 *buf, cmr_u8 dual_flag) {
    ...
    case CALIBRATION_FLAG_BOKEH_GLD:
        ...
        OtpBkDataPath = NULL;
        break;
    case CALIBRATION_FLAG_BOKEH_GLD_ONLINE:
        OtpDataPath = OTP_CALI_BOKEH_GLD_ONLINE_PATH;
        OtpBkDataPath = NULL;
        break;
...
cmr_u16 read_cmei_from_file(cmr_u8 *buf,cmr_u16 buf_size, cmr_u8 dual_flag) {
    ...
    switch (dual_flag) {
    case CALIBRATION_FLAG_BOKEH:
        OtpDataPath = BOKEH_CMEI_PATH;
        OtpBkDataPath = BOKEHBK_CMEI_PATH;
        break;
    case CALIBRATION_FLAG_OZ1:
        OtpDataPath = OZ1_CMEI_PATH;
        OtpBkDataPath = OZ1BK_CMEI_PATH;
        break;
    case CALIBRATION_FLAG_OZ2:
        OtpDataPath = OZ2_CMEI_PATH;
        OtpBkDataPath = OZ2BK_CMEI_PATH;
        break;
    default:
    ...
    do {
        if (access(OtpDataPath, F_OK) == 0) {
            ...
            if((header_ptr->len) > buf_size) {
                SENSOR_LOGE("read %s buf exceed assigned bug", OtpDataPath);
                fclose(fileHandle);
                break;
            }
    ...
    do {
        if (access(OtpBkDataPath, F_OK) == 0) {
            ...
            if((header_ptr->len) > buf_size) {
                SENSOR_LOGE("read %s buf exceed assigned bug", OtpBkDataPath);
                fclose(fileHandle);
                break;
            }
        ...
        if (check_checksum(buf, header_ptr->len, header_ptr->checksum)) {
            ...
            if (fileHandle != NULL) {
                if (CALI_OTP_HEAD_SIZE != fwrite(header, sizeof(char),
                                                 ...
                                                 fileHandle)) {
                    ret = 0;
                    SENSOR_LOGE("%s:origin cmei header write fail!",
                                OtpDataPath);
                } else {
                    ret = 1;
                    SENSOR_LOGI("%s:origin cmei header write success!", OtpDataPath);
                }
                ...
                fclose(fileHandle);
                SENSOR_LOGI("%s:cmei write finished, ret:%d", OtpDataPath,
                            ret);
            } else {
...
cmr_u8 write_cmei_to_file (cmr_u8 *buf,cmr_u16 buf_size, cmr_u8 dual_flag) {
    ...
    header_ptr->magic = OTP_HEAD_MAGIC;
    header_ptr->len = buf_size;
    header_ptr->checksum = (cmr_u32)calc_checksum(buf, buf_size);
    header_ptr->version = OTP_VERSION;
    header_ptr->data_type = OTPDATA_TYPE_CALIBRATION;
    header_ptr->has_calibration = 1;

    // 1 write pid backup file
    if (0 != access("/mnt/vendor/productinfo/otpdata", F_OK)) {
        SENSOR_LOGE("access /mnt/vendor/productinfo/otpdata cmei failed!");
        ret = mkdir("/mnt/vendor/productinfo/otpdata",
                    S_IRWXU | S_IRWXG | S_IRWXO);
        if (-1 == ret) {
            SENSOR_LOGE("mkdir /mnt/vendor/productinfo/otpdata cmei failed!");
            ret = 0;
        } else {
            ret = 1;
        }
    ...
    if (fileProductinfoHandle != NULL) {
        if (CALI_OTP_HEAD_SIZE != fwrite(header_buf, sizeof(char),
                                         ...
                                         fileProductinfoHandle)) {
            ret = 0;
            SENSOR_LOGE("%s :backup cmei header write fail!", OtpBkDataPath);
        } else {
            ret = 1;
            SENSOR_LOGI("%s: backup cmei header write success!",
                        OtpBkDataPath);
        }

        if (ret && (buf_size == fwrite(buf, sizeof(char), buf_size,
                                       fileProductinfoHandle))) {
    ...
    if (fileVendorHandle != NULL) {
        ...
        if (CALI_OTP_HEAD_SIZE != fwrite(header_buf, sizeof(char),
                                         ...
                                         fileVendorHandle)) {
            ret = 0;
            SENSOR_LOGE("%s:origin cmei header write fail!", OtpDataPath);
        } else {
            ret = 1;
            SENSOR_LOGI("%s:origin cmei header write success!", OtpDataPath);
        }
        ...
        if (ret &&  (buf_size ==
                    fwrite(buf, sizeof(char), buf_size, fileVendorHandle))) {
