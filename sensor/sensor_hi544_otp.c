...
#define hi544_TRULY_02 0x02
#define hi544_RAW_PARAM_COM 0x00
#define hi544_TRULY_OTP_DATA_ADDR         0x3D00
#define hi544_TRULY_OTP_LOAD_ADDR         0x3D81
#define hi544_TRULY_OTP_BANK_ADDR         0x3D84

#define hi544_TRULY_LENC_START_ADDR       0x5800
#define hi544_TRULY_LENC_REG_SIZE         62

#define hi544_TRULY_OTP_LENC_GROUP_ADDR   0x3D00

#define hi544_TRULY_OTP_WB_GROUP_ADDR     0x3D05
#define hi544_TRULY_OTP_WB_GROUP_SIZE     9

#define hi544_TRULY_GAIN_RH_ADDR          0x3400
#define hi544_TRULY_GAIN_RL_ADDR          0x3401
#define hi544_TRULY_GAIN_GH_ADDR          0x3402
#define hi544_TRULY_GAIN_GL_ADDR          0x3403
#define hi544_TRULY_GAIN_BH_ADDR          0x3404
#define hi544_TRULY_GAIN_BL_ADDR          0x3405

#define hi544_TRULY_GAIN_DEFAULT_VALUE    0x0400 // 1x gain

#define hi544_TRULY_OTP_MID               hi544_TRULY_02
#define  hi544_TRULY_BG_RATIO_TYPICAL (0x4E)
#define  hi544_TRULY_RG_RATIO_TYPICAL (0x4A)

struct hi544_otp_info {
...
LOCAL void _hi544_truly_otp_read_enable(void)
{
	Sensor_WriteReg(hi544_TRULY_OTP_LOAD_ADDR, 0x01);
	usleep(15*1000); // sleep > 10ms
}

// Disable OTP read function
LOCAL void _hi544_truly_otp_read_disable(void)
{
	Sensor_WriteReg(hi544_TRULY_OTP_LOAD_ADDR, 0x00);
}

LOCAL void _hi544_truly_otp_read(uint16_t otp_addr, uint8_t* otp_data)
{
	_hi544_truly_otp_read_enable();
	*otp_data = Sensor_ReadReg(otp_addr);
	_hi544_truly_otp_read_disable();
}

/*******************************************************************************
* Function    :  _hi544_truly_otp_clear
* Description :  Clear OTP buffer
* Parameters  :  none
* Return      :  none
*******************************************************************************/
LOCAL void _hi544_truly_otp_clear(void)
{
	// After read/write operation, the OTP buffer should be cleared to avoid accident write
	uint8_t i;
	for (i=0; i<32; i++)
	{
		Sensor_WriteReg(hi544_TRULY_OTP_DATA_ADDR+i, 0x00);
	}
}

/*******************************************************************************
* Function    :  _hi544_truly_otp_check_wb_group
...
* Return      :  0, group index is empty
                ...
                -1, group index error
*******************************************************************************/
LOCAL int8_t _hi544_truly_otp_check_wb_group(uint8_t index)
{
	uint16_t otp_addr = hi544_TRULY_OTP_WB_GROUP_ADDR + index * hi544_TRULY_OTP_WB_GROUP_SIZE;
	uint8_t  flag;
	...
	// select bank 0
	Sensor_WriteReg(hi544_TRULY_OTP_BANK_ADDR, 0x08);

	_hi544_truly_otp_read(otp_addr, &flag);
...
/*******************************************************************************
* Function    :  _hi544_truly_otp_read_wb_group
* Description :  Read group value and store it in OTP Struct
* Parameters  :  [in] index : index of otp group (0, 1, 2)
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/
LOCAL int8_t _hi544_truly_otp_read_wb_group(int8_t index)
{
	uint16_t otp_addr;
	uint8_t mid;
	...
	otp_addr = hi544_TRULY_OTP_WB_GROUP_ADDR + index * hi544_TRULY_OTP_WB_GROUP_SIZE;

	// select bank 0
	Sensor_WriteReg(hi544_TRULY_OTP_BANK_ADDR, 0x08);

	_hi544_truly_otp_read(otp_addr, &mid);
	if ((mid&0x7f) != hi544_TRULY_OTP_MID)
	{
		return -1;
	}

	_hi544_truly_otp_read(otp_addr+2, &truly_rg_ratio);
...
/*******************************************************************************
* Function    :  _hi544_truly_otp_apply_wb
...
*******************************************************************************/
LOCAL uint32_t _hi544_truly_otp_apply_wb(uint8_t golden_rg, uint8_t golden_bg)
{
	uint16_t gain_r = hi544_TRULY_GAIN_DEFAULT_VALUE;
	uint16_t gain_g = hi544_TRULY_GAIN_DEFAULT_VALUE;
	uint16_t gain_b = hi544_TRULY_GAIN_DEFAULT_VALUE;
	...
	if (ratio_r != 1)
	{
		gain_r = (uint16_t)(hi544_TRULY_GAIN_DEFAULT_VALUE * ratio_r);
		Sensor_WriteReg(hi544_TRULY_GAIN_RH_ADDR, gain_r >> 8);
		Sensor_WriteReg(hi544_TRULY_GAIN_RL_ADDR, gain_r & 0x00ff);
	}

	if (ratio_g != 1)
	{
		gain_g = (uint16_t)(hi544_TRULY_GAIN_DEFAULT_VALUE * ratio_g);
		Sensor_WriteReg(hi544_TRULY_GAIN_GH_ADDR, gain_g >> 8);
		Sensor_WriteReg(hi544_TRULY_GAIN_GL_ADDR, gain_g & 0x00ff);
	}

	if (ratio_b != 1)
	{
		gain_b = (uint16_t)(hi544_TRULY_GAIN_DEFAULT_VALUE * ratio_b);
		Sensor_WriteReg(hi544_TRULY_GAIN_BH_ADDR, gain_b >> 8);
...
LOCAL uint32_t _hi544_truly_otp_apply_wb(uint8_t golden_rg, uint8_t golden_bg)
{
	...
	{
		gain_r = hi544_TRULY_GAIN_DEFAULT_VALUE * ratio_r / OTP_MULTIPLE_FAC;
		Sensor_WriteReg(hi544_TRULY_GAIN_RH_ADDR, gain_r >> 8);
		Sensor_WriteReg(hi544_TRULY_GAIN_RL_ADDR, gain_r & 0x00ff);
	}
	...
	{
		gain_g = hi544_TRULY_GAIN_DEFAULT_VALUE * ratio_g / OTP_MULTIPLE_FAC;
		Sensor_WriteReg(hi544_TRULY_GAIN_GH_ADDR, gain_g >> 8);
		Sensor_WriteReg(hi544_TRULY_GAIN_GL_ADDR, gain_g & 0x00ff);
	}
	...
	{
		gain_b = hi544_TRULY_GAIN_DEFAULT_VALUE * ratio_b / OTP_MULTIPLE_FAC;
		Sensor_WriteReg(hi544_TRULY_GAIN_BH_ADDR, gain_b >> 8);
		Sensor_WriteReg(hi544_TRULY_GAIN_BL_ADDR, gain_b & 0x00ff);
	}
...
/*******************************************************************************
* Function    :  _hi544_truly_otp_update_wb
...
/*******************************************************************************
* Function    :  _hi544_truly_otp_check_lenc_group
* Description :  Check OTP Space Availability
...
* Return      :  0, group index is empty
                ...
                -1, group index error
*******************************************************************************/
LOCAL int8_t _hi544_truly_otp_check_lenc_group(uint8_t index)
{
	uint16_t otp_addr = hi544_TRULY_OTP_LENC_GROUP_ADDR;
	uint8_t  flag;
	uint8_t  bank;
	...
	bank = 0x08 | (index*2 + 1);
	Sensor_WriteReg(hi544_TRULY_OTP_BANK_ADDR, bank);
...
/*******************************************************************************
* Function    :  _hi544_truly_otp_read_lenc_group
* Description :  Read group value and store it in OTP Struct
* Parameters  :  [in] int index : index of otp group (0, 1, 2)
* Return      :  group index (0, 1, 2)
                 -1, error
*******************************************************************************/
LOCAL int8_t _hi544_truly_otp_read_lenc_group(int index)
{
	uint16_t otp_addr;
	uint8_t bank;
	uint8_t i;
	...
	otp_addr = hi544_TRULY_OTP_LENC_GROUP_ADDR+1;
	...
	otp_addr = hi544_TRULY_OTP_LENC_GROUP_ADDR;
...
/*******************************************************************************
* Function    :  _hi544_truly_otp_apply_lenc
...
LOCAL void _hi544_truly_otp_apply_lenc(void)
{
	...
	Sensor_WriteReg(hi544_TRULY_LENC_START_ADDR, data);
...
/*******************************************************************************
* Function    :  _hi544_truly_otp_update_lenc
...
LOCAL uint32_t _hi544_truly_Identify_otp(void* param_ptr)
{
...
LOCAL uint32_t  _hi544_truly_update_otp(void* param_ptr)
{
	uint16_t temp;
