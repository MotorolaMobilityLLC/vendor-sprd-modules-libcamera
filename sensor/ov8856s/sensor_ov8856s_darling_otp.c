...
#define MODULE_ID_ov8856s_darling		0x0005    //xxx: sensor P/N;  yyy: module vendor
...
static struct otp_info_t s_ov8856s_darling_otp_info={0x00};
#define RG_RATIO_TYPICAL_ov8856s_darling    	0x013F
#define BG_RATIO_TYPICAL_ov8856s_darling		0x012C
#define R_TYPICAL_ov8856s_darling		0x0000
#define G_TYPICAL_ov8856s_darling		0x0000
#define B_TYPICAL_ov8856s_darling		0x0000

static uint32_t ov8856s_darling_read_otp_info(SENSOR_HW_HANDLE handle,void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
	otp_info->rg_ratio_typical=RG_RATIO_TYPICAL_ov8856s_darling;
	otp_info->bg_ratio_typical=BG_RATIO_TYPICAL_ov8856s_darling;
	otp_info->r_typical=R_TYPICAL_ov8856s_darling;
	otp_info->g_typical=G_TYPICAL_ov8856s_darling;
	otp_info->b_typical=B_TYPICAL_ov8856s_darling;
...
static void ov8856s_darling_enable_awb_otp(SENSOR_HW_HANDLE handle)
{
	/*TODO enable awb otp update*/
	Sensor_WriteReg(0x5000, 0x77);
}

static uint32_t ov8856s_darling_update_awb(SENSOR_HW_HANDLE handle,void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
	ov8856s_darling_enable_awb_otp(handle);
...
static void ov8856s_darling_enable_lsc_otp(SENSOR_HW_HANDLE handle)
{
	/*TODO enable lsc otp update*/
	Sensor_WriteReg(0x5000, 0x77);
}

static uint32_t ov8856s_darling_update_lsc(SENSOR_HW_HANDLE handle,void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
...
static uint32_t ov8856s_darling_test_awb(SENSOR_HW_HANDLE handle,void *param_ptr)
{
	uint32_t flag = 1;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;
	...
	if(!strcmp(value,"on")){
		...
		otp_info->rg_ratio_typical=RG_RATIO_TYPICAL_ov8856s_darling*1.5;
		otp_info->bg_ratio_typical=BG_RATIO_TYPICAL_ov8856s_darling*1.5;
		otp_info->r_typical=G_TYPICAL_ov8856s_darling;
		otp_info->g_typical=G_TYPICAL_ov8856s_darling;
		otp_info->b_typical=G_TYPICAL_ov8856s_darling;
...
static uint32_t ov8856s_darling_test_lsc(SENSOR_HW_HANDLE handle)
{
	uint32_t flag = 1;
	char value[PROPERTY_VALUE_MAX];
...
static uint32_t ov8856s_darling_update_otp(SENSOR_HW_HANDLE handle,void *param_ptr)
{
	uint32_t rtn = SENSOR_SUCCESS;
	struct otp_info_t *otp_info=(struct otp_info_t *)param_ptr;

	/*update awb*/
	if(ov8856s_darling_test_awb(handle,otp_info)){
		rtn=ov8856s_darling_update_awb(handle,param_ptr);
	...
	/*update lsc*/
	if(ov8856s_darling_test_lsc(handle)){
		rtn=ov8856s_darling_update_lsc(handle,param_ptr);
		if(rtn!=SENSOR_SUCCESS)
		{
...
static uint32_t ov8856s_darling_identify_otp(SENSOR_HW_HANDLE handle,void *param_ptr)
{
	...
	if(rtn!=SENSOR_SUCCESS){
		...
		return rtn;
	} else if(MODULE_ID_ov8856s_darling !=otp_info->module_id){
