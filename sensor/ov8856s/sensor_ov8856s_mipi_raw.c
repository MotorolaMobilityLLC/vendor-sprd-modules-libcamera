...
#include "../af_zzz.h"
#endif
#include "sensor_ov8856s_raw_param_main.c"
...
static SENSOR_REG_TAB_INFO_T s_ov8856s_resolution_tab_raw[SENSOR_MODE_MAX] = {
	...
	{ADDR_AND_LEN_OF_ARRAY(ov8856s_preview_setting),
	 PREVIEW_WIDTH, PREVIEW_HEIGHT, EX_MCLK,
	 SENSOR_IMAGE_FORMAT_RAW},
	{ADDR_AND_LEN_OF_ARRAY(ov8856s_snapshot_setting),
 ...
 *
 *============================================================================*/
static unsigned long ov8856s_get_resolution_trim_tab(SENSOR_HW_HANDLE handle,uint32_t param)
{
 ...
 *============================================================================*/
static uint32_t ov8856s_before_snapshot(SENSOR_HW_HANDLE handle,uint32_t param)
{
	...
	uint32_t prv_linetime = s_ov8856s_resolution_trim_tab[preview_mode].line_time;
	uint32_t cap_linetime = s_ov8856s_resolution_trim_tab[capture_mode].line_time;

	s_current_default_frame_length = ov8856s_get_default_frame_length(handle,capture_mode);
...
static struct sensor_i2c_reg_tab ov8856s_shutter_tab = {
	.settings = ov8856s_shutter_reg,
	.size = ARRAY_SIZE(ov8856s_shutter_reg),
};
...
static struct sensor_i2c_reg_tab ov8856s_again_tab = {
	.settings = ov8856s_again_reg,
	.size = ARRAY_SIZE(ov8856s_again_reg),
};
...
struct sensor_i2c_reg_tab ov8856s_dgain_tab = {
	.settings = ov8856s_dgain_reg,
	.size = ARRAY_SIZE(ov8856s_dgain_reg),
};
...
static uint16_t ov8856s_calc_exposure(SENSOR_HW_HANDLE handle,
...
static unsigned long ov8856s_read_aec_info(SENSOR_HW_HANDLE handle, unsigned long param)
{
	...
	frame_interval = (uint16_t)(((exposure_line + dummy_line) * (s_ov8856s_resolution_trim_tab[mode].line_time)) / 1000000);
	SENSOR_PRINT("mode = %d, exposure_line = %d, dummy_line= %d, frame_interval= %d ms",
