...
cmr_int otp_parser_map_v04(void * raw_data, enum otp_parser_cmd cmd, cmr_uint eeprom_num, cmr_int camera_id, cmr_int raw_height, cmr_int raw_width, void *result);
cmr_int otp_parser_map_v05(void * raw_data, enum otp_parser_cmd cmd, cmr_uint eeprom_num, cmr_int camera_id, cmr_int raw_height, cmr_int raw_width, void *result);
...
cmr_int _otp_get_map_version(void * raw_data, cmr_u16 *map_version)
{
    ...
    case 0x0005:
        ...
        break;
    case 0x0100:
        if(raw_otp[0] == 0x53 && raw_otp[1] == 0x50 && raw_otp[2] == 0x52 && raw_otp[3] == 0x44){
            version = 0x0100;
        }else{
            version = 0x0001;
        }
...
** Otp map 0.4 and otp map 0.5 differs in the size of dual-camera data, with 230 (0.4) and 256 (0.5)
** Otp map 1.0 can get each section start address and size in module info
************************************************************/
cmr_int otp_parser(void * raw_data, enum otp_parser_cmd cmd, cmr_uint eeprom_num, cmr_int camera_id, cmr_int raw_height, cmr_int raw_width, void *result)
{
    ...
    if(OTP_PARSER_MAP_VERSION==cmd){
        struct otp_parser_map_version *map_ver = (struct otp_parser_map_version *)result;
    ...
    case 0x0003:
    case 0x0004:
        rtn = otp_parser_map_v04(raw_data, cmd, eeprom_num, camera_id, raw_height, raw_width, result);
        break;
    case 0x0005:
        rtn = otp_parser_map_v05(raw_data, cmd, eeprom_num, camera_id, raw_height, raw_width, result);
        break;
    case 0x0100:
        rtn = otp_parser_map_v10(raw_data, cmd, camera_id, result);
        break;
    default:
        CMR_LOGV("map version is %u, no parser", map_version);
        break;
    }

exit:
    CMR_LOGI("X rtn = %ld", rtn);
    return rtn;
}
