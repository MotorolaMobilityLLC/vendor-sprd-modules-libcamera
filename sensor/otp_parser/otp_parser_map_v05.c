...
static cmr_uint _otp_parser_get_lsc_chn_len(cmr_int raw_height, cmr_int raw_width, cmr_int grid)
{
    ...
    lsc_otp_width = ((half_width % grid) > 0) ? (half_width / grid + 2) : (half_width / grid + 1);
    lsc_otp_height = ((half_height % grid) > 0) ? (half_height / grid + 2) : (half_height / grid + 1);

    lsc_otp_len_chn = ((lsc_otp_width * lsc_otp_height) * 14 % 8) ? (((lsc_otp_width * lsc_otp_height) * 14 / 8)+1) : ((lsc_otp_width * lsc_otp_height) * 14 / 8);
    lsc_otp_len_chn = lsc_otp_len_chn % 2 ? lsc_otp_len_chn + 1 : lsc_otp_len_chn;
...
static cmr_uint _otp_parser_get_start_offset_v05(cmr_u8 * raw_data, cmr_int eeprom_num, cmr_int camera_id, cmr_int height, cmr_int width)
{
    ...
    //awb
    camera_otp_info[camera_id].awb_info.data_offset = camera_otp_info[camera_id].af_info.data_offset + camera_otp_info[camera_id].af_info.data_size + 1;
    ...
    //lsc
    camera_otp_info[camera_id].lsc_info.data_offset = camera_otp_info[camera_id].awb_info.data_offset + camera_otp_info[camera_id].awb_info.data_size + 1;
    lsc_chn_len = _otp_parser_get_lsc_chn_len(height, width, lsc_grid);
    if(resolution==2){
        camera_otp_info[camera_id].lsc_info.data_size = lsc_chn_len*4 + 16 + 148;   // 148 bytes is reserved in 2M otp map
    }else{
    ...
    if(OTP_EEPROM_SINGLE_CAM_DUAL == eeprom_num || OTP_EEPROM_DUAL_CAM_DUAL== eeprom_num){
        if(0==camera_id || 1==camera_id){
            ...
            //for slave module
            camera_otp_info[camera_id].ae_info.data_offset = camera_otp_info[camera_id].lsc_info.data_offset + camera_otp_info[camera_id].lsc_info.data_size + 1;
            switch(resolution){
            case 2:
    ...
    if((OTP_EEPROM_SINGLE_CAM_DUAL == eeprom_num || OTP_EEPROM_DUAL_CAM_DUAL== eeprom_num)&&(0==camera_id || 1==camera_id)){
        camera_otp_info[camera_id].dualcam_info.data_offset = camera_otp_info[camera_id].ae_info.data_offset + camera_otp_info[camera_id].ae_info.data_size + 1;
        camera_otp_info[camera_id].dualcam_info.data_size = 256;//the only difference between map 0.4 and 0.5
    }else{
...
static cmr_int _otp_parser_af_v05(void * raw_data, cmr_uint start_offset, cmr_uint data_size, void *result)
{
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    CMR_LOGV("af inf=%u, macro=%u, posture=%u", af_data->inf_position, af_data->macro_position, af_data->posture);

    return rtn;
}

static cmr_int _otp_parser_awb_v05(void * raw_data, cmr_uint start_offset, cmr_uint data_size, void *result)
{
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    CMR_LOGV("awb1: (r, b, b)=(%u, %u, %u), awb2:(r, b, b)=(%u, %u, %u)", awb_data->awb1_r_gain,awb_data->awb1_g_gain, awb_data->awb1_b_gain,
            awb_data->awb2_r_gain, awb_data->awb2_g_gain, awb_data->awb2_b_gain);

    return rtn;
}

static cmr_int _otp_parser_lsc_v05(void * raw_data, cmr_uint start_offset, cmr_uint data_size, cmr_uint height, cmr_uint width, void *result)
{
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    lsc_chn_len = _otp_parser_get_lsc_chn_len(height, width, lsc_data->lsc_grid);

    lsc_data->lsc_table_size = lsc_chn_len *4; // exclude oc and checksum bytes
    lsc_data->lsc_table_addr = &lsc_buf[16];

    CMR_LOGV("lsc oc r(%u, %u), gr(%u, %u), gb(%u, %u), b(%u, %u)", lsc_data->oc_r_x, lsc_data->oc_r_y, lsc_data->oc_gr_x, lsc_data->oc_gr_y,
...
static cmr_int _otp_parser_ae_v05(void * raw_data, cmr_uint start_offset, cmr_uint data_size, void *result)
{
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    ae_data->exp_1x_gain = (ae_buf[5] << 24) | (ae_buf[4] << 16) | (ae_buf[3] << 8) | ae_buf[2];
    ae_data->exp_2x_gain = (ae_buf[9] << 24) | (ae_buf[8] << 16) | (ae_buf[7] << 8) | ae_buf[6];
    ae_data->exp_4x_gain = (ae_buf[13] << 24) | (ae_buf[12] << 16) | (ae_buf[11] << 8) | ae_buf[10];
    ae_data->exp_8x_gain = (ae_buf[17] << 24) | (ae_buf[16] << 16) | (ae_buf[15] << 8) | ae_buf[14];
...
static cmr_int _otp_parser_dualcamera_v05(void * raw_data, cmr_uint start_offset, cmr_uint data_size, void *result)
{
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    //map 0.5 have two checksum in dual camera
    rtn = _otp_parser_checksum(raw_data, dual_start_offset, 230, dual_start_offset+230);
    if(OTP_PARSER_SUCCESS!=rtn){
            CMR_LOGE("dual camera checksum value is error!");
            return rtn;
    }

    rtn = _otp_parser_checksum(raw_data, dual_start_offset + 231, 25, dual_start_offset+231+25);
    if(OTP_PARSER_SUCCESS!=rtn){
            CMR_LOGE("dual camera checksum value is error!");
            return rtn;
    }
    ...
    CMR_LOGV("dual camera dual_flag=%u, data_addr=%p, data_size=%u", dual_data->dual_flag, dual_data->data_3d_data, dual_data->data_3d_data_size);

    return rtn;
}

cmr_int otp_parser_map_v05(void * raw_data, enum otp_parser_cmd cmd, cmr_int eeprom_num, cmr_int camera_id, cmr_int raw_height, cmr_int raw_width, void *result)
{
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    rtn = _otp_parser_get_start_offset_v05((cmr_u8 *)raw_data, eeprom_num, camera_id, raw_height, raw_width);
    ...
    case OTP_PARSER_AF:
            ...
            rtn = _otp_parser_af_v05(raw_data, start_offset, data_size, result);
            break;
    case OTP_PARSER_AWB:
            start_offset = camera_otp_info[camera_id].awb_info.data_offset;
            data_size = camera_otp_info[camera_id].awb_info.data_size;
            rtn = _otp_parser_awb_v05(raw_data, start_offset, data_size, result);
            break;
    case OTP_PARSER_LSC:
    ...
    case OTP_PARSER_AE:
            ...
            rtn = _otp_parser_ae_v05(raw_data, start_offset, data_size, result);
            break;
    case OTP_PARSER_DUAL_CAMERA:
            ...
            rtn = _otp_parser_dualcamera_v05(raw_data, start_offset, data_size, result);
            break;
    default:
