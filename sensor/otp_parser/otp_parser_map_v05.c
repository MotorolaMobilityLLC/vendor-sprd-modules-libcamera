...
static cmr_uint _otp_parser_get_start_offset_v05(cmr_u8 *raw_data,
                                                 cmr_int eeprom_num,
                                                 cmr_int camera_id,
                                                 cmr_int height,
                                                 cmr_int width) {
    ...
    if ((OTP_EEPROM_SINGLE_CAM_DUAL == eeprom_num ||
        ...
        camera_otp_info[camera_id].bokeh_info.data_size =
            256; // the only difference between map 0.4 and 0.5
    } else {
...
static cmr_int _otp_parser_af_v05(void *raw_data, cmr_uint start_offset,
...
static cmr_int _otp_parser_awb_v05(void *raw_data, cmr_uint start_offset,
                                   cmr_uint data_size, void *result) {
    cmr_int rtn = OTP_PARSER_SUCCESS;
...
static cmr_int _otp_parser_lsc_v05(void *raw_data, cmr_uint start_offset,
...
static cmr_int _otp_parser_pdaf1_v05(void *raw_data, cmr_uint start_offset,
                                     cmr_uint data_size, void *result) {
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    if (0 == start_offset || 0 == data_size) {
        CMR_LOGI("there is no pdaf_sprd data");
        rtn = OTP_PARSER_CMD_ERR;
        return rtn;
    }
    ...
    rtn = _otp_parser_checksum(raw_data, pdaf1_start_offset, pdaf1_data_size,
                               pdaf1_end_offset);
    if (OTP_PARSER_SUCCESS != rtn) {
        CMR_LOGI("pdaf_sprd checksum error");
        return rtn;
    }
...
static cmr_int _otp_parser_pdaf2_v05(void *raw_data, cmr_uint start_offset,
                                     cmr_uint data_size, void *result) {
    cmr_int rtn = OTP_PARSER_SUCCESS;
...
static cmr_int _otp_parser_ae_v05(void *raw_data, cmr_uint start_offset,
                                  cmr_uint data_size, void *result) {
    cmr_int rtn = OTP_PARSER_SUCCESS;
...
static cmr_int _otp_parser_dualcamera_v05(void *raw_data, cmr_uint start_offset,
                                          cmr_uint data_size, void *result) {
    cmr_int rtn = OTP_PARSER_SUCCESS;
    ...
    rtn = _otp_parser_checksum(raw_data, dual_start_offset, 230,
                               dual_start_offset + 230);
    if (OTP_PARSER_SUCCESS != rtn) {
        CMR_LOGI("dual camera checksum error");
        return rtn;
    }

    rtn = _otp_parser_checksum(raw_data, dual_start_offset + 231, 25,
                               dual_start_offset + 231 + 25);
    if (OTP_PARSER_SUCCESS != rtn) {
        CMR_LOGI("dual camera checksum error");
        return rtn;
    }
...
cmr_int otp_parser_map_v05(void *raw_data, enum otp_parser_cmd cmd,
    ...
    rtn = _otp_parser_get_start_offset_v05((cmr_u8 *)raw_data, eeprom_num,
    ...
    case OTP_PARSER_LSC:
        ...
        rtn = _otp_parser_lsc_v05(raw_data, start_offset, data_size, raw_height,
                                  raw_width, result);
        break;
    ...
    case OTP_PARSER_SECTION_BOKEH:
        ...
        rtn = _otp_parser_dualcamera_v05(raw_data, start_offset, data_size,
                                         result);
        break;
    default:
