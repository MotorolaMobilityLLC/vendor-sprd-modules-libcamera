...
LOCAL unsigned long HI255_InitExt(unsigned long param)	//wujinyou, 2012.11.14
{
	...
	SENSOR_LOGI("HI255_InitExt start ;%ld;\n",param);
...
LOCAL unsigned long _HI255_PowerOn(unsigned long power_on)
{
	...
	SENSOR_LOGI("SENSOR: _HI255_PowerOn (1:on, 0:off): %d \n", power_on);
...
LOCAL unsigned long _HI255_set_brightness(unsigned long level)
{
	...
	SENSOR_LOGI("sensor: terry HI255_set_brightness = 0x%02x.\n", level);
	return 0;
}
...
LOCAL unsigned long _HI255_set_saturation(unsigned long level)
{
	...
	SENSOR_LOGI("sensor: terry HI255_set_saturation = 0x%02x.\n", level);
	return 0;
}
...
LOCAL unsigned long _HI255_set_image_effect(unsigned long effect_type)
{
	...
	SENSOR_LOGI("sensor: terry HI255_set_image_effect = 0x%02x.\n", effect_type);
	return 0;
}
...
LOCAL unsigned long _HI255_set_anti_flicker(unsigned long mode)
{
	...
	SENSOR_LOGI("sensor: terry _HI255_set_anti_flicker = 0x%02x", mode);
...
{//65.6 us
	...
	SENSOR_LOGI("sensor: terry _HI255_set_video_mode = 0x%02x", mode);
...
LOCAL unsigned long _HI255_set_awb(unsigned long mode)
{
	...
	SENSOR_LOGI("sensor: terry HI255_set_awb = 0x%02x", mode);
	return 0;
}
...
LOCAL unsigned long _HI255_BeforeSnapshot(unsigned long param)
{
	...
	param = param&0xffff;
	SENSOR_LOGE("%d,%ld.",cap_mode,param);

	SENSOR_LOGE("HI255: HI255_before_snapshot\n");
	if(SENSOR_MODE_PREVIEW_ONE>=param)
	{
	 	return SENSOR_SUCCESS;
	}
	SENSOR_LOGE("terry  _HI255_BeforeSnapshot =%ld.\n",param);
