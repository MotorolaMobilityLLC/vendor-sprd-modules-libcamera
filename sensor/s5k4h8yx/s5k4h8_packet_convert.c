...
int32_t s5k4h8_awb_package_convert(void *src_data, void *target_buf, uint32_t target_buf_size, uint32_t *real_size)
{
...
static void s5k4h8_change_pattern(uint8_t *dst, uint8_t *src, uint32_t chn_size, uint32_t pattern)
{
	uint32_t i = 0;
...
int32_t s5k4h8_lsc_package_convert(void *src_data, void *target_buf, uint32_t target_buf_size, uint32_t *real_size)
{
#if 1
	int i = 0;
	...
	*real_size = sizeof(struct lsc_target_packet) + LSC_GAIN_SIZE_PER_CH*LSC_CHN_NUM;
...
#else
	{
		...
		uint32_t pattern = 1;
#endif
		s5k4h8_change_pattern(dst_lsc, src_lsc, chn_gain_size, pattern);
	}
#endif
#else
