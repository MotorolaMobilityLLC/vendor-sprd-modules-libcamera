...
cmr_int sensor_ic_drv_create(struct sensor_ic_drv_init_para *init_param,
    ...
    *sns_ic_drv_handle = NULL;
    sns_drv_cxt = (struct sensor_ic_drv_cxt *)malloc(sizeof(struct sensor_ic_drv_cxt));
    if(!sns_drv_cxt) {
        ...
        sns_drv_cxt->ops_cb.set_mode_wait_done = init_param->ops_cb.set_mode_wait_done;
        sns_drv_cxt->ops_cb.get_mode = init_param->ops_cb.get_mode;

        *sns_ic_drv_handle = (cmr_handle)sns_drv_cxt;
    }
    CMR_LOGI("sns_drv_cxt:0x%x",sns_drv_cxt);
    return ret;
}
...
cmr_int sensor_ic_get_private_data(cmr_handle handle, cmr_uint cmd, void**param){
    ...
    if(cmd >= SENSOR_CMD_GET_MAX){
        SENSOR_LOGE("invalid cmd, cmd:0x%x",cmd);
...
cmr_int sensor_ic_set_match_fps_info(cmr_handle handle,
    ...
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    SENSOR_MODE_FPS_INFO_T *fps_info = (SENSOR_STATIC_INFO_T*)fps_info_ptr;
    SENSOR_LOGV("vendor_num:%d", vendor_num);
...
cmr_int sensor_ic_set_match_resolution_info(cmr_handle handle,
    ...
    for(i = 0; i < vendor_num; i++) {
         if(sns_drv_cxt->module_id == res_ptr[i].module_id){
            sns_drv_cxt->res_tab_info = &res_ptr[i].reg_tab;
            break;
         }
    }
    if(i == vendor_num) {
...
cmr_int sensor_ic_set_match_trim_info(cmr_handle handle,
    ...
    for(i = 0; i < vendor_num; i++) {
        if(sns_drv_cxt->module_id == trim_ptr[i].module_id) {
            sns_drv_cxt->trim_tab_info = &trim_ptr[i].trim_info;
            break;
        }
    }
    if(i == vendor_num) {
...
void sensor_ic_print_static_info(cmr_s8 *log_tag,
    ...
    SENSOR_LOGI("%s: exp_valid_frame_num: %d",
                      log_tag, ex_info->exp_valid_frame_num);
    ...
    SENSOR_LOGI("%s: sensor version info is: %s",
               log_tag, ex_info->sensor_version_info);
