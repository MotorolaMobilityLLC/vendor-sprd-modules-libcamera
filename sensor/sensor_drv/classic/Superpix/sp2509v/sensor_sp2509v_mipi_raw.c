...
#define LOG_TAG "sp2509v"

#include "sensor_sp2509v_mipi_raw_1lane.h"
...
static void sp2509v_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp2509v_drv_write_gain(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp2509v_drv_write_frame_length(cmr_handle handle,
                                           struct sensor_aec_i2c_tag *aec_info,
                                           cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp2509v_drv_write_shutter(cmr_handle handle,
                                      struct sensor_aec_i2c_tag *aec_info,
                                      cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 *============================================================================*/
static void sp2509v_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
...
static void sp2509v_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                  struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    sp2509v_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int sp2509v_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    BOOLEAN power_down = g_sp2509v_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_sp2509v_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int sp2509v_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int sp2509v_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        sp2509v_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_sp2509v_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_sp2509v_mipi_raw_info.sensor_version_info;
...
static cmr_int sp2509v_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int sp2509v_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGI("sensor sp2509v: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = sp2509v_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = sp2509v_drv_get_fps_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
        ret = sns_drv_cxt->is_sensor_close = 1;
        break;
    case SENSOR_VAL_TYPE_GET_PDAF_INFO:
        // ret = sp2509v_drv_get_pdaf_info(handle, param_ptr->pval);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int sp2509v_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, sp2509v_PID_ADDR);

    if (sp2509v_PID_VALUE == pid_value) {
        ver_value =
            hw_sensor_read_reg(sns_drv_cxt->hw_handle, sp2509v_VER_ADDR);
        SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                    ver_value);
        if (sp2509v_VER_VALUE == ver_value) {
            SENSOR_LOGI("this is sp2509v sensor");
 ...
 *============================================================================*/
static cmr_int sp2509v_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    sp2509v_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                              &sp2509v_aec_info);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.frame_length);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    sp2509v_drv_write_gain(handle, &sp2509v_aec_info, cap_gain);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.again);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int sp2509v_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    sp2509v_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                              &sp2509v_aec_info);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.frame_length);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int sp2509v_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    sp2509v_drv_calc_gain(handle, param, &sp2509v_aec_info);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.again);
    sp2509v_drv_write_reg2sensor(handle, sp2509v_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int sp2509v_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &sp2509v_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    sp2509v_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                              &sp2509v_aec_info);
    sp2509v_drv_calc_gain(handle, info->gain, &sp2509v_aec_info);

    return ret_value;
}

static cmr_int sp2509v_drv_set_master_FrameSync(cmr_handle handle,
                                                cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int sp2509v_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int sp2509v_drv_stream_off(cmr_handle handle, cmr_uint param) {
...
static cmr_int
sp2509v_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    sp2509v_drv_write_frame_length(
        sns_drv_cxt, &sp2509v_aec_info,
        sns_drv_cxt->sensor_ev_info.preview_framelength);
    sp2509v_drv_write_gain(sns_drv_cxt, &sp2509v_aec_info,
                           sns_drv_cxt->sensor_ev_info.preview_gain);
    sp2509v_drv_write_shutter(sns_drv_cxt, &sp2509v_aec_info,
                              sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_sp2509v_module_info_tab),
                                    s_sp2509v_module_info_tab);
    sensor_ic_set_match_resolution_info(
        sns_drv_cxt, ARRAY_SIZE(s_sp2509v_resolution_tab_raw),
        s_sp2509v_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_sp2509v_resolution_trim_tab),
                                  s_sp2509v_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_sp2509v_static_info), s_sp2509v_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_sp2509v_mode_fps_info),
                                 s_sp2509v_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    sp2509v_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int sp2509v_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int sp2509v_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                            void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = sp2509v_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_sp2509v_ops_tab = {
    .create_handle = sp2509v_drv_handle_create,
    .delete_handle = sp2509v_drv_handle_delete,
    .get_data = sp2509v_drv_get_private_data,
    /*---------------------------------------*/
    .power = sp2509v_drv_power_on,
    .identify = sp2509v_drv_identify,
    .ex_write_exp = sp2509v_drv_write_exposure,
    .write_gain_value = sp2509v_drv_write_gain_value,
    .read_aec_info = sp2509v_drv_read_aec_info,

    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = sp2509v_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = sp2509v_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = sp2509v_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = sp2509v_drv_access_val,
    }};
