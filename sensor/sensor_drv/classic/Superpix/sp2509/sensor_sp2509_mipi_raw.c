...
#if 1 // defined(CONFIG_CAMERA_ISP_VERSION_V3) ||
      // defined(CONFIG_CAMERA_ISP_VERSION_V4)
...
static const SENSOR_REG_T sp2509_snapshot_setting1[] = {
    {0xfd, 0x00}, {0x2f, 0x10}, {0x34, 0x00}, {0x30, 0x1d}, {0x33, 0x05},
    {0xfd, 0x01}, {0x44, 0x00}, {0x2a, 0x5c}, {0x2c, 0x60}, {0x25, 0x06},
    {0x03, 0x01}, {0x04, 0x71}, {0x09, 0x00}, {0x0a, 0x0a}, {0x06, 0x0a},
    {0x24, 0x80}, {0x01, 0x01}, {0xfb, 0x63}, {0xfd, 0x01}, {0x16, 0x04},
    {0x1c, 0x09}, {0x21, 0x66}, {0x6a, 0x00}, {0x6b, 0x00}, {0x84, 0x00},
    {0x85, 0x10}, {0x86, 0x10}, {0x13, 0x60}, {0x11, 0x20}, {0x33, 0x60},
    {0xd0, 0x03}, {0xd1, 0x01}, {0xd2, 0x20}, {0xd3, 0x01}, {0xd4, 0xa0},
    {0x58, 0x10}, {0x75, 0xa0}, {0x51, 0x14}, {0x52, 0x10}, {0x19, 0x74},
    {0xfd, 0x01}, {0x9d, 0x96}, {0xa1, 0x04}, {0xb1, 0x01}, {0xac, 0x01},
};
...
static SENSOR_REG_TAB_INFO_T s_sp2509_resolution_tab_raw[SENSOR_MODE_MAX] = {
    ...
    /*{ADDR_AND_LEN_OF_ARRAY(sp2509_preview_setting),
     ...
     SENSOR_IMAGE_FORMAT_RAW},*/
    {ADDR_AND_LEN_OF_ARRAY(sp2509_snapshot_setting), SNAPSHOT_WIDTH,
...
static const SENSOR_REG_T
    s_sp2509_preview_size_video_tab[SENSOR_VIDEO_MODE_MAX][1] = {
        /*video mode 0: ?fps */
        {{0xffff, 0xff}},
        /* video mode 1:?fps */
        {{0xffff, 0xff}},
...
static const SENSOR_REG_T
    s_sp2509_capture_size_video_tab[SENSOR_VIDEO_MODE_MAX][1] = {
        /*video mode 0: ?fps */
        {{0xffff, 0xff}},
        /* video mode 1:?fps */
        {{0xffff, 0xff}},
...
static uint32_t sp2509_set_video_mode(SENSOR_HW_HANDLE handle, uint32_t param) {
    ...
    sensor_reg_ptr =
        (SENSOR_REG_T_PTR)&s_sp2509_video_info[mode].setting_ptr[param];
...
SENSOR_INFO_T g_sp2509_mipi_raw_info = {
     ...
     */
    {{sp2509_PID_ADDR, sp2509_PID_VALUE}, {sp2509_VER_ADDR, sp2509_VER_VALUE}},
    /* voltage of avdd */
    SENSOR_AVDD_2800MV,
...
LOCAL uint32_t _sp2509_init_mode_fps_info(SENSOR_HW_HANDLE handle) {
    ...
    if (!s_sp2509_mode_fps_info.is_init) {
        ...
        for (i = 0; i < NUMBER_OF_ARRAY(s_sp2509_resolution_trim_tab); i++) {
            ...
            // frame_line
            tempfps = s_sp2509_resolution_trim_tab[i].line_time *
            ...
            if (0 != tempfps) {
                ...
                if (s_sp2509_mode_fps_info.sensor_mode_fps[i].max_fps > 30) {
                    ...
                    s_sp2509_mode_fps_info.sensor_mode_fps[i]
                        .high_fps_skip_num =
            ...
            SENSOR_PRINT("mode %d,tempfps %d,frame_len %d,line_time: %d ", i,
                         tempfps, s_sp2509_resolution_trim_tab[i].frame_line,
...
#define param_update(x1, x2)                                                   \
    sprintf(name, "/data/sp2509_%s.bin", x1);                                  \
 ...
 *
 *============================================================================*/
static uint32_t sp2509_get_default_frame_length(SENSOR_HW_HANDLE handle,
                                                uint32_t mode) {
...
static void sp2509_write_gain(SENSOR_HW_HANDLE handle, float gain) {
    ...
    SENSOR_PRINT("real_gain = 0x%x gain_a %x %x", (uint32_t)gain,
                 (uint8_t)gain_a, (uint8_t)gain_d);
    Sensor_WriteReg(0xfd, 0x01);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp2509_write_frame_length(SENSOR_HW_HANDLE handle,
                                      uint32_t frame_len) {
    Sensor_WriteReg(0xfd, 0x01);
 ...
 *============================================================================*/
static uint16_t sp2509_update_exposure(SENSOR_HW_HANDLE handle,
    ...
    SENSOR_PRINT("current shutter = %d, fr_len = %d, dummy_line=%d cur_fr_len "
                 "%d dest_fr_len %d",
                 shutter, fr_len, dummy_line, cur_fr_len, dest_fr_len);
...
module_id=0x%x",s_sp2509_raw_param_tab[i].param_id);
                ...
                if(PNULL!=s_sp2509_raw_param_tab[i].identify_otp){
                        ...
                        if(SENSOR_SUCCESS==s_sp2509_raw_param_tab[i].identify_otp(otp_info)){
                                if (s_sp2509_raw_param_tab[i].param_id==
otp_info->module_id) {
...
module_id=0x%x",s_sp2509_raw_param_tab[i].param_id);
                                        ...
                                        SENSOR_PRINT("identify module_id failed!
table module_id=0x%x, otp
module_id=0x%x",s_sp2509_raw_param_tab[i].param_id,otp_info->module_id);
                                }
                        }
                        else{
...
/*static uint32_t sp2509_apply_otp(struct otp_info_t *otp_info, int id)
{
        ...
        if (PNULL != s_sp2509_raw_param_tab[id].cfg_otp) {
                ...
                if(SENSOR_SUCCESS==s_sp2509_raw_param_tab[id].cfg_otp(otp_info)){
                        SENSOR_PRINT("apply otp parameters sucess!
module_id=0x%x",s_sp2509_raw_param_tab[id].param_id);
                        ret = SENSOR_SUCCESS;
                }
                else{
 ...
 *
 *============================================================================*/
static unsigned long sp2509_get_resolution_trim_tab(SENSOR_HW_HANDLE handle,
                                                    uint32_t param) {
 ...
 *============================================================================*/
static uint32_t sp2509_before_snapshot(SENSOR_HW_HANDLE handle,
                                       uint32_t param) {
    ...
    uint32_t prv_linetime =
        s_sp2509_resolution_trim_tab[preview_mode].line_time;
    uint32_t cap_linetime =
        s_sp2509_resolution_trim_tab[capture_mode].line_time;

    s_current_default_frame_length =
        sp2509_get_default_frame_length(handle, capture_mode);
 ...
 *============================================================================*/
static uint32_t sp2509_write_exposure_dummy(SENSOR_HW_HANDLE handle,
    ...
    s_current_default_frame_length =
        sp2509_get_default_frame_length(handle, size_index);

    s_sensor_ev_info.preview_shutter =
        sp2509_update_exposure(handle, exposure_line, dummy_line);

    return ret_value;
}

LOCAL unsigned long sp2509_write_exposure(SENSOR_HW_HANDLE handle,
                                          unsigned long param) {
    uint32_t ret_value = SENSOR_SUCCESS;
    ...
    ret_value = sp2509_write_exposure_dummy(handle, expsure_line, dummy_line,
                                            size_index);

    return ret_value;
}

LOCAL unsigned long sp2509_ex_write_exposure(SENSOR_HW_HANDLE handle,
                                             unsigned long param) {
    uint32_t ret_value = SENSOR_SUCCESS;
    uint16_t exposure_line = 0x00;
    uint16_t dummy_line = 0x00;
    ...
    size_index = ex->size_index;
    SENSOR_PRINT(
        "current mode = %d, exposure_line = %d, dummy_line=%d frame_length %d",
        size_index, exposure_line, dummy_line, s_current_default_frame_length);
    s_current_default_frame_length =
        sp2509_get_default_frame_length(handle, size_index);
    s_current_default_line_time =
        s_sp2509_resolution_trim_tab[size_index].line_time;

    ret_value = sp2509_write_exposure_dummy(handle, exposure_line, dummy_line,
                                            size_index);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t sp2509_write_gain_value(SENSOR_HW_HANDLE handle,
                                        unsigned long param) {
    uint32_t ret_value = SENSOR_SUCCESS;
    float real_gain = 0;
...
static struct sensor_reg_tag sp2509_shutter_reg[] = {
    {0xfd, 0x01}, {0x03, 0x00}, {0x04, 0x00}, {0x01, 0x01},
};

static struct sensor_i2c_reg_tab sp2509_shutter_tab = {
    .settings = sp2509_shutter_reg, .size = ARRAY_SIZE(sp2509_shutter_reg),
};

static struct sensor_reg_tag sp2509_again_reg[] = {
    {0xfd, 0x01}, {0x24, 0x00}, {0x37, 0x00}, {0x01, 0x01},
};

static struct sensor_i2c_reg_tab sp2509_again_tab = {
    .settings = sp2509_again_reg, .size = ARRAY_SIZE(sp2509_again_reg),
};

static struct sensor_reg_tag sp2509_dgain_reg[] = {
    {0xfd, 0x02}, {0x10, 0x00}, {0x11, 0x00}, {0x13, 0x00}, {0x14, 0x00},
};

struct sensor_i2c_reg_tab sp2509_dgain_tab = {
    .settings = sp2509_dgain_reg, .size = ARRAY_SIZE(sp2509_dgain_reg),
};
...
static uint16_t sp2509_calc_exposure(SENSOR_HW_HANDLE handle, uint32_t shutter,
...
static unsigned long sp2509_read_aec_info(SENSOR_HW_HANDLE handle,
    ...
    frame_interval =
        (uint16_t)(((exposure_line + dummy_line) *
                    (s_sp2509_resolution_trim_tab[mode].line_time)) /
                   1000000);
    SENSOR_PRINT(
    ...
    s_current_default_frame_length =
        sp2509_get_default_frame_length(handle, mode);
    s_current_default_line_time = s_sp2509_resolution_trim_tab[mode].line_time;

    s_sensor_ev_info.preview_shutter = sp2509_calc_exposure(
        handle, exposure_line, dummy_line, mode, &sp2509_aec_info);
 ...
 *
 *============================================================================*/
static void sp2509_increase_hdr_exposure(SENSOR_HW_HANDLE handle,
 ...
 *
 *============================================================================*/
static void sp2509_decrease_hdr_exposure(SENSOR_HW_HANDLE handle,
                                         uint8_t ev_divisor) {
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t sp2509_set_hdr_ev(SENSOR_HW_HANDLE handle,
                                  unsigned long param) {
...
static unsigned long _sp2509_SetSlave_FrameSync(SENSOR_HW_HANDLE handle,
                                                unsigned long param) {
...
static uint32_t sp2509_get_static_info(SENSOR_HW_HANDLE handle,
                                       uint32_t *param) {
    uint32_t rtn = SENSOR_SUCCESS;
    ...
    ex_info->adgain_valid_frame_num =
        s_sp2509_static_info.adgain_valid_frame_num;
    ...
    SENSOR_LOGI("SENSOR_sp2509: exp_valid_frame_num: %d",
                ex_info->exp_valid_frame_num);
    SENSOR_LOGI("SENSOR_sp2509: clam_level: %d", ex_info->clamp_level);
    SENSOR_LOGI("SENSOR_sp2509: adgain_valid_frame_num: %d",
                ex_info->adgain_valid_frame_num);
    SENSOR_LOGI("SENSOR_sp2509: sensor name is: %s", ex_info->name);
    SENSOR_LOGI("SENSOR_sp2509: sensor version info is: %s",
                ex_info->sensor_version_info);
...
static uint32_t sp2509_get_fps_info(SENSOR_HW_HANDLE handle, uint32_t *param) {
    ...
    fps_info->max_fps =
        s_sp2509_mode_fps_info.sensor_mode_fps[sensor_mode].max_fps;
    fps_info->min_fps =
        s_sp2509_mode_fps_info.sensor_mode_fps[sensor_mode].min_fps;
    fps_info->is_high_fps =
        s_sp2509_mode_fps_info.sensor_mode_fps[sensor_mode].is_high_fps;
    fps_info->high_fps_skip_num =
        s_sp2509_mode_fps_info.sensor_mode_fps[sensor_mode].high_fps_skip_num;
    SENSOR_LOGI("SENSOR_sp2509: mode %d, max_fps: %d", fps_info->mode,
                fps_info->max_fps);
    SENSOR_LOGI("SENSOR_sp2509: min_fps: %d", fps_info->min_fps);
    SENSOR_LOGI("SENSOR_sp2509: is_high_fps: %d", fps_info->is_high_fps);
    SENSOR_LOGI("SENSOR_sp2509: high_fps_skip_num: %d",
                fps_info->high_fps_skip_num);
...
static unsigned long sp2509_access_val(SENSOR_HW_HANDLE handle,
                                       unsigned long param) {
    uint32_t rtn = SENSOR_SUCCESS;
    SENSOR_VAL_T *param_ptr = (SENSOR_VAL_T *)param;
    uint16_t tmp;

    SENSOR_LOGI("SENSOR_sp2509: _sp2509_access_val E param_ptr = %p",
                param_ptr);
    if (!param_ptr) {
        return rtn;
    }
