...
static void sp0a09z_drv_write_reg2sensor(cmr_handle handle, struct sensor_i2c_reg_tab *reg_info)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp0a09z_drv_write_gain(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 gain)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp0a09z_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp0a09z_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static void sp0a09z_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
                                                  cmr_u16 mode, struct sensor_aec_i2c_tag *aec_info)
{
	...
	if (dest_fr_len != cur_fr_len){
		...
		sp0a09z_drv_write_frame_length(handle, aec_info, dest_fr_len);
	}
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
	sp0a09z_drv_write_shutter(handle, aec_info, shutter);
...
static void sp0a09z_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) 
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	sp0a09z_drv_write_gain(handle, aec_info,sensor_gain);
...
static cmr_int sp0a09z_drv_get_static_info(cmr_handle handle, cmr_u32 *param) 
{
    ...
    ex_info->name = (cmr_s8 *)g_sp0a09z_mipi_raw_info.name;
    ex_info->sensor_version_info = (cmr_s8 *)g_sp0a09z_mipi_raw_info.sensor_version_info;
...
static cmr_int sp0a09z_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
    ...
    sp0a09z_drv_calc_exposure(handle,cap_shutter, 0 , capture_mode, &sp0a09z_aec_info);
	sp0a09z_drv_write_reg2sensor(handle, sp0a09z_aec_info.frame_length);
	sp0a09z_drv_write_reg2sensor(handle, sp0a09z_aec_info.shutter);

	sns_drv_cxt->sensor_ev_info.preview_gain=cap_gain;
	sp0a09z_drv_write_gain(handle, &sp0a09z_aec_info, cap_gain);
	sp0a09z_drv_write_reg2sensor(handle, sp0a09z_aec_info.again);
	sp0a09z_drv_write_reg2sensor(handle, sp0a09z_aec_info.dgain);
...
static cmr_int sp0a09z_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	sp0a09z_drv_calc_exposure(handle,exposure_line, dummy_line, size_index, &sp0a09z_aec_info);
	sp0a09z_drv_write_reg2sensor(handle, sp0a09z_aec_info.frame_length);
	sp0a09z_drv_write_reg2sensor(handle, sp0a09z_aec_info.shutter);
...
static cmr_int sp0a09z_drv_read_aec_info(cmr_handle handle, cmr_uint param) 
{
    ...
    sp0a09z_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &sp0a09z_aec_info);
    sp0a09z_drv_calc_gain(handle,info->gain, &sp0a09z_aec_info);

    return ret_value;
}

static cmr_int sp0a09z_drv_set_master_FrameSync(cmr_handle handle, cmr_uint param) 
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int sp0a09z_drv_stream_on(cmr_handle handle, cmr_uint param)
{
	...
	//sp0a09z_drv_set_slave_FrameSync(handle, param);
#endif   
	/*TODO*/
...
static cmr_int sp0a09z_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle) 
{
	...
	sp0a09z_drv_write_frame_length(sns_drv_cxt, &sp0a09z_aec_info, sns_drv_cxt->sensor_ev_info.preview_framelength);
	sp0a09z_drv_write_gain(sns_drv_cxt, &sp0a09z_aec_info, sns_drv_cxt->sensor_ev_info.preview_gain);
	sp0a09z_drv_write_shutter(sns_drv_cxt, &sp0a09z_aec_info, sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_sp0a09z_module_info_tab), s_sp0a09z_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_sp0a09z_resolution_tab_raw), s_sp0a09z_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_sp0a09z_resolution_trim_tab), s_sp0a09z_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_sp0a09z_static_info), s_sp0a09z_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_sp0a09z_mode_fps_info), s_sp0a09z_mode_fps_info);
...
static cmr_int sp0a09z_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
#if defined(CONFIG_DUAL_MODULE)
	//.read_aec_info = sp0a09z_drv_read_aec_info,
#endif
