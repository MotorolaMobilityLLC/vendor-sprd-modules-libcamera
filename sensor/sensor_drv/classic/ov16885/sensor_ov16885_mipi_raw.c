...
static void ov16885_drv_write_reg2sensor(cmr_handle handle, struct sensor_i2c_reg_tab *reg_info)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov16885_drv_write_gain(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 gain)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov16885_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov16885_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static void ov16885_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
...
static void ov16885_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) 
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int ov16885_drv_get_static_info(cmr_handle handle, cmr_u32 *param) 
{
    ...
    ex_info->sensor_version_info = (cmr_s8 *)g_ov16885_mipi_raw_info.sensor_version_info;
...
static cmr_int ov16885_drv_ov4c_init(cmr_handle handle, cmr_u32 *param) 
{
    //      #include "fcell.h"
    cmr_int rtn = SENSOR_SUCCESS;
//    ov4c_init();//unsigned short *xtalk_data, unsigned short *otp_data, int otpdpc_len)
#if 1
    int ret = E_OV_FCD_OK;
  ...
  FILE *fp1 = fopen(fcellXtalkFile, "rb");
   ...
   init.xtalk = pFcellXtalk;//xtalk_data;//pFcellXtalk;//TODO
        ...
        hw_sensor_read_i2c(sns_drv_cxt->hw_handle, 0xa0 >> 1, param_ptr,0x160f+600 << 16 |SENSOR_I2C_REG_16BIT);
    //    for(int i = 0; i <600; i++)
    //            param_ptr[i] = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x160f+i);
    //    SENSOR_LOGI("xtalk: pid_value = %x, ver_value = %x", param_ptr[0x160f], param_ptr[0x160f+1]);

    init.xtalk = param_ptr+0x160f;//xtalk_data;//pFcellXtalk;//TODO
#endif
...
static cmr_int ov16885_drv_identify(cmr_handle handle, cmr_uint param)
{
	...
	if (ov16885_PID_VALUE == pid_value) {
		ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov16885_VER_ADDR);
		...
		if (ov16885_VER_VALUE == ver_value) {
			...
			ret_value = SENSOR_SUCCESS;
                ...
                property_get("debug.camera.4in1.capture", value1, "0");
                if (!strcmp(value1, "1"))
                    ov16885_drv_ov4c_init(sns_drv_cxt, &param);
               // ov16885_drv_ov4c_process(sns_drv_cxt, &ret);
...
static cmr_int ov16885_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
    ...
    ov16885_drv_calc_exposure(handle,cap_shutter, 0 , capture_mode,&ov16885_aec_info);
...
static cmr_int ov16885_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	ov16885_drv_calc_exposure(handle,exposure_line, dummy_line, size_index, &ov16885_aec_info);
	ov16885_drv_write_reg2sensor(handle, ov16885_aec_info.frame_length);
	ov16885_drv_write_reg2sensor(handle, ov16885_aec_info.shutter);
...
static cmr_int ov16885_drv_read_aec_info(cmr_handle handle, cmr_uint param) 
{
    ...
    ov16885_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &ov16885_aec_info);
    ov16885_drv_calc_gain(handle,info->gain, &ov16885_aec_info);

    return ret_value;
}

static cmr_int ov16885_drv_set_master_FrameSync(cmr_handle handle, cmr_uint param) 
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int ov16885_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle) 
{
	...
	ov16885_drv_write_frame_length(sns_drv_cxt, &ov16885_aec_info, sns_drv_cxt->sensor_ev_info.preview_framelength);
	ov16885_drv_write_gain(sns_drv_cxt, &ov16885_aec_info, sns_drv_cxt->sensor_ev_info.preview_gain);
	ov16885_drv_write_shutter(sns_drv_cxt, &ov16885_aec_info, sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_ov16885_module_info_tab), s_ov16885_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_ov16885_resolution_tab_raw), s_ov16885_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_ov16885_resolution_trim_tab), s_ov16885_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_ov16885_static_info), s_ov16885_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_ov16885_mode_fps_info), s_ov16885_mode_fps_info);
...
static cmr_int ov16885_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
