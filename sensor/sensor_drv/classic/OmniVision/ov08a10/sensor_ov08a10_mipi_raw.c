...
#define LOG_TAG "ov08a10"

// #if defined(OV08A10_SHINE_MIPI_4LANE)
// #include "sensor_ov08a10_mipi_raw_4lane.h"
// #else
#include "sensor_ov08a10_mipi_raw_2lane.h"
//#endif
...
static void ov08a10_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov08a10_drv_write_gain(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov08a10_drv_write_frame_length(cmr_handle handle,
                                           struct sensor_aec_i2c_tag *aec_info,
                                           cmr_u32 frame_len) {
  SENSOR_IC_CHECK_PTR_VOID(aec_info);
  SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov08a10_drv_write_shutter(cmr_handle handle,
                                      struct sensor_aec_i2c_tag *aec_info,
                                      cmr_u32 shutter) {
  SENSOR_IC_CHECK_PTR_VOID(aec_info);
  SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 *============================================================================*/
static void ov08a10_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
  ...
  //if (dest_fr_len != cur_fr_len) {
    ...
    ov08a10_drv_write_frame_length(handle, aec_info, dest_fr_len);
  //}
  sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
  ov08a10_drv_write_shutter(handle, aec_info, shutter);
...
static void ov08a10_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                  struct sensor_aec_i2c_tag *aec_info) {
  SENSOR_IC_CHECK_HANDLE_VOID(handle);
  struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08a10_drv_power_on(cmr_handle handle, cmr_uint power_on) {
  ...
  BOOLEAN power_down = g_ov08a10_mipi_raw_info.power_down_level;
  BOOLEAN reset_level = g_ov08a10_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08a10_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int ov08a10_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
  cmr_int rtn = SENSOR_SUCCESS;
  ...
  if (!fps_info->is_init) {
    ov08a10_drv_init_fps_info(handle);
  }
  ...
  ex_info->name = (cmr_s8 *)g_ov08a10_mipi_raw_info.name;
  ex_info->sensor_version_info =
      (cmr_s8 *)g_ov08a10_mipi_raw_info.sensor_version_info;
...
static cmr_int ov08a10_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
  cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08a10_drv_access_val(cmr_handle handle, cmr_uint param) {
  ...
  SENSOR_LOGI("sensor ov08a10: param_ptr->type=%x", param_ptr->type);

  switch (param_ptr->type) {
  case SENSOR_VAL_TYPE_GET_STATIC_INFO://16
    ret = ov08a10_drv_get_static_info(handle, param_ptr->pval);
    break;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08a10_drv_identify(cmr_handle handle, cmr_uint param) {
  ...
  pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov08a10_PID_ADDR);

  if (ov08a10_PID_VALUE == pid_value) {
      ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov08a10_VER_ADDR);
      SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                  ver_value);
      if (ov08a10_VER_VALUE == ver_value) {
          SENSOR_LOGI("this is ov08a10 sensor");
 ...
 *============================================================================*/
static cmr_int ov08a10_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
  ...
  ov08a10_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                            &ov08a10_aec_info);
  ov08a10_drv_write_reg2sensor(handle, ov08a10_aec_info.frame_length);
  ov08a10_drv_write_reg2sensor(handle, ov08a10_aec_info.shutter);

  sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
  ov08a10_drv_write_gain(handle, &ov08a10_aec_info, cap_gain);
  ov08a10_drv_write_reg2sensor(handle, ov08a10_aec_info.again);
  ov08a10_drv_write_reg2sensor(handle, ov08a10_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int ov08a10_drv_write_exposure(cmr_handle handle, cmr_uint param) {
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int ov08a10_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
  cmr_int ret_value = SENSOR_SUCCESS;

  SENSOR_IC_CHECK_HANDLE(handle);
  struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

  ov08a10_drv_calc_gain(handle, param, &ov08a10_aec_info);
  ov08a10_drv_write_reg2sensor(handle, ov08a10_aec_info.again);
  ov08a10_drv_write_reg2sensor(handle, ov08a10_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int ov08a10_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
  cmr_int ret_value = SENSOR_SUCCESS;
  ...
  ov08a10_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                            &ov08a10_aec_info);
  ov08a10_drv_calc_gain(handle, info->gain, &ov08a10_aec_info);

  return ret_value;
}

static cmr_int ov08a10_drv_set_master_FrameSync(cmr_handle handle,
                                                cmr_uint param) {
  SENSOR_IC_CHECK_HANDLE(handle);
  struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08a10_drv_stream_on(cmr_handle handle, cmr_uint param) {
  SENSOR_IC_CHECK_HANDLE(handle);
  struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
// ov08a10_drv_set_master_FrameSync(handle, param);
#endif
  }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08a10_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
...
static cmr_int
ov08a10_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
  ...
  ov08a10_drv_write_frame_length(
      sns_drv_cxt, &ov08a10_aec_info,
      sns_drv_cxt->sensor_ev_info.preview_framelength);
  ov08a10_drv_write_gain(sns_drv_cxt, &ov08a10_aec_info,
                         sns_drv_cxt->sensor_ev_info.preview_gain);
  ov08a10_drv_write_shutter(sns_drv_cxt, &ov08a10_aec_info,
                            sns_drv_cxt->sensor_ev_info.preview_shutter);

  sensor_ic_set_match_module_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_ov08a10_module_info_tab),
                                  s_ov08a10_module_info_tab);
  sensor_ic_set_match_resolution_info(sns_drv_cxt,
                                      ARRAY_SIZE(s_ov08a10_resolution_tab_raw),
                                      s_ov08a10_resolution_tab_raw);
  sensor_ic_set_match_trim_info(sns_drv_cxt,
                                ARRAY_SIZE(s_ov08a10_resolution_trim_tab),
                                s_ov08a10_resolution_trim_tab);
  sensor_ic_set_match_static_info(
      sns_drv_cxt, ARRAY_SIZE(s_ov08a10_static_info), s_ov08a10_static_info);
  sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_ov08a10_mode_fps_info),
                               s_ov08a10_mode_fps_info);

  /*init exif info,this will be deleted in the future*/
  ov08a10_drv_init_fps_info(sns_drv_cxt);

  /*add private here*/
  return ret;
}

static cmr_int ov08a10_drv_handle_delete(cmr_handle handle, void *param) {
  cmr_int ret = SENSOR_SUCCESS;
...
void *sensor_ic_open_lib(void) { return &g_ov08a10_mipi_raw_info; }

static cmr_int ov08a10_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                            void **param) {
  cmr_int ret = SENSOR_SUCCESS;
  SENSOR_IC_CHECK_HANDLE(handle);
  SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = ov08a10_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_ov08a10_ops_tab = {
    .create_handle = ov08a10_drv_handle_create,
    .delete_handle = ov08a10_drv_handle_delete,
    .get_data = ov08a10_drv_get_private_data,
    /*---------------------------------------*/
    .power = ov08a10_drv_power_on,
    .identify = ov08a10_drv_identify,
    .ex_write_exp = ov08a10_drv_write_exposure,
    ...
    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = ov08a10_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = ov08a10_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = ov08a10_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = ov08a10_drv_access_val,
    }};
