...
static void ov8856_drv_write_reg2sensor(cmr_handle handle, struct sensor_i2c_reg_tab *reg_info)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static void ov8856_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
...
static void ov8856_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) 
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int ov8856_drv_get_static_info(cmr_handle handle, cmr_u32 *param) 
{
    ...
    ex_info->sensor_version_info = (cmr_s8 *)g_ov8856_shine_mipi_raw_info.sensor_version_info;
...
{
	return s_ov8856_shine_mipi_raw_info_ptr;
}
LOCAL uint32_t Sensor_InitRawTuneInfo(struct sensor_ic_drv_cxt * sns_drv_cxt)
{
	uint32_t rtn=0x00;

	//struct sensor_drv_context *sensor_cxt = (struct sensor_drv_context *)sns_drv_cxt->caller_handle;
	//struct sensor_raw_info* raw_sensor_ptr = sensor_cxt->sensor_info_ptr
	...
	//isp_raw_para_update_from_file(&g_ov5640_mipi_raw_info,0);

	SENSOR_LOGI("Bill, initRaw ISP_BLK_IIRCNR_YRANDOM 0x%x", ISP_BLK_IIRCNR_YRANDOM);
	for (i=0; i<mode_common_ptr->block_num; i++) {
		...
		SENSOR_LOGI("Bill, initRaw block_id 0x%x", header->block_id);
		...
		case	ISP_BLK_PRE_WAVELET_V1: {
				...
				static struct sensor_pwd_level pwd_param[] = {
					#include "paramerers_4lane/isp_nr.h"
				};
				#undef _NR_PWD_PARAM_
				block->param_ptr = (isp_uint *)pwd_param;
				block->pwd_level.bypass = 1; //Bill
			}
			break;
		...
		case	ISP_BLK_RGB_PRECDN: {
				...
				block->rgb_precdn_level.bypass = 1;
			}
			break;
		...
		case	ISP_BLK_YUV_PRECDN: {
				...
				block->yuv_precdn_level.bypass = 1;
			}
			break;
...
static cmr_int ov8856_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
    ...
    ov8856_drv_calc_exposure(handle,cap_shutter, 0 , capture_mode,&ov8856_aec_info);
...
static cmr_int ov8856_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	ov8856_drv_calc_exposure(handle,exposure_line, dummy_line, size_index, &ov8856_aec_info);
	ov8856_drv_write_reg2sensor(handle, ov8856_aec_info.frame_length);
	ov8856_drv_write_reg2sensor(handle, ov8856_aec_info.shutter);
...
static cmr_int ov8856_drv_read_aec_info(cmr_handle handle, cmr_uint param) 
{
    ...
    ov8856_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &ov8856_aec_info);
    ov8856_drv_calc_gain(handle,info->gain, &ov8856_aec_info);

    return ret_value;
}

static cmr_int ov8856_drv_set_master_FrameSync(cmr_handle handle, cmr_uint param) 
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int ov8856_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
