...
#define FPS_INFO       s_ov8856_mode_fps_info
#define STATIC_INFO    s_ov8856_static_info
#define MODULE_INFO    s_ov8856_module_info_tab
/*==============================================================================
 * Description:
 * write group-hold on to sensor registers
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_drv_group_hold_on(cmr_handle handle) { SENSOR_LOGI("E"); }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_drv_group_hold_off(cmr_handle handle) {
    SENSOR_LOGI("E");
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_u16 ov8856_drv_read_gain(cmr_handle handle) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_drv_write_gain(cmr_handle handle, float gain) {
    float gain_a = gain;
    float gain_d = 0x400;
    ...
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x3508, ((cmr_u16)gain_a >> 8) & 0x07);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x3509, (cmr_u16)gain_a & 0xff);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x5019, ((cmr_u16)gain_d >> 8) & 0x07);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x501a, (cmr_u16)gain_d & 0xff);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x501b, ((cmr_u16)gain_d >> 8) & 0x07);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x501c, (cmr_u16)gain_d & 0xff);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x501d, ((cmr_u16)gain_d >> 8) & 0x07);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x501e, (cmr_u16)gain_d & 0xff);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x501f, ((cmr_u16)gain_d >> 8) & 0x07);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x5020, (cmr_u16)gain_d & 0xff);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_u16 ov8856_drv_read_frame_length(cmr_handle handle) {
    cmr_u32 frame_len;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    return sns_drv_cxt->sensor_ev_info.preview_framelength;
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_u16 ov8856_drv_read_shutter(cmr_handle handle) {
    cmr_u32 value = 0x00;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_drv_write_shutter(cmr_handle handle, uint32_t shutter) {
    cmr_u16 value = 0x00;
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static cmr_int ov8856_drv_write_exposure_dummy(cmr_handle handle,
                                                           cmr_u32 shutter,
                                                           cmr_u32 dummy_line,
                                                           cmr_u16 size_index) {
    ...
    cur_fr_len = ov8856_drv_read_frame_length(handle);

    if (shutter < SENSOR_MIN_SHUTTER)
        shutter = SENSOR_MIN_SHUTTER;

    if (dest_fr_len != cur_fr_len)
        ov8856_drv_write_frame_length(handle, dest_fr_len);
write_sensor_shutter:
    /* write shutter to sensor registers */
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    ov8856_drv_write_shutter(handle, shutter);
...
static cmr_int ov8856_drv_get_static_info(cmr_handle handle,
                                       cmr_uint *param) {
    cmr_int rtn = SENSOR_SUCCESS;
...
static cmr_int ov8856_drv_get_fps_info(cmr_handle handle, cmr_uint *param) {
    cmr_int rtn = SENSOR_SUCCESS;
...
static cmr_int ov8856_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    case SENSOR_VAL_TYPE_SHUTTER:
        *((cmr_u32 *)param_ptr->pval) = ov8856_drv_read_shutter(handle);
        break;
    case SENSOR_VAL_TYPE_READ_OTP_GAIN:
        *((cmr_u32 *)param_ptr->pval) = ov8856_drv_read_gain(handle);
        break;
 ...
 *============================================================================*/
static cmr_int ov8856_drv_before_snapshot(cmr_handle handle,
                                                    cmr_int param) {
    ...
    cap_shutter = ov8856_drv_write_exposure_dummy(handle, cap_shutter, 0, 0);
    cap_gain = gain;
    ov8856_drv_write_gain(handle, cap_gain);
 ...
 *============================================================================*/
static cmr_int ov8856_drv_write_exposure(cmr_handle handle,
                                           cmr_int param) {
    ...
    ret_value = ov8856_drv_write_exposure_dummy(handle, exposure_line, dummy_line,
                                            size_index);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int ov8856_drv_write_gain_value(cmr_handle handle,
                                                      cmr_int param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    float real_gain = 0;
    SENSOR_IC_CHECK_HANDLE(handle);
    ...
    ov8856_drv_write_gain(handle, real_gain);
...
static cmr_u16 ov8856_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    line_time = trim_info[mode].line_time;
...
static void ov8856_drv_calc_gain(float gain, struct sensor_aec_i2c_tag *aec_info) {
    float gain_a = gain;
    float gain_d = 0x400;
    ...
    aec_info->again->settings[1].reg_value = ((cmr_u16)gain_a >> 8) & 0x07;
    aec_info->again->settings[2].reg_value = (cmr_u16)gain_a & 0xff;

    aec_info->dgain->settings[0].reg_value = ((cmr_u16)gain_d >> 8) & 0x07;
    aec_info->dgain->settings[1].reg_value = (cmr_u16)gain_d & 0xff;
    aec_info->dgain->settings[2].reg_value = ((cmr_u16)gain_d >> 8) & 0x07;
    aec_info->dgain->settings[3].reg_value = (cmr_u16)gain_d & 0xff;
    aec_info->dgain->settings[4].reg_value = ((cmr_u16)gain_d >> 8) & 0x07;
    aec_info->dgain->settings[5].reg_value = (cmr_u16)gain_d & 0xff;
    aec_info->dgain->settings[6].reg_value = ((cmr_u16)gain_d >> 8) & 0x07;
...
static cmr_int ov8856_drv_read_aec_info(cmr_handle handle, void *param) {
    ...
    sns_drv_cxt->sensor_ev_info.preview_shutter = ov8856_calc_exposure(
    ...
    ov8856_drv_calc_gain(real_gain, &ov8856_aec_info);

    return ret_value;
}

cmr_int ov8856_drv_set_frameSync(cmr_handle handle,cmr_uint param) {
    UNUSED(param);
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int ov8856_drv_stream_on(cmr_handle handle, cmr_uint param) {
    ...
    ov8856_drv_set_frameSync(handle, param);
...
static cmr_int ov8856_drv_handle_create(
...
static cmr_int ov8856_drv_handle_delete(cmr_handle handle, cmr_int *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int ov8856_drv_get_private_data(cmr_handle handle,
