...
static void ov8856_write_gain(SENSOR_HW_HANDLE handle,float gain)
{
	...
	//SENSOR_PRINT("real_gain = %f %d %d", gain,(uint16_t)gain_a,(uint16_t)gain_d);
 ...
 *============================================================================*/
static uint16_t ov8856_write_exposure_dummy(SENSOR_HW_HANDLE handle,uint32_t shutter,uint32_t dummy_line,uint16_t size_index)
{
	...
	if (ov8856_PID_VALUE == pid_value) {
		...
		if (ov8856_VER_VALUE == ver_value) {
			...
			if(PNULL!=s_ov8856_raw_param_tab_ptr->identify_otp){
				SENSOR_PRINT("identify module_id=0x%x",s_ov8856_raw_param_tab_ptr->param_id);
				//set default value
				memset(s_ov8856_otp_info_ptr, 0x00, sizeof(struct otp_info_t));
				ret_value = s_ov8856_raw_param_tab_ptr->identify_otp(handle,s_ov8856_otp_info_ptr);
				
				if(SENSOR_SUCCESS == ret_value ){
					SENSOR_PRINT("identify otp sucess! module_id=0x%x, module_name=%s",s_ov8856_raw_param_tab_ptr->param_id,MODULE_NAME);
				} else{
 ...
 *============================================================================*/
static unsigned long ov8856_write_exposure(SENSOR_HW_HANDLE handle,unsigned long param)
{
	...
	frame_interval = (uint16_t)(((exposure_line + dummy_line) * (s_ov8856_resolution_trim_tab[size_index].line_time)) / 1000000);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t ov8856_write_gain_value(SENSOR_HW_HANDLE handle,unsigned long param)
{
	uint32_t ret_value = SENSOR_SUCCESS;
	float real_gain = 0;
...
#endif
unsigned long _ov8856_SetMaster_FrameSync(SENSOR_HW_HANDLE handle, unsigned long param)
{
