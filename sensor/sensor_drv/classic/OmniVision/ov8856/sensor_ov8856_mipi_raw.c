...
#ifdef FEATURE_OTP
#include "parameters/sensor_ov8856_darling_otp.c"
#else
#include "parameters/ov8856_darling_otp.h"
#endif
...
static SENSOR_REG_TAB_INFO_T s_ov8856_resolution_tab_raw[SENSOR_MODE_MAX] = {
    ...
    {ADDR_AND_LEN_OF_ARRAY(ov8856_preview_setting), PREVIEW_WIDTH,
     PREVIEW_HEIGHT, EX_MCLK, SENSOR_IMAGE_FORMAT_RAW},
    {ADDR_AND_LEN_OF_ARRAY(ov8856_snapshot_setting), SNAPSHOT_WIDTH,
...
SENSOR_INFO_T g_ov8856_mipi_raw_info = {
     ...
     */
    {{ov8856_PID_ADDR, ov8856_PID_VALUE}, {ov8856_VER_ADDR, ov8856_VER_VALUE}},
    /* voltage of avdd */
    SENSOR_AVDD_2800MV,
 ...
 *
 *============================================================================*/
static uint32_t ov8856_get_default_frame_length(SENSOR_HW_HANDLE handle,
                                                uint32_t mode) {
 ...
 *============================================================================*/
static void ov8856_group_hold_on(SENSOR_HW_HANDLE handle) { SENSOR_PRINT("E"); }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_write_gain(SENSOR_HW_HANDLE handle, float gain) {
    float gain_a = gain;
    float gain_d = 0x400;
    ...
    // SENSOR_PRINT("real_gain = %f %d %d",
    // gain,(uint16_t)gain_a,(uint16_t)gain_d);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_write_frame_length(SENSOR_HW_HANDLE handle,
                                      uint32_t frame_len) {
 ...
 *============================================================================*/
static uint16_t ov8856_write_exposure_dummy(SENSOR_HW_HANDLE handle,
                                            uint32_t shutter,
                                            uint32_t dummy_line,
                                            uint16_t size_index) {
...
static uint32_t ov8856_init_mode_fps_info(SENSOR_HW_HANDLE handle) {
    ...
    SENSOR_PRINT("ov8856_init_mode_fps_info:E");
    if (!s_ov8856_mode_fps_info.is_init) {
        uint32_t i, modn, tempfps = 0;
        SENSOR_PRINT("ov8856_init_mode_fps_info:start init");
        for (i = 0; i < NUMBER_OF_ARRAY(s_ov8856_resolution_trim_tab); i++) {
            // max fps should be multiple of 30,it calulated from line_time and
            // frame_line
            tempfps = s_ov8856_resolution_trim_tab[i].line_time *
            ...
            if (0 != tempfps) {
                ...
                if (s_ov8856_mode_fps_info.sensor_mode_fps[i].max_fps > 30) {
                    ...
                    s_ov8856_mode_fps_info.sensor_mode_fps[i]
                        .high_fps_skip_num =
            ...
            SENSOR_PRINT("mode %d,tempfps %d,frame_len %d,line_time: %d ", i,
                         tempfps, s_ov8856_resolution_trim_tab[i].frame_line,
    ...
    SENSOR_PRINT("ov8856_init_mode_fps_info:X");
    return rtn;
}

static uint32_t ov8856_get_static_info(SENSOR_HW_HANDLE handle,
                                       uint32_t *param) {
    uint32_t rtn = SENSOR_SUCCESS;
    ...
    ex_info->adgain_valid_frame_num =
        s_ov8856_static_info.adgain_valid_frame_num;
    ...
    ex_info->sensor_version_info = (cmr_s8 *)g_ov8856_mipi_raw_info.sensor_version_info;
...
static uint32_t ov8856_get_fps_info(SENSOR_HW_HANDLE handle, uint32_t *param) {
    ...
    fps_info->max_fps =
        s_ov8856_mode_fps_info.sensor_mode_fps[sensor_mode].max_fps;
    fps_info->min_fps =
        s_ov8856_mode_fps_info.sensor_mode_fps[sensor_mode].min_fps;
    fps_info->is_high_fps =
        s_ov8856_mode_fps_info.sensor_mode_fps[sensor_mode].is_high_fps;
    fps_info->high_fps_skip_num =
        s_ov8856_mode_fps_info.sensor_mode_fps[sensor_mode].high_fps_skip_num;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static unsigned long ov8856_access_val(SENSOR_HW_HANDLE handle,
                                       unsigned long param) {
    ...
    SENSOR_PRINT("sensor ov8856: param_ptr->type=%x", param_ptr->type);
    ...
    case SENSOR_VAL_TYPE_READ_OTP_GAIN:
        *((uint32_t *)param_ptr->pval) = ov8856_read_gain(handle);
        break;
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = ov8856_get_static_info(handle, param_ptr->pval);
        break;
...
static uint32_t ov8856_identify(SENSOR_HW_HANDLE handle, uint32_t param) {
    ...
    if (ov8856_PID_VALUE == pid_value) {
        ...
        if (ov8856_VER_VALUE == ver_value) {
            ...
            if (PNULL != s_ov8856_raw_param_tab_ptr->identify_otp) {
                SENSOR_PRINT("identify module_id=0x%x",
                             s_ov8856_raw_param_tab_ptr->param_id);
                // set default value
                memset(s_ov8856_otp_info_ptr, 0x00, sizeof(struct otp_info_t));
                ret_value = s_ov8856_raw_param_tab_ptr->identify_otp(
                    handle, s_ov8856_otp_info_ptr);

                if (SENSOR_SUCCESS == ret_value) {
                    SENSOR_PRINT(
                        "identify otp sucess! module_id=0x%x, module_name=%s",
                        s_ov8856_raw_param_tab_ptr->param_id, MODULE_NAME);
                } else {
 ...
 *
 *============================================================================*/
static unsigned long ov8856_get_resolution_trim_tab(SENSOR_HW_HANDLE handle,
                                                    uint32_t param) {
 ...
 *============================================================================*/
static uint32_t ov8856_before_snapshot(SENSOR_HW_HANDLE handle,
                                       uint32_t param) {
    ...
    uint32_t prv_linetime =
        s_ov8856_resolution_trim_tab[preview_mode].line_time;
    uint32_t cap_linetime =
        s_ov8856_resolution_trim_tab[capture_mode].line_time;

    s_current_default_frame_length =
        ov8856_get_default_frame_length(handle, capture_mode);
 ...
 *============================================================================*/
static unsigned long ov8856_write_exposure(SENSOR_HW_HANDLE handle,
                                           unsigned long param) {
    ...
    frame_interval =
        (uint16_t)(((exposure_line + dummy_line) *
                    (s_ov8856_resolution_trim_tab[size_index].line_time)) /
                   1000000);
    SENSOR_PRINT(
        "mode = %d, exposure_line = %d, dummy_line= %d, frame_interval= %d ms",
        size_index, exposure_line, dummy_line, frame_interval);
    s_current_default_frame_length =
        ov8856_get_default_frame_length(handle, size_index);
    s_current_default_line_time =
        s_ov8856_resolution_trim_tab[size_index].line_time;

    ret_value = ov8856_write_exposure_dummy(handle, exposure_line, dummy_line,
                                            size_index);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t ov8856_write_gain_value(SENSOR_HW_HANDLE handle,
                                        unsigned long param) {
    uint32_t ret_value = SENSOR_SUCCESS;
    float real_gain = 0;
...
static uint16_t ov8856_calc_exposure(SENSOR_HW_HANDLE handle, uint32_t shutter,
...
static unsigned long ov8856_read_aec_info(SENSOR_HW_HANDLE handle,
                                          unsigned long param) {
    ...
    frame_interval =
        (uint16_t)(((exposure_line + dummy_line) *
                    (s_ov8856_resolution_trim_tab[mode].line_time)) /
                   1000000);
    SENSOR_PRINT(
        "mode = %d, exposure_line = %d, dummy_line= %d, frame_interval= %d ms",
        mode, exposure_line, dummy_line, frame_interval);
    s_current_default_frame_length =
        ov8856_get_default_frame_length(handle, mode);
    s_current_default_line_time = s_ov8856_resolution_trim_tab[mode].line_time;

    s_sensor_ev_info.preview_shutter = ov8856_calc_exposure(
        handle, exposure_line, dummy_line, mode, &ov8856_aec_info);
...
#endif
unsigned long _ov8856_SetMaster_FrameSync(SENSOR_HW_HANDLE handle,
                                          unsigned long param) {
    Sensor_WriteReg(0x3000, 0x20); // bit 5 0 input 1 output
    Sensor_WriteReg(0x300e, 0x20);
    Sensor_WriteReg(0x3816, 0x00);
    Sensor_WriteReg(0x3817, 0x00);
    Sensor_WriteReg(0x3818, 0x00);
    Sensor_WriteReg(0x3819, 0x10);
...
static uint32_t ov8856_stream_on(SENSOR_HW_HANDLE handle, uint32_t param) {
    SENSOR_PRINT("E");
    _ov8856_SetMaster_FrameSync(handle, param);
...
static SENSOR_IOCTL_FUNC_TAB_T s_ov8856_ioctl_func_tab = {
    ...
    .ex_write_exp = ov8856_write_exposure,
