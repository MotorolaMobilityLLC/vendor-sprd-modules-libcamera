 ...
 *
 *============================================================================*/
static uint32_t ov13855_get_default_frame_length(SENSOR_HW_HANDLE handle,uint32_t mode)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov13855_write_frame_length(SENSOR_HW_HANDLE handle,uint32_t frame_len)
{
 ...
 *============================================================================*/
static uint16_t ov13855_write_exposure_dummy(SENSOR_HW_HANDLE handle,uint32_t shutter,uint32_t dummy_line,uint16_t size_index)
{
...
static uint32_t ov13855_init_mode_fps_info(SENSOR_HW_HANDLE handle)
{
	...
	if(!s_ov13855_mode_fps_info.is_init) {
		...
		for(i = 0;i < NUMBER_OF_ARRAY(s_ov13855_resolution_trim_tab); i++) {
			...
			tempfps = s_ov13855_resolution_trim_tab[i].line_time*s_ov13855_resolution_trim_tab[i].frame_line;
				if(0 != tempfps) {
				...
				if(s_ov13855_mode_fps_info.sensor_mode_fps[i].max_fps > s_ov13855_static_info.max_fps) {
			...
			SENSOR_PRINT("mode %d,max_fps: %d ",
					i,s_ov13855_mode_fps_info.sensor_mode_fps[i].max_fps);
...
static uint32_t ov13855_get_static_info(SENSOR_HW_HANDLE handle, uint32_t *param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	...
	ex_info->adgain_valid_frame_num = s_ov13855_static_info.adgain_valid_frame_num;
...
static uint32_t ov13855_get_fps_info(SENSOR_HW_HANDLE handle, uint32_t *param)
{
	...
	fps_info->max_fps = s_ov13855_mode_fps_info.sensor_mode_fps[sensor_mode].max_fps;
	fps_info->min_fps = s_ov13855_mode_fps_info.sensor_mode_fps[sensor_mode].min_fps;
	fps_info->is_high_fps = s_ov13855_mode_fps_info.sensor_mode_fps[sensor_mode].is_high_fps;
	fps_info->high_fps_skip_num = s_ov13855_mode_fps_info.sensor_mode_fps[sensor_mode].high_fps_skip_num;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static unsigned long ov13855_access_val(SENSOR_HW_HANDLE handle,unsigned long param)
{
	...
	//		*((uint32_t*)param_ptr->pval) = ov13855_read_shutter(handle);
			break;
		case SENSOR_VAL_TYPE_READ_OTP_GAIN:
	//		*((uint32_t*)param_ptr->pval) = ov13855_read_gain(handle);
			break;
...
static uint32_t ov13855_identify(SENSOR_HW_HANDLE handle,uint32_t param)
{
	...
	if (ov13855_PID_VALUE == pid_value) {
		...
		//		SENSOR_PRINT("identify module_id=0x%x",s_ov13855_raw_param_tab_ptr->param_id);
		//		//set default value
		//		memset(s_ov13855_otp_info_ptr, 0x00, sizeof(struct otp_info_t));
		//		ret_value = s_ov13855_raw_param_tab_ptr->identify_otp(handle,s_ov13855_otp_info_ptr);
		//		if(SENSOR_SUCCESS == ret_value ){
		//			SENSOR_PRINT("identify otp sucess! module_id=0x%x, module_name=%s",s_ov13855_raw_param_tab_ptr->param_id,MODULE_NAME);
		//		} else{
 ...
 *
 *============================================================================*/
static unsigned long ov13855_get_resolution_trim_tab(SENSOR_HW_HANDLE handle,uint32_t param)
{
 ...
 *============================================================================*/
static uint32_t ov13855_before_snapshot(SENSOR_HW_HANDLE handle,uint32_t param)
{
	...
	uint32_t prv_linetime = s_ov13855_resolution_trim_tab[preview_mode].line_time;
	uint32_t cap_linetime = s_ov13855_resolution_trim_tab[capture_mode].line_time;

	s_current_default_frame_length = ov13855_get_default_frame_length(handle,capture_mode);
 ...
 *============================================================================*/
static unsigned long ov13855_write_exposure(SENSOR_HW_HANDLE handle,unsigned long param)
{
	...
	frame_interval = (uint16_t)(((exposure_line + dummy_line) * (s_ov13855_resolution_trim_tab[size_index].line_time)) / 1000000);
	SENSOR_PRINT("mode = %d, exposure_line = %d, dummy_line= %d, frame_interval= %d ms",
		size_index, exposure_line, dummy_line, frame_interval);
	s_current_default_frame_length = ov13855_get_default_frame_length(handle,size_index);
	s_current_default_line_time = s_ov13855_resolution_trim_tab[size_index].line_time;

	ret_value = ov13855_write_exposure_dummy(handle, exposure_line, dummy_line, size_index);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t ov13855_write_gain_value(SENSOR_HW_HANDLE handle,unsigned long param)
{
	uint32_t ret_value = SENSOR_SUCCESS;
	float real_gain = 0;
...
static struct sensor_i2c_reg_tab ov13855_shutter_tab = {
	.settings = ov13855_shutter_reg,
	.size = ARRAY_SIZE(ov13855_shutter_reg),
};
...
static struct sensor_i2c_reg_tab ov13855_again_tab = {
	.settings = ov13855_again_reg,
	.size = ARRAY_SIZE(ov13855_again_reg),
};
...
struct sensor_i2c_reg_tab ov13855_dgain_tab = {
	.settings = ov13855_dgain_reg,
	.size = ARRAY_SIZE(ov13855_dgain_reg),
};
...
static uint16_t ov13855_calc_exposure(SENSOR_HW_HANDLE handle,
...
static unsigned long ov13855_read_aec_info(SENSOR_HW_HANDLE handle, unsigned long param)
{
	...
	frame_interval = (uint16_t)(((exposure_line + dummy_line) * (s_ov13855_resolution_trim_tab[mode].line_time)) / 1000000);
	SENSOR_PRINT("mode = %d, exposure_line = %d, dummy_line= %d, frame_interval= %d ms",
		mode, exposure_line, dummy_line, frame_interval);
	s_current_default_frame_length = ov13855_get_default_frame_length(handle,mode);
	s_current_default_line_time = s_ov13855_resolution_trim_tab[mode].line_time;

	s_sensor_ev_info.preview_shutter =
		ov13855_calc_exposure(handle,
...
unsigned long _ov13855_SetMaster_FrameSync(SENSOR_HW_HANDLE handle, unsigned long param)
{
