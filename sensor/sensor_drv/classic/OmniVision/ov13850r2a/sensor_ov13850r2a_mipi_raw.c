...
#define LOG_TAG "ov13850r2a_sharkl2"

//#define MIPI_NUM_2LANE

#ifdef MIPI_NUM_2LANE
#include "sensor_ov13850r2a_mipi_raw_2lane.h"
#else
#include "sensor_ov13850r2a_mipi_raw_4lane.h"
#endif
...
static void
ov13850r2a_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov13850r2a_drv_write_gain(struct sensor_aec_i2c_tag *aec_info,
                                      cmr_u32 gain) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    if (aec_info->again->size > 1) {
        /*TODO*/
        if (SENSOR_MAX_GAIN < gain)
            gain = SENSOR_MAX_GAIN;

        aec_info->again->settings[0].reg_value = (gain >> 8) & 0x07;
 ...
 *============================================================================*/
static void
ov13850r2a_drv_write_frame_length(struct sensor_aec_i2c_tag *aec_info,
                                  cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov13850r2a_drv_write_shutter(struct sensor_aec_i2c_tag *aec_info,
                                         cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    cmr_u32 value = 0x00;
...
static void ov13850r2a_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        ov13850r2a_drv_write_frame_length(aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    ov13850r2a_drv_write_shutter(aec_info, shutter);
}

static void ov13850r2a_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                     struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    ov13850r2a_drv_write_gain(aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov13850r2a_drv_power_on(cmr_handle handle, cmr_uint power_on) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov13850r2a_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int ov13850r2a_drv_get_static_info(cmr_handle handle,
                                              uint32_t *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        ov13850r2a_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_ov13850r2a_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_ov13850r2a_mipi_raw_info.sensor_version_info;
...
static cmr_int ov13850r2a_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov13850r2a_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGI("sensor ov13850r2a: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = ov13850r2a_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = ov13850r2a_drv_get_fps_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
        ret = sns_drv_cxt->is_sensor_close = 1;
        break;
    case SENSOR_VAL_TYPE_GET_PDAF_INFO:
        // ret = ov13850r2a_drv_get_pdaf_info(handle, param_ptr->pval);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov13850r2a_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov13850r2a_PID_ADDR);

    if (ov13850r2a_PID_VALUE == pid_value) {
        ver_value =
            hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov13850r2a_VER_ADDR);
        SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                    ver_value);
        if (ov13850r2a_VER_VALUE == ver_value) {
            cmr_u16 ver1_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle,
                                                    ov13850r2a_VER1_ADDR);
            if (ov13850r2a_VER1_VALUE == ver1_value) {

                SENSOR_PRINT("Identify: PID = %x, VER = %x, ver1=%x", pid_value,
                             ver_value, ver1_value);
                SENSOR_PRINT_HIGH("this is ov13850r2a sensor");
                // ov13850r2a_init_mode_fps_info(handle);
                // ov13850r2a_init_mode_fps_info(handle);
                ret_value = SENSOR_SUCCESS;
            } else {
                SENSOR_LOGE("sensor identify fail, PID = %x, VER = %x, ver1=%x",
                            pid_value, ver_value, ver1_value);
            }
 ...
 *============================================================================*/
static cmr_int ov13850r2a_drv_before_snapshot(cmr_handle handle,
                                              cmr_uint param) {
    ...
    ov13850r2a_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                                 &ov13850r2a_aec_info);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.frame_length);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    ov13850r2a_drv_write_gain(&ov13850r2a_aec_info, cap_gain);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.again);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int ov13850r2a_drv_write_exposure(cmr_handle handle,
                                             cmr_uint param) {
    ...
    ov13850r2a_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                                 &ov13850r2a_aec_info);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.frame_length);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int ov13850r2a_drv_write_gain_value(cmr_handle handle,
                                               cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    ov13850r2a_drv_calc_gain(handle, param, &ov13850r2a_aec_info);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.again);
    ov13850r2a_drv_write_reg2sensor(handle, ov13850r2a_aec_info.dgain);
...
static cmr_int ov13850r2a_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &ov13850r2a_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    ov13850r2a_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                                 &ov13850r2a_aec_info);
    ov13850r2a_drv_calc_gain(handle, info->gain, &ov13850r2a_aec_info);

    return ret_value;
}

static cmr_int ov13850r2a_drv_set_master_FrameSync(cmr_handle handle,
                                                   cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov13850r2a_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
// ov13850r2a_drv_set_master_FrameSync(handle,param);
#endif
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov13850r2a_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    cmr_uint stream_off_flag = 0xf0;
    stream_off_flag = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x0100);

    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x0100, 0x00);

    /*
if (!sns_drv_cxt->is_sensor_close && stream_off_flag==1) {
    usleep(100 * 1000);
}
    else{
             usleep(5 * 1000);
...
static cmr_int
ov13850r2a_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_ov13850r2a_module_info_tab),
                                    s_ov13850r2a_module_info_tab);
    sensor_ic_set_match_resolution_info(
        sns_drv_cxt, ARRAY_SIZE(s_ov13850r2a_resolution_tab_raw),
        s_ov13850r2a_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_ov13850r2a_resolution_trim_tab),
                                  s_ov13850r2a_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_ov13850r2a_static_info),
                                    s_ov13850r2a_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_ov13850r2a_mode_fps_info),
                                 s_ov13850r2a_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    ov13850r2a_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int ov13850r2a_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int ov13850r2a_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                               void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
static struct sensor_ic_ops s_ov13850r2a_ops_tab = {
    .create_handle = ov13850r2a_drv_handle_create,
    .delete_handle = ov13850r2a_drv_handle_delete,
    .get_data = ov13850r2a_drv_get_private_data,
    .power = ov13850r2a_drv_power_on,
    .identify = ov13850r2a_drv_identify,
    .ex_write_exp = ov13850r2a_drv_write_exposure,
    .write_gain_value = ov13850r2a_drv_write_gain_value,

#if defined(CONFIG_DUAL_MODULE)
//.read_aec_info = ov13850r2a_drv_read_aec_info,
#endif

    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = ov13850r2a_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = ov13850r2a_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = ov13850r2a_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = ov13850r2a_drv_access_val,
    }};
