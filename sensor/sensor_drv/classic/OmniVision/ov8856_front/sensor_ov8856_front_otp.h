...
#define R_TYPICAL_ov8856 0x0000
#define G_TYPICAL_ov8856 0x0000
#define B_TYPICAL_ov8856 0x0000
...
static struct otp_info_t ov8856_otp_info;

static uint32_t ov8856_read_otp_info(cmr_handle handle, void *param_ptr) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    uint32_t rtn = SENSOR_SUCCESS;
    struct otp_info_t *otp_info =
        &ov8856_otp_info; //(struct otp_info_t *)param_ptr;
    // otp_info->rg_ratio_typical = RG_TYPICAL_ov8856;
    // otp_info->bg_ratio_typical = BG_TYPICAL_ov8856;
    otp_info->r_typical = R_TYPICAL_ov8856;
    otp_info->g_typical = G_TYPICAL_ov8856;
    otp_info->b_typical = B_TYPICAL_ov8856;
    ...
    if ((otp_flag & 0xc0) == 0x40) {
        ...
        addr = 0x701c; // base address of info group 2
    }
    ...
    if (addr != 0) {
        ...
        property_get("debug.ov8856.rg.typical", value0, "0");
        if (!strcmp(value0, "0")) {
            otp_info->rg_ratio_typical =
                (hw_sensor_read_reg(sns_drv_cxt->hw_handle, addr + 8) << 2) +
                ((temp >> 2) & 0x03);
        } else {
            otp_info->rg_ratio_typical = atoi(value0);
        }

        property_get("debug.ov8856.bg.typical", value1, "0");
        if (!strcmp(value1, "0")) {
...
static uint32_t ov8856_update_awb(cmr_handle handle, void *param_ptr) {
    uint32_t rtn = SENSOR_SUCCESS;
    struct otp_info_t *otp_info =
        &ov8856_otp_info; //(struct otp_info_t *)param_ptr;
    SENSOR_IC_CHECK_HANDLE(handle);
...
static uint32_t ov8856_update_lsc(cmr_handle handle, void *param_ptr) {
    uint32_t rtn = SENSOR_SUCCESS;
    struct otp_info_t *otp_info =
        &ov8856_otp_info; //(struct otp_info_t *)param_ptr;
    SENSOR_IC_CHECK_HANDLE(handle);
...
static uint32_t ov8856_update_otp(cmr_handle handle, void *param_ptr) {
    uint32_t rtn = SENSOR_SUCCESS;
    struct otp_info_t *otp_info =
        &ov8856_otp_info; //(struct otp_info_t *)param_ptr;
    SENSOR_IC_CHECK_HANDLE(handle);
    ...
    property_get("debug.ov8856.otp.awb.on", value0, "1");
    if (!strcmp(value0, "1")) {
        rtn = ov8856_update_awb(handle, param_ptr);
    ...
    property_get("debug.ov8856.otp.lsc.on", value1, "1");
    if (!strcmp(value1, "1")) {
        rtn = ov8856_update_lsc(handle, param_ptr);
        if (rtn != SENSOR_SUCCESS) {
...
static uint32_t ov8856_read_otp(cmr_handle handle, void *param_ptr) {
    uint32_t rtn = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    ov8856_read_otp_info(handle, param_ptr);
    rtn = ov8856_update_otp(handle, param_ptr);
