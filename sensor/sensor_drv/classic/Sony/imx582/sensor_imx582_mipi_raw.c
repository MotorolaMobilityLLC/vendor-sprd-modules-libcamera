...
#define LOG_TAG "imx582_mipi_raw"

#include "sensor_imx582_mipi_raw.h"
...
static void imx582_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void imx582_drv_write_gain(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void imx582_drv_write_frame_length(cmr_handle handle,
                                          struct sensor_aec_i2c_tag *aec_info,
                                          cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void imx582_drv_write_shutter(cmr_handle handle,
                                     struct sensor_aec_i2c_tag *aec_info,
                                     cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 *============================================================================*/
static void imx582_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        imx582_drv_write_frame_length(handle, aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    imx582_drv_write_shutter(handle, aec_info, shutter);
...
static void imx582_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    imx582_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx582_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    BOOLEAN power_down = g_imx582_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_imx582_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx582_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int imx582_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        imx582_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_imx582_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_imx582_mipi_raw_info.sensor_version_info;
...
static cmr_int imx582_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
...
static const struct pd_pos_info imx582_pd_pos_l[] = {
    {3, 1}, {11, 1},
    {1, 3}, {9, 3},
    {5, 5}, {13, 5},
    {7, 7}, {15, 7},
...
static const struct pd_pos_info imx582_pd_pos_r[] = {
    {2, 1}, {10, 1},
    {0, 3}, {8, 3},
    {4, 5}, {12, 5},
    {6, 7}, {14, 7},
...
struct pdaf_coordinate_tab imx582_pd_coordinate_table[] = {
    {.number = 4,
     .pos_info = {0, 1, 0, 1},
    },
    {.number = 4,
...
struct pdaf_block_descriptor imx582_pd_seprator_helper = {
    .block_width = 4,
    .block_height = 8,
    .coordinate_tab = NULL,
    .line_width = 4,
    .block_pattern = CROSS_PATTERN,
    .pd_line_coordinate = imx582_pd_coordinate_table,
};

cmr_int imx582_drv_pdaf_data_process(void *buffer_handle);

static cmr_int imx582_drv_get_pdaf_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    pd_pos_is_right_size = NUMBER_OF_ARRAY(imx582_pd_is_right);
    pd_pos_row_size = NUMBER_OF_ARRAY(imx582_pd_row);
    pd_pos_col_size = NUMBER_OF_ARRAY(imx582_pd_col);
    ...
    pdaf_info->pd_is_right = (cmr_u16 *)imx582_pd_is_right;
    pdaf_info->pd_pos_row = (cmr_u16 *)imx582_pd_row;
    pdaf_info->pd_pos_col = (cmr_u16 *)imx582_pd_col;
    pdaf_info->pd_pos_r = (struct pd_pos_info *)imx582_pd_pos_r;
    pdaf_info->pd_pos_l = (struct pd_pos_info *)imx582_pd_pos_l;
    pdaf_info->pd_pos_size = NUMBER_OF_ARRAY(imx582_pd_pos_r);
    ...
    pdaf_info->descriptor = &imx582_pd_seprator_helper;
    pdaf_info->pdaf_format_converter = imx582_drv_pdaf_data_process;
...
cmr_int imx582_drv_pdaf_data_process(void *buffer_handle) {
    ...
    imx582_drv_get_pdaf_info(NULL, (cmr_u32 *)(&pdaf_info));
    sensor_pdaf_format_convertor(buffer_handle, s_imx582_static_info[0].static_info.pdaf_supported, (cmr_u32 *)(&pdaf_info));
    return SENSOR_SUCCESS;
}
/*
#include "parameters/param_manager.c"
static cmr_int imx582_drv_set_raw_info(cmr_handle handle, cmr_u8 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    cmr_u8 vendor_id = (cmr_u8)*param;
    SENSOR_LOGI("*param %x %x", *param, vendor_id);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    s_imx582_mipi_raw_info_ptr = imx582_drv_init_raw_info(sns_drv_cxt->sensor_id, vendor_id, 0, 0);
...
static cmr_int imx582_drv_get_4in1_info(cmr_handle handle, cmr_u32 *param) {
	cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx582_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGI("sensor imx582: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = imx582_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = imx582_drv_get_fps_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
        ret = sns_drv_cxt->is_sensor_close = 1;
        break;
    case SENSOR_VAL_TYPE_GET_4IN1_INFO:
        ret = imx582_drv_get_4in1_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_SET_OTP_DATA:
        ret = 0;//imx582_drv_ov4c_init(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_4IN1_PROC:
        ret = 0;//imx582_drv_ov4c_process(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_4IN1_DEINIT:
        ret = 0;//imx582_drv_ov4c_deinit(handle, param_ptr->pval);
        break;
//    case SENSOR_VAL_TYPE_SET_RAW_INFOR:
//        ret = imx582_drv_set_raw_info(handle, param_ptr->pval);
//        break;
	case SENSOR_VAL_TYPE_GET_PDAF_INFO:
        ret = imx582_drv_get_pdaf_info(handle, param_ptr->pval);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx582_drv_identify(cmr_handle handle, cmr_uint param) {
    cmr_u16 pid_value = 0x00;
    cmr_u16 ver_value = 0x00;
    cmr_int ret_value = SENSOR_FAIL;
    cmr_u16 pid_value2 = 0x00;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    SENSOR_LOGI("mipi raw identify");

    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, imx582_PID_ADDR);
    pid_value2 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x0018);
    SENSOR_LOGE("---Hui---Identify: pid_value2 = %x", pid_value2);
    if (imx582_PID_VALUE == pid_value) {
        ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, imx582_VER_ADDR);
        SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                    ver_value);
        if (imx582_VER_VALUE == ver_value) {
            SENSOR_LOGI("---Hui---this is imx582 sensor");
 ...
 *============================================================================*/
static cmr_int imx582_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    imx582_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                             &imx582_aec_info);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.frame_length);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    imx582_drv_write_gain(handle, &imx582_aec_info, cap_gain);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.again);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int imx582_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    imx582_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                             &imx582_aec_info);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.frame_length);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int imx582_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    imx582_drv_calc_gain(handle, param, &imx582_aec_info);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.again);
    imx582_drv_write_reg2sensor(handle, imx582_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int imx582_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &imx582_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    imx582_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                             &imx582_aec_info);
    imx582_drv_calc_gain(handle, info->gain, &imx582_aec_info);

    return ret_value;
}

static cmr_int imx582_drv_set_master_FrameSync(cmr_handle handle,
                                               cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int imx582_drv_set_pdaf_mode(cmr_handle handle, cmr_uint param) {
    UNUSED(param);
...
static cmr_int imx582_drv_set_qsc_data(cmr_handle handle, cmr_uint param) {
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    for (int i = 0; i < 2304; i++) {
        hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xC500 + i, *(param_ptr + 0x1910 + i));   //hui
        SENSOR_LOGV("Hui--imx582_qsc address {0x%x, 0x%x}", 0xC500 + i, *(param_ptr + 0x1910 + i));
    }
...
static cmr_int imx582_drv_set_LRC_data(cmr_handle handle, cmr_uint param) {
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    struct sensor_drv_context *sensor_cxt = (struct sensor_drv_context *)(sns_drv_cxt->caller_handle);
    cmr_u8 *param_ptr = NULL;
   ...
   // SENSOR_LOGD("LRC area control is 0x3038 -> 0x%x", lrc_area_control);
    for (int i = 0; i < 192; i++) {
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x7B10 + i, *(param_ptr + 0x1348 + i));   //hui
	SENSOR_LOGV("Hui--imx582_lrc address {0x%x, 0x%x}", 0x7B10 + i, *(param_ptr + 0x1348 + i));
    }
    for (int i = 0; i < 192; i++) {
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x7C00 + i, *(param_ptr + 0x1348 + 192 + i));  //hui
	SENSOR_LOGV("Hui--imx582_lrc address {0x%x, 0x%x}", 0x7C00 + i, *(param_ptr + 0x1348 + 192 + i));
    }
exit:
    return 0;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx582_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    if(sensor_mode == 2)
	imx582_drv_set_LRC_data(handle, param);
    if (sensor_mode > 2)
        imx582_drv_set_qsc_data(handle, param);
    else
...
//	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x0856, 0x01); //default 00
   ...
   // imx582_drv_set_pdaf_mode(handle, 1);
/*    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x0315d, 0x02);
    ...
    if (!strcmp(value2, "1") && sns_drv_cxt->is_multi_mode) {
        //imx582_drv_set_master_FrameSync(handle, param);
        // imx582_drv_set_slave_FrameSync(handle, param);
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx582_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
...
static cmr_int
imx582_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    imx582_drv_write_frame_length(
        sns_drv_cxt, &imx582_aec_info,
        sns_drv_cxt->sensor_ev_info.preview_framelength);
    imx582_drv_write_gain(sns_drv_cxt, &imx582_aec_info,
                          sns_drv_cxt->sensor_ev_info.preview_gain);
    imx582_drv_write_shutter(sns_drv_cxt, &imx582_aec_info,
                             sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_imx582_module_info_tab),
                                    s_imx582_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt,
                                        ARRAY_SIZE(s_imx582_resolution_tab_raw),
                                        s_imx582_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_imx582_resolution_trim_tab),
                                  s_imx582_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_imx582_static_info), s_imx582_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_imx582_mode_fps_info),
                                 s_imx582_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    imx582_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int imx582_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int imx582_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = imx582_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_imx582_ops_tab = {
    .create_handle = imx582_drv_handle_create,
    .delete_handle = imx582_drv_handle_delete,
    /*get privage data*/
    .get_data = imx582_drv_get_private_data,
    /*common interface*/
    .power = imx582_drv_power_on,
    .identify = imx582_drv_identify,
    .ex_write_exp = imx582_drv_write_exposure,
    .write_gain_value = imx582_drv_write_gain_value,

#if defined(CONFIG_DUAL_MODULE)
    .read_aec_info = imx582_drv_read_aec_info,
#endif

    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = imx582_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = imx582_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = imx582_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = imx582_drv_access_val,
    }};
