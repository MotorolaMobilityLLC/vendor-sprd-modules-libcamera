...
#endif
#ifdef IMX616_CPHY
#include "sensor_imx616_mipi_raw_cphy.h"
#else
#include "sensor_imx616_mipi_raw.h"
#endif
/*==============================================================================
 * Description:
...
static void imx616_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void imx616_drv_write_gain(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void imx616_drv_write_frame_length(cmr_handle handle,
                                          struct sensor_aec_i2c_tag *aec_info,
                                          cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void imx616_drv_write_shutter(cmr_handle handle,
                                     struct sensor_aec_i2c_tag *aec_info,
                                     cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 *============================================================================*/
static void imx616_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        imx616_drv_write_frame_length(handle, aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    imx616_drv_write_shutter(handle, aec_info, shutter);
...
static void imx616_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    imx616_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    BOOLEAN power_down = g_imx616_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_imx616_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int imx616_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        imx616_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_imx616_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_imx616_mipi_raw_info.sensor_version_info;
...
static cmr_int imx616_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
...
static cmr_int imx616_drv_get_pdaf_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    pd_pos_is_right_size = NUMBER_OF_ARRAY(imx616_pd_is_right);
    pd_pos_row_size = NUMBER_OF_ARRAY(imx616_pd_row);
    pd_pos_col_size = NUMBER_OF_ARRAY(imx616_pd_col);
    ...
    pdaf_info->pd_is_right = (cmr_u16 *)imx616_pd_is_right;
    pdaf_info->pd_pos_row = (cmr_u16 *)imx616_pd_row;
    pdaf_info->pd_pos_col = (cmr_u16 *)imx616_pd_col;
...
static cmr_int imx616_drv_set_raw_info(cmr_handle handle, cmr_u8 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    cmr_u8 vendor_id = (cmr_u8)*param;
    SENSOR_LOGI("*param %x %x", *param, vendor_id);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    s_imx616_mipi_raw_info_ptr =
imx616_drv_init_raw_info(sns_drv_cxt->sensor_id, vendor_id, 0, 0);

    return rtn;
}
*/

static cmr_s64 imx616_drv_get_shutter_skew(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    cmr_u16 height = 0;
    cmr_u32 line_time = 0;
    ...
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    height = s_imx616_resolution_tab_raw[0].reg_tab[shutter_skew_info->sns_mode].height;
    line_time = s_imx616_resolution_trim_tab[0].trim_info[shutter_skew_info->sns_mode].line_time;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGI("sensor imx616: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = imx616_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = imx616_drv_get_fps_info(handle, param_ptr->pval);
        break;
    ...
    case SENSOR_VAL_TYPE_4IN1_PROC:
        ret = 0; // imx616_drv_ov4c_process(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_4IN1_DEINIT:
        ret = 0; // imx616_drv_ov4c_deinit(handle, param_ptr->pval);
        break;
    //    case SENSOR_VAL_TYPE_SET_RAW_INFOR:
    //        ret = imx616_drv_set_raw_info(handle, param_ptr->pval);
    //        break;
    case SENSOR_VAL_TYPE_GET_PDAF_INFO:
        ret = imx616_drv_get_pdaf_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_SHUTTER_SKEW_DATA:
        ret = imx616_drv_get_shutter_skew(handle, param_ptr->pval);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, imx616_PID_ADDR);

    if (imx616_PID_VALUE == pid_value) {
        ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, imx616_VER_ADDR);
 ...
 *============================================================================*/
static cmr_int imx616_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    imx616_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                             &imx616_aec_info);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.frame_length);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    imx616_drv_write_gain(handle, &imx616_aec_info, cap_gain);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.again);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int imx616_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    imx616_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                             &imx616_aec_info);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.frame_length);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int imx616_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    imx616_drv_calc_gain(handle, param, &imx616_aec_info);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.again);
    imx616_drv_write_reg2sensor(handle, imx616_aec_info.dgain);
...
static cmr_int imx616_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    ...
    info->aec_i2c_info_out = &imx616_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    imx616_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                             &imx616_aec_info);
    imx616_drv_calc_gain(handle, info->gain, &imx616_aec_info);

    return ret_value;
}

static cmr_int imx616_drv_set_master_FrameSync(cmr_handle handle,
                                               cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int imx616_drv_set_pdaf_mode(cmr_handle handle, cmr_uint param) {
    UNUSED(param);
...
static cmr_int imx616_drv_set_xtalk_data(cmr_handle handle, cmr_uint param) {
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    //	cmr_u8 *param_ptr = (cmr_u8 *)param;
    cmr_u8 param_ptr[0x1000] = {0x00};

    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x3621, 0x1);

	SENSOR_LOGI("xtalk: reg = %x, value = %x", 0x3621, hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x3621));
#if 1
    hw_sensor_read_i2c(sns_drv_cxt->hw_handle, 0xa2 >> 1, param_ptr,
                       0x1000 << 16 | SENSOR_I2C_REG_16BIT);
    for (int i = 0; 3 * i < 1560; i++){
        hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xc500 + 3 * i,*(param_ptr + 0x7d2 + 3*i)); //*(xtalk_data.data_addr + i));
        hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xc500 + 3 * i + 1,*(param_ptr + 0x7d2 + 3*i + 1)); //*(xtalk_data.data_addr + i));
        hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xc500 + 3 * i + 2,*(param_ptr + 0x7d2 + 3*i + 2)); //*(xtalk_data.data_addr + i));

    }
/*   for (int i = 0; i < 1560; i++){
        SENSOR_LOGI("xtalk: reg = %x, value = %x param %x",
        0xc500+i,  hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0xc500+i), *(param_ptr + 0x7d2 + i));
     }*/

	 //  hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x3621, hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x3621) | 0x1);
//    const char *fcellXtalkFile = XTALK_DATA;
    ...
    otp_parser_ptr(param_ptr, OTP_PARSER_SECTION_XTALK_4IN1,
                   OTP_EEPROM_INDEPENDENT, sns_drv_cxt->sensor_id, IMG_HEIGHT,
                   IMG_WIDTH, &xtalk_data);
    //	  otp_parser_ptr(param_ptr, OTP_PARSER_SECTION_DPC_4IN1,
    //OTP_EEPROM_INDEPENDENT,
    //					 sns_drv_cxt->sensor_id, IMG_HEIGHT, IMG_WIDTH,
    //&dpc_data);
    for (int i = 0; i < 288; i++)
        hw_sensor_write_reg_8bits(sns_drv_cxt->hw_handle, 0x53c0 + i,
                                  *(xtalk_data.data_addr + i));
    ...
    // hw_sensor_write_reg_8bits(sns_drv_cxt->hw_handle, 0x5000, 0x40 |
    // hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x5000));
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
// imx616_drv_set_pdaf_mode(handle, 1);
/*    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x0315d, 0x02);
...
//    imx616_drv_set_master_FrameSync(handle, param);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
    cmr_u16 value = 0;
...
static cmr_int
imx616_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_imx616_module_info_tab),
                                    s_imx616_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt,
                                        ARRAY_SIZE(s_imx616_resolution_tab_raw),
                                        s_imx616_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_imx616_resolution_trim_tab),
                                  s_imx616_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_imx616_static_info), s_imx616_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_imx616_mode_fps_info),
                                 s_imx616_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    imx616_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int imx616_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int imx616_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
void *sensor_ic_open_lib(void) { return &g_imx616_mipi_raw_info; }
 ...
 * .power = imx616_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_imx616_ops_tab = {
    .create_handle = imx616_drv_handle_create,
    .delete_handle = imx616_drv_handle_delete,
    /*get privage data*/
    .get_data = imx616_drv_get_private_data,
    /*common interface*/
    .power = imx616_drv_power_on,
    .identify = imx616_drv_identify,
    .ex_write_exp = imx616_drv_write_exposure,
    .write_gain_value = imx616_drv_write_gain_value,

#if defined(CONFIG_DUAL_MODULE)
    .read_aec_info = imx616_drv_read_aec_info,
#endif

    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = imx616_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = imx616_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = imx616_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = imx616_drv_access_val,
    }};
