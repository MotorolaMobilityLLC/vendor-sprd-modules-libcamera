...
ifeq ($(strip $(TARGET_PRODUCT)),sp9861e_3h10_vmm)
LOCAL_TUNING_PATH := parameters/tuning_bin/jcx
endif

include $(CLEAR_VARS)
LOCAL_MODULE := imx258_mipi_raw_tuning.bin
...
include $(CLEAR_VARS)
LOCAL_MODULE := imx258_mipi_raw_pdotp.bin
LOCAL_SRC_FILES := $(LOCAL_TUNING_PATH)/$(LOCAL_MODULE)
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH := $(TUNING_PATH)
ifeq ($(PLATFORM_VERSION),8.0.0)
...
include $(CLEAR_VARS)
LOCAL_MODULE := imx258_mipi_raw_cbc.bin
LOCAL_SRC_FILES := $(LOCAL_TUNING_PATH)/$(LOCAL_MODULE)
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_PATH := $(TUNING_PATH)
ifeq ($(PLATFORM_VERSION),8.0.0)
