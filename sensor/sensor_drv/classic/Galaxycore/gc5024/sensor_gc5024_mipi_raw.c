...
#define LOG_TAG "gc5024_sharkl2" 

#define MIPI_NUM_2LANE


#ifdef MIPI_NUM_2LANE
#include "sensor_gc5024_mipi_raw_2lane.h"
#else
#include "sensor_gc5024_mipi_raw_2lane_sprd.h"
#endif
...
static void gc5024_drv_write_reg2sensor(cmr_handle handle,struct sensor_i2c_reg_tab *reg_info){
...
static void gc5024_drv_write_gain(struct sensor_aec_i2c_tag *aec_info, cmr_u32 gain)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
	if(aec_info->again->size>=10){
		/*TODO*/
	...
	if(gain < 0x40)
		...
		if ((ANALOG_GAIN_1 <= gain) && (gain < ANALOG_GAIN_2)) {
			aec_info->again->settings[1].reg_value = 0x1c;
			aec_info->again->settings[2].reg_value = 0x10;
			aec_info->again->settings[3].reg_value = 0x02;
			aec_info->again->settings[4].reg_value = 0x01;
			aec_info->again->settings[5].reg_value = 0x02;
			aec_info->again->settings[6].reg_value = 0x01;
			aec_info->again->settings[7].reg_value = 0x00;
					
			temp = gain;
			aec_info->again->settings[8].reg_value = (temp >> 6) & 0xff;
			aec_info->again->settings[9].reg_value = (temp << 2) & 0xfc;
			

		} else if ((ANALOG_GAIN_2 <= gain) && (gain < ANALOG_GAIN_3)) {
			
			aec_info->again->settings[1].reg_value = 0x1f;
			aec_info->again->settings[2].reg_value = 0x10;
			aec_info->again->settings[3].reg_value = 0x02;
			aec_info->again->settings[4].reg_value = 0x01;
			aec_info->again->settings[5].reg_value = 0x02;
			aec_info->again->settings[6].reg_value = 0x01;
			aec_info->again->settings[7].reg_value = 0x01;
			...
			aec_info->again->settings[1].reg_value = 0x22;
			aec_info->again->settings[2].reg_value = 0x10;
			aec_info->again->settings[3].reg_value = 0x02;
			aec_info->again->settings[4].reg_value = 0x02;
			aec_info->again->settings[5].reg_value = 0x02;
			aec_info->again->settings[6].reg_value = 0x02;
			aec_info->again->settings[7].reg_value = 0x02;
			...
			aec_info->again->settings[1].reg_value = 0x3b;
			aec_info->again->settings[2].reg_value = 0x18;
			aec_info->again->settings[3].reg_value = 0x03;
			aec_info->again->settings[4].reg_value = 0x02;
			aec_info->again->settings[5].reg_value = 0x03;
			aec_info->again->settings[6].reg_value = 0x02;
			aec_info->again->settings[7].reg_value = 0x03;
			...
			aec_info->again->settings[5].reg_value = 0x03;
			aec_info->again->settings[6].reg_value = 0x02;
			aec_info->again->settings[7].reg_value = 0x04;
			...
			aec_info->again->settings[5].reg_value = 0x03;
			aec_info->again->settings[6].reg_value = 0x02;
			aec_info->again->settings[7].reg_value = 0x05;
			...
			aec_info->again->settings[5].reg_value = 0x03;
			aec_info->again->settings[6].reg_value = 0x02;
			aec_info->again->settings[7].reg_value = 0x06;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc5024_drv_write_frame_length(struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len){
   SENSOR_IC_CHECK_PTR_VOID(aec_info);


   
	if(aec_info->frame_length->size>=3){
		/*TODO*/
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc5024_drv_write_shutter(struct sensor_aec_i2c_tag *aec_info, cmr_u32 shutter){
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
	cmr_u32 value = 0x00;
	...
	if(aec_info->shutter->size>=6){
		/*TODO*/
		if(shutter<=20){
			aec_info->shutter->settings[1].reg_value = 0x09;
			aec_info->shutter->settings[2].reg_value = 0x53;	
		}
		else{
			aec_info->shutter->settings[1].reg_value = 0x49;
			aec_info->shutter->settings[2].reg_value = 0x4b;			
		}
		
		value = (shutter) & 0xff;
		aec_info->shutter->settings[4].reg_value = value;		
		
		value = (shutter >> 0x08) & 0x3f;
		aec_info->shutter->settings[5].reg_value = value;
		
		/*END*/
	}
}

static void gc5024_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
	...
	if (dest_fr_len != cur_fr_len){
		...
		gc5024_drv_write_frame_length(aec_info,dest_fr_len);
	}
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
	gc5024_drv_write_shutter(aec_info,shutter);
}

static void gc5024_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) {
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	*/
	...
	//sensor_gain = sensor_gain * SENSOR_BASE_GAIN / ISP_BASE_GAIN;
	sensor_gain=256-256*ISP_BASE_GAIN/sensor_gain;
	...
	gc5024_drv_write_gain(aec_info,sensor_gain);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc5024_drv_power_on(cmr_handle handle, cmr_uint power_on)
{
    ...
    if (SENSOR_TRUE == power_on) {
		...
		usleep(2 * 1000);   //1000
		SENSOR_LOGI("(windsor  %lu", dvdd_val);		
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc5024_drv_init_fps_info(cmr_handle handle) 
{
...
static cmr_int gc5024_drv_get_static_info(cmr_handle handle, uint32_t *param) 
{
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        gc5024_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_gc5024_mipi_raw_info.name;
    ex_info->sensor_version_info = (cmr_s8 *)g_gc5024_mipi_raw_info.sensor_version_info;
...
static cmr_int gc5024_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) 
{
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc5024_drv_access_val(cmr_handle handle, cmr_uint param)
{
	...
	SENSOR_LOGI("sensor gc5024: param_ptr->type=%x", param_ptr->type);
	
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_GET_STATIC_INFO:
			ret = gc5024_drv_get_static_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_GET_FPS_INFO:
			ret = gc5024_drv_get_fps_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
			ret = sns_drv_cxt->is_sensor_close = 1;
			break;
		case SENSOR_VAL_TYPE_GET_PDAF_INFO:
			//ret = gc5024_drv_get_pdaf_info(handle, param_ptr->pval);
			break;
		default:
			break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc5024_drv_identify(cmr_handle handle, cmr_uint param)
{
	...
	pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, gc5024_PID_ADDR);

	if (gc5024_PID_VALUE == pid_value) {
		ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, gc5024_VER_ADDR);
		SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value, ver_value);
		
		//if(1)
		if (gc5024_VER_VALUE == ver_value) 
		{
			SENSOR_LOGI("this is gc5024 sensor");
 ...
 *============================================================================*/
static cmr_int gc5024_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
	...
	gc5024_drv_calc_exposure(handle,cap_shutter,0,capture_mode,&gc5024_aec_info);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.frame_length);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.shutter);

	sns_drv_cxt->sensor_ev_info.preview_gain=cap_gain;
	gc5024_drv_write_gain(&gc5024_aec_info, cap_gain);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.again);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int gc5024_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	gc5024_drv_calc_exposure(handle,exposure_line,dummy_line,size_index,&gc5024_aec_info);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.frame_length);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc5024_drv_write_gain_value(cmr_handle handle, cmr_uint param)
{
	cmr_int ret_value = SENSOR_SUCCESS;
	...
	gc5024_drv_calc_gain(handle,param,&gc5024_aec_info);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.again);
	gc5024_drv_write_reg2sensor(handle,gc5024_aec_info.dgain);
...
static cmr_int gc5024_drv_read_aec_info(cmr_handle handle,cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &gc5024_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    gc5024_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &gc5024_aec_info);
    gc5024_drv_calc_gain(handle,info->gain, &gc5024_aec_info);

    return ret_value;
}

static cmr_int gc5024_drv_set_master_FrameSync(cmr_handle handle,cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc5024_drv_stream_on(cmr_handle handle, cmr_uint param)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc5024_drv_stream_off(cmr_handle handle, cmr_uint param)
{
	SENSOR_LOGI("E");
...
static cmr_int gc5024_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle) 
{
    ...
    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_gc5024_module_info_tab), s_gc5024_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_gc5024_resolution_tab_raw), s_gc5024_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_gc5024_resolution_trim_tab), s_gc5024_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_gc5024_static_info), s_gc5024_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_gc5024_mode_fps_info), s_gc5024_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    gc5024_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int gc5024_drv_handle_delete(cmr_handle handle, void *param) 
{
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int gc5024_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
static struct sensor_ic_ops s_gc5024_ops_tab = {
    .create_handle = gc5024_drv_handle_create,
    .delete_handle = gc5024_drv_handle_delete,
    .get_data = gc5024_drv_get_private_data,
	.power = gc5024_drv_power_on,
	.identify = gc5024_drv_identify,
	.ex_write_exp = gc5024_drv_write_exposure,
	.write_gain_value = gc5024_drv_write_gain_value,
	
#if defined(CONFIG_DUAL_MODULE)
	//.read_aec_info = gc5024_drv_read_aec_info,
#endif

    .ext_ops = {
        [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = gc5024_drv_before_snapshot,
        [SENSOR_IOCTL_STREAM_ON].ops = gc5024_drv_stream_on,
        [SENSOR_IOCTL_STREAM_OFF].ops = gc5024_drv_stream_off,
        /* expand interface,if you want to add your sub cmd ,
         *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
         */
        [SENSOR_IOCTL_ACCESS_VAL].ops = gc5024_drv_access_val,
    }
};
