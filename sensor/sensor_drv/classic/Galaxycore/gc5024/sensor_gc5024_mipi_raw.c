...
#define TSI_MIPI_NUM_2LANE

#ifdef TSI_MIPI_NUM_2LANE
...
static void gc5024_drv_write_reg2sensor(cmr_handle handle,
...
static void gc5024_drv_write_gain(struct sensor_aec_i2c_tag *aec_info,
                                  cmr_u32 gain) {
    ...
    if (aec_info->again->size >= 10) {
        ...
        if ((ANALOG_GAIN_1 <= gain) && (gain < ANALOG_GAIN_2)) {
            ...
            aec_info->again->settings[1].reg_value = 0x00;

            temp = gain;
            aec_info->again->settings[2].reg_value = (temp >> 6) & 0xff;
            aec_info->again->settings[3].reg_value = (temp << 2) & 0xfc;

        } else if ((ANALOG_GAIN_2 <= gain) && (gain < ANALOG_GAIN_3)) {

            aec_info->again->settings[1].reg_value = 0x01;
            ...
            aec_info->again->settings[1].reg_value = 0x02;
            ...
            aec_info->again->settings[1].reg_value = 0x03;
            ...
            aec_info->again->settings[1].reg_value = 0x04;
            ...
            aec_info->again->settings[1].reg_value = 0x05;
            ...
            aec_info->again->settings[1].reg_value = 0x06;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc5024_drv_write_frame_length(struct sensor_aec_i2c_tag *aec_info,
                                          cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc5024_drv_write_shutter(struct sensor_aec_i2c_tag *aec_info,
                                     cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    cmr_u32 value = 0x00;
...
static void gc5024_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int gc5024_drv_get_static_info(cmr_handle handle, uint32_t *param) {
    ...
    ex_info->sensor_version_info =
        (cmr_s8 *)g_gc5024_mipi_raw_info.sensor_version_info;
...
static cmr_int gc5024_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    gc5024_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                             &gc5024_aec_info);
...
static cmr_int gc5024_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    gc5024_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                             &gc5024_aec_info);
    gc5024_drv_write_reg2sensor(handle, gc5024_aec_info.frame_length);
    gc5024_drv_write_reg2sensor(handle, gc5024_aec_info.shutter);
...
static cmr_int gc5024_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    ...
    gc5024_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                             &gc5024_aec_info);
...
static cmr_int gc5024_drv_set_master_FrameSync(cmr_handle handle,
                                               cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int
gc5024_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_gc5024_module_info_tab),
                                    s_gc5024_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt,
                                        ARRAY_SIZE(s_gc5024_resolution_tab_raw),
                                        s_gc5024_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_gc5024_resolution_trim_tab),
                                  s_gc5024_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_gc5024_static_info), s_gc5024_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_gc5024_mode_fps_info),
                                 s_gc5024_mode_fps_info);
...
static cmr_int gc5024_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
