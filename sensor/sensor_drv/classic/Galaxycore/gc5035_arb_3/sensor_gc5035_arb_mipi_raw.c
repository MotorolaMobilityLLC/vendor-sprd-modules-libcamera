...
static cmr_u16 gc5035_arb_read_mid(cmr_handle handle)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    cmr_u16 mid_val = 0x00;
    cmr_u16 i = 0;
    uint8_t buff[12] = {0};
    ...
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x67, 0xc0);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x59, 0x3f);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x55, 0x84);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x65, 0x80);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x66, 0x03);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xfe, 0x00);
    SENSOR_LOGD("Enable OTP!");
    ...
    for (i = 0; i < 12; i++){
        buff[i] = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x6c);
        SENSOR_LOGD("otp buff[%d] = 0x%x", i, buff[i]);
    }
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xf3, 0x00);
    ...
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xfe, 0x00);
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xfa, 0x00);
    SENSOR_LOGD("Disable OTP!");

    if((buff[0] & 0x0C) == 0x04){
        mid_val = buff[1];
        SENSOR_LOGD("OTP group 1 is valid!");
    }
    else if ((buff[0] & 0x03) == 0x01){
        mid_val = buff[7];
        SENSOR_LOGD("OTP group 2 is valid!");
    }
    return mid_val;
}
