...
#define LOG_TAG "gc030at_mipi_raw"

#include "sensor_gc030at_mipi_raw.h"
...
static void gc030at_drv_write_reg2sensor(cmr_handle handle,
...
static void gc030at_drv_write_gain(cmr_handle handle,
    ...
    if ((ANALOG_GAIN_1 <= gain) && (gain < ANALOG_GAIN_2)) {
        ...
        temp = gain;
        aec_info->again->settings[1].reg_value = (temp >> 6) & 0xff;
        aec_info->again->settings[2].reg_value = (temp << 2) & 0xfc;
        ...
        aec_info->again->settings[0].reg_value = 0x02;
        ...
        aec_info->again->settings[0].reg_value = 0x03;
        ...
        aec_info->again->settings[0].reg_value = 0x04;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc030at_drv_write_frame_length(cmr_handle handle,
                                          struct sensor_aec_i2c_tag *aec_info,
                                          cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc030at_drv_write_shutter(cmr_handle handle,
                                     struct sensor_aec_i2c_tag *aec_info,
                                     cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    ...
    if (aec_info->shutter->size) {
        ...
        if (shutter > 4095)
            shutter = 4095;
        aec_info->shutter->settings[0].reg_value = 0;
        aec_info->shutter->settings[2].reg_value = shutter & 0xFF;
        aec_info->shutter->settings[1].reg_value = (shutter >> 8) & 0x0F;
 ...
 *============================================================================*/
static void gc030at_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        gc030at_drv_write_frame_length(handle, aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    gc030at_drv_write_shutter(handle, aec_info, shutter);
...
static void gc030at_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    gc030at_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030at_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    BOOLEAN power_down = g_gc030at_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_gc030at_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030at_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int gc030at_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        gc030at_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_gc030at_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_gc030at_mipi_raw_info.sensor_version_info;
...
static cmr_int gc030at_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030at_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGI("sensor gc030at: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = gc030at_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = gc030at_drv_get_fps_info(handle, param_ptr->pval);
        break;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030at_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, gc030at_PID_ADDR);

    if (gc030at_PID_VALUE == pid_value) {
        ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, gc030at_VER_ADDR);
        SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                    ver_value);
        if (gc030at_VER_VALUE == ver_value) {
            SENSOR_LOGI("this is gc030at sensor");
 ...
 *============================================================================*/
static cmr_int gc030at_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    gc030at_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                             &gc030at_aec_info);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.frame_length);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    gc030at_drv_write_gain(handle, &gc030at_aec_info, cap_gain);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.again);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int gc030at_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    gc030at_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                             &gc030at_aec_info);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.frame_length);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc030at_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    gc030at_drv_calc_gain(handle, param, &gc030at_aec_info);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.again);
    gc030at_drv_write_reg2sensor(handle, gc030at_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int gc030at_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &gc030at_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    gc030at_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                             &gc030at_aec_info);
    gc030at_drv_calc_gain(handle, info->gain, &gc030at_aec_info);

    return ret_value;
}

static cmr_int gc030at_drv_set_master_FrameSync(cmr_handle handle,
                                               cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030at_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    if (!strcmp(value1, "1")) {
        ...
        gc030at_drv_set_master_FrameSync(handle, param);
#endif
    }
    ...
    hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x25, 0xe2);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030at_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
...
static cmr_int
gc030at_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    gc030at_drv_write_frame_length(
        sns_drv_cxt, &gc030at_aec_info,
        sns_drv_cxt->sensor_ev_info.preview_framelength);
    gc030at_drv_write_gain(sns_drv_cxt, &gc030at_aec_info,
                          sns_drv_cxt->sensor_ev_info.preview_gain);
    gc030at_drv_write_shutter(sns_drv_cxt, &gc030at_aec_info,
                             sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_gc030at_module_info_tab),
                                    s_gc030at_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt,
                                        ARRAY_SIZE(s_gc030at_resolution_tab_raw),
                                        s_gc030at_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_gc030at_resolution_trim_tab),
                                  s_gc030at_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_gc030at_static_info), s_gc030at_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_gc030at_mode_fps_info),
                                 s_gc030at_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    gc030at_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int gc030at_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int gc030at_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = gc030at_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_gc030at_ops_tab = {
    .create_handle = gc030at_drv_handle_create,
    .delete_handle = gc030at_drv_handle_delete,
    .get_data = gc030at_drv_get_private_data,
    .power = gc030at_drv_power_on,
    .identify = gc030at_drv_identify,
    .ex_write_exp = gc030at_drv_write_exposure,
    .write_gain_value = gc030at_drv_write_gain_value,

#if defined(CONFIG_DUAL_MODULE)
    .read_aec_info = gc030at_drv_read_aec_info,
#endif

    .ext_ops = {[SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = gc030at_drv_before_snapshot,
                [SENSOR_IOCTL_STREAM_ON].ops = gc030at_drv_stream_on,
                [SENSOR_IOCTL_STREAM_OFF].ops = gc030at_drv_stream_off,
                /* expand interface,if you want to add your sub cmd ,
                 *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
                 */
                [SENSOR_IOCTL_ACCESS_VAL].ops = gc030at_drv_access_val}};
