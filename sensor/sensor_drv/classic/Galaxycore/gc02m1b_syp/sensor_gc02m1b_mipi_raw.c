...
#define LOG_TAG "gc02m1b_syp_ums512"
...
static void gc02m1b_drv_write_reg2sensor(cmr_handle handle, struct sensor_i2c_reg_tab *reg_info) 
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc02m1b_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc02m1b_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static void gc02m1b_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
                                                  cmr_u16 mode, struct sensor_aec_i2c_tag *aec_info)
{
	...
	if (dest_fr_len != cur_fr_len){
		...
		gc02m1b_drv_write_frame_length(handle, aec_info, dest_fr_len);
	}
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
	gc02m1b_drv_write_shutter(handle, aec_info, shutter);
...
static void gc02m1b_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) 
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	gc02m1b_drv_write_gain(handle, aec_info,sensor_gain);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc02m1b_drv_init_fps_info(cmr_handle handle) 
{
...
static cmr_int gc02m1b_drv_get_static_info(cmr_handle handle, cmr_u32 *param) 
{
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        gc02m1b_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_gc02m1b_mipi_raw_info.name;
    ex_info->sensor_version_info = (cmr_s8 *)g_gc02m1b_mipi_raw_info.sensor_version_info;
...
static cmr_int gc02m1b_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) 
{
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc02m1b_drv_access_val(cmr_handle handle, cmr_uint param)
{
	...
	SENSOR_LOGI("sensor gc02m1b: param_ptr->type=%x", param_ptr->type);
	
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_GET_STATIC_INFO:
			ret = gc02m1b_drv_get_static_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_GET_FPS_INFO:
			ret = gc02m1b_drv_get_fps_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
			ret = sns_drv_cxt->is_sensor_close = 1;
			break;
		case SENSOR_VAL_TYPE_GET_PDAF_INFO:
			//ret = gc02m1b_drv_get_pdaf_info(handle, param_ptr->pval);
			break;
		default:
			break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc02m1b_drv_identify(cmr_handle handle, cmr_uint param)
{
	...
	pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, GC02M1B_PID_ADDR);

	if (GC02M1B_PID_VALUE == pid_value) {
		ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, GC02M1B_VER_ADDR);
		SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value, ver_value);
		if (GC02M1B_VER_VALUE == ver_value) {
			SENSOR_LOGI("this is gc02m1b_syp_uni sensor");
            sensor_rid_save_sensor_name(SENSOR_HWINFOR_BACKAUX2_CAM_NAME, "3_gc02m1_syp");
 ...
 *============================================================================*/
static cmr_int gc02m1b_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
...
*/
    ...
    gc02m1b_drv_calc_exposure(handle,cap_shutter, 0 , capture_mode,&gc02m1b_aec_info);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.frame_length);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.shutter);

	sns_drv_cxt->sensor_ev_info.preview_gain=cap_gain;
	gc02m1b_drv_write_gain(handle, &gc02m1b_aec_info, cap_gain);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.again);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int gc02m1b_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	gc02m1b_drv_calc_exposure(handle,exposure_line, dummy_line, size_index, &gc02m1b_aec_info);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.frame_length);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc02m1b_drv_write_gain_value(cmr_handle handle, cmr_uint param)
{
	cmr_int ret_value = SENSOR_SUCCESS;
	...
	gc02m1b_drv_calc_gain(handle,param, &gc02m1b_aec_info);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.again);
	gc02m1b_drv_write_reg2sensor(handle, gc02m1b_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int gc02m1b_drv_read_aec_info(cmr_handle handle, cmr_uint param) 
{
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &gc02m1b_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    gc02m1b_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &gc02m1b_aec_info);
    gc02m1b_drv_calc_gain(handle,info->gain, &gc02m1b_aec_info);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc02m1b_drv_stream_on(cmr_handle handle, cmr_uint param)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int gc02m1b_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle) 
{
    ...
    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_gc02m1b_module_info_tab), s_gc02m1b_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_gc02m1b_resolution_tab_raw), s_gc02m1b_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_gc02m1b_resolution_trim_tab), s_gc02m1b_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_gc02m1b_static_info), s_gc02m1b_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_gc02m1b_mode_fps_info), s_gc02m1b_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    gc02m1b_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int gc02m1b_drv_handle_delete(cmr_handle handle, void *param) 
{
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int gc02m1b_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
{
     return &g_gc02m1b_mipi_raw_info;
}
 ...
 * .power = gc02m1b_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_gc02m1b_ops_tab = {
    .create_handle = gc02m1b_drv_handle_create,
    .delete_handle = gc02m1b_drv_handle_delete,
    .get_data = gc02m1b_drv_get_private_data,
/*---------------------------------------*/
    .power = gc02m1b_drv_power_on,
    .identify = gc02m1b_drv_identify,
    .ex_write_exp = gc02m1b_drv_write_exposure,
    .write_gain_value = gc02m1b_drv_write_gain_value,

#if defined(CONFIG_DUAL_MODULE)
	.read_aec_info = gc02m1b_drv_read_aec_info,
#endif

    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = gc02m1b_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = gc02m1b_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = gc02m1b_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = gc02m1b_drv_access_val,
    }
};
