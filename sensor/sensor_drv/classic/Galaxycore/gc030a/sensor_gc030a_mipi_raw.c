...
static void gc030a_drv_write_reg2sensor(cmr_handle handle,
...
static void gc030a_drv_write_gain(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc030a_drv_write_shutter(cmr_handle handle,
                                     struct sensor_aec_i2c_tag *aec_info,
                                     cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 *============================================================================*/
static void gc030a_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        gc030a_drv_write_frame_length(handle, aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    gc030a_drv_write_shutter(handle, aec_info, shutter);
...
static void gc030a_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    gc030a_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_power_on(cmr_handle handle, cmr_uint power_on) {
...
static cmr_int gc030a_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    ex_info->name = (cmr_s8 *)g_gc030a_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_gc030a_mipi_raw_info.sensor_version_info;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_access_val(cmr_handle handle, cmr_uint param) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, gc030a_PID_ADDR);

    if (gc030a_PID_VALUE == pid_value) {
        ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, gc030a_VER_ADDR);
 ...
 *============================================================================*/
static cmr_int gc030a_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    gc030a_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                             &gc030a_aec_info);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.frame_length);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    gc030a_drv_write_gain(handle, &gc030a_aec_info, cap_gain);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.again);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int gc030a_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    gc030a_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                             &gc030a_aec_info);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.frame_length);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc030a_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    gc030a_drv_calc_gain(handle, param, &gc030a_aec_info);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.again);
    gc030a_drv_write_reg2sensor(handle, gc030a_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int gc030a_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &gc030a_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    gc030a_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                             &gc030a_aec_info);
    gc030a_drv_calc_gain(handle, info->gain, &gc030a_aec_info);

    return ret_value;
}

static cmr_int gc030a_drv_set_master_FrameSync(cmr_handle handle,
                                               cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    if (!strcmp(value1, "1")) {
        ...
        gc030a_drv_set_master_FrameSync(handle, param);
#endif
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
...
gc030a_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    gc030a_drv_write_frame_length(
        sns_drv_cxt, &gc030a_aec_info,
        sns_drv_cxt->sensor_ev_info.preview_framelength);
    gc030a_drv_write_gain(sns_drv_cxt, &gc030a_aec_info,
                          sns_drv_cxt->sensor_ev_info.preview_gain);
    gc030a_drv_write_shutter(sns_drv_cxt, &gc030a_aec_info,
                             sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_gc030a_module_info_tab),
                                    s_gc030a_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt,
                                        ARRAY_SIZE(s_gc030a_resolution_tab_raw),
                                        s_gc030a_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_gc030a_resolution_trim_tab),
                                  s_gc030a_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_gc030a_static_info), s_gc030a_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_gc030a_mode_fps_info),
                                 s_gc030a_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    gc030a_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int gc030a_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
#if defined(CONFIG_DUAL_MODULE)
    .read_aec_info = gc030a_drv_read_aec_info,
#endif

    .ext_ops = {[SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = gc030a_drv_before_snapshot,
