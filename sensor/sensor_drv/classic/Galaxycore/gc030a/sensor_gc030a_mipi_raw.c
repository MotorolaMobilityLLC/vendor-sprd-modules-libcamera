...
#include "sensor_gc030a_mipi_raw.h"

#define LOG_TAG "gc030a_mipi_raw"

#define MIPI_RAW_INFO g_gc030a_mipi_raw_info
#define RES_TRIM_TAB s_gc030a_resolution_trim_tab
#define FPS_INFO s_gc030a_mode_fps_info
#define STATIC_INFO s_gc030a_static_info
#define VIDEO_INFO s_gc030a_video_info
#define MODULE_INFO s_gc030a_module_info_tab
#define RES_TAB_RAW s_gc030a_resolution_tab_raw
 ...
 *
 *============================================================================*/
static cmr_int gc030a_drv_set_video_mode(cmr_handle handle, cmr_int param) {
 ...
 *============================================================================*/
static cmr_int gc030a_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int gc030a_drv_get_static_info(cmr_handle handle, cmr_int *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        gc030a_drv_init_fps_info(handle);
    }
    ...
    sensor_ic_print_static_info("gc030a", ex_info);

    return rtn;
}

static cmr_int gc030a_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc030a_drv_group_hold_on(void) {
    SENSOR_LOGI("E");
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc030a_drv_group_hold_off(void) {
    SENSOR_LOGI("E");
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint16_t gc030a_drv_read_gain(void) {
    uint16_t gain_h = 0;
    uint16_t gain_l = 0;
...
static void gc030a_drv_write_gain(cmr_handle handle, cmr_int gain) {
    cmr_u16 temp = 0x00;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint16_t gc030a_drv_read_frame_length(void) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc030a_drv_write_frame_length(cmr_handle handle,
                                          uint32_t frame_len) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t gc030a_drv_read_shutter(void) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc030a_drv_write_shutter(cmr_handle handle, cmr_u32 shutter) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static cmr_u16 gc030a_drv_update_exposure(cmr_handle handle, cmr_u32 shutter,
                                          cmr_u32 dummy_line) {
    ...
    cur_fr_len = gc030a_drv_read_frame_length();

    if (shutter < SENSOR_MIN_SHUTTER)
        shutter = SENSOR_MIN_SHUTTER;

    if (dest_fr_len != cur_fr_len)
        gc030a_drv_write_frame_length(handle, dest_fr_len);
write_sensor_shutter:
    /* write shutter to sensor registers */
    gc030a_drv_write_shutter(handle, shutter);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_power_on(cmr_handle handle, cmr_int power_on) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    struct module_cfg_info *module_info = sns_drv_cxt->module_info;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_access_val(cmr_handle handle, cmr_int param) {
    ...
    SENSOR_LOGI("sensor gc030a: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_SHUTTER:
        *((cmr_u32 *)param_ptr->pval) = gc030a_drv_read_shutter();
        break;
    case SENSOR_VAL_TYPE_READ_OTP_GAIN:
        *((cmr_u32 *)param_ptr->pval) = gc030a_drv_read_gain();
        break;
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = gc030a_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = gc030a_drv_get_fps_info(handle, param_ptr->pval);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_init_exif_info(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_identify(cmr_handle handle, cmr_int param) {
    UNUSED(param);
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0xf0);
    if (0x03 == pid_value) {
        ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0xf1);
        SENSOR_LOGI("Identify: PID = %x, VER = %x", pid_value, ver_value);
        if (0x0a == ver_value) {
            SENSOR_LOGI("this is gc030a sensor");
            // gc030a_drv_InitExifInfo();
 ...
 *
 *============================================================================*/
static cmr_int gc030a_drv_get_trim_tab(cmr_handle handle, cmr_int param) {
    UNUSED(param);
    return (unsigned long)RES_TRIM_TAB;
}
 ...
 *============================================================================*/
static cmr_int gc030a_drv_before_snapshot(cmr_handle handle, uint32_t param) {
    ...
    cap_shutter = gc030a_drv_update_exposure(handle, cap_shutter, 0);
    cap_gain = gain;
    gc030a_drv_write_gain(handle, cap_gain);
 ...
 *============================================================================*/
static cmr_int gc030a_drv_write_exposure(cmr_handle handle, cmr_int param) {
    ...
    sns_drv_cxt->sensor_ev_info.preview_shutter =
        gc030a_drv_update_exposure(handle, exposure_line, dummy_line);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc030a_drv_write_gain_value(cmr_handle handle, cmr_u32 param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    cmr_u32 real_gain = 0;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    gc030a_drv_write_gain(handle, real_gain);
 ...
 *
 *============================================================================*/
static void gc030a_drv_increase_hdr_exposure(cmr_handle handle,
    ...
    if (shutter_multiply >= ev_multiplier) {
        gc030a_drv_update_exposure(
            handle, sns_drv_cxt->hdr_info.capture_shutter * ev_multiplier, 0);
        gc030a_drv_write_gain(handle, sns_drv_cxt->hdr_info.capture_gain);
    } else {
        ...
        gc030a_drv_update_exposure(
            ...
            0);
        gc030a_drv_write_gain(handle, gain);
    }
}
 ...
 *
 *============================================================================*/
static void gc030a_drv_decrease_hdr_exposure(cmr_handle handle,
                                             cmr_u8 ev_divisor) {
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc030a_drv_set_hdr_ev(cmr_handle handle, cmr_int param) {
    ...
    case SENSOR_HDR_EV_LEVE_0:
        ev_divisor = 2;
        gc030a_drv_decrease_hdr_exposure(handle, ev_divisor);
        break;
    case SENSOR_HDR_EV_LEVE_1:
        ev_multiplier = 1;
        gc030a_drv_increase_hdr_exposure(handle, ev_multiplier);
        break;
 ...
 *sensor_drv_u.h
 *============================================================================*/
static cmr_int gc030a_drv_ext_func(cmr_handle handle, cmr_int param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_EXT_FUN_PARAM_T_PTR ext_ptr = (SENSOR_EXT_FUN_PARAM_T_PTR)param;
    SENSOR_IC_CHECK_PTR(ext_ptr);
    ...
    switch (ext_ptr->cmd) {
    case SENSOR_EXT_EV:
        ret = gc030a_drv_set_hdr_ev(handle, param);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_stream_on(cmr_handle handle, uint32_t param) {
    UNUSED(param);
    SENSOR_LOGI("E");
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc030a_drv_stream_off(cmr_handle handle, cmr_int param) {
    UNUSED(param);
    SENSOR_LOGI("E");
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int
gc030a_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    gc030a_drv_init_exif_info(sns_drv_cxt, &sns_drv_cxt->exif_ptr);
    /*add private here*/
    return ret;
}

static cmr_int gc030a_drv_handle_delete(cmr_handle handle, cmr_int *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int gc030a_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
static struct sensor_ic_ops s_gc030a_ops_tab = {
    .create_handle = gc030a_drv_handle_create,
    .delete_handle = gc030a_drv_handle_delete,
    .get_data = gc030a_drv_get_private_data,
    .power = gc030a_drv_power_on,
    .identify = gc030a_drv_identify,
    .ex_write_exp = gc030a_drv_write_exposure,
    .write_gain_value = gc030a_drv_write_gain_value,
    .ext_ops = {
            [SENSOR_IOCTL_EXT_FUNC].ops = gc030a_drv_ext_func,
            [SENSOR_IOCTL_VIDEO_MODE].ops = gc030a_drv_set_video_mode,
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = gc030a_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = gc030a_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = gc030a_drv_stream_off,
            [SENSOR_IOCTL_ACCESS_VAL].ops = gc030a_drv_access_val,
    }};
