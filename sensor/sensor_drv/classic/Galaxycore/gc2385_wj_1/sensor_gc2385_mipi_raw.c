...
static void gc2385_drv_write_reg2sensor(cmr_handle handle, struct sensor_i2c_reg_tab *reg_info)
{
...
static void gc2385_drv_write_gain(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 gain)
{
		...
		if(aec_info->again->size)
	{
		...
		{
			aec_info->again->settings[1].reg_value = 0x73;
			aec_info->again->settings[2].reg_value = 0xa2;
			//analog gain
			aec_info->again->settings[3].reg_value = 0x00;
		...
		else if((ANALOG_GAIN_2<= gain)&&(gain < ANALOG_GAIN_3))
		{
			...
			aec_info->again->settings[2].reg_value = 0xa2;
			//analog gain
			aec_info->again->settings[3].reg_value = 0x01;
		...
		{
			aec_info->again->settings[1].reg_value = 0x73;
			aec_info->again->settings[2].reg_value = 0xa3;
			//analog gain
			aec_info->again->settings[3].reg_value = 0x04;
			temp = 256*gain /ANALOG_GAIN_5;
		}
		else if((ANALOG_GAIN_6<= gain )&&(gain< ANALOG_GAIN_7))
		{
			aec_info->again->settings[1].reg_value = 0x74;
			aec_info->again->settings[2].reg_value = 0xa3;
			//analog gain
			aec_info->again->settings[3].reg_value = 0x05;
		...
		else if((ANALOG_GAIN_7<= gain )&&(gain< ANALOG_GAIN_8))
		{
			...
			aec_info->again->settings[2].reg_value = 0xa3;
			//analog gain
			aec_info->again->settings[3].reg_value = 0x06;
		...
		else if((ANALOG_GAIN_8<= gain )&&(gain< ANALOG_GAIN_9))
		{
			...
			aec_info->again->settings[2].reg_value = 0xa3;
			//analog gain
			aec_info->again->settings[3].reg_value = 0x07;
			temp = 256*gain /ANALOG_GAIN_8;
		}
		else
		{
			aec_info->again->settings[1].reg_value = 0x75;
			aec_info->again->settings[2].reg_value = 0xa4;
			//analog gain
			aec_info->again->settings[3].reg_value = 0x08;
			temp = 256*gain /ANALOG_GAIN_9;
		}
	    aec_info->again->settings[0].reg_value =0x00;
		aec_info->again->settings[4].reg_value = temp>>8;
		aec_info->again->settings[5].reg_value = temp&0xff;	
		/*END*/
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc2385_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc2385_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    if(aec_info->shutter->size){
		/*TODO*/
		aec_info->shutter->settings[1].reg_value = (shutter>>8)&0x3f;
 ...
 *============================================================================*/
static void gc2385_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
                                                  cmr_u16 mode, struct sensor_aec_i2c_tag *aec_info)
{
	...
	if (dest_fr_len != cur_fr_len){
		...
		gc2385_drv_write_frame_length(handle, aec_info, dest_fr_len);
	}
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
	gc2385_drv_write_shutter(handle, aec_info, shutter);
...
static void gc2385_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) 
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	gc2385_drv_write_gain(handle, aec_info,sensor_gain);
...
static cmr_int gc2385_drv_get_static_info(cmr_handle handle, cmr_u32 *param) 
{
    ...
    ex_info->sensor_version_info = (cmr_s8 *)g_gc2385_mipi_raw_info.sensor_version_info;
...
*/
    ...
    gc2385_drv_calc_exposure(handle,cap_shutter, 0 , capture_mode, &gc2385_aec_info);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.frame_length);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.shutter);

	sns_drv_cxt->sensor_ev_info.preview_gain=cap_gain;
	gc2385_drv_write_gain(handle, &gc2385_aec_info, cap_gain);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.again);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.dgain);
...
static cmr_int gc2385_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	gc2385_drv_calc_exposure(handle,exposure_line, dummy_line, size_index, &gc2385_aec_info);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.frame_length);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.shutter);
...
static cmr_int gc2385_drv_write_gain_value(cmr_handle handle, cmr_uint param)
{
	...
	gc2385_drv_calc_gain(handle,param, &gc2385_aec_info);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.again);
	gc2385_drv_write_reg2sensor(handle, gc2385_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int gc2385_drv_read_aec_info(cmr_handle handle, cmr_uint param) 
{
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &gc2385_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    gc2385_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &gc2385_aec_info);
    gc2385_drv_calc_gain(handle,info->gain, &gc2385_aec_info);

    return ret_value;
}

static cmr_int gc2385_drv_set_master_FrameSync(cmr_handle handle, cmr_uint param) 
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int gc2385_drv_stream_on(cmr_handle handle, cmr_uint param)
{
	...
	//gc2385_drv_set_master_FrameSync(handle, param);
	//gc2385_drv_set_slave_FrameSync(handle, param);
#endif   
	/*TODO*/
	usleep(100*1000);
...
static cmr_int gc2385_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle) 
{
	...
	gc2385_drv_write_frame_length(sns_drv_cxt, &gc2385_aec_info, sns_drv_cxt->sensor_ev_info.preview_framelength);
	gc2385_drv_write_gain(sns_drv_cxt, &gc2385_aec_info, sns_drv_cxt->sensor_ev_info.preview_gain);
	gc2385_drv_write_shutter(sns_drv_cxt, &gc2385_aec_info, sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_gc2385_module_info_tab), s_gc2385_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_gc2385_resolution_tab_raw), s_gc2385_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_gc2385_resolution_trim_tab), s_gc2385_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_gc2385_static_info), s_gc2385_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_gc2385_mode_fps_info), s_gc2385_mode_fps_info);
...
static cmr_int gc2385_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
#if defined(CONFIG_DUAL_MODULE)
	//.read_aec_info = gc2385_drv_read_aec_info,
#endif
