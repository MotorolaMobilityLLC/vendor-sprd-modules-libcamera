...
static cmr_int gc2375a_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    if (gc2375a_PID_VALUE == pid_value) {
        ver_value =
            hw_sensor_read_reg(sns_drv_cxt->hw_handle, gc2375a_VER_ADDR);
...
gc2375a_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    gc2375a_drv_write_frame_length(
        &gc2375a_aec_info, sns_drv_cxt->sensor_ev_info.preview_framelength);
    gc2375a_drv_write_gain(&gc2375a_aec_info,
                           sns_drv_cxt->sensor_ev_info.preview_gain);
    gc2375a_drv_write_shutter(&gc2375a_aec_info,
                              sns_drv_cxt->sensor_ev_info.preview_shutter);
    ...
    sensor_ic_set_match_resolution_info(
        sns_drv_cxt, ARRAY_SIZE(s_gc2375a_resolution_tab_raw),
    ...
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_gc2375a_mode_fps_info),
