...
static void gc5035_drv_write_gain(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 gain)
{
	...
	cmr_u32 temp_gain;
	cmr_int gain_index;
	cmr_u16 GC5035_AGC_Param[MAX_GAIN_INDEX][2] = {
		{  256,  0 },
		{  302,  1 },
		{  358,  2 },
		{  425,  3 },
...
static cmr_u8 gc5035_read_otp(cmr_handle handle, cmr_u8 addr)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_u32 gc5035_read_otp_info(cmr_handle handle, void *param_ptr)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

 	cmr_u32 rtn = SENSOR_SUCCESS;
	struct gc5035_otp *s_gc_info_ptr = &gc5035_otp_info;
	...
	flagdd = gc5035_read_otp(handle, DPC_FLAG_OFFSET);
	SENSOR_PRINT("GC5035_OTP_DD : flag_dd = 0x%x\n", flagdd);

	switch (flagdd & 0x03) {
	case 0x00:
		SENSOR_PRINT("GC5035_OTP_DD is Empty !!\n");
		s_gc_info_ptr->dd_flag = 0x00;
		break;
	case 0x01:
		SENSOR_PRINT("GC5035_OTP_DD is Valid!!\n");
		s_gc_info_ptr->dd_num = gc5035_read_otp(handle, DPC_TOTAL_NUMBER_OFFSET) + gc5035_read_otp(handle, DPC_ERROR_NUMBER_OFFSET);
		s_gc_info_ptr->dd_flag = 0x01;
		break;
	case 0x02:
	case 0x03:
		SENSOR_PRINT("GC5035_OTP_DD is Invalid !!\n");
	...
	/* chip regs */
	s_gc_info_ptr->reg_flag = gc5035_read_otp(handle, REG_INFO_FLAG_OFFSET);

	if ((s_gc_info_ptr->reg_flag & 0x03) == 0x01)
		for (i = 0; i < REG_INFO_SIZE; i++) {
			temp = gc5035_read_otp(handle, REG_INFO_PAGE_OFFSET+ REG_INFO_SIZE*8 * i);
			for (j = 0; j < 2; j++)
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {
					s_gc_info_ptr->regs[s_gc_info_ptr->reg_num][0] = (temp >> (4 * j)) & 0x07;
					s_gc_info_ptr->regs[s_gc_info_ptr->reg_num][1] =
						gc5035_read_otp(handle,REG_INFO_ADDR_OFFSET + REG_INFO_SIZE *8 * i + 0x10 * j);
					s_gc_info_ptr->regs[s_gc_info_ptr->reg_num][2] =
						gc5035_read_otp(handle,REG_INFO_VALUE_OFFSET + REG_INFO_SIZE*8 * i + 0x10 * j);
...
static cmr_u8 gc5035_gcore_load_data(cmr_handle handle, void *param_ptr)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	rtn = gc5035_read_otp_info(handle, s_gc_info_ptr);

	/* Load DD */
	if (s_gc_info_ptr->dd_flag == 0x01) {
		SENSOR_PRINT("GC5035_OTP_DD Load start !\n");
		hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xfe, 0x00);	
		...
		hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x01, (s_gc_info_ptr->dd_num >>8) & 0x07);
		hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x02, s_gc_info_ptr->dd_num & 0xff);
	...
	/* sensor regs */
	SENSOR_PRINT("GC5035_OTP_UPDATE_CHIPVERSION: reg_num = %d\n", s_gc_info_ptr->reg_num);

	if ((s_gc_info_ptr->reg_flag & 0x03) == 0x01){
		for (i = 0; i < s_gc_info_ptr->reg_num; i++) {
			hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xfe, s_gc_info_ptr->regs[i][0]);
			hw_sensor_write_reg(sns_drv_cxt->hw_handle, s_gc_info_ptr->regs[i][1], s_gc_info_ptr->regs[i][2]);
			SENSOR_PRINT("GC5035_OTP_UPDATE_CHIP_VERSION: P%d:0x%x -> 0x%x\n",
			s_gc_info_ptr->regs[i][0], s_gc_info_ptr->regs[i][1], s_gc_info_ptr->regs[i][2]);
		}
	}
...
static cmr_int gc5035_drv_identify(cmr_handle handle, cmr_uint param)
{
	...
	SENSOR_IC_CHECK_HANDLE(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	struct gc5035_otp *otp_info = &gc5035_otp_info;
	...
	if (GC5035_PID_VALUE == pid_value) {
		...
		if (GC5035_VER_VALUE == ver_value) {
			...
			if(gc5035_gcore_load_data(handle, otp_info) != SENSOR_SUCCESS)
...
static cmr_int gc5035_drv_set_slave_FrameSync(cmr_handle handle, cmr_uint param)
{
	SENSOR_IC_CHECK_HANDLE(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	//hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x3002, 0x40);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xfe, 0x00);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x85, 0x03); 
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xfe, 0x00);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x00, 0x09);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x82, 0x08);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x83, 0x10);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x86, 0x04);
	...
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x8b, 0x20);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x84, 0x80);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x87, 0x09);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x85, 0x53);
