...
#define LOG_TAG "itel_gc6153_pike2"

#include "sensor_gc6153_mipi_yuv.h"
#define GC6153_OTHER_POWER_PIN_CTL
...
static void gc6153_drv_write_reg2sensor(cmr_handle handle,struct sensor_i2c_reg_tab *reg_info)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc6153_drv_write_gain(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 gain)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc6153_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc6153_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static void gc6153_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line,
                                                  cmr_u16 mode, struct sensor_aec_i2c_tag *aec_info)
{
	...
	if (dest_fr_len != cur_fr_len){
		...
		gc6153_drv_write_frame_length(handle, aec_info,dest_fr_len);
	}
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
	gc6153_drv_write_shutter(handle, aec_info,shutter);
...
static void gc6153_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info)
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	gc6153_drv_write_gain(handle, aec_info,sensor_gain);
...
static cmr_int gc6153_drv_power_on(cmr_handle handle, cmr_uint power_on)
{
    ...
    BOOLEAN power_down = g_gc6153_mipi_yuv_info.power_down_level;
    BOOLEAN reset_level = g_gc6153_mipi_yuv_info.reset_pulse_level;
    ...
    if (SENSOR_TRUE == power_on)
	{
            ...
            usleep(50);//change must delay 10us,mclk power on by yuze.zhong@transsion.2019-7-13
...
#ifdef GC6153_OTHER_POWER_PIN_CTL
static cmr_int gc6153_drv_power_hardwave_standby_mode(cmr_handle handle, cmr_uint power_on)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    BOOLEAN power_down = g_gc6153_mipi_yuv_info.power_down_level;
    ...
    //if(!POWER)
    //{
        //if sensor pwdn is high when hardwave standby mode, have to power off it when other sensor close
        if(power_on){
            hw_sensor_power_down(sns_drv_cxt->hw_handle, power_down);
            SENSOR_LOGD("done1");
        }else
...
static cmr_int gc6153_drv_ioctl(cmr_handle handle, int cmd, void *param)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    //struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    {
#ifdef GC6153_OTHER_POWER_PIN_CTL
        case SENSOR_DRIVER_CMD_SELF_HAEDWAVE_STANDBY:
            //hardwave standby mode
            gc6153_drv_power_hardwave_standby_mode(handle, SENSOR_TRUE );
            break;
        case SENSOR_DRIVER_CMD_OTHERS_POWER_OFF_CALLBACK:
             //power off pwdn pin
            gc6153_drv_power_hardwave_standby_mode(handle, SENSOR_FALSE);
            break;
#endif
        default:
            break;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6153_drv_init_fps_info(cmr_handle handle)
{
...
static cmr_int gc6153_drv_get_static_info(cmr_handle handle, void *param)
{
    ...
    if (!fps_info->is_init) {
        gc6153_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_gc6153_mipi_yuv_info.name;
    ex_info->sensor_version_info = (cmr_s8 *)g_gc6153_mipi_yuv_info.sensor_version_info;
...
static cmr_int gc6153_drv_get_brightness(cmr_handle handle, cmr_u32 *param)
{
	//struct sensor_aec_reg_info *info = (struct sensor_aec_reg_info *)param;
	cmr_u32 gc6153_bvinfo = 0;
	cmr_u16 bv16bit = 0;
	SENSOR_IC_CHECK_HANDLE(handle);
	SENSOR_IC_CHECK_PTR(param);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	usleep(1 * 1000);
	bv16bit = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x93);
	if (bv16bit==0xffff)
	{
		gc6153_bvinfo = -2; //-2 read fail
		*param = gc6153_bvinfo;
	    return SENSOR_FAIL;
	}
    else{
        gc6153_bvinfo = (cmr_u16)bv16bit;
    }
    *param = gc6153_bvinfo;
	SENSOR_LOGI("gc6153_bvinfo = 0x%x,  originsl=0x%x", *param,(cmr_u16)bv16bit);
    return SENSOR_SUCCESS;
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6153_drv_access_val(cmr_handle handle, cmr_uint param)
{
	...
	SENSOR_LOGV("sensor gc6153: param_ptr->type=%x", param_ptr->type);

	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_GET_STATIC_INFO:
			ret = gc6153_drv_get_static_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_GET_FPS_INFO:
			ret = gc6153_drv_get_fps_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
			ret = sns_drv_cxt->is_sensor_close = 1;
			break;
		case SENSOR_VAL_TYPE_GET_PDAF_INFO:
			//ret = gc6153_drv_get_pdaf_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_GET_BV:
	        ret = gc6153_drv_get_brightness(handle, param_ptr->pval);
	        break;
		default:
			break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6153_drv_identify(cmr_handle handle, cmr_uint param)
{
	cmr_u16 pid_valueH = 0x00;
	cmr_u16 pid_valueL = 0x00;	
	cmr_int ret_value = SENSOR_FAIL;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

	SENSOR_LOGI("mipi yuv identify");

    pid_valueH = hw_sensor_read_reg(sns_drv_cxt->hw_handle, GC6153_PID_ADDR);

    if (GC6153_PID_VALUE == pid_valueH) {
        pid_valueL = hw_sensor_read_reg(sns_drv_cxt->hw_handle, GC6153_VER_ADDR);
        SENSOR_LOGI("Identify: pid_valueH = %x, pid_valueL = %x", pid_valueH,
                    pid_valueL);
        if (GC6153_VER_VALUE == pid_valueL) {
            SENSOR_LOGI("this is gc6153 sensor");
            ret_value = SENSOR_SUCCESS;
        } else {
            SENSOR_LOGE("sensor identify fail, pid_valueH = %x, pid_valueL = %x",
                        pid_valueH, pid_valueL);
        }
    } else {
        SENSOR_LOGE("sensor identify fail, pid_valueH = %x, pid_valueL = %x",
                    pid_valueH, pid_valueL);
 ...
 *============================================================================*/
static cmr_int gc6153_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
	...
	gc6153_drv_calc_exposure(handle,cap_shutter,0,capture_mode,&gc6153_aec_info);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.frame_length);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.shutter);

	sns_drv_cxt->sensor_ev_info.preview_gain=cap_gain;
	gc6153_drv_write_gain(handle, &gc6153_aec_info, cap_gain);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.again);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int gc6153_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	gc6153_drv_calc_exposure(handle,exposure_line,dummy_line,size_index,&gc6153_aec_info);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.frame_length);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc6153_drv_write_gain_value(cmr_handle handle, cmr_uint param)
{
	cmr_int ret_value = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

	gc6153_drv_calc_gain(handle,param,&gc6153_aec_info);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.again);
	gc6153_drv_write_reg2sensor(handle,gc6153_aec_info.dgain);
 ...
 *============================================================================*/
 #if 0
static cmr_int gc6153_drv_read_aec_info(cmr_handle handle,cmr_uint param )
{
    cmr_int ret_value = SENSOR_SUCCESS;
    //struct sensor_aec_reg_info *info = (struct sensor_aec_reg_info *)param;
	cmr_u8 gc6153_bvinfo=0;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	hw_sensor_write_reg(sns_drv_cxt->hw_handle,0xfe,0x00);
	usleep(1 * 1000);
	gc6153_bvinfo=hw_sensor_read_reg(sns_drv_cxt->hw_handle,0x93);
	SENSOR_LOGI("gc6153_drv_read_aec_info gc6153_bvinfo = 0x%x",gc6153_bvinfo);

    *((cmr_u8 *)param) = gc6153_bvinfo;
	return SENSOR_SUCCESS;
}
#endif
static cmr_int gc6153_drv_set_master_FrameSync(cmr_handle handle,cmr_uint param)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int gc6153_drv_stream_on(cmr_handle handle, cmr_uint param)
{
	...
	//gc6153_drv_set_master_FrameSync(handle,param);
#endif
	/*TODO*/

	//hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xf1, 0x03);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6153_drv_stream_off(cmr_handle handle, cmr_uint param)
{
	SENSOR_LOGI("E");
	...
	//hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0xf1, 0x40);
...
static cmr_int gc6153_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle)
{
    ...
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_gc6153_resolution_trim_tab), s_gc6153_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_gc6153_static_info), s_gc6153_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_gc6153_mode_fps_info), s_gc6153_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    gc6153_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int gc6153_drv_handle_delete(cmr_handle handle, void *param)
{
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int gc6153_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = gc6153_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_gc6153_ops_tab = {
    .create_handle = gc6153_drv_handle_create,
    .delete_handle = gc6153_drv_handle_delete,
    .get_data = gc6153_drv_get_private_data,
/*---------------------------------------*/
	.power = gc6153_drv_power_on,
	.identify = gc6153_drv_identify,
	.ex_write_exp = gc6153_drv_write_exposure,
	.write_gain_value = gc6153_drv_write_gain_value,
    .ioctl = gc6153_drv_ioctl,
	//.read_aec_info = gc6153_drv_read_aec_info,
    .ext_ops = {
        [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = gc6153_drv_before_snapshot,
        [SENSOR_IOCTL_STREAM_ON].ops = gc6153_drv_stream_on,
        [SENSOR_IOCTL_STREAM_OFF].ops = gc6153_drv_stream_off,
        /* expand interface,if you want to add your sub cmd ,
         *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
         */
        [SENSOR_IOCTL_ACCESS_VAL].ops = gc6153_drv_access_val,
    }
};
