...
#define LOG_TAG "gc2375_wj_2_9863a1c10"

//#define MIPI_NUM_2LANE

#ifdef MIPI_NUM_2LANE
#include "sensor_gc2375_mipi_raw_2lane.h"
#else
#include "sensor_gc2375_mipi_raw_1lane.h"
#endif
...
static void gc2375_drv_write_reg2sensor(cmr_handle handle, struct sensor_i2c_reg_tab *reg_info) 
{
...
static void gc2375_drv_write_gain(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 gain) 
{
    ...
    /*TODO*/
		...
		{
			aec_info->again->settings[1].reg_value = 0x0b;
			aec_info->again->settings[2].reg_value = 0x0c;
			aec_info->again->settings[3].reg_value = 0x0e;
			aec_info->again->settings[4].reg_value = 0x00;
			temp = gain;
		} else if ((ANALOG_GAIN_2 <= gain) && (gain < ANALOG_GAIN_3)) 
		{
			aec_info->again->settings[1].reg_value = 0x0c;
			aec_info->again->settings[2].reg_value = 0x0e;
			aec_info->again->settings[3].reg_value = 0x0e;
		...
		} else if ((ANALOG_GAIN_4 <= gain) && (gain < ANALOG_GAIN_5)) 
		{
			...
			aec_info->again->settings[2].reg_value = 0x0e;
			aec_info->again->settings[3].reg_value = 0x0e;
			aec_info->again->settings[4].reg_value = 0x03;
		...
		} else if ((ANALOG_GAIN_5 <= gain) && (gain < ANALOG_GAIN_6)) 
		{
			...
			aec_info->again->settings[2].reg_value = 0x0e;
			aec_info->again->settings[3].reg_value = 0x0e;
			aec_info->again->settings[4].reg_value = 0x04;
			temp = 64 * gain / ANALOG_GAIN_5;
		} else if ((ANALOG_GAIN_6 <= gain) && (gain < ANALOG_GAIN_7)) 
		{
			aec_info->again->settings[1].reg_value = 0x0e;
			aec_info->again->settings[2].reg_value = 0x0e;
			aec_info->again->settings[3].reg_value = 0x0e;
			aec_info->again->settings[4].reg_value = 0x05;
		...
		} else if ((ANALOG_GAIN_7 <= gain) && (gain < ANALOG_GAIN_8)) 
		{
			...
			aec_info->again->settings[2].reg_value = 0x0c;
			aec_info->again->settings[3].reg_value = 0x0e;
			aec_info->again->settings[4].reg_value = 0x06;
		...
		} else if ((ANALOG_GAIN_8 <= gain) && (gain < ANALOG_GAIN_9)) 
		{
			...
			aec_info->again->settings[2].reg_value = 0x0e;
			aec_info->again->settings[3].reg_value = 0x0e;
			aec_info->again->settings[4].reg_value = 0x07;
		...
		} else 
		{
			...
			aec_info->again->settings[2].reg_value = 0x0e;
			aec_info->again->settings[3].reg_value = 0x0e;
			aec_info->again->settings[4].reg_value = 0x08;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc2375_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc2375_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static void gc2375_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
...
//	if (dest_fr_len != cur_fr_len){
		...
		gc2375_drv_write_frame_length(handle, aec_info, dest_fr_len);
//	}
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
//	shutter = shutter + dummy_line;
	gc2375_drv_write_shutter(handle, aec_info, shutter);
...
static void gc2375_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) 
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	gc2375_drv_write_gain(handle, aec_info,sensor_gain);
...
static cmr_int gc2375_drv_get_static_info(cmr_handle handle, cmr_u32 *param) 
{
    cmr_int rtn = SENSOR_SUCCESS;
...
/*==============================================================================
 * Description:
 * for cover function, read brightness from sensor
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc2375_drv_get_brightness(cmr_handle handle, cmr_u32 *param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    //////////////////////////////////////////////////////////////// for debug
    SENSOR_LOGI("Read reg(0x03)=0x%x\n",hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x03));
    SENSOR_LOGI("Read reg(0x04)=0x%x\n",hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x04));
    SENSOR_LOGI("Read reg(0x20)=0x%x\n",hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x20));
    SENSOR_LOGI("Read reg(0x22)=0x%x\n",hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x22));
    SENSOR_LOGI("Read reg(0x26)=0x%x\n",hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x26));
    SENSOR_LOGI("Read reg(0xb6)=0x%x\n",hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0xb6));
    SENSOR_LOGI("Read reg(0x8f)=0x%x\n",hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x8f));
    //////////////////////////////////////////////////////////////// debug end
    *param = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x3b);
...
static cmr_int gc2375_drv_access_val(cmr_handle handle, cmr_uint param)
{
	...
	switch(param_ptr->type)
	{
    	...
    	case SENSOR_VAL_TYPE_GET_BV:
			ret = gc2375_drv_get_brightness(handle, param_ptr->pval);
			break;
		default:
			break;
    }
...
static cmr_int gc2375_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
    ...
    gc2375_drv_calc_exposure(handle,cap_shutter, 0 , capture_mode,&gc2375_aec_info);
	...
	gc2375_drv_write_gain(handle, &gc2375_aec_info, cap_gain);
...
static cmr_int gc2375_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	gc2375_drv_calc_exposure(handle,exposure_line, dummy_line, size_index, &gc2375_aec_info);
	gc2375_drv_write_reg2sensor(handle, gc2375_aec_info.frame_length);
	gc2375_drv_write_reg2sensor(handle, gc2375_aec_info.shutter);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int gc2375_drv_read_aec_info(cmr_handle handle, cmr_uint param) 
{
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &gc2375_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    gc2375_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &gc2375_aec_info);
    gc2375_drv_calc_gain(handle,info->gain, &gc2375_aec_info);

    return ret_value;
}

static cmr_int gc2375_drv_set_master_FrameSync(cmr_handle handle, cmr_uint param) 
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int gc2375_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle) 
{
	...
	gc2375_drv_write_frame_length(sns_drv_cxt, &gc2375_aec_info, sns_drv_cxt->sensor_ev_info.preview_framelength);
	gc2375_drv_write_gain(sns_drv_cxt, &gc2375_aec_info, sns_drv_cxt->sensor_ev_info.preview_gain);
	gc2375_drv_write_shutter(sns_drv_cxt, &gc2375_aec_info, sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_gc2375_module_info_tab), s_gc2375_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_gc2375_resolution_tab_raw), s_gc2375_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_gc2375_resolution_trim_tab), s_gc2375_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_gc2375_static_info), s_gc2375_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_gc2375_mode_fps_info), s_gc2375_mode_fps_info);
...
static cmr_int gc2375_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
