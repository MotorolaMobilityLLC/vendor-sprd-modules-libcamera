...
#define LOG_TAG "s5k4h8yx_sprocomm"

//#define MIPI_NUM_2LANE

#ifdef MIPI_NUM_2LANE
#include "sensor_s5k4h8yx_mipi_raw_2lane.h"
#else
#include "sensor_s5k4h8yx_mipi_raw_4lane.h"
#endif
 ...
 *============================================================================*/
static void s5k4h8yx_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void s5k4h8yx_drv_write_gain(cmr_handle handle,
    ...
    if (aec_info->again->size) {
        /*TODO*/
        aec_info->again->settings[0].reg_value = gain & 0xffff;
        /*END*/
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void s5k4h8yx_drv_write_frame_length(cmr_handle handle,
                                            struct sensor_aec_i2c_tag *aec_info,
                                            cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    ...
    if (aec_info->frame_length->size) {
        /*TODO*/
        aec_info->frame_length->settings[0].reg_value = frame_len & 0xffff;
        /*END*/
    }
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void s5k4h8yx_drv_write_shutter(cmr_handle handle,
                                       struct sensor_aec_i2c_tag *aec_info,
                                       cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 *============================================================================*/
static void s5k4h8yx_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (cur_fr_len > shutter) {
        fps =
            1000000000.0 /
            (cur_fr_len * (float)(sns_drv_cxt->trim_tab_info[mode].line_time));
    } else {
        fps = 1000000000.0 /
              ((shutter + dummy_line) *
               (float)(sns_drv_cxt->trim_tab_info[mode].line_time));
    }
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        s5k4h8yx_drv_write_frame_length(handle, aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    s5k4h8yx_drv_write_shutter(handle, aec_info, shutter);
...
static void s5k4h8yx_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                   struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    s5k4h8yx_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k4h8yx_drv_power_on(cmr_handle handle, cmr_uint power_on) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k4h8yx_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int s5k4h8yx_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        s5k4h8yx_drv_init_fps_info(handle);
    }
...
static cmr_int s5k4h8yx_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k4h8yx_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGI("sensor s5k4h8yx: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = s5k4h8yx_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = s5k4h8yx_drv_get_fps_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
        ret = sns_drv_cxt->is_sensor_close = 1;
        break;
    case SENSOR_VAL_TYPE_GET_PDAF_INFO:
        // ret = s5k4h8yx_drv_get_pdaf_info(handle, param_ptr->pval);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k4h8yx_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    if (s5k4h8yx_PID_VALUE == pid_value) {
        ver_value =
            hw_sensor_read_reg(sns_drv_cxt->hw_handle, s5k4h8yx_VER_ADDR);
        SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                    ver_value);
        if (s5k4h8yx_VER_VALUE == ver_value) {
            SENSOR_LOGI("this is s5k4h8yx sensor");

            ver_value =
                hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x000C) & 0xff;
            SENSOR_LOGI("s5k4h8yx lsc/otp version: 0x%x", ver_value);
 ...
 *============================================================================*/
static cmr_int s5k4h8yx_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    s5k4h8yx_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                               &s5k4h8yx_aec_info);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.frame_length);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    s5k4h8yx_drv_write_gain(handle, &s5k4h8yx_aec_info, cap_gain);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.again);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int s5k4h8yx_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    s5k4h8yx_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                               &s5k4h8yx_aec_info);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.frame_length);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int s5k4h8yx_drv_write_gain_value(cmr_handle handle,
                                             cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    s5k4h8yx_drv_calc_gain(handle, param, &s5k4h8yx_aec_info);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.again);
    s5k4h8yx_drv_write_reg2sensor(handle, s5k4h8yx_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int s5k4h8yx_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &s5k4h8yx_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    s5k4h8yx_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                               &s5k4h8yx_aec_info);
    s5k4h8yx_drv_calc_gain(handle, info->gain, &s5k4h8yx_aec_info);

    return ret_value;
}

static cmr_int s5k4h8yx_drv_set_master_FrameSync(cmr_handle handle,
                                                 cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k4h8yx_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    if (!strcmp(value1, "1")) {
        ...
        // s5k4h8yx_drv_set_master_FrameSync(handle,param);
#endif
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k4h8yx_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
...
static cmr_int
s5k4h8yx_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    s5k4h8yx_drv_write_frame_length(
        sns_drv_cxt, &s5k4h8yx_aec_info,
        sns_drv_cxt->sensor_ev_info.preview_framelength);
    s5k4h8yx_drv_write_gain(sns_drv_cxt, &s5k4h8yx_aec_info,
                            sns_drv_cxt->sensor_ev_info.preview_gain);
    s5k4h8yx_drv_write_shutter(sns_drv_cxt, &s5k4h8yx_aec_info,
                               sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_s5k4h8yx_module_info_tab),
                                    s_s5k4h8yx_module_info_tab);
    sensor_ic_set_match_resolution_info(
        sns_drv_cxt, ARRAY_SIZE(s_s5k4h8yx_resolution_tab_raw),
        s_s5k4h8yx_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_s5k4h8yx_resolution_trim_tab),
                                  s_s5k4h8yx_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_s5k4h8yx_static_info),
                                    s_s5k4h8yx_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_s5k4h8yx_mode_fps_info),
                                 s_s5k4h8yx_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    s5k4h8yx_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int s5k4h8yx_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int s5k4h8yx_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                             void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
{
     return &g_s5k4h8yx_mipi_raw_info;
}
...
static struct sensor_ic_ops s_s5k4h8yx_ops_tab = {
    ...
    .ex_write_exp = s5k4h8yx_drv_write_exposure,
    .write_gain_value = s5k4h8yx_drv_write_gain_value,

#if defined(CONFIG_DUAL_MODULE)
//.read_aec_info = xxx_drv_read_aec_info,
#endif
