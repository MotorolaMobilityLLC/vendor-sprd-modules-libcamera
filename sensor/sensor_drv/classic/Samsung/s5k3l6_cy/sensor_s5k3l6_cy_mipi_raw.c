...
#define LOG_TAG "s5k3l6_cy"

#include "sensor_s5k3l6_cy_mipi_raw_4lane.h"
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void s5k3l6_drv_write_gain(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void s5k3l6_drv_write_shutter(cmr_handle handle,
                                     struct sensor_aec_i2c_tag *aec_info,
                                     cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
...
static void s5k3l6_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    ...
    s5k3l6_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k3l6_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    BOOLEAN power_down = g_s5k3l6_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_s5k3l6_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k3l6_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int s5k3l6_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    ...
    if (!fps_info->is_init) {
        s5k3l6_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_s5k3l6_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_s5k3l6_mipi_raw_info.sensor_version_info;
...
static cmr_int s5k3l6_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
...
static cmr_int s5k3l6_drv_get_pdaf_info(cmr_handle handle, cmr_u32 *param) {
    ...
    pd_pos_is_right_size = NUMBER_OF_ARRAY(s5k3l6_pd_is_right);
    pd_pos_row_size = NUMBER_OF_ARRAY(s5k3l6_pd_row);
    pd_pos_col_size = NUMBER_OF_ARRAY(s5k3l6_pd_col);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k3l6_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, s5k3l6_PID_ADDR);
    if (s5k3l6_PID_VALUE == pid_value) {
        ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, s5k3l6_VER_ADDR);
        SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                    ver_value);
        if (s5k3l6_VER_VALUE == ver_value) {
            SENSOR_LOGI("this is s5k3l6 sensor");
 ...
 *============================================================================*/
static cmr_int s5k3l6_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    s5k3l6_drv_calc_exposure(handle, cap_shutter, 0, capture_mode, cap_exptime,
                             &s5k3l6_aec_info);
    ...
    s5k3l6_drv_write_gain(handle, &s5k3l6_aec_info, cap_gain);
    s5k3l6_drv_write_reg2sensor(handle, s5k3l6_aec_info.again);
    s5k3l6_drv_write_reg2sensor(handle, s5k3l6_aec_info.dgain);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int s5k3l6_drv_write_gain_value(cmr_handle handle, cmr_uint param) {

    cmr_int ret_value = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    s5k3l6_drv_calc_gain(handle, param, &s5k3l6_aec_info);
    s5k3l6_drv_write_reg2sensor(handle, s5k3l6_aec_info.again);
    s5k3l6_drv_write_reg2sensor(handle, s5k3l6_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int s5k3l6_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    ...
    info->aec_i2c_info_out = &s5k3l6_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;
    exp_time = info->exp.exp_time;

    s5k3l6_drv_calc_exposure(handle, exposure_line, dummy_line, mode, exp_time,
                             &s5k3l6_aec_info);
    s5k3l6_drv_calc_gain(handle, info->gain, &s5k3l6_aec_info);

    return ret_value;
}

static cmr_int s5k3l6_drv_set_master_FrameSync(cmr_handle handle,
                                               cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int s5k3l6_drv_set_Slave_FrameSync(cmr_handle handle,
                                              cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int s5k3l6_drv_stream_on(cmr_handle handle, cmr_uint param) {
...
static cmr_int
s5k3l6_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    s5k3l6_drv_write_frame_length(
        sns_drv_cxt, &s5k3l6_aec_info,
        sns_drv_cxt->sensor_ev_info.preview_framelength);
    s5k3l6_drv_write_gain(sns_drv_cxt, &s5k3l6_aec_info,
                          sns_drv_cxt->sensor_ev_info.preview_gain);
    s5k3l6_drv_write_shutter(sns_drv_cxt, &s5k3l6_aec_info,
                             sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_s5k3l6_module_info_tab),
                                    s_s5k3l6_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt,
                                        ARRAY_SIZE(s_s5k3l6_resolution_tab_raw),
                                        s_s5k3l6_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_s5k3l6_resolution_trim_tab),
                                  s_s5k3l6_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_s5k3l6_static_info), s_s5k3l6_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_s5k3l6_mode_fps_info),
                                 s_s5k3l6_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    s5k3l6_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int s5k3l6_drv_handle_delete(cmr_handle handle, void *param) {
...
static cmr_int s5k3l6_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = s5k3l6_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_s5k3l6_ops_tab = {
    .create_handle = s5k3l6_drv_handle_create,
    .delete_handle = s5k3l6_drv_handle_delete,
    /*get privage data*/
    .get_data = s5k3l6_drv_get_private_data,
    /*common interface*/
    .power = s5k3l6_drv_power_on,
    ...
    .ext_ops = {
            ...
            [SENSOR_IOCTL_STREAM_ON].ops = s5k3l6_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = s5k3l6_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
            */
            [SENSOR_IOCTL_ACCESS_VAL].ops = s5k3l6_drv_access_val,
    }};
