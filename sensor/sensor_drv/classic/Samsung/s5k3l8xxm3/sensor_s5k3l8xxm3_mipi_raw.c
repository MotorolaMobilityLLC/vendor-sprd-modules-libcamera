 ...
 *
 *============================================================================*/
static uint32_t s5k3l8xxm3_get_default_frame_length(SENSOR_HW_HANDLE handle,
                                                    uint32_t mode) {
...
static void s5k3l8xxm3_write_gain(SENSOR_HW_HANDLE handle, float gain) {
    ...
    SENSOR_LOGI("_s5k3l8xxm3: real_gain:0x%x, param: 0x%x", (uint32_t)real_gain,
    param);
*/
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void s5k3l8xxm3_write_frame_length(SENSOR_HW_HANDLE handle,
                                          uint32_t frame_len) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void s5k3l8xxm3_write_shutter(SENSOR_HW_HANDLE handle,
                                     uint32_t shutter) {
 ...
 *============================================================================*/
static uint16_t s5k3l8xxm3_write_exposure_dummy(SENSOR_HW_HANDLE handle,
                                                uint32_t shutter,
                                                uint32_t dummy_line,
                                                uint16_t size_index) {
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t s5k3l8xxm3_power_on(SENSOR_HW_HANDLE handle,
                                    uint32_t power_on) {
...
static uint32_t s5k3l8xxm3_init_mode_fps_info(SENSOR_HW_HANDLE handle) {
    ...
    if (!s_s5k3l8xxm3_mode_fps_info.is_init) {
        ...
        for (i = 0; i < NUMBER_OF_ARRAY(s_s5k3l8xxm3_resolution_trim_tab);
             i++) {
            // max fps should be multiple of 30,it calulated from line_time and
            // frame_line
            tempfps = s_s5k3l8xxm3_resolution_trim_tab[i].line_time *
            ...
            if (0 != tempfps) {
                ...
                if (s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[i].max_fps >
                    ...
                    s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[i].is_high_fps =
                        1;
                    s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[i]
                        .high_fps_skip_num =
                        s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[i].max_fps /
                        30;
                }
...
static uint32_t s5k3l8xxm3_get_pdaf_info(SENSOR_HW_HANDLE handle,
                                         uint32_t *param) {
    uint32_t rtn = SENSOR_SUCCESS;
...
static uint32_t s5k3l8xxm3_get_static_info(SENSOR_HW_HANDLE handle,
                                           uint32_t *param) {
    uint32_t rtn = SENSOR_SUCCESS;
    ...
    ex_info->adgain_valid_frame_num =
        s_s5k3l8xxm3_static_info.adgain_valid_frame_num;
    ...
    ex_info->sensor_version_info =
        g_s5k3l8xxm3_mipi_raw_info.sensor_version_info;
...
static uint32_t s5k3l8xxm3_get_fps_info(SENSOR_HW_HANDLE handle,
                                        uint32_t *param) {
    uint32_t rtn = SENSOR_SUCCESS;
    ...
    fps_info->max_fps =
        s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[sensor_mode].max_fps;
    fps_info->min_fps =
        s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[sensor_mode].min_fps;
    fps_info->is_high_fps =
        s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[sensor_mode].is_high_fps;
    fps_info->high_fps_skip_num =
        s_s5k3l8xxm3_mode_fps_info.sensor_mode_fps[sensor_mode]
            .high_fps_skip_num;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static unsigned long s5k3l8xxm3_access_val(SENSOR_HW_HANDLE handle,
                                           unsigned long param) {
 ...
 *
 *============================================================================*/
static unsigned long s5k3l8xxm3_get_resolution_trim_tab(SENSOR_HW_HANDLE handle,
                                                        uint32_t param) {
 ...
 *============================================================================*/
static uint32_t s5k3l8xxm3_before_snapshot(SENSOR_HW_HANDLE handle,
                                           uint32_t param) {
    ...
    uint32_t prv_linetime =
        s_s5k3l8xxm3_resolution_trim_tab[preview_mode].line_time;
    uint32_t cap_linetime =
        s_s5k3l8xxm3_resolution_trim_tab[capture_mode].line_time;

    s_current_default_frame_length =
        s5k3l8xxm3_get_default_frame_length(handle, capture_mode);
    ...
    prv_shutter =
        s_sensor_ev_info.preview_shutter; // s5k3l8xxm3_read_shutter();
 ...
 *============================================================================*/
static unsigned long s5k3l8xxm3_write_exposure(SENSOR_HW_HANDLE handle,
                                               unsigned long param) {
    uint32_t ret_value = SENSOR_SUCCESS;
    ...
    SENSOR_PRINT("size_index=%d, exposure_line = %d, dummy_line=%d", size_index,
                 exposure_line, dummy_line);
    s_current_default_frame_length =
        s5k3l8xxm3_get_default_frame_length(handle, size_index);
    s_current_default_line_time =
        s_s5k3l8xxm3_resolution_trim_tab[size_index].line_time;

    ret_value = s5k3l8xxm3_write_exposure_dummy(handle, exposure_line,
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t s5k3l8xxm3_write_gain_value(SENSOR_HW_HANDLE handle,
                                            unsigned long param) {
    uint32_t ret_value = SENSOR_SUCCESS;
    float real_gain = 0;
...
static unsigned long s5k3l8xxm3_read_aec_info(SENSOR_HW_HANDLE handle,
                                              unsigned long param) {
    ...
    frame_interval =
        (uint16_t)(((exposure_line + dummy_line) *
                    (s_s5k3l8xxm3_resolution_trim_tab[mode].line_time)) /
                   1000000);
    SENSOR_PRINT(
    ...
    s_current_default_frame_length =
        s5k3l8xxm3_get_default_frame_length(handle, mode);
    s_current_default_line_time =
        s_s5k3l8xxm3_resolution_trim_tab[mode].line_time;

    s_sensor_ev_info.preview_shutter = s5k3l8xxm3_calc_exposure(
...
static unsigned long _s5k3l8xxm3_SetMaster_FrameSync(SENSOR_HW_HANDLE handle,
                                                     unsigned long param) {
