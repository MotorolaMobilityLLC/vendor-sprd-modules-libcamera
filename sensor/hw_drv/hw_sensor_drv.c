...
static cmr_int _hw_sensor_set_avdd(cmr_handle hw_handle,
                         cmr_u32 vdd_value) {
    cmr_int ret = HW_SUCCESS;
    CHECK_HANDLE(hw_handle);
    struct hw_drv_cxt *hw_drv_cxt = (struct hw_drv_cxt*)hw_handle;
...
static cmr_int _hw_sensor_set_dvdd(cmr_handle hw_handle,
                         cmr_u32 vdd_value) {
    cmr_int ret = HW_SUCCESS;
    CHECK_HANDLE(hw_handle);
    struct hw_drv_cxt *hw_drv_cxt = (struct hw_drv_cxt*)hw_handle;
...
static cmr_int _hw_sensor_set_iovdd(cmr_handle hw_handle,
                          cmr_u32 vdd_value) {
    cmr_int ret = HW_SUCCESS;
    CHECK_HANDLE(hw_handle);
    struct hw_drv_cxt *hw_drv_cxt = (struct hw_drv_cxt*)hw_handle;
 ...
 **/
cmr_int hw_sensor_set_voltage(cmr_handle hw_handle,
...
cmr_int hw_sensor_reset(cmr_handle hw_handle, cmr_u32 level, cmr_u32 pulse_width) {
    cmr_int ret = HW_SUCCESS;
    cmr_u32 rst_val[2];
...
cmr_int hw_sensor_mipi_init(cmr_handle hw_handle,
    ...
    if (0 != ret) {
        HW_LOGE("failed, ret=%ld, lane=%d, bps=%d,fd=%d", ret, lane_num,
                                              bps,hw_drv_cxt->fd_sensor);
        ret = -1;
    }
    // usleep(15*1000);
    ATRACE_END();
...
cmr_int hw_Sensor_SendRegTabToSensor(cmr_handle hw_handle,
