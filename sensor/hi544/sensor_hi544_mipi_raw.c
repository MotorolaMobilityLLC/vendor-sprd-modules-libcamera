...
#include "sensor_hi544_raw_param_v3.c"
#else
#include "sensor_hi544_raw_param_v2.c"
#endif
...
LOCAL unsigned long _hi544_access_val(unsigned long param);
LOCAL uint32_t _hi544_write_otp_gain(uint32_t *param);
LOCAL uint32_t _hi544_read_otp_gain(uint32_t *param);
...
LOCAL unsigned long _hi544_PowerOn(unsigned long power_on)
{
	...
	SENSOR_LOGI("SENSOR_hi544: _hi544_Power_On(1:on, 0:off): %d", power_on);
...
LOCAL uint32_t _hi544_write_otp(unsigned long param)
{
	uint32_t rtn=SENSOR_SUCCESS;
	...
	SENSOR_LOGI("SENSOR_hi544: _hi544_write_otp E");
	...
	for (i=0; i<len; i++) // write mode
	{
		cmd_val[0] = 0x01;
		cmd_val[1] = 0x0A;
		cmd_val[2] = ((start_addr+i) >>8)&0xff;
		Sensor_WriteI2C(hi544_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
		cmd_val[0] = 0x01;
		cmd_val[1] = 0x0B;
		cmd_val[2] = (start_addr+i)&0xff;
		Sensor_WriteI2C(hi544_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
		cmd_val[0] = 0x01;
		cmd_val[1] = 0x02;
		cmd_val[2] = 0x02;
	...
	SENSOR_LOGI("SENSOR_hi544: _hi544_write_otp X");

	return rtn;
}

LOCAL uint32_t _hi544_read_otp(unsigned long param)
{
	uint32_t rtn=SENSOR_SUCCESS;
	SENSOR_OTP_PARAM_T* param_ptr = (SENSOR_OTP_PARAM_T*)param;
	uint32_t start_addr = param_ptr->start_addr;
	...
	SENSOR_LOGI("SENSOR_hi544: _hi544_read_otp E");
	...
	SENSOR_LOGI("SENSOR_hi544: _hi544_read_otp X");

	return rtn;

}

LOCAL uint32_t _hi544_write_vcm(uint32_t *param)
{
	uint32_t ret_value = SENSOR_SUCCESS;
		...
		SENSOR_LOGI("SENSOR_hi544: _write_vcm, ret =  %d, MSL:%x, LSL:%x\n", ret_value, cmd_val[0], cmd_val[1]);
...
LOCAL uint32_t _hi544_read_vcm(uint32_t *param)
{
	uint32_t ret_value = SENSOR_SUCCESS;
		...
		SENSOR_LOGI("SENSOR_hi544: _read_vcm, ret =  %d, MSL:%x, LSL:%x\n", ret_value, cmd_val[0], cmd_val[1]);
...
LOCAL unsigned long _hi544_access_val(unsigned long param)
{
	...
	SENSOR_LOGI("SENSOR_hi544: _hi544_access_val E");
	if(!param_ptr){
		return rtn;
	}

	SENSOR_LOGI("SENSOR_hi544: param_ptr->type=%x",param_ptr->type);
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_SHUTTER:
			*((uint32_t*)param_ptr->pval) = _hi544_get_shutter();
			break;
		case SENSOR_VAL_TYPE_READ_VCM:
//			rtn = _hi544_read_vcm(param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_WRITE_VCM:
//			rtn = _hi544_write_vcm(param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_READ_OTP:
//			rtn = _hi544_read_otp((unsigned long)param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_WRITE_OTP:
//			rtn = _hi544_write_otp((unsigned long)param_ptr->pval);
			break;
...
//				*p=&calibration_info;
		...
		case SENSOR_VAL_TYPE_GET_AFPOSITION:
			*(uint32_t*)param_ptr->pval = cur_af_pos;
			break;
/*		case SENSOR_VAL_TYPE_WRITE_OTP_GAIN:
			rtn = _hi544_write_otp_gain(param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_READ_OTP_GAIN:
			rtn = _hi544_read_otp_gain(param_ptr->pval);
			break;*/
		default:
			break;
	}

	SENSOR_LOGI("SENSOR_hi544: _hi544_access_val X");
...
#endif
	SENSOR_LOGI("SENSOR_hi544: write_gain:0x%lx, real_gain:0x%x\n", param, real_gain);
...
LOCAL uint32_t _hi544_write_otp_gain(uint32_t *param)
{
	...
	SENSOR_LOGI("SENSOR_hi544: write_gain:0x%x\n", *param);

	ret_value = Sensor_WriteReg(0x003a, (*param&0xff)<<8);  /* code */
	ret_value = Sensor_WriteReg(0x0046, 0x00);
...
LOCAL unsigned long _hi544_write_af(unsigned long param)
{
	...
	SENSOR_LOGI("SENSOR_hi544: _write_af %ld", param);
...
LOCAL unsigned long _hi544_flash(unsigned long param)
{
	SENSOR_LOGI("SENSOR_hi544: param=%ld", param);
...
int _hi544_get_shutter(void)
{
	...
	SENSOR_LOGI("SENSOR_hi544: _hi544_get_shutter %d", shutter);
...
static uint32_t _hi544_SetEV(unsigned long param)
{
	uint32_t rtn = SENSOR_SUCCESS;
...
LOCAL uint32_t _hi544_read_otp_gain(uint32_t *param)
{
	...
	*param = value;
	SENSOR_LOGI("SENSOR_hi544: gain: 0x%x", value);
