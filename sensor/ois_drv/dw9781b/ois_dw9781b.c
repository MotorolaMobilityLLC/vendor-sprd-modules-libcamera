 ...
 * limitations under the License.
 ver: 1.0
 */
#define LOG_TAG "ois_dw9781b"
#include "ois_dw9781b.h"
...
static int dw9781b_drv_create(struct ois_drv_init_para *input_ptr,
...
struct sns_ois_drv_entry dw9781b_drv_entry = {
    .default_work_mode = 0,
    .ois_ops = {
            .create = dw9781b_drv_create,
            .delete = dw9781b_drv_delete,
            .ois_enforce = dw9781b_drv_enforce,
            .ois_calibrate = dw9781b_drv_calibrate,
        },
};

static int dw9781b_drv_calibrate(cmr_handle sns_ois_drv_handle) {
    uint32_t ret_value = OIS_SUCCESS;
    ret_value = _dw9781b_drv_reset(sns_ois_drv_handle);
    ret_value = _dw9781b_drv_gyro_offset_calibration(sns_ois_drv_handle);
    ret_value = _dw9781b_drv_save_cali_data(sns_ois_drv_handle);
    return ret_value;
}
...
static int dw9781b_drv_enforce(cmr_handle sns_ois_drv_handle) {
    ...
    case 2:
        /*Power Down */
        ret_value = hw_sensor_grc_write_i2c(ois_drv_cxt->hw_handle, DW9781B_VCM_SLAVE_ADDR,
        ...
        ret_value = hw_sensor_grc_write_i2c(ois_drv_cxt->hw_handle, DW9781B_VCM_SLAVE_ADDR,
                                            0xEBF1, 0x56FA, BITS_ADDR16_REG16);
        usleep(20 * 1000);
        pid_value = hw_sensor_grc_read_i2c(ois_drv_cxt->hw_handle, DW9781B_VCM_SLAVE_ADDR, 0x7000, BITS_ADDR16_REG16);
        ...
        if(atoi(value)) {
            CMR_LOGI("dw9781b OIS with 0x7105-> 0x%x, 0x7016-> 0x%x, 0x7049-> 0x%x, 0x704a-> 0x%x, 0x70fc-> 0x%x, 0x70fd-> 0x%x,",
                      hw_sensor_grc_read_i2c(ois_drv_cxt->hw_handle, DW9781B_VCM_SLAVE_ADDR, 0x7105, BITS_ADDR16_REG16),
                      hw_sensor_grc_read_i2c(ois_drv_cxt->hw_handle, DW9781B_VCM_SLAVE_ADDR, 0x7106, BITS_ADDR16_REG16),
                      hw_sensor_grc_read_i2c(ois_drv_cxt->hw_handle, DW9781B_VCM_SLAVE_ADDR, 0x704A, BITS_ADDR16_REG16),
