...
static cmr_int ultrawide_open(cmr_handle ipm_handle, struct ipm_open_in *in,
    ...
    if (in->frame_size.width * in->frame_size.height < 8000000) {
        img_warp_grid_config_default(&ultrawide_handle->warp_param);

        ultrawide_handle->warp_param.input_info.input_width =
            in->frame_size.width;
        ...
        ultrawide_handle->warp_param.input_info.crop_width =
            in->frame_rect.width;
        ultrawide_handle->warp_param.input_info.crop_height =
            in->frame_rect.height;
...
static cmr_int ultrawide_transfer_frame(cmr_handle class_handle,
    ...
    if (ultrawide_handle->warp_inst != NULL) {
        ...
        img_warp_grid_run(ultrawide_handle->warp_inst, &input, &output, NULL);
