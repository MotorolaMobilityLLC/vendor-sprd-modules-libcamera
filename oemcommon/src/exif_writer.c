...
/*
*@	Name :
*@	Description:	Get string size in which the terminating null
...
LOCAL BOOLEAN Jpeg_WriteIFD(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
    ...
    JPEG_WRITE_DATA(Jpeg_WriteL, context_ptr, ifd_info_ptr->count,
                    return FALSE);
...
LOCAL BOOLEAN Jpeg_WriteRationalIFD(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
                                    IFD_INFO_T *ifd_ptr, uint32 *ifd_offset_ptr,
    ...
    for (i = 0; i < ifd_ptr->count; i++) {
        ...
        JPEG_WRITE_DATA(Jpeg_WriteL, context_ptr, ptr->denominator,
                        return FALSE);
        ptr++;
    }
...
*@	Note:           return TRUE if successful else return FALSE
*/
LOCAL JPEG_RET_E Jpeg_WriteExifInteroperabilityInfo(
    JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
    EXIF_INTEROPERABILITY_INFO_T *inter_ptr, uint32 ifh_offset,
    uint32 begin_offset, uint32 *end_offset_ptr) {
    uint16 entries = 0;
    uint32 ifd_offset = 0;
    ...
    if (valid.InteroperabilityIndex
        ...
        if (!Jpeg_WriteByteIFD(context_ptr, &ifd_info, &ifd_offset,
                               &ifd_value_offset)) {
            return JPEG_FAILED;
        }
...
*@	Note:           return TRUE if successful else return FALSE
*/
LOCAL JPEG_RET_E Jpeg_WriteExifSpecInfo(
    JPEG_WRITE_STREAM_CONTEXT_T *context_ptr, EXIF_SPECIFIC_INFO_T *spec_ptr,
    EXIF_INTEROPERABILITY_INFO_T *inter_ptr, uint32 ifh_offset,
    uint32 begin_offset, uint32 *end_offset_ptr) {
    uint16 entries = 0;
    uint32 ifd_offset = 0;
    ...
    if (NULL != pic_taking_cond_ptr) {
        JPEG_PRINT_LOW("%x, %x \n", pic_taking_cond_ptr->FocalLength.numerator,
                       pic_taking_cond_ptr->ExposureProgram);
    }
#endif
    /***********************************************************************/
    // write IFD
    /***********************************************************************/
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ExposureTime) {
        ...
        if (!Jpeg_WriteRationalIFD(context_ptr, &ifd_info, &ifd_offset,
                                   &ifd_value_offset)) {
            return JPEG_FAILED;
        }
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ExposureProgram) {
        ...
        if (!Jpeg_WriteShortIFD(context_ptr, &ifd_info, &ifd_offset,
                                &ifd_value_offset)) {
            return JPEG_FAILED;
        }
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.SpectralSensitivity) {
        ifd_info.tag = IFD_SPECTRALSENSITIVITY;
        ifd_info.type = IFD_ASCII;
        ifd_info.count =
            _GetAsciiStringSize(pic_taking_cond_ptr->SpectralSensitivity) + 1;
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ISOSpeedRatings
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ShutterSpeedValue) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ApertureValue) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.BrightnessValue) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ExposureBiasValue) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.MaxApertureValue) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.SubjectDistance) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.MeteringMode) {
        // size of any
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.LightSource) {
        // size of any
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.FocalLength) {
    ...
    if (PNULL != date_time_ptr && date_time_ptr->valid.SubSecTimeOriginal) {
        ...
        ifd_info.type = IFD_ASCII;
        ifd_info.count =
            _GetAsciiStringSize(date_time_ptr->SubSecTimeOriginal) + 1;
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
    ...
    if (PNULL != date_time_ptr && date_time_ptr->valid.SubSecTimeDigitized) {
        ...
        ifd_info.type = IFD_ASCII;
        ifd_info.count =
            _GetAsciiStringSize(date_time_ptr->SubSecTimeDigitized) + 1;
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
...
#ifdef EXIF_DEBUG
        JPEG_PRINT_LOW("baisc_ptr->PixelXDimension = %d.",
                       baisc_ptr->PixelXDimension);
#endif
        if (!Jpeg_WriteLongIFD(context_ptr, &ifd_info, &ifd_offset,
                               &ifd_value_offset)) {
            return JPEG_FAILED;
        }
    ...
    if (PNULL != inter_ptr) {
        ...
        ret = Jpeg_WriteExifInteroperabilityInfo(context_ptr, inter_ptr,
                                                 ifh_offset, ifd_value_offset,
                                                 &ifd_value_offset);
        if (JPEG_SUCCESS != ret) {
            return ret;
        }
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.FlashEnergy) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.SpatialFrequencyResponse
        ...
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
        ifd_info.value_ptr =
            (void *)pic_taking_cond_ptr->SpatialFrequencyResponse.ptr;
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.FocalPlaneXResolution) {
        ifd_info.tag = IFD_FOCALPLANEXRESOLUTION;
        ifd_info.type = IFD_RATIONAL;
        ifd_info.count = 1;
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
        ifd_info.value_ptr =
            (void *)&pic_taking_cond_ptr->FocalPlaneXResolution;
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.FocalPlaneYResolution) {
        ifd_info.tag = IFD_FOCALPLANEYRESOLUTION;
        ifd_info.type = IFD_RATIONAL;
        ifd_info.count = 1;
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
        ifd_info.value_ptr =
            (void *)&pic_taking_cond_ptr->FocalPlaneYResolution;
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.FocalPlaneResolutionUnit) {
        ifd_info.tag = IFD_FOCALPLANERESOLUTIONUNIT;
        ifd_info.type = IFD_SHORT;
        ifd_info.count = 1;
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
        ifd_info.value_ptr =
            (void *)&pic_taking_cond_ptr->FocalPlaneResolutionUnit;
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.SubjectLocation) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ExposureIndex) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.SensingMethod) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.CustomRendered) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.ExposureMode) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.WhiteBalance) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.DigitalZoomRatio) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.FocalLengthIn35mmFilm) {
        ifd_info.tag = IFD_FOCALLENGTHIN35MMFILM;
        ifd_info.type = IFD_SHORT;
        ifd_info.count = 1;
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
        ifd_info.value_ptr =
            (void *)&pic_taking_cond_ptr->FocalLengthIn35mmFilm;
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.SceneCaptureType) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.GainControl) {
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.DeviceSettingDescription
        ...
        ifd_info.value_offset.long_value = ifd_value_offset - ifh_offset;
        ifd_info.value_ptr =
            (void *)pic_taking_cond_ptr->DeviceSettingDescription.ptr;
    ...
    if (PNULL != pic_taking_cond_ptr &&
        pic_taking_cond_ptr->valid.SubjectDistanceRange) {
...
LOCAL JPEG_RET_E Jpeg_WriteExifGPSInfo(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
                                       ...
                                       uint32 ifh_offset, uint32 begin_offset,
...
*****************************************************************************/
LOCAL JPEG_RET_E Jpeg_WriteExifIFD(
    JPEG_WRITE_STREAM_CONTEXT_T *context_ptr, EXIF_PRIMARY_INFO_T *primary_ptr,
    EXIF_SPECIFIC_INFO_T *spec_ptr, EXIF_GPS_INFO_T *gps_ptr,
    EXIF_INTEROPERABILITY_INFO_T *inter_ptr,
    EXIF_THUMBNAIL_INFO_T *thumbnail_ptr, BOOLEAN is_next_ifd_exist,
    uint32 ifh_offset, uint32 begin_offset, uint32 *end_offset_ptr) {
    uint16 entries = 0;
    uint32 ifd_offset = 0;
    uint32 ifd_value_offset = 0;
    IFD_INFO_T ifd_info; //            = {0};
...

    {
        ...
        ifd_info.count = 1;
        ifd_info.value_offset.long_value = 0;
        ifd_info.value_offset.short_value[0] =
            primary_ptr->basic.ResolutionUnit;
    ...
    if (PNULL != primary_ptr->data_char_ptr &&
        data_char_ptr->valid.WhitePoint) /*lint !e613*/
    {
    ...
    if (PNULL != spec_ptr) {
        ...
        ret =
            Jpeg_WriteExifSpecInfo(context_ptr, spec_ptr, inter_ptr, ifh_offset,
    ...
    if (PNULL != thumbnail_ptr && PNULL != thumbnail_ptr->stream_buf_ptr &&
        ...
        if (!Jpeg_WriteLongIFD(context_ptr, &ifd_info, &thumbnail_ifd_offset,
                               &ifd_value_offset)) {
            return JPEG_FAILED;
        }
    }
    ...
    if (is_next_ifd_exist) {
        JPEG_WRITE_DATA(Jpeg_WriteL, context_ptr, ifd_value_offset - ifh_offset,
                        return JPEG_FAILED);
    } else {
        JPEG_WRITE_DATA(Jpeg_WriteL, context_ptr, 0, return JPEG_FAILED);
    }
...
LOCAL JPEG_RET_E Jpeg_WriteIFH(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
    ...
    JPEG_WRITE_DATA(Jpeg_WriteW, context_ptr, EXIF_BIG_ENDIAN,
                    return JPEG_FAILED);
    JPEG_WRITE_DATA(Jpeg_WriteW, context_ptr, EXIF_IFH_FIX_VALUE,
                    return JPEG_FAILED);
...
**                  end_offset_ptr:   pointer of end position when writing done,
**                                    output parameter
**                  app1_length:      length of the whole app1 exclued the APP1
*marker
**	Note:           return JPEG_SUCESS if successful
*****************************************************************************/
LOCAL JPEG_RET_E Jpeg_WriteAPP1Header(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
                                      uint32 begin_offset,
                                      uint32 *end_offset_ptr,
                                      uint16 app1_length) {
    uint32 i = 0;
    ...
    for (i = 0; i < 6; i++) {
        JPEG_WRITE_DATA(Jpeg_WriteC, context_ptr, exif_id[i],
                        return JPEG_FAILED);
    }
...
**	Note:           return JPEG_SUCESS if successful
*****************************************************************************/
PUBLIC JPEG_RET_E Jpeg_WriteAPP1(uint8 *target_buf, uint32 target_buf_size,
                                 JINF_EXIF_INFO_T *exif_info_ptr,
                                 uint8 *thumbnail_buf_ptr,
                                 uint32 thumbnail_size, uint32 *app1_size_ptr) {
    ...
    JPEG_PRINT_LOW("[Jpeg_WriteAPP1] start 1, %x, %x, %x, %x, %x \n",
                   (uint32)target_buf, target_buf_size, (uint32)exif_info_ptr,
                   (uint32)thumbnail_buf_ptr, thumbnail_size);
#endif
    ...
    ret = Jpeg_WriteExifIFD(&context, primary_ptr, spec_ptr, gps_ptr, inter_ptr,
                            thumbnail_ptr, is_ifd1_exist, ifh_offset,
                            begin_offset, &end_offset);
    ...
    if (is_ifd1_exist) {
        ...
        ret = Jpeg_WriteExifIFD(&context, primary_ptr, spec_ptr, gps_ptr,
                                inter_ptr, thumbnail_ptr, is_next_ifd_exist,
                                ifh_offset, begin_offset, &end_offset);
    ...
    *app1_size_ptr = app1_length + 2;
    ...
    begin_offset = 0;
    ret =
        Jpeg_WriteAPP1Header(&context, begin_offset, &end_offset, app1_length);
    if (JPEG_SUCCESS != ret) {
...
**	Note:           return JPEG_SUCESS if successful
*****************************************************************************/
LOCAL JPEG_RET_E Jpeg_Write_APP3_ExifIFD(
    JPEG_WRITE_STREAM_CONTEXT_T *context_ptr, EXIF_ISP_INFO_T *exif_isp_info,
    BOOLEAN is_next_ifd_exist, uint32 ifh_offset, uint32 begin_offset,
    uint32 *end_offset_ptr) {
    uint16 entries = 0;
    uint32 ifd_offset = 0;
    uint32 ifd_value_offset = 0;
...
**                  end_offset_ptr:   pointer of end position when writing done,
**                                    output parameter
**                  app3_length:      length of the whole app3 exclued the APP3
*marker
**	Note:           return JPEG_SUCESS if successful
*****************************************************************************/
LOCAL JPEG_RET_E Jpeg_WriteAPP3Header(JPEG_WRITE_STREAM_CONTEXT_T *context_ptr,
                                      uint32 begin_offset,
                                      uint32 *end_offset_ptr,
                                      uint16 app3_length) {
    uint32 i = 0;
    ...
    JPEG_PRINT_LOW(
        "Jpeg_WriteAPP3Header :E!!!begin_offset = %d, app3_length = %d\n",
...
**	Note:           return JPEG_SUCESS if successful
*****************************************************************************/
PUBLIC JPEG_RET_E Jpeg_WriteAPP3(uint8 *target_buf, uint32 target_buf_size,
    ...
    ret = Jpeg_Write_APP3_ExifIFD(&context, exif_isp_info, is_ifd1_exist,
                                  ifh_offset, begin_offset, &end_offset);
    if (JPEG_SUCCESS != ret) {
    ...
    *app3_size_ptr = app3_length;
    ...
    begin_offset = 0;
    ret =
        Jpeg_WriteAPP3Header(&context, begin_offset, &end_offset, app3_length);
    if (JPEG_SUCCESS != ret) {
...
PUBLIC JPEG_RET_E Jpeg_WriteAPP4(uint8 *target_buf, uint32 target_buf_size,
...
/*****************************************************************************
**	Name :
**	Description:	and the EXIF info and write the output jpeg to the
*memory
**	Author:			Shan.he
**  Parameters:
...
LOCAL JPEG_RET_E JPEG_AddExifToMemory(JINF_WEXIF_IN_PARAM_T *in_param_ptr,
    ...
    ret =
        Jpeg_WriteAPP1(app1_buf_ptr, app1_buf_size, in_param_ptr->exif_info_ptr,
                       in_param_ptr->thumbnail_buf_ptr,
                       in_param_ptr->thumbnail_buf_size, &app1_size);
    if (JPEG_SUCCESS != ret) {
    ...
    if (NULL != in_param_ptr->exif_isp_info && NULL != app3_buf_ptr &&
        app3_buf_size > 0) {
        ret = Jpeg_WriteAPP3(app3_buf_ptr, app3_buf_size,
                             in_param_ptr->exif_isp_info, &app3_size);
        if (JPEG_SUCCESS == ret) {
            free_buf_size = (unsigned long)in_param_ptr->src_jpeg_buf_ptr -
                            (unsigned long)in_param_ptr->target_buf_ptr -
                            app1_size;
    ...
    if (NULL != in_param_ptr->exif_isp_debug_info.addr &&
        NULL != app4_buf_ptr && app4_buf_size > 0) {
        ret = Jpeg_WriteAPP4(app4_buf_ptr, app4_buf_size,
                             in_param_ptr->exif_isp_debug_info, &app4_size);
        if (JPEG_SUCCESS == ret) {
            free_buf_size = (unsigned long)in_param_ptr->src_jpeg_buf_ptr -
                            (unsigned long)in_param_ptr->target_buf_ptr -
                            app1_size - app3_size;
    ...
    target_buf_ptr =
        in_param_ptr->src_jpeg_buf_ptr - app1_size - app3_size - app4_size;
    out_param_ptr->output_buf_ptr = target_buf_ptr;
    out_param_ptr->output_size =
        app1_size + app3_size + app4_size + in_param_ptr->src_jpeg_size;
...
LOCAL JPEG_RET_E JPEG_AddExifToFile(JINF_WEXIF_IN_PARAM_T *in_param_ptr) {
    ...
    if (JPEG_SUCCESS == ret) {
        if (app3_size > 0) {
            ...
            if (!in_param_ptr->wrtie_file_func(
                    write_ptr, write_offset, write_size, &actual_write_size) ||
                write_size != actual_write_size) {
                JPEG_PRINT_LOW(
                    "[JPEG_AddExifToFile] write src jpeg failed, write_size= %d,\
								actual_write_size = %d",
                    write_size, actual_write_size);
                return JPEG_FAILED;
            }
    ...
    write_ptr =
        in_param_ptr->src_jpeg_buf_ptr + JPEG_HEADER_BYTE; // exclude SOI marker
    if (!in_param_ptr->wrtie_file_func(write_ptr, write_offset, write_size,
                                       &actual_write_size) ||
        write_size != actual_write_size) {
        JPEG_PRINT_LOW(
...
JINF_RET_E IMGJPEG_WriteExif(JINF_WEXIF_IN_PARAM_T *in_param_ptr,
    ...
    if ((NULL == in_param_ptr->exif_info_ptr) &&
        (NULL == in_param_ptr->src_jpeg_buf_ptr) &&
        (NULL == in_param_ptr->temp_buf_ptr)) {
    ...
    JPEG_PRINT_LOW("src buf = 0x%lx, size = %d, thumb  buf = 0x%lx, size = %d,\
			temp buf = 0x%lx, size = %d, write file = 0x%lx",
                   (unsigned long)in_param_ptr->src_jpeg_buf_ptr,
                   in_param_ptr->src_jpeg_size,
                   (unsigned long)in_param_ptr->thumbnail_buf_ptr,
                   in_param_ptr->thumbnail_buf_size,
                   (unsigned long)in_param_ptr->temp_buf_ptr,
                   in_param_ptr->temp_buf_size,
    ...
    if (PNULL != in_param_ptr->wrtie_file_func) {
        ...
        if ((unsigned long)in_param_ptr->src_jpeg_buf_ptr <=
            (unsigned long)in_param_ptr->target_buf_ptr) {
            JPEG_PRINT_LOW("[IMGJPEG_WriteExif] src jpeg buffer address must "
                           "be bigger than thumbnail jpeg buffer adder");
...
**					date_time_ptr: 	pointer of the date
*time string buffer
**					date_time_size:	size of the data time
*string buffer, should be at least 20 bytes
**	Note:           return JINF_SUCCESS if successful
*****************************************************************************/
JINF_RET_E IMGJPEG_FormatDateTime(JINF_DATE_T *date_ptr, JINF_TIME_T *time_ptr,
                                  EXIF_ASCII_T *date_time_ptr,
                                  uint32 date_time_size) {
    if (date_time_size < 20) {
        return JINF_MEMORY_NOT_ENOUGH;
    }

    sprintf((char *)date_time_ptr, "%04d:%02d:%02d %02d:%02d:%02d",
            date_ptr->year, date_ptr->month, date_ptr->day, time_ptr->hour,
            time_ptr->minute, time_ptr->second);
