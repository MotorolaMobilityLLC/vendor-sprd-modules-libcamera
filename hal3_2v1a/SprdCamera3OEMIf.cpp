...
SprdCamera3OEMIf::SprdCamera3OEMIf(int cameraId, SprdCamera3Setting *setting)
    : mSetCapRatioFlag(false), mVideoCopyFromPreviewFlag(false),
      ...
      mPreviewDcamAllocBufferCnt(0), mPreviewFrameNum(0), mRecordFrameNum(0),
      mIsRecording(false), mPreAllocCapMemInited(0), mIsPreAllocCapMemDone(0),
...
int SprdCamera3OEMIf::rawPostProc(buffer_handle_t *raw_buff,
    ...
    ret = mHalOem->ops->camera_raw_post_proc(mCameraHandle, &raw_frame,
                                             &yuv_frame, sbs_info);
...
void SprdCamera3OEMIf::receiveJpegPicture(struct camera_frame_type *frame) {
    ...
    if (mSprdZslEnabled == 0) {
        setCamPreformaceScene(CAM_CAPTURE_E_LEVEL_NL);
    } else {
