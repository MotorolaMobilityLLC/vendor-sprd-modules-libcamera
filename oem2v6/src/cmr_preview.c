...
#define ZSL_ULTRA_WIDE_ALLOC_CNT 5
...
static cmr_int camera_get_mem_size(cmr_handle preview_handle, cmr_u32 camera_id);
...
cmr_int cmr_preview_deinit(cmr_handle preview_handle) {
    ...
    for (i = 0; i < CAMERA_ID_MAX; i++) {
        ...
        if (handle->prev_cxt[i].prev_param.is_ultra_wide && cxt->sn_cxt.warp_otp_data.otp_ptr) {
            CMR_LOGD("free ultra opt data %p\n", cxt->sn_cxt.warp_otp_data.otp_ptr);
            free(cxt->sn_cxt.warp_otp_data.otp_ptr);
...
cmr_int cmr_preview_get_fdr_free_buffer(cmr_handle preview_handle, cmr_u32 camera_id,
                                 cmr_u32 is_rgb, struct buffer_cfg *free_buf_cfg) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    struct prev_handle *handle = (struct prev_handle *)preview_handle;
    struct prev_context *prev_cxt = NULL;
    struct buffer_cfg *used_buf_cfg = NULL;
    cmr_u32 i = 0, j = 0, k = 0, mem_num, mem_size;
    cmr_u32 fd = 0;
    cmr_s32 *fd_array;
    cmr_uint *phys_addr_array, *virt_addr_array;
    ...
    if (is_rgb == 0) {
        mem_num = prev_cxt->cap_fdr_mem_num;
        mem_size = (cmr_u32)prev_cxt->cap_fdr_mem_size;
        fd_array = prev_cxt->cap_fdr_fd_array;
        phys_addr_array = prev_cxt->cap_fdr_phys_addr_array;
        virt_addr_array = prev_cxt->cap_fdr_virt_addr_array;
    } else {
        mem_num = prev_cxt->cap_rgb_mem_num;
        mem_size = (cmr_u32)prev_cxt->cap_rgb_mem_size;
        fd_array = prev_cxt->cap_rgb_fd_array;
        phys_addr_array = prev_cxt->cap_rgb_phys_addr_array;
        virt_addr_array = prev_cxt->cap_rgb_virt_addr_array;
    }

    CMR_LOGD("E cap_fdr_mem_num:%d, is_rgb %d\n", mem_num, is_rgb);
    for(i = 0; i < mem_num; i++) {
        fd = fd_array[i];
        ...
        if (j >= used_buf_cfg->count) {
            ...
            free_buf_cfg->count++;
            free_buf_cfg->length = mem_size;
            free_buf_cfg->is_fdr = 1;
            free_buf_cfg->addr[k].addr_y = phys_addr_array[i];
            free_buf_cfg->addr_vir[k].addr_y = virt_addr_array[i];
            free_buf_cfg->fd[k] = fd_array[i];
...
cmr_int cmr_preview_set_fdr_used_buffer(cmr_handle preview_handle, cmr_u32 camera_id,
    ...
    cmr_u32 i = 0, j = 0, mem_num, mem_size;
    cmr_s32 *fd_array;
    cmr_uint *phys_addr_array, *virt_addr_array;
    ...
    for(i=0; i < mem_num; i++) {
        if (fd == fd_array[i]) {
            CMR_LOGD("get correct fd %d  index %d", fd, i);
            break;
        }
    }

    if (i >= mem_num) {
        CMR_LOGE("failed to get valid slot for fd %d, is_rgb %d\n", fd, is_rgb);
        return ret;
    }
    ...
    j = buf_cfg->count;
    buf_cfg->count++;
    buf_cfg->length = mem_size;
    buf_cfg->is_fdr = 1;
    buf_cfg->addr[j].addr_y = phys_addr_array[i];
    buf_cfg->addr_vir[j].addr_y = virt_addr_array[i];
    buf_cfg->fd[j] = fd_array[i];
...
cmr_int prev_alloc_cap_buf(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    total_mem_size = camera_get_mem_size(handle, camera_id);
    if (total_mem_size == 0) {
        CMR_LOGE("get total mem size fail!");
        return CMR_CAMERA_NO_MEM;
    }
    sum = 1;
    ...
    for (i = 0; i < CMR_CAPTURE_MEM_SUM; i++) {
        ...
        ret = camera_arrange_capture_buf(
                ...
                ret, is_need_scaling, 1, buf_4in1_flag);
    }
...
cmr_int prev_free_zsl_raw_buf(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (!is_restart) {
        ...
        if (prev_cxt->cap_rgb_mem_num) {
	        mem_ops->free_mem(CAMERA_SNAPSHOT_ZSL_RGB, handle->oem_handle,
	                      prev_cxt->cap_rgb_phys_addr_array,
	                      prev_cxt->cap_rgb_virt_addr_array,
	                      prev_cxt->cap_rgb_fd_array,
	                      prev_cxt->cap_rgb_mem_num);
	        cmr_bzero(prev_cxt->cap_rgb_phys_addr_array,
	                 sizeof(prev_cxt->cap_rgb_phys_addr_array));
	        cmr_bzero(prev_cxt->cap_rgb_virt_addr_array,
	                  sizeof(prev_cxt->cap_rgb_phys_addr_array));
	        cmr_bzero(prev_cxt->cap_rgb_fd_array,
	                  sizeof(prev_cxt->cap_rgb_phys_addr_array));
	        prev_cxt->cap_rgb_mem_num = 0;
        }
    }
...
cmr_int prev_set_prev_param_lightly(struct prev_handle *handle,
    ...
    if (ZOOM_INFO != zoom_param->mode) {
        ...
        float aspect_ratio = 1.0 * prev_cxt->actual_prev_size.width /
                    prev_cxt->actual_prev_size.height;
        CMR_LOGD("preview aspect_ratio %f", aspect_ratio);
        if (zoom_param->zoom_info.crop_region.width > 0 && PLATFORM_ID == 0x0401) {
            chn_param.cap_inf_cfg.cfg.src_img_rect = camera_apply_rect_and_ratio(
                    zoom_param->zoom_info.pixel_size, zoom_param->zoom_info.crop_region,
                    chn_param.cap_inf_cfg.cfg.src_img_rect, aspect_ratio);
...
cmr_int prev_cap_ability(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (ZOOM_INFO != zoom_param->mode) {
        ...
        float aspect_ratio = 1.0 * cap_size->width / cap_size->height;
        CMR_LOGD("aspect_ratio %f, is_fov_fusion %d, app_ratio %f",
            aspect_ratio, cxt->is_fov_fusion, cxt->app_ratio);
        if (PLATFORM_ID == 0x0401) {
            if (zoom_param->zoom_info.crop_region.width == 0 ||
                (cxt->is_fov_fusion == 1 && cxt->app_ratio < 2.0)) {
                CMR_LOGD("camera_get_trim_rect2");
                ret = camera_get_trim_rect2(
                ...
                CMR_LOGD("camera_apply_rect_and_ratio");
                img_cap->src_img_rect = camera_apply_rect_and_ratio(
                            zoom_param->zoom_info.pixel_size, zoom_param->zoom_info.crop_region,
                            img_cap->src_img_rect, aspect_ratio);
            }
        } else {
            ret = camera_get_trim_rect2(
                        &img_cap->src_img_rect, zoom_param->zoom_info.capture_aspect_ratio,
...
cmr_int prev_fd_close(struct prev_handle *handle, cmr_u32 camera_id) {
    ...
    if (prev_cxt->fd_handle) {
        ret = handle->ops.isp_ioctl(
            handle->oem_handle, COM_ISP_SET_AI_SET_FD_ON_OFF, &isp_cmd_parm);
        ret = handle->ops.isp_ioctl(
            handle->oem_handle, COM_ISP_SET_FACE_AREA, &isp_cmd_parm);
...
cmr_int prev_fd_cb(cmr_u32 class_type, struct ipm_frame_out *cb_param) {
    ...
    for (i = 0; i < frame_type.face_num; i++) {
        ...
        frame_type.face_info[i].fd_cb_ptr.srx = cb_param->face_area.range[i].fd_ptr.srx;
        frame_type.face_info[i].fd_cb_ptr.sry = cb_param->face_area.range[i].fd_ptr.sry;
        frame_type.face_info[i].fd_cb_ptr.ex = cb_param->face_area.range[i].fd_ptr.ex;
...
static void load_ultrawide_otp(struct prev_handle *handle, cmr_u32 sensor_id) {
    cmr_u32 *spw_otp_info;
    cmr_u8 *otp_data = NULL;
    cmr_u32 buf_size;
    cmr_u32 spw_otp_size = 0;
    cmr_u32 read_byte = 0;
    char value1[PROPERTY_VALUE_MAX] = "0";
    char value2[PROPERTY_VALUE_MAX] = "0";
    char value3[PROPERTY_VALUE_MAX] = "0";
    cmr_handle oem_handle = handle->oem_handle;
    ...
    if (cxt->sn_cxt.warp_otp_data.otp_ptr != NULL) {
        CMR_LOGI("cam%d, warp otp data is inited: ptr %p, size %ld\n", sensor_id,
                cxt->sn_cxt.warp_otp_data.otp_ptr, cxt->sn_cxt.warp_otp_data.otp_size);
        return;
    }

    buf_size = 256 * sizeof(cmr_u32);
    spw_otp_info = (cmr_u32 *)malloc(buf_size);
    if (spw_otp_info == NULL) {
        CMR_LOGE("failed to get warp otp data memory\n");
        cxt->sn_cxt.warp_otp_data.otp_ptr = NULL;
        cxt->sn_cxt.warp_otp_data.otp_size = 0;
        return;
    }
    ...
    if (phyPtr->otp_version == 11 && phyPtr->module_vendor_id > 0 &&
        ...
        CMR_LOGD("only support otp 1.1, module_vendor_id %d,  module vendor is %s",
            phyPtr->module_vendor_id,
    ...
    if (spw_otp_size > 0) {
        cxt->sn_cxt.warp_otp_data.otp_ptr = spw_otp_info;
        cxt->sn_cxt.warp_otp_data.otp_size = spw_otp_size;
        CMR_LOGD("cam%d load ultrawide otp success, otp_size %d, ptr %p",
                sensor_id, spw_otp_size, spw_otp_info);
    } else {
        free(spw_otp_info);
        cxt->sn_cxt.warp_otp_data.otp_ptr = NULL;
        cxt->sn_cxt.warp_otp_data.otp_size = 0;
        CMR_LOGD("cam%d, load ultrawide otp failed, otp_size %d", sensor_id, spw_otp_size);
    }
}
...
cmr_int prev_ultra_wide_open(struct prev_handle *handle, cmr_u32 camera_id) {
    ...
    load_ultrawide_otp(handle, camera_id);
    sensor_info = &handle->prev_cxt[camera_id].sensor_info;
    ...
    in_param.otp_data = cxt->sn_cxt.warp_otp_data;
...
cmr_int camera_get_mem_size(cmr_handle preview_handle, cmr_u32 camera_id) {
    struct prev_handle *handle = (struct prev_handle *)preview_handle;
    struct prev_context *prev_cxt = NULL;
    cmr_int ret = CMR_CAMERA_SUCCESS;
    cmr_u32 is_raw_capture = 0;
    ...
    CMR_LOGD("cap mem size 0x%x", total_mem_size);
...
cmr_preview_realloc_buffer_for_fdr(cmr_handle preview_handle, cmr_u32 camera_id, cmr_u32 need_rgb) {
    ...
    struct camera_context *cxt = (struct camera_context *)handle->oem_handle;
    struct memory_param *mem_ops = NULL;
    int fdr_total_frame_num = FDR_FRM_ALLOC_CNT;
    ...
    if(fdr_total_frame_num <= FDR_FRM_ALLOC_CNT) {
        ...
        prev_cxt->cap_fdr_mem_num = FDR_FRM_ALLOC_CNT;
        CMR_LOGE("max fdr mem num %d is exceed 10", fdr_total_frame_num);
    }
    ...
    ret = mem_ops->alloc_mem(CAMERA_SNAPSHOT_ZSL_RAW, handle->oem_handle,
               ...
               (cmr_u32 *)&prev_cxt->cap_fdr_mem_num,
                ...
                prev_cxt->cap_fdr_fd_array);
    if (ret) {
        CMR_LOGE("fail to alloc fdr raw buffer\n");
        prev_cxt->cap_fdr_mem_num = 0;
        prev_cxt->cap_rgb_mem_num = 0;
        ret = CMR_CAMERA_FAIL;
        goto exit;
    ...
    ret = mem_ops->alloc_mem(CAMERA_SNAPSHOT_ZSL_RGB, handle->oem_handle,
               ...
               (cmr_u32 *)&prev_cxt->cap_rgb_mem_num,
                ...
                prev_cxt->cap_rgb_fd_array);
    if (ret) {
        CMR_LOGE("fail to alloc fdr rgb buffer\n");
        prev_cxt->cap_rgb_mem_num = 0;
        ret = CMR_CAMERA_FAIL;
        goto exit;
    }
    CMR_LOGD("cam%d, size %d,  num %d, rgb fd0 %d, fd1 %d\n", camera_id,
		prev_cxt->cap_rgb_mem_size, prev_cxt->cap_rgb_mem_num,
		prev_cxt->cap_rgb_fd_array[0], prev_cxt->cap_rgb_fd_array[1]);
