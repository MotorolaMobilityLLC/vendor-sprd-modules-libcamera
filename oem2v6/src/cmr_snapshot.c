...
static cmr_int snp_ips_req_proc(struct snp_context *cxt, struct frm_info *frame)
{
	...
	if (dump_all & 1) {
		...
		snprintf(fname, 256, "%sframe%03d_aaa.yuv", CAMERA_DUMP_PATH, frame->frame_real_id);
		fp = fopen(fname, "wb");
		if (fp != NULL) {
...
save_exif:
	...
	ips_param.isp_debug_info = snp_req->isp_debug_info;
	ips_param.isp_debug_info_size = snp_req->isp_debug_info_size;
...
static cmr_int snp_ips_req_thumb(struct snp_context *cxt, void *data)
{
	...
	mean.is_sync = 1;
	if (snp_pm->is_mfsr && frm_param)
		src.rect = frm_param->mfsr_param.frame_crop;
	...
	if (dump_all & 4) {
		...
		snprintf(fname, 256, "%sframe%03d_bbthumb0.yuv", CAMERA_DUMP_PATH, frame->frame_real_id);
		fp = fopen(fname, "wb");
		if (fp != NULL) {
			CMR_LOGI("gtang dump file %s, fp = %p, size %d\n", fname, fp, buf_size);
	...
	struct ips_request_t *req, cmr_u32 cb_type, void *cb_data)
{
	...
	case IPS_CB_JPEG_DONE:
		...
		encInfo->size = frm->buf_size;
		CMR_LOGD("req id %d, jpeg datap %p, size %d, time %04d.%06d s, monotime 0x%llx\n",
			req->request_id, encInfo->outPtr, encInfo->size, frm->sec, frm->usec, frm->monoboottime);
...
{
	cmr_int ret = CMR_CAMERA_SUCCESS;
	int iret, i, is_mfnr = 0, is_mfsr = 0;
	...
	if (cxt->req_param.is_hdr) {
		...
		is_mfnr = 1;
	} else if (cxt->req_param.is_mfsr) {
		new_req->proc_steps[i++].type = IPS_TYPE_MFSR;
		is_mfsr = 1;
	}
	...
	if (is_mfsr) {
		new_req->proc_steps[i++].type = IPS_TYPE_SHARP;
	}
	if (cxt->req_param.dre_flag) {
		new_req->proc_steps[i++].type = IPS_TYPE_DREPRO;
	}
	...
	if (ret) {
		CMR_LOGE("fail to init req id %d\n", new_req->request_id);
		cmr_ips_destroy_req(cxt->ips_handle, &new_req);
		return ret;
	}
...
static cmr_int snp_ips_stop_proc(struct snp_context *cxt)
{
	...
	for (i = 0; i < 32; i++) {
		...
		if (cxt->snap_req[i].isp_debug_info)
			free(cxt->snap_req[i].isp_debug_info);
		cxt->snap_req[i].isp_debug_info = NULL;
		cxt->snap_req[i].isp_debug_info_size = 0;
		CMR_LOGD("detory end\n");
	}
	cxt->cur_req = NULL;
	sem_post(&cxt->ips_sem);
...
cmr_int cmr_snapshot_deinit(cmr_handle snapshot_handle) {
    ...
    if (cxt->ips_handle) {
        ...
        if (ret)
            CMR_LOGE("fail to deinit cmr_ips\n");
