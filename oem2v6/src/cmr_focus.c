...
struct img_rect const_sensor_rect = {
    .start_x = 716, .start_y = 476, .width = 206, .height = 272,
};
struct af_context {
...
static cmr_int af_set_mode(cmr_handle af_handle, cmr_u32 came_id,
                           cmr_u32 af_mode);
...
static cmr_int af_stop(cmr_handle af_handle, cmr_u32 camera_id,
                       cmr_u32 is_need_abort_msg);
...
static cmr_int focus_rect_param_to_isp(SENSOR_EXT_FUN_PARAM_T focus_rect,
                                       struct isp_af_win *p_isp_af_param);
static cmr_int wait_isp_focus_result(cmr_handle af_handle, cmr_u32 camera_id,
                                     cmr_u32 is_need_abort_msg);
...
cmr_int cmr_focus_init(struct af_init_param *parm_ptr, cmr_u32 camera_id,
                       cmr_handle *af_handle) {
...
cmr_int cmr_focus_deinit_notice(cmr_handle af_handle) {
    ...
    sem_post(&af_cxt->isp_af_sem); // fastly quit af process,--for safty quit
                                   // post one times
...
cmr_int cmr_focus_stop(cmr_handle af_handle, cmr_u32 camera_id,
                       cmr_u32 is_need_abort_msg) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    struct af_context *af_cxt = (struct af_context *)af_handle;
    CMR_MSG_INIT(message);
    ...
    CMR_LOGD("is_need_abort_msg 0x%x", is_need_abort_msg);
    af_cxt->af_cancelled |= 0x01;
    af_cxt->af_cancelled |= (is_need_abort_msg & 0x1) << 1;
    CMR_LOGD("af_cancelled 0x%x", af_cxt->af_cancelled);
...
cmr_int af_thread_proc(struct cmr_msg *message, void *data) {
    ...
    case CMR_EVT_AF_START:
        ...
        if (!af_cxt->evt_cb || !af_cxt->ops.get_preview_status) {
            CMR_LOGE("invalid param, 0x%p, 0x%p", af_cxt->evt_cb,
                     af_cxt->ops.get_preview_status);
            break;
        }
...
cmr_int cmr_focus_isp_handle(cmr_handle af_handle, cmr_u32 evt_type,
                             cmr_u32 came_id, void *data) {
    UNUSED(came_id);
    ...
    CMR_LOGV("evt_type = 0x%x", evt_type);

    switch (evt_type) {
    case FOCUS_EVT_ISP_AF_NOTICE:
        CMR_LOGD("af_mode %d, af_busy %d", af_cxt->af_mode, af_cxt->af_busy);
...
cmr_int cmr_focus_sensor_handle(cmr_handle af_handle, cmr_u32 evt_type,
                                cmr_u32 camera_id, void *data) {
    UNUSED(data);
...
cmr_int cmr_focus_set_param(cmr_handle af_handle, cmr_u32 came_id,
                            enum camera_param_type id, void *param) {
    ...
    default:
        CMR_LOGD("unsupported parmater id= %d", id);
        break;
    }
...
cmr_int af_get_focusmove_flag(cmr_handle af_handle) {
    ...
    CMR_LOGD("move_done %ld", is_move_done);
...
cmr_int af_isp_done(cmr_handle af_handle, void *data) {
    ...
    pthread_mutex_lock(&af_cxt->af_isp_caf_mutex);
    CMR_LOGD("AF done, valid_win 0x%x, isp_af_timeout %d", isp_af->valid_win,
             af_cxt->isp_af_timeout);
...
/*
 *set caf mode to isp ;
 *if to sensor ,first  save af mode to settings,then  get and  set af mode in
 *af_start
 *
 */
cmr_int af_set_mode(cmr_handle af_handle, cmr_u32 came_id, cmr_u32 af_mode) {
    ...
    CMR_LOGD("af_mode,af_cxt->af_mode %d,%d", af_mode, af_cxt->af_mode);

    if (af_mode < CAMERA_FOCUS_MODE_MAX) {
        if (af_cxt->af_mode == af_mode) {
            CMR_LOGD("some mode, no need set again");
            goto exit;
        }
    ...
    ret =
        af_cxt->ops.get_sensor_info(af_cxt->oem_handle, came_id, &sensor_info);
    if (ret) {
    ...
    if (CAM_IMG_FMT_BAYER_MIPI_RAW == sensor_info.image_format) {
        ...
        ret = af_cxt->ops.af_isp_ioctrl(af_cxt->oem_handle, COM_ISP_SET_AF_MODE,
                                        &isp_cmd);
    } else {
...
cmr_int af_start(cmr_handle af_handle, cmr_u32 camera_id) {
    ...
    ret = af_cxt->ops.get_sensor_info(af_cxt->oem_handle, camera_id,
                                      &sensor_info);
    if (ret) {
        CMR_LOGE("failed to get sensor info, ret= %ld", ret);
        ret = CMR_CAMERA_FAIL;
    ...
    if (CAM_IMG_FMT_BAYER_MIPI_RAW == sensor_info.image_format) {
        ...
        cxt->focus_rect.x =
            (isp_af_param.win[i].start_x + isp_af_param.win[i].end_x) / 2;
        cxt->focus_rect.y =
...
cmr_int af_quit(cmr_handle af_handle, cmr_u32 camera_id) {
    ...
    if (CAM_IMG_FMT_BAYER_MIPI_RAW == sensor_info.image_format) {
        ...
        ret = af_cxt->ops.af_isp_ioctrl(
            af_cxt->oem_handle, COM_ISP_SET_AF_STOP,
        ...
        af_cxt->ops.af_sensor_ioctrl(af_cxt->oem_handle, COM_SN_SET_FOCUS,
                                     (struct common_sn_cmd_param *)&af_param);
    }
...
cmr_int af_stop(cmr_handle af_handle, cmr_u32 came_id,
                cmr_u32 is_need_abort_msg) {
...
cmr_int af_need_exit(cmr_handle af_handle, cmr_u32 *is_need_abort_msg) {
    ...
    *is_need_abort_msg = (af_cxt->af_cancelled >> 1) & 0x1;
    ...
    if (ret) {
        CMR_LOGI("af_cancelled val: 0x%x, 0x%x", af_cxt->af_cancelled,
                 *is_need_abort_msg);
    }
...
cmr_int af_check_area(cmr_handle af_handle, struct img_rect *sensor_rect_ptr,
                      struct img_rect *rect_ptr, cmr_u32 rect_num) {
    ...
    for (i = 0; i < rect_num; i++) {
        ...
        CMR_LOGD("rect_ptr->start_x =%d sensor_mode_info->trim_start_x=%d",
                 rect_ptr->start_x, sensor_mode_info->trim_start_x);
        CMR_LOGD("rect_ptr->start_y =%d sensor_mode_info->trim_start_y=%d",
                 rect_ptr->start_y, sensor_mode_info->trim_start_y);

        if ((0 == rect_ptr[i].width) || (0 == rect_ptr[i].height) ||
            (rect_ptr->width > sensor_mode_info->trim_width) ||
            (rect_ptr->height > sensor_mode_info->trim_height) ||
            ((rect_ptr->start_x + rect_ptr->width) >
             (sensor_mode_info->trim_start_x + sensor_mode_info->trim_width)) ||
            ((rect_ptr->start_y + rect_ptr->height) >
             (sensor_mode_info->trim_start_y +
...
exit:
    ...
    CMR_LOGD("ret %ld, result %s", ret,
             (ret == CMR_CAMERA_SUCCESS) ? "ok" : "error");
    return ret;
}

cmr_int focus_rect_parse(cmr_handle af_handle,
                         SENSOR_EXT_FUN_PARAM_T_PTR p_focus_rect) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    struct af_context *af_cxt = (struct af_context *)af_handle;
    uint32_t *ptr = NULL;
    ...
    case CAMERA_FOCUS_MODE_AUTO:
        ...
        if (zone_cnt) {
            ...
            CMR_LOGV("x %d y %d ,w %d h %d", af_param.zone[0].x,
                     af_param.zone[0].y, af_param.zone[0].w,
                     af_param.zone[0].h);
            ...
            if (CMR_CAMERA_SUCCESS !=
                af_check_area(af_handle, &const_sensor_rect,
                              (struct img_rect *)&af_rect_zone[0], 1)) {
                af_param.zone_cnt = 0;

                CMR_LOGV("af_check_area af_param.zone_cnt=%d",
                         af_param.zone_cnt);
            }
        }
        break;
    ...
    case CAMERA_FOCUS_MODE_AUTO_MULTI:
        if (0 == zone_cnt) {
            ...
            af_param.zone[0].w = *ptr++;
            af_param.zone[0].h = *ptr++;
            CMR_LOGE("x %d y %d ,w %d h %d", af_param.zone[0].x,
                     af_param.zone[0].y, af_param.zone[0].w,
                     af_param.zone[0].h);
            ...
            if (CMR_CAMERA_SUCCESS !=
                ...
                af_param.param = SENSOR_EXT_FOCUS_TRIG;
                af_param.zone_cnt = 0;
                CMR_LOGE("af_check_area af_param.zone_cnt=%d",
                         af_param.zone_cnt);
            }
    ...
    case CAMERA_FOCUS_MODE_MACRO:
        ...
        af_param.zone[0].w = *ptr++;
        af_param.zone[0].h = *ptr++;
        CMR_LOGD("x %d y %d ,w %d h %d", af_param.zone[0].x, af_param.zone[0].y,
...
cmr_int focus_rect_param_to_isp(SENSOR_EXT_FUN_PARAM_T focus_rect,
                                struct isp_af_win *p_isp_af_param) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    cmr_int i = 0;
    struct isp_af_win isp_af_param;
    ...
    CMR_LOGD("mode %d valid_win %d", isp_af_param.mode, isp_af_param.valid_win);
    ...
    for (i = 0; i < focus_rect.zone_cnt; i++) {
        ...
        isp_af_param.win[i].end_x =
            focus_rect.zone[i].x + focus_rect.zone[i].w - 1;
        isp_af_param.win[i].end_y =
            focus_rect.zone[i].y + focus_rect.zone[i].h - 1;
        CMR_LOGD("af_win num:%d, x:%d y:%d e_x:%d e_y:%d",
                 isp_af_param.valid_win, isp_af_param.win[i].start_x,
                 isp_af_param.win[i].start_y, isp_af_param.win[i].end_x,
                 isp_af_param.win[i].end_y);
...
cmr_int wait_isp_focus_result(cmr_handle af_handle, cmr_u32 camera_id,
                              cmr_u32 is_need_abort_msg) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    struct timespec ts;
    struct af_context *af_cxt = (struct af_context *)af_handle;
