...
cmr_int camera_fdr_handle(void *data, void *privdata) {
    ...
    CMR_LOGD("channel_id:%d, fd=0x%xd w=%d, h=%d addr: 0x%lx, vir_addr: 0x%lx",
             frame->channel_id, (cmr_u32)frame->fd, frame->length,
             frame->height, frame->yaddr, frame->yaddr_vir);
