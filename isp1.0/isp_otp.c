...
cmr_int Sensor_Ioctl(SENSOR_IOCTL_CMD_E sns_cmd, void *arg)
{
	...
	func_ptr = *(long *) ((long) func_tab_ptr + sns_cmd * S_BIT_2);
...
int write_ev(uint32_t ev)
{
	...
	if(!Sensor_isRAW())
	SCI_Trace_Dcam("%s:0x%x\n",__func__, ev);
	ret = isp_ioctl(NULL, ISP_CTRL_EV, (void*)&ev);
...
int write_ae(uint32_t ae)
{
	...
	if (0 == ae) {//lock
        ...
        ret = isp_ioctl(NULL, ISP_CTRL_ALG, (void*)&cmd_param);
	} else {//unlock
...
int write_frame_rate(uint32_t rate)
{
	...
	if(!Sensor_isRAW())
		return 0;
	SCI_Trace_Dcam("%s:0x%x\n",__func__, rate);
    ret = isp_ioctl(NULL, ISP_CTRL_VIDEO_MODE, (void*)&cmd_param);
...
int write_af_mode(uint32_t af_mode)
{
	...
	ret = isp_ioctl(NULL, ISP_CTRL_AF, &isp_af_param);
...
int write_otp_start_3A( uint32_t data_size, uint8_t *data_buf)
{
    ...
    isp_ioctl(NULL, ISP_CTRL_ALG, (void*)&cmd_param);
...
{
	int ret = 0;
/*	ret = _hi544_ReadGain(gain_val);*/
	*gain_val = Sensor_Ioctl(SENSOR_IOCTL_READ_GAIN, gain_val);
