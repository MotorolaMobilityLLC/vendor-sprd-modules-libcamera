...
static void _set_smart_param(uint32_t handler_id, struct isp_awb_param *awb_param, struct smart_light_calc_result *smart_result)
{
	...
	if (!((SMART_HUE_SAT_GAIN_UNIT == hue_sat_gain.r)
		&& (SMART_HUE_SAT_GAIN_UNIT == hue_sat_gain.g)
		&& (SMART_HUE_SAT_GAIN_UNIT == hue_sat_gain.b))) {
...
static void _set_init_param(struct isp_awb_param* awb_param, struct isp_awb_init_param *init_param)
{
	...
	ISP_LOGI("map data = %p, len=%d", init_param->map_data.addr, init_param->map_data.len);
	...
	ISP_LOGI("pos lut = %p, w=%d, h=%d",
			init_param->weight_of_pos_lut.weight, init_param->weight_of_pos_lut.w,
...
uint32_t isp_awb_ctrl_set(uint32_t handler_id, uint32_t cmd, void *param0, void *param1)
{
	...
	ISP_LOGI("cmd = %d, param0 = %p, param1=%p", cmd, param0, param1);
