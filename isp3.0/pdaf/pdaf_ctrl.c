...
static cmr_int pdafctrl_deinit_adpt(cmr_handle handle)
{
	...
	ret = cxt->pdaf_adpt_ops->adpt_deinit(cxt->adpt_handle);
	return ret;
}
			      ...
			      struct pdaf_ctrl_param_out *out)
{
	...
	ret = cxt->pdaf_adpt_ops->adpt_ioctrl(cxt->adpt_handle, cmd, in, out);
	return ret;
}
				 ...
				 struct pdaf_ctrl_process_out *out)
{
	...
	ret = cxt->pdaf_adpt_ops->adpt_process(cxt->adpt_handle, in, out);
	return ret;
}
...
static cmr_int pdafctrl_destroy_thread(cmr_handle handle)
{
	...
	if (pdaf_thread_cxt->ctrl_thr_handle) {
		...
		pdaf_thread_cxt->ctrl_thr_handle = (cmr_handle) NULL;
	}
exit:
		     ...
		     struct pdaf_ctrl_init_out *out, cmr_handle *handle)
{
	...
	*handle = NULL;
	...
	/* find vendor adpter */
	ret = adpt_get_ops(ADPT_LIB_PDAF, &in->pdaf_lib_info, &cxt->pdaf_adpt_ops);
	if (ret) {
	...
	cxt->cb_ops = in->pdaf_ctrl_cb_ops;
