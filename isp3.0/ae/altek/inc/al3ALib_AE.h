...
#pragma pack(4)    /* new alignment setting  */
typedef struct {
    BOOL    param_mode_int;   // true: interger, false: float
    UINT32  uinfo[3];
    float   finfo[3];
}  ae_gyro_info_t;
#pragma pack(pop)  /* restore old alignment setting from stack  */
...
typedef struct {
    ...
    wbgain_data_t awb_balanced_wb;   // used for flash estimation
...
typedef struct {
    ...
    /* bracket parameter  */
    UINT8     valid_exp_num;
    ae_single_expo_set_t   bracket_expo[AL_MAX_EXP_ENTRIES];

    /* Single shot */
    ae_single_expo_set_t   still_expo;

    /*  preview  */
    ae_single_expo_set_t   cur_expo;

    /* initial */
    ae_single_expo_set_t   init_expo;

    /* flash off report  */                              // would be reset after calling  set param : AE_SET_PARAM_RESET_FLASH_EST
    ae_single_expo_set_t   flashoff_expo;

    /* flash output report  */                         // would be reset after calling  set param : AE_SET_PARAM_RESET_FLASH_EST
    int      IsNeedFlash;   //1: need flash, 0: without flash
    ...
    UINT32   ISO;
    ae_metering_mode_type_t ae_metering_mode;
} ae_exposure_param_t;
#pragma pack(pop)  /* restore old alignment setting from stack  */
...
#pragma pack(4)    /* new alignment setting  */
typedef struct {
    BOOL    bScriptMode;     // current script mode status, TRUE: turned on, FALSE: turned off
    ...
    BOOL    bIsNeedFlashTurnOn;    // this would indicate flash turn on request, framework need to turn on flag when set to on (1), if value is 0, means script mode request turn off flash
...
#pragma pack(4)    /* new alignment setting  */
typedef struct {
    ae_script_mode_t  script_mode;
    ae_script_param_t  script_param;
} ae_script_mode_data_t;
#pragma pack(pop)  /* restore old alignment setting from stack  */
...
typedef struct {
    ...
    /* initial UI setting */
    AL3A_OPERATION_MODE  al_3Aop_mode;
    ae_iso_mode_t  ISOLevel;
    ...
    ae_antiflicker_mode_t  ae_set_antibaning_mode;
    INT32 ae_ui_evcomp;      // +2 ~ -2 EV, scale 1000
    INT32 ae_3RD_evcomp;    // +2 ~ -2 EV, scale 1000
    ae_metering_mode_type_t  ae_metering_mode;   // initial value suggest use: AE_METERING_INTELLIWT
    AL3A_FE_UI_FLASH_MODE  flash_mode;                    // initial value suggest use: AL3A_UI_FLASH_AUTO

    /*  basic control param  */
    BOOL ae_enable;  // for initial, should be true
    BOOL ae_lock;      // for initial, should be false

    /*  sensor info setting, RAW size, fps , etc. */
    ae_sensor_info_t    preview_sensor_info;   // normal preview mode setting
    ae_sensor_info_t    capture_sensor_info;   // if need to use capture mode (use capture exposure table or special mode usage)

    /* initial AWB info */
    ae_awb_info_t    ae_awb_info;

    /* OTP data */
    calibration_data_t   ae_calib_wb_gain;       // if no calibration data is prepared, set {0.0, 0.0, 0.0} would let AE lib use its default value
    // but remember to set OTP data via set_param, refer to command AE_SET_PARAM_OPT_WB_DAT

} ae_set_parameter_init_t;
#pragma pack(pop)  /* restore old alignment setting from stack  */
...
typedef struct {
    ...
    ae_set_parameter_init_t  ae_initial_setting;
    ae_sensor_info_t    normal_sensor_info;    // normal mode, preview sensor info
    ae_sensor_info_t    capture_sensor_info;
    calibration_data_t   ae_calib_wb_gain;
    ...
    /* UI command  */
    ae_metering_mode_type_t  ae_metering_mode;
    AL3A_OPERATION_MODE  al_3Aop_mode;   // mode change

    ae_iso_mode_t  ISOLevel;      // ISO level, if set AD gain level, ISO level should be set to auto (follow manual AD gain)
    UINT32 manual_ISO;          // user DEF level , user define ISO speed
    ae_gain_mode_t  AdgainLevel;   // AD gain level, if set ISO level, AD gain level should be set to auto ( follow manual ISO)
    UINT32 manual_ADgain;    // manual AD gain, sensor AD gain, scale 100
    ae_gain_mode_t  ISPgainLevel;   /* reserved */
    ...
    ae_converge_level_type_t   converge_speedLV;

    UINT16 uwDigitalZoom;    //100: 1.0, 200: scale by 2x, default value 100 if never set digital zoom

    rect_roi_config_t  ae_set_roi_setting;    // should base on RAW croped domain
    rect_roi_config_t  ae_set_face_roi_setting;   // should base on RAW croped domain

    BOOL  redeye_mode;         /* reserved  */ // red eye flash
    ae_scene_mode_t  ae_scene_mode;   // refer to ae_scene_mode_t
    BOOL ae_enable;
    BOOL ae_lock;
    ...
    /* calibration related */
    ae_script_mode_t   script_mode;   // for calibration used, framework should indicate current calibration mode for AE or for FE
    ...
    /* capture command  */
    ae_exposure_bracket_param_t   ae_bracket_param;

    /* frame based update command  */
    ae_sof_notify_param_t     sof_notify_param;    // current sof notify param, should be updated for each SOF received

    ae_gyro_info_t   ae_gyro_info;
    ae_awb_info_t    ae_awb_info;
    ae_af_info_t     ae_af_info;
    ae_motion_info_t  ae_motion_info;
...
#pragma pack(4)    /* new alignment setting  */
typedef struct {
    ae_set_param_type_t  ae_set_param_type;
    BOOL   bforceUpdateFlg;   // strongerly suggest not use this flag, default suggest : flase, if turn on, this set parameter would directly impact current buffer & shadow buffer
    // shadow buffer normally would be update once when AE lib estimation/processing API is calling
    ae_set_param_content_t    set_param;
}  ae_set_param_t;
#pragma pack(pop)  /* restore old alignment setting from stack  */
...
#pragma pack(4)    /* new alignment setting  */
typedef struct {
    ae_get_param_type_t   ae_get_param_type;
    union {
        ae_exposure_param_t  ae_get_expo_param;
        ae_init_exposure_param_t  ae_get_init_expo_param;
        alHW3a_AE_CfgInfo_t  alHW3A_AEConfig;
        calibration_data_t   calib_data;
        wbgain_data_t        wb_data;
...
#pragma pack(4)    /* new alignment setting  */
typedef struct {
    ae_HW_config_t  ae_hw_config_set;   //sensor, HW3A config info
...
#pragma pack(4)    /* new alignment setting  */
typedef struct {
    Report_update_t rpt_3a_update;   // store each module report update result
    ...
    ae_state_t  ae_est_status;  // mapping to ae_LibStates
    fe_state_t  fe_est_status;  // mapping to ae_FlashStates

    /* AE current exposure result  */   // From SOF notify
    UINT32  udCur_ISO;    // current ISO speed
    UINT32  udCur_exposure_time;
    UINT32  udCur_exposure_line;
    UINT32  udCur_sensor_ad_gain;
    ...
    ae_antiflicker_mode_t    flicker_mode;
    ae_metering_mode_type_t  ae_metering_mode;   // AE metering mode
    ae_script_mode_t  ae_script_mode;   // if script mode is turned on, framework should switch to specical flow, monitor info changing of ae_script_info
    ...
    float  ae_Exif_Tv;
    float  ae_Exif_Sv;
    ae_config_info_t  ae_config_info_param;
    ...
    calibration_data_t  general_calib_data;
    UINT32 udADGainx100PerISO100Gain;   // store ISO speed --> AD gain relation, ex: 100 --> ISO100 use AD gain 1.0

    /* request from 3A libs  */
    BOOL   ae_request_HW3A_cfg;      // if this status = true, AE Ctrl layer should call config wrapper again for setting

    /* script mode related info/stats  */
    ae_script_param_t  ae_script_info;
...
void alAELib_GetLibVersion( alAElib_Version_t* AE_LibVersion );

// public function declaration
typedef void (* alAELib_intial )( void ** ae_init_buffer );
typedef void (* alAELib_deinit )( void * ae_obj );
typedef UINT32 (* alAELib_set_param )( ae_set_param_t *param, ae_output_data_t *ae_output , void *ae_runtime_dat );
typedef UINT32 (* alAELib_get_param )( ae_get_param_t *param, void *ae_runtime_dat );
typedef UINT32 (* alAELib_estimation )( void * HW3a_stats_Data, void *ae_runtime_dat, ae_output_data_t *ae_output );
...
typedef struct {
    ...
    alAELib_intial    initial;
    alAELib_deinit    deinit;
    alAELib_set_param set_param;
    alAELib_get_param get_param;
    alAELib_estimation   estimation;
} alAERuntimeObj_t;
#pragma pack(pop)  /* restore old alignment setting from stack  */

BOOL alAElib_loadFunc( alAERuntimeObj_t *aec_run_obj, UINT32 identityID );  // return value true : success, flase: with error
