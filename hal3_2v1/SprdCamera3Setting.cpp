...
typedef struct {
    ...
    int32_t available_fps_ranges[20];
...
static int64_t largest_yuv_min_duration[CAMERA_ID_COUNT];
...
const cam_dimension_t default_sensor_max_sizes[CAMERA_ID_COUNT] = {
    ...
    {5312, 3984},
#elif defined(CONFIG_BACK_EXT_CAMERA_SUPPORT_20M)
    {5120, 3840},
...
// ANDROID_BLACK_LEVEL_LOCK,
#ifdef CONFIG_CAMERA_FRAME_MANUAL
    ANDROID_SENSOR_SENSITIVITY, ANDROID_LENS_APERTURE,
    ANDROID_LENS_FILTER_DENSITY, ANDROID_LENS_FOCUS_DISTANCE,
#endif
...
int SprdCamera3Setting::initStaticParametersforSensorInfo(int32_t cameraId) {
    ...
    // android.sensor.rollingShutterSkew,(>= 0 and < MinFrameDuration).
            ...
            if ((stream_info[i].stream_sizes_tbl.width <= largest_sensor_w &&
                ...
                if (scaler_formats[j] == HAL_PIXEL_FORMAT_YCbCr_420_888) {
                    if (stream_size > 8000000) {
                        HAL_LOGD("YUV %d*%d output in ~200ms in sharkl3"
                                 "offline so change min frame duration",
                                 stream_info[i].stream_sizes_tbl.width,
                                 stream_info[i].stream_sizes_tbl.height);
                        stream_min_duration = 200000000L;
                    } else if ((stream_info[i].stream_sizes_tbl.width >
                                ISP_PATH2_MAX_CAPABILITY) ||
                               /* in case max YUV for sub camera on  path2,
                                  capability is reduced*/
                               (cameraId == 2 &&
                                (stream_info[i].stream_sizes_tbl.width ==
                                 largest_sensor_w))) {
                        HAL_LOGD("YUV %d*%d output in ~100ms in sharkl3"
                                 "offline so change min frame duration",
                                 stream_info[i].stream_sizes_tbl.width,
                                 stream_info[i].stream_sizes_tbl.height);
                    ...
                    if (stream_size > 8000000) {
                        ...
                        stream_min_duration = 100000000L;
                               ...
                               (cameraId == 2 &&
                                (stream_info[i].stream_sizes_tbl.width ==
                                 largest_sensor_w))) {
                        HAL_LOGD("YUV %d*%d output in ~66ms in sharkle"
                                 "offline so change min frame duration",
                                 stream_info[i].stream_sizes_tbl.width,
                                 stream_info[i].stream_sizes_tbl.height);
                ...
                if (stream_info[i].stream_sizes_tbl.width *
                    ...
                    if (scaler_formats[j] == HAL_PIXEL_FORMAT_YCbCr_420_888)
                        largest_yuv_min_duration[cameraId] =
                            stream_min_duration;
                }
            }
        }
...
int SprdCamera3Setting::initStaticParameters(int32_t cameraId) {
    ...
    /* Disable BURST_CAPTURE capability, if device does not support atleast
       10fps burst for MAX YUV size: CTS testStaticBurstCharachterstics */
    if (largest_yuv_min_duration[cameraId] >= 100000000L) {
...
//Bit2 indicates whether the skin is supported.
    ...
    property_get("persist.vendor.cam.auto.tracking.enable", prop, "1");
...
#define FILL_CAM_INFO_ARRAY(Array, Start, Num, Flag)                           \
    ...
    FILL_CAM_INFO(s_setting[cameraId].controlInfo.ae_available_fps_ranges, 2,
                  20, ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES)
...
int SprdCamera3Setting::constructDefaultMetadata(int type,
    ...
    if (characteristicsInfo.exists(
        ...
        for (i = 0; i < count; i++) {
            if (characteristicsInfo
                    .find(ANDROID_REQUEST_AVAILABLE_CHARACTERISTICS_KEYS)
                    .data.i32[i] == ANDROID_CONTROL_AVAILABLE_MODES) {
    ...
    if (characteristicsInfo.exists(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS)) {
        ...
        for (i = 0; i < count; i++) {
            if (characteristicsInfo.find(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS)
                    .data.i32[i] == ANDROID_CONTROL_MODE) {
    ...
    if (characteristicsInfo.exists(
        ...
        for (i = 0; i < count; i++) {
            if (characteristicsInfo
                    .find(ANDROID_REQUEST_AVAILABLE_CHARACTERISTICS_KEYS)
                    .data.i32[i] == ANDROID_CONTROL_AE_LOCK_AVAILABLE) {
    ...
    if (characteristicsInfo.exists(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS)) {
        ...
        for (i = 0; i < count; i++) {
            if (characteristicsInfo.find(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS)
                    .data.i32[i] == ANDROID_CONTROL_AE_LOCK) {
    ...
    if (characteristicsInfo.exists(
        ...
        for (i = 0; i < count; i++) {
            if (characteristicsInfo
                    .find(ANDROID_REQUEST_AVAILABLE_CHARACTERISTICS_KEYS)
                    .data.i32[i] == ANDROID_CONTROL_AWB_LOCK_AVAILABLE) {
    ...
    if (characteristicsInfo.exists(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS)) {
        ...
        for (i = 0; i < count; i++) {
            if (characteristicsInfo.find(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS)
                    .data.i32[i] == ANDROID_CONTROL_AWB_LOCK) {
...
int SprdCamera3Setting::constructDefaultResultMetadata(void *result_metadata) {
    ...
    int32_t valueI32 = 0;
    int64_t valueI64 = 0;
    struct isp_pfc_per_frame_cxt *pfc_metadata = NULL;
    if (result_metadata == NULL) {
        HAL_LOGE("meta data is NULL");
        return 0;
    }

    pfc_metadata = (struct isp_pfc_per_frame_cxt *)result_metadata;
    HAL_LOGV("E");
...
#define GET_VALUE_IF_DIF(x, y, tag)                                            \
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
        s_setting[mCameraId].pfcinfo[pfc_index].capture_intent =
    ...
    if (frame_settings.exists(ANDROID_EDGE_MODE)) {
        ...
        s_setting[mCameraId].pfcinfo[pfc_index].edgeInfo.mode =
            s_setting[mCameraId].edgeInfo.mode;
#endif
    ...
    if (frame_settings.exists(ANDROID_NOISE_REDUCTION_MODE)) {
        ...
        s_setting[mCameraId].pfcinfo[pfc_index].noiseInfo.reduction_mode =
            s_setting[mCameraId].noiseInfo.reduction_mode;
#endif
    ...
    if (frame_settings.exists(ANDROID_SHADING_MODE)) {
        ...
        s_setting[mCameraId].pfcinfo[pfc_index].shadingInfo.mode =
            s_setting[mCameraId].shadingInfo.mode;
    ...
    if (frame_settings.exists(ANDROID_TONEMAP_MODE)) {
        ...
        s_setting[mCameraId].pfcinfo[pfc_index].toneInfo.mode =
            s_setting[mCameraId].toneInfo.mode;
#endif
    ...
    if (frame_settings.exists(ANDROID_JPEG_QUALITY) && is_capture) {
        ...
        s_setting[mCameraId].pfcinfo[pfc_index].jpgInfo.quality =
            s_setting[mCameraId].jpgInfo.quality;
    ...
    if (frame_settings.exists(ANDROID_CONTROL_MODE)) {
        ...
        GET_VALUE_IF_DIF(s_setting[mCameraId].controlInfo.mode, valueU8,
...
camera_metadata_t *SprdCamera3Setting::translateLocalToFwMetadata() {
    ...
    camMetadata.update(
        ...
        &(s_setting[mCameraId].resultInfo.ae_exposure_compensation), 1);
    ...
    HAL_LOGI("[PFC] sensitivity %d ae_exposure_compensation = %d",
             s_setting[mCameraId].resultSensorInfo.sensitivity,
             s_setting[mCameraId].resultInfo.ae_exposure_compensation);
#else
    camMetadata.update(
        ANDROID_CONTROL_AE_TARGET_FPS_RANGE,
...
#else
    if ((s_setting[mCameraId].pfcinfo[pfc_index].jpgInfo.orientation_original ==
             90 ||
         s_setting[mCameraId].pfcinfo[pfc_index].jpgInfo.orientation_original ==
             270) &&
        (hardware_level_limited)) {
    ...
    if (ANDROID_STATISTICS_FACE_DETECT_MODE_OFF !=
            ...
            (s_setting[mCameraId].statisticsInfo.face_detect_mode) ||
#else
            (s_setting[mCameraId].pfcinfo[pfc_index].face_detect_mode) ||
#endif
        (isFaceBeautyOn(s_setting[mCameraId].sprddefInfo))) {
#define MAX_ROI 10
    ...
    */
    ...
    if (mCameraId == 0) {
        ...
        camMetadata.update(ANDROID_SPRD_CALIBRATION_VCM_RESULT,
                           &(s_setting[mCameraId].vcm_result), 1);
        if (s_setting[mCameraId].vcm_num > 0) {
            HAL_LOGD("vcm_result %d vcm_num %d vcm_dac[0] %d vcm_dac[1] %d "
                     "vcm_dac[2] "
                     "%d",
                     s_setting[mCameraId].vcm_result,
                     s_setting[mCameraId].vcm_num,
                     s_setting[mCameraId].vcm_dac[0],
                     s_setting[mCameraId].vcm_dac[1],
                     s_setting[mCameraId].vcm_dac[2]);
        }
