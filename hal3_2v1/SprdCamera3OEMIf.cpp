...

{
	...
	if(mMultiCameraMatchZsl == NULL){
		mMultiCameraMatchZsl = (multi_camera_zsl_match_frame*) malloc(sizeof(multi_camera_zsl_match_frame));
...
int SprdCamera3OEMIf::start(camera_channel_type_t channel_type, uint32_t frame_number)
{
	...
	HAL_LOGI("channel_type = %d, frame_number = %d", channel_type, frame_number);
...
int SprdCamera3OEMIf::takePicture()
{
	...
	if (mTakePictureMode == SNAPSHOT_NO_ZSL_MODE ||
		...
		HAL_LOGI("finish stopPreviewInternal in takePicture. preview state = %d", getPreviewState());
	...
	HAL_LOGI("JPEG thumbnail size = %d x %d", jpeg_thumb_size.width, jpeg_thumb_size.height);
	...
	if (mTakePictureMode == SNAPSHOT_NO_ZSL_MODE) {
		if ((mCaptureMode == CAMERA_ISP_TUNING_MODE || mCaptureMode == CAMERA_ISP_SIMULATION_MODE) &&
			mIspToolStart) {
			wait_raw = false;
		}
	}
	...
	if (mIsTempChanged) {
		switch (tempStates) {
			...
			case CAMERA_HIGH_TEMP:
				fps_param.min_fps = fps_param.max_fps = (controlInfo.ae_target_fps_range[1] + 10)/2;
				HAL_LOGD("high temp, set camera min_fps: %lu, max_fps: %lu", fps_param.min_fps, fps_param.max_fps);
			...
			case CAMERA_DANGER_TEMP:
				...
				HAL_LOGD("danger temp, set camera min_fps: %lu, max_fps: %lu", fps_param.min_fps, fps_param.max_fps);
				SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_RANGE_FPS, (cmr_uint)&fps_param);
				break;
			default:
...
bool SprdCamera3OEMIf::setCameraPreviewDimensions()
{
	...
	HAL_LOGD("preview: width=%d, height=%d, video: width=%d, height=%d, capture: width=%d, height=%d",
...
void SprdCamera3OEMIf::setCameraPreviewMode(bool isRecordMode)
{
	...
	HAL_LOGD("min_fps=0x%lx, max_fps=0x%lx, video_mode=0x%lx", fps_param.min_fps, fps_param.max_fps, fps_param.video_mode);
#if 1 //for cts
...
bool SprdCamera3OEMIf::WaitForBurstCaptureDone()
{
	...
	HAL_LOGD("jpeg_stream %p, callback_stream %p", jpeg_stream, callback_stream);
...
bool SprdCamera3OEMIf::startCameraIfNecessary()
{
	...
	if (!isCameraInit()) {
		...
		if (CMR_CAMERA_SUCCESS != mHalOem->ops->camera_init(mCameraId, camera_cb, this, 0, &mCameraHandle, (void*)Callback_Malloc, (void*)Callback_Free)) {
...
int SprdCamera3OEMIf::overwritePreviewFrameMemInit(struct SprdCamera3OEMIf::OneFrameMem *one_frame_mem_ptr)
{
	...
	HAL_LOGD("%d %d 0x%x", overwrite_offset, addr_offset, mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][0]);
	memset(ptr->virt_addr, mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][0], overwrite_offset);
	overwrite_offset = ptr->width * ptr->height / 3;
	HAL_LOGD("%d %d 0x%x", overwrite_offset, addr_offset, mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][1]);
	memset((ptr->virt_addr + addr_offset), mShakeTest.diff_yuv_color[mShakeTest.mShakeTestColorCount][1], overwrite_offset);
...
void SprdCamera3OEMIf::overwritePreviewFrame(struct camera_frame_type *frame)
{
	...
	memcpy((void *)(frame->y_vir_addr + overwrite_offset),(void *)ptr->virt_addr, (ptr->width * ptr->height));
		...
		if(pre_stream) {
			if(pic_stream) {
				...
				if(ret != NO_ERROR)
					...
					ret = pre_stream->getQBufAddrForNum(frame_num, &addr_vir, &addr_phy, &ion_fd);
			} else {
				ret = pre_stream->getQBuffFirstVir(&addr_vir);
				if(ret == NO_ERROR)
...
void SprdCamera3OEMIf::receiveRawPicture(struct camera_frame_type *frame)
{
	...
	HAL_LOGD("E, mReDisplayHeap = %p,frame->y_vir_addr 0x%lx ", mReDisplayHeap,frame->y_vir_addr);
	...
	if(callback_flag){
		...
		if (atoi(prop) == 1)
		{
			...
			HAL_LOGD("3dcalibration yuv picture: width:%d, height:%d, fd:%d,"
					"yaddp:0x%lx, uaddp:0x%lx, vaddp:0x%lx, yaddv:0x%lx, uaddv:0x%lx, vaddv:0x%lx",
		...
		if ((dst_width == frame->width) && (dst_height == frame->height)) {
			HAL_LOGI("dst_width = %d, dst_height = %d, frame->y_vir_addr 0x%lx", dst_width, dst_height, frame->y_vir_addr);
		...
		void* parm4)
{
	...
	case CAMERA_EVT_CB_FRAME:
		...
		case SPRD_INTERNAL_PREVIEW_STOPPING:
			HAL_LOGD("camera cb: discarding preview frame while stopping preview");
			break;
...
int SprdCamera3OEMIf::flush_buffer(camera_flush_mem_type_e  type, int index, void *v_addr, void *p_addr, int size)
{
	...
	if (pHeapIon) {
		HAL_LOGV("index=%d,vaddr=%p, paddr=%p,size=0x%zx", index, v_addr, p_addr,size);
		ret = pHeapIon->flush_ion_buffer(v_addr, p_addr, size);
		if (ret) {
			HAL_LOGW("abnormal ret=%d", ret);
	...
	case ANDROID_SPRD_EIS_ENABLED:
		{
			...
			SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SPRD_EIS_ENABLED, sprddefInfo.sprd_eis_enabled);
		}
		break;
	...
	case ANDROID_SPRD_PIPVIV_ENABLED:
		{
			...
			if(sprddefInfo.sprd_pipviv_enabled == 1) {
				...
				SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_SPRD_PIPVIV_ENABLED, 0);
			}
		}
		break;
#endif
	...
	case ANDROID_SPRD_SET_TOUCH_INFO:
		{
			...
			SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_TOUCH_XY, (cmr_uint)&touch_param);
...
int SprdCamera3OEMIf::Callback_VideoFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i;
...
int SprdCamera3OEMIf::Callback_VideoMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr,cmr_s32 *fd)
{
...
int SprdCamera3OEMIf::Callback_ZslFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd, cmr_u32 sum)
{
	cmr_u32 i;
	Mutex::Autolock l(&mPrevBufLock);
...
int SprdCamera3OEMIf::Callback_ZslMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	sprd_camera_memory_t *memory = NULL;
	cmr_int              i = 0;
...
int SprdCamera3OEMIf::Callback_PreviewMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	sprd_camera_memory_t *memory = NULL;
	cmr_uint i = 0;
	SPRD_DEF_Tag sprddefInfo;
...
int SprdCamera3OEMIf::Callback_CaptureFree(cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 * fd, cmr_u32 sum)
{
	cmr_u32 i;
...
int SprdCamera3OEMIf::Callback_CaptureMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_s32 *fd)
{
	sprd_camera_memory_t *memory = NULL;
	cmr_int              i = 0;
...
int SprdCamera3OEMIf::Callback_OtherMalloc(enum camera_mem_cb_type type, cmr_u32 size, cmr_u32 *sum_ptr, cmr_uint *phy_addr, cmr_uint *vir_addr,cmr_s32 *fd)
{
	sprd_camera_memory_t *memory = NULL;
	cmr_u32 i;
	cmr_u32 mem_size;
		...
		if(stream) {
			...
			if(ret == NO_ERROR) {
				if(addr_vir != (cmr_uint)NULL) {
					...
					mHalOem->ops->camera_set_preview_buffer(mCameraHandle, addr_phy, addr_vir,fd);
				} else
		...
		if(stream) {
			...
			if(ret == NO_ERROR) {
				if(addr_vir != (cmr_uint)NULL) {
					...
					mHalOem->ops->camera_set_video_buffer(mCameraHandle, addr_phy, addr_vir, fd);
				} else
...
ZslBufferQueue SprdCamera3OEMIf::popZSLQueue(uint64_t need_timestamp)
{
	...
	if(mMultiCameraMatchZsl->match_frame1.heap_array == NULL && mMultiCameraMatchZsl->match_frame3.heap_array == NULL){
		frame = mZSLQueue.begin();
		zsl_frame = static_cast<ZslBufferQueue>(*frame);
		mZSLQueue.erase(frame);
		mIsUnpopped = true;
	...
	if(mCameraId == 1){
		if(ns2ms(abs((int)((int64_t)mMultiCameraMatchZsl->match_frame1.frame.timestamp - (int64_t)need_timestamp))) > 1500){
	...
	if(mCameraId == 3){
		if(ns2ms(abs((int)((int64_t)mMultiCameraMatchZsl->match_frame3.frame.timestamp - (int64_t)need_timestamp))) > 1500){
			HAL_LOGD("match timestmp is too long,no use it");
			for(frame = mZSLQueue.begin(); frame != mZSLQueue.end();frame++){
				if(frame->frame.isMatchFlag != 1){
					zsl_frame = static_cast<ZslBufferQueue>(*frame);
...
void SprdCamera3OEMIf::matchZSLQueue(ZslBufferQueue frame)
{
	...
	if(match_ZSLQueue->empty()){
		...
		while(itor1 != match_ZSLQueue->end()) {
	        int diff = (int64_t)frame.frame.timestamp - (int64_t)itor1->frame.timestamp;
	        if(abs(diff) < DUALCAM_TIME_DIFF) {
				...
				while(itor2 != mMultiCameraMatchZsl->cam1_ZSLQueue->end()){
					if(itor2->frame.timestamp == mMultiCameraMatchZsl->match_frame1.frame.timestamp ){
				...
				while(itor2 != mMultiCameraMatchZsl->cam3_ZSLQueue->end()){
					if(itor2->frame.timestamp == mMultiCameraMatchZsl->match_frame3.frame.timestamp ){
						itor2->frame.isMatchFlag = 0;
						break;
					}
					itor2++;
				...
				if(mCameraId == 1){
					...
					mMultiCameraMatchZsl->match_frame3 = static_cast<ZslBufferQueue>(*itor1);
				}
				if(mCameraId == 3){
					mMultiCameraMatchZsl->match_frame1 = static_cast<ZslBufferQueue>(*itor1);
...
void SprdCamera3OEMIf::setZslBuffers()
{
	...
	releaseZSLQueue();
	for (i = 0; i < mZslHeapNum; i++) {
		mHalOem->ops->camera_set_zsl_buffer(mCameraHandle, mZslHeapArray[i]->phys_addr, (cmr_uint)mZslHeapArray[i]->data, mZslHeapArray[i]->fd);
	}
}
...
vsOutFrame SprdCamera3OEMIf::processEIS(vsInFrame frame_in)
{
	...
	if(mGyromaxtimestamp){
		...
		do{
			...
			if(++count >= 4 || (NO_ERROR != mReadGyroCond.waitRelative(mReadGyroLock,30000000))){
