...
#ifdef CONFIG_CAMERA_ADJUST_FPS_IN_RANGE
...
SprdCamera3OEMIf::SprdCamera3OEMIf(int cameraId, SprdCamera3Setting *setting)
    : mSetCapRatioFlag(false), mVideoCopyFromPreviewFlag(false),
      mUsingSW3DNR(0), mVideoProcessedWithPreview(false), mRedisplayFum(0),
      ...
      m3dnrGraphicHeapNum(0), m3dnrGraphicPathHeapNum(0), mGraphicBufNum(0),
      mSubRawHeapSize(0), mPathRawHeapNum(0), mPathRawHeapSize(0),
      mSmall3dnrGraphicHeapNum(0), mSmall3dnrGraphicPathHeapNum(0),
      mSmallSubRawHeapSize(0), mSmallPathRawHeapNum(0),
      mSmallPathRawHeapSize(0), mPreviewDcamAllocBufferCnt(0),
      ...
      mSysPerformace(NULL), mGyroInit(0), mGyroExit(0), mEisPreviewInit(false),
      ...
      mIsIspToolMode(0), mIsUltraWideMode(false), mLastCafDoneTime(0)

{
    ...
    memset(&mPerFrameResult, 0, sizeof(isp_pfc_per_frame_cxt));
    memset(&mSaveFrameResult, 0, sizeof(isp_pfc_per_frame_cxt));
#endif
    char prop[PROPERTY_VALUE_MAX] = {
        0,
    };
    property_get("persist.vendor.cam.3dnr.version", prop, "0");
    mUsingSW3DNR = atoi(prop);
...
int SprdCamera3OEMIf::start(camera_channel_type_t channel_type,
    ...
    case CAMERA_CHANNEL_TYPE_REGULAR: {
        ...
        SPRD_DEF_Tag sprddefInfo;
        mSetting->getSPRDDEFTag(&sprddefInfo);
        if (sprddefInfo.sprd_eis_enabled == 1 && mGyroInit) {
    ...
    case CAMERA_CHANNEL_TYPE_PICTURE: {
        ...
        if (mSprdReprocessing) {
            ...
            if (mHDRPlusFillState) {
                ...
                ret = reprocessYuvForJpeg(mHDRPlusBackupFrm_info.yaddr,
                                          mHDRPlusBackupFrm_info.yaddr_vir,
                                          mHDRPlusBackupFrm_info.fd);
...
int SprdCamera3OEMIf::zslTakePicture() {
    ...
    mZslSnapshotTime = (t1.tv_sec) * 1000000000LL + t1.tv_nsec;
...
status_t SprdCamera3OEMIf::setAePrecaptureSta(uint8_t state) {
    ...
    mSetting->getCONTROLTag(&resultInfo);
    resultInfo.ae_state = state;
    mSetting->setResultTag(&resultInfo);
    HAL_LOGD("[PFC] ae sta %d", state);
#else
    mSetting->getCONTROLTag(&controlInfo);
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
void SprdCamera3OEMIf::setRequestFrameInfo(struct req_frame_info *info) {
    HAL_LOGD("[PFC] call CAMERA_PARAM_REQ_FRAME_INFO");
    SET_PARM(mHalOem, mCameraHandle, CAMERA_PARAM_REQ_FRAME_INFO,
             (cmr_uint)info);
}
#endif
...
bool SprdCamera3OEMIf::setCameraCaptureDimensions() {
    ...
    if (!(MODE_BOKEH == mMultiCameraMode || MODE_BLUR == mMultiCameraMode)) {
    ...
    if (isRecordMode) {
        ...
        if (sprddefInfo.sprd_3dnr_enabled == 1) {
            if (mUsingSW3DNR == 1) {
                ...
                fps_param.min_fps = 10;
                HAL_LOGD("sw 3dnr mode, adjust max fps to 20, min fps to 10");
            } else {
                fps_param.max_fps = 30;
        ...
        if ((mSprdAppmodeId == -1) &&
            ...
            if (mRawWidth == picW &&
                /*in case max YUV for sub camera or  path3 is used*/
                ((mCameraId == 2) || (mRawWidth > ISP_PATH2_MAX_CAPABILITY))) {
...
bool SprdCamera3OEMIf::startCameraIfNecessary() {
    ...
    defined(CONFIG_CAMERA_SUPPORT_ULTRA_WIDE)
        ...
        HAL_LOGV("sprd_3dcalibration_cap_size w=%d, h=%d",
                 sprddefInfo.sprd_3dcalibration_cap_size[0],
                 sprddefInfo.sprd_3dcalibration_cap_size[1]);
...
void SprdCamera3OEMIf::freeAllCameraMemIon() {
    ...
    if (mUsingSW3DNR == 2) {
        Callback_Sw3DNRCapturePathFree(0, 0, 0, 0);
        Callback_Sw3DNRSmallPathFree(0, 0, 0, 0);
    }
#endif
...
void SprdCamera3OEMIf::deinitCapture(bool isPreAllocCapMem) {
    ...
    SPRD_DEF_Tag sprddefInfo;
    mSetting->getSPRDDEFTag(&sprddefInfo);
    if ((mUsingSW3DNR == 1) && (sprddefInfo.sprd_3dnr_enabled)) {
        Callback_Sw3DNRCaptureFree(0, 0, 0, 0);
        Callback_Sw3DNRCapturePathFree(0, 0, 0, 0);
    }
#ifdef CONFIG_CAMERA_3DNR_CAPTURE_SW

    if ((sprddefInfo.sprd_3dnr_enabled && mUsingSW3DNR == 2)) {
        Callback_Sw3DNRCapturePathFree(0, 0, 0, 0);
        Callback_Sw3DNRSmallPathFree(0, 0, 0, 0);
    }
#endif
...
int SprdCamera3OEMIf::startPreviewInternal() {
    ...
    if ((sprddefInfo.sprd_3dnr_enabled == 1) && (mUsingSW3DNR == 1)) {
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
void SprdCamera3OEMIf::updateResultMetadata(uint32_t frame_num,
                                            uint32_t is_save_metadata,
                                            uint32_t is_restore_metadata) {
    ...
    mSetting->getResultSENSORTag(&resultSensorInfo);
    mSetting->getSPRDDefResultfInfo(&sprddeResultfInfo);
    HAL_LOGV("E");

    if (frame_num == 0)
        mSetting->constructDefaultResultMetadata(&mPerFrameResult);

    mPerFrameResult.request_frame_num = frame_num;

    if (!is_restore_metadata) {
        if (CMR_CAMERA_SUCCESS !=
            mHalOem->ops->camera_get_isp_perFrame_result(mCameraHandle,
                                                         &mPerFrameResult)) {
            HAL_LOGE("[PFC] param null");
            goto exit;
        }
    } else {
        memcpy(&mPerFrameResult, &mSaveFrameResult,
               sizeof(isp_pfc_per_frame_cxt));
        HAL_LOGD("Copying into mPerFrameResult frame_num = %d", frame_num);
    }
    if (is_save_metadata) {
        memcpy(&mSaveFrameResult, &mPerFrameResult,
               sizeof(isp_pfc_per_frame_cxt));
        HAL_LOGD("Copying into mSaveFrameResult  frame_num = %d", frame_num);
    }

    /*ANDROID_CONTROL_AE_TARGET_FPS_RANGE*/
    resultInfo.ae_target_fps_range[0] = mPerFrameResult.range_fps.min_fps;
    resultInfo.ae_target_fps_range[1] = mPerFrameResult.range_fps.max_fps;
    /*ANDROID_CONTROL_AE_LOCK*/
    resultInfo.ae_lock = mPerFrameResult.ae_lock;
    /*ANDROID_CONTROL_AWB_LOCK*/
    resultInfo.awb_lock = mPerFrameResult.awb_lock;
    resultInfo.ae_exposure_compensation = mPerFrameResult.ev_level;
    resultInfo.antibanding_mode = mPerFrameResult.antibanding_mode;
    resultInfo.effect_mode = mPerFrameResult.effect_mode;
    /*ANDROID_CONTROL_AWB_MODE*/
    resultInfo.awb_mode = mPerFrameResult.awb_mode;
    /*ANDROID_CONTROL_SCENE_MODE*/
    resultInfo.scene_mode = mPerFrameResult.scene_mode;
    sprddeResultfInfo.brightness = mPerFrameResult.brightness;
    sprddeResultfInfo.contrast = mPerFrameResult.contrast;
    sprddeResultfInfo.saturation = mPerFrameResult.saturation;
    sprddeResultfInfo.iso = mPerFrameResult.get_iso;
    if (0 != mPerFrameResult.request_frame_num)
        resultSensorInfo.sensitivity = mPerFrameResult.get_sensitivity;
    else
        resultSensorInfo.sensitivity = sensorInfo.sensitivity;

    resultSensorInfo.exposure_time = mPerFrameResult.exposure_time;
    HAL_LOGI("[PFC] [Debug] Frame: [%d] [fps: [%d %d] ae_lock: %d awb_mode: %d "
             "awb_lock: %d scene_mode: %d effect_mode: %d ev_level: %d "
             "antibanding_mode: %d iso: %d sensitivity: %d] exposure_time = %d",
             frame_num, mPerFrameResult.range_fps.min_fps,
             mPerFrameResult.range_fps.max_fps, mPerFrameResult.ae_lock,
             mPerFrameResult.awb_mode, mPerFrameResult.awb_lock,
             mPerFrameResult.scene_mode, mPerFrameResult.effect_mode,
             mPerFrameResult.ev_level, mPerFrameResult.antibanding_mode,
             mPerFrameResult.get_iso, mPerFrameResult.get_sensitivity,
             mPerFrameResult.exposure_time);

    mSetting->setResultTag(&resultInfo);
    mSetting->setResultSENSORTag(resultSensorInfo);
    mSetting->setSPRDDefResultfInfo(sprddeResultfInfo);
...
void SprdCamera3OEMIf::receivePreviewFrame(struct camera_frame_type *frame) {
    ...
    if ((!mRecordingMode) && (mUpdateRangeFpsCount >= UPDATE_RANGE_FPS_COUNT)) {
        ...
        if (mPrvBufferTimestamp == 0) {
            ...
            if (mIsUpdateRangeFps) {
                ...
                HAL_LOGV("timestamp is %lld", timestamp);
    ...
    if (ret)
        ...
        mSetting->setRollingShutterTag(rollingShutterSkew);
    }
    mSetting->setSensorTimestampTag(buffer_timestamp);
    ...
    if (PREVIEW_ZSL_FRAME != frame->type && isFaceBeautyOn(sprddefInfo)) {
        ...
        } else if (!isPreviewing() && frame->type == PREVIEW_FRAME) {
...
void SprdCamera3OEMIf::HandleStopPreview(enum camera_cb_type cb, void *parm4) {
    ...
    CONTROL_Tag resultInfo;
    mSetting->getResultTag(&resultInfo);
    if (ANDROID_CONTROL_AE_STATE_LOCKED != resultInfo.ae_state) {
        resultInfo.ae_state = ANDROID_CONTROL_AE_STATE_INACTIVE;
    }
    resultInfo.awb_state = ANDROID_CONTROL_AWB_STATE_INACTIVE;
    HAL_LOGD("[PFC] ae_state %d awb_state %d", resultInfo.ae_state,
...
void SprdCamera3OEMIf::HandleTakePicture(enum camera_cb_type cb, void *parm4) {
    ...
    case CAMERA_EVT_CB_INVALIDATE_CACHE: {
        ion_buffer = (cam_ion_buffer_t *)parm4;
        if (ion_buffer)
            invalidateCache(ion_buffer->fd, ion_buffer->addr_vir,
                            ion_buffer->addr_phy, ion_buffer->size);
        break;
    }
    case CAMERA_RSP_CB_SUCCESS: {
...
// mSetting->setAeCONTROLTag(&controlInfo);
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
        resultInfo.ae_state = ANDROID_CONTROL_AE_STATE_LOCKED;
        mSetting->setResultTag(&resultInfo);
        HAL_LOGI("[PFC] CAMERA_EVT_CB_AE_LOCK_NOTIFY, ae_state = %d",
                 resultInfo.ae_state);
#else
...
// mSetting->setAeCONTROLTag(&controlInfo);
        ...
        mSetting->setResultTag(&resultInfo);
        HAL_LOGI("[PFC] CAMERA_EVT_CB_AE_UNLOCK_NOTIFY, ae_state = %d",
                 resultInfo.ae_state);
#else
    ...
    case CAMERA_EVT_CB_AI_SCENE: {
        SPRD_DEF_Tag sprdInfo;
        mSetting->getSPRDDEFTag(&sprdInfo);
        sprdInfo.sprd_ai_scene_type_current = *(cmr_u8 *)parm4;
        mSetting->setSPRDDEFTag(sprdInfo);
        HAL_LOGD("sprdInfo.sprd_ai_scene_type_current :%u",
                 sprdInfo.sprd_ai_scene_type_current);
    } break;
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
    case CAMERA_EVT_CB_AWB_LOCK_NOTIFY:
        resultInfo.awb_state = ANDROID_CONTROL_AWB_STATE_LOCKED;
        mSetting->setResultTag(&resultInfo);
        HAL_LOGI("[PFC] CAMERA_EVT_CB_AWB_LOCK_NOTIFY, awb_state = %d",
                 resultInfo.awb_state);
        break;
    case CAMERA_EVT_CB_AWB_UNLOCK_NOTIFY:
        resultInfo.awb_state = ANDROID_CONTROL_AWB_STATE_SEARCHING;
        mSetting->setResultTag(&resultInfo);
        HAL_LOGI("[PFC] CAMERA_EVT_CB_AWB_UNLOCK_NOTIFY, awb_state = %d",
                 resultInfo.awb_state);
        break;
#endif
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
void SprdCamera3OEMIf::HandleConvertToAndroid(enum camera_cb_type cb,
                                              void *parm4) {
    CONTROL_Tag controlInfo;
    uint8_t param;
    struct isp_pfc_per_frame_cxt *dst = (struct isp_pfc_per_frame_cxt *)parm4;

    if (dst == NULL) {
        HAL_LOGE("[PFC] param is NULL");
        return;
    }
    HAL_LOGD("[PFC] cb = %d", cb);
    switch (cb) {
    case CAMERA_EVT_CB_CONVERT_AWB_MODE: {
        mSetting->androidDrvAwbModeToAwbMode(dst->awb_mode, &param);
        dst->awb_mode = param;
        HAL_LOGV("[PFC] converted back to hal compatible awb mode %d",
                 dst->awb_mode);
        break;
    }
    case CAMERA_EVT_CB_CONVERT_SCENE_MODE: {
        mSetting->androidDrvSceneModeToSceneMode(dst->scene_mode, &param);
        dst->scene_mode = param;
        HAL_LOGV("[PFC] converted back to hal compatible scene mode %d",
                 dst->scene_mode);
        break;
    }
    case CAMERA_EVT_CB_CONVERT_ANTIBANDING_MODE: {
        mSetting->drvAntibandingModeToAndroidAntibandingMode(
            dst->antibanding_mode, &param);
        dst->antibanding_mode = param;
        HAL_LOGV("[PFC] converted back to hal compatible antibanding mode %d",
                 dst->antibanding_mode);
        break;
    }
    case CAMERA_EVT_CB_CONVERT_EFFECT_MODE: {
        mSetting->drvModeToAndroidEffectMode(dst->effect_mode, &param);
        dst->effect_mode = param;
        HAL_LOGV("[PFC] converted back to hal compatible effect mode %d",
                 dst->effect_mode);
        break;
    }
    case CAMERA_EVT_CB_CONVERT_EV: {
        mSetting->getCONTROLTag(&controlInfo);
        HAL_LOGV("[PFC] converted before convert ev level %d", dst->ev_level);
        HAL_LOGV("[PFC] ae_compensation_range[1] %d",
                 controlInfo.ae_compensation_range[1]);
        if (dst->ev_level > 6) {
            dst->ev_level = controlInfo.ae_exposure_compensation;
            HAL_LOGV("dst->ev_level = %d", dst->ev_level);
        } else {
            dst->ev_level = 2 * dst->ev_level -
                            6; // current expsoure level is between (-6,6)
            HAL_LOGV("dst->ev_level = %d", dst->ev_level);
        }
        HAL_LOGV("[PFC] converted back to hal compatible ev level %d",
                 dst->ev_level);
        break;
    }
    default:
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
    case CAMERA_FUNC_CONVERT_PARAM:
        obj->HandleConvertToAndroid(cb, parm4);
        break;
#endif
...
int SprdCamera3OEMIf::SetCameraParaTag(cmr_int cameraParaTag) {
    ...
    case ANDROID_CONTROL_AE_MODE:
        ...
        HAL_LOGD("ae_mode:%d, drvAeMode:%d, mFlashMode:%d", controlInfo.ae_mode,
                 drvAeMode, mFlashMode);
    ...
    case ANDROID_CONTROL_AE_REGIONS: {
        ...
        HAL_LOGD(" ae_regions [0] %d [1] %d [2] %d [3] %d [4] %d",
                 controlInfo.ae_regions[0], controlInfo.ae_regions[1],
                 controlInfo.ae_regions[2], controlInfo.ae_regions[3],
                 controlInfo.ae_regions[4]);
        ...
        HAL_LOGV("after crop ae region is %d %d %d %d",
    ...
    case ANDROID_SPRD_APP_MODE_ID: {
        ...
        if ((CAMERA_MODE_CONTINUE != mSprdAppmodeId) &&
            ...
            (false == mRecordingMode) && (mUsingSW3DNR != THREEDNR_SW_1)) {
    ...
    {
        SPRD_DEF_Tag sprddefInfo;
        int threednr_mode = THREEDNR_NONE;
        mSetting->getSPRDDEFTag(&sprddefInfo);
        HAL_LOGD("sprddefInfo.sprd_3dnr_enabled=%d ",
                 sprddefInfo.sprd_3dnr_enabled);
#ifdef CONFIG_CAMERA_3DNR_CAPTURE_SW
        if (mSprdAppmodeId == CAMERA_MODE_3DNR_PHOTO) {
            char prop[PROPERTY_VALUE_MAX] = {
                0,
            };
            property_get("persist.vendor.cam.3dnr.version", prop, "0");
...
#endif
        if (sprddefInfo.sprd_3dnr_enabled) {
            threednr_mode = mUsingSW3DNR ? mUsingSW3DNR : THREEDNR_HW;
        }
...
int SprdCamera3OEMIf::Callback_Sw3DNRCapturePathFree(cmr_uint *phy_addr,
    ...
    HAL_LOGD("mPathRawHeapNum %d sum %d", mPathRawHeapNum, sum);

    for (i = 0; i < m3dnrGraphicPathHeapNum; i++) {
        ret = mapper.unlock(
            (const native_handle_t *)m3DNRGraphicPathArray[i].native_handle);
        if (ret != NO_ERROR) {
            HAL_LOGE("mapper.unlock fail %p",
                     m3DNRGraphicPathArray[i].native_handle);
            return ret;
        }
        if (m3DNRGraphicPathArray[i].bufferhandle != NULL) {
            ...
            m3DNRGraphicPathArray[i].native_handle = NULL;
            m3DNRGraphicPathArray[i].private_handle = NULL;
        }
        HAL_LOGD("graphicBuffer_handle 0x%lx",
                 m3DNRGraphicPathArray[i].private_handle);
    }
    m3dnrGraphicPathHeapNum = 0;
    return 0;
}

int SprdCamera3OEMIf::Callback_Sw3DNRSmallPathFree(cmr_uint *phy_addr,
                                                   cmr_uint *vir_addr,
                                                   cmr_s32 *fd, cmr_u32 sum) {
    uint32_t i = 0;
    int ret = 0;
    Callback_SmallPathFree(0, 0, 0, 0);
    GraphicBufferMapper &mapper = GraphicBufferMapper::get();
    HAL_LOGD("mPathRawHeapNum %d sum %d", mSmallPathRawHeapNum, sum);

    for (i = 0; i < mSmall3dnrGraphicPathHeapNum; i++) {
        ret =
            mapper.unlock((const native_handle_t *)mSmall3DNRGraphicPathArray[i]
                              .native_handle);
        if (ret != NO_ERROR) {
            HAL_LOGE("mapper.unlock fail %p",
                     mSmall3DNRGraphicPathArray[i].native_handle);
            return ret;
        }
        if (mSmall3DNRGraphicPathArray[i].bufferhandle != NULL) {
            mSmall3DNRGraphicPathArray[i].bufferhandle.clear();
            mSmall3DNRGraphicPathArray[i].bufferhandle = NULL;
            mSmall3DNRGraphicPathArray[i].native_handle = NULL;
            mSmall3DNRGraphicPathArray[i].private_handle = NULL;
        }
        HAL_LOGD("graphicBuffer_handle 0x%lx",
                 mSmall3DNRGraphicPathArray[i].private_handle);
    }
    mSmall3dnrGraphicPathHeapNum = 0;
    return 0;
}
...
int SprdCamera3OEMIf::Callback_Sw3DNRCapturePathMalloc(
    ...
    cmr_s32 *fd, void **handle, cmr_uint width, cmr_uint height) {
    int ret = 0;
    cmr_uint small_image_width = 0;
    cmr_uint small_image_height = 0;
    sprd_camera_memory_t *memory = NULL;
    cmr_int i = 0;
    void *vaddr = NULL;
...
cap_malloc:
    ...
    HAL_LOGI(
        "size %d sum %d mPathRawHeapNum %d"
        "mPathRawHeapSize %d width %d height %d m3dnrGraphicPathHeapNum %d",
    ...
    if (0 == mPathRawHeapNum) {
        ...
        for (i = 0; i < (cmr_int)sum; i++) {
            ...
            if (mPathRawHeapArray[mPathRawHeapNum] == NULL) {
                memory = (sprd_camera_memory_t *)malloc(
                    sizeof(sprd_camera_memory_t));
                if (NULL == memory) {
                    HAL_LOGE("error memory is null.");
                    goto mem_fail;
                }
                memset(memory, 0, sizeof(sprd_camera_memory_t));
                mPathRawHeapArray[i] = memory;
                mPathRawHeapNum++;
            }

            graphicBuffer =
                new GraphicBuffer(width, height, HAL_PIXEL_FORMAT_YCrCb_420_SP,
                                  1, yuvTextUsage, "sw_3dnr");

            nativeHandle = (native_handle_t *)graphicBuffer->handle;

            mPathRawHeapArray[i]->fd = ADP_BUFFD(nativeHandle);
            mPathRawHeapArray[i]->phys_addr = 0;
            mPathRawHeapArray[i]->phys_size = size;
            ...
            mPathRawHeapArray[i]->data = (void *)vaddr;
            m3DNRGraphicPathArray[m3dnrGraphicPathHeapNum].bufferhandle =
                graphicBuffer;
            m3DNRGraphicPathArray[m3dnrGraphicPathHeapNum].private_handle =
                graphicBuffer.get();
            m3DNRGraphicPathArray[m3dnrGraphicPathHeapNum].native_handle =
                nativeHandle;
            // ADP_FAKESETBUFATTR_CAMERAONLY(nativeHandle, size, width, height);
            *phy_addr++ = (cmr_uint)mPathRawHeapArray[i]->phys_addr;
            *vir_addr++ = (cmr_uint)mPathRawHeapArray[i]->data;
            *fd++ = mPathRawHeapArray[i]->fd;
            *handle = graphicBuffer.get();
            HAL_LOGI("mPathRawHeapNum %d *handle %p fd 0x%x", mPathRawHeapNum,
                     *handle, mPathRawHeapArray[i]->fd);
        ...
        if ((mPathRawHeapNum >= sum) && (mPathRawHeapSize >= size)) {
            ...
            for (i = 0; i < (cmr_int)sum; i++) {
                ADP_FAKESETBUFATTR_CAMERAONLY(
                    m3DNRGraphicArray[i].native_handle, size, width, height);
                *phy_addr++ = (cmr_uint)mPathRawHeapArray[i]->phys_addr;
                *vir_addr++ = (cmr_uint)mPathRawHeapArray[i]->data;
                *fd++ = mPathRawHeapArray[i]->fd;
                *handle = m3DNRGraphicArray[i].bufferhandle.get();
...
int SprdCamera3OEMIf::Callback_Sw3DNRSmallPathMalloc(
    cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr,
    cmr_s32 *fd, void **handle, cmr_uint width, cmr_uint height) {
    int ret = 0;
    cmr_uint small_image_width = 0;
    ...
    int usage = GRALLOC_USAGE_SW_READ_OFTEN | GRALLOC_USAGE_SW_WRITE_OFTEN;
cap_malloc:
    if (mSmallPathRawHeapNum >= MAX_SUB_RAWHEAP_NUM) {
        LOGE("Callback_CapturePathMalloc: error mPathRawHeapNum %d",
             mPathRawHeapNum);
        return -1;
    }
    if ((mSmallPathRawHeapNum + sum) >= MAX_SUB_RAWHEAP_NUM) {
        LOGE("Callback_CapturePathMalloc: malloc is too more %d %d",
             mSmallPathRawHeapNum, sum);
        return -1;
    }
    small_image_width = *phy_addr;
    small_image_height = *vir_addr;
    HAL_LOGI("small image size %ld, %ld", small_image_width,
             small_image_height);
    Rect bounds(width, height);
    HAL_LOGI("size %d sum %d mSmallPathRawHeapNum %d"
             "mSmallPathRawHeapSize %d width %d height %d "
             "mSmall3dnrGraphicPathHeapNum %d",
             size, sum, mSmallPathRawHeapNum, mSmallPathRawHeapSize, width,
             height, mSmall3dnrGraphicPathHeapNum);

    if (0 == mSmallPathRawHeapNum) {
        mSmallPathRawHeapSize = size;
        for (i = 0; i < (cmr_int)sum; i++) {

            if (mSmallPathRawHeapArray[mSmallPathRawHeapNum] == NULL) {
                memory = (sprd_camera_memory_t *)malloc(
                    sizeof(sprd_camera_memory_t));
                if (NULL == memory) {
                    HAL_LOGE("error memory is null.");
                ...
                memset(memory, 0, sizeof(sprd_camera_memory_t));
                mSmallPathRawHeapArray[i] = memory;
                mSmallPathRawHeapNum++;
            }
            ...
            mSmallPathRawHeapArray[i]->fd = ADP_BUFFD(nativeHandle);
            mSmallPathRawHeapArray[i]->phys_addr = 0;
            mSmallPathRawHeapArray[i]->phys_size = size;
            ret = mapper.lock((const native_handle_t *)nativeHandle, usage,
                              bounds, &vaddr);
            if (ret) {
                HAL_LOGE("mapper.lock failed, ret=%d", ret);
            ...
            mSmallPathRawHeapArray[i]->data = (void *)vaddr;
            mSmall3DNRGraphicPathArray[mSmall3dnrGraphicPathHeapNum]
                .bufferhandle = graphicBuffer;
            mSmall3DNRGraphicPathArray[mSmall3dnrGraphicPathHeapNum]
                .private_handle = graphicBuffer.get();
            mSmall3DNRGraphicPathArray[mSmall3dnrGraphicPathHeapNum]
                .native_handle = nativeHandle;
            // ADP_FAKESETBUFATTR_CAMERAONLY(nativeHandle, size, width, height);
            *phy_addr++ = (cmr_uint)mSmallPathRawHeapArray[i]->phys_addr;
            *vir_addr++ = (cmr_uint)mSmallPathRawHeapArray[i]->data;
            *fd++ = mSmallPathRawHeapArray[i]->fd;
            *handle = graphicBuffer.get();
            HAL_LOGI("mSmallPathRawHeapNum %d *handle %p fd 0x%x vaddr %p",
                     mSmallPathRawHeapNum, *handle,
                     mSmallPathRawHeapArray[i]->fd, (void *)vaddr);

            handle++;
            mSmall3dnrGraphicPathHeapNum++;
        }
    } else {
        if ((mSmallPathRawHeapNum >= sum) && (mSmallPathRawHeapSize >= size)) {
            HAL_LOGD("use pre-alloc cap mem");
            for (i = 0; i < (cmr_int)sum; i++) {
                ADP_FAKESETBUFATTR_CAMERAONLY(
                    mSmall3DNRGraphicArray[i].native_handle, size, width,
                    height);
                *phy_addr++ = (cmr_uint)mSmallPathRawHeapArray[i]->phys_addr;
                *vir_addr++ = (cmr_uint)mSmallPathRawHeapArray[i]->data;
                *fd++ = mSmallPathRawHeapArray[i]->fd;
                *handle = mSmall3DNRGraphicArray[i].bufferhandle.get();
                handle++;
            }
        } else {
            Callback_Sw3DNRSmallPathFree(0, 0, 0, 0);
...
int SprdCamera3OEMIf::Callback_SmallPathFree(cmr_uint *phy_addr,
                                             cmr_uint *vir_addr, cmr_s32 *fd,
                                             cmr_u32 sum) {
    cmr_u32 i;

    LOGI("Callback_CapturePathFree: mPathRawHeapNum %d sum %d",
         mSmallPathRawHeapNum, sum);

    for (i = 0; i < mSmallPathRawHeapNum; i++) {
        if (NULL != mSmallPathRawHeapArray[i]) {
            freeCameraMem(mSmallPathRawHeapArray[i]);
        }
        mSmallPathRawHeapArray[i] = NULL;
    }
    mSmallPathRawHeapNum = 0;
    mSmallPathRawHeapSize = 0;
...
//	}
    ...
    if ((type == CAMERA_PREVIEW_SCALE_3DNR) && (mUsingSW3DNR == 1)) {
        for (i = 0; i < sum; i++) {
            if (NULL != m3DNRPrevScaleHeapReserverd[i]) {
                freeCameraMem(m3DNRPrevScaleHeapReserverd[i]);
            }
    ...
    *fd = 0;
    ...
    if (type == CAMERA_PREVIEW_RESERVED || type == CAMERA_VIDEO_RESERVED ||
        ...
        if (mCommonHeapReserved == NULL) {
            mem_size = CAMERA_ALIGNED_16(mLargestPictureWidth) *
            ...
            if (mUsingSW3DNR == 2) {
                ...
                if (sprddefInfo.sprd_3dnr_enabled) {
                    HAL_LOGD(
                        "size=%d, sum=%d, mem_type=%d sprd_3dnr_enabled %d",
                        size, sum, type, sprddefInfo.sprd_3dnr_enabled);
                    mem_size = size;
                } else {
                    mem_size =
                        mLargestPictureWidth * mLargestPictureHeight * 3 / 2;
                }
            } else {
    ...
    } else if ((type == CAMERA_PREVIEW_SCALE_3DNR) && (mUsingSW3DNR == 1)) {
        for (i = 0; i < sum; i++) {
...
int SprdCamera3OEMIf::Callback_Free(enum camera_mem_cb_type type,
    ...
    } else if (CAMERA_SNAPSHOT_SW3DNR_SMALL_PATH == type) {
#ifdef CONFIG_CAMERA_3DNR_CAPTURE_SW
        ret = camera->Callback_Sw3DNRSmallPathFree(phy_addr, vir_addr, fd, sum);
#endif
...
int SprdCamera3OEMIf::Callback_GPUMalloc(enum camera_mem_cb_type type,
    ...
    case CAMERA_SNAPSHOT_SW3DNR_SMALL_PATH: {
#ifdef CONFIG_CAMERA_3DNR_CAPTURE_SW
        ret = camera->Callback_Sw3DNRSmallPathMalloc(
            size, sum, phy_addr, vir_addr, fd, handle, *width, *height);
#endif
        break;
    }
    case CAMERA_SNAPSHOT_SW3DNR_PATH: {
