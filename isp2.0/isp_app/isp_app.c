...
#define ISP_CTRL_EVT_MASK                (uint32_t)(ISP_CTRL_EVT_INIT \
					...
					|ISP_CTRL_EVT_AEM2|ISP_CTRL_EVT_MONITOR_STOP)
...
static int32_t _awb_calc(isp_ctrl_context* handle, struct isp_ae_info *ae_info, struct isp_awb_statistic_info *stat_info, struct awb_ctrl_calc_result* result)
{
	uint32_t rtn = ISP_SUCCESS;
...
static uint32_t _smart_init(isp_ctrl_context* handle)
{
	...
	ISP_LOGV("ISP_SMART: handle=%p", smart_handle);
...
static int32_t _ispCfg(isp_handle isp_handler)
{
	...
	rtn = isp_dev_init(handle->handle_device, handle->camera_id);
...
static int32_t _ispSetTuneParam(isp_handle isp_handler)
{
	...
	param_data = output.param_data;
	for (i=0; i<output.param_num; i++) {
		/*work around start ae monitor issue*/
		if (param_data->id == ISP_BLK_AE_V1) {
			struct isp_dev_raw_aem_info* aem_info = (struct isp_dev_raw_aem_info*)(param_data->data_ptr);
			ISP_LOGE("hait: ptr:%p, size: %d, cmd:0x%x\n", param_data->data_ptr, param_data->data_size, param_data->cmd);
			aem_info->bypass = 0;
#ifdef AE_MONITOR_CHANGE
			aem_info->mode = 1;
#else
...
static int32_t _ispAwbModeIOCtrl(isp_handle isp_handler, void* param_ptr, int(*call_back)())
{
	...
	struct isp_pm_param_data ioctl_data = {NULL, 0};
...
static int32_t _ispVideoModeIOCtrl(isp_handle isp_handler, void* param_ptr, int(*call_back)())
{
	...
	struct isp_pm_ioctl_input input = {NULL, 0};
	struct isp_pm_ioctl_output output = {NULL, 0, 0};
...
static int32_t _ispHdrIOCtrl(isp_handle isp_handler, void *param_ptr, int(*call_back)())
{
	...
	hdr_ev_param.param = (*(uint32_t *)param_ptr) & 0xFF;
...
static isp_int  _isp_ctrl_routine(struct cmr_msg *message, void *client_data)
{
	...
	if (rtn) {
		ISP_LOGI("evt %d Error happened", evt);
