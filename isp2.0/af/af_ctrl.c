...
static int32_t _af_cfg_afm_win(af_handle_t handle, uint32_t type, uint32_t win_num, struct af_win_rect *win_pos)
{
	...
	AF_LOGE("isp w %d, h %d",af_cxt->plat_info.isp_w,af_cxt->plat_info.isp_h);

	if (0 == win_num) {
		w = af_cxt->plat_info.isp_w;
		h = af_cxt->plat_info.isp_h;
		win_num = 1;
...
static int32_t _af_set_start(af_handle_t handle, struct af_trig_info *trig_info )
{
	...
	monitor_set.type = 1;
	rtn = af_cxt->set_monitor(af_cxt->caller,&monitor_set);
	return rtn;
}


static int32_t _af_end_proc(af_handle_t handle, struct af_result_param *result )
{
	int32_t rtn = AF_SUCCESS;
	struct af_context_t *af_cxt = (struct af_context_t *)handle;
	struct af_monitor_set monitor_set;
...
static int32_t _caf_trig_af_start(af_handle_t handle)
{
	...
	rtn = af_ioctrl(handle,AF_CMD_SET_AF_START,(void*)&trig_info,NULL);
				...
				void *param0, void *param1)
{
	...
	case AF_CMD_SET_TUNING_MODE:
		...
		if (af_cxt->is_running) {
			...
			rtn = _af_end_proc(handle,&af_cxt->af_result);
		}
	...
	case AF_CMD_SET_AF_FINISH:
		if (af_cxt->is_running) {
			rtn = _af_end_proc(handle,(struct af_result_param *)param0);
		}
		break;
