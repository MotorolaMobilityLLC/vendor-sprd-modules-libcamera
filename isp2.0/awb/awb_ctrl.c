...
//		return AWB_CTRL_ERROR;
	...
	AWB_CTRL_LOGE(" fangbing debug envi = [%d, %d], weight=[%d, %d]", result->value[0], result->value[1],  result->weight[0], result->weight[1]);
					...
					struct awb_ctrl_calc_result *result)
{
	...
	bv_param = cxt->tuning_param[work_mode].bv_range;
	...
	if(AWB_CTRL_SCENEMODE_AUTO == cxt->scene_mode) {
		if(AWB_CTRL_WB_MODE_AUTO != cxt->wb_mode) {
			...
			cxt->output_gain.r = cxt->tuning_param[work_mode].mwb_gain[mawb_id].r;
			cxt->output_gain.g = cxt->tuning_param[work_mode].mwb_gain[mawb_id].g;
			cxt->output_gain.b = cxt->tuning_param[work_mode].mwb_gain[mawb_id].b;
		}
	} else {
		scene_mode = cxt->scene_mode;
		if(AWB_CTRL_SCENEMODE_USER_0 ==scene_mode) {
		cxt->output_gain.r = cxt->tuning_param[work_mode].scene_gain[scene_mode].r;
		cxt->output_gain.g = cxt->tuning_param[work_mode].scene_gain[scene_mode].g;
		cxt->output_gain.b = cxt->tuning_param[work_mode].scene_gain[scene_mode].b;
		}
	}
...
uint32_t _awb_set_recgain(struct awb_ctrl_cxt *cxt, void *param)
{
	...
	rtn = awb_ctrl_ioctrl(cxt, AWB_CTRL_CMD_GET_GAIN, (void *)&awb_gain, NULL);
