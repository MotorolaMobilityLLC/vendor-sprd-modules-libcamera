...
static uint32_t g_stream_buf_id =
    1;  // record the bsm buf id for switch buf. the init buf is 0.
uint32_t g_stream_buf_size = 0;

uint32_t JPEGENC_Poll_VLC_BSM(uint32_t time, uint32_t buf_len,
...
LOCAL void JPEGENC_init_fw_param(JPEGENC_PARAMS_T *jpegenc_params,
...
#endif
  SCI_TRACE_LOW("[JPEG_EncInitFwParam] jpeg fw info.slice_height=%d.",
                slice_height);

  enc_fw_info_ptr->width = jpegenc_params->width;
  enc_fw_info_ptr->height = jpegenc_params->height;
  enc_fw_info_ptr->quant_level = (JPEG_QUALITY_E)(
      ((uint32_t)(jpegenc_params->quality) >= (uint32_t)JPEG_QUALITY_MAX)
          ? JPEG_QUALITY_HIGH
  ...
  enc_fw_info_ptr->yuv_0_info.y_data_ptr =
      (uint8 *)((unsigned long)jpegenc_params->yuv_phy_buf);
  enc_fw_info_ptr->yuv_0_info.u_data_ptr =
      (uint8 *)((unsigned long)jpegenc_params->yuv_u_phy_buf);
  ...
  enc_fw_info_ptr->yuv_1_info.y_data_ptr =
      enc_fw_info_ptr->yuv_0_info.y_data_ptr;
  enc_fw_info_ptr->yuv_1_info.u_data_ptr =
      enc_fw_info_ptr->yuv_0_info.u_data_ptr;
  enc_fw_info_ptr->yuv_1_info.v_data_ptr =
      enc_fw_info_ptr->yuv_0_info.v_data_ptr;
  ...
  enc_fw_info_ptr->stream_buf0 =
      (uint8_t *)((unsigned long)jpegenc_params->stream_phy_buf[0]);
  enc_fw_info_ptr->stream_buf1 =
      (uint8_t *)jpegenc_params->stream_virt_buf
          [0];  // enc_fw_info_ptr->stream_buf0+enc_fw_info_ptr->bitstream_buf_len;
  jpeg_fw_codec->g_stream_buf_ptr =
      (uint8_t *)jpegenc_params->stream_virt_buf[0];
  ...
  SCI_TRACE_LOW(
      ...
      "addr = 0x%lx, encode add = %lx,  encode size = 0x%x",
      enc_fw_info_ptr->bitstream_buf_len,
      (unsigned long)enc_fw_info_ptr->stream_buf0,
      (unsigned long)jpeg_fw_codec->g_stream_buf_ptr,
      (unsigned long)enc_fw_info_ptr->enc_buf.buf_ptr,
      enc_fw_info_ptr->enc_buf.buf_size);

  SCI_TRACE_LOW(
      "[JPEG_EncInitFwParam] ping mcu info = %d, pong mcu info = %d, width = "
      "%d, height = %d",
      enc_fw_info_ptr->yuv_0_info.input_mcu_info,
      enc_fw_info_ptr->yuv_1_info.input_mcu_info, enc_fw_info_ptr->width,
...
LOCAL JPEG_RET_E JPEGENC_start_encode(JPEGENC_PARAMS_T *jpegenc_params) {
  ...
  SCI_TRACE_LOW("jpegenc_params->set_slice_height: slice_height %d.\n",
                slice_height);
  SCI_PASSERT(jpegenc_params,
              ("[JPEG_6600L_StartEncode], context_ptr is NULL"));
  ...
  if (jpegenc_params->height > slice_height) {
    ret_value = JPEG_HWEncStart(jpegenc_params->width, slice_height,
                                &jpeg_enc_out_param);
  } else {
    ret_value = JPEG_HWEncStart(jpegenc_params->width, jpegenc_params->height,
                                &jpeg_enc_out_param);
  }

  SCI_TRACE_LOW(
      "[JPEG_6600L_StartEncode] start enc, src_aligned_width = %d, slice "
      "height = %d",
...
PUBLIC JPEG_RET_E JPEGENC_stop_encode(JPEGENC_PARAMS_T *jpegenc_params) {
  ...
  if (JPEG_SUCCESS == jpeg_ret_value) {
    ...
    {
            SCI_TRACE_LOW("[JPEG_StopEncode] jpeg stream buffer is not enough,
    stream size = %d, target buffer size = %d",
                                            jpegenc_params->stream_size,
    jpegenc_params->stream_buf_len);
            return JPEG_MEMORY_NOT_ENOUGH;
    }*/
  }

  SCI_TRACE_LOW("[JPEG_6600L_StopEncode] start enc, stream_size = %d",
                jpegenc_params->stream_size);
...
void JPGEENC_Clear_INT(uint32_t mask) {
  ...
  value = JPG_READ_REG(JPG_GLB_REG_BASE + GLB_INT_CLR_OFFSET,
                       "read the interrupt clear bits.");
  value |= mask;
  JPG_WRITE_REG(JPG_GLB_REG_BASE + GLB_INT_CLR_OFFSET, value,
                "clear the VLC interrupt.");
}
...
void JPEGENC_Handle_BSM_INT(jpegenc_callback callback) {
  ...
  SCI_TRACE_LOW(
      "JPEGENC_Poll_MEA_BSM JPEG_HWSet_BSM_Buf_WriteOnly after.stream_buf_id: "
      "%d.\n",
      stream_buf_id);
...
uint32_t JPEGENC_Poll_MEA_BSM(uint32_t time, uint32_t buf_len,
                              uint32_t slice_num, jpegenc_callback callback,
...
// 1: bsm switch;
// other error;
static uint32_t _Encode_NextSlice(uint32_t time,
                                  JPEGENC_SLICE_NEXT_T *update_parm_ptr) {
  uint32_t ret = 0;
  uint32_t value;
  uint32_t vsp_time_out_cnt = 0;
  int jpg_fd = -1;
  ...
  if (PNULL != update_parm_ptr) {
    JpegEnc_HwTopUpdateYUVAddr(update_parm_ptr->yuv_phy_buf,
                               update_parm_ptr->yuv_u_phy_buf,
                               update_parm_ptr->yuv_v_phy_buf);
  }
  jpg_fd = jpeg_fw_codec->fd;
  JPEG_HWUpdateMBIOBufInfo();
...
int JPEGCODEC_Open(void) {
  ...
  if (0 > (jpg_fd = open(SPRD_JPG_DRIVER, O_RDWR))) {
    ...
    jpg_addr = mmap(NULL, SPRD_JPG_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,
                    jpg_fd, 0);
...
int JPEGENC_Slice_Start(JPEGENC_PARAMS_T *jpegenc_params,
                        JPEGENC_SLICE_OUT_T *out_ptr) {
  int jpg_fd = -1;
  void *jpg_addr = NULL;
  int ret = 0;
  ...
  slice_height = jpegenc_params->set_slice_height
                     ? jpegenc_params->set_slice_height
                     : SLICE_HEIGHT;
  slice_num = (jpegenc_params->height % slice_height)
                  ? (jpegenc_params->height / slice_height + 1)
                  : (jpegenc_params->height / slice_height);
...
uint32_t JPEGENC_Slice_Next(JPEGENC_SLICE_NEXT_T *update_parm_ptr,
                            JPEGENC_SLICE_OUT_T *out_ptr) {
  int jpg_fd = -1;
  void *jpg_addr = NULL;
  uint32_t ret = 0;
