...
static cmr_int camera_isp_ctrl_done(struct camera_context *cxt, cmr_u32 cmd,
                                    void *data);
static cmr_uint camera_isp_to_param(cmr_uint cmd, cmr_uint cmd_value);
#else
...
static cmr_u32 camera_get_channel_id(cmr_u32 channel_bits);
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
cmr_int camera_isp_ctrl_done(struct camera_context *cxt, cmr_u32 cmd,
                             void *data) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    //(void *)data;
    cmr_int oem_cb;
    cmr_u32 scene_mode = 0;
    cmr_uint cmd_value, param;

    if (cxt == NULL) {
        CMR_LOGE("[PFC] param null");
        ret = -CMR_CAMERA_INVALID_PARAM;
        goto exit;
    }

    if (cmd >= ISP_CTRL_MAX) {
        CMR_LOGE("[PFC] isp wrong cmd %d", cmd);
        ret = -CMR_CAMERA_INVALID_PARAM;
        goto exit;
    }

    if (data == NULL) {
        CMR_LOGV("[PFC] EXIT  %d", cmd);
        goto exit;
    }
    ...
    case ISP_CTRL_SET_AE_LOCK_UNLOCK: {
        ...
        cxt->camera_cb(oem_cb, cxt->client_data, CAMERA_FUNC_AE_STATE_CALLBACK,
                       NULL);
        CMR_LOGV("[PFC] [Debug] Result queue callback cmd, 0x%x", cmd);
    } break;
    case ISP_CTRL_SET_AE_AWB_LOCK_UNLOCK: {
        if (*(cmr_int *)data == 1)
            oem_cb = CAMERA_EVT_CB_AWB_LOCK_NOTIFY;
        else
            oem_cb = CAMERA_EVT_CB_AWB_UNLOCK_NOTIFY;
        cxt->camera_cb(oem_cb, cxt->client_data, CAMERA_FUNC_AE_STATE_CALLBACK,
                       NULL);
        CMR_LOGV("[PFC] [Debug] Result queue callback cmd, 0x%x", cmd);
    } break;
    case ISP_CTRL_AWB_MODE: {
        cmd_value = *(cmr_u32 *)data;
        param = camera_isp_to_param(cmd, cmd_value);
        cmr_sensor_set_exif(cxt->sn_cxt.sensor_handle, cxt->camera_id,
                            SENSOR_EXIF_CTRL_LIGHTSOURCE, param);
        cmr_sensor_set_exif(cxt->sn_cxt.sensor_handle, cxt->camera_id,
                            SENSOR_EXIF_CTRL_WHITEBALANCE, param);
        CMR_LOGV("[PFC] Result queue callback cmd, 0x%x", cmd);
    } break;
    case ISP_CTRL_SCENE_MODE: {
        scene_mode = *(cmr_u32 *)data;
        cmr_sensor_set_exif(cxt->sn_cxt.sensor_handle, cxt->camera_id,
                            SENSOR_EXIF_CTRL_SCENECAPTURETYPE, scene_mode);
        CMR_LOGV("[PFC] [Debug] Result queue callback cmd, 0x%x", cmd);
    } break;
    default:
        break;
...
cmr_int camera_is_need_change_fmt(cmr_handle oem_handle,
    ...
    if (is_snp_frm &&
        (1 == camera_get_hdr_flag(cxt) || 1 == camera_get_sw_3dnr_flag(cxt))) {
...
void camera_grab_handle(cmr_int evt, void *data, void *privdata) {
    ...
    if ((chn_bits == cxt->snp_cxt.channel_bits) &&
            ...
            if (frm_id >= CMR_CAPTURE_MEM_SUM) {
                if (1 == camera_get_hdr_flag(cxt)) {
                    ret = cmr_preview_get_hdr_buf(cxt->prev_cxt.preview_handle,
                                                  cxt->camera_id, frame,
                                                  &vir_addr_y);
                } else if (THREEDNR_SW_1 == camera_get_3dnr_flag(cxt)) {
                    ret = cmr_preview_get_3dnr_buf_extra(
                        cxt->prev_cxt.preview_handle, cxt->camera_id, frame,
                        &vir_addr_y, 1, &threednr_handle);
                } else if (THREEDNR_SW_2 == camera_get_3dnr_flag(cxt)) {
                    ret = cmr_preview_get_3dnr_buf_extra_small(
                        cxt->prev_cxt.preview_handle, cxt->camera_id, frame,
                        &vir_addr_y, &threednr_handle,
                        &(ipm_in_param.small_src_frame));
                }
                out_param.size =
                ...
                if (THREEDNR_SW_1 == camera_get_3dnr_flag(cxt)) {
                    ret = cmr_preview_get_3dnr_buf_extra(
                        cxt->prev_cxt.preview_handle, cxt->camera_id, &frame,
        ...
        if (THREEDNR_SW_2 == camera_get_3dnr_flag(cxt)) {
            ret = cmr_preview_get_3dnr_target_buf(cxt->prev_cxt.preview_handle,
                                                  cxt->camera_id,
                                                  &ipm_in_param.dst_frame);
        } else {
            ipm_in_param.dst_frame =
                cxt->snp_cxt.post_proc_setting.chn_out_frm[0];
        }
        ...
        if (1 == camera_get_hdr_flag(cxt)) {
            ret = ipm_transfer_frame(ipm_cxt->hdr_handle, &ipm_in_param,
                                     &imp_out_param);
        } else if (1 == camera_get_sw_3dnr_flag(cxt)) {
            imp_out_param.dst_frame.size.width =
                cxt->ipm_cxt.threenr_small_width;
            imp_out_param.dst_frame.size.height =
                cxt->ipm_cxt.threenr_small_height;
            imp_out_param.private_data = threednr_handle;
            CMR_LOGI("y_addr 0x%lx in->src_frame.fd 0x%x "
                     "ipm_in_param.small_src_frame.fd 0x%x",
                     ipm_in_param.src_frame.addr_vir.addr_y,
                     ipm_in_param.src_frame.fd,
                     ipm_in_param.small_src_frame.fd);
...
void camera_grab_evt_cb(cmr_int evt, void *data, void *privdata) {
    ...
    case CMR_GRAB_TIME_OUT:
        ...
        break;
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
    case CMR_GRAB_SOF_DONE:
        valid_flag = 1;
        isp_ioctl(cxt->isp_cxt.isp_handle, ISP_CTRL_SET_VALID_FRAME,
                  (void *)&valid_flag);
        break;
    case CMR_GRAB_TX_RESERVED:
        isp_ioctl(cxt->isp_cxt.isp_handle, ISP_CTRL_SET_TX_RESERVED, NULL);
        break;
#endif
    default:
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
cmr_uint camera_isp_to_param(cmr_uint cmd, cmr_uint cmd_value) {
    ...
    case COM_ISP_SET_AWB_MODE: {
        switch (in_param) {
        case ISP_AWB_AUTO:
            out_param = CAMERA_WB_AUTO;
            break;
        case ISP_AWB_INDEX1:
            out_param = CAMERA_WB_INCANDESCENT;
            break;
        case ISP_AWB_INDEX4:
            out_param = CAMERA_WB_FLUORESCENT;
            break;
        case ISP_AWB_INDEX5:
            out_param = CAMERA_WB_DAYLIGHT;
            break;
        case ISP_AWB_INDEX6:
            out_param = CAMERA_WB_CLOUDY_DAYLIGHT;
            break;
        case ISP_AWB_OFF:
            out_param = CAMERA_WB_MAX;
            break;
        default:
            break;
        }
...
cmr_int camera_isp_convert(struct camera_context *cxt, cmr_u32 cmd,
                           void *data) {
    cmr_int ret = CMR_CAMERA_SUCCESS;
    //(void *)data;
    cmr_int oem_cb;
    cmr_u32 scene_mode = 0;
    // cmr_uint cmd_value, param;
    struct isp_pfc_per_frame_cxt *dst = (struct isp_pfc_per_frame_cxt *)data;

    ISP_LOGD("E");
    if (dst == NULL) {
        ISP_LOGE("[PFC] data is NULL");
        ret = -CMR_CAMERA_INVALID_PARAM;
        goto exit;
    }
    ...
    switch (cmd) {
    case ISP_CTRL_AWB_MODE: {
        ISP_LOGV("[PFC] awb_mode %d", dst->awb_mode);
        if (dst->awb_mode != CAMERA_WB_MAX)
            dst->awb_mode =
                camera_isp_to_param(COM_ISP_SET_AWB_MODE, dst->awb_mode);
        ISP_LOGD("[PFC] awb_mode after oem convert %d", dst->awb_mode);
        oem_cb = CAMERA_EVT_CB_CONVERT_AWB_MODE;
        cxt->camera_cb(oem_cb, cxt->client_data, CAMERA_FUNC_CONVERT_PARAM,
                       data);
    } break;
    case ISP_CTRL_SCENE_MODE: {
        ISP_LOGV("[PFC] scene_mode %d", dst->scene_mode);
        oem_cb = CAMERA_EVT_CB_CONVERT_SCENE_MODE;
        cxt->camera_cb(oem_cb, cxt->client_data, CAMERA_FUNC_CONVERT_PARAM,
                       data);
    } break;
    case ISP_CTRL_FLICKER: {
        ISP_LOGV("[PFC] flicker %d", dst->antibanding_mode);
        oem_cb = CAMERA_EVT_CB_CONVERT_ANTIBANDING_MODE;
        cxt->camera_cb(oem_cb, cxt->client_data, CAMERA_FUNC_CONVERT_PARAM,
                       data);
    } break;
    case ISP_CTRL_SPECIAL_EFFECT: {
        ISP_LOGV("[PFC] effect_mode %d", dst->effect_mode);
        oem_cb = CAMERA_EVT_CB_CONVERT_EFFECT_MODE;
        cxt->camera_cb(oem_cb, cxt->client_data, CAMERA_FUNC_CONVERT_PARAM,
                       data);
    } break;
    case ISP_CTRL_EV: {
        ISP_LOGV("[PFC] ev_level %d", dst->ev_level);
        oem_cb = CAMERA_EVT_CB_CONVERT_EV;
        cxt->camera_cb(oem_cb, cxt->client_data, CAMERA_FUNC_CONVERT_PARAM,
                       data);
    } break;
    default:
        ISP_LOGE("case not handled");
        break;
    }
exit:
    ISP_LOGD("X");
...
cmr_int camera_isp_evt_cb(cmr_handle oem_handle, cmr_u32 evt, void *data,
    ...
    if ((sub_type & ISP_EVT_MASK) == 0) {
        ret = camera_isp_ctrl_done(
            cxt, cmd, data /*after applied settings passed cmd_value*/);
        goto exit;
    }
#else
    if ((sub_type & ISP_EVT_MASK) == 0) {
    ...
    case ISP_AE_STAB_CALLBACK:
        if (data) {
            if (*(cmr_s32 *)data == 1) {
                CMR_LOGD("stop skip frame when flash on");
    ...
    case ISP_CONVERT_CALLBACK:
        CMR_LOGD("[PFC] ISP_CONVERT_CALLBACK");
        camera_isp_convert(cxt, cmd, data);
        break;
#endif
...
cmr_int camera_preview_cb(cmr_handle oem_handle, enum preview_cb_type cb_type,
    ...
    if (param) {
        ...
        if (camera_get_hdr_flag(cxt) == 1 && cxt->hdr_skip_frame_enable == 1 &&
            cb_type == PREVIEW_EVT_CB_FRAME && cxt->snp_cxt.zsl_frame == 1) {
            struct camera_frame_type *prev_frame =
                (struct camera_frame_type *)param;
...
cmr_int camera_ipm_cb(cmr_u32 class_type, struct ipm_frame_out *cb_param) {
    ...
    struct frm_info frame;
    struct img_frm *buf_ptr = NULL;
    ;
    cmr_u32 capture_size = 0;
    ...
    capture_size = cxt->snp_cxt.request_size.width *
                   cxt->snp_cxt.request_size.height * 3 / 2;
    ...
    if (1 == camera_get_hdr_flag(cxt)) {
        if (cb_param->is_plus == 1) { // normal pic need backup first. and wait
            ...
            ret = cmr_snapshot_receive_data(cxt->snp_cxt.snapshot_handle,
                                            SNAPSHOT_EVT_REPROCESS, &frame);
        }
    } else if (1 == camera_get_sw_3dnr_flag(cxt)) {
        frame = cxt->snp_cxt.cur_frm_info;
        buf_ptr = &cxt->snp_cxt.post_proc_setting.chn_out_frm[0];
        CMR_LOGI("cb_param->dst_frame.addr_vir.addr_y %p capture_size %d "
                 "target_yuv %p",
                 cb_param->dst_frame.addr_vir.addr_y, capture_size,
                 buf_ptr->addr_vir.addr_y);
        memcpy((void *)buf_ptr->addr_vir.addr_y,
               (void *)cb_param->dst_frame.addr_vir.addr_y, capture_size);
        cmr_snapshot_memory_flush(cxt->snp_cxt.snapshot_handle,
                                  &cb_param->dst_frame);
        camera_post_share_path_available((cmr_handle)cxt);
        cxt->ipm_cxt.frm_num = 0;
...
void camera_set_3dnr_flag(struct camera_context *cxt, cmr_u32 threednr_flag) {
    ...
    cxt->snp_cxt.is_sw_3dnr =
        (THREEDNR_SW_1 == threednr_flag || THREEDNR_SW_2 == threednr_flag) ? 1
                                                                           : 0;
    CMR_LOGD(
        "Set 3dnr is : %d, and sw 3dnr is : %d,threednr_flag=%d,app_mode=%d",
        cxt->snp_cxt.is_3dnr, cxt->snp_cxt.is_sw_3dnr, threednr_flag,
        cxt->app_mode);
...
cmr_u32 camera_get_channel_id(cmr_u32 channel_bits) {
    int i;
...
void camera_snapshot_channel_handle(cmr_handle oem_handle, void *param) {
    ...
    if (1 == is_need_resume) {
        frame.channel_id = camera_get_channel_id(cxt->snp_cxt.channel_bits);
        ret = cmr_preview_receive_data(cxt->prev_cxt.preview_handle,
...
cmr_int camera_isp_init(cmr_handle oem_handle) {
    ...
    CMR_LOGV("get sensor info ret %ld  name %s version %s", ret,
             sn_cxt->sensor_info.name, sn_cxt->sensor_info.sensor_version_info);
    ...
    if (NULL != sns_ex_info_ptr) {
        if ((NULL == sns_ex_info_ptr->name) ||
            ...
            camera_copy_sensor_ex_info_to_isp(&isp_param.ex_info,
                                              sns_ex_info_ptr);
        }
    } else {
    ...
    CMR_LOGI(
        ...
        isp_param.size.w, isp_param.size.h,
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
...
cmr_int camera_ipm_open_module(cmr_handle oem_handle) {
    ...
    if (1 == camera_get_sw_3dnr_flag(cxt) && 1 != cxt->is_3dnr_video) {
        ...
        if (ret) {
            ...
            cxt->ipm_cxt.threenr_small_width = out_param.small_image_width;
            cxt->ipm_cxt.threenr_small_height = out_param.small_image_height;
...
cmr_int camera_capture_highflash(cmr_handle oem_handle, cmr_u32 camera_id) {
    ...
    if ((1 != camera_get_hdr_flag(cxt)) && 1 != camera_get_sw_3dnr_flag(cxt)) {
        /*open flash*/
        memset(&setting_param, 0, sizeof(setting_param));
        setting_param.ctrl_flash.capture_mode.capture_mode = snp_cxt->snp_mode;
        setting_param.camera_id = camera_id;
...
cmr_int camera_isp_start_video(cmr_handle oem_handle,
    ...
    isp_param.dv_mode = dv_mode;
    {
        ...
        CMR_LOGD("isp sensor max w h, %d %d",
                 isp_param.resolution_info.sensor_max_size.w,
                 isp_param.resolution_info.sensor_max_size.h);
        CMR_LOGD("isp sensor output w h, %d %d",
                 isp_param.resolution_info.sensor_output_size.w,
                 isp_param.resolution_info.sensor_output_size.h);
        CMR_LOGD("isp sensor crop startx start w h, %d %d %d %d",
        ...
        CMR_LOGD("isp_param:sensor fps info:is_high_fps %d, high_fps_skip_num "
                 "is  %d",
                 isp_param.sensor_fps.is_high_fps,
                 isp_param.sensor_fps.high_fps_skip_num);
        CMR_LOGD("isp_param:sensor fps info:max_fps is %d, min_fps is %d",
...
cmr_int camera_channel_cfg(cmr_handle oem_handle, cmr_handle caller_handle,
    ...
    CMR_LOGV("sensor_max_size: width=%d, height=%d",
...
cmr_int camera_channel_start(cmr_handle oem_handle, cmr_u32 channel_bits,
    ...
    if (cxt->is_start_snapshot != 1) {
        ...
        if ((channel_bits & OFFLINE_CHANNEL_BIT) && is_zsl_enable == 0 &&
            video_snapshot_type != VIDEO_SNAPSHOT_VIDEO &&
            !(THREEDNR_NONE != camera_get_3dnr_flag(cxt))) {
...
cmr_int camera_isp_ioctl(cmr_handle oem_handle, cmr_uint cmd_type,
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
    struct req_frame_info req_frm_info;
#endif
    if (!oem_handle || !param_ptr) {
        CMR_LOGE("in parm error");
        ret = -CMR_CAMERA_INVALID_PARAM;
...
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
        CMR_LOGD("[PFC] af mode %d", param_ptr->cmd_value);
#endif
        break;
    ...
    case COM_ISP_SET_REQ_FRAME_INFO:
        ...
        ptr_flag = 1;
        req_frm_info.frame_num = param_ptr->req_info.frame_num;
        req_frm_info.is_only_capture = param_ptr->req_info.is_only_capture;
        req_frm_info.is_input_capture = param_ptr->req_info.is_input_capture;
        isp_param_ptr = (void *)&req_frm_info;
        CMR_LOGD("[PFC] call ISP_CTRL_SET_REQ_FRAME_INFO frame_num %u",
                 req_frm_info.frame_num);
        break;
    case COM_ISP_GET_PER_FRAME_RESULT:
        isp_cmd = ISP_CTRL_GET_PER_FRAME_RESULT;
        ptr_flag = 1;
        isp_param_ptr = (void *)param_ptr->per_frame_res;
        CMR_LOGI("[PFC] isp_param_ptr %p param_ptr->per_frame_res %p",
    ...
    if (ptr_flag) {
        ...
        if (ret) {
            CMR_LOGD("failed isp ioctl %ld", ret);
        }
...
cmr_int camera_get_preview_param(cmr_handle oem_handle,
    ...
    if (CAMERA_SNAPSHOT != is_snapshot) {
        ...
        if (property_get_bool("persist.vendor.cam.ai.scence.enable", 0)) {
            ...
            cxt->app_mode = setting_param.cmd_type_value;
    ...
    if (!strcmp(value, "false")) {
        ...
        out_param_ptr->is_sw_3dnr = camera_get_sw_3dnr_flag(cxt);
    }
...
exit:
    CMR_LOGD(
        ...
        "%d, prev rot %ld snp rot %d rot snp %d, zoom mode %ld fd %ld is dv "
        "%d "
        "tool eb %d, q %d thumb q %d enc angle %d thumb size %d %d, frame "
        "cnt "
        "%d, out_param_ptr->flip_on %d, is_3dnr %d,is_4in1 "
        "%d, limited_4in1_width %d, limited_4in1_height %d",
...
cmr_int camera_set_preview_param(cmr_handle oem_handle,
    ...
    start_param.threednr_small_width = cxt->ipm_cxt.threenr_small_width;
    start_param.threednr_small_height = cxt->ipm_cxt.threenr_small_height;
...
cmr_int camera_get_snapshot_param(cmr_handle oem_handle,
    ...
    cxt->is_snapshot = out_ptr->is_snapshot;
    camera_set_3dnr_flag(cxt, setting_param.cmd_type_value);
    ...
    out_ptr->channel_id = camera_get_channel_id(chn_bits);
exit:
...
cmr_int camera_set_setting(cmr_handle oem_handle, enum camera_param_type id,
    ...
    case CAMERA_PARAM_SPRD_AUTO_HDR_ENABLED:
        ...
        break;
#ifdef CONFIG_CAMERA_PER_FRAME_CONTROL
    case CAMERA_PARAM_REQ_FRAME_INFO:
        CMR_LOGD("[PFC] call cmr_setting_ioctl");
        setting_param.req_info = *(struct req_frame_info *)param;
        CMR_LOGV("frame_num %u", setting_param.req_info.frame_num);
        ret = cmr_setting_ioctl(cxt->setting_cxt.setting_handle, id,
                                &setting_param);
        break;
#endif
...
camera_get_isp_per_frame_result(cmr_handle oem_handle,
    ...
    isp_param.per_frame_res = perFrame_res;

    ret =
        camera_isp_ioctl(oem_handle, COM_ISP_GET_PER_FRAME_RESULT, &isp_param);

    CMR_LOGI(
        "[PFC] [Debug] Frame: [%d] [fps range: [%d %d] ae_lock: %d awb_mode: "
        "%d awb_lock: %d scene_mode: %d effect_mode: %d ev_level: %d "
        "antibanding_mode: %d get_iso %d get_sensitivity %d exposure_time %d]",
        perFrame_res->request_frame_num,
        isp_param.per_frame_res->range_fps.min_fps,
        isp_param.per_frame_res->range_fps.max_fps,
        isp_param.per_frame_res->ae_lock, isp_param.per_frame_res->awb_mode,
        isp_param.per_frame_res->awb_lock, isp_param.per_frame_res->scene_mode,
        isp_param.per_frame_res->effect_mode, isp_param.per_frame_res->ev_level,
        isp_param.per_frame_res->antibanding_mode,
        isp_param.per_frame_res->get_iso,
        isp_param.per_frame_res->get_sensitivity,
        isp_param.per_frame_res->exposure_time);
...
cmr_int camera_local_stop_preview(cmr_handle oem_handle) {
    ...
    if (prev_ret) {
        CMR_LOGE("failed to stop prev %ld", prev_ret);
    }
...
cmr_int camera_local_start_snapshot(cmr_handle oem_handle,
    ...
    snp_param.is_snapshot = is_snapshot;
...
#ifdef CONFIG_CAMERA_FRAME_MANUAL
        if (cxt->cmr_set_env != 1) {
            if (THREEDNR_NONE == camera_get_3dnr_flag(cxt) &&
...
cmr_int camera_local_set_param(cmr_handle oem_handle, enum camera_param_type id,
    ...
    if (ret) {
        CMR_LOGD("failed to set param %ld", ret);
    }
exit:
    return ret;
}
...
cmr_int camera_local_image_sw_algorithm_processing(
    ...
    if (sw_algorithm_type == SPRD_CAM_IMAGE_SW_ALGORITHM_HDR) {
        if (cxt->ipm_cxt.hdr_version.major != 1) {
            memcpy(&ipm_in_param.ev[0], &cxt->snp_cxt.hdr_ev[0],
                   ((HDR_CAP_NUM)-1) * sizeof(float));
        }
...
cmr_int camera_local_start_capture(cmr_handle oem_handle) {
    ...
    else if (THREEDNR_NONE != camera_get_3dnr_flag(cxt))
...
cmr_int camera_local_reprocess_yuv_for_jpeg(cmr_handle oem_handle,
    ...
    ret = cmr_snapshot_receive_data(cxt->snp_cxt.snapshot_handle,
                                    SNAPSHOT_EVT_REPROCESS, frm_data);
    camera_post_share_path_available(oem_handle);
