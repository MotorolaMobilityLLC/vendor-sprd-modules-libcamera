...
void camera_grab_evt_cb(cmr_int evt, void* data, void* privdata)
{
	...
	case CMR_GRAB_TX_DONE:
        if(0 == cxt->camera_id){
            ...
            af_ts.capture = 0;
            isp_ioctl(cxt->isp_cxt.isp_handle, ISP_CTRL_SET_DCAM_TIMESTAMP, (void*)&af_ts);
        }
...
void camera_snapshot_cb_to_hal(cmr_handle oem_handle, enum snapshot_cb_type cb, enum snapshot_func_type func, void* param)
{
	...
	case SNAPSHOT_EXIT_CB_DONE:
        if(0 == cxt->camera_id){
            ...
            af_ts.capture = 1;
            isp_ioctl(cxt->isp_cxt.isp_handle, ISP_CTRL_SET_DCAM_TIMESTAMP, &af_ts);
        }
...
cmr_int camera_isp_deinit(cmr_handle  oem_handle)
{
	...
	if(cxt->lsc_malloc_flag == 1) {
		if (cxt->hal_free) {
			cxt->hal_free(CAMERA_ISP_LSC,&cxt->isp_lsc_phys_addr,&cxt->isp_lsc_virt_addr, &cxt->lsc_mfd,
                            ...
                            struct cmr_op_mean *mean)
{
	...
	/* from sharkl2, jpeg support mirror/flip/rotation, mirror feature always use jpeg if jpeg support it */
										...
										struct channel_start_param *param_ptr, cmr_u32 *channel_id, struct img_data_end *endian)
{
	...
	CMR_LOGV("param_ptr->cap_inf_cfg.buffer_cfg_isp %d", param_ptr->cap_inf_cfg.buffer_cfg_isp);
