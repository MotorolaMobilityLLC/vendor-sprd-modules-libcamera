...
static cmr_int uvde_open(cmr_handle ipm_handle, struct ipm_open_in *in,
                         struct ipm_open_out *out,
                         cmr_handle *out_class_handle);
static cmr_int uvde_close(cmr_handle class_handle);
static cmr_int uvde_transfer_frame(cmr_handle class_handle,
...
static cmr_int uvde_start(cmr_handle class_handle, cmr_uint thread_id,
                          proc_func func_ptr, proc_cb cb, void *param);
...
static void add_border_uv(cmr_u8 *dst, cmr_u8 *src, cmr_u32 w, cmr_u32 h,
                          cmr_u32 border_w, cmr_u32 border_h);

static struct class_ops uvde_ops_tab_info = {
    uvde_open, uvde_close, uvde_transfer_frame, uvde_pre_proc, uvde_post_proc,
};
...
static cmr_int uvde_transfer_frame(cmr_handle class_handle,
    ...
    if (0 == uv_denoise_alg) {
        ...
        add_border_uv((cmr_u8 *)ext_src, (cmr_u8 *)cnr_in,
                      denoise_in->InputWidth / 2, denoise_in->InputHeight / 2,
                      12, 12);
    }
    ...
    CMR_LOGI("isp_uv_denoise, uv_denoise_level=%d (%d, %d, %d)",
             uv_denoise_level, max6delta, max4delta, max2delta);
    ...
    if (0 == uv_denoise_alg) {
        ...
        DstOffsetFour =
            3 * part0_h / 2 * denoise_in->InputWidth * sizeof(cmr_s8);
        ...
        for (thread_id = 0; thread_id < THREAD_UVDE; thread_id++) {
            uvde_start(uvde_handle, thread_id, uv_proc_func_neon0, uv_proc_cb,
                       (void *)&uv_params[thread_id]);
        }
        ...
        SrcOffsetTwo =
            (part0_h / 2 - 2) * denoise_in->InputWidth * sizeof(cmr_s8);
        DstOffsetTwo = part0_h / 2 * denoise_in->InputWidth * sizeof(cmr_s8);
        ...
        SrcOffsetFour =
            (3 * part0_h / 2 - 2) * denoise_in->InputWidth * sizeof(cmr_s8);
        DstOffsetFour =
            3 * part0_h / 2 * denoise_in->InputWidth * sizeof(cmr_s8);
        ...
        for (thread_id = 0; thread_id < THREAD_UVDE; thread_id++) {
            uvde_start(uvde_handle, thread_id, uv_proc_func_neon1, uv_proc_cb,
                       (void *)&uv_params[thread_id]);
        }
        ...
        for (thread_id = 0; thread_id < THREAD_UVDE; thread_id++) {
            uvde_start(uvde_handle, thread_id, uv_proc_func_neon2, uv_proc_cb,
                       (void *)&uv_params[thread_id]);
        }
    } else {
        goto EXIT;
    ...
    if (uvde_handle->is_inited) {
        for (thread_id = 0; thread_id < THREAD_UVDE; thread_id++) {
            if (uvde_handle->thread_handles[thread_id]) {
                ret =
                    cmr_thread_destroy(uvde_handle->thread_handles[thread_id]);
                if (ret) {
...
static cmr_int uvde_start(cmr_handle class_handle, cmr_uint thread_id,
    ...
    uvde_start_ptr =
        (struct uvde_start_param *)malloc(sizeof(struct uvde_start_param));
    if (!uvde_start_ptr) {
        CMR_LOGE("no mem");
        return CMR_CAMERA_NO_MEM;
    }
...
void add_border_uv(cmr_u8 *dst, cmr_u8 *src, cmr_u32 w, cmr_u32 h,
                   cmr_u32 border_w, cmr_u32 border_h) {
    cmr_u32 i, j;
    cmr_u8 *src_ptr;
    cmr_u8 *dst_ptr;
