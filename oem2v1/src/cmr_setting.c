...
static cmr_int setting_set_flash_mode(struct setting_component *cpt,
    ...
    if (setting_is_rawrgb_format(cpt, parm)) {
        ...
        if (init_in->setting_isp_ioctl) {
            ...
            if (parm->camera_id == 1 &&
                !strcmp(FRONT_CAMERA_FLASH_TYPE, "led")) {
                isp_param.camera_id = parm->camera_id;
...
static cmr_int setting_ctrl_flash(struct setting_component *cpt,
    ...
    if (is_active) {
        ...
        /*disable*/
        if (setting_is_need_flash(cpt, parm) || FLASH_NEED_QUIT == cpt->flash_need_quit) {
            /*open flash*/
...
static cmr_int cmr_setting_parms_init() {
    ...
    cmr_add_cmd_fun_to_table(CAMERA_PARAM_RAW_CAPTURE_SIZE,
    ...
    cmr_add_cmd_fun_to_table(SETTING_GET_RAW_CAPTURE_SIZE,
