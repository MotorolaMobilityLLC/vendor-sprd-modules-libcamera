...
cmr_int cmr_preview_init(struct preview_init_param *init_param_ptr,
    ...
    *preview_handle_ptr = (cmr_handle)handle;
    CMR_LOGD("handle 0x%lx created!", (cmr_uint)handle);
...
cmr_int prev_preview_frame_handle(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (prev_cxt->prev_param.is_ultra_wide) {
        ...
        if (prev_cxt->prev_param.is_3dnr != THREEDNR_SW_1) {
...
cmr_int prev_alloc_prev_buf(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (prev_cxt->prev_param.prev_rot) {
        ...
        if (is_restart && IMG_ANGLE_0 != prev_cxt->prev_param.prev_rot) {
            buffer->count = 0;
            for (i = 0; i < PREV_ROT_FRM_ALLOC_CNT; i++) {
                if (1 == *(&prev_cxt->prev_rot_frm_is_lock[0] + i)) {
                    ...
                    buffer->addr_vir[buffer->count].addr_u =
                        prev_cxt->prev_rot_frm[i].addr_vir.addr_u;
                    buffer->fd[buffer->count] = prev_cxt->prev_rot_frm[i].fd;
                    buffer->count++;
                }
            }
        }
...
cmr_int prev_alloc_video_buf(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (prev_cxt->prev_param.prev_rot) {
        ...
        if (is_restart && IMG_ANGLE_0 != prev_cxt->prev_param.prev_rot) {
            buffer->count = 0;
            for (i = 0; i < PREV_ROT_FRM_ALLOC_CNT; i++) {
                if (1 == *(&prev_cxt->video_rot_frm_is_lock[0] + i)) {
                    buffer->addr[buffer->count].addr_y =
                        prev_cxt->video_rot_frm[i].addr_phy.addr_y;
                    buffer->addr[buffer->count].addr_u =
                        prev_cxt->video_rot_frm[i].addr_phy.addr_u;
                    ...
                    buffer->addr_vir[buffer->count].addr_u =
                        prev_cxt->video_rot_frm[i].addr_vir.addr_u;
                    buffer->fd[buffer->count] = prev_cxt->video_rot_frm[i].fd;
                    buffer->count++;
                }
            }
        }
...
cmr_int prev_alloc_cap_buf(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (!is_restart) {
        ...
        if ((FRAME_HDR_PROC == prev_cxt->prev_param.frame_ctrl) &&
            ...
            // alloc buffer for 3dnr YUV
            if (prev_cxt->prev_param.is_sw_3dnr) {
                ...
                ret = mem_ops->gpu_alloc_mem(
                    ...
                    &real_height);
                if (THREEDNR_SW_2 == prev_cxt->prev_param.is_3dnr) {
                    ret = mem_ops->gpu_alloc_mem(
    ...
    if (FRAME_HDR_PROC == prev_cxt->prev_param.frame_ctrl) {
        ...
        if (prev_cxt->prev_param.is_3dnr == THREEDNR_SW_2) {
            for (i = 0; i < CAP_3DNR_NUM; i++) {
                if ((IMG_DATA_TYPE_YUV420 == prev_cxt->cap_org_fmt ||
                     IMG_DATA_TYPE_YVU420 == prev_cxt->cap_org_fmt) &&
                    is_normal_cap) {
                    ...
                    buffer->addr[i].addr_u =
                        buffer->addr[i].addr_y + channel_size;
                    buffer->addr_vir[i].addr_y =
                        prev_cxt->cap_3dnr_virt_addr_path_array[i];
                    buffer->addr_vir[i].addr_u =
                        buffer->addr_vir[i].addr_y + channel_size;
                    ...
                    buffer->slave_frame_info.frame_addr_vir_array[i].addr_u =
                        buffer->slave_frame_info.frame_addr_vir_array[i]
                            .addr_y +
                        channel_small_size;
...
cmr_int prev_alloc_zsl_buf(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (prev_cxt->prev_param.cap_rot) {
        ...
        if (is_restart && IMG_ANGLE_0 != prev_cxt->prev_param.prev_rot) {
            buffer->count = 0;
            for (i = 0; i < ZSL_ROT_FRM_CNT; i++) {
                if (1 == *(&prev_cxt->cap_zsl_rot_frm_is_lock[0] + i)) {
                    buffer->addr[buffer->count].addr_y =
                        prev_cxt->cap_zsl_rot_frm[i].addr_phy.addr_y;
                    buffer->addr[buffer->count].addr_u =
                        prev_cxt->cap_zsl_rot_frm[i].addr_phy.addr_u;
                    ...
                    buffer->addr_vir[buffer->count].addr_u =
                        prev_cxt->cap_zsl_rot_frm[i].addr_vir.addr_u;
                    buffer->fd[buffer->count] = prev_cxt->cap_zsl_rot_frm[i].fd;
                    buffer->count++;
                }
            }
        }
...
cmr_int prev_alloc_4in1_buf(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    if (!is_restart) {
        ...
        prev_cxt->cap_4in1_mem_valid_num = 0;
        mem_ops->alloc_mem(CAMERA_4IN1_PROC, handle->oem_handle,
...
cmr_int prev_set_cap_param(struct prev_handle *handle, cmr_u32 camera_id,
    ...
    ret = prev_alloc_4in1_buf(handle, camera_id, is_restart, is_capture_zsl);
...
cmr_int prev_fd_open(struct prev_handle *handle, cmr_u32 camera_id) {
    ...
    CMR_LOGD("is_support_fd %ld, is_fd_on %ld, ai_scene_enable %ld",
