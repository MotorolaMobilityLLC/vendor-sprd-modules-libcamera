...
camera_memory_t *_get_memory(private_handle_t *pBuf) {
    ...
    MemoryHeapIon *mHeap = new MemoryHeapIon(
        "/dev/ion", pBuf->size, 0, (1 << 31) | ION_HEAP_ID_MASK_SYSTEM);
...
int start_camera(void **pdev) {
    ...
    if (opts.pdev == NULL) {
        ...
        ops_status = HAL_MODULE_INFO_SYM.common.methods->open(
            &HAL_MODULE_INFO_SYM.common, opts.cameraId, (hw_device_t **)&tmp);
        if (ops_status != 0) {
...
void start_preview(camera3_device_t *pdev, CameraParameters *para) {
    ...
    sc.num_streams = 1;
    sc.streams = (camera3_stream_t **)malloc(sizeof(camera3_stream_t *) *
                                             sc.num_streams);
    if (!sc.streams) {
    ...
    md = (camera_metadata_t *)pdev->ops->construct_default_request_settings(
        pdev, CAMERA3_TEMPLATE_PREVIEW);
    if (!md) {
        CMR_LOGE("fail");
    }
    ...
    //	private_handle_t(int flags, int usage, int size, uint64_t base, int
    //lock_state, int fb_file, int fb_offset):

    struct private_handle_t prv(0, 0, (int)(cs.width * cs.height) * 2,
                                (uint64_t)0, -1, 0, 0, 0);
    prv.fd = -1;
...
int main(int argc, char **arg) {
    ...
    case WK_PREVIEW: {
        ...
        //prv(-1,(uint32_t)(cs.width*cs.height)*2,0);
        struct private_handle_t prv(0, 0, (int)(cs.width * cs.height) * 2,
                                    (uint64_t)0, -1, 0, 0, 0);
        prv.fd = -1;
        prv.size = cs.width * cs.height * 2;
    ...
    case WK_TAKEPIC: {
        ...
        CMR_LOGE("camera3_dev construct_default_request_settings");
        md = (camera_metadata_t *)pdev->ops->construct_default_request_settings(
            pdev, CAMERA3_TEMPLATE_STILL_CAPTURE);
        if (!md) {
            CMR_LOGE("fail");
        }
